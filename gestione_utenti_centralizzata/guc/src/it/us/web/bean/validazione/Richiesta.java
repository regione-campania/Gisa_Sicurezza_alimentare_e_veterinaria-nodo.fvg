...
public class Richiesta {
	...
	private int qualificaId;
	private String profiloProfessionale;
	private int profiloProfessionaleId;
	...
	public int getQualificaId() {
		return qualificaId;
	}

	public void setQualificaId(int qualificaId) {
		this.qualificaId = qualificaId;
	}
	...
	public int getProfiloProfessionaleId() {
		return profiloProfessionaleId;
	}

	public void setProfiloProfessionaleId(int profiloProfessionaleId) {
		this.profiloProfessionaleId = profiloProfessionaleId;
	}
	...
	public Richiesta(Connection db, String numeroRichiesta) {
		...
		sql = "select * from opzioni.get_info_richiesta(?)"; 
		try {
			pst = db.prepareStatement(sql);
			pst.setString(1, numeroRichiesta);
			ResultSet rs = pst.executeQuery();
			while (rs.next()){
				qualificaId = rs.getInt("id_qualifica");
				qualifica = rs.getString("qualifica");
				profiloProfessionaleId = rs.getInt("id_profilo_professionale");
				profiloProfessionale = rs.getString("profilo_professionale");
			}
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public String processa(Connection db, int userId) {
		...
		try {
			...
			while (rs.next()){
				...
				try {
					//System.out.println("### Processa INFO - "+ numeroRichiesta);
					JSONObject jsonEsito = new JSONObject(esito);
					if((jsonEsito.has("Esito") && jsonEsito.get("Esito").equals("OK")) || jsonEsito.has("CodiceFiscale")){
						String user_id = (String)jsonEsito.get("Username");
						sql = "select * from opzioni.processa_info( ?, ?)";
						pst = db.prepareStatement(sql);
						pst.setString(1, numeroRichiesta);
						pst.setInt(2, Integer.parseInt(user_id.replaceAll("spid_usr_", "")));
						ResultSet rs2 = pst.executeQuery();
						//System.out.println("### Processa INFO - id_utente "+ user_id);
						if(!jsonEsito.has("CodiceFiscale") && jsonEsito.get("Esito").equals("OK") ){
							//inserimento
							while(rs2.next()){
								//GISA ONLY
								if(rs2.getString(1).equals("OK")){
									sql = "select * from opzioni.collega_info( ?, ?);";
									pst = db.prepareStatement(sql);
									pst.setString(1, numeroRichiesta);
						...
						}else if(jsonEsito.has("CodiceFiscale")){
							//modifica
							while(rs2.next()){
								//GISA ONLY
								if(rs2.getString(1).equals("OK")){
									sql = "select * from opzioni.modifica_info( ?, ?);";
									pst = db.prepareStatement(sql);
									pst.setString(1, numeroRichiesta);
									pst.setString(2, user_id);
									pst.executeQuery();
