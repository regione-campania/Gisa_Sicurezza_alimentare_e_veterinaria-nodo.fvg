					...
					for (int i = 0; i<jsonEsitoListaEndPoint.length(); i++){
						...
						if (jsonEndPointRisultato!=null && jsonEndPointRisultato.has("Esito")) {
							...
							if (jsonEndPointRisultato.get("Esito").equals("OK")){
								...
								try {String resp = UrlUtil.getUrlResponse(urlReloadUtenti); System.out.println("resp: " + resp); } catch (Exception e) {}
								
								
								if(false)
								{
									if(ric.getIdRuoloBdu()==24 && jsonEndPoint.get("EndPoint").equals("BDR"))
									{
										System.out.println("[GUC] [VALIDAZIONE] RELOAD UTENTI AUTOMATICA IN VAM" );
										EndPointConnector epcInserimentoENDPOINTSecondario = listaEndPointConnector.getByIdOperazioneIdEndPoint(Operazione.INSERTUTENTE, (EndPoint.VAM));
										String urlReloadUtentiSecondaria = epcInserimentoENDPOINTSecondario.getUrlReloadUtenti() +jsonEsito.get("Username");
										System.out.println("[GUC] [VALIDAZIONE] RELOAD UTENTI AUTOMATICA IN VAM: " + urlReloadUtentiSecondaria);
						...
						//Flusso 342
						try {String resp = UrlUtil.getUrlResponse("http://srvGISAL/gisa_nt/ReloadUtenti"); } catch (Exception e) {}
					}
				}
