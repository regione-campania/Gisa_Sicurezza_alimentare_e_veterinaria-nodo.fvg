...
public class LineaProduttiva extends GenericBean {
	...
	public void queryRecord(Connection db, int id )
	{
		...
		"where rslp.trashed_date is null and opu_relazione_attivita_produttive_aggregazioni.id = ?" +
	...
	public static boolean verificaLineaProduttiva(int idLinea1, int idLinea2, int idLineaProduttivaToMatch1, int idLineaProduttivaToMatch2 ){
	...
	public static boolean verificaLineaProduttivaPerCatture(int idLinea1, int idLinea2, boolean checkCattura){
		...
		if (idLinea2 <= 0){ //Non ho scelto un detentore diverso dal proprietario
			idLinea2 = idLinea1;
		}
		...
		try{
			...
			if ((lineaProdProprietario == idAttivitaSindaco || lineaProdProprietario == idAttivitaSindacoFR) && lineaProdDetentore == 4 && !checkCattura){
				ret = false;
			}
			else{
				ret = true;
