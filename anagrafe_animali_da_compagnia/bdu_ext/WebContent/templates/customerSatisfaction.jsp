...
<div style="display: none" id="dialogCustomerSatisfaction" title="Sei soddisfatto di come è andata l'operazione appena eseguita ?" width="610px" height="290px" >
   <table style="width: 100%" class="noborder">
   
   
   <tr>
   <td align="center"><button type ="button" style="width: 200px" value="SI" onclick="saveSatisfaction( '<%=data_operazione.getTime() %>', '<%=utenteCollegato.getUsername() %>', 'si', '', '<%=commandOld%>',document.getElementById('iniTime').value,document.getElementById('endTime').value)"><b>SI, CONTINUA</b><img src="images/ok.png" width="30px" height="30px"></button></td>
   ...
   <td>
   	...
   	<input type = "button" value="CONTINUA" onclick="saveSatisfaction(  '<%=data_operazione.getTime() %>', '<%=utenteCollegato.getUsername() %>', 'no', document.getElementById('cusatisfaction').value, '<%=commandOld%>',document.getElementById('iniTime').value,document.getElementById('endTime').value)">
   </td>
   </tr>
