...
{ 
	...
	document.getElementById('searchcodeIdprovinciaSedeLegale').value=addressObj.provinciaId;
	document.getElementById('searchcodeIdComuneSedeLegale').value=addressObj.comuneId;
...
{ 
    ...
    {
    	document.getElementById("searchcodeIdComune").value = addressObj.comuneId;
    	document.getElementById("searchcodeIdComuneSelect").value = addressObj.comuneId;
	}
    document.getElementById('cap').value= addressObj.cap ;
    loadModalWindowUnlock();
}
...
{ 
    ...
    if(addressObj.comune!='')
    {
    	document.getElementById("idComuneModificaResidenza").value = addressObj.comuneId;
    	document.getElementById("idComuneModificaResidenzaSelect").value = addressObj.comuneId;
    }
    if(addressObj.prov!='')
    {
    	document.getElementById("idProvinciaModificaResidenza").value = addressObj.provinciaId;
    	document.getElementById("idProvinciaModificaResidenzaSelect").value = addressObj.provinciaId;
    }
    document.getElementById('cap').value= addressObj.cap ;
    document.getElementById("idAslNuovoProprietarioSelect").value = addressObj.aslSel;
    document.getElementById("idAslNuovoProprietario").value = addressObj.aslSel;
    loadModalWindowUnlock();
}
...
{ 
    ...
    if(document.getElementById('via').value != '')
    	getCoordinate('via','searchcodeIdComuneSelect','searchcodeIdprovinciaSelect','cap','latitudine','longitudine');
    else 
    	alert('Attenzione! inserire prima l\'indirizzo');
    loadModalWindowUnlock();
}
...
<jsp:include page="customerSatisfaction.jsp"></jsp:include>
 ...
 %> <a
				onclick="javascript:window.open('guida_2016/Manuale_bdu.pdf')" href="#" target="_new" title="<%=fileGuidaAtteso%>">Guida Utente</a> | 
<%
					if(false)
					{
%>
...
%>							
						 ...
						 <!--|  <a href="#" onclick="window.open('Login.do?command=LoginDigemon')"
      ...
      |</dhv:permission> <img src="images/icons/stock_exit-16.gif" border="0"
					...
					href="Login.do?command=Logout" class="s">Esci da BDU</a> |<img
					...
					height="16" width="16" /> <dhv:evaluate
						...
						%>
					</dhv:evaluate> <b class="highlight"><%=User.getRole()%></b> 
<%
						if(ApplicationProperties.getProperty("flusso_342").equals("true"))	
						{
%>
						 / <b class="highlight"><%= User.getUserRecord().getQualifica() != null && !User.getUserRecord().getQualifica().equals("") ? User.getUserRecord().getQualifica() : "Qualifica" %></b>
						 / <b class="highlight"><%= User.getUserRecord().getProfilo_professionale() != null && !User.getUserRecord().getProfilo_professionale().equals("") ? User.getUserRecord().getProfilo_professionale() : "Profilo Professionale" %></b>
<%
						}
%>
					<dhv:evaluate
