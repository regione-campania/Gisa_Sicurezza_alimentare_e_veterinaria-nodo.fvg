CREATE OR REPLACE VIEW public.elenco_veterinari_asl AS 
 SELECT a.user_id AS code,
    upper(btrim((c.namelast::text || ' '::text) || c.namefirst::text) )   AS description,
   ...
   FROM contact c,
    access a
  WHERE c.user_id = a.user_id AND a.role_id in (18,20,29,34);



  CREATE OR REPLACE VIEW public.elenco_veterinari_asl_with_group_asl AS 
 select 201 as code,'ASL AVELLINO' as description,false as default_item,-1 as level,true as enabled,201 as asl,null as entered,null as modified,true as isgroup union
 SELECT *,false as isgroup  from elenco_veterinari_asl WHERE level = 201 union
 select 202 as code,'ASL BENEVENTO' as description,false as default_item,-1 as level,true as enabled,202 as asl,null as entered,null as modified,true as isgroup union
 SELECT *,false as isgroup  from elenco_veterinari_asl WHERE level = 202 union
 select 203 as code,'ASL CASERTA' as description,false as default_item,-1 as level,true as enabled,203 as asl,null as entered,null as modified,true as isgroup union
 SELECT *,false as isgroup  from elenco_veterinari_asl WHERE level = 203 union
 select 204 as code,'ASL NAPOLI 1' as description,false as default_item,-1 as level,true as enabled,204 as asl,null as entered,null as modified,true as isgroup union
 SELECT *,false as isgroup  from elenco_veterinari_asl WHERE level = 204 union
 select 205 as code,'ASL NAPOLI 2' as description,false as default_item,-1 as level,true as enabled,205 as asl,null as entered,null as modified,true as isgroup union
 SELECT *,false as isgroup  from elenco_veterinari_asl WHERE level = 205 union
 select 206 as code,'ASL NAPOLI 3' as description,false as default_item,-1 as level,true as enabled,206 as asl,null as entered,null as modified,true as isgroup union
 SELECT *,false as isgroup  from elenco_veterinari_asl WHERE level = 206 union
 select 207 as code,'ASL SALERNO' as description,false as default_item,-1 as level,true as enabled,207 as asl,null as entered,null as modified,true as isgroup union
 SELECT *,false as isgroup  from elenco_veterinari_asl WHERE level = 207  
 order by asl, isgroup desc, description;

  

update evento_html_fields set ordine_campo = 7 where id = 80;

insert into evento_html_fields(id_tipologia_evento,  nome_campo,tipo_campo,      label_campo,         tabella_lookup,ordine_campo) 
                        values(                  2,'veterinari',  'select', 'Veterinari asl','elenco_veterinari_asl_with_group_asl',           6);

alter table evento_html_fields add column select_size integer default 1;
alter table evento_html_fields add column select_multiple boolean default false;
update evento_html_fields set select_multiple = true, select_size = 10 where id_tipologia_evento = 2 and nome_campo = 'veterinari';
alter table evento_html_fields add column no_order boolean default false;
update evento_html_fields set no_order = true where id_tipologia_evento = 2 and nome_campo = 'veterinari';
update access_ set enabled = false where user_id in (12404,11708);
alter table evento_sterilizzazione add column veterinario_asl_1 integer default -1;
alter table evento_sterilizzazione add column veterinario_asl_2 integer default -1;



DROP FUNCTION public_functions.dbi_bdu_registrazioni_data_evento(integer, integer, integer, integer);
...
rel_nuovo.ragione_sociale as nuovo_proprietario,
rel_vecchio.ragione_sociale as vecchio_proprietario, 
