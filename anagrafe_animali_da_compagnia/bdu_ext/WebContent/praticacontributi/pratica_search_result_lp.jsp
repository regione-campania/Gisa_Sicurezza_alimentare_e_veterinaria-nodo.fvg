...
<table cellpadding="4" cellspacing="0" width="100%" class="pagedList" >
	<tr>
	    ...
	    <th width="10%" >
    		<strong>
    			<dhv:label name="">Elenco comuni coinvolti</dhv:label>
    		</strong>
  		</th> 
	   ...
	   <!-- Flusso 251: disabilitazione flusso. commentare le due colonne -->
	   <th width="10%" >
    		<strong>
...
%>  
 ...
 <tr class="row<%=rowid %>">
        ...
        <%-- 	<a href="javascript:displayMenu('select<%= i %>','menuContact', '<%= thisContact.getOrgId() %>', '<%= thisContact.getId() %>', '<%= thisContact.getPrimaryContact() %>','<%= (((thisContact.getTrashedDate() != null) || !thisContact.getEnabled()) || ((thisContact.getOrgTrashedDate() != null) || !thisContact.getOrgEnabled()))%>');"
      	...
      	<!-- Flusso 251: modifiche del 03/08 - INIZIO -->
 			<td valign="center"><%=thisContact.getComuniElencoNomeTruncate()%>&nbsp;&nbsp;&nbsp;<a id="elencocomunicoinvolti" href='PraticaContributi.do?command=ElencoComuni&id=<%=thisContact.getId()%>' onclick='openListaComuni("PraticaContributi.do?command=ElencoComuni&id=<%=thisContact.getId()%>"); loadModalWindowUnlock(); return false;'>[...]</a></td>
 		 <!-- Flusso 251: modifiche del 03/08 - FINE -->
