...
<link type="text/css" documentale_url="" href="anagrafe_animali/documenti/screen.css" rel="stylesheet" />
	
<link rel="stylesheet" documentale_url="" href="anagrafe_animali/documenti/print.css" type="text/css" media="print" />

<jsp:useBean id="dati_decesso" class="org.aspcfs.modules.registrazioniAnimali.base.EventoDecesso"
	scope="request" />	
...
<jsp:useBean id="aslList" class="org.aspcfs.utils.web.LookupList"
	scope="request" />
<jsp:useBean id="causeDecessoList" class="org.aspcfs.utils.web.LookupList"
	scope="request" />
...
<input type="submit" name="stampa" class="buttonClass"
    ...
    <jsp:param name="idAnimale" value="<%=thisAnimale.getIdAnimale() %>" />
     <jsp:param name="idSpecie" value="<%=thisAnimale.getIdSpecie() %>" />
      <jsp:param name="idTipo" value="PrintDichiarazioneDecesso" />
       <jsp:param name="idMicrochip" value="<%=value_microchip %>" />
</jsp:include>
	<!-- input type="submit" name="Timbra PDF" class="buttonClass"
	onclick="window.location.href='GestioneDocumenti.do?command=ListaDocumentiByTipo&tipo=PrintDichiarazioneDecesso&IdSpecie=<%=thisAnimale.getIdSpecie() %>&IdAnimale=<%=thisAnimale.getIdAnimale() %>&id_microchip=<%=value_microchip %>'" value="Gestione PDF" /-->
...
<div class="title1">DICHIARAZIONE DI DECESSO <%=specieList.getSelectedValue(thisAnimale.getIdSpecie()).toUpperCase() %></div>
&nbsp; &nbsp; &nbsp; &nbsp; &nbsp;
...
<div class="dott_solid" style="margin-top: 0px;">&nbsp; <%=(proprietario.getCodFiscale() != null && !("").equals(proprietario.getCodFiscale()))? proprietario.getCodFiscale() : "" %></div>

<div class="clear1"></div>
<div class="nodott" style="margin-top: 0px;">e residente a</div>
<div class="dott_solid" style="margin-top: 0px;">&nbsp; <%=(sedeOperativa != null) ? sedeOperativa.getDescrizioneComune() : ""%></div>
<div class="clear1"></div>
...
<div class="dott_solid" style="margin-top: 0px;">&nbsp; <%=(sedeOperativa != null) ? (sedeOperativa.getCap()) : ""%></div>
<div class="nodott" style="margin-top: 0px;">tel.</div>
<div class="dott_solid" style="margin-top: 0px;">&nbsp; <%=(linea_proprietario!=null && linea_proprietario.getTelefono1() != null) ? (linea_proprietario.getTelefono1()) : ""%></div>
<div class="clear1"></div>
...
<div class="nodott" style="margin-top: 0px;">che il <%=thisAnimale.getNomeSpecieAnimale() %> di cui sopra è deceduto in data</div>
<div class="dott_solid" style="margin-top: 0px;">&nbsp; <%=toDateasString(dati_decesso.getDataDecesso())%></div>
<div class="clear1"></div>

<div class="nodott" style="margin-top: 0px;">causa</div>
<div class="dott_solid" style="margin-top: 0px;">&nbsp; <%=causeDecessoList.getSelectedValue(dati_decesso.getIdCausaDecesso())%></div>
</br>
</br>
