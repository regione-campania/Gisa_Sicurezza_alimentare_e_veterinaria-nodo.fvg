...
public class HtmlSelect extends ArrayList {
  ...
  public void setTypeTimeZone() {
    ...
    for (int i = 0; i < timeZones.length; i++) {
      ...
      if (gmt != Math.round(gmt)) {
    	  logger.info("[CANINA] - HtmlSelect-> GMT Offset: " + gmt);
      }
      int fraction = 0;
      if (gmt != gmtInt) {
  ...
  public void addItem(int tmp1, String tmp2,String title) {
	    HtmlOption option = new HtmlOption(String.valueOf(tmp1), tmp2);
	    HashMap<String, String> hash = new HashMap<String, String>();
	    if(title!=null)
	    	hash.put("title", title);
	    if(hash!=null && !hash.isEmpty())
	    	option.setAttributeList(hash);
	    this.add(option);
	  }
  ...
  public void build() {
    ...
    for (int i = 0; i < this.size(); i++) {
      ...
      String attributes = "";
      if(thisOption.getAttributes()!=null)
    	  attributes = thisOption.getAttributes();
