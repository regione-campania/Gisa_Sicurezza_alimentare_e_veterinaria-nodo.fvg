...
//    int roleId = this.getUser(userId).getRoleId();
  ...
  public boolean buildHierarchyListbyUserId(Connection db, String username) throws SQLException {
	    //NOTE: The UserList does a joined query that gets the user and contact
	    //data at the same time.  That's why the buildContact is disabled.
	  ...
	  java.sql.PreparedStatement pstToRemove = db.prepareStatement("select user_id from access_ where username = ? and trashed_date is null and data_scadenza = (select max (data_scadenza) from access_ where username = ? and trashed_date is null ) ");
	  ...
	  while (rsToRemove.next()){
	   ...
	   java.sql.PreparedStatement pst = db.prepareStatement("select user_id from access where username = ? and trashed_date is null");
	   pst.setString(1,username );
	   java.sql.ResultSet rs = pst.executeQuery();
	   while (rs.next())
	   {
	    int userId = rs.getInt(1);
	    User uu = new User(db,userId);
	    uu.setBuildContact(true);
	    uu.setBuildHierarchy(true);
	    uu.buildResources(db);
	    //Combine the lists
