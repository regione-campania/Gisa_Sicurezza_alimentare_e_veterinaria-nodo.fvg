...
//      }
     ...
     // SystemStatus systemStatus = (SystemStatus) globalStatus.get(ce.getUrl());
      ...
      ConnectionPool sqlDriver =
              (ConnectionPool) servlet.getServletConfig().getServletContext().getAttribute("ConnectionPool");
      SystemStatus systemStatus = (SystemStatus) globalStatus.get(sqlDriver.getUrl());
      if (systemStatus == null) {
        ...
        try {
          ...
          systemStatus = SecurityHook.retrieveSystemStatus(
              servlet.getServletConfig().getServletContext(), db,  thisSite.getLanguage());
        } catch (Exception e) {
        } finally {
          ((ConnectionPool) servlet.getServletConfig().getServletContext().getAttribute(
              "ConnectionPool")).closeConnection(db, null);
        }
      }
   ...
   * @return Description of the Return Value
   */
  public static Site retrieveSite(ServletContext context) {
    ConnectionPool sqlDriver =
        (ConnectionPool) context.getAttribute("ConnectionPool");
    ApplicationPrefs prefs = (ApplicationPrefs) context.getAttribute(
        "applicationPrefs");
    SiteList sites = SiteUtils.getSiteList(prefs, sqlDriver);
    if (sites.size() == 1) {
      return (Site) sites.get(0);
    }
    return null;
   ...
   */
  public static synchronized SystemStatus retrieveSystemStatus(ServletContext context, Connection db, String language) throws SQLException {
    ...
    ConnectionPool sqlDriver =
            (ConnectionPool) context.getAttribute("ConnectionPool");
    if (!statusList.containsKey(sqlDriver.getUrl())) {
    ...
    //  newSystemStatus.setConnectionElement((ConnectionElement) ce.clone());
      ...
      newSystemStatus.setFileLibraryPath(
          prefs.get("FILELIBRARY") + sqlDriver.getDbName() + fs);
      ...
      if (prefs.has("WEBSERVER.URL")) {
        ...
        SiteList sites = SiteUtils.getSiteList(prefs, cp);
        if (sites.size() > 0) {
      ...
      statusList.put(sqlDriver.getUrl(), newSystemStatus);
      if (System.getProperty("DEBUG") != null) {
//        System.out.println(
...
//          Constants.SYSTEM_PROJECT_NAME_LIST, projectNameCache);
    ...
    return (SystemStatus) statusList.get(sqlDriver.getUrl());
  }
}
