...
public class MainMenuHook implements ControllerMainMenuHook {
	...
	public void generateMenu(HttpServletRequest request, String actionPath) {
		...
		// Use the connection element and system status to access the cached
		// permissions table
		ConnectionPool ce = (ConnectionPool) request.getSession().getAttribute("ConnectionPool");
		...
		systemStatus = (SystemStatus) ((Hashtable) context.getAttribute("SystemStatus")).get(thisUser
				.getConnectionPoll().getUrl());
		...
		while (menuItemsList.hasNext()) {
			...
			if ("".equals(thisMenu.getPermission())
					|| (systemStatus != null && systemStatus.hasPermission(thisUser.getRoleId(),
							thisMenu.getPermission() + (isOfflineMode ? "-offline" : "")))) {
				...
				if ((thisModule.getMenuKey() != null && thisMenu.hasActionName(thisModule.getMenuKey()))
					...
					menuList.append("<a href=\"" + thisMenu.getLink() + "\">");
					if (shortHtml != null) {
						menu.append(shortHtml);
						menuList.append(shortHtml);
					} else {
						menu.append(thisMenu.getShortHtml());
						menuList.append(thisMenu.getShortHtml());
					}
					menuList.append("</a>");
					menu.append("</a>");
					menu.append("</th>");
					graphicMenu.append("<a href='" + thisMenu.getLink() + "'><img border='0' src='images/"
							+ thisMenu.getGraphicOn() + "' width='" + thisMenu.getGraphicWidth() + "' height='"
							+ thisMenu.getGraphicHeight() + "'></a>");
					if (longHtml != null) {
						smallMenuList.add("<a " + addSelectedClass(thisMenu) + "href='" + thisMenu.getLink() + "'>"
								+ longHtml + "</a>");
					} else {
						smallMenuList.add("<a " + addSelectedClass(thisMenu) + "href='" + thisMenu.getLink() + "'>"
								+ thisMenu.getLongHtml() + "</a>");
					}
					// Build the submenu
					...
					while (j.hasNext()) {
						...
						if ("".equals(thisMenu.getPermission())
								|| (systemStatus != null && systemStatus.hasPermission(thisUser.getRoleId(),
										newItem.getPermission() + (isOfflineMode ? "-offline" : "")))) {
							menuList.append("<li>");
							menuList.append(newItem.getHtml());
							menuList.append("</li>");
						}
					...
					// The user is not on this link, set the off state of the
					// menu
					...
					// Build the submenu IN QUESTO CASO MI SERVE SOLO PER IL
					// LAYOUT EXT
					Iterator j = thisMenu.getSubmenuItems().iterator();
					menuList.append("<ul>");
					while (j.hasNext()) {
...
//
//						// if
//						// (newItem.getName().equals(thisModule.getSubmenuKey()))
//						// {
//						// newItem.setIsActive(true);
//						// newItem.setHtmlClass("submenuItemSelected");
//						// }
...
//						menuList.append("</li>");
					...
					if (thisMenu.hasGraphicRollover()) {
						if (shortHtml != null) {
							graphicMenu.append(" onMouseOut=\"MM_swapImgRestore()\" onMouseOver=\"MM_swapImage('"
									+ shortHtml + "','','images/" + thisMenu.getGraphicRollover() + "',1)\"");
						} else {
							graphicMenu.append(" onMouseOut=\"MM_swapImgRestore()\" onMouseOver=\"MM_swapImage('"
									+ thisMenu.getShortHtml() + "','','images/" + thisMenu.getGraphicRollover()
									+ "',1)\"");
						}
					}
					graphicMenu.append(">");
					...
					if (pageTitle != null) {
						smallMenuList.add("<a " + addNormalClass(thisMenu) + "href='" + thisMenu.getLink() + "'>"
								+ pageTitle + "</a>");
					} else {
						smallMenuList.add("<a " + addNormalClass(thisMenu) + "href='" + thisMenu.getLink() + "'>"
								+ thisMenu.getLongHtml() + "</a>");
		...
		theMenus[0] = menu.toString();// +
										// "<td style=\"width:100%; border: 0px; background-image: none !important; background: #EDEDED; border-bottom: 1px #000 solid; cursor: default\"><img border=\"0\" src=\"images/blank.gif\" /></td>";
		theMenus[1] = "<td width=\"" + menuWidth + "\" nowrap>" + graphicMenu.toString() + "</td>";
		theMenus[2] = smallMenu.toString();
		theMenus[3] = menu.toString();
		theMenus[4] = menuTops.toString();// +
											// "<td style=\"width:100%; border: 0px; background-image: none !important; background: #FFF; cursor: default\"><img border=\"0\" src=\"images/blank.gif\" /></td>";
	 ...
	 *
	 * @param thisItem
	 *            The feature to be added to the NormalClass attribute
	 ...
	 *
	 * @param thisItem
	 *            The feature to be added to the SelectedClass attribute
	 * @return Description of the Return Value
	 */
