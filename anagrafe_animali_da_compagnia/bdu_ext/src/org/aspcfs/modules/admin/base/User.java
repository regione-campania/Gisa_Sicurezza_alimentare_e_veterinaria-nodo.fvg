...
public class User extends GenericBean {
  ...
  //Informazione per individuare il comune degli utenti con ruolo "Utente Comune"
  protected int idComune = -1;
  private int idImportatore = -1;
  
  
  protected int idLineaProduttivaRiferimento = -1;  /**
  														id relazione stabilimento lineaproduttiva per utente tipo referente canile per tenere traccia del canile a cui fa riferimento. 
  															Eventualmente di altri operatori per permettere l'accesso alla BDU*/
	...
	//Flusso 342
		...
		private String profilo_professionale;
	  

		public String getProfilo_professionale() {
			return profilo_professionale;
		}
		public void setProfilo_professionale(String profilo_professionale) {
			this.profilo_professionale = profilo_professionale;
		}
...
/**
  ...
  public int getIdLineaProduttivaRiferimento() {
	return idLineaProduttivaRiferimento;
}

public void setIdLineaProduttivaRiferimento(int idLineaProduttivaRiferimento) {
	this.idLineaProduttivaRiferimento = idLineaProduttivaRiferimento;
}
...
/**
  ...
  public void updateLogin(Connection db) throws SQLException {
    if (this.id > -1) {
      ...
      try {
	    ...
	    //  pst.setBoolean(5, this.getHidden());
	    //  pst.setInt(6, this.getModifiedBy());
  ...
  public void buildRecord(Connection db, int userId) throws SQLException {
    ...
    StringBuffer sql = new StringBuffer();
    sql.append(
        "SELECT a.comune, a.username, a." + DatabaseUtils.addQuotes(db, "password") + ", a.role_id, a.last_login, a.manager_id, " +
            "a.site_id AS siteid, " +
            ...
            "a.currency, a." + DatabaseUtils.addQuotes(db, "language") + ", a.webdav_password, a.hidden, a.allow_webdav_access, a.allow_httpapi_access, a.id_linea_produttiva_riferimento, " +
           ...
           // "d.description AS departmentname, " +
            "ca.city AS city, ca.postalcode AS postalcode " +
            "FROM " + DatabaseUtils.addQuotes(db, "access_") + " a " +
          ...
          //  "LEFT JOIN organization o ON (c.org_id = o.org_id) " +
           // "LEFT JOIN lookup_department d ON (c.department = d.code) " +
            "LEFT JOIN " + DatabaseUtils.addQuotes(db, "access_") + " m_usr ON (a.manager_id = m_usr.user_id) " +
    ...
    if (rs.next()) {
      ...
      if (contactId>0){
    	  Contact c = new Contact(db, contactId);
    	  setContact(c);
      }
  ...
  protected boolean isValidNoPass(Connection db, ActionContext context)
    ...
    if (managerId > 0 && id > -1 && alias == -1) {
      if (managerId == id) {
        ...
        ConnectionPool ce = (ConnectionPool) context.getRequest()
            .getSession().getServletContext().getAttribute("ConnectionPool");
  ...
  protected void buildRecord(ResultSet rs) throws SQLException {
    ...
    try {
		rs.findColumn("id_importatore");
		 idImportatore = rs.getInt("id_importatore");
    ...
    try {
		rs.findColumn("id_linea_produttiva_riferimento");
		idLineaProduttivaRiferimento = rs.getInt("id_linea_produttiva_riferimento");
	} catch (SQLException sqlex) {
		// System.out.println("not found");
	}
	...
	public ArrayList<String> getCoordinateUltimoAccesso(Connection db) throws SQLException
	{
		...
		if (this.getAccess_position_lat()<=0 && this.getAccess_position_lon()<=0 )
		{
			PreparedStatement pst = db.prepareStatement("select access_position_lat,access_position_lon,access_position_err,max(entered) as entered "+
					"from access_log " +
					"where  user_id = ? and access_position_lat>0 and access_position_lon>0 " +
			...
			if (rs.next())
			{
				...
				getCoordinate.add(lat+"");
				getCoordinate.add(lon+"");
				getCoordinate.add(err);
				getCoordinate.add(entered+"");
			}
