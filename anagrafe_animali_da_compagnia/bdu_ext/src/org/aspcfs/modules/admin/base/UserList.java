...
public class UserList extends Vector implements SyncableList {
  ...
  public PreparedStatement prepareList(Connection db) throws SQLException {
    ...
    sqlCount.append(
        "SELECT COUNT(*) AS recordcount " +
          ...
          //  "LEFT JOIN contact_emailaddress ce ON (c.contact_id = ce.contact_id) " +
           // "LEFT JOIN " + DatabaseUtils.addQuotes(db, "access") + " m_usr ON (a.manager_id = m_usr.user_id) " +
            ...
            "WHERE a.user_id > -1 and a.trashed_date is null ");
    ...
    sqlSelect.append(
        "a.comune, a.username, a." + DatabaseUtils.addQuotes(db, "password") + ", a.role_id, a.last_login, a.manager_id, " +
            "a.site_id AS siteid,a.id_linea_produttiva_riferimento , a.id_importatore, " +
            ...
            "a.currency, a." + DatabaseUtils.addQuotes(db, "language") + ",a.id_importatore, a.webdav_password, a.hidden, a.allow_webdav_access, a.allow_httpapi_access, " +
            "r." + DatabaseUtils.addQuotes(db, "role") + " AS systemrole, r.role_type, " +
            "true AS mgr_enabled, " +
            "b.description AS site_id_name, " +
            "c.*, true AS orgenabled, null AS orgtrasheddate, " +
            "'n.d' AS departmentname, ca.city AS city, ca.postalcode AS postalcode, " +
            "'n.d' AS industry_name, " +
            "'n.d' AS source_name, " +
            "'n.d' AS stage_name, " +
            "'n.d' AS rating_name , 'n.d' as import_name " +
    		...
    		"WHERE a.user_id > -1 and a.trashed_date is null ");
            //"WHERE a.username in ('izzillo.llpp','battaglinina1','sysadmna2','sysadmbn1','DS_SB_TORIELLO','DS_MC_FACCHIANO', 'marinana1','sysadmna1', 'sysadmsa1','sysadmsa2','s.andreottola_ref','sysadmin','b.sansone','guiros.lp','guiros.llpp','s.squitieri3','51214', 's.squitieri','contributi_hd','rg_contributi','5909','26009','comunena1','TOR560') and a.user_id > -1 and a.trashed_date is null ");
    PreparedStatement pst = db.prepareStatement(
  ...
  public final static Contact retrieveUserContact(ActionContext context, int userId) {
    ConnectionPool ce = (ConnectionPool) context.getSession().getAttribute(
        "ConnectionPool");
    if (ce == null) {
      return null;
    }
