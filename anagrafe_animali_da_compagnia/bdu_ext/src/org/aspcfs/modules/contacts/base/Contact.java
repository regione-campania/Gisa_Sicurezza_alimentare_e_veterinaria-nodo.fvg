...
public class Contact extends GenericBean {
	...
	//Veterinari privati
	private int idProvinciaIscrizioneOrdine = -1;
	...
	public int getIdProvinciaIscrizioneOrdine() {
		return idProvinciaIscrizioneOrdine;
	}

	public void setIdProvinciaIscrizioneOrdine(int idProvinciaIscrizioneOrdine) {
		this.idProvinciaIscrizioneOrdine = idProvinciaIscrizioneOrdine;
	}
	...
	public void queryRecord(Connection db, int contactId) throws SQLException {
		...
		sql.append(
				"SELECT c.*, true AS orgenabled, null AS orgtrasheddate," +
				" '' as departmentname, " +
				//" ca.city AS city, " +
				" '' AS city, " +
...
//		pst.setBoolean(3, true);
	...
	public int updateDatiContatto(Connection db) throws SQLException {
		int resultCount = -1;
		try {
	...
	//	orgId = DatabaseUtils.getInt(rs, "org_id");
		...
		try {
			rs.findColumn("nr_iscrione_albo_vet_privato");
			nrIscrizioneOrdine = rs.getString("nr_iscrione_albo_vet_privato");
		...
		try {
			rs.findColumn("id_provincia_iscrizione_albo_vet_privato");
			idProvinciaIscrizioneOrdine = rs.getInt("id_provincia_iscrizione_albo_vet_privato");
		} catch (SQLException sqlex) {
			// System.out.println("not found");
		}
	}
