...
public class PartiteCommercialiList extends Vector implements SyncableList {
	...
	public ResultSet queryList(Connection db, PreparedStatement pst) throws SQLException {
		...
		sqlCount.append("Select COUNT(* ) AS recordcount from public_functions.get_lista_partite(?) ");
		pagedListInfo.setColumnToSortBy("nr_certificato");
		if (pagedListInfo != null) {
			//Get the total number of records matching filter
			pst = db.prepareStatement(sqlCount.toString());
			// UnionAudit(sqlFilter,db);
			pst.setInt(1, idImportatore);
		...
		sqlSelect.append("select distinct * from public_functions.get_lista_partite(?) "  );
	...
	//	items = prepareFilter(pst);
	 ...
	 /* public ResultSet queryList(Connection db, PreparedStatement pst) throws SQLException {
		    ...
		    sqlCount.append(
		           ...
		           sqlCount.append(" WHERE p.id_partita_commerciale >= 0 ");
		    ...
		    if (pagedListInfo != null) {
		     ...
		     // UnionAudit(sqlFilter,db);
		      ...
		      pagedListInfo.setColumnToSortBy("p.nr_certificato");
		      pagedListInfo.appendSqlTail(db, sqlOrder);
		            
		      //Optimize SQL Server Paging
		      //sqlFilter.append("AND o.org_id NOT IN (SELECT TOP 10 org_id FROM organization " + sqlOrder.toString());
		    } else {
		      sqlOrder.append("ORDER BY p.nr_certificato ");
		    }
		    ...
		    sqlSelect.append(
		    		...
		    		"from partita_commerciale p "  );
		    	 	...
		    	 	sqlSelect.append("WHERE p.id_partita_commerciale >= 0 ");
