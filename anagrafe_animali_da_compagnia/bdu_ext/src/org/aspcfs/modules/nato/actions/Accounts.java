package org.aspcfs.modules.nato.actions;
...
import org.aspcfs.modules.nato.base.McNonStandard;
...
public final class Accounts extends CFSModule
{
	...
	public String executeCommandList2(ActionContext context) throws SQLException
	{
		...
		//	PreparedStatement	stat	= db.prepareStatement( "select * from mc_non_standard m,lookup_asset_manufacturer l,lookup_asset_status k,lookup_asset_vendor v, lookup_codici_izs p where m.razza=l.code and m.mantello=k.code and v.code=m.taglia and m.esito_esame1=p.code " );
			PreparedStatement	stat	= db.prepareStatement( "select * from mc_non_standard m,lookup_razza l,lookup_mantello k,lookup_taglia v where m.razza=l.code and m.mantello=k.code and v.code=m.taglia " );
			ResultSet			rs		= stat.executeQuery();
								
			TableFacade tf = TableFacadeFactory.createTableFacade( "1", context.getRequest() );
			tf.setEditable(true);
			...
			tf.getTable().getRow().getColumn( "indirizzo" ).setTitle( "Indirizzo/ numero di telefono" );
				...
				cg.getCellRenderer().setCellEditor( 
						...
						{	
							public Object getValue(Object item, String property, int rowCount)
							{
								...
								try {
									alimenti = new LookupList(db,"lookup_razza");
								} catch (SQLException e) {
									// TODO Auto-generated catch block
									e.printStackTrace();
								}
				...
				cg.getCellRenderer().setCellEditor( 
						...
						{	
							public Object getValue(Object item, String property, int rowCount)
							{
								...
								try {
									mantello= new LookupList(db,"lookup_mantello");
								} catch (SQLException e) {
									// TODO Auto-generated catch block
									e.printStackTrace();
								}
				...
				cg.getCellRenderer().setCellEditor( 
						...
						{	
							public Object getValue(Object item, String property, int rowCount)
							{
								...
								try {
									taglia = new LookupList(db,"lookup_taglia");
								} catch (SQLException e) {
									// TODO Auto-generated catch block
									e.printStackTrace();
								}
...
//				cg.setFilterable( false );
				...
				cg.getCellRenderer().setCellEditor( 
						...
						{	
							public Object getValue(Object item, String property, int rowCount)
							{
								LookupList esito = null;
								try {
									esito= new LookupList(db,"lookup_codici_izs");
									esito.addItem(-1,"--Nessuno--");
									esito.setSelectName("--Nessuno--");
								} catch (SQLException e) {
									// TODO Auto-generated catch block
									e.printStackTrace();
								}
								...
								String temp	= (String) (new HtmlCellEditor()).getValue(item, property, rowCount);
								String id	= (String) (new HtmlCellEditor()).getValue(item, "esito_esame1", rowCount);

								String serial_number	= (String) (new HtmlCellEditor()).getValue(item, "serial_number", rowCount);
								temp = (temp == null || "".equals(temp.trim())) ? ("-") : (temp);
								return esito.getHtmlSelect("esito_esame1_"+serial_number, id);
							}
						}
		...
		String tabella = tf.render();
 		...
 		//  logger.severe("[CANINA] - EXCEPTION nella action executeCommandList2 della classe Accounts");
 		  e.printStackTrace();	
  		}
  		finally{
																			...
																			if(kiaveNomeColonna.equals("tipo_esame1"))
																			{
																				...
																				//	dist.setTipo_esame2(valore);
																					...
																					if(kiaveNomeColonna.equals("esito_esame1"))
																					{
																						...
																						{
																							//dist.setEsito_esame2(Integer.valueOf(valore));
																						}
																							...
																							if(kiaveNomeColonna.equals("data_esame1"))
																							{
																								...
																								if(kiaveNomeColonna.equals("data_esame2")){
																									SimpleDateFormat ss=new SimpleDateFormat("dd/MM/yyyy");
																									java.util.Date d=ss.parse(valore);
																									//dist.setData_esame2(new java.sql.Date(d.getTime()));
...
c++;
		...
		private String validateColumn(WorksheetColumn colonna, String changedValue) {
			...
			if(nomeColonna.equals("data_di_nascita")){
				...
				try{
			 	...
			 	//	  logger.severe("[CANINA] - EXCEPTION nel metodo validateColumn della classe Accounts");
		...
		if(nomeColonna.equals("data_esame1")){
				...
				if(nomeColonna.equals("serial_number")){
					String changedValue1 = colonna.getChangedValue();
					if(changedValue1.length()>13)
					{
							colonna.setError("Il Campo microchips deve avere lunghezza massima pari a 13 caratteri alfanumerici"); 
							errore="Il Campo microchips deve avere lunghezza pari a 13";
					}
			...
			{
			if(nomeColonna.equals("")){
			String changedValue1 = colonna.getChangedValue();   

			if(changedValue1.equals(""))
			{
					colonna.setError("Il Campo microchips deve essere valorizzato"); 
					errore="Il Campo microchips deve essere valorizzato";
				}
				if(errore.equals("  "))
					colonna.removeError();
