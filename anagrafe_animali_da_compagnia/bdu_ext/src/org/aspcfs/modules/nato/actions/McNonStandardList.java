...
public final class McNonStandardList extends CFSModule
{
	...
	public String executeCommandAdd(ActionContext context) {
		...
		try {
			...
			coll.add(new McNonStandard("",-1,-1,-1,null,"","","","","","","","",-1,null));
	...
	public String executeCommandList(ActionContext context) {
		...
		try {
			...
			String modificatipi="update mc_non_standard set razza=? ,mantello=?, taglia=?, esito_esame1=?  where serial_number=?";
			...
			for(String s : listaMatricole){
				int i=1;
					...
					String tipo4=context.getRequest().getParameter("esito_esame1_"+s);
					...
					if ((tipo4!=null)){
					 int	esito=Integer.parseInt(tipo4);
							pst3.setInt(i++, esito);
						}
			
					
				if((tipo!=null)||(tipo2!=null)||(tipo3!=null)||(tipo4!=null)){
				...
				for(String s : uniquePropertyValues){
					...
					for (WorksheetColumn colonna : columns) { 
						...
						if (colonna.hasError()) {  
							...
							errori += ""+colonna.getError()+"\n";
							context.getRequest().setAttribute("errore", errori);
					...
					PreparedStatement pst2=db.prepareStatement("" +
							"select * from mc_non_standard m,lookup_asset_manufacturer l,lookup_asset_status k,lookup_asset_vendor v,lookup_codici_izs p where m.razza=l.code and m.mantello=k.code and v.code=m.taglia and serial_number='"+s+"'");
						//	"select * from mc_non_standard m,lookup_asset_manufacturer l,lookup_asset_status k,lookup_asset_vendor v,lookup_codici_izs p where m.razza=l.code and m.mantello=k.code and v.code=m.taglia and serial_number='"+s+"'");
					...
					/*	String esito =context.getRequest().getParameter("esito_esame1_");
						if (esito!=null){
						dist.setEsito_esame1(Integer.parseInt(esito));
						}
						*/
