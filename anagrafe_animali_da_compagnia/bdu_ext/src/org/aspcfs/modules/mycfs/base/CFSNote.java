...
public class CFSNote extends GenericBean {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -9089138927747958049L;
	protected transient Logger logger = Logger.getLogger("MainLogger");
	...
	private String telefonoIndicato = "";
	private String emailIndicata = "";
	 ...
	 *             Description of Exception
	 */
	public CFSNote(Connection db, String noteId, int myId, String listView)
			throws SQLException {
	 ...
	 *             Description of Exception
	 */
	public CFSNote(Connection db, int noteId, int myId, String listView)
			throws SQLException {
	 ...
	 *             Description of the Exception
	 */
	private void queryRecord(Connection db, int noteId, int myId,
			String listView) throws SQLException {
		if (noteId == -1) {
			throw new SQLException("CFS Note ID not specified");
		}
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT m.*, ml.id AS ml_id, ml.sent_to, ml.status, ml.viewed, ml.enabled, ml.sent_from, "
				...
				+ "WHERE m.id > -1 AND (m.id = ml.id) " + "AND m.id = ? ");
	...
	private void queryRecord(Connection db, int noteId) throws SQLException {
		...
		StringBuffer sql = new StringBuffer();
		sql.append("SELECT m.* " + "FROM cfsinbox_message m "
				+ "WHERE m.id = ? ");
		PreparedStatement pst = db.prepareStatement(sql.toString());
		pst.setInt(1, noteId);
		ResultSet rs = pst.executeQuery();
		...
		try {
			if (id != -1) {
				PreparedStatement pst = db
						.prepareStatement("SELECT ct_eb.namefirst as sent_namefirst, ct_eb.namelast as sent_namelast, ct_eb.company AS sent_company,ml.status "
								+ "FROM cfsinbox_messagelink ml "
								+
								// "LEFT JOIN access acc on (ml.sent_to = acc.user_id)  "
								// + MIO DA CANCELLARE
								"LEFT JOIN contact ct_eb ON (ml.sent_to = ct_eb.contact_id) "
								+ "WHERE ml.id > -1 " + "AND ml.id = ? ");
				pst.setInt(1, this.getId());
				ResultSet rs = pst.executeQuery();
				while (rs.next()) {
					String recipient = Contact.getNameFirstLastOrCompany(
							rs.getString("sent_namefirst"),
							rs.getString("sent_namelast"),
							rs.getString("sent_company"));
	...
	public String getEnteredDateTimeString() {
		try {
			return DateFormat.getDateTimeInstance(DateFormat.SHORT,
					DateFormat.SHORT).format(entered);
		} catch (NullPointerException e) {
		}
		return ("");
	}
	...
	public String getModifiedDateTimeString() {
		try {
			return DateFormat.getDateTimeInstance(DateFormat.SHORT,
					DateFormat.SHORT).format(modified);
		} catch (NullPointerException e) {
		}
		return ("");
	}
	...
	public String getTelefonoIndicato() {
		return telefonoIndicato;
	}

	public void setTelefonoIndicato(String telefonoIndicato) {
		this.telefonoIndicato = telefonoIndicato;
	}

	public String getEmailIndicata() {
		return emailIndicata;
	}

	public void setEmailIndicata(String emailIndicata) {
		this.emailIndicata = emailIndicata;
	}
	...
	public boolean insert(Connection db) throws SQLException {
		...
		try {
			...
			sql.append("INSERT INTO cfsinbox_message "
					+ "("
					+ (id > -1 ? "id, " : "")
					+ "enteredby, modifiedby, body, reply_id, type, delete_flag, tipo, telefono_indicato, email_indicata) "
					+ "VALUES (" + (id > -1 ? "?, " : "")
					+ "?, ?, ?, ?, ?, ?, ?, ?, ?) ");
			int i = 0;
			PreparedStatement pst = db.prepareStatement(sql.toString());
			...
			pst.setInt(++i, this.tipo);
			pst.setString(++i, this.telefonoIndicato);
			pst.setString(++i, this.emailIndicata);
			pst.execute();
			pst.close();
	 ...
	 *             Description of Exception
	 */
	public boolean insertLink(Connection db, boolean isUser)
			throws SQLException {
		try {
			int i = 0;
			PreparedStatement pst = db
					.prepareStatement("INSERT INTO cfsinbox_messagelink "
							+ "(id, sent_to, sent_from) " + "VALUES (?, ?, ?) ");
			...
			if (!isUser) {
				pst = db.prepareStatement("UPDATE cfsinbox_messagelink "
						+ "SET status = 3 " + "WHERE  id = ? AND sent_to = ? ");
				pst.setInt(1, this.getId());
	...
	public boolean delete(Connection db, int myId) throws SQLException {
		try {
			...
			String inboxQuery = "SELECT count(*) as inboxcount FROM cfsinbox_messagelink "
					+ "WHERE id =" + this.getId() + " " + "AND status <> 3 ";
			 ...
			 */
			ContactHistory.deleteObject(db, OrganizationHistory.CFSNOTE,
					this.getId());
			if ((outboxCount == 0 && inboxCount == 1)
					|| (outboxCount == 1 && inboxCount == 0)) {
				pst = db.prepareStatement("DELETE FROM cfsinbox_messagelink "
						+ "WHERE id = ? ");
				pst.setInt(1, this.getId());
				pst.execute();
				pst.close();
				pst = db.prepareStatement("DELETE FROM cfsinbox_message "
						+ "WHERE id = ? ");
				pst.setInt(1, this.getId());
				pst.execute();
				pst.close();
				...
				/*
				 * atleast one of the sender or the recipient(s) has not deleted
				 * message
				 */
				if (!getCurrentView().equalsIgnoreCase("sent")) {
					 ...
					 */
					pst = db.prepareStatement("UPDATE cfsinbox_message "
							+ "SET delete_flag = ? " + "WHERE  id = ? ");
			...
			if (System.getProperty("DEBUG") != null) {
				logger.severe("[CANINA] - CFSNote-> " + e.toString());
				throw new SQLException(e.getMessage());
			}
		} finally {
		}
	...
	protected int update(Connection db, boolean override) throws SQLException {
		...
		sql.append("UPDATE cfsinbox_message " + "SET subject = ?, body = ?, "
	...
	protected void buildRecord(ResultSet rs) throws SQLException {
		...
		if (!getCurrentView().equalsIgnoreCase("sent")) {
			...
			// contact table
			sentName = rs.getString("sent_namefirst") + " "
					+ rs.getString("sent_namelast");
			telefonoIndicato = rs.getString("telefono_indicato");
			emailIndicata = rs.getString("email_indicata");
		} else {
			// outbox case
			this.setId(rs.getInt("id"));
			subject = rs.getString("subject");
