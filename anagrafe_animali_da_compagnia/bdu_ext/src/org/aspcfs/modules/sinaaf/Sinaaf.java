...
public class Sinaaf extends Thread {
	...
	public String[] aggiornamento(Connection db, int userId, String id, String code, String entita, String method) throws Exception
	{
		...
		if(lista.isEmpty())
		{
			toReturn[3] = "Entità già inviata e sincronizzata";
		}
	...
	if(ApplicationProperties.getProperty("BDU2SINAC_ATTIVO").equals("true"))
		...
		String net = ApplicationProperties.getProperty("BDU2SINAC_HOST_L");
			...
			try {
				  ...
				  URL url = new URL("http://"+net+":"+ApplicationProperties.getProperty("BDU2SINAC_PORT")+"/"+ApplicationProperties.getProperty("BDU2SINAC_SYNC_APP")+"?json={\"token\":\""+idEntita+"@"+entita+"\",\"ambiente\":\""+ApplicationProperties.getProperty("AMBIENTE")+"\",\"forza_allineamento\":\"false\"}"); 
		...
		if(ApplicationProperties.getProperty("BDU2SINAC_ATTIVO").equals("true"))
	{
		...
		try {
			...
			String net = ApplicationProperties.getProperty("BDU2SINAC_HOST_L");
			  ...
			  URL url = new URL("http://"+net+":"+ApplicationProperties.getProperty("BDU2SINAC_PORT")+"/"+ApplicationProperties.getProperty("BDU2SINAC_SYNC_APP")+"?json={\"token\":\""+id+"@"+entita+"\",\"ambiente\":\""+ApplicationProperties.getProperty("AMBIENTE")+"\",\"forza_allineamento\":\"false\"}"); 
			  HttpURLConnection connection = (HttpURLConnection) url.openConnection();
			  System.out.println(url);
			  BufferedReader read = new BufferedReader(new InputStreamReader(connection.getInputStream()));
			  String line = read.readLine();
	...
	public void unificaProprietari(Connection db, int userId,int idDaRimuovere, int idDaConservare, String entita, ArrayList<Integer> idEventoAggiornati) throws Exception
	{
		if(ApplicationProperties.getProperty("BDU2SINAC_UNIFICA_ON").equals("true"))
		{
			...
			try {
				  ...
				  URL url = new URL("http://"+net+":"+ApplicationProperties.getProperty("BDU2SINAC_PORT")+"/"+ApplicationProperties.getProperty("BDU2SINAC_UNIFICAZIONE")+"?json={\"id_eventi_da_aggiornare\":\""+new Gson().toJson(idEventoAggiornati)+"\",\"id_prop_da_eliminare\":\""+idDaRimuovere+"\",\"id_prop_in_sostituzione\":\""+idDaConservare+"\",\"ambiente\":\""+ApplicationProperties.getProperty("AMBIENTE")+"\"}"); 
				  HttpURLConnection connection = (HttpURLConnection) url.openConnection();
				  System.out.println(url);
				  BufferedReader read = new BufferedReader(new InputStreamReader(connection.getInputStream()));
				  String line = read.readLine();
