...
public class StabilimentoList extends Vector implements SyncableList {
	...
	public ResultSet queryStabilimento(Connection db, PreparedStatement pst,int idLineaProduttiva) throws SQLException {
		...
		sqlSelect.append("select s.*, r.codice_sinaaf from opu_stabilimento s join opu_relazione_stabilimento_linee_produttive r on s.id=r.id_stabilimento where 1=1 and r.id =? and r.trashed_date is null ");
