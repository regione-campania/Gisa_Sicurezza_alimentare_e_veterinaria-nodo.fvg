...
public class CFSModule {
	...
	public static final int VETERINARIO_PRIVATO = 24 ;
	 ...
	 * @since 1.1
	 */
	protected Connection getConnection(ActionContext context) {
		...
		try {
			ConnectionPool sqlDriver = (ConnectionPool) context.getServletContext().getAttribute(
					"ConnectionPool");
			return sqlDriver.getConnection(context);
	...
	protected Connection getConnectionVam(ActionContext context) {
	...
	protected Connection getConnectionGisa(ActionContext context) {
	...
	protected Connection getConnectionImportatori(ActionContext context) {
		

		
		try {
			ConnectionPool sqlDriver = (ConnectionPool) context.getServletContext().getAttribute(
					"ConnectionPoolImportatori");
			return sqlDriver.getConnection(context);
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
...
/*	protected Connection getConnectionVam(ActionContext context, ConnectionElement ce) throws SQLException {
		ConnectionPool sqlDriver = (ConnectionPool) context.getServletContext().getAttribute(
		"ConnectionPool");
		return sqlDriver.getConnectionVam(ce);
	}
	
	protected Connection getConnectionVam(ActionContext context) throws SQLException {
		ConnectionElement ce = (ConnectionElement) context.getSession().getAttribute(
		"ConnectionElementVam");
		return this.getConnectionVam(context, ce);
	}*/
...
//		"ConnectionElement");
		...
		ConnectionPool ce = (ConnectionPool)context.getSession().getServletContext().getAttribute("ConnectionPool");
	 ...
	 * @return The systemStatus value
	 */
	protected SystemStatus getSystemStatus(ActionContext context, ConnectionPool ce) {
		//if (ce != null) {
//	    ConnectionPool sqlDriver = (ConnectionPool) context.getServletContext().getAttribute(
//	            "ConnectionPool");
...
//		"ConnectionElement");
	...
	protected boolean hasPermission(ActionContext context, String permission) {
		...
		if (ambiente.equals("SLAVE") && (permission.endsWith("add") || permission.endsWith("edit") || permission.endsWith("delete"))){
			...
			SystemStatus systemStatus = this.getSystemStatus(context);
			return (systemStatus.hasPermission(thisUser.getRoleId(), permission + (CFSModule.isOfflineMode(context)?"-offline":"")));
		}
	}
	
	protected Connection getConnectionBdu(ActionContext context) {
		...
		try {
			ConnectionPool sqlDriver = (ConnectionPool) context.getServletContext().getAttribute(
					"ConnectionPoolBdu");
			return sqlDriver.getConnection(null);
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	protected boolean isUgualeAslAnimaleAslUtente(ActionContext context, Animale animale){
		
		UserBean thisUser = (UserBean) context.getSession().getAttribute("User");
		return ( animale.getIdAslRiferimento() == Integer.parseInt(ApplicationProperties.getProperty("ID_ASL_FUORI_REGIONE")) || 
				animale.getIdAslRiferimento() == thisUser.getSiteId() || thisUser.getSiteId() == -1 );
	...
	public static String getDbName(ActionContext context) {
		ConnectionPool ce = (ConnectionPool) context.getSession().getServletContext().getAttribute(
		"ConnectionPool");
		if (ce != null) {
			return ce.getDbName();
		} else {
	 ...
	 * @return The dbName value
	 */
	public static String getDbName(ConnectionPool ce) {
		if (ce != null) {
			return ce.getDbName();
		} else {
			return null;
	 ...
	 * @return The path value
	 */
	protected String getPath(ActionContext context, ConnectionPool ce, String moduleFolderName) {
		ApplicationPrefs prefs = (ApplicationPrefs) context.getServletContext().getAttribute(
		"applicationPrefs");
		return (prefs.get("FILELIBRARY") +
	...
	protected void freeConnection(ActionContext context, Connection db) {
		//if (db != null) {
			ConnectionPool sqlDriver = (ConnectionPool) context.getServletContext().getAttribute(
					"ConnectionPool");
			sqlDriver.closeConnection(db, context);
	//	}
		db = null;
	}
	
	protected void freeConnection(ActionContext context, Connection db, PreparedStatement st) {
	//	if (db != null) {
			ConnectionPool sqlDriver = (ConnectionPool) context.getServletContext().getAttribute(
					"ConnectionPool");
			sqlDriver.closeConnection(db, context, st);
//		}
		db = null;
	}
	
	
	protected void freeConnection(ActionContext context, Connection db, PreparedStatement st, ResultSet rs) {
		//if (db != null) {
			ConnectionPool sqlDriver = (ConnectionPool) context.getServletContext().getAttribute(
					"ConnectionPool");
			sqlDriver.closeConnection(db, context, st, rs );
//		}
		db = null;
	}
	 ...
	 * @since 1.1
	 */
	protected void freeConnectionVam(ActionContext context, Connection db) {
		if (db != null) {
			ConnectionPool sqlDriver = (ConnectionPool) context.getServletContext().getAttribute(
					"ConnectionPoolVam");
			sqlDriver.closeConnection(db, context);
...
/*	protected void freeConnectionVam(ActionContext context, Connection db) {
		if (db != null) {
			ConnectionPool sqlDriver = (ConnectionPool) context.getServletContext().getAttribute(
					"ConnectionPool");
			sqlDriver.freeVam(db);
		}
		db = null;
	}*/

	protected void freeConnectionImportatori(ActionContext context, Connection db) {
		//if (db != null) {
			ConnectionPool sqlDriver = (ConnectionPool) context.getServletContext().getAttribute(
					"ConnectionPoolImportatori");
			sqlDriver.closeConnection(db, context);
...
//		"ConnectionElement");
	...
	public int getUserAsl(ActionContext context) {
		return UserUtils.getUserAslRif(context.getRequest());
	}
	public int getUserRole(ActionContext context) {
		return UserUtils.getRole(context.getRequest());
	}
	
	public String getUserName(ActionContext context) {
		return UserUtils.getUserUsername(context.getRequest());
	}


	  public static String getCurrentDataTot(ActionContext context) {
		  String ritData = "";
		  UserBean userBean = (UserBean) context.getSession().getAttribute("User");
		  User user = userBean.getUserRecord();
		  Date calendar = new Date();
		  String pattern = "dd_MM_yyyy_hh:mm:ss";
		  SimpleDateFormat formatter = new  SimpleDateFormat(pattern, user.getLocale()) ;
		  
		  ritData = formatter.format( calendar ).replace(":", "_");
