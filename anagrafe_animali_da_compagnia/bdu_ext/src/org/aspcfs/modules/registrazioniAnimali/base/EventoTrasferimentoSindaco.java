...
public class EventoTrasferimentoSindaco extends Evento {

	public static final int idTipologiaDB = 55;

	public EventoTrasferimentoSindaco() {
		super();
	...
	public boolean insert(Connection db) throws SQLException {
		...
		try {
			...
			id = DatabaseUtils.getNextSeq(db, "evento_trasferimento_sindaco_id_seq");
			// sql.append("INSERT INTO animale (");

			sql.append("INSERT INTO evento_trasferimento_sindaco("
			...
			this.id = DatabaseUtils.getCurrVal(db, "evento_trasferimento_sindaco_id_seq", id);
	...
	public EventoTrasferimentoSindaco(ResultSet rs) throws SQLException {
		buildRecord(rs);
	}
	...
	public EventoTrasferimentoSindaco(Connection db, int idEventoPadre) throws SQLException {

		// super(db, idEventoPadre);

		PreparedStatement pst = db
				.prepareStatement("Select *, e.id_evento as idevento, e.id_asl as idaslinserimentoevento from evento e left join evento_trasferimento_sindaco f on (e.id_evento = f.id_evento) where e.id_evento = ?");
	...
	public EventoTrasferimentoSindaco salvaRegistrazione(int userId, int userRole, int userAsl, Animale thisAnimale,
		...
		try {
			...
			thisAnimale.setIdAslRiferimento(((Stabilimento) soggettoAdded.getListaStabilimenti().get(0)).getIdAsl());
			...
			if (oldAnimale.isFlagCircuitoCommerciale()) {
				...
				LineaProduttiva lp = (LineaProduttiva) stab.getListaLineeProduttive().get(0);
					...
					this.setIdTipologiaEvento(EventoTrasferimentoSindaco.idTipologiaDB);
			} else {
				this.setIdTipologiaEvento(EventoTrasferimentoSindaco.idTipologiaDB);
			}
	...
	public EventoTrasferimentoSindaco build(ResultSet rs) throws Exception {
		try {
