...
import org.aspcfs.modules.microchip.base.MicrochipImport;
 ...
 *
 * @author Mathur
 * @version $Id: AccountContactsImports.java 18141 2006-12-28 21:56:39Z matt $
 * @created June 21, 2004
 */
public final class MicrochipImports extends CFSModule {
  ...
  public String executeCommandView(ActionContext context) {
    ...
    pagedListInfo.setLink("MicrochipImports.do?command=View");
    pagedListInfo.setDefaultListView("my");
    try {
      db = this.getConnection(context);
      
      //LOG PER I VETERINARI PRIVATI
      if ( getUserRole(context) == VETERINARIO_PRIVATO ){
    	  LogBean lb = new LogBean();
    	  lb.store( getUserAsl(context), getUserId(context), 2,"Visualizza Import:View", context, db);
      }
      ...
      thisList.setIdTipologiaImport(Import.TIPOLOGIA_IMPORT_MICROCHIP);
      thisList.buildList(db);

      //update record counts of running imports
      thisList.updateRecordCounts();
      
      if ( getUserRole(context) == VETERINARIO_PRIVATO ) { //VETERINARIO PRIVATO
  ...
  public String executeCommandNew(ActionContext context) {
    ...
    try {
      ...
      String data = getCurrentDataTot(context);
      context.getRequest().setAttribute("dataCorrente", data);
    ...
    if ( getUserRole(context) == VETERINARIO_PRIVATO ) { //RUOLO VETERINARIO
      context.getRequest().setAttribute("veterinario", true);
    }
  ...
  public String executeCommandSave(ActionContext context) {
    ...
    int roleID = getUserRole(context);
    
    SystemStatus systemStatus = this.getSystemStatus(context);
    MicrochipImport thisImport = (MicrochipImport) context.getFormBean();
    try {
      ...
      if ( getUserRole(context) == VETERINARIO_PRIVATO ){
    	  LogBean lb = new LogBean();
    	  lb.store( getUserAsl(context), getUserId(context), 2,"Import MC: Save", context, db);
      }
      
      String filePath = this.getPath(context, "microchips");
     ...
     // thisImport.setSourceType(sourceType);
      ...
      thisImport.setTipologiaImport(org.aspcfs.modules.passaporti.base.PassaportoImport.TIPOLOGIA_IMPORT_MICROCHIP);
    ...
    if (fileRecordInserted && contactRecordInserted && roleID != VETERINARIO_PRIVATO) {
      return getReturn(context, "Save");
    }
    if (fileRecordInserted && contactRecordInserted && roleID == VETERINARIO_PRIVATO) {
        return getReturn(context, "SaveVet");
      }
  ...
  public String executeCommandDetails(ActionContext context) {
    ...
    try {
      ...
      MicrochipImport thisImport = new MicrochipImport(
    ...
    //LOG PER I VETERINARI PRIVATI
      if ( getUserRole(context) == VETERINARIO_PRIVATO ){
    	  LogBean lb = new LogBean();
    	  lb.store( getUserAsl(context), getUserId(context), 2,"Dettaglio Import id:"+importId, context, db);
      }
  ...
  public String executeCommandInitValidate(ActionContext context) {
    ...
    try {
      ...
      PropertyMap thisMap = new PropertyMap(propertyFile, "microchipImport");
      ...
      String filePath = this.getPath(context, "microchips") + getDatePath(
          importFile.getModified()) + importFile.getFilename();

      if (FileUtils.fileExists(filePath)) {
        //Run the validator(only maps fields to properties in this iteration)
        MicrochipImportValidate validator = new MicrochipImportValidate();
  ...
  public String executeCommandProcess(ActionContext context) {
    ...
    try {
      ...
      MicrochipImportValidate validator = (MicrochipImportValidate) context.getRequest().getAttribute(
          "ImportValidator");
      MicrochipImport thisImport = (MicrochipImport) context.getRequest().getAttribute(
          "ImportDetails");
      if (!isRecordAccessPermitted(context, thisImport)){
        return ("PermissionError");
      }
...
//      
      ...
      if ( getUserRole(context) == VETERINARIO_PRIVATO) { //IMPORT DI UN VETERINARIO PRIVATO
    	  codiceFiscale =  ( (UserBean) context.getSession().getAttribute("User") ).getContact().getCodiceFiscale() ;
      }
      
      if (activeImport == null) {
        //run
        thisImport.setSystemStatus(systemStatus);
        thisImport.setPropertyMap(thisMap); //validator.getPropertyMap()
        thisImport.setLookupAccount(true);
        thisImport.setFilePath(validator.getFilePath());
        thisImport.setUserId(this.getUserId(context));
        thisImport.setUserRoleId(this.getUserRole(context));
      //  thisImport.setConnectionElement(ce);
        thisImport.setFileItem(thisItem);
        thisImport.setCodiceFiscale(codiceFiscale);
        thisImport.setHasPermissionCaricamentoUnina(hasPermission(context, "microchip_unina-add"));
        //queue the import
    ...
    try {
      ...
      if (dependencies.size() == 0) {
        ...
        htmlDialog.setDeleteUrl(
            "javascript:window.location.href='MicrochipImports.do?command=Delete&importId=" + importId + "'");
      } else {
        htmlDialog.setHeader(systemStatus.getLabel("confirmdelete.header"));
        htmlDialog.addButton(
            systemStatus.getLabel("button.deleteAll"), "javascript:window.location.href='MicrochipImports.do?command=Delete&importId=" + importId + "'");
        htmlDialog.addButton(
  ...
  public String executeCommandDelete(ActionContext context) {
    ...
    try {
      ...
      if (recordDeleted > 0) {
        ...
        importFile.delete(db, this.getPath(context, "microchips"));
      }
    } catch (Exception e) {
      context.getRequest().setAttribute("Error", e);
      return ("SystemError");
    ...
    context.getRequest().setAttribute(
        "refreshUrl", "MicrochipImports.do?command=View");
    ...
    try {
      ...
      if (version == null) {
        ...
        if (fileDownload.fileExists()) {
          ...
          db = null;
          System.err.println(
              "MicrochipImports-> Trying to send a file that does not exist");
          context.getRequest().setAttribute(
              "actionError", systemStatus.getLabel(
  ...
  public String executeCommandDownloadProduttori(ActionContext context) throws Exception
  {
	if (!(hasPermission(context, "microchip-view"))) {
	      return ("PermissionError");
	}
    ...
    Connection db = null;
    String  query = "select code_prod code,description prod_distr from lookup_produttore_microchips where enabled order by description ";
	...
	try
	{
	        	...
	        	org.aspcfs.utils.Estrazione.generaReportZipPOI(db, query, context, "elencoProdDistr");
  ...
  public String executeCommandViewResults(ActionContext context) {
    ...
    pagedListInfo.setLink(
        "MicrochipImports.do?command=ViewResults&importId=" + importId);
...
*/
   ...
   */
  private void buildValidateFormElements(Connection db, ActionContext context, MicrochipImport thisImport) throws SQLException {
