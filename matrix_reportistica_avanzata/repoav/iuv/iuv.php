...
<head>
	...
	<title>Reportistica avanzata - IUV</title>
	...
	<script src="costruisciQuery.js?v24"></script>
...
<body bg>  
	...
	<header>
		...
		<h1>Reportistica Avanzata - Macroarea IUV</h1>
...
<!--	<button type="button" style="width: 150px; height: 25px;"id="inverti">Inverti righe/colonne</button> -->
	...
	<div class="right-div">
		...
		<button type="button" onClick="exportToExcelDettaglio()">Dettaglio</button>
		<?php 
			if($_SESSION["id_asl"] == -1){
				echo '<button type="button" onclick="exportRendicontazioneCompleta()">Rendicontazione</button>';
				echo '<button type="button" onclick="exportDettaglioVamBdu()">Rendicontazione Bdu/Vam</button>';
			}
		 ?>
...
function exportDettaglioVamBdu() {
	var url = `sheet_vam_bdu.php?di=${dataStart}&df=${dataEnd}`;
	console.log(url);
	window.open(url);
}
...
function exportRendicontazioneCompleta() {
		...
		if(ANNO_GLOB == null)
			y = anno_php;
		d3.json("sheet_rendicontazione_asl.php?estrazione=IUV&anno="+y, function(error, data) {
			if (error)
				throw error;
			console.log(data)
			...
			data.piani.forEach(function(d){
				...
				var filename = "RendicontazioneIUV_"+y+"_"+d2.replace("/", "_")+".xls";
				console.log(q);
...
function exportToExcelDettaglio(){
	...
	window.open('sheet_iuv_dettaglio.php?di=' + dataStart + "&df=" + dataEnd);
}
...
function query() {
	...
	q = costruisciQuery(<?php echo $_SESSION['id_asl']?>);
	//console.log(q);
