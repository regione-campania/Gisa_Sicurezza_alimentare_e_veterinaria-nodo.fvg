function costruisciQuery(idAsl){
	...
	let w_idAsl = 'and a.id_asl > 200';
	if(idAsl != -1)
		w_idAsl = 'and a.id_asl = ' + idAsl;
	...
	var q = 'select replace('+riga+', \'.\', \'\') riga, a.id as id_r,  replace('+colonna+', \'.\', \'\') colonna, p.id as id_c, sum(coalesce(eseguiti,0)) eseguiti, (coalesce(t.sum, 0)) as sum	\n\
	 ...
	 join matrix.struttura_asl a on a.n_livello = 1 and a.anno = '+ANNO_GLOB+' and p.p_id = '+id_c_new+' '+w_idAsl+' \n\
	 join matrix.struttura_piani sp2 on sp2.cod_raggruppamento = p.cod_raggruppamento and sp2.id_gisa in  (	\n\
	 			select id_gisa from "Analisi_dev".vw_struttura_piani_ref_iuv where id_ref  in (select pd.id from matrix.vw_tree_nodes_piani_descr pd where pd.p_id = '+id_c_new+')	\n\
			)	\n\
	 ...
	 left join(	\n\
		 ...
		 ra.mvw_iuv mi 	\n\
		 join matrix.struttura_piani i on i.cod_raggruppamento = mi.cod_raggruppamento and i.anno = '+ANNO_GLOB+'	\n\
