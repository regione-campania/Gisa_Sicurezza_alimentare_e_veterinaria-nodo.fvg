...
<form action="vam.cc.felv.AddEdit.us" name="form" method="post" class="marginezero">    
	  ...
	  <h4 class="titolopagina">
		...
		<c:if test="${modify }" >
			Modifica Esame Felv
    			...
    			<input type="hidden" name="id" value="${felv.id }" />
		</c:if>
    </h4>  
    ...
    <table class="tabella">
    	...
    	<tr>
    		...
    		<td>
    			 <input 
    			 	...
    			 	readonly="readonly" 
    			 	style="width:246px;" 
					<c:if test="${modify }"> value="<fmt:formatDate type="date" value="${felv.dataRichiesta}" pattern="dd/MM/yyyy" />" </c:if>
    		...
    		<td>
    			 <input 
    			 	...
    			 	readonly="readonly" 
    			 	style="width:246px;" 
					value="<fmt:formatDate type="date" value="${felv.dataEsito }" pattern="dd/MM/yyyy" />" />
        		...
        		<select name="esito">	
					<option value="">Selezionare Esito</option>			
           			<option value="Negativo"
           				<c:if test="${felv.esito == 'Negativo' && modify}">
  		          			<c:out value="selected=selected"></c:out> 
    					</c:if>   
           			>Negativo</option>
   					<option value="Positivo"
           				<c:if test="${felv.esito == 'Positivo' && modify}">
  		          			<c:out value="selected=selected"></c:out> 
    					</c:if>   
           			>Positivo</option>
	    		</select> 
        ...
        <tr>
    		<td style="text-align: right;">    
    			...
    			<input type="button" value="Annulla" onclick="location.href='vam.cc.felv.List.us'">
    		</td>
    		<td>&nbsp;</td>
        </tr>
