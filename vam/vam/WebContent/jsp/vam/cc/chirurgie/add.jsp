...
Operazione da registrare: Sterilizzazione
...
<c:if test="${cc.accettazione.animale.lookupSpecie.id==specie.cane}">
	<iframe 
		...
		src="<%=CaninaRemoteUtil.getLoginUrl( (BUtente)session.getAttribute( "utente" ), null, (Animale)request.getAttribute("animale"), (LookupOperazioniAccettazione)request.getAttribute("operazioneSterilizzazione"), ((CartellaClinica)session.getAttribute("cc")).getAccettazione(), null,connection,request,"2" ) %>">
	</iframe>
</c:if>
<c:if test="${cc.accettazione.animale.lookupSpecie.id==specie.gatto}">
	<iframe 
...
<c:if test="${cc.accettazione.animale.lookupSpecie.id==specie.cane}">
	<script type="text/javascript">
		...
		{
			location.href='vam.cc.chirurgie.Add.us?';
		}
...
<c:if test="${cc.accettazione.animale.lookupSpecie.id==specie.sinantropo}">
	<script type="text/javascript">
		...
		{
			location.href='vam.cc.chirurgie.List.us?';
		};
	</script>
</c:if>
