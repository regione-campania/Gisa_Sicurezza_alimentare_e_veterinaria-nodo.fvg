...
<form action="vam.cc.fip.AddEdit.us" name="form" method="post" class="marginezero" >    
	  ...
	  <h4 class="titolopagina">
		...
		<c:if test="${modify }" >
			Modifica Esame Fip 
    			...
    			<input type="hidden" name="id" value="${fip.id }" />
		</c:if>
    </h4>  
    ...
    <table class="tabella">
    	...
    	<tr>
    		...
    		<td>
    			 <input 
    			 	...
    			 	readonly="readonly" 
    			 	style="width:246px;" 
					<c:if test="${modify }"> value="<fmt:formatDate type="date" value="${fip.dataRichiesta}" pattern="dd/MM/yyyy" />" </c:if>
    		...
    		<td>
    			 <input 
    			 	...
    			 	readonly="readonly" 
    			 	style="width:246px;" 
					value="<fmt:formatDate type="date" value="${fip.dataEsito }" pattern="dd/MM/yyyy" />" />
        		...
        		<select name="esito">	
					<option value="">Selezionare Esito</option>			
           			<option value="Negativo"
           				<c:if test="${fip.esito == 'Negativo' && modify}">
  		          			<c:out value="selected=selected"></c:out> 
    					</c:if>   
           			>Negativo</option>
   					<option value="Positivo"
           				<c:if test="${fip.esito == 'Positivo' && modify}">
  		          			<c:out value="selected=selected"></c:out> 
    					</c:if>   
           			>Positivo</option>
	    		</select>
        ...
        <tr>
    		<td style="text-align: right;">    
    			...
    			<input type="button" value="Annulla" onclick="location.href='vam.cc.fip.List.us'">
    		</td>
    		<td>&nbsp;</td>
        </tr>
