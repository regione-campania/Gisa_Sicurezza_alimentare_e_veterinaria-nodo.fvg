...
<h4 class="titolopagina">
     	Lista esami delle urine
</h4>
	
<input type="button" value="Aggiungi un esame" onclick="if(${cc.dataChiusura!=null}){ 
	if(myConfirm('Cartella Clinica chiusa. Vuoi procedere?')){attendere(), location.href='vam.cc.esamiUrine.ToAdd.us'}
	}else{attendere(), location.href='vam.cc.esamiUrine.ToAdd.us'}">

<div class="area-contenuti-2">
	<form action="vam.cc.esamiUrine.List.us" method="post">
		
		<jmesa:tableModel items="${cc.esamiUrine }" id="esamiUrine" var="es" filterMatcherMap="it.us.web.util.jmesa.MyFilterMatcherMap">
			<jmesa:htmlTable styleClass="tabella">
				<jmesa:htmlRow>
					...
					<jmesa:htmlColumn filterable="false" sortable="false"			title="Operazioni">
							<a href="vam.cc.esamiUrine.Detail.us?id=${es.id}">Dettaglio</a>
							<a id="mod" href="vam.cc.esamiUrine.ToEdit.us?id=${es.id}" onclick="if(${cc.dataChiusura!=null}){ 
	...
	<script type="text/javascript">
		...
		function onInvokeExportAction(id)
		{
			...
			location.href = 'vam.cc.esamiUrine.List.us?' + parameterString;
		}
	</script>
</div>