...
Inserimento Registrazioni ${accettazione.animale.lookupSpecie.description }: <b>${accettazione.animale.identificativo }</b>
<br/>
Operazioni da registrare: ${accettazione.operazioniRichiesteBdrNonEseguite } ${accettazione.tipoTrasferimento}  

<div id="continue_buttons" style="visibility: hidden;">
<br/><br/>
<input type="button" value="Inserimento Registrazioni Completato" onclick="javascript:if(registrazioneInserita('${accettazione.animale.identificativo}',${accettazione.animale.lookupSpecie.id},${idTipoRegBdr})){attendere(),logoutFromFelina( inserimentoRegistrazioniCompletato );}" />
</br></br>
...
<iframe 
	...
	src="<%=CaninaRemoteUtil.getLoginUrl( (BUtente)session.getAttribute( "utente" ), null, (Animale)request.getAttribute("animale"), (LookupOperazioniAccettazione)request.getAttribute("operazione"), (Accettazione)request.getAttribute("accettazione"), null,connection,request,"2" ) %>">
</iframe>
...
{
	location.href='vam.bdr.felina.RegistrazioniInterattiveInserite.us?idAccettazione=${accettazione.id }&idTipoRegBdr=${idTipoRegBdr}';
}
</script>
