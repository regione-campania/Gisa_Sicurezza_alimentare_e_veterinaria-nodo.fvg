...
public class Edit extends GenericAction {
	...
	public void execute() throws Exception
	{
		...
		tipoIntervento.setModified(new Date());		
		tipoIntervento.setModifiedBy(utente);
		...
		validaBean( tipoIntervento , new ToEdit() );
		
		try {
			persistence.update(tipoIntervento);	
		...
		catch (RuntimeException e)
		{
			...
			logger.error("Cannot edit Tipo Intervento" + e.getMessage());
			throw e;	
		}
		
		setMessaggio("Tipo Intervento modificato");
	...
	{
		return "Modifica Altro intervento";
	}
}
