...
//		can( gui, "w" );
	...
	public void execute() throws Exception
	{
		...
		EsameCitologico esame = (EsameCitologico) persistence.find(EsameCitologico.class, interoFromRequest("id") );	
		...
		req.setAttribute( "specieAnimale",  esame.getAnimale().getLookupSpecie().getId() );
		
		ServicesStatus status = new ServicesStatus();
		
		HashMap<String, Object> fetchAnimale = AnimaliUtil.fetchAnimale( esame.getAnimale().getIdentificativo(), persistence, utente, status, connectionBdu,req );
		...
		if(!esame.getAnimale().getDecedutoNonAnagrafe())
		{
			...
			{
				RegistrazioniSinantropiResponse rsr = SinantropoUtil.getInfoDecesso(persistence, esame.getAnimale());
		...
		if(esame.getAnimale().getDecedutoNonAnagrafe())
		{
			Iterator<Accettazione> accettazioni = esame.getAnimale().getAccettaziones().iterator();
		...
		else
		{
			...
			else if (specie == Specie.SINANTROPO) 
			{
				...
				Detenzioni detenzioni = SinantropoUtil.getLastActiveDetentore( persistence, esame.getAnimale().getIdentificativo() );
		...
		req.setAttribute("anagraficaAnimale", AnimaliUtil.getAnagrafica(esame.getAnimale(), cane, gatto, persistence, utente, status, connectionBdu,req));
		...
		gotoPage("onlybody", "/jsp/vam/diagnosiCitologica/detailLLPP.jsp" );
