...
public class FindAnimaleLLPP extends GenericAction  implements Specie
{
	...
	public void execute() throws Exception
	{
		...
		AnimaleAnagrafica animaleAnagrafica = AnimaliUtil.getAnagrafica(animale, null, null, persistence, utente, status, connectionBdu,req);
		...
		else if(animale.getDataMorte()!=null || (animaleAnagrafica!=null && animaleAnagrafica.getStatoAttuale()!=null && animaleAnagrafica.getStatoAttuale().toUpperCase().contains("DECE")))
		{
			//String pathVam = Application.get("VAM_PROTOCOLLO") + Application.get("VAM_NOME_HOST") + Application.get("VAM_PORTA") + "/vam";
			ClientServizioCentralizzato sclient = new ClientServizioCentralizzato();
			JSONObject mappaEndPoints = sclient.getMappaEndpointsSca();
			...
			setErrore("Animale con microchip " + identificativo + " risulta deceduto" );
			redirectTo( pathVam + "/vam.altreDiagnosi.ToFindAnimaleLLPP.us",false );	
		}
		else 
		{
			...
			gotoPage( "onlybody","/jsp/vam/altreDiagnosi/addLLPP.jsp");
		}
	}
