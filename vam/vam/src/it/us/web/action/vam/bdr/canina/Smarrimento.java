...
import it.us.web.action.vam.cc.decessi.Add;
...
public class Smarrimento extends GenericAction
{
	...
	public void execute() throws Exception
	{
		...
		final Logger logger = LoggerFactory.getLogger(Smarrimento.class);
		
		logger.info("Esecuzione smarrimento cane in Vam");
		Accettazione				accettazione	= (Accettazione) persistence.find( Accettazione.class, interoFromRequest( "idAccettazione" ) );
		
		LookupOperazioniAccettazione smarr = (LookupOperazioniAccettazione) persistence.find( LookupOperazioniAccettazione.class, IdOperazioniBdr.smarrimento );
		...
		CaninaRemoteUtil.eseguiSmarrimento(
				accettazione.getAnimale(), 
				stringaFromRequest( "dataSmarrimento" ), 
				stringaFromRequest( "luogoSmarrimento" ), 
				stringaFromRequest( "noteSmarrimento" ), 
				booleanoFromRequest( "sanzione" ), 
				stringaFromRequest( "importoSanzione" ), 
				utente,req);
		...
		setMessaggio( "Registrazione di smarrimento inserita con successo in BDR" );
