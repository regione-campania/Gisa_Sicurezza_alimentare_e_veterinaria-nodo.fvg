...
public class AccettazioneDAONoH extends GenericDAO {
	public static final Logger logger = LoggerFactory.getLogger(AccettazioneDAONoH.class);

	public static AccettazioneNoH getAccettazione(int id, Connection conn) {
		PreparedStatement stat = null;
		ResultSet rs = null;
		AccettazioneNoH acc = new AccettazioneNoH();
	...
	private static HashSet<CartellaClinicaNoH> getCartellaClinicas( int idAcc, Connection connection) throws SQLException
	{
		HashSet<CartellaClinicaNoH> ccs = new HashSet<CartellaClinicaNoH>();
	...
	public static HashSet<LookupOperazioniAccettazione> getOperazioniRichieste( int idAcc, Connection connection) throws SQLException
	{
		...
		String sql = " select op.verso_assoc_canili, op.id, op.inbdr, op.description , op.scelta_asl, op.effettuabile_da_morto, "
				...
				+ " op.enabled_in_page, op.id_bdr, op.intra_fuori_asl, op.obbligo_cc "
	...
	public static HashSet<AttivitaBdrNoH> getAttivitaBdrs( int idAcc, Connection connection) throws SQLException
	{
		...
		String sql = " select att.id as idAtt,att.tipo_operazione, op.id as idOp, op.inbdr, att.id_registrazione_bdr "
				...
				+ " where att.accettazione = ? and trashed_date is null";
		PreparedStatement st = connection.prepareStatement(sql);
		st.setInt(1, idAcc);
	...
	private static AnimaleNoH getAnimale(ResultSet rs,Connection conn) throws SQLException 
	{
		AnimaleNoH animale = new AnimaleNoH();
	...
	private static HashSet<AccettazioneNoH> getAccettaziones( int idAnimale, Connection connection) throws SQLException
	{
		HashSet<AccettazioneNoH> pis = new HashSet<AccettazioneNoH>();
	...
	public static ArrayList<AccettazioneNoH> getAccettazioniMultiple( String idAccMultipla, Connection connection) throws SQLException
	{
		ArrayList<AccettazioneNoH> accs = new ArrayList<AccettazioneNoH>();
		...
		while(rs.next())
		{
			AccettazioneNoH acc = AccettazioneDAONoH.getAccettazione(rs.getInt("id"), connection);
			accs.add(acc);
		}
		return accs;
	}
