...
public class ParameterUtils
{
	...
	public static ArrayList<Parameter> list( HttpServletRequest req, String prefisso, boolean idDaNome )
	{
		...
		while(e.hasMoreElements())
		{
			...
			if( nome_parametro.startsWith( prefisso ) )
			{
				...
				try
				{
					...
					if(idDaNome)
					{
						id = p.getNome().replace( prefisso, "" );
					}
					else
					{
						id = p.getValore().replace( prefisso, "" );
					}
