...
if($operation == 'get_ruoli'){
	...
	$result = pg_query($conn, "select * from opzioni.get_lista_qualifiche()");
    $i = 0;
    while ($row = pg_fetch_assoc($result)) {
        $response->qualifiche[$i]['id'] = $row['id'];
        $response->qualifiche[$i]['descr'] = $row['descrizione'];
        $i++;
    }
	
	$result = pg_query($conn, "select * from opzioni.get_lista_profili_professionali()");
    $i = 0;
    while ($row = pg_fetch_assoc($result)) {
        $response->profili_professionali[$i]['id'] = $row['id'];
        $response->profili_professionali[$i]['descr'] = $row['descrizione'];
        $i++;
    }

    $result = pg_query($conn, "select * from spid.spid_tipologia_utente order by ord");
    $i = 0;
    while ($row = pg_fetch_assoc($result)) {
    ...
    $result = pg_query($conn, "select * from spid.spid_tipo_richiesta");
    $i = 0;
    while ($row = pg_fetch_assoc($result)) {
    ...
    $result = pg_query($conn, "select * from dbi_get_ruoli_utente() where id_ruolo in(3373,3368,3367,3369,3370,3366,3365,3375, 3391)");
    $i = 0;
    while ($row = pg_fetch_assoc($result)) {
    ...
    $result = pg_query($conn, "select * from dbi_get_ruoli_utente() 
        where id_ruolo in(96,44,16,221,42,21,45,97,43,19,46,98,33,56,41,59,222)
        or id_ruolo between 3378 and 3390");
    $i = 0;
    while ($row = pg_fetch_assoc($result)) {
    ...
    $conn = getConnection("digemon_u");
    //flusso 340
    $result = pg_query($conn, "select * from dbi_get_ruoli_utente() where id_ruolo in (99);");
    $i = 0;
    while ($row = pg_fetch_assoc($result)) {
        $response->ruoli_digemon[$i]['id'] = $row['id_ruolo'];
        $response->ruoli_digemon[$i]['descr'] = "SI'";
        $i++;
    }

    $result = pg_query($conn, "select * from dbi_get_ruoli_utente() where id_ruolo in (243) order by id_ruolo;");
    $i = 0;
    while ($row = pg_fetch_assoc($result)) {
        $response->ruoli_digemon_asl[$i]['id'] = $row['id_ruolo'];
        $response->ruoli_digemon_asl[$i]['descr'] = $row['descrizione_ruolo'];
        $i++;
    }
    pg_close($conn);
    ...
    $comune = $_POST['comune'];
    $nominativo_referente = str_replace("'", "''", $_POST['nominativo_referente']);
	...
	$profilo_professionale = $_POST['profilo_professionale'];
	$altro_profilo = $_POST['altro_profilo'];
    ...
    while ($row = pg_fetch_assoc($result)) {
		...
		if($qualifica != null && $qualifica > 0 ){	
			$resultQualifica = pg_query("select * from opzioni.insert_richiesta_qualifica('$richiesta',$qualifica);");
			if(!$resultQualifica)
				die("KO inserimento qualifica");
		}
		
		if($profilo_professionale != null && $profilo_professionale > 0 ){
			$resultProfiloProfessionale = pg_query("select * from opzioni.insert_richiesta_profilo_professionale('$richiesta',$profilo_professionale,'$altro_profilo');");
			if(!$resultProfiloProfessionale)
				die("KO inserimento profilo professionale");
		}
