...
public class Sql
{
	...
	public static final String UTENTE_FROM_CODICE_FISCALE = "SELECT * FROM " + 
			TABLE_UTENTI + " WHERE enabled and codice_fiscale = ? ";
			//+ " AND " + TABLE_UTENTI  + ".DATE_DELETE IS NULL " ;
	...
	public static final String PASSWORD_CHECK_ACCESSO_UNICO_SPID =   " SELECT * FROM guc_utenti gu WHERE ( USERNAME = ?  or ?) AND ENABLED = TRUE AND (upper(CODICE_FISCALE) = upper(?) or upper(CODICE_FISCALE) in (select upper(codice_fiscale) from guc_utenti where username = ? and enabled) ) and (? is false or upper(?) in (select upper(codice_fiscale) from guc_cf_white_list)) ";
	...
	public static final String PASSWORD_CHECK_ACCESSO_ADMIN_SPID =   "select * from guc_utenti gu where enabled = true and codice_fiscale in (select codice_fiscale from guc_cf_white_list where codice_fiscale = ? and password = md5(?));";
