...
<dhv:container name="aziendeagricole" selected="documents" object="OrgDetails" hideContainer='<%= "true".equals(request.getParameter("actionplan")) %>' param='<%= "orgId=" + OrgDetails.getOrgId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId|actionplan") %>'>
  <%
    ...
    String documentFolderAdd ="AziendeAgricoleDocumentsFolders.do?command=Add&orgId="+OrgDetails.getOrgId();
    String documentFileAdd = "AziendeAgricoleDocuments.do?command=Add&orgId="+ OrgDetails.getOrgId();
    String documentFolderModify = "AziendeAgricoleDocumentsFolders.do?command=Modify&orgId="+ OrgDetails.getOrgId();
    String documentFolderList = "AziendeAgricoleDocuments.do?command=View&orgId="+OrgDetails.getOrgId()+"&popup=true";
    String documentFileDetails = "AziendeAgricoleDocuments.do?command=Details&orgId="+ OrgDetails.getOrgId();
...
%>    
  <tr>
    <td class="row<%= rowid %>" width="100%">
      ...
      <a href="AziendeAgricoleDocuments.do?command=Download&orgId=<%= OrgDetails.getOrgId() %>&folderId=<%= thisFile.getFolderId() %>&fid=<%= thisFile.getId() %>"><%= toHtml(thisFile.getSubject()) %></a>
    </td>
...
&nbsp;<br/>
<form method="post" name="inputForm" action="AziendeAgricoleDocuments.do?command=Upload&popup=true" onSubmit="return checkFileForm(this);" enctype="multipart/form-data">
