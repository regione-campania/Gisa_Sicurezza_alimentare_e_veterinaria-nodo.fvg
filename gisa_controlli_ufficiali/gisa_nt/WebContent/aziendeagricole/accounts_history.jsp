...
<%@ page import="java.util.*,org.aspcfs.modules.aziendeagricole.base.*,org.aspcfs.modules.contacts.base.*" %>
...
<jsp:useBean id="historyList" class="org.aspcfs.modules.aziendeagricole.base.OrganizationHistoryList" scope="request"/>
...
<SCRIPT LANGUAGE="JavaScript" type="text/javascript">
  ...
  function reopen() {
    window.location.href='AziendeAgricoleHistory.do?command=View&orgId=<%= OrgDetails.getOrgId() + (isPopup(request)?"&popup=true":"") %>';
  }
</script>
...
<dhv:container name="aziendeagricole" selected="history" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId") %>'>
<dhv:evaluate if="<%= OrgDetails.getTrashedDate() == null %>"><dhv:permission name="aziendeagricole-history-add">
<a href=" javascript:popURL('AziendeAgricoleHistory.do?command=AddNote&orgId=<%= OrgDetails.getOrgId() %>','Note','575','200','yes','yes');" ><dhv:label name="accounts.accountHistory.addANote">Add a Note</dhv:label></a><br /><br />
  ...
  <table cellpadding="4" cellspacing="0" width="100%" class="pagedList">
    <tr>
      ...
      <th nowrap>
        <strong><a href="AziendeAgricoleHistory.do?command=View&orgId=<%= OrgDetails.getOrgId() %>&column=c.namelast"><dhv:label name="">Inserito da</dhv:label></a></strong>
        <%= orgHistoryListInfo.getSortIcon("history.enteredby") %>
      </th>
      <th nowrap>
        <strong><a href="AziendeAgricoleHistory.do?command=View&orgId=<%= OrgDetails.getOrgId() %>&column=history.entered"><dhv:label name="accounts.accounts_calls_list.Entered">Entered</dhv:label></a></strong>
        <%= orgHistoryListInfo.getSortIcon("history.entered") %>
      </th>
      <th nowrap>
        <strong><a href="AziendeAgricoleHistory.do?command=View&orgId=<%= OrgDetails.getOrgId() %>&column=c.namelast"><dhv:label name="">Modificato da</dhv:label></a></strong>
        <%= orgHistoryListInfo.getSortIcon("history.enteredby") %>
      </th>
      <th nowrap>
        <strong><a href="AziendeAgricoleHistory.do?command=View&orgId=<%= OrgDetails.getOrgId() %>&column=history.modified"><dhv:label name="accounts.accounts_contacts_calls_details.Modified">Modified</dhv:label></a></strong>
