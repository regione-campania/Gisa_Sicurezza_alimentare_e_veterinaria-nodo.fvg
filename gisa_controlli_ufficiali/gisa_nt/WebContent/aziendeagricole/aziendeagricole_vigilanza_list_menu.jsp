...
<script language="javascript">
  ...
  function modify() {
    window.location.href='AziendeAgricoleVigilanza.do?command=ModifyTicket&orgId=' + thisOrgId + '&id=' + thisTicId + '&return=list<%= addLinkParams(request, "popup|popupType|actionId") %>';
  }

  function deleteTic() {
    popURL('AziendeAgricoleVigilanza.do?command=ConfirmDelete&orgId=' + thisOrgId + '&id=' + thisTicId + '&popup=true<%= isPopup(request) ? "&popupType=inline":"" %>', 'Delete_ticket','320','200','yes','no');
  }
...
<div id="menuTicContainer" class="menu">
  <div id="menuTicContent">
    <table id="menuTicTable" class="pulldown" width="170" cellspacing="0">
      <dhv:permission name="aziendeagricole-vigilanza-view">
