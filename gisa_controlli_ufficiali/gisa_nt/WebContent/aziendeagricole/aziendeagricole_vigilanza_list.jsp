...
<%@ page import="java.util.*,java.text.DateFormat,org.aspcfs.modules.aziendeagricole.base.*,org.aspcfs.modules.vigilanza.base.Ticket,com.zeroio.iteam.base.*" %>
...
<%@ include file="aziendeagricole_vigilanza_list_menu.jsp" %>
...
<a href="AziendeAgricole.do?command=Search"><dhv:label name="">Ricerca Aziende Agricole</dhv:label></a> >
<a href="AziendeAgricole.do?command=Details&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label name="">Dettaglio Aziende Agricole</dhv:label></a> >
...
<dhv:container name="aziendeagricole" selected="vigilanza" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId") %>'>
  <dhv:evaluate if="<%= !OrgDetails.isTrashed() %>">
    ...
    <dhv:permission name="aziendeagricole-vigilanza-add"><a href="AziendeAgricoleVigilanza.do?command=Add&orgId=<%= OrgDetails.getOrgId() %><%= addLinkParams(request, "popup|popupType|actionId") %>"><dhv:label name="accounts.richiesta.add">Aggiungi Nuovo Controllo Ufficiale</dhv:label></a></dhv:permission>
  </dhv:evaluate>
  ...
  %>
   ...
   <td width="10%" valign="top" nowrap>
		...
		<td width="10%" valign="top" nowrap>
			<a href="AziendeAgricoleVigilanza.do?command=TicketDetails&id=<%= thisTic.getId() %>&orgId=<%= thisTic.getOrgId()%><%= addLinkParams(request, "popup|popupType|actionId") %>"><zeroio:tz timestamp="<%= thisTic.getAssignedDate() %>" dateOnly="true" timeZone="<%= thisTic.getAssignedDateTimeZone() %>" showTimeZone="false" default="&nbsp;"/>
     		</a>
		</td>
...
<tr class="row<%= rowid %>">
      <td colspan="7" valign="top">
        ...
        %>
          <a href="AziendeAgricoleVigilanzaDocuments.do?command=Download&stream=true&tId=<%= thisTic.getId() %>&fid=<%= thisFile.getId() %>"><img src="images/file-audio.gif" border="0" align="absbottom"></a>
        <%
              }
            }
          }
