...
function apriVerbale(idCampione,tipoAnalisi)
{
	...
	document.location.href="RiproduzioneAnimaleCampioni.do?command=StampaScheda&tipoAnalisi="+tipoAnalisi+"&idCampione="+idCampione+"&tipoAlimenti="+tipoAlimenti;
		
}
</script>

<%@ include file="../utils23/initPage.jsp" %>
<form id="details" name="details" action="RiproduzioneAnimaleCampioni.do?command=ModifyTicket&auto-populate=true" method="post">
...
<dhv:container name="riproduzioneanimale" selected="vigilanza" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId() %>' hideContainer='<%= isPopup(request) || (defectCheck != null && !"".equals(defectCheck.trim())) %>'>
	...
	else
	{
		...
		<tr>
		<td>
			<a href = "RiproduzioneAnimaleCampioni.do?command=StampaScheda&TipoAnalisi=<%=TicketDetails.getTipoCampione()%>&idCampione=<%=TicketDetails.getId() %>">Stampa PRI-18-Mod-2-Verbale-campione-Viruss</a>
		</td>
	</tr>		
	<% }
	%> 
...
if(request.getAttribute("Messaggio2")!=null)
{
	...
	function doSubmit(idTicket){
		...
		document.details.action="RiproduzioneAnimaleCampioni.do?command=Chiudi&id="+idTicket+"&chiudiCu=true'"
