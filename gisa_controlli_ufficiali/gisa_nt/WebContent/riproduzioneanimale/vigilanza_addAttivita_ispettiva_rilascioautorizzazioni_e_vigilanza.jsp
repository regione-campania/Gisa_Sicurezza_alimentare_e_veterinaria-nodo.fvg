...
var result;
 ...
 <% if (Boolean.parseBoolean((String)request.getAttribute("linee_pregresse"))) {%>
   <script>
   		...
   		if (r == true) {
        openPopupLarge('Accounts.do?command=PrepareUpdateLineePregresse&orgId=<%=OrgDetails.getOrgId() %>');
        window.location.href='RiproduzioneAnimale.do?command=Details&orgId=<%= OrgDetails.getOrgId() %>';
    } else {
...
costruisci_rel_ateco_attivita('<%= OrgDetails.getOrgId() %>', 'id_linea_sottoposta_a_controllo');">
 ...
 <%
    ...
    TipoCampione.setJsEvent("onChange=javascript:reloadAddCU('RiproduzioneAnimaleVigilanza.do?command=Add&orgId="+OrgDetails.getOrgId()+"&tipoCampione='+this.value)"); 


%>


<form method="post" name="addticket" action="RiproduzioneAnimaleVigilanza.do?command=Insert&auto-populate=true"  >
<%-- Trails --%>
<table class="trails" cellspacing="0">
<tr>
<td>
  <a href="RiproduzioneAnimale.do">Strutture di Riproduzione Animale</a> > 
  <a href="RiproduzioneAnimale.do?command=Search"><dhv:label name="">Risultati Ricerca</dhv:label></a> >
...
<input type="submit" value="Annulla" onClick="javascript:this.form.action='RiproduzioneAnimaleVigilanza.do?command=ViewVigilanza&orgId=<%=OrgDetails.getOrgId()%>'">
<br>
...
<br>
<input type="button" value="<dhv:label name="button.inserta">Inserisci </dhv:label>" name="Save" onClick="<%--javascript:this.form.action='RiproduzioneAnimaleVigilanza.do?command=InsertContinua&auto-populate=true';--%>return controlloCuSorveglianza()">
