...
<%@ page import="java.util.*,java.text.DateFormat,org.aspcfs.modules.riproduzioneanimale.base.*,com.zeroio.iteam.base.*" %>
...
<dhv:container name="riproduzioneanimale" selected="documents" object="OrgDetails" hideContainer='<%= "true".equals(request.getParameter("actionplan")) %>' param='<%= "orgId=" + OrgDetails.getOrgId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId|actionplan") %>'>
  <%
    String permission_doc_folders_add ="riproduzioneanimale-riproduzioneanimale-documents-add";
    String permission_doc_files_upload = "riproduzioneanimale-riproduzioneanimale-documents-add";
    String permission_doc_folders_edit = "riproduzioneanimale-riproduzioneanimale-documents-edit";
    String documentFolderAdd ="RiproduzioneAnimaleDocumentsFolders.do?command=Add&orgId="+OrgDetails.getOrgId();
    String documentFileAdd = "RiproduzioneAnimaleDocuments.do?command=Add&orgId="+ OrgDetails.getOrgId();
    String documentFolderModify = "RiproduzioneAnimaleDocumentsFolders.do?command=Modify&orgId="+ OrgDetails.getOrgId();
    String documentFolderList = "RiproduzioneAnimaleDocuments.do?command=View&orgId="+OrgDetails.getOrgId()+"&popup=true";
    String documentFileDetails = "RiproduzioneAnimaleDocuments.do?command=Details&orgId="+ OrgDetails.getOrgId();
...
%>    
  <tr>
    <td class="row<%= rowid %>" width="100%">
      ...
      <a href="RiproduzioneAnimaleDocuments.do?command=Download&orgId=<%= OrgDetails.getOrgId() %>&folderId=<%= thisFile.getFolderId() %>&fid=<%= thisFile.getId() %>"><%= toHtml(thisFile.getSubject()) %></a>
    </td>
...
&nbsp;<br/>
<form method="post" name="inputForm" action="RiproduzioneAnimaleDocuments.do?command=Upload&popup=true" onSubmit="return checkFileForm(this);" enctype="multipart/form-data">
