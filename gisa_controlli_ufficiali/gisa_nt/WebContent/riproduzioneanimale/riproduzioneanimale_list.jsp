...
<%@ page import="java.util.*,org.aspcfs.modules.riproduzioneanimale.base.*, org.aspcfs.modules.base.*" %>
...
<%@ include file="riproduzioneanimale_list_menu.jsp" %>
...
<table cellpadding="8" cellspacing="0" border="0" width="100%" class="pagedList">
  <tr>
    ...
    <th nowrap <% ++columnCount; %>>
      <dhv:label name="">N.Autorizzazione</dhv:label>
    </th>
    <th nowrap <% ++columnCount; %>>
...
%>
  <tr class="row<%= rowid %>">
	<td>
      <a href="RiproduzioneAnimale.do?command=Details&orgId=<%=thisOrg.getOrgId()%>"><%= toHtml(thisOrg.getName()) %></a>
	</td>
    <td>
	...
	<% Iterator iaddress = thisOrg.getAddressList().iterator();
       ...
       <td>
          ...
          while (iaddress.hasNext()) {
        	  ...
        	  org.aspcfs.modules.riproduzioneanimale.base.OrganizationAddress thisAddress = (org.aspcfs.modules.riproduzioneanimale.base.OrganizationAddress)iaddress.next(); 
...
<%} else {%>
  <tr class="containerBody">
    <td colspan="<%= SearchOrgListInfo.getSearchOptionValue("searchcodeOrgSiteId").equals(String.valueOf(Constants.INVALID_SITE))?columnCount+1:columnCount %>">
      <dhv:label name="">Nessuna Struttura di Riproduzione Animale trovato.</dhv:label><br />
      <a href="RiproduzioneAnimale.do?command=SearchForm"><dhv:label name="">Modifica Ricerca</dhv:label></a>.
    </td>
  </tr>
<%}%>
</table>
