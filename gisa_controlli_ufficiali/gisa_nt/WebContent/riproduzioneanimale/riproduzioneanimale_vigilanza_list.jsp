...
<%@ page import="java.util.*,java.text.DateFormat,org.aspcfs.modules.riproduzioneanimale.base.*,org.aspcfs.modules.vigilanza.base.Ticket,com.zeroio.iteam.base.*" %>
...
<%@ include file="riproduzioneanimale_vigilanza_list_menu.jsp" %>
...
<tr>
<td>
<a href="RiproduzioneAnimale.do"><dhv:label name="">Strutture di Riproduzione Animale</dhv:label></a> > 
<a href="RiproduzioneAnimale.do?command=Details&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label name="">Dettaglio Strutture di Riproduzione Animale</dhv:label></a> >
...
<dhv:container name="riproduzioneanimale" selected="vigilanza" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId") %>'>
  <dhv:evaluate if="<%= !OrgDetails.isTrashed() %>">
    ...
    <dhv:permission name="riproduzioneanimale-vigilanza-add"><a href="RiproduzioneAnimaleVigilanza.do?command=Add&orgId=<%= OrgDetails.getOrgId() %><%= addLinkParams(request, "popup|popupType|actionId") %>"><dhv:label name="accounts.richiesta.add">Aggiungi Nuovo Controllo Ufficiale</dhv:label></a></dhv:permission>
  </dhv:evaluate>
