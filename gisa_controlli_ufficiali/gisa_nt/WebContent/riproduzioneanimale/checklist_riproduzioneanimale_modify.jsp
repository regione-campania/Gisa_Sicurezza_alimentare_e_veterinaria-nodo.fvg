...
<form name="addAccountAudit"  method="post" action="CheckListRiproduzioneAnimale.do?command=Update&auto-populate=true&return<%= request.getParameter("return") %>" onSubmit="return checkForm();">
<%-- Trails --%>
...
<td width="100%">
  ...
  <a href="RiproduzioneAnimaleVigilanza.do?command=ViewVigilanza&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label name="">Controlli Ufficiali</dhv:label></a> >
      <a href="RiproduzioneAnimaleVigilanza.do?command=TicketDetails&id=<%= request.getAttribute("idCon")%>&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label name="">Controllo Ufficiale</dhv:label></a> >
  <a href="CheckListRiproduzioneAnimale.do?command=View&id=<%= Audit.getId() %>"><dhv:label name="accounts.Audit">Check List</dhv:label></a> >
...
<dhv:container name="riproduzioneanimale" selected="audit" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId() %>'>


  <input type="button" value="Annulla" onClick="window.location.href='CheckListRiproduzioneAnimale.do?command=View&id=<%=Audit.getId()%>';this.form.dosubmit.value='false';" />
