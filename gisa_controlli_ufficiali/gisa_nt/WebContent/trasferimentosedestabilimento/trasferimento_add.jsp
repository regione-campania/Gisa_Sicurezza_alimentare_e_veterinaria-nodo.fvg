...
function checkForm(form){
	...
	var vecchi = document.getElementById("vecchi");
	...
	if (vecchi.checked && idComune != <%=StabilimentoDettaglio.getSedeOperativa().getComune() %> ){
		msg+="\nCOMUNE modificato. Impossibile conservare attuali numeri di registrazione.";
		esito=false;
	}
	...
	if (esito==false){
		...
		msg = "Dati inseriti per la nuova sede operativa: \n\nASL: "+asl+"\t\tCOMUNE: "+comune+"\t\tINDIRIZZO: "+toponimo+" "+via+" "+civico+"\t\tCAP: "+cap+"\nLAT: "+lat+"\t\tLON: "+lon;
		
		msg+="\n\nATTENZIONE : lo Stabilimento con Numero Registrazione <%=StabilimentoDettaglio.getNumero_registrazione()%> sarà cessato.";
...
<center>
<font size="3px">Operazione di trasferimento sede operativa per lo stabilimento:<br/>
...
<form id="cambioIndirizzo" name="cambioIndirizzo" action="TrasferimentoSedeStabilimento.do?command=Insert&auto-populate=true" method="post">
...
<th></th>
<th>DATI ATTUALI</th>
<th>DATI NUOVA SEDE OPERATIVA</th>
</tr>
...
<% if (AslList.size()==1) { %><br/><font color="red">Per i trasferimenti fuori ASL <br/>e' necessario rivolgersi all'Help Desk</font> <%} %>
</td>
</tr>
...
<tr><th>INDIRIZZO</th>
<td><%=StabilimentoDettaglio.getSedeOperativa().getDescrizioneToponimo() %> <%=StabilimentoDettaglio.getSedeOperativa().getVia() %> <%=StabilimentoDettaglio.getSedeOperativa().getCivico() %> </td>
<td><%=ToponimiList.getHtmlSelect("toponimo", 100)%> <input type="text" id="via" name="via"/> <input type="text" id="civico" name="civico" placeholder="CIVICO"/></td>
</tr>

<tr><th>CAP</th>
<td><%=StabilimentoDettaglio.getSedeOperativa().getCap() %> </td>
...
<tr><th>COORDINATE</th>
<td><%=StabilimentoDettaglio.getSedeOperativa().getLatitudine() %> <%=StabilimentoDettaglio.getSedeOperativa().getLongitudine() %> </td>
...
<tr style="display:none"><th>NUMERI DI REGISTRAZIONE</th>
<td colspan="2"> <input type="radio" name="numeri" id="nuovi" value="nuovi" checked/> Genera nuovi <input type="radio" name="numeri" id="vecchi" value="vecchi" onClick="return false"/> Conserva attuali </td>
</tr>

<tr><td colspan="3" align="center">
<input type="button" value="CONFERMA" onClick="checkForm(this.form)"/>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<input type="button" value="Annulla" onClick="loadModalWindow();window.location.href='OpuStab.do?command=Details&stabId=<%=StabilimentoDettaglio.getIdStabilimento() %>'; return false;"/>
