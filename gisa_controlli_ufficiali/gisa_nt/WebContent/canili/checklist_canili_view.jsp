...
<form name="viewAccountAudit" action="CheckListCanili.do?command=Modify&auto-populate=true&id=<%=Audit.getId()%>&return=details<%= addLinkParams(request, "popup|popupType|actionId") %>" method="post">
    ...
    <tr>
       <td nowrap align="right">
        ...
        <%--<a href="Accounts.do?command=PrintReport&id=<%= OrgDetails.getId() >"><dhv:label name="accounts.osa.print">Stampa Scheda Audit</dhv:label></a>--%>
         ...
         <input type="button" value="Stampa Scheda Check List Provvisorio"	onClick='javascript:location.href="CheckListCanili.do?command=Stampa&id=<%= Audit.getId() %>";'>
      <%
    }
    else {
    %>
        <input type="button" value="Stampa Scheda Check List"	onClick='javascript:location.href= "CheckListCanili.do?command=Stampa&id=<%= Audit.getId() %>";'>
    <%	
    }
    --%>
