<%-- 
  ...
  - 
  - Author(s): 
  - Version: $Id: document_store_center_file_details.jsp 12404 2005-08-05 17:37:07Z mrajkowski $
  - Description: 
  --%>
...
<table border="0" cellpadding="1" cellspacing="0" width="100%">
  <tr class="subtab">
    <td>
      <% String documentLink = "DocumentManagementAsl.do?command=DocumentStoreCenter&section=File_Library&documentStoreId="+documentStore.getId(); %>
...
%>
  <tr class="row<%= rowid %>">
    <td rowspan="2" valign="middle" nowrap>
        ...
        <a href="DocumentStoreManagementFilesAsl.do?command=Download&documentStoreId=<%= documentStore.getId() %>&fid=<%= FileItem.getId() %>&ver=<%= thisVersion.getVersion() %>"><dhv:label name="button.download">Download</dhv:label></a><br />
        <img src="images/icons/stock_zoom-page-16.gif" border="0" align="absmiddle" height="16" width="16"/>
        <a href="javascript:popURL('DocumentStoreManagementFilesAsl.do?command=Download&documentStoreId=<%= documentStore.getId() %>&fid=<%= FileItem.getId() %>&ver=<%= thisVersion.getVersion() %>&view=true', 'Content', 640,480, 1, 1);"><dhv:label name="accounts.accounts_documents_list_menu.ViewFileContents">View File Contents</dhv:label></a><br />
      
      <dhv:evaluate if="<%= !documentStore.isTrashed() %>" >
      <dhv:evaluate if="<%= documentStore.getSiteId()==user.getSiteId() %>" >
      
        <dhv:permission name="documentcenter-documents-files-asl-delete-delete">
          <img src="images/icons/stock_delete-16.gif" border="0" align="absmiddle" height="16" width="16"/>
          <a href="javascript:confirmDelete('DocumentStoreManagementFilesAsl.do?command=Delete&documentStoreId=<%= documentStore.getId() %>&fid=<%= FileItem.getId() %>&ver=<%= thisVersion.getVersion() %>&folderId=<%= FileItem.getFolderId() %>');"><dhv:label name="project.deleteVersion">Delete Version</dhv:label></a><br />
        </dhv:permission>
              </dhv:evaluate>
      </dhv:evaluate>
      &nbsp;
