...
<form name="details" action="ImbarcazioniProvvedimenti.do?command=ModifyTicket&auto-populate=true" method="post">
...
<td>
<td>
  <a href="Imbarcazioni.do">Operatori Abusivi</a> > 
  <a href="Imbarcazioni.do?command=Search"><dhv:label name="accounts.SearchResults">Search Results</dhv:label></a> >
  <a href="Imbarcazioni.do?command=Details&orgId=<%=OrgDetails.getOrgId()%>">Scheda Imbarcazione</a> >
  <a href="Imbarcazioni.do?command=ViewVigilanza&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label name="">Controlli Ufficiali</dhv:label></a> >
...
<dhv:container name="imbarcazioni" selected="vigilanza" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId() %>' >
	...
	%>
		...
		<input type="button"
			value="Ripristina"
			onClick="javascript:this.form.action='ImbarcazioniProvvedimenti.do?command=Restore&id=<%= TicketDetails.getId()%>';submit();">
	...
	<dhv:permission name="reopen-reopen-view">
		<input type="button"
			...
			onClick="javascript:this.form.action='ImbarcazioniProvvedimenti.do?command=ReopenTicket&id=<%= TicketDetails.getId()%><%= (defectCheck != null && !"".equals(defectCheck.trim())?"&defectCheck="+defectCheck:"") %>';submit();">
	</dhv:permission>
	<%
		} else {
	%>
	<dhv:permission name="imbarcazioni-imbarcazioni-campioni-edit">
		<input type="button"
			...
			onClick="javascript:this.form.action='ImbarcazioniProvvedimenti.do?command=ModifyTicket&auto-populate=true<%= (defectCheck != null && !"".equals(defectCheck.trim())?"&defectCheck="+defectCheck:"") %>';submit();">
	</dhv:permission>
	...
	<dhv:permission name="imbarcazioni-imbarcazioni-campioni-edit">
		<input type="button"
			...
			onClick="javascript:this.form.action='ImbarcazioniProvvedimenti.do?command=Chiudi&id=<%= TicketDetails.getId() %>';if( confirm('Sei sicuro di voler chiudere il campione? \n Attenzione! La pratica verrà chiusa e non sarà più possibile fare modifiche \n sulla scheda se non con il permesso del supervisore o dell amministratore') ){submit()};">		
	</dhv:permission>
...
if(request.getAttribute("Messaggio2")!=null)
{
	...
	function doSubmit(idTicket){
		...
		document.details.action="ImbarcazioniProvvedimenti.do?command=Chiudi&id="+idTicket+"&chiudiCu=true'"
