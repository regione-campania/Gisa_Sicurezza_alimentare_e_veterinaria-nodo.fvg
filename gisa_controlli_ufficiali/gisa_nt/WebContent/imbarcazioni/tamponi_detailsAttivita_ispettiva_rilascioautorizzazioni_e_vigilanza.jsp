...
<form name="details" action="ImbarcazioniTamponi.do?command=ModifyTicket&auto-populate=true" method="post">
...
<%@ include file="ticket_header_include_tamponi.jsp"%>
	...
	%>
	<dhv:permission name="imbarcazioni-imbarcazioni-tamponi-delete">
		<input type="button"
			value="Ripristina"
			onClick="javascript:this.form.action='ImbarcazioniTamponi.do?command=Restore&id=<%= TicketDetails.getId()%>';submit();">
	</dhv:permission>
	<%
	...
	<dhv:permission name="reopen-reopen-view">
		<input type="button"
			...
			onClick="javascript:this.form.action='ImbarcazioniTamponi.do?command=ReopenTicket&id=<%= TicketDetails.getId()%><%= (defectCheck != null && !"".equals(defectCheck.trim())?"&defectCheck="+defectCheck:"") %>';submit();">
	</dhv:permission>
	<%
		} else {
	%>
	<dhv:permission name="imbarcazioni-imbarcazioni-tamponi-edit">
		<input type="button"
			...
			onClick="javascript:this.form.action='ImbarcazioniTamponi.do?command=ModifyTicket&auto-populate=true<%= (defectCheck != null && !"".equals(defectCheck.trim())?"&defectCheck="+defectCheck:"") %>';submit();">
	</dhv:permission>
	<%--
	...
	<dhv:permission name="imbarcazioni-imbarcazioni-tamponi-delete">
		...
		<input type="button"
			...
			onClick="javascript:popURL('ImbarcazioniTamponi.do?command=ConfirmDelete&id=<%= TicketDetails.getId() %>&orgId=<%=OrgDetails.getOrgId() %>&return=searchResults&popup=true', 'Delete_ticket','320','200','yes','no');">
		<%
			} else {
		%>
		<input type="button"
			value="<dhv:label name="global.button.delete">Delete</dhv:label>"
			onClick="javascript:popURL('ImbarcazioniTamponi.do?command=ConfirmDelete&id=<%= TicketDetails.getId() %>&orgId=<%=OrgDetails.getOrgId() %>&popup=true', 'Delete_ticket','320','200','yes','no');">
		<%
			}
		%>
	</dhv:permission>
...
if(request.getAttribute("Messaggio2")!=null)
{
	...
	function doSubmit(idTicket){
		...
		document.details.action="ImbarcazioniTamponi.do?command=Chiudi&id="+idTicket+"&chiudiCu=true'"
