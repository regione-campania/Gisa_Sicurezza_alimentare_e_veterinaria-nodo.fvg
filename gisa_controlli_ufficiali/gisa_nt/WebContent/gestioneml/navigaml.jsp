...
<div ng-controller="AttCtrl">
		...
		<td>Norma</td>	
		<td>
 		<select ng-model="selectedNorma"  style="width: 70%" name = "idNorma"
 		ng-change="changedValueMacroarea(selectedNorma); changedValueAggregazione(-1); changedValueAttivita(-1);" 
  		ng-options="item.norma for item in itemsNorma track by item.id">
  		<option value="-1" >Seleziona Norma</option>
		</select>  
		</td>
		</tr>
		<tr><td><br/></td></tr>
		...
		<td>Macroarea</td>	
		<td>
 		<select ng-model="selectedMacroarea"  style="width: 70%" name = "idMacroarea"
 		ng-change="changedValueAggregazione(selectedMacroarea); changedValueAttivita(-1);" 
		...
		<td>Aggregazione</td>	
		<td>
		<select ng-model="selectedAggregazione"  style="width: 70%" name = "idAggregazione"
		...
		<td> 
		<select ng-model="selectedAttivita" style="width: 70%" name = "idLineaProduttiva" required="required"
...
<!--   		<tr><td colspan="2"> -->
<!-- 		<b>{{selectedNorma.norma}}  - {{selectedMacroarea.macroarea}}  - {{selectedAggregazione.aggregazione}} - {{selectedAttivita.lineaAttivita}}</b> -->
<!-- 		</td></tr> -->
...
angular.module('attivita', []).controller('AggCtrl', function ($scope, $http) {
    ...
    $http({
        ...
        url: 'GestioneMasterList.do?command=CostruisciMasterList&rev=<%=rev%>&flagFisso=<%=flagFisso %>&flagMobile=<%=flagMobile %>&flagApicoltura=<%=flagApicoltura %>&flagRegistrabili=<%=flagRegistrabili %>&flagRiconoscibili=<%=flagRiconoscibili %>&flagSintesis=<%=flagSintesis %>&flagBdu=<%=flagBdu %>&flagVam=<%=flagVam %>&flagNoScia=<%=flagNoScia %>&tipologia=aggregazione&idMacroarea='+item.id,
...
angular.module('attivita', []).controller('AttCtrl', function ($scope, $http) {
    ...
    $http({
        ...
        data: JSON.stringify(data),
        url: 'GestioneMasterList.do?command=CostruisciMasterList&rev=<%=rev%>&flagFisso=<%=flagFisso %>&flagMobile=<%=flagMobile %>&flagApicoltura=<%=flagApicoltura %>&flagRegistrabili=<%=flagRegistrabili %>&flagRiconoscibili=<%=flagRiconoscibili %>&flagSintesis=<%=flagSintesis %>&flagBdu=<%=flagBdu %>&flagVam=<%=flagVam %>&flagNoScia=<%=flagNoScia %>&tipologia=norma',
    }).success(function(data, status, headers, config) {
        $scope.itemsNorma = data;
    }).error(function(data, status, headers, config) {
        alert( "failure");
    });
    

    $scope.changedValueMacroarea = function(item) {
    	 ...
    	 $http({
    	        ...
    	        data: JSON.stringify(data),
    	        url: 'GestioneMasterList.do?command=CostruisciMasterList&rev=<%=rev%>&flagFisso=<%=flagFisso %>&flagMobile=<%=flagMobile %>&flagApicoltura=<%=flagApicoltura %>&flagRegistrabili=<%=flagRegistrabili %>&flagRiconoscibili=<%=flagRiconoscibili %>&flagSintesis=<%=flagSintesis %>&flagBdu=<%=flagBdu %>&flagVam=<%=flagVam %>&flagNoScia=<%=flagNoScia %>&tipologia=macroarea&idNorma='+item.id,
    	    }).success(function(data, status, headers, config) {
    ...
    $scope.resetValueAggregazione = function(item) {
      	 $http({
      	        method: 'POST',
      	        datatype:"json",
      	        header: {
      	        ...
      	        data: JSON.stringify(data),
      	        url: 'GestioneMasterList.do?command=CostruisciMasterList&rev=<%=rev%>&flagFisso=<%=flagFisso %>&flagMobile=<%=flagMobile %>&flagApicoltura=<%=flagApicoltura %>&flagRegistrabili=<%=flagRegistrabili %>&flagRiconoscibili=<%=flagRiconoscibili %>&flagSintesis=<%=flagSintesis %>&flagBdu=<%=flagBdu %>&flagVam=<%=flagVam %>&flagNoScia=<%=flagNoScia %>&tipologia=aggregazione&idMacroarea='+'',
      	    }).success(function(data, status, headers, config) {
      	        $scope.itemsAggregazione= data;
      	    }).error(function(data, status, headers, config) {
      	        alert( "failure");
