...
<%@page import="org.aspcfs.modules.trasportoanimali.base.Organization"%>
...
<form name="viewAccountAudit" action="CheckListTrasporto.do?command=Modify&auto-populate=true&id=<%=Audit.getId()%>&return=details<%= addLinkParams(request, "popup|popupType|actionId") %>" method="post">
...
<tr>
<td width="100%">
  <a href="TrasportoAnimali.do">Trasporto</a> >
  <a href="TrasportoAnimali.do?command=Search"><dhv:label name="accounts.SearchResults">Search Results</dhv:label></a> >
  <a href="TrasportoAnimali.do?command=Details&orgId=<%= OrgDetails.getOrgId() %>">Scheda Trasporto</a> >
...
<%--<dhv:permission name="trasportoanimali-report-view">--%>
    ...
    <tr>
       <td nowrap align="right">
        ...
        <%--<a href="Accounts.do?command=PrintReport&id=<%= OrgDetails.getId() >"><dhv:label name="accounts.osa.print">Stampa Scheda Audit</dhv:label></a>--%>
         ...
         <input type="button" value="Stampa Scheda Check List Provvisorio"	onClick='javascript:location.href="CheckListTrasporto.do?command=Stampa&id=<%= Audit.getId() %>";'>
      <%
    }
    else {
    %>
        <input type="button" value="Stampa Scheda Check List"	onClick='javascript:location.href= "CheckListTrasporto.do?command=Stampa&id=<%= Audit.getId() %>";'>
    <%	
    }
    --%>
...
<dhv:evaluate if="<%= !OrgDetails.isTrashed() %>">
  <input type="hidden" name="idC" value="<%=Audit.getIdControllo() %>">
     ...
     <dhv:permission name="trasporti-trasporti-audit-edit"><input type="submit" value="Modifica"></dhv:permission>
