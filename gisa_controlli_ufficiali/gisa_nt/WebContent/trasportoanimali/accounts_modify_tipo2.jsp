...
function checkSubmit(form){
  ...
  function checkForm(form) {
		...
		if (form.address2latitude.value){
     	...
     	if (form.address3latitude != null && form.address3latitude.value) {
        	 //alert(!isNaN(form.address2latitude.value));
        		if ((orgSelected == 1)  ){
        			/*if ( isNaN(form.address2latitude.value)||(form.address2latitude.value < 4431788.049190) || (form.address3latitude.value > 4593983.337630)){
         			 message += "- Valore errato per il campo Latitudine, il valore deve essere compresotra  4431788.049190 e 4593983.337630  (Sede Operativa)\r\n";
     	 ...
     	 if (form.address3longitude != null && form.address3longitude.value) {
        	 //alert(!isNaN(form.address2longitude.value));
        		if ((orgSelected == 1)  ) {
        			/*if (isNaN(form.address2longitude.value)||(form.address2longitude.value < 2417159.584320) || (form.address3longitude.value > 2587487.362260)){
         			 message += "- Valore errato per il campo Longitudine, il valore deve essere compreso tra 2417159.584320 e 2587487.362260  (Sede Operativa)\r\n";
	    ...
	    if (form.specieA.value == "-1" || form.specieA.value == ""){
	       ...
	       if (checkNullString(document.getElementById("citta").value)){
		...
		<!-- Data richiesta autorizzazione -->
    		...
    		<tr class="containerBody">
      			...
      			<td>
      			 ...
      			 <input class="date_picker" type="text" id="date1" name="date1" size="10" 	value="<%= toDateasString(OrgDetails.getDate1()) %>"/>
  ...
  <tr class="containerBody" style="display: none">
    ...
    <td>
      ...
      %>
		...
		value="<%= toDateasString(OrgDetails.getDataNascitaRappresentante()) %>"/>
...
<table cellpadding="4" cellspacing="0" border="0" width="100%" class="details">
  <tr>
   ...
   <tr class="containerBody">
    ...
    if((thisAddress.getType()==5)){%>
            <input type="text" name="address2city" id="citta" value="<%=thisAddress.getCity() %>"><font color= 'red'>*</font>
       <%--}else if((thisAddress.getType()==7)){ 
       <input type="text" size="28" id="citta" name="address<%= acount %>city" maxlength="80" value="<%=thisAddress.getCity() %>">
    <%}else if(thisAddress.getType()==5){%>
     <input type="text" name="address2city" id="" value="<%=thisAddress.getCity() %>"><font color= "red">*</font>--%>
  ...
  <tr class="containerBody">
    ...
    if((thisAddress.getType()==5)){%>
       ...
       <%--}else if((thisAddress.getType()==7)){ %>
					<input type="text" size="28" name="address3state" maxlength="80" value="<%= toHtmlValue(thisAddress.getState()) %>">
    <%}else if(thisAddress.getType()==5){ %>
               <input type="text" size="10" name="address2state" maxlength="80" value="<%=OrgDetails.getState()%>"> <font color= "red">*</font>
    <%}--%>
    <%}else{ %>
