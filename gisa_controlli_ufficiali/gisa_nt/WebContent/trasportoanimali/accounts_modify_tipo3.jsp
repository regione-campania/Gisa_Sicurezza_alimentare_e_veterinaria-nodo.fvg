...
value="<%= (OrgDetails.getDate1()==null)?(""):(toDateasString(OrgDetails.getDate1()))%>"/><font color="red">*</font>
    ...
    <td>
      ...
      <dhv:label name="stabilimenti.stabilimenti_add.Individual">Individual</dhv:label>
  			...
  			<tr class="containerBody">
    			...
    			<td>
      				<input type="text" size="50" maxlength="50" name="accountNumber" value="<%= toHtmlValue( OrgDetails.getAccountNumber()) %>"><font color="red">*</font>
    			</td>
  			</tr>
			     ...
			     <td>
			      <%= SpecieA.getHtmlSelect("specieA",multipleSelects6)%><font color="red">*</font> <%= showAttribute(request, "nameError") %>
			     	<div id="des" <%=((OrgDetails.getCodice10()!=null && !OrgDetails.getCodice10().equals(""))?("style=\"display:block\""):("style=\"display:none\""))%>><br><b>Descrizione:</b>
    					<br><textarea rows="8" cols="40" id="descrizione" name="codice10"><%= toHtmlValue(OrgDetails.getCodice10()) %></textarea></div>
...
<table cellpadding="4" cellspacing="0" border="0" width="100%" class="details">
  ...
  <tr class="containerBody">
    ...
    <%--if(thisAddress.getType()==5){ %>
                <input type="text" size="40" name="address2line1" maxlength="80" value="<%= toHtmlValue(thisAddress.getStreetAddressLine1()) %>"> <dhv:evaluate if="<%= thisAddress.getType() != 7 && thisAddress.getType() != 1%>"><font color= "red">*</font></dhv:evaluate>
      <%}else{ --%>
            <input type="text" size="40" id="address<%= acount %>line1" name="address<%= acount %>line1" maxlength="80" value="<%= toHtmlValue(thisAddress.getStreetAddressLine1()) %>"> <dhv:evaluate if="<%= thisAddress.getType() != 7 && thisAddress.getType() != 1%>"><font color= "red">*</font></dhv:evaluate>
      <%--} --%>
    </td>
  </tr>
  <dhv:evaluate if="<%=thisAddress.getType() == 1 %>">
  ...
  <tr class="containerBody">
    ...
    <td>   
		 <input type="text" size="28" name="address<%= acount %>state" id="address<%= acount %>state"  maxlength="80" value="<%=toHtmlValue(thisAddress.getState())%>">
    </td>
  </tr>
  ...
  <tr class="containerBody">
    ...
    <td><input type="text" name="address<%= acount %>latitude" id="address<%= acount %>latitude" size="30" value="<%= ((thisAddress.getLatitude() != 0.0 || thisAddress.getLongitude() != 0.0) ? String.valueOf(thisAddress.getLatitude()) : "") %>"> <dhv:evaluate if="<%= thisAddress.getType() != 6 %>"></dhv:evaluate></td>
  </tr>
  <tr class="containerBody">
    <td class="formLabel" nowrap><dhv:label name="accounts.address.longitude">Longitude</dhv:label></td>
    <td><input type="text" name="address<%= acount %>longitude"  id="address<%= acount %>longitude" size="30" value="<%= ((thisAddress.getLatitude() != 0.0 || thisAddress.getLongitude() != 0.0) ? String.valueOf(thisAddress.getLongitude()) : "") %>"> <dhv:evaluate if="<%= thisAddress.getType() != 6 %>"></dhv:evaluate></td>
  </tr>
