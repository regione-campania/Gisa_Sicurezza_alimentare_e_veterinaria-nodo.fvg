...
<form name="details" action="TrasportiVigilanza.do?command=ModifyTicket&auto-populate=true" method="post">
<table class="trails" cellspacing="0">
<tr>
<td>
  <a href="TrasportoAnimali.do">Trasporto Animali</a> > 
  <a href="TrasportoAnimali.do?command=Search"><dhv:label name="accounts.SearchResults">Search Results</dhv:label></a> >
  <a href="TrasportoAnimali.do?command=Details&orgId=<%=TicketDetails.getOrgId()%>"><dhv:label name="">Dettaglio Trasporto</dhv:label></a> >
  <a href="TrasportoAnimali.do?command=ViewVigilanza&orgId=<%=TicketDetails.getOrgId()%>"><dhv:label name="vigilanza">Tickets</dhv:label></a> >
...
%>
<%-- 
<dhv:permission name="trasporto-generate-barcode-view">
...
--%>
<dhv:container name="trasportoanimali" selected="vigilanza" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId() %>' hideContainer='<%= isPopup(request) || (defectCheck != null && !"".equals(defectCheck.trim())) %>'>
<dhv:container name="trasportovigilanza" selected="details" object="TicketDetails"
	param="<%= param1 %>" hideContainer='<%= isPopup(request) || (defectCheck != null && !"".equals(defectCheck.trim())) %>'>
...
<table cellpadding="4" cellspacing="0" width="100%" class="empty">
  ...
  <tr>
	...
	<table cellpadding="4" cellspacing="0" width="100%" class="details">
		...
		<%@ include file="../controlliufficiali/controlli_ufficiali_view.jsp" %>
			<dhv:evaluate if="<%= (TicketDetails.getCategoriaTrasportata() > -1 || TicketDetails.getCategoriaTrasportata() == 0 ) %>">
			    <tr class="containerBody">
			      <td nowrap class="formLabel">
			        <dhv:label name="">Categoria Trasportata</dhv:label>
			      </td>
			      <td>
			     	<%= CategoriaTrasportata.getSelectedValue(TicketDetails.getCategoriaTrasportata()) %>
			       </td>
			    </tr>
			</dhv:evaluate>
	</table>
   ...
   <tr class="row<%=rowid%>">
     ...
     <td>
	   ...
	   <a href="CheckListTrasporto.do?command=View&id=<%= thisAudit.getId()%>&aggiorna=<%=thisAudit.isAggiornaCategoria() %>"><%= toHtml(OrgCategoriaRischioList2.getSelectedValue(thisAudit.getTipoChecklist())) %></a>
	</td>
  ...
  <%}else{%>
   <tr class="containerBody">
      <td colspan="5">
        <dhv:label name="">Nessuna Check List compilata.</dhv:label>
      </td>
   </tr>
   <%}%> 
