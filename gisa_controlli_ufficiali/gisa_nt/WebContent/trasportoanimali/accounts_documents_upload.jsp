...
<%@ page import="java.util.*,org.aspcfs.modules.trasportoanimali.base.*,com.zeroio.iteam.base.*,org.aspcfs.modules.base.Constants" %>
...
<form method="post" name="inputForm" action="TrasportoAnimaliDocuments.do?command=Upload<%= addLinkParams(request, "popup|popupType|actionId|actionplan") %>" enctype="multipart/form-data" onSubmit="return checkFileForm(this);">
...
<a href="TrasportoAnimaliDocuments.do?command=View&orgId=<%= OrgDetails.getOrgId() %>"><dhv:label name="trasportoanimali.trasportoanimali_documents_details.Documents">Documenti</dhv:label></a> >
<dhv:label name="trasportoanimali.trasportoanimali_documents_upload.UploadDocument">Upload Document</dhv:label>
</td>
</tr>
</table>
<%-- End Trails --%>
</dhv:evaluate>
<dhv:container name="trasportoanimali" selected="documents" object="OrgDetails" hideContainer='<%= "true".equals(request.getParameter("actionplan")) %>' param='<%= "orgId=" + OrgDetails.getOrgId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId|actionplan") %>'>
<table border="0" cellpadding="4" cellspacing="0" width="100%">
  <tr class="subtab">
    <td>
      <% String documentLink = "TrasportoAnimaliDocuments.do?command=View&orgId="+OrgDetails.getOrgId()+ addLinkParams(request, "popup|popupType|actionId|actionplan"); %>
  ...
  <input type="submit" value="Annulla" onClick="javascript:this.form.dosubmit.value='false';this.form.action='TrasportoAnimaliDocuments.do?command=View&orgId=<%= OrgDetails.getOrgId() %>&folderId=<%= (String)request.getAttribute("folderId") %><%= addLinkParams(request, "popup|popupType|actionId|actionplan") %>';" />
</dhv:container>
</form>
</body>
