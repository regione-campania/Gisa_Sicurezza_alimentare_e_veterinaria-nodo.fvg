...
<%@ page import="java.util.*,org.aspcfs.modules.trasportoanimali.base.*,org.aspcfs.modules.contacts.base.*" %>
...
<jsp:useBean id="historyList" class="org.aspcfs.modules.trasportoanimali.base.OrganizationHistoryList" scope="request"/>
...
<SCRIPT LANGUAGE="JavaScript" type="text/javascript">
  ...
  function reopen() {
    window.location.href='TrasportoAnimaliContactsHistory.do?command=View&contactId = <%= ContactDetails.getId() %>';
  }
</script>
<%-- Trails --%>
<table class="trails" cellspacing="0">
 <tr><td>
  <a href="TrasportoAnimali.do"><dhv:label name="accounts.accounts">Accounts</dhv:label></a> > 
  <a href="TrasportoAnimali.do?command=Search"><dhv:label name="accounts.SearchResults">Search Results</dhv:label></a> >
  <a href="TrasportoAnimali.do?command=Details&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label name="accounts.details">Account Details</dhv:label></a> >
  <a href="ContactsReq.do?command=View&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label name="accounts.Contacts">Contacts</dhv:label></a> >
  <a href="ContactsReq.do?command=Details&id=<%=ContactDetails.getId()%>&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label name="accounts.accounts_contacts_add.ContactDetails">Contact Details</dhv:label></a> >
  <dhv:label name="reports.helpdesk.ticket.history.history">History</dhv:label>
 </td></tr>
</table>
<%-- End Trails --%>
<dhv:container name="trasportoanimali" selected="history" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId") %>'>
  <dhv:container name="trasportoanimalicontacts" selected="history" object="ContactDetails" param='<%= "id=" + ContactDetails.getId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId") %>'>
<form name="history" action="TrasportoAnimaliContactsHistory.do?command=View&contactId=<%= ContactDetails.getId() %>" method="post" onSubmit="return checkForm(this);">
<input type="hidden" name="searchcodeContactId" value="<%= ContactDetails.getId() %>"/>
  <table cellpadding="4" cellspacing="0" width="100%" class="empty">
    <tr>
      <td>
        <a href=" javascript:popURL('TrasportoAnimaliContactsHistory.do?command=AddNote&contactId=<%= ContactDetails.getId() %>','Note','575','200','yes','yes');" ><dhv:label name="accounts.accountHistory.addANote">Add a Note</dhv:label></a>
      </td>
      <td>
  ...
  <table cellpadding="4" cellspacing="0" width="100%" class="pagedList">
    <tr>
      ...
      <th nowrap>
        <strong><a href="TrasportoAnimaliContactsHistory.do?command=View&contactId = <%= ContactDetails.getId() %>&column=modified"><dhv:label name="admin.dateTime">Date/Time</dhv:label></a></strong>
        <%= accountContactHistoryListInfo.getSortIcon("modified") %>
      </th>
      <th nowrap>
        <strong><a href="TrasportoAnimaliContactsHistory.do?command=View&contactId = <%= ContactDetails.getId() %>&column=contact_id"><dhv:label name="accounts.accountasset_include.Contact">Contact</dhv:label></a></strong>
        <%= accountContactHistoryListInfo.getSortIcon("contact_id") %>
      <th nowrap>
        <strong><a href="TrasportoAnimaliContactsHistory.do?command=View&contactId = <%= ContactDetails.getId() %>&column=type"><dhv:label name="reports.accounts.type">Type</dhv:label></a></strong>
