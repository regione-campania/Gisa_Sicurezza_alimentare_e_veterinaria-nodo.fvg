...
<body onload="verificaChiusuraNc(<%=request.getAttribute("Chiudi")%>,<%=request.getAttribute("numSottoAttivita")%>,'<%=request.getAttribute("Messaggio2")%>',<%=TicketDetails.getIdControlloUfficiale() %>,'TrasportiVigilanza')">

<form name="details" action="TrasportiNonConformita.do?command=ModifyTicket&auto-populate=true" method="post">
<%-- Trails --%>
...
<td>
  ...
  <%--a href="TrasportoAnimali.do?command=ViewNonConformita&orgId=<%=TicketDetails.getOrgId()%>"><dhv:label name="nonconformita">Tickets</dhv:label></a> --%>
...
<dhv:container name="trasportoanimalinonconformita" selected="details" object="TicketDetails"
	param="<%= param1 %>"
