...
<form name="details" action="AccountProvvedimenti.do?command=ModifyTicket&auto-populate=true" method="post">
...
<td>
<td>
  <a href="Accounts.do">Operatori</a> > 
  <a href="Accounts.do?command=Search"><dhv:label name="accounts.SearchResults">Search Results</dhv:label></a> >
  <a href="Accounts.do?command=Details&orgId=<%=OrgDetails.getOrgId()%>">Scheda Operatore Abusivo</a> >
  <a href="Accounts.do?command=ViewVigilanza&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label name="">Controlli Ufficiali</dhv:label></a> >
  <a href="AccountVigilanza.do?command=TicketDetails&id=<%= TicketDetails.getIdControlloUfficiale()%>&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label name="">Controllo Ufficiale</dhv:label></a> >
...
<dhv:container name="accounts" selected="vigilanza" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId() %>' 
	...
	%>
		...
		<input type="button"
			value="Ripristina"
			onClick="javascript:this.form.action='AccountProvvedimenti.do?command=Restore&id=<%= TicketDetails.getId()%>';submit();">
	...
	<dhv:permission name="reopen-reopen-view">
		<input type="button"
			...
			onClick="javascript:this.form.action='AccountProvvedimenti.do?command=ReopenTicket&id=<%= TicketDetails.getId()%><%= (defectCheck != null && !"".equals(defectCheck.trim())?"&defectCheck="+defectCheck:"") %>';submit();">
	</dhv:permission>
	<%
		} else {
	%>
		...
		<input type="button"
			...
			onClick="javascript:this.form.action='AccountProvvedimenti.do?command=ModifyTicket&auto-populate=true<%= (defectCheck != null && !"".equals(defectCheck.trim())?"&defectCheck="+defectCheck:"") %>';submit();">
...
if(request.getAttribute("Messaggio2")!=null)
{
	...
	function doSubmit(idTicket){
		...
		document.details.action="AccountProvvedimenti.do?command=Chiudi&id="+idTicket+"&chiudiCu=true'"
