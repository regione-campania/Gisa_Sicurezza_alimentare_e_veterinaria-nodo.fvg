...
<form name="details" action="AccountSequestri.do?command=UpdateTicket&auto-populate=true<%= addLinkParams(request, "popup|popupType|actionId") %>" onSubmit="return checkForm(this); " method="post">
...
<%} %>  <% if (request.getParameter("return") == null) {%>
  <a href="AccountSequestri.do?command=TicketDetails&id=<%=TicketDetails.getId()%>"><dhv:label name="sequestri.dettagli">Scheda Sequestro/Blocco</dhv:label></a> >
  <%}%>
...
<dhv:container name="accounts" selected="sequestri" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId") %>'>
  <dhv:container name="accountssequestri" selected="details" object="TicketDetails" param='<%= "id=" + TicketDetails.getId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId") %>'>
