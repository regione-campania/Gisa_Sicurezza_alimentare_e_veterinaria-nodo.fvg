	      	 ...
	      	 //alert(!isNaN(form.address3latitude.value));
	      		if ((orgSelected == 1)  ){
	      			if (isNaN(form.address3latitude.value) ||  (form.address3latitude.value < 39.988475) || (form.address3latitude.value > 41.503754)){
	       			 message += "- Valore errato per il campo Latitudine, il valore deve essere compreso tra 39.988475 e 41.503754 (Attività mobile)\r\n";
	       				 formTest = false;
	       			}		 
	      		}
	   	 }   
	     	 ...
	     	 //alert(!isNaN(form.address2longitude.value));
	     		if ((orgSelected == 1)  ){
	     			if (isNaN(form.address3longitude.value) ||  (form.address3longitude.value < 13.7563172) || (form.address3longitude.value > 15.8032837)){
	      			 message += "- Valore errato per il campo Longitudine, il valore deve essere compreso tra 13.7563172 e 15.8032837 (Attività mobile)\r\n";
	      				 formTest = false;
	      			}		 
	     		}
	  	 }   
...
<body onLoad="javascript:initializeClassification();caricaCodici();abilitaDistributoriCampi('<%=OrgDetails.getTipoDest() %>');abilita_codici_ateco_vuoti();nascondi_div_se_in_modifica_solo_linea_attivita('<%= request.getParameter("definisciLdA") %>'); inizializzaProvenienzaEstera('<%=OrgDetails.getIdNazione()%>')">
<form name="addAccount" action="Accounts.do?command=Update&orgId=<%= OrgDetails.getOrgId() %>&auto-populate=true<%= (request.getParameter("popup") != null?"&popup=true":"") %>" onsubmit="return verificaEsistenzaImpresa()" method="post">
<%
  boolean popUp = false;
...
<tr>
<td>
<a href="Accounts.do"><dhv:label name="accounts.accounts">Stabilimenti 852</dhv:label></a> > 
<% if (request.getParameter("return") != null) {%>
	<% if (request.getParameter("return").equals("list")) {%>
	<a href="Accounts.do?command=Search"><dhv:label name="accounts.SearchResults">Search Results</dhv:label></a> >
	<%} else if (request.getParameter("return").equals("dashboard")) {%>
	<a href="Accounts.do?command=Dashboard">Cruscotto</a> >
	<%}%>
<%} else {%>
<a href="Accounts.do?command=Search"><dhv:label name="accounts.SearchResults">Search Results</dhv:label></a> >
...
<% if (request.getParameter("return") != null && "list".equals(request.getParameter("return"))) {%>
  <input type="submit" value="Annulla"  onClick="window.location.href='Accounts.do'" />
<% } else if (isPopup(request)) { %>
  <input type="button" value="Annulla" onclick="javascript:window.close();" />
<% } else { %>
  <input type="submit" value="Annulla"  onClick="window.location.href='Accounts.do?command=Details&orgId=<%= OrgDetails.getOrgId() %>'" />
<% } %>
<br />
<dhv:formMessage />
	...
	<table cellpadding="4" cellspacing="0" border="0" width="100%" class="details">
	  ...
	  <% if (request.getParameter("definisciLdA") != null) 
		  ...
		  <tr class="containerBody">
			  ...
			  <td>
			   <input type="hidden" id="id_attivita_masterlist" name="id_attivita_masterlist" value="<%=((linea_attivita_principale != null)? toHtmlValue( linea_attivita_principale.getId_attivita_masterlist() ):"-1") %>">
...
<tr>
    ...
    <td>
      <input type="checkbox" id="no_piva" name="no_piva" onclick="javascript:gestisciPIVA(this)"/>Partita IVA non obbligatoria
    </td>
  </tr>
   ...
   <tr class="containerBody">
      ...
      <dhv:label name="">Vendita con canali non convenzionali</dhv:label>
			...
			<td>
         <input type = "checkbox" name="flag_vendita" <% if(OrgDetails.isFlagVenditaCanali()==true){ %> checked="checked"<%} %> />
			</td>
		</tr>
 <% if (request.getParameter("definisciLdA") == null) {%>
...
<%-- 	  <input style="background-color: lightgray" readonly="readonly" type="text" size="20" id="codiceFiscaleCorrentista" name="codiceFiscaleCorrentista" value="<%= toHtmlValue(OrgDetails.getCodiceFiscaleCorrentista()) %>"><font color="red">*</font>	--%>
		<input type="hidden" id="id_attivita_masterlist" name="id_attivita_masterlist" value="<%= (linea_attivita_principale != null )? linea_attivita_principale.getId_attivita_masterlist() : ""%>">
	  <input style="background-color: lightgray" readonly="readonly" type="text" size="20" id="codiceFiscaleCorrentista" name="codiceFiscaleCorrentista" value="<%=(linea_attivita_principale!=null) ? toHtmlValue( linea_attivita_principale.getCodice_istat() ) : "" %>" onchange="costruisci_rel_ateco_attivita('codiceFiscaleCorrentista', 'id_rel_principale' );"   ><font color="red">*</font>
	  &nbsp;[<a href="javascript:popLookupSelectorCustomImprese('codiceFiscaleCorrentista','alertText','id_attivita_masterlist','attivita_852_ateco_masterlist','');"><dhv:label name="accounts.accounts_add.select">Seleziona</dhv:label></a>]
...
<input readonly type="text" id="dateI" name="dateI" size="10" value="<%= ((OrgDetails.getDateI()!=null)?(toHtml(DateUtils.getDateAsString(OrgDetails.getDateI(),Locale.ITALY))):("")) %>"/>
		<a href="#" onClick="cal19.select(document.forms[0].dateI,'anchor19','dd/MM/yyyy'); return false;" NAME="anchor19" ID="anchor19">
		<img src="images/icons/stock_form-date-field-16.gif" border="0" align="absmiddle"/></a>				          	</td>
        ...
        <input readonly type="text" id="dateF" name="dateF" size="10" value="<%= ((OrgDetails.getDateF()!=null)?(toHtml(DateUtils.getDateAsString(OrgDetails.getDateF(),Locale.ITALY))):("")) %>"/>
		<a href="#" onClick="cal19.select(document.forms[0].dateF,'anchor19','dd/MM/yyyy'); return false;" NAME="anchor19" ID="anchor19">
		<img src="images/icons/stock_form-date-field-16.gif" border="0" align="absmiddle"/></a>				           	</td>
  ...
  <dhv:include name="organization.date1" none="true">
    <tr class="containerBody">
      ...
      <%} %>
		...
		value="<%= (OrgDetails.getDataPresentazione()==null)?(""):(getLongDate(OrgDetails.getDataPresentazione()))%>"/>
 ...
 <tr>
  ...
  {
	  %>
	  <input type = "hidden" name = "stageId" value = "<%= OrgDetails.getStageId()%>">
	  <%
  }
  %>  
  <tr class="containerBody">
...
<table cellpadding="4" cellspacing="0" border="0" width="100%" class="details">
  ...
  <tr class="containerBody" id = "locale1_indirizzo">
    ...
    <td>
         ...
         <input type="text" size="40" name="address4line" maxlength="80" value="">
 <font id = "address_loc" style="display: none" color = "red">*</font>
  ...
  <tr class="containerBody" id = "locale1_city">
    ...
    <td>
            <input type="text" size="28" required="required" id="address4city<%=i %>" name="address4city<%=i %>" maxlength="80" value="<%=address.getCity() %>">
  ...
  <tr class="containerBody" id = "locale1_indirizzo">
    ...
    <td>
 		...
 		<%if (i==1)
 			{%>
         <input type="text" size="40" required="required" name="address4line1<%=i %>" maxlength="80" value="<%= toHtmlValue(address.getStreetAddressLine1()) %>">
 		<%} else
 		{%>
  ...
  <tr class="containerBody" id = "locale1_zip">
    ...
    <td>
      <input type="text" size="10" required="required" name="address4zip<%=i %>" maxlength="12" value="<%=address.getZip() %>">
    </td>
  </tr>
  <tr class="containerBody" id = "locale1_prov">
    <td nowrap class="formLabel">
    ...
    <td>
       <input type="text" size="25" name="address4state<%=i %>" required="required"   value="<%=toHtml(address.getState()) %>">
    </td>
  </tr>
...
<%@ include file="/utils23/bean_modification.jsp"%>
