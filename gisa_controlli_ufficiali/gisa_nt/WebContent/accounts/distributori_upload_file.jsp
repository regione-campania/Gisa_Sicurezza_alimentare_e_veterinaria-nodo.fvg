...
<form method="post" name="inputForm" action="Accounts.do?command=UploadDoc" enctype="multipart/form-data" onSubmit="return checkFileForm(this);">
<dhv:evaluate if="<%= !isPopup(request) %>">
<%-- Trails --%>
<table class="trails" cellspacing="0">
<tr>
<td width="100%">
  <a href="Accounts.do"><dhv:label name="a">Distributori Automatici</dhv:label></a> >
...
<br>
  <a href="Accounts.do?command=AllImportRecords&orgid=<%=orgID %>" > Visualizza Tutti gli Import </a>
  <br><br>
  
   (Nota : Scaricare il Template del file da Importare da <a href = "templatedistributori.xls" > Qui </a>)
    <br>
    <%if (request.getAttribute("NoFile")!=null) {%>
    <font color = "red">Attenzione: il file non è valido. Scaricare il file giusto dalla Bacheca!</font>
    <%} %>
  <br>
  ...
  <input type="button" value="Annulla" onClick="location.href='Accounts.do?command=Details&orgId=<%=orgID %>'" /><br />
  <dhv:formMessage />
  <br />
</form>
</body>