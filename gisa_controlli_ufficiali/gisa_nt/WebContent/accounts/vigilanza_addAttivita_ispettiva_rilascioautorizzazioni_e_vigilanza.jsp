...
<% if ((!"1".equals(tipoTrasporto)) && !Boolean.parseBoolean((String)request.getAttribute("is_farmacia")) && Boolean.parseBoolean((String)request.getAttribute("linee_pregresse"))) {%>
<script>
alert("ATTENZIONE! Per aggiungere un controllo ufficiale è necessario importare prima lo stabilimento nella NUOVA ANAGRAFICA.\nRivolgersi ad HELP DESK.");		
// if (r == true) { 
<%-- 	window.location.href='OpuStab.do?command=CaricaImport&orgId=<%= OrgDetails.getOrgId() %>'; --%>
// }  
// else
	window.location.href='Accounts.do?command=Details&orgId=<%= OrgDetails.getOrgId() %>';
...
<%} %>
 ...
 <% if (Boolean.parseBoolean((String)request.getAttribute("linee_pregresse")) && !Boolean.parseBoolean((String)request.getAttribute("is_farmacia"))) {%>
   <script>
 ...
 <% } %>
<body onload = "provaFunzione('addticket'); gestioneVisibilitaCodiceAteco('addticket');
onloadAllerta('addticket'); resetElementiNucleoIspettivo('<%=TicketDetails.getNucleoasList().size() %>'); 
costruisci_rel_ateco_attivita('<%= OrgDetails.getOrgId() %>', 'id_linea_sottoposta_a_controllo'); ">
 ...
 <%
    ...
    TipoCampione.setJsEvent("onChange=javascript:reloadAddCU('AccountVigilanza.do?command=Add&orgId="+OrgDetails.getOrgId()+"&tipoCampione='+this.value)"); 
...
<form method="post" name="addticket" action="AccountVigilanza.do?command=Insert&auto-populate=true">
<%-- Trails --%>
...
<%
	if(!popUp){
%>
<input type="submit" value="Annulla" onClick="javascript:this.form.action='Accounts.do?command=ViewVigilanza&orgId=<%=OrgDetails.getOrgId()%>'">
<% } else { %>
