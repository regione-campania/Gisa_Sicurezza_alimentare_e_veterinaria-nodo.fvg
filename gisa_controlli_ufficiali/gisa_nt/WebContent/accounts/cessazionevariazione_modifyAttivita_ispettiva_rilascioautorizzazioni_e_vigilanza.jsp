...
<%@page import="java.util.Date"%><jsp:useBean id="OrgDetails" class="org.aspcfs.modules.accounts.base.Organization" scope="request"/>
...
function updateSubList1() {
  ...
  if(orgId != '-1'){
    ...
    var url = "TroubleTicketsCessazionevariazione.do?command=CategoryJSList&form=details&catCode=" + escape(value)+'&orgId='+orgId;
...
function updateSubList2() {
  ...
  var value = sel.options[sel.selectedIndex].value;
  var url = "TroubleTicketsCessazionevariazione.do?command=CategoryJSList&form=details&subCat1=" + escape(value)+'&orgId='+orgId;
  window.frames['server_commands'].location.href=url;
}
...
function updateSubList3() {
  ...
  var value = sel.options[sel.selectedIndex].value;
  var url = "TroubleTicketsCessazionevariazione.do?command=CategoryJSList&form=details&subCat2=" + escape(value)+'&orgId='+orgId;
  window.frames['server_commands'].location.href=url;
}
</dhv:include>
<dhv:include name="ticket.subCat3" none="true">
  function updateSubList4() {
    ...
    var value = sel.options[sel.selectedIndex].value;
    var url = "TroubleTicketsCessazionevariazione.do?command=CategoryJSList&form=details&subCat3=" + escape(value)+'&orgId='+orgId;
    window.frames['server_commands'].location.href=url;
  }
</dhv:include>
function updateUserList() {
  ...
  var url = "TroubleTicketsCessazionevariazione.do?command=DepartmentJSList&form=details&dept=Assigned&orgSiteId="+ orgSite +"&populateResourceAssigned=true&resourceAssignedDepartmentCode=" + escape(value);
  window.frames['server_commands'].location.href=url;
}
function updateResolvedByUserList() {
  ...
  var value = sel.options[sel.selectedIndex].value;
  var orgSite = document.forms['details'].elements['orgSiteId'].value;
  var url = "TroubleTicketsCessazionevariazione.do?command=DepartmentJSList&form=details&dept=Resolved&orgSiteId="+ orgSite + "&populateResolvedBy=true&resolvedByDepartmentCode=" + escape(value);
  window.frames['server_commands'].location.href=url;
}
...
<body>
<form name="details" action="AccountCessazionevariazione.do?command=UpdateTicket&auto-populate=true<%= addLinkParams(request, "popup|popupType|actionId") %>" onSubmit="return checkForm(this);" method="post">
...
<td>
  ...
  <a href="AccountCessazionevariazione.do?command=TicketDetails&id=<%=TicketDetails.getId()%>"><dhv:label name="cessazionevariazione.dettagli">Scheda Cessazione</dhv:label></a> >
  <%}%>
  <dhv:label name="cessazionevariazione.modify">Modifica Cessazione</dhv:label>
</td>
</tr>
</table>
<%-- End Trails --%>
...
<dhv:container name="accounts" selected="cessazionevariazione" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId") %>'>
  <dhv:container name="accountscessazionevariazione" selected="details" object="TicketDetails" param='<%= "id=" + TicketDetails.getId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId") %>'>
      ...
      <dhv:evaluate if="<%= TicketDetails.getClosed() != null %>" >
          <dhv:permission name="accounts-accounts-cessazionevariazione-edit">
           ...
           <dhv:evaluate if='<%= "list".equals(request.getParameter("return"))%>' >
            <input type="submit" value="Annulla" onClick="javascript:this.form.action='Accounts.do?command=ViewCessazionevariazione&orgId=<%=OrgDetails.getOrgId()%><%= addLinkParams(request, "popup|popupType|actionId") %>'" />
           </dhv:evaluate>
           <dhv:evaluate if='<%= !"list".equals(request.getParameter("return"))%>' >
            <input type="submit" value="Annulla" onClick="javascript:this.form.action='AccountCessazionevariazione.do?command=TicketDetails&id=<%= TicketDetails.getId() %><%= addLinkParams(request, "popup|popupType|actionId") %>'" />
           </dhv:evaluate>
      </dhv:evaluate>
      <dhv:evaluate if="<%= TicketDetails.getClosed() == null %>" >
          <dhv:permission name="accounts-accounts-cessazionevariazione-edit">
	...
	<tr>
      <th colspan="2">
        <strong><dhv:label name="Campioni.information">Informazioni Cessazione</dhv:label></strong>
      </th>
	</tr>
...
<%--</dhv:evaluate>  --%>
 ...
 <tr class="containerBody">
      ...
      <td>
      	...
      	String data = sdf.format(new Date(TicketDetails.getAssignedDate().getTime()) );
      	 %>
        <input readonly="readonly" type="text" id="assignedDate" name="assignedDate" value = "<%=data %>" size="10" />
