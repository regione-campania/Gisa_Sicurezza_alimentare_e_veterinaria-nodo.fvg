...
<script language="javascript">
  ...
  //Menu link functions
  function details() {
    window.location.href='AccountCessazionevariazione.do?command=TicketDetails&id=' + thisTicId+'<%= addLinkParams(request, "popup|popupType|actionId") %>';
  }

  function modify() {
    window.location.href='AccountCessazionevariazione.do?command=ModifyTicket&id=' + thisTicId + '&return=list<%= addLinkParams(request, "popup|popupType|actionId") %>';
  }

  function deleteTic() {
    popURL('AccountCessazionevariazione.do?command=ConfirmDelete&orgId=' + thisOrgId + '&id=' + thisTicId + '&popup=true<%= isPopup(request) ? "&popupType=inline":"" %>', 'Delete_ticket','320','200','yes','no');
  }
...
<div id="menuTicContainer" class="menu">
  <div id="menuTicContent">
    <table id="menuTicTable" class="pulldown" width="170" cellspacing="0">
      <dhv:permission name="accounts-accounts-cessazionevariazione-view">
