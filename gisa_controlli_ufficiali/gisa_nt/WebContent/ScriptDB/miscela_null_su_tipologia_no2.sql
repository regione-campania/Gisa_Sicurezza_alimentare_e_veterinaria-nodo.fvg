...
ALTER TABLE public.ricerca_anagrafiche
  ...
  alter table organization drop column miscela;

  alter table organization add column miscela boolean;
...
ALTER TABLE public.organization DISABLE TRIGGER miscela;



update organization set miscela = true where tipologia = 2;
update organization set miscela = false where org_id in (select org_id from storico_miscela where valore_precedente);
update organization set miscela = true where org_id in (select org_id from storico_miscela where valore_precedente is false);
...
ALTER TABLE public.organization ENABLE TRIGGER miscela;


drop TRIGGER miscela on organization;
drop FUNCTION trigger_miscela();



-- Function: public_functions.import_acquacoltura(text, text, text, text, text, text, integer, text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, text)

-- DROP FUNCTION public_functions.import_acquacoltura(text, text, text, text, text, text, integer, text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, text);
...
INSERT INTO organization(
                        org_id,                   "name",  account_number, entered, enteredby, modified, modifiedby, enabled,    site_id, codice_fiscale, date1,                                         tipologia,    specie_allev,             orientamento_prod, tipologia_strutt, id_allevamento, codice_fiscale_rappresentante, cf_correntista, specie_allevamento,miscela   )
    VALUES (       outputOrgId,       inputdenominazione, inputcodazienda,   now(),  idUtente,    now(),   idUtente,    TRUE, inputIdAsl,        inputcf, inputDataInizio::timestamp without time zone ,         2, inputDescSpecie,     inputTipologiaProduttiva ,     inputTipoAll,        allevId,                       inputcf,    inputCfResp, inputSpecie::integer, true ) ;
