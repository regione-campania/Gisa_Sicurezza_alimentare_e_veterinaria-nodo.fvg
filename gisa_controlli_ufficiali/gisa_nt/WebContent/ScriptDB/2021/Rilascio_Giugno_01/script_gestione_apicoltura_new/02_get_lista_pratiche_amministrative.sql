-- Function: public.get_lista_pratiche_amministrative(integer, integer, text, text, text)

DROP FUNCTION public.get_lista_pratiche_amministrative(integer, integer, text, text, text);

CREATE OR REPLACE FUNCTION public.get_lista_pratiche_amministrative(
...
select distinct
       ...
       coalesce(case oo.partita_iva when '' then null else trim(oo.partita_iva) end,trim(oo.codice_fiscale_impresa))::text as partita_iva,
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION public.get_lista_pratiche_amministrative(integer, integer, text, text, text)
  OWNER TO postgres;
