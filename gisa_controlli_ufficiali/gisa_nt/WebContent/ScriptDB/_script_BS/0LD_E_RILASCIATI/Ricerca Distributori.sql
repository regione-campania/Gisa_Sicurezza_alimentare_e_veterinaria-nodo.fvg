-- CHI: Bartolo Sansone	
-- COSA: Uniformo ricerca distributori
-- QUANDO: 14/12/2017


--NOTA: Bisogna capire come mettere nome
-- 1. Aggiungere "targa" in coda a tutte le dbi di inserimento in ricerche
-- 2. Aggiungere "targa" in coda a tutte le parti di ricerca_anagrafiche
-- 1 e 2 Bis: controllare come si prende la targa nell'inserimento di OPU, duplicare per la matricola e imitare in ricerca anagrafiche
-- 3: cancellare e ripopolare la tabella delle ricerche 

-- View: public.opu_stabilimento_mobile_distributori

-- DROP VIEW public.opu_stabilimento_mobile_distributori;

CREATE OR REPLACE VIEW public.opu_stabilimento_mobile_distributori AS 
 SELECT split_part(lista.indice, '_'::text, 1) AS indice,
    split_part(lista.indice, '_'::text, 2)::integer AS id_rel_stab_linea,
    lista.matricola,
    lista.comune,
    lista.data_installazione,
    lista.indirizzo,
    lista.provincia,
    lista.ubicazione,
    lista.asl_distributore,
    lista.cap
   FROM crosstab('
   select distinct  (v.indice||''_''|| id_rel_stab_linea), f.nome_campo, v.valore_campo
   from linee_mobili_fields_value v
   left join linee_mobili_html_fields f on v.id_linee_mobili_html_fields = f.id
   left join ml8_linee_attivita_nuove_materializzata ml on ml.id_nuova_linea_attivita= f.id_linea
   where f.enabled and ml.id_lookup_tipo_linee_mobili=2  and v.enabled 
   order by (v.indice ||''_''||  id_rel_stab_linea )'::text) lista(indice text, asl_distributore text, cap text, comune text, data_installazione text, indirizzo text, matricola text, provincia text, ubicazione text);

ALTER TABLE public.opu_stabilimento_mobile_distributori
  OWNER TO postgres;
GRANT ALL ON TABLE public.opu_stabilimento_mobile_distributori TO postgres;
GRANT SELECT ON TABLE public.opu_stabilimento_mobile_distributori TO usr_ro;
GRANT SELECT ON TABLE public.opu_stabilimento_mobile_distributori TO report;
...
ALTER FUNCTION public.api_insert_into_ricerche_anagrafiche_old_materializzata(integer)
  ...
  -- Function: public.altri_org_insert_into_ricerche_anagrafiche_old_materializzata(integer)

-- DROP FUNCTION public.altri_org_insert_into_ricerche_anagrafiche_old_materializzata(integer);

CREATE OR REPLACE FUNCTION public.altri_org_insert_into_ricerche_anagrafiche_old_materializzata(orgid integer)
  RETURNS boolean AS
$BODY$
DECLARE
BEGIN
...
SELECT global_org_view.riferimento_id,
    ...
    global_org_view.id_controllo_ultima_categorizzazione,id_linea,
        NULL::text as matricola
   FROM global_org_view
 where riferimento_id =orgId
);
...
$BODY$
  LANGUAGE plpgsql VOLATILE STRICT
  COST 100;
ALTER FUNCTION public.altri_org_insert_into_ricerche_anagrafiche_old_materializzata(integer)
  OWNER TO postgres;
...
alter table ricerche_anagrafiche_old_materializzata add column matricola text;
  
  
--NON LANCIARE (LO FA RITA)
