...
-- Chi: Bartolo Sansone
-- Quando: 19/12/2017
-- Cosa: VAF Laboratori HACCP

-- GISA
-- View: public.view_anagrafica_attivita_laboratori_haccp

-- DROP VIEW public.view_anagrafica_attivita_laboratori_haccp;

CREATE OR REPLACE VIEW public.view_anagrafica_attivita_laboratori_haccp AS 
...
and o.tipologia = 152;
ALTER TABLE public.view_anagrafica_attivita_laboratori_haccp
  OWNER TO postgres;
COMMENT ON VIEW public.view_anagrafica_attivita_laboratori_haccp
  IS 'LABORATORI HACCP';

  
  -- View: public.laboratori_haccp_campi_estesi

-- DROP VIEW public.laboratori_haccp_campi_estesi;

CREATE OR REPLACE VIEW public.laboratori_haccp_campi_estesi AS 
  

 SELECT DISTINCT o.org_id as riferimento_id,
       m.description as materiale, d.description as denominazione, p.norma as norma, case when accreditata then 'ACCREDITATA' else 'NON ACCREDITATA' end as accreditamento, e.description as ente from 
       organization o
       left join laboratorihaccp_elenco_prove p on o.org_id = p.org_id
left join lookup_matrici_labhaccp m on m.code = p.codice_matrice
left join lookup_denominazioni_labhaccp d on d.code = p.codice_denominazione
left join lookup_ente_labhaccp e on e.code = p.codice_ente
where p.trashed_date is null and o.trashed_date is null and o.tipologia = 152;
ALTER TABLE public.laboratori_haccp_campi_estesi
  OWNER TO postgres;
COMMENT ON VIEW public.laboratori_haccp_campi_estesi
  IS 'LABORATORI HACCP - CAMPI ESTESI';

  
  
-- Function: public_functions.anagrafica_infasamento_laboratori_haccp()

-- DROP FUNCTION public_functions.anagrafica_infasamento_laboratori_haccp();

CREATE OR REPLACE FUNCTION public_functions.anagrafica_infasamento_laboratori_haccp()
  RETURNS text AS
$BODY$
 DECLARE
	recImpresa record;
...
FOR recImpresa IN
	...
	from view_anagrafica_attivita_laboratori_haccp anag 
limit 2
...
LOOP
	for recCampiEstesi in 
		select 
			json_build_object('materiale', estesi.materiale,
								   'denominazione',estesi.denominazione,
								   'norma',estesi.norma,
								   'accreditamento', estesi.accreditamento,
								   'ente',estesi.ente) json_laboratori_haccp
		from 
			laboratori_haccp_campi_estesi estesi 
		where 
	...
	FOR recLinea IN
		...
		(
			select distinct
				...
				attivita,
				'ALT-HAC-HAC' as codiceunivoco,--inserire codice
				n_reg,
				num_riconoscimento,
	...
	LOOP
		qry:= 'select ret_operazione,ret_id_impresa from public_functions.anagrafica_inserisci_osa_estesi(' || '''' || replace(CAST(recImpresa.soggetto_fisico as text),'''','''''') || '''' || ',' || 
									     ...
									     '''' || replace(CAST(recLinea.linee_attivita as text),'''','''''') || '''' || ',' || 
									     									     '''' || replace(CAST(recCampiEstesi.json_laboratori_haccp as text),'''','''''') || '''' || ',' ||
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION public_functions.anagrafica_infasamento_laboratori_haccp()
  OWNER TO postgres;
