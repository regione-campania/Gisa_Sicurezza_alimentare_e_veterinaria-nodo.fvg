...
--------------------------------------------------------------------------------------------------------------
--------------------------------------------------------------------------------------------------------------
-------------------------------------AGGIUSTO CODICI MAPPING DI ORSA------------------------------------------
--------------------------------------------------------------------------------------------------------------
--------------------------------------------------------------------------------------------------------------
--------------------------------------------------------------------------------------------------------------

update mapping_ateco_ml8 set ml8_codice_1 = 'MS.010-MS.010.100' where ateco in ('10.86.00', '10.39.00');
update mapping_ateco_ml8 set ml8_codice_1 = 'MS.060-MS.060.100' where ateco in ('46.21.10');
update mapping_ateco_ml8 set ml8_codice_1 = 'MS.060-MS.060.300' where ateco in ('47.81.01','47.81.02','47.81.03');
update mapping_ateco_ml8 set ml8_codice_univoco  = 'MS.060-MS.060.100-852IT7A001' where ateco in ('46.21.10','46.31.10','','46.31.20','46.32.10','46.32.20','46.33.10','46.33.20','46.34.10','46.34.20','46.36.00','46.37.01','46.37.02','46.38.10','46.38.20','46.38.30','46.38.90','46.39.10','46.38.20','46.39.20');

alter table mapping_ateco_ml8 add column fisso boolean, add column mobile boolean;

update mapping_ateco_ml8 set fisso = (select fisso from master_list_flag_linee_attivita where codice_univoco = ml8_codice_univoco), mobile = (select mobile from master_list_flag_linee_attivita where codice_univoco = ml8_codice_univoco);

update mapping_ateco_ml8 set fisso= false, mobile = true where ml8_codice_1 ='MS.060-MS.060.300' and fisso is null and mobile is null;
update mapping_ateco_ml8 set fisso= true, mobile = false where ml8_codice_1 ='MS.010-MS.010.100' and fisso is null and mobile is null;
update mapping_ateco_ml8 set fisso=false, mobile = true where ateco = '49.20.00' and ml8_codice_1 = 'MS.090-MS.090.100';
update mapping_ateco_ml8 set fisso=false, mobile = true where (fisso is null and mobile is null) and ml8_codice_univoco = 'MS.090-MS.090.100';
update mapping_ateco_ml8 set ml8_codice_univoco = null, ml8_codice_1 = 'MS.050-MS.050.200-005', map_completo = false, fisso = true, mobile = false where ateco in ('55.90.20', '55.20.30');
update mapping_ateco_ml8 set note = concat_ws(';', note, 'NON SI PUO'' STABILIRE SE SIA FISSO O MOBILE PERCHE'' MAPPATO A LIVELLO MACROAREA') where ml8_codice_1 = 'MS.000';

--------------------------------------------------------------------------------------------------------------
-------------------------------------QUERY DI VERIFICA: DEVE ESSERE 0-----------------------------------------
--------------------------------------------------------------------------------------------------------------

select istat.short_description, map.* from mapping_ateco_ml8 map
left join lookup_codistat istat on istat.description = map.ateco
  where  map.ateco not in ('00.00.00', '52.24.20', '01.25.00', '10.11.00', '10.41.30', '10.51.10', '46.23.00', '47.99.20')
  and (map.ml8_codice_1 is null or map.ml8_codice_1 not in ('FARM', 'MS.000'))
and map.note is null and map.da_scartare is not true
 and (map.fisso is null or map.mobile is null)

--------------------------------------------------------------------------------------------------------------
--------------------------------------------------------------------------------------------------------------
--------------------------------------------------------------------------------------------------------------
------------------------------------GESTIONE 852 ESCLUSIVAMENTE FISSI-----------------------------------------
--------------------------------------------------------------------------------------------------------------
--------------------------------------------------------------------------------------------------------------
--------------------------------------------------------------------------------------------------------------

--------------------------------------------------------------------------------------------------------------
-------------------------------------CREAZIONE VISTA 852 ESCLUSIVAMENTE FISSI---------------------------------
--------------------------------------------------------------------------------------------------------------

CREATE VIEW organization_852_fissi as 
select org_id,  id_norma, id_tipologia_operatore, ateco, codice_univoco_ml, map_completo, id_la, id_linea
 from
(

select o.org_id as org_id,  id_opu_lookup_norme_master_list as id_norma, o.tipologia as id_tipologia_operatore, cod.description as ateco, COALESCE(mapping.ml8_codice_univoco,ml8_codice_1,ml8_codice_2) as codice_univoco_ml,
mapping.map_completo as map_completo
,  mapping.fisso ,mapping.mobile
, i.id as id_la, ml8.id_nuova_linea_attivita as id_linea 
...
left join ml8_linee_attivita_nuove_materializzata ml8 on (ml8.codice_attivita = COALESCE(mapping.ml8_codice_univoco,ml8_codice_1,ml8_codice_2) or (concat_ws('-', ml8.codice_macroarea, ml8.codice_aggregazione) = COALESCE(mapping.ml8_codice_univoco,ml8_codice_1,ml8_codice_2) and ml8.id_attivita = -1) or (ml8.codice_macroarea = COALESCE(mapping.ml8_codice_univoco,ml8_codice_1,ml8_codice_2) and ml8.id_attivita = -1 and ml8.id_aggregazione = -1))
where o.tipologia = 1 and o.trashed_date is null 
and i.trashed_date is null and cod.enabled 
and cod.description not in ('00.00.00', '47.99.20', '46.23.00',  '01.25.00', '10.11.00', '10.41.30', '10.51.10', '47.73.10', '01.45.00', '01.47.00', '01.49.10')
and COALESCE(mapping.ml8_codice_univoco,ml8_codice_1,ml8_codice_2) not in ('MS.000')
and fisso 
and fisso and o.org_id not in (
select o.org_id
from organization o
left join la_imprese_linee_attivita i on i.org_id = o.org_id
left join la_rel_ateco_attivita rel on i.id_rel_ateco_attivita=rel.id
...
--------------------------------------------------------------------------------------------------------------
-------------------------------------AGGIORNO IL CAMPO DI LA IMPRESE------------------------------------------
--------------------------------------------------------------------------------------------------------------

update la_imprese_linee_attivita set note_ht = 'Id_attivita_masterlist resettato prima di import 852. Valore precedente: '  || id_attivita_masterlist || '.', id_attivita_masterlist = null where id_attivita_masterlist>0;

select 
'update la_imprese_linee_attivita set id_attivita_masterlist = ' || id_linea || ', note_ht = ''Id_attivita_masterlist valorizzata per import 852 fissi 26/04'' where id = ' ||id_la||';'
 from organization_852_fissi

 --da fare con nohup
columns names: NO, no quoting
nohup psql -Upostgres -h 10.1.15.150 gisa < fissi_1.sql
...
(
 ...
 SELECT DISTINCT o.org_id AS riferimento_id,
    ...
    CASE WHEN o.tipologia=1 then CONCAT( COALESCE(mltemp2.macroarea, mltemp.macroarea, tsa.macroarea), '->', COALESCE(mltemp2.aggregazione, mltemp.aggregazione, tsa.aggregazione), '->', COALESCE(mltemp2.attivita, mltemp.attivita, tsa.attivita)) else
	...
	COALESCE(lai.id,lai.id, tsa.id, el.id, o.org_id) AS id_linea,
   ...
   FROM organization o
     ...
     LEFT JOIN lista_linee_vecchia_anagrafica_stabilimenti_soggetti_a_scia tsa ON tsa.org_id = o.org_id AND tsa.id_tipologia_operatore <> 1
...
CREATE OR REPLACE VIEW public.ricerca_anagrafiche AS 
 SELECT DISTINCT o.org_id AS riferimento_id,
   ...
   FROM organization o
     ...
     LEFT JOIN organization_address oa7 ON oa7.org_id = o.org_id AND oa7.address_type = 7 AND oa7.trasheddate IS NULL
  WHERE o.org_id <> 0 AND o.tipologia <> 0 AND o.trashed_date IS NULL AND o.import_opu IS NOT TRUE 
  --OR o.trashed_date = '1970-01-01 00:00:00'::timestamp without time zone) 
  AND ((o.tipologia = ANY (ARRAY[1, 151, 802, 152, 10, 20, 2, 800, 801])) OR o.tipologia = 3 AND o.direct_bill = false)
...
--da fare con nohup
columns names: NO, no quoting
nohup psql -Upostgres -h 10.1.15.150 gisa < fissi_2.sql
tail -f nohup.out | while read line; do echo `date` "$line" ; done

-- Fix ricerche

update lookup_tipo_ricerca_anagrafica set enabled = false where code = 3;
...
-- Disabilito trigger organization
...
select distinct  'update organization set trashed_date = ''1970-01-01'', note_hd =''852 ARCHIVIATO COME DA ACCORDI CON ORSA PER ATECO 00.00.00'' where org_id='||o.org_id||';'
--cod.description,
--i.id as id_la
from organization o
left join la_imprese_linee_attivita i on i.org_id = o.org_id
...
where o.tipologia = 1 and o.trashed_date is null
and i.trashed_date is null --and cod.enabled
and cod.description in ('00.00.00')
and o.org_id not in (
...
where o.tipologia = 1 and o.trashed_date is null
and i.trashed_date is null --and cod.enabled
and cod.description not in ('00.00.00')
);

 --da fare con nohup
columns names: NO, no quoting
nohup psql -Upostgres -h 10.1.15.150 gisa < rita_1.sql
tail -f nohup.out | while read line; do echo `date` "$line" ; done

select distinct 'update organization set trashed_date = ''1970-01-01'', note_hd =''852 ARCHIVIATO COME DA ACCORDI CON ORSA PER ATECO ASSOCIATI AD ALLEVAMENTI'' where org_id='||o.org_id||';'
--cod.description,
--i.id as id_la
from organization o
left join la_imprese_linee_attivita i on i.org_id = o.org_id
...
where o.tipologia = 1 and o.trashed_date is null
and i.trashed_date is null --and cod.enabled
and cod.description in ('01.45.00',
'01.47.00',
'01.50.00',
'01.49.10',
'82.92.10',
...
where o.tipologia = 1 and o.trashed_date is null
and i.trashed_date is null --and cod.enabled
and cod.description not in ('01.45.00',
'01.47.00',
'01.50.00',
'01.49.10',
'82.92.10',
...
'01.63.00')
 ...
 --da fare con nohup
columns names: NO, no quoting
nohup psql -Upostgres -h 10.1.15.150 gisa < rita_2.sql
tail -f nohup.out | while read line; do echo `date` "$line" ; done

select distinct 'update organization set trashed_date = now(), note_hd =''852 ARCHIVIATO COME DA ACCORDI CON ORSA PER ATECO ASSOCIATI A STABILIMENTI NON MAPPATI, SENZA CU'' where org_id='||o.org_id||';'
--cod.description,
--i.id as id_la
from organization o
left join la_imprese_linee_attivita i on i.org_id = o.org_id
...
'10.89.01')
 ...
 --da fare con nohup
columns names: NO, no quoting
nohup psql -Upostgres -h 10.1.15.150 gisa < rita_3.sql
tail -f nohup.out | while read line; do echo `date` "$line" ; done

select 'update organization set trashed_date = ''1970-01-01'', note_hd =''852 ARCHIVIATO COME DA ACCORDI CON ORSA PER ATECO 00.00.00'' where org_id='||o.org_id||';'
--cod.description,
--i.id as id_la
from organization o
left join la_imprese_linee_attivita i on i.org_id = o.org_id
...
and cod.description not in ('00.00.00')
 ...
 --da fare con nohup
columns names: NO, no quoting
nohup psql -Upostgres -h 10.1.15.150 gisa < rita_4.sql
tail -f nohup.out | while read line; do echo `date` "$line" ; done

-- Abilito trigger organization
...
select 'update la_imprese_linee_attivita set id_attivita_masterlist = null where id='||id||';' from la_imprese_linee_attivita  
where id_attivita_masterlist  = -999 and trashed_date is null and org_id not in
(select i.org_id
--cod.description,
--i.id as id_la
from organization o
...
'52.24.20')
 ...
 --da fare con nohup
columns names: NO, no quoting
nohup psql -Upostgres -h 10.1.15.150 gisa < rita_5.sql
tail -f nohup.out | while read line; do echo `date` "$line" ; done
...
-- Creo vista 852 ibridi

CREATE VIEW organization_852_ibridi as 
select org_id,  id_norma, id_tipologia_operatore, ateco, codice_univoco_ml, map_completo, id_la, id_linea
 from
(
...
select org_id,  id_norma, id_tipologia_operatore, codice_univoco_ml, map_completo, id_la,id_linea,
'update la_imprese_linee_attivita set id_attivita_masterlist = ' || id_linea || ', note_ht = ''Id_attivita_masterlist valorizzata per import 852 ibridi 29/06'' where id = ' ||id_la||';'
 from organization_852_ibridi

  -- Aggiorno tutte le restanti linee ibride
  
update la_imprese_linee_attivita set id_attivita_masterlist = -666, note_ht = 'Id_attivita_masterlist valorizzata -666 per import 852 ibridi 29/06' where org_id in (select org_id from organization_852_ibridi) and id_attivita_masterlist is null;
 insert into ml8_linee_attivita_nuove_materializzata(
 id_nuova_linea_attivita, 
 id_macroarea, id_aggregazione, id_attivita, 
 codice_macroarea, codice_aggregazione, codice_attivita,
  macroarea, aggregazione, attivita, 
  id_norma, norma, descrizione, 
  livello, id_padre, path_id, path_descrizione, codice, path_codice)
 values (
 -666, 
 -666, -666, -666, 
 'TEMP.NON.MAPP.', 'TEMP.NON.MAPP.', 'TEMP.NON.MAPP.', 
 'MACROAREA TEMPORANEAMENTE NON MAPPATA', 'AGGREGAZIONE TEMPORANEAMENTE NON MAPPATA', 'LINEA DI ATTIVITA TEMPORANEAMENTE NON MAPPATA', 
 -1, null, 'TEMPORANEAMENTE NON MAPPATA', 
 3, -666, '-666;-666;-666', 'MACROAREA TEMPORANEAMENTE NON MAPPATA -> AGGREGAZIONE TEMPORANEAMENTE NON MAPPATA ->LINEA DI ATTIVITA TEMPORANEAMENTE NON MAPPATA', 'TEMP.NON.MAPP.', 'TEMP.NON.MAPP.->TEMP.NON.MAPP.->TEMP.NON.MAPP.');

delete from lista_linee_vecchia_anagrafica_stabilimenti_soggetti_a_scia where  org_id in (select org_id from organization_852_fissi  UNION select org_id from organization_852_ibridi );
...
when  row_number() over() = 3 then tl3.description
 ...
 left join lookup_tipo_locale tl on (o.tipo_locale = tl.code) 
  left join lookup_tipo_locale tl2 on (o.tipo_locale2 = tl2.code) 
   left join lookup_tipo_locale tl3 on (o.tipo_locale3 = tl3.code) ','o.org_id = #orgid#','NA','print');insert into scheda_operatore_metadati(tipo_operatore, label, attributo, sql_campo, sql_origine, sql_condizione, ordine, destinazione) values ('1','LISTA LINEE PRODUTTIVE','capitolo','','','','Q','');insert into scheda_operatore_metadati(tipo_operatore, label, attributo, sql_campo, sql_origine, sql_condizione, ordine, destinazione) values ('1','ATTIVITA','','COALESCE(norma ||''<br/>''||macroarea||''<br/>''||aggregazione||''<br/>''||attivita, '''')
','ricerche_anagrafiche_old_materializzata','riferimento_id = #orgid# and riferimento_id_nome_col = ''org_id''','QQ','');insert into scheda_operatore_metadati(tipo_operatore, label, attributo, sql_campo, sql_origine, sql_condizione, ordine, destinazione) values ('1','ELENCO DIFFIDE','capitolo','','','','ZZZZ','');insert into scheda_operatore_metadati(tipo_operatore, label, attributo, sql_campo, sql_origine, sql_condizione, ordine, destinazione) values ('1','DIFFIDATO PER ','',' 	lookup_norme.description || '', ''|| CASE WHEN diffida.stato_diffida = ''0'' then ''[DIFFIDA ATTIVA PER I PROSSIMI 5 ANNI]'' WHEN diffida.stato_diffida = ''1'' then '' [DIFFIDA NON PIU'''' ATTIVA IN QUANTO L''''OSA E'''' STATO SANZIONATO PER QUESTA NORMA]'' end ||'', ''|| ''data diffida: ''|| to_char(t.assigned_date,''dd/mm/yyyy'') as assigned_date','ticket t JOIN ticket nonconf ON nonconf.ticketid = t.id_nonconformita AND nonconf.tipologia = 8 JOIN ticket cu ON cu.id_controllo_ufficiale::text = nonconf.id_controllo_ufficiale::text AND cu.tipologia = 3 left join norme_violate_sanzioni diffida on diffida.idticket=t.ticketid left join lookup_norme on lookup_norme.code=diffida.id_norma','t.ticketid > 0 AND t.tipologia = 11 and current_timestamp< (cu.assigned_date + interval ''5 years'') and t.trashed_date is null and cu.trashed_date is null and nonconf.trashed_date is null and t.org_id = #orgid# order by assigned_date DESC ','ZZZZ','');insert into scheda_operatore_metadati(tipo_operatore, label, attributo, sql_campo, sql_origine, sql_condizione, ordine, destinazione) values ('1','INFORMAZIONE PRIMARIA','capitolo','','','','a','');insert into scheda_operatore_metadati(tipo_operatore, label, attributo, sql_campo, sql_origine, sql_condizione, ordine, destinazione) values ('1','ASL ','asl','asl.description as descr_asl ','lookup_site_id asl left join organization o on o.site_id = asl.code ','o.org_id = #orgid#','ab','print');insert into scheda_operatore_metadati(tipo_operatore, label, attributo, sql_campo, sql_origine, sql_condizione, ordine, destinazione) values ('1','CARATTERE','','case when source =1 then ''temporanea dal ''|| to_char(data_in_carattere,''dd/mm/yyyy'') || ''  al ''||to_char(data_fine_carattere,''dd/mm/yyyy'') else ''permanente''  end ||
CASE WHEN cessazione and source=1 then '' Cessazione automatica'' else '''' end 
as carattere 
','organization ','org_id = #orgid#','dab','');insert into scheda_operatore_metadati(tipo_operatore, label, attributo, sql_campo, sql_origine, sql_condizione, ordine, destinazione) values ('1','PROSSIMO CONTROLLO CON LA TECNICA DELLA SORVEGLIANZA','','case when prossimo_controllo is null then to_char(current_timestamp, ''dd/mm/yy'') else to_char(prossimo_controllo, ''dd/mm/yy'') end as prossimo_controllo ','organization ','org_id  = #orgid#','eab','');insert into scheda_operatore_metadati(tipo_operatore, label, attributo, sql_campo, sql_origine, sql_condizione, ordine, destinazione) values ('1','STATO STABILIMENTO','','case 
when (cessato = 1 and source = 1) or (source = 1 and data_fine_carattere is not null and data_fine_carattere<current_timestamp) then ''Cessato in data ''
...
when  row_number() over() = 3 then tl3.description
 ...
 left join lookup_tipo_locale tl on (o.tipo_locale = tl.code) 
  left join lookup_tipo_locale tl2 on (o.tipo_locale2 = tl2.code) 
   left join lookup_tipo_locale tl3 on (o.tipo_locale3 = tl3.code) ','o.org_id = #orgid#','NAB','screen');insert into scheda_operatore_metadati(tipo_operatore, label, attributo, sql_campo, sql_origine, sql_condizione, ordine, destinazione) values ('1','DESCRIZIONE STORICA DELLE LINEE DI ATTIVITA'' (MASTER LIST , ATECO, ECC.)','','CONCAT(lcd.description, '' '', lcd.short_description, '' '', la.categoria, '' '', la.linea_attivita, ''<br/>'', ''<b>Macroarea</b>: '', ml8.macroarea, ''<br/>'', ''<b>Aggregazione</b>: '', '' '', ml8.aggregazione, ''<br/>'', ''<b>Attivita</b>: '', ml8.attivita) ','la_imprese_linee_attivita lda
left join la_rel_ateco_attivita rat on (lda.id_rel_ateco_attivita = rat.id)
left join lookup_codistat lcd on (rat.id_lookup_codistat = lcd.code)
left join ml8_linee_attivita_nuove_materializzata ml8 on ml8.id_nuova_linea_attivita = lda.id_attivita_masterlist,
la_rel_ateco_attivita rel, la_linee_attivita la, lookup_codistat cod','lda.trashed_date is null and lda.org_id= #orgid# and
lda.id_rel_ateco_attivita=rel.id
and rel.id_linee_attivita=la.id and rel.id_lookup_codistat=cod.code
 and lda.trashed_date is null','QQQ','');insert into scheda_operatore_metadati(tipo_operatore, label, attributo, sql_campo, sql_origine, sql_condizione, ordine, destinazione) values ('1','DETTAGLI ADDIZIONALI','capitolo','','','','Z','screen');
