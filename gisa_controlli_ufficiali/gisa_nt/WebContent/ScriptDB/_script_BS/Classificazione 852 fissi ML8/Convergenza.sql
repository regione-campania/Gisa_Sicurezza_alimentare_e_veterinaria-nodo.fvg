-- Faccio convergere le istanze 200,300,400,500 nell'istanza 100

-- Creo colonna per data convergenza
alter table la_imprese_linee_attivita drop column data_scadenza_per_convergenza timestamp without time zone;

-- Aggiorno le istanze
update la_imprese_linee_attivita set trashed_date = now(), data_scadenza_per_convergenza = now(), modified_by = 1, note_ht = concat_ws(';', note_ht, 'Istanza disabilitata per convergenza verso istanza con id '||100) where id in (200,300,400,500) and trashed_date is null;

-- Aggiorno i cu
update linee_attivita_controlli_ufficiali set id_linea_attivita = 100, note = concat_ws(';', note, 'Istanza '||id_linea_attivita||' modificata per convergenza verso istanza con id '||100) where id_linea_attivita in (200,300,400,500);
...
CREATE OR REPLACE VIEW public.ricerca_anagrafiche AS 
 SELECT DISTINCT o.org_id AS riferimento_id,
    ...
    COALESCE(mltemp2.aggregazione, mltemp.aggregazione, tsa.aggregazione) AS aggregazione,
        CASE
            WHEN o.tipologia = 1 THEN concat(COALESCE(lcd.description, ''), '->', COALESCE(mltemp2.macroarea, mltemp.macroarea, tsa.macroarea), '->', COALESCE(mltemp2.aggregazione, mltemp.aggregazione, tsa.aggregazione), '->', COALESCE(mltemp2.attivita, mltemp.attivita, tsa.attivita))::character varying
            ELSE COALESCE(mltemp2.path_descrizione, mltemp.path_descrizione::text::character varying, ((((tsa.macroarea || '->'::text) || tsa.aggregazione) || '->'::text) ||
            CASE
  ...
  WHERE o.org_id <> 0 AND o.tipologia <> 0 AND o.trashed_date IS NULL AND o.import_opu IS NOT TRUE AND ((o.tipologia = ANY (ARRAY[1, 151, 802, 152, 10, 20, 2, 800, 801])) OR o.tipologia = 3 AND o.direct_bill = false)
UNION
...
(
 ...
 SELECT DISTINCT o.org_id AS riferimento_id,
    ...
    COALESCE(mltemp2.macroarea, mltemp.macroarea, tsa.macroarea) AS macroarea,
    COALESCE(mltemp2.aggregazione, mltemp.aggregazione, tsa.aggregazione) AS aggregazione,
    CASE WHEN o.tipologia=1 then CONCAT( COALESCE(lcd.description, ''), '->', COALESCE(mltemp2.macroarea, mltemp.macroarea, tsa.macroarea), '->', COALESCE(mltemp2.aggregazione, mltemp.aggregazione, tsa.aggregazione), '->', COALESCE(mltemp2.attivita, mltemp.attivita, tsa.attivita)) else
...
CREATE OR REPLACE VIEW public.ricerca_anagrafiche AS 
 SELECT DISTINCT o.org_id AS riferimento_id,
    ...
    COALESCE(mltemp2.aggregazione, mltemp.aggregazione, tsa.aggregazione) AS aggregazione,
        CASE
            WHEN o.tipologia = 1 THEN concat(COALESCE(mltemp2.macroarea, mltemp.macroarea, tsa.macroarea), '->', COALESCE(mltemp2.aggregazione, mltemp.aggregazione, tsa.aggregazione), '->', COALESCE(mltemp2.attivita, mltemp.attivita, tsa.attivita))::character varying
            ELSE COALESCE(mltemp2.path_descrizione, mltemp.path_descrizione::text::character varying, ((((tsa.macroarea || '->'::text) || tsa.aggregazione) || '->'::text) ||
            CASE
                WHEN tsa.attivita IS NOT NULL THEN concat_ws('->'::text, tsa.attivita, tsa.attivita_specifica)
                ELSE ''::text
