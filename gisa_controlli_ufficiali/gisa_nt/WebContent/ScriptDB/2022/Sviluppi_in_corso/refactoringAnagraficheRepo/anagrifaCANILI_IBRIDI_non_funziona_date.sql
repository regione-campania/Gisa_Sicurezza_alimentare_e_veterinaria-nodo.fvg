...
-- mancano le info aggiuntive + campi estesi
DROP FUNCTION digemon.dbi_get_anagrafica_canili(text,text,text);
CREATE OR REPLACE FUNCTION digemon.dbi_get_anagrafica_canili(
	...
	ELSE
		RETURN QUERY select * 
				...
				WHERE 
					...
					and ( r."codice macroarea" in ('IUV') and r."codice aggregazione" in ('CAN')
					or    r."codice macroarea" in ('IUV') and r."codice aggregazione" in ('ADCA')
					or    r."codice macroarea" in ('VET') and r."codice aggregazione" in ('OSPV','CLIV','AMBV') and r."codice attivita" in ('PU') 
					)
				union 
				-- vecchia anagrafica CANILI
				select * 
					FROM digemon.dbi_get_canili_org(_data_1,_data_2)
				) a; --nuova anagrafica
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION digemon.dbi_get_anagrafica_canili(text,text, text)
  OWNER TO postgres;

select * from digemon.dbi_get_anagrafica_canili()
select * from digemon.dbi_get_anagrafica_canili('2020-01-01','2021-01-01')

select r.progressivo::integer
				FROM 
					digemon.dbi_get_anagrafica_scia_new('2020-01-01','2021-01-01') r  
				WHERE 
					--r."data inserimento"::date between '2020-01-01'::date and '2021-01-01'::date 
					--and r.progressivo::integer >= 1
					--and 
					( r."codice macroarea" in ('IUV') and r."codice aggregazione" in ('CAN')
					or    r."codice macroarea" in ('IUV') and r."codice aggregazione" in ('ADCA')
					or    r."codice macroarea" in ('VET') and r."codice aggregazione" in ('OSPV','CLIV','AMBV') and r."codice attivita" in ('PU') 
					) and r."data inserimento"::date between '2020-01-01' and '2021-01-01'
					and r.progressivo::integer >=1
