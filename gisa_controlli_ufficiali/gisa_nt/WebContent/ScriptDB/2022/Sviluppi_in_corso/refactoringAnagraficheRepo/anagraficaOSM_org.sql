-- Function: digemon.estrazione_controlli_audit(timestamp without time zone, timestamp without time zone, integer, boolean)

CREATE OR REPLACE FUNCTION digemon.dbi_get_osm_org(IN _data_1 text default null::text, IN _data_2 text default null::text)
...
RETURN QUERY
	with cte as (
			select distinct  
			 ...
			 , case when tab1.account_number is null or tab1.account_number='' then 'non definito' 
				else case when tipologia = 800 then 'ait'|| account_number 
				else account_number end 
			end as numero_registrazione_stabilimento
			...
			WHERE 
				...
				and tab1.tipologia in (800,801) and tab1.trashed_date is null
		)
		select 
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION digemon.dbi_get_osm_org(text,text)
  OWNER TO postgres;
