digemon.dbi_get_anagrafica_giardini_zoologici(IN _data1 text,  IN _data2 text, IN _limit_record text ) --ok
digemon.dbi_get_anagrafica_posti_controllo_frontalieri(IN _data1 text,  IN _data2 text, IN _limit_record text )
digemon.dbi_get_anagrafica_mangimi_dedicati(IN _data1 text,  IN _data2 text, IN _limit_record text )
digemon.dbi_get_farmacie_norma_362(IN _data1 text,  IN _data2 text, IN _limit_record text )
digemon.dbi_get_utilizzatori_animali_fini_scientifici(IN _data1 text,  IN _data2 text, IN _limit_record text )


CREATE OR REPLACE FUNCTION digemon.dbi_get_anagrafica_giardini_zoologici(
	...
	ELSE
		RETURN QUERY select * 
				...
				WHERE 
					...
					r."codice macroarea"='GIAR' and r."codice aggregazione"='GIAR' and r."codice attivita"='ZOO'; 
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION digemon.dbi_get_anagrafica_giardini_zoologici(text,text, text)
  OWNER TO postgres;

select * from digemon.dbi_get_anagrafica_giardini_zoologici('2021-01-01','2021-05-31')
select * from digemon.dbi_get_anagrafica_giardini_zoologici()
