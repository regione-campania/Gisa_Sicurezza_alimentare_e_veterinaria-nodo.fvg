...
-- colonie
union
select o.org_id as org_id, id_opu_lookup_norme_master_list as id_norma, o.tipologia as id_tipologia_operatore,'IUV-CF-CF' as codice_univoco_ml,true as map_completo
from organization o
join opu_lookup_norme_master_list_rel_tipologia_organzation norma on norma.tipologia_organization =o.tipologia
where o.tipologia = 16 and trashed_date is null


-- acque di rete
union
select o.org_id as org_id, id_opu_lookup_norme_master_list as id_norma, o.tipologia as id_tipologia_operatore,'AQRE−AQRE−AQRE' as codice_univoco_ml,true as map_completo
from organization o
join opu_lookup_norme_master_list_rel_tipologia_organzation norma on norma.tipologia_organization =o.tipologia
where o.tipologia = 14 and trashed_date is null

);
  update opu_lookup_norme_master_list_rel_tipologia_organzation set codice_univoco_ml = 'AQRE−AQRE−AQRE',  macroarea_temp = null, aggregazione_temp = null, linea_temp = null where tipologia_organization = 14;
...
select * from ricerche_anagrafiche_old_materializzata  where tipologia_operatore in (14)

insert into lookup_tipo_scheda_operatore (code, description, titolo, firma_data, nome_file) values (43, 'Acque di rete', 'Scheda Punto di prelievo', false, 'SchedaPuntoDiPrelievo');


update scheda_operatore_metadati set enabled = false where tipo_operatore = 43;insert into scheda_operatore_metadati(tipo_operatore, label, attributo, sql_campo, sql_origine, sql_condizione, ordine, destinazione) values ('43','INFORMAZIONE PRIMARIA','capitolo','','','','-1','');insert into scheda_operatore_metadati(tipo_operatore, label, attributo, sql_campo, sql_origine, sql_condizione, ordine, destinazione) values ('43','ASL DI RIFERIMENTO','asl','asl.description as descr_asl ','lookup_site_id asl left join organization o on o.site_id = asl.code ','o.org_id = #orgid#','-1','');insert into scheda_operatore_metadati(tipo_operatore, label, attributo, sql_campo, sql_origine, sql_condizione, ordine, destinazione) values ('43','DENOMINAZIONE','','case when name <>'''' then name else account_number end','organization ','org_id = #orgid#','-1','');insert into scheda_operatore_metadati(tipo_operatore, label, attributo, sql_campo, sql_origine, sql_condizione, ordine, destinazione) values ('43','CODICE PUNTO DI PRELIEVO','','account_number','organization ','org_id = #orgid#','-1','');insert into scheda_operatore_metadati(tipo_operatore, label, attributo, sql_campo, sql_origine, sql_condizione, ordine, destinazione) values ('43','TIPOLOGIA','','t.description ||
'' (''||
COALESCE(t.identificativo,'''')
|| '')''','organization o left join lookup_tipo_acque t on t.code = o.tipo_struttura','org_id = #orgid#','-1','');insert into scheda_operatore_metadati(tipo_operatore, label, attributo, sql_campo, sql_origine, sql_condizione, ordine, destinazione) values ('43','ENTE GESTORE','','banca','organization ','org_id = #orgid#
','-1','');insert into scheda_operatore_metadati(tipo_operatore, label, attributo, sql_campo, sql_origine, sql_condizione, ordine, destinazione) values ('43','STATO','','case when cessato = 0 then ''ATTIVO'' else ''NON ATTIVO'' end','organization ','org_id = #orgid#
','-1','');insert into scheda_operatore_metadati(tipo_operatore, label, attributo, sql_campo, sql_origine, sql_condizione, ordine, destinazione) values ('43','UBICAZIONE','capitolo','','','','-1','');insert into scheda_operatore_metadati(tipo_operatore, label, attributo, sql_campo, sql_origine, sql_condizione, ordine, destinazione) values ('43','INDIRIZZO','','case when upper(oaope.addrline1) is not null then upper(oaope.addrline1) else  '' '' end || ''  ''|| case when upper(oaope.city) is not null  then upper(oaope.city) else  '' '' end || '' ''|| case when oaope.postalcode is not null  then oaope.postalcode else  '' '' end || '' ''|| case when oaope.state is not null  then oaope.state else  '' '' end as ind_sede_operativa ','organization o left join organization_address oaope on (o.org_id = oaope.org_id and oaope.address_type = 5) ','o.org_id = #orgid#','-1','');insert into scheda_operatore_metadati(tipo_operatore, label, attributo, sql_campo, sql_origine, sql_condizione, ordine, destinazione) values ('43','LISTA LINEE PRODUTTIVE','capitolo','','','','-1','');insert into scheda_operatore_metadati(tipo_operatore, label, attributo, sql_campo, sql_origine, sql_condizione, ordine, destinazione) values ('43','ATTIVITA','','norma ||''<br/>''||path_attivita_completo','ricerche_anagrafiche_old_materializzata','riferimento_id = #orgid# and riferimento_id_nome_col = ''org_id''','-1','');insert into scheda_operatore_metadati(tipo_operatore, label, attributo, sql_campo, sql_origine, sql_condizione, ordine, destinazione) values ('43','DESCRIZIONE STORICA DELLE LINEE DI ATTIVITA'' (MASTER LIST , ATECO, ECC.)','','''ACQUE DI RETE''','organization ','org_id = #orgid#','-1','');insert into scheda_operatore_metadati(tipo_operatore, label, attributo, sql_campo, sql_origine, sql_condizione, ordine, destinazione) values ('43','DETTAGLI ADDIZIONALI','capitolo','','','','-1','');insert into scheda_operatore_metadati(tipo_operatore, label, attributo, sql_campo, sql_origine, sql_condizione, ordine, destinazione) values ('43','NOTE','','notes ','organization ','org_id = #orgid#','-1','');insert into scheda_operatore_metadati(tipo_operatore, label, attributo, sql_campo, sql_origine, sql_condizione, ordine, destinazione) values ('43','INSERITO DA','','c.namefirst || '' '' || c.namelast ||  '' '' || to_char(o.entered, ''dd/MM/yyyy'')','organization o left join access_ a on a.user_id = o.enteredby left join contact_ c on c.contact_id = a.contact_id','o.org_id = #orgid#','-1','');insert into scheda_operatore_metadati(tipo_operatore, label, attributo, sql_campo, sql_origine, sql_condizione, ordine, destinazione) values ('43','MODIFICATO DA','','c.namefirst || '' '' || c.namelast ||  '' '' || to_char(o.modified, ''dd/MM/yyyy'')','organization o left join access_ a on a.user_id = o.modifiedby left join contact_ c on c.contact_id = a.contact_id','o.org_id = #orgid#','-1','');
