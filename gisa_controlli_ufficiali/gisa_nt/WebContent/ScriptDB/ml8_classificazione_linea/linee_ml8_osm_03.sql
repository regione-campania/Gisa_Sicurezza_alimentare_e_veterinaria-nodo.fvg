...
update mapping_osm_ml8 set map_completo = false where codice_univoco_ml8 = 'MG';
  ...
  update opu_lookup_norme_master_list_rel_tipologia_organzation set codice_univoco_ml = null,  macroarea_temp = 'MANGIMISTICA IN GENERE (stabilimenti registrati)' where tipologia_organization = 801;
  update opu_lookup_norme_master_list_rel_tipologia_organzation set codice_univoco_ml = null, macroarea_temp = 'MANGIMISTICA IN GENERE (stabilimenti riconosciuti)' where tipologia_organization = 800;
...
select * from ricerche_anagrafiche_old_materializzata  where tipologia_operatore in (800, 801)
