-- ML8 - riproduzione animale 
update opu_lookup_norme_master_list_rel_tipologia_organzation set macroarea_temp =null, aggregazione_temp = null, linea_temp = null where tipologia_organization = 8;
...
CREATE OR REPLACE VIEW public.global_org_view AS 
 SELECT DISTINCT o.org_id AS riferimento_id,
    ...
    COALESCE(ml8.attivita, norme.linea_temp,mltemp.path_descrizione) AS attivita,
    case when mltemp.macroarea is not null then mltemp.path_descrizione 
    else ''::text
    end as path_attivita_completo,
    case when mltemp.macroarea is not null then null
