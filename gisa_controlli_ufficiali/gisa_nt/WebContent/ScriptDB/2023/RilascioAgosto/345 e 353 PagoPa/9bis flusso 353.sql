-- Lanciare solo dopo tutti gli script del 345
...
in
 ...
 string_agg(DISTINCT concat_ws('#'::text, pagopa_pv.identificativo_univoco_versamento, COALESCE(pagopa_pv.header_file_ricevuta, pagopa_pv.header_file_avviso), pagopa_pv.stato_pagamento), '##'::text) AS pagopa_pv_iuv,
  string_agg(DISTINCT concat_ws('#'::text, pagopa_no.identificativo_univoco_versamento, COALESCE(pagopa_no.header_file_ricevuta, pagopa_no.header_file_avviso), pagopa_no.stato_pagamento), '##'::text) AS pagopa_no_iuv,
...
-- Dato che la vista potrebbe essere cambiata, eventualmente gli unici pezzi da modificare sono:
