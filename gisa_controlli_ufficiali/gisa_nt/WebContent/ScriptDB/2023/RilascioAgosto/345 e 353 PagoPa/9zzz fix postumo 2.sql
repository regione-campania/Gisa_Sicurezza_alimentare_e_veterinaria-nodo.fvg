...
in
 ...
 case when count(distinct pagopa_no.*)<=0 then NULL when  count(distinct pagopa_no.*) = count(distinct pagopa_no_rate_pagate.*) then true else false end AS pagopa_no_tutte_rate_pagate,
  case 
  when count(distinct pagopa_no.*)>0 and count(distinct pagopa_no.*) = count(distinct pagopa_no_rate_pagate.*) and pagopa_importo_no.importo_totale_versamento::double precision = get_registro_trasgressori_importo_effettivamente_introitato_2(s.id_sanzione) then true 
...
LEFT JOIN pagopa_sanzioni_importo_ordinanza pagopa_importo_no on pagopa_importo_no.id_sanzione = s.id_sanzione and pagopa_importo_no.trashed_date is null and pagopa_importo_no.principale is true	  	  
