...
when _tipopagamento = 'NO' and _rigenerato is true and _numerorate = 1 then ' - RATA UNICA AUTOMATICA' --DI RIGENERAZIONE '||(select count(*) from pagopa_sanzioni_importo_ordinanza where id_sanzione = _idsanzione and trashed_date is null)		  
when _tipopagamento = 'NO' and _rigenerato is true and _numerorate > 1 then ' - RATA '||_indice||' AUTOMATICA '--||(select count(*) from pagopa_sanzioni_importo_ordinanza where id_sanzione = _idsanzione and trashed_date is null)		  
else '' end) 
...
CREATE OR REPLACE FUNCTION public.pagopa_get_info_sanzione(
	_id integer)
    RETURNS text
    LANGUAGE 'plpgsql'
    COST 100
...
s.ticketid as "ID SANZIONE",
rt.progressivo||'\'||substring(rt.anno::text, 3, 2) as "REGISTRO TRASGRESSORI"

from ticket s
join ticket cu on cu.ticketid = s.id_controllo_ufficiale::integer and cu.trashed_date is null and cu.tipologia = 3
join registro_trasgressori_values rt on rt.id_sanzione = s.ticketid and rt.trashed_date is null

where s.ticketid = _id
) aa;

IF _json IS NULL THEN
SELECT row_to_json(bb) into _json FROM (SELECT 'Nessuna sanzione trovata con questi dati' as "ERRORE") bb;
END IF;
