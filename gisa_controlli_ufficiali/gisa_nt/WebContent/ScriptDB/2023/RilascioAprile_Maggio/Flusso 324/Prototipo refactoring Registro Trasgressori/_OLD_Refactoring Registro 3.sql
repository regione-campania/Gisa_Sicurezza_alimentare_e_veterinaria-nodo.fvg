...
in
 ...
 string_agg(DISTINCT concat_ws('#'::text, pagopa_pv.identificativo_univoco_versamento, pagopa_pv.url_file_avviso,    pagopa_pv.stato_pagamento), '##'::text) AS pagopa_pv_iuv,
  ...
  COALESCE(string_agg(DISTINCT case when pagopa_no.stato_pagamento = 'PAGAMENTO COMPLETATO' then rate[pagopa_no.indice] end, ', '), '') AS pagopa_no_rate_pagate,
 case when count(pagopa_no.*)<=0 then NULL when  count(pagopa_no.*) = count(pagopa_no_rate_pagate.*) then true else false end AS pagopa_no_tutte_rate_pagate,
  case when count(pagopa_no.*)>0 and count(pagopa_no.*) = count(pagopa_no_rate_pagate.*) then true else rt.pratica_chiusa end AS pratica_chiusa
