...
CREATE OR REPLACE FUNCTION public.get_registro_trasgressori_rate_pagate()
    RETURNS TABLE(id_sanzione_rp integer, rate_pagate_rp text) 
    LANGUAGE 'plpgsql'
    COST 100
...
id_sanzione_rp, rate_pagate_rp

in 


SELECT id_sanzione, COALESCE(string_agg(rate[indice], ',' order by indice asc), '') FROM pagopa_pagamenti 
where tipo_pagamento = 'NO' and trashed_date is null and stato_pagamento = 'PAGAMENTO COMPLETATO' 
group by pagopa_pagamenti.id_sanzione
...
CREATE OR REPLACE FUNCTION public.get_registro_trasgressori_tutte_rate_pagate()
    RETURNS TABLE(id_sanzione_trp integer, tutte_rate_pagate_trp boolean) 
    LANGUAGE 'plpgsql'
    COST 100
    VOLATILE STRICT PARALLEL UNSAFE
    ROWS 1000
...
id_sanzione_trp, tutte_rate_pagate_trp
...
id_sanzione, case when tot_avvisi = tot_pagati then true else false end as tutte_pagate FROM
(select
count(p.id_sanzione) as tot_avvisi, p.id_sanzione , (select count(*) FROM pagopa_pagamenti where id_sanzione = p.id_sanzione and tipo_pagamento = 'NO' and trashed_date is null and stato_pagamento = 'PAGAMENTO COMPLETATO') as tot_pagati

FROM pagopa_pagamenti p where p.tipo_pagamento = 'NO' and p.trashed_date is null 
...
in
 ...
 string_agg(DISTINCT concat_ws('#'::text, pagopa_pv.identificativo_univoco_versamento, pagopa_pv.url_file_avviso,    pagopa_pv.stato_pagamento), '##'::text) AS pagopa_pv_iuv,
  ...
  COALESCE(pagopa_no_rate_pagate.rate_pagate_rp, '') AS pagopa_no_rate_pagate,
  pagopa_no_tutte_rate_pagate.tutte_rate_pagate_trp AS pagopa_no_tutte_rate_pagate
  ...
  LEFT JOIN (select id_sanzione_rp, rate_pagate_rp from get_registro_trasgressori_rate_pagate()) pagopa_no_rate_pagate on pagopa_no_rate_pagate.id_sanzione_rp = s.id_sanzione
    LEFT JOIN (select id_sanzione_trp, tutte_rate_pagate_trp from get_registro_trasgressori_tutte_rate_pagate()) pagopa_no_tutte_rate_pagate on pagopa_no_tutte_rate_pagate.id_sanzione_trp = s.id_sanzione

  
  GROUP BY s.anno_controllo, s.id_controllo, s.id_sanzione, s.data_accertamento, s.asl, s.ente1, s.ente2, s.ente3, s.numero_verbale, s.numero_verbale_sequestro, s.trasgressore, s.obbligato, s.importo_sanzione_ridotta, s.importo_sanzione_ultraridotta, rt.id, pagopa_pv_r.check, pagopa_pv_ur.check, pagopa_pv_r.data_esito_singolo_pagamento, pagopa_pv_ur.data_esito_singolo_pagamento, pagopa_no_rate_pagate.rate_pagate_rp, pagopa_no_tutte_rate_pagate.tutte_rate_pagate_trp
...
select * from registro_trasgressori_get_dati_registro(2023)
