...
in
 ...
 string_agg(DISTINCT concat_ws('#'::text, pagopa_pv.identificativo_univoco_versamento, pagopa_pv.url_file_avviso,    pagopa_pv.stato_pagamento), '##'::text) AS pagopa_pv_iuv,
  ...
  (SELECT COALESCE(get_registro_trasgressori_rate_pagate, '') FROM get_registro_trasgressori_rate_pagate(s.id_sanzione)) AS pagopa_no_rate_pagate, 
 (SELECT get_registro_trasgressori_tutte_rate_pagate FROM get_registro_trasgressori_tutte_rate_pagate(s.id_sanzione)) AS pagopa_no_tutte_rate_pagate, 
case when (SELECT get_registro_trasgressori_tutte_rate_pagate FROM get_registro_trasgressori_tutte_rate_pagate(s.id_sanzione)) is true then true else rt.pratica_chiusa end as pratica_chiusa
...
select * from registro_trasgressori_get_dati_registro(2021, 2, -1, -1) ;
