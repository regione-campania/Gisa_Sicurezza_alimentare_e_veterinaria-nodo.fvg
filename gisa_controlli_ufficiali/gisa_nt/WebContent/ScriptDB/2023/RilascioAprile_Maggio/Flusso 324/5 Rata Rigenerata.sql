alter table pagopa_pagamenti add column rigenerato boolean default false;

DROP FUNCTION IF EXISTS public.pagopa_genera_pagamento(integer, integer, text, text, text, text, integer, integer, integer);
...
when _tipopagamento = 'NO' and _rigenerato is true then ' - RATA '||_indice||' di RIGENERAZIONE '||(select count(*) from pagopa_sanzioni_importo_ordinanza where id_sanzione = _idsanzione and trashed_date is null)		  
else '' end) 
