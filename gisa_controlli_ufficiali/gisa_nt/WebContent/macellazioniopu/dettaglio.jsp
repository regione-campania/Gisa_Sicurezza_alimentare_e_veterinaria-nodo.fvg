...
<%@page import="org.aspcfs.modules.macellazioniopu.base.Campione"%><%@page import="org.aspcfs.modules.macellazioniopu.base.Organi"%>
<%@page import="org.aspcfs.modules.macellazioniopu.base.PatologiaRilevata"%>

<%@page import="org.aspcfs.modules.util.imports.ApplicationProperties"%>

<%@page import="org.aspcfs.modules.macellazioniopu.base.TipoRicerca"%><jsp:useBean id="Tampone"			class="org.aspcfs.modules.macellazioniopu.base.Tampone"	scope="request" />
...
function openPopupErrataCorrigeArt17(idCapo, altId, tipo){
        ...
        var result;
        	  ...
        	  window.open('MacellazioniDocumentiOpu.do?command=ViewModuleErrataCorrigeArt17&altId='+altId+'&idCapo='+idCapo+'&tipoMacello='+tipo,'popupSelect',
...
function openPopupListaErrataCorrigeArt17(idCapo, altId, tipo){
        ...
        var result;
        	  ...
        	  window.open('MacellazioniDocumentiOpu.do?command=ViewListaModuleErrataCorrigeArt17&altId='+altId+'&idCapo='+idCapo+'&tipoMacello='+tipo,'popupSelect',
              'height=800px,width=1280px,toolbar=no,directories=no,status=no,continued from previous linemenubar=no,scrollbars=yes,resizable=yes ,modal=yes');
...
function openPopupCancellazioneCapo(idCapo){
    var res;
    var result;
   	  window.open('MacellazioniOpu.do?command=PrepareCancellazioneCapo&idCapo='+idCapo,'popupSelect',
...
<table class="trails" cellspacing="0">
	<tr>
		<td>
			...
			%>
			<a href="MacellazioniOpu.do?command=List&altId=<%=OrgDetails.getAltId() %>">Macellazioni</a> > Dettaglio
		</td>
	</tr>
</table>
...
<dhv:permission name="stabilimenti-stabilimenti-macellazioni-view">
	<a href="MacellazioniOpu.do?command=PrintInvioCampioniTBC&file=invio_campioni_TBC.xml&<%=param2 %>">Invio campioni TBC</a>
</dhv:permission>

<dhv:permission name="stabilimenti-stabilimenti-macellazioni-view">
	<a href="MacellazioniOpu.do?command=PrintModelloLBE&file=modello_LBE.xml&<%=param2 %>">Modello LBE</a>
</dhv:permission>

<dhv:permission name="stabilimenti-stabilimenti-macellazioni-view">
	<a href="MacellazioniOpu.do?command=PrintModello1033TBC&file=modello_10_33_TBC.xml&<%=param2 %>">Modello 10 33 TBC</a>
</dhv:permission>
<%} %>
...
<dhv:permission name="stabilimenti-stabilimenti-macellazioni-edit">
	<input type="button" value="Modifica" onclick="location.href='MacellazioniOpu.do?command=ModificaCapo&cdSedutaMacellazione=<%=Capo.getCd_seduta_macellazione()%>&vpmData=<%=(Capo.getVpm_data()==null ? ("") : (toDateasString(Capo.getVpm_data())))%>&mavamData=<%=(Capo.getMavam_data()==null ? ("") : (toDateasString(Capo.getMavam_data())))%>&id=<%=Capo.getId() %>&altId=<%=OrgDetails.getAltId() %>'" />
</dhv:permission>

<dhv:permission name="stabilimenti-stabilimenti-macellazioni-delete">
	<input type="button" value="Elimina" onclick="if(confirm('Sicuro di voler eliminare la registrazione?')){location.href='MacellazioniOpu.do?command=Delete&id=<%=Capo.getId() %>&altId=<%=OrgDetails.getAltId() %>'}" />
</dhv:permission>
<%} else{ 
...
%>
<dhv:permission name="stabilimenti-stabilimenti-macellazioni-edit">
	<input type="button" value="Inserisci Esito Campioni" onclick="location.href='MacellazioniOpu.do?command=ModificaCapo&id=<%=Capo.getId() %>&altId=<%=OrgDetails.getAltId() %>'" />
</dhv:permission>
<%	
}else
{
