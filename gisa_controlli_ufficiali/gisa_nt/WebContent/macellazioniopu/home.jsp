...
function openPopup(url){
	...
	if(!confirm("Il capo selezionato sarà macellato in libero consumo in data "+dataMac+" con destinatario carni: <%=OrgDetails.getOperatore().getRagioneSociale()%>"))
		return false;
...
<table class="trails" cellspacing="0">
	<tr>
		...
		%> <a href="OpuStab.do?command=Search"><dhv:label
			...
			href="OpuStab.do?command=Details&stabId=<%=OrgDetails.getIdStabilimento()%>">Scheda
				Stabilimento</a> > <%
 	}
  ...
  <div id="tabellaSessioni" style="display:none">
   ...
   <table border="1" bordercolor="#729FCF" align="center"
	...
	<form name="macellazioniForm"
		action="MacellazioniOpu.do?command=AddSedutaMacellazione"
		method="post">
	...
	<br />
		...
		action="MacellazioniOpu.do?command=AddSedutaMacellazioneNuovaData"
		method="post">
		<input type="hidden" name="altId" value="<%=OrgDetails.getAltId()%>" />
...
<table border="1" bordercolor="#729FCF" align="center"
		...
		<tr>
			<td>
				...
				%>
				<form name="macellazioniForm" action="MacellazioniOpu.do?command=List"
					method="post">
...
if(request.getAttribute("sessioneCorrente")!=null)
{
%>		<a href="MacellazioniOpu.do?command=NuovoCapo&<%=param1%>&vpmData=<%=vpmData%>&cdSedutaMacellazione=<%=cdSedutaMacellazione%>"
...
<!--         <img src="gestione_documenti/images/pdf_icon.png" width="20"/>  <a href="#" onClick="openPopup('guida/guidaimportmacelli.pdf');	return false; ">  -->
...
%>
	...
	<dhv:permission name="stabilimenti-stabilimenti-macellazioni-view">
		<a
			href="MacellazioniDocumentiOpu.do?command=ToRegistroMacellazioni&<%=param1%>"
			style="text-decoration: none"><input type="button"
			value="Registro macellazioni" /></a>
	</dhv:permission>

	<dhv:permission name="stabilimenti-stabilimenti-macellazioni-view">
		<a href="MacellazioniDocumentiOpu.do?command=ToArt17&<%=param1%>"
			style="text-decoration: none"><input type="button"
			value="Articolo 17" /></a>
	</dhv:permission>

	<dhv:permission name="stabilimenti-stabilimenti-macellazioni-view">
		<a href="MacellazioniDocumentiOpu.do?command=ToMod10&<%=param1%>"
			style="text-decoration: none"><input type="button"
			value="Modello 10" /></a>
	</dhv:permission>
	...
	<dhv:permission name="stabilimenti-stabilimenti-macellazioni-liberoconsumomassivo-add">
		<a href="MacellazioniOpu.do?command=ToLiberoConsumoMassivo&<%=param1%>"
			style="text-decoration: none"><input type="button"
	...
	%>
	<dhv:permission name="stabilimenti-stabilimenti-macellazioni-view">
		<a href="MacellazioniDocumentiOpu.do?command=ToMortiStalla&<%=param1%>">Anim.
	...
	<dhv:permission name="stabilimenti-stabilimenti-macellazioni-view">
		<a href="MacellazioniDocumentiOpu.do?command=ToBSE&<%=param1%>">Modulo
			BSE</a>
	</dhv:permission>
	...
	<dhv:permission name="stabilimenti-stabilimenti-macellazioni-view">
		<a href="MacellazioniDocumentiOpu.do?command=ToAbbattimento&<%=param1%>">Abbattimento</a>
	</dhv:permission>
	...
	<dhv:permission name="stabilimenti-stabilimenti-macellazioni-view">
		<a
			href="MacellazioniOpu.do?command=PrintBRCRilevazioneMacelli&file=BRC_rilevazione_macelli.xml&<%=param1%>">BRC
			rilevazione macelli</a>
	</dhv:permission>

	<dhv:permission name="stabilimenti-stabilimenti-macellazioni-view">
		<a
			href="MacellazioniOpu.do?command=PrintTBCRilevazioneMacelli&file=TBC_rilevazione_macelli.xml&<%=param1%>">TBC
			rilevazione macelli</a>
	</dhv:permission>

	<dhv:permission name="stabilimenti-stabilimenti-macellazioni-view">
		<a href="MacellazioniOpu.do?command=ToModelloIdatidosi&<%=param1%>">Modello
			idatidosi</a>
	</dhv:permission>

	<dhv:permission name="stabilimenti-stabilimenti-macellazioni-view">
		<a
			href="MacellazioniOpu.do?command=PrintModelloMarchi&file=modello_marchi.xml&<%=param1%>">Modello
			marchi</a>
	</dhv:permission>

	<dhv:permission name="stabilimenti-stabilimenti-macellazioni-view">
		<a
			href="MacellazioniOpu.do?command=PrintMacellazioneAnimaliInfetti&file=macellazione_animali_infetti.xml&<%=param1%>">Macellazione
			animali infetti</a>
	</dhv:permission>

	<dhv:permission name="stabilimenti-stabilimenti-macellazioni-view">
		<a
			href="MacellazioniOpu.do?command=PrintDisinfezioneMezziTrasporto&file=disinfezione_mezzi_di_trasporto.xml&<%=param1%>">Disinfezione
...
<br/>
	...
	<form name="macellazioniForm" method="post"
		action="MacellazioniOpu.do?command=List">
		<input type="hidden" name="altId" value="<%=OrgDetails.getAltId()%>" />
	...
	<script type="text/javascript">
		...
		function onInvokeExportAction(id) {
			...
			//location.href = 'MacellazioniOpu.do?command=List&' + parameterString;
		}
	</script>
...
var dataCorrente = gg+mm+aaaa;
	if(document.getElementById('comboSessioniMacellazione2').value!="" && document.getElementById('comboSessioniMacellazione2NumSess').value!="")
	{
		if (confrontaDate(dataCorrente, document.getElementById('comboSessioniMacellazione2').value) < 0)
			...
			location.href="MacellazioniOpu.do?command=List&altId=<%=OrgDetails.getAltId()%>&comboSessioniMacellazione=" + document.getElementById('comboSessioniMacellazione2').value+"-"+document.getElementById('comboSessioniMacellazione2NumSess').value;
	}
	else
	{
