...
<jsp:useBean id="Tampone"			class="org.aspcfs.modules.macellazioniopu.base.Tampone"		scope="request" />
...
<%@page import="org.aspcfs.modules.macellazioniopu.base.Casl_Non_Conformita_Rilevata"%>
<%@page import="org.aspcfs.modules.macellazioniopu.base.ProvvedimentiCASL"%>
<%@page import="org.aspcfs.modules.macellazioniopu.base.Organi"%>
...
<jsp:useBean id="CapoLog"			class="org.aspcfs.modules.macellazioniopu.base.CapoLog"		scope="request" />
	...
	<tr>
		<td>
			...
			%>
					...
					<a href="OpuStab.do?command=Details&stabId=<%=OrgDetails.getIdStabilimento()%>">Scheda Stabilimento</a> >
			<%
					...
					%>
			<a href="MacellazioniOpu.do?command=List&altId=<%=OrgDetails.getAltId()%>">Macellazioni</a> > <%= update ? "Modifica Capo" : "Aggiungi Capo" %>
		</td>
	</tr>
...
<form name="main" action="MacellazioniOpu.do?command=<%=(update) ? ("Update") : ("Save") %>" method="post" onsubmit="return controllaForm( false );" >
            ...
            <tr>
             ...
             <tr class="containerBody">
            	<td class="formLabel">Codice Azienda di provenienza</td>
            	 <td>
                	<input type="text" onchange="searchBdn(this.value,'1')" name="cd_codice_azienda_provenienza" value="<%=toHtmlValue( Capo.getCd_codice_azienda_provenienza() ) %>" /><font color="red" >*</font>
					
				
<br>			
			<textarea rows="5" cols="33"  name = "cd_info_azienda_provenienza" id = "infoAzienda" ><%=toHtml2( Capo.getCd_info_azienda_provenienza())%></textarea>
...
<!--***************************************************************-->
	...
	<input id="liberoConsumoButton" type="button" value="Libero Consumo >>" onclick="if(controllaForm( true )){document.main.action='MacellazioniOpu.do?command=ToLiberoConsumo&cdSedutaMacellazione=<%=cdSedutaMacellazione%>&vpmData=<%=vpmData%>&mavamData=<%=mavamData%>';settaSoloCD('false');document.main.submit();};" />
...
<%--              	<zeroio:dateSelect field="vpm_data" form="main" showTimeZone="false" timestamp="<%=Capo.getVpm_data() %>" />  --%>
					<input readonly type="text" id="vpm_data" name="vpm_data"  size="10" value="" />&nbsp; 
