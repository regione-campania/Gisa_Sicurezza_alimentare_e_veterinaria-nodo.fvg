...
<a href="ImpiantiMacellazione.do?command=Dashboard">Cruscotto</a> >
<%}%>
...
<dhv:evaluate if="<%=OrgDetails.isTrashed()%>">
    <!--<dhv:permission name="impiantiMacellazione-impiantiMacellazione-edit">
      <input type="button" value="Ripristina"	onClick="javascript:window.location.href='ImpiantiMacellazione.do?command=Restore&orgId=<%= OrgDetails.getOrgId() %>';">
    </dhv:permission>-->
</dhv:evaluate>
...
<% if((OrgDetails.getCessato()==0) ||(OrgDetails.getCessato()==2)) {%>
  <dhv:evaluate if="<%=(OrgDetails.getEnabled())%>">
    <dhv:permission name="impiantiMacellazione-impiantiMacellazione-edit"><input type="button"  value="Modifica"	onClick="javascript:window.location.href='ImpiantiMacellazione.do?command=Modify&orgId=<%= OrgDetails.getOrgId() %><%= addLinkParams(request, "popup|actionplan") %>';"></dhv:permission>
  </dhv:evaluate>
  <%} %>
  <dhv:evaluate if="<%=!(OrgDetails.getEnabled())%>">
    <dhv:permission name="impiantiMacellazione-impiantiMacellazione-edit">
