...
window.location.href="GlobalSearch.do?command=StampaExportExcelCampioni";
}
</script>
...
%>
	...
	<% }else if( (thisOrg.getTipologia_operatore().toLowerCase().contains("anagrafica stabilimenti"))) {%> 
		<a id="<%= toHtml(thisOrg.getTicketId()) %>" href="OpuStabCampioni.do?command=TicketDetails&id=<%=thisOrg.getTicketId()%>&stabId=<%=thisOrg.getOrgId()%>"><%= toHtml(thisOrg.getTicketId()) %></a>						
	<%
		} else { %>
			N.D.
		<% } 
