...
<dhv:permission name="farmacosorveglianza-farmacosorveglianza-add"><a href="Farmacosorveglianza.do?command=AddFcie"><dhv:label name="">Aggiungi Operatore Farmaceutico</dhv:label></a></dhv:permission>
...
<table cellpadding="8" cellspacing="0" border="0" width="100%" class="pagedList">
  <tr>
    ...
    <th nowrap <% ++columnCount; %>>
      <%--strong><a href="Farmacosorveglianza.do?command=SearchFcie&column=o.ragione_sociale"--%><dhv:label name="">Impresa</dhv:label><%--/a></strong>
...
%>
  <tr class="row<%= rowid %>">
	...
	<td>
      <a href="Farmacosorveglianza.do?command=DetailsFcie&idFarmacia=<%=thisOrg.getIdFarmacia()%>"><%= toHtml(thisOrg.getRagioneSociale()) %></a>
	</td>
...
<%-- if(thisOrg.getAccountSize() > 0) {%>
    <td valign="top" nowrap>
      <%= OrgCategoriaRischioList.getSelectedValue(thisOrg.getAccountSize()) %>
       &nbsp;
       [<a href="Farmacosorveglianza.do?command=ModificaCatRischio&orgId=<%=thisOrg.getOrgId()%>"><dhv:label name="">Modifica</dhv:label></a>]
    </td>
<%}else { %>
	<td valign="top" nowrap>
      Nessuna
       &nbsp;
       [<a href="Farmacosorveglianza.do?command=ModificaCatRischio&orgId=<%=thisOrg.getOrgId()%>"><dhv:label name="accountsa.accounts_add.select">Aggiungi</dhv:label></a>]
    </td>
    <%} --%>
...
<%} else {%>
  <tr class="containerBody">
    <td colspan="<%= SearchOrgListInfoFcie.getSearchOptionValue("searchcodeOrgSiteId").equals(String.valueOf(Constants.INVALID_SITE))?columnCount+1:columnCount %>">
      <dhv:label name="">Nessun Operatore Farmaceutico trovato con i parametri di ricerca specificati.</dhv:label><br />
      <a href="Farmacosorveglianza.do?command=SearchFormFcie"><dhv:label name="">Modifica Ricerca</dhv:label></a>.
    </td>
  </tr>
<%}%>
</table>
