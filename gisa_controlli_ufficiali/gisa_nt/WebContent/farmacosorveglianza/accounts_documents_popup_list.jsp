...
<dhv:container name="parafarmacie" selected="documents" object="OrgDetails" hideContainer='<%= "true".equals(request.getParameter("actionplan")) %>' param='<%= "orgId=" + OrgDetails.getOrgId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId|actionplan") %>'>
  <%
    ...
    String documentFolderAdd ="FarmacieDocumentsFolders.do?command=Add&orgId="+OrgDetails.getOrgId();
    String documentFileAdd = "FarmacieDocuments.do?command=Add&orgId="+ OrgDetails.getOrgId();
    String documentFolderModify = "FarmacieDocumentsFolders.do?command=Modify&orgId="+ OrgDetails.getOrgId();
    String documentFolderList = "FarmacieDocuments.do?command=View&orgId="+OrgDetails.getOrgId()+"&popup=true";
    String documentFileDetails = "FarmacieDocuments.do?command=Details&orgId="+ OrgDetails.getOrgId();
...
%>    
  <tr>
    <td class="row<%= rowid %>" width="100%">
      ...
      <a href="FarmacieDocuments.do?command=Download&orgId=<%= OrgDetails.getOrgId() %>&folderId=<%= thisFile.getFolderId() %>&fid=<%= thisFile.getId() %>"><%= toHtml(thisFile.getSubject()) %></a>
    </td>
...
&nbsp;<br/>
<form method="post" name="inputForm" action="FarmacieDocuments.do?command=Upload&popup=true" onSubmit="return checkFileForm(this);" enctype="multipart/form-data">
