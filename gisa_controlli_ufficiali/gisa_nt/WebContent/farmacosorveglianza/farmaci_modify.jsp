...
<%@ page import="java.util.*,java.text.DateFormat,org.aspcfs.modules.farmacosorveglianza.base.Farmaci,org.aspcfs.controller.SystemStatus"%>
...
<body >
<form name="addAccount" action="FarmacosorveglianzaFar.do?command=UpdateFar&idFarmaco=<%= OrgDetails.getIdFarmaco() %>&auto-populate=true<%= (request.getParameter("popup") != null?"&popup=true":"") %>" onSubmit="return doCheck(this);" method="post">
<%
  boolean popUp = false;
...
<%-- if (request.getParameter("return") != null) {%>
	...
	<a href="Farmacosorveglianza.do?command=DashboardFar">Cruscotto</a> >
	<%}%>
<%} else {%>
<a href="Farmacosorveglianza.do?command=SearchFar"><dhv:label name="accounts.SearchResults">Search Results</dhv:label></a> >--%>
<a href="FarmacosorveglianzaFar.do?command=DetailsFar&idFarmaco=<%=OrgDetails.getIdFarmaco()%>"><dhv:label name="">Scheda Farmaco</dhv:label></a> >
<%--}--%>
...
<dhv:container name="farmacosorveglianza" selected="details" object="OrgDetails" param='<%= "idFarmaco=" + OrgDetails.getIdFarmaco() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId") %>'>
...
<% if (request.getParameter("return") != null && "list".equals(request.getParameter("return"))) {%>
  <input type="submit" value="Annulla" onClick="javascript:this.form.action='FarmacosorveglianzaFar.do?command=DetailsFar&idFarmaco=<%= OrgDetails.getIdFarmaco() %>';this.form.dosubmit.value='false';" />
