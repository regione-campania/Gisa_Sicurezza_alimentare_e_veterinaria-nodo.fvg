...
<script>
function stampaAllegatoI(idAsl, anno){
	if (idAsl!='-1')
		window.open('Farmacosorveglianza.do?command=StampaAllegatoI&asl='+idAsl+'&anno='+anno);
	else
		window.open('Farmacosorveglianza.do?command=StampaAllegatoI&anno='+anno);
}
</script>
...
<br>
		...
		<td>
		<table align="left">
			<tr><td><a href = "#" onClick="stampaAllegatoI('<%=user.getSiteId() %>', '<%=request.getAttribute("anno") %>');">Stampa Report</a></td></tr>
			</table>
	...
	<h2><font color = "red">ATTENZIONE! DOPO AVER COMPILATO L'ALLEGATO I CLICCARE SULL'IMMAGINE DEL DISCHETTO PER SALVARE IL LAVORO ESEGUITO. QUANDO IL LAVORO è COMPLETATO E QUINDI è STATO SALVATO CLICCANDO SUL DISCHETTO SI PUO CHIUDERE L'ALLEGATO CLICCANDO SUL LINK IN BASSO "CHIUDI PRATICA"</font></h2>
			<center><table>
		<tr><td colspan="2">
...
HashMap<Integer,ArrayList<RigaAllegatoI>> collezione = (HashMap<Integer,ArrayList<RigaAllegatoI>>) request.getAttribute("AllegatoI");
			...
			%>
			<table align="left" >
			<tr><td><a href = "#" onClick="stampaAllegatoI('-1', '<%=request.getAttribute("anno") %>')">Stampa Report</a></td></tr>
			</table>
			<br/>
			<center><table>
		<tr><td colspan="2">
