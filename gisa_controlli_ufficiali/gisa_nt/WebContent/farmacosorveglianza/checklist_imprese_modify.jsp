...
<form name="addAccountAudit"  method="post" action="CheckListFarmacosorveglianza.do?command=Update&auto-populate=true&return<%= request.getParameter("return") %>" onSubmit="return checkForm();">
<%-- Trails --%>
...
<a href="Farmacosorveglianza.do?command=SearchFcie"><dhv:label name="accounts.SearchResults">Search Results</dhv:label></a> >
 ...
 <a href="CheckListFarmacosorveglianza.do?command=View&id=<%= Audit.getId() %>"><dhv:label name="accounts.Audit">Check List</dhv:label></a> >
...
<dhv:container name="farmacosorveglianza" selected="audit" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId() %>'>


  <input type="button" value="Annulla" onClick="window.location.href='CheckListFarmacosorveglianza.do?command=View&id=<%=Audit.getId()%>';this.form.dosubmit.value='false';" />
