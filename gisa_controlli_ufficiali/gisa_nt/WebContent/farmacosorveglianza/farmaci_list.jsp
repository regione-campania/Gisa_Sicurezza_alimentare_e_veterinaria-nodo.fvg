...
<jsp:useBean id="OrgList" class="org.aspcfs.modules.farmacosorveglianza.base.FarmaciList" scope="request"/>
...
<dhv:permission name="farmacosorveglianza-farmacosorveglianza-add"><a href="FarmacosorveglianzaFar.do?command=AddFar"><dhv:label name="">Aggiungi Farmaco</dhv:label></a></dhv:permission>
...
<table cellpadding="8" cellspacing="0" border="0" width="100%" class="pagedList">
  <tr>
    ...
    <th nowrap <% ++columnCount; %>>
      <%--strong><a href="FarmacosorveglianzaFar.do?command=SearchFar&column=o.ragione_sociale"--%><dhv:label name="">Nome Commerciale</dhv:label><%--/a></strong>
...
<zeroio:debug value='<%="JSP::accounts_list.jsp "+ SearchOrgListInfoFcie.getSearchOptionValue("searchcodeOrgSiteId")+" == "+(String.valueOf(Constants.INVALID_SITE)) %>'/>
      <%--dhv:evaluate if='<%= SearchOrgListInfoFcie.getSearchOptionValue("searchcodeOrgSiteId").equals(String.valueOf(Constants.INVALID_SITE)) %>'--%>
        <th <% ++columnCount; %>>
          <strong><dhv:label name="">Tipo Somministrazione</dhv:label></strong>
        </th>
        <th <% ++columnCount; %>>
          <strong><dhv:label name="">Confezione</dhv:label></strong>
        </th>
      <%--/dhv:evaluate--%>
<%--    </dhv:include> --%>
...
<%
    ...
    i++;
    rowid = (rowid != 1 ? 1 : 2);
    Farmaci thisOrg = (Farmaci)j.next();
%>
  <tr class="row<%= rowid %>">
    
	<td>
      <a href="FarmacosorveglianzaFar.do?command=DetailsFar&idFarmaco=<%=thisOrg.getIdFarmaco()%>"><%= toHtml(thisOrg.getNomeCommerciale()) %></a>
	</td>
      
	<td nowrap>
      <%= toHtml(thisOrg.getPrincipioAttivo()) %>&nbsp;
	</td>
	<td nowrap>
      <%= idTiposomministrazione.getSelectedValue(thisOrg.getIdTiposomministrazione()) %>&nbsp;
	</td>
	<td nowrap>
      <%= idConfezione.getSelectedValue(thisOrg.getIdConfezione()) %>&nbsp;
	</td>
	<%-- %><td nowrap>
       <%= toHtml(thisOrg.getTipoSomministrazione()) %>
	</td>--%>
<%-- if(thisOrg.getAccountSize() > 0) {%>
    <td valign="top" nowrap>
      <%= OrgCategoriaRischioList.getSelectedValue(thisOrg.getAccountSize()) %>
       &nbsp;
       [<a href="FarmacosorveglianzaFar.do?command=ModificaCatRischio&orgId=<%=thisOrg.getOrgId()%>"><dhv:label name="">Modifica</dhv:label></a>]
    </td>
<%}else { %>
	<td valign="top" nowrap>
      Nessuna
       &nbsp;
       [<a href="FarmacosorveglianzaFar.do?command=ModificaCatRischio&orgId=<%=thisOrg.getOrgId()%>"><dhv:label name="accountsa.accounts_add.select">Aggiungi</dhv:label></a>]
    </td>
    <%} --%>
...
<%} else {%>
  <tr class="containerBody">
    <td colspan="<%= SearchOrgListInfoFcie.getSearchOptionValue("searchcodeOrgSiteId").equals(String.valueOf(Constants.INVALID_SITE))?columnCount+1:columnCount %>">
      <dhv:label name="">Nessun Farmaco trovato con i parametri di ricerca specificati.</dhv:label><br />
      <a href="FarmacosorveglianzaFar.do?command=SearchFormFar"><dhv:label name="">Modifica Ricerca</dhv:label></a>.
    </td>
  </tr>
<%}%>
</table>
