...
<body onload="verificaChiusuraNc(<%=request.getAttribute("Chiudi")%>,<%=request.getAttribute("numSottoAttivita")%>,'<%=request.getAttribute("Messaggio2")%>',<%=TicketDetails.getIdControlloUfficiale() %>,'LabHaccpNonConformita')">

<form name="details" action=LabHaccpNonConformita.do?command=ModifyTicket&auto-populate=true" method="post">
<%-- Trails --%>
...
<tr>
<td>
  <a href="LaboratoriHACCP.do">Laboratori</a> > 
  <a href="LaboratoriHACCP.do?command=Search"><dhv:label name="accounts.SearchResults">Search Results</dhv:label></a> >
  <a href="LaboratoriHACCP.do?command=Details&orgId=<%=TicketDetails.getOrgId()%>">Scheda Laboratorio</a> >
