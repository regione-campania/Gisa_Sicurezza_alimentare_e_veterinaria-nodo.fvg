...
<b>Non conformita' <u>significative/minori</u> rilevate</b>: <label class="layout"><%= toHtmlFill(mod.getControlloNcSignificative(), 20)%></label><br/>
Punteggio*: <label class="layout"><%= toHtmlFill(mod.getControlloNcSignificativePunti(), 2)%></label>  <br/>
Valutazione del rischio caratterizzato dalla presenza delle n.c. minori (motivazioni di fatto) <label class="layout"><%= toHtmlFill(mod.getControlloNcSignificativeVal(), 50)%></label> <br/>
Follow up delle non conformita'  minori (dispositivo del provvedimento amministrativo): visto il Reg. UE 2017/625, il Reg UE 2016/429, la L. 241/90, il D.L.vo 27/21 ed il P.C.R.P., s. si impone la risoluzione delle non conformita' entro il termine di n. <input class="editField" type="text" name="ncSignificativeGiorniRisoluzione" id="ncSignificativeGiorniRisoluzione" value="<%=toHtml(mod.getNcSignificativeGiorniRisoluzione())%>" size="2" maxlength="3" onkeyup="this.value=this.value.replace(/[^0-9]+/,'')"/> giorni dalla data odierna (per il diritto alla difesa: vedi in basso). <br/><br/>
Fermo restando che la mancata risoluzione delle non conformita' nel termine concesso comporta la contestazione di un illecito amministrativo, si comunica che:<br>
<input type="checkbox" id="ncSignificativeUlterioreIspezione" name="ncSignificativeUlterioreIspezione" value="S" <%=(mod!=null && toHtml(mod.getNcSignificativeUlterioreIspezione()).equals("S")) ? "checked=\"checked\"" : ""%>/> alla scadenza del termine, si procedera' ad una ulteriore ispezione per la verifica della risoluzione delle non conformita'. <br>
<input type="checkbox" id="ncSignificativeProveInviate" name="ncSignificativeProveInviate" value="S" <%=(mod!=null && toHtml(mod.getNcSignificativeProveInviate()).equals("S")) ? "checked=\"checked\"" : ""%>/> le prove dell'avvenuta risoluzione dovranno essere inviate, esclusivamente via pec, all'indirizzo pec dell'ufficio riportato in intestazione; esse possono consistere in documenti, attestati, foto, filmati o qualsiasi altro efficace atto scritto o supporto. <br>
<input type="checkbox" id="ncSignificativeVerifica" name="ncSignificativeVerifica" value="S" <%=(mod!=null && toHtml(mod.getNcSignificativeVerifica()).equals("S")) ? "checked=\"checked\"" : ""%>/> si e' concordato con la parte che la verifica avverra' in data <input class="editField" type="date" name="ncSignificativeVerificaData" id="ncSignificativeVerificaData" value="<%=toHtml(mod.getNcSignificativeVerificaData())%>" size="10"/> alle ore <input class="editField" type="time" name="ncSignificativeVerificaOra" id="ncSignificativeVerificaOra" value="<%=toHtml(mod.getNcSignificativeVerificaOra())%>" size="5" maxlength="5"/> in videochiamata sulla piattaforma <input class="editField" type="text" name="ncSignificativeVerificaPiattaforma" id="ncSignificativeVerificaPiattaforma" value="<%=toHtml(mod.getNcSignificativeVerificaPiattaforma())%>" size="10"/> nella stanza virtuale <input class="editField" type="text" name="ncSignificativeVerificaStanza" id="ncSignificativeVerificaStanza" value="<%=toHtml(mod.getNcSignificativeVerificaStanza())%>" size="10"/><br>
<b>Non conformita' <u>gravi/maggiori</u> rilevate</b>: <label class="layout"><%= toHtml(mod.getControlloNcGravi())%></label><br/>
...
2) <input type="checkbox" id="ncGraviAccertamento" name="ncGraviAccertamento" value="S" <%=(mod!=null && toHtml(mod.getNcGraviAccertamento()).equals("S")) ? "checked=\"checked\"" : ""%>/> si procedera', se del caso e/o previo ulteriori accertamenti, alla notifica di illecito/i amministrativo/i  con atto/i  a parte<br/>
3) <input type="checkbox" id="ncGraviDiffidaEliminazione" name="ncGraviDiffidaEliminazione" value="S" <%=(mod!=null && toHtml(mod.getNcGraviDiffidaEliminazione()).equals("S")) ? "checked=\"checked\"" : ""%>/> visto il D.L. 91/14 si diffida l'€™operatore a eliminare la non conformita'  grave n. <input class="editField" type="text" name="ncGraviDiffidaEliminazioneNumero" id="ncGraviDiffidaEliminazioneNumero" value="<%=toHtml(mod.getNcGraviDiffidaEliminazioneNumero())%>" size="10"/> entro il termine di trenta giorni da oggi. Avverso tale diffida possono essere inviati scritti difensivi a dg04.uod02@pec.regione.campania.it<br/>
...
6) <input type="checkbox" id="ncGraviBlocco" name="ncGraviBlocco" value="S" <%=(mod!=null && toHtml(mod.getNcGraviBlocco()).equals("S")) ? "checked=\"checked\"" : ""%>/> visto il Reg. UE 2017/625, il Reg UE 2016/429, la L. 241/90, il D.L.vo 27/21, la L 283/62, il DPR 327/80 ed il P.C.R.P. si e' proceduto al blocco/sequestro sanitario delle cose/animali elencate nel/i verbale/in. <input class="editField" type="text" name="ncGraviBloccoDesc" id="ncGraviBloccoDesc" value="<%=toHtml(mod.getNcGraviBloccoDesc())%>" size="10"/> che forma parte integrante del presente provvedimento amministrativo<br/>  
7) <input type="checkbox" id="ncGraviDispone" name="ncGraviDispone" value="S" <%=(mod!=null && toHtml(mod.getNcGraviDispone()).equals("S")) ? "checked=\"checked\"" : ""%>/> visto Reg. UE 2017/625, il Reg UE 2016/429, la L. 241/90, il D.L.vo 27/21, la L 283/62, il DPR 327/80 ed il P.C.R.P. si dispone che (dispositivo del provvedimento amministrativo):<input class="editField" type="text" name="ncGraviDisponeDesc" id="ncGraviDisponeDesc" value="<%=toHtml(mod.getNcGraviDisponeDesc())%>" size="20"/><br/>
Valutazione del rischio caratterizzato dalla presenza delle non conformita' gravi/maggiori (motivazioni di fatto da redigere solo nei sopraelencati casi <b>6)</b> e <b>7</b>)):<br/>
...
(diritto alla difesa nei casi <b>6)</b> e <b>7)</b>: vedi sotto)<br/> 
...
Ai sensi degli art. 21 bis e quater L. 241/90 il presente provvedimento ha efficacia immediata in quanto cautelare e urgente. Essendo susseguente ad ispezione, al presente provvedimento si applica la deroga ex art.7 p. 1 L. 241/90 in materia di comunicazione di avvio del procedimento. Le informazioni sul trattamento dei dati personali sono consultabili sul seguente link: http://www.gisacampania.it/gisasuap.html. <br>
Note: <input class="editField" type="text" name="note" id="note" value="<%=toHtml(mod.getNote())%>" size="50"/><br/>
Consci delle sanzioni previste per le dichiarazioni mendaci, gli ispettori dichiarano che per nessuno di loro esistono conflitti di interesse nello svolgimento della presente ispezione.<br>
...
<u>Diritto alla difesa:</u>avverso il presente provvedimento amministrativo si puo': <b>1)</b> richiedere all'ufficio di annullare/modificare il provvedimento in autotutela ex art. 21 nonies L. 241/80 <b>2)</b> presentare ricorso gerarchico ex DPR 1199/71 entro 30 giorni da oggi, inviandolo esclusivamente tramite pec  agli indirizzi indi cati in intestazione <b><3)</b> ricorrere al TAR entro 60 giorni da oggi  <b>4)</b> presentare ricorso straordinario al Presidente della Repubblica entro 120 giorni da oggi<br/>
N.C. = Non Conformita'              -                     n. 7 punti per ogni n.c. significativa, n. 50 punti per ogni n.c. grave.<br/><br/>   
...
<u>RISULTANZE DELL'ISPEZIONE:</u> limitatamente per quanto riguarda gli oggetti dell'ispezione, non si sono riscontrate non conformita'. 
Le informazioni sul trattamento dei dati personali sono consultabili sul seguente link: http://www.gisacampania.it/gisasuap.html. Consci delle sanzioni previste per le dichiarazioni mendaci, gli ispettori dichiarano che per nessuno di loro esistono conflitti d'interesse nello svolgimento della presente ispezione. <br>
Note: <input class="editField" type="text" name="note" id="note" value="<%=toHtml(mod.getNote())%>" size="50"/><br/>
...
<input type="hidden" id="rev" name="rev" value="9"/>  
