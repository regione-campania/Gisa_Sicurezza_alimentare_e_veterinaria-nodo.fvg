...
<%@ page import="java.util.*,com.zeroio.iteam.base.*,org.aspcfs.modules.osa.base.*" %>
...
<table cellpadding="0" cellspacing="0" width="100%" border="1" rules="cols">
  <tr class="section">
    ...
    <dhv:evaluate if="<%= FileFolder.getParentId() != -1 %>">
      <a href="OsAnimaliDocumentsFolders.do?command=SaveMove&orgId=<%= OrgDetails.getOrgId() %>&id=<%= FileFolder.getId() %>&popup=true&folderId=0&return=OsAnimaliDocuments&param=<%= OrgDetails.getOrgId() %>&param2=<%= FileFolder.getParentId() %>"><dhv:label name="accounts.accounts_documents_file_move.Home">Home</dhv:label></a>
    </dhv:evaluate>
    <dhv:evaluate if="<%= FileFolder.getParentId() == -1 %>">
...
%>    
  <tr class="row<%= rowid %>">
    ...
    <dhv:evaluate if="<%= FileFolder.getParentId() != thisFolder.getId() %>">  
      <a href="OsAnimaliDocumentsFolders.do?command=SaveMove&orgId=<%= OrgDetails.getId() %>&id=<%= FileFolder.getId() %>&popup=true&folderId=<%= thisFolder.getId() %>&return=OsAnimaliDocuments&param=<%= OrgDetails.getOrgId() %>&param2=<%= FileFolder.getParentId() %>"><%= toHtml(thisFolder.getSubject()) %></a>
    </dhv:evaluate>
    <dhv:evaluate if="<%= FileFolder.getParentId() == thisFolder.getId() %>">
