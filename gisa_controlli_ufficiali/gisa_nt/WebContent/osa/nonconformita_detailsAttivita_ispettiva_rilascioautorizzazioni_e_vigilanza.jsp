...
<body onload="verificaChiusuraNc(<%=request.getAttribute("Chiudi")%>,<%=request.getAttribute("numSottoAttivita")%>,'<%=request.getAttribute("Messaggio2")%>',<%=TicketDetails.getIdControlloUfficiale() %>,'OSAnimaliVigilanza')">

<form name="details" action=OSAnimaliNonConformita.do?command=ModifyTicket&auto-populate=true" method="post">
<%-- Trails --%>
...
<tr>
<td>
  <a href="OsAnimali.do">Laboratori</a> > 
  <a href="OsAnimali.do?command=Search"><dhv:label name="accounts.SearchResults">Search Results</dhv:label></a> >
  <a href="OsAnimali.do?command=Details&orgId=<%=TicketDetails.getOrgId()%>">Scheda Osa</a> >
