...
<%@page import="org.aspcfs.modules.osa.base.Organization"%>
...
<form name="viewAccountAudit" action="CheckListOsa.do?command=Modify&auto-populate=true&id=<%=Audit.getId()%>&return=details<%= addLinkParams(request, "popup|popupType|actionId") %>" method="post">
...
<td width="100%">
  ...
  <a href="OsAnimali.do?command=Details&orgId=<%= OrgDetails.getOrgId() %>">Scheda Operatore Sperimentazione Animale</a> >
  <a href="OsAnimali.do?command=ViewVigilanza&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label name="">Controlli Ufficiali</dhv:label></a> >
      <a href="OSAnimaliVigilanza.do?command=TicketDetails&id=<%=Audit.getIdControllo()%>&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label name="">Controllo Ufficiale</dhv:label></a> >
    ...
    <tr>
       <td nowrap align="right">
        ...
        <%--<a href="Accounts.do?command=PrintReport&id=<%= OrgDetails.getId() %>"><dhv:label name="accounts.osa.print">Stampa Scheda Audit</dhv:label></a>--%>
         ...
         <input type="button" value="Stampa Scheda Check List"	onClick='javascript:location.href="CheckListOsa.do?command=Stampa&id=<%= Audit.getId() %>";'>
      <%
    }
    else {
    %>
...
<dhv:evaluate if="<%= !OrgDetails.isTrashed() %>">
  <input type="hidden" name="idC" value="<%=Audit.getIdControllo() %>">
     ...
     <dhv:permission name="osa-osa-audit-edit"><input type="submit" value="Modifica"></dhv:permission>
