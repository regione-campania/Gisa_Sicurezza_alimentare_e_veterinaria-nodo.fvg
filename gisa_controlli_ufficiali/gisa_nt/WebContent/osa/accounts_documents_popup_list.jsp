...
<%@ page import="java.util.*,java.text.DateFormat,org.aspcfs.modules.osa.base.*,com.zeroio.iteam.base.*" %>
...
<dhv:container name="osa" selected="documents" object="OrgDetails" hideContainer='<%= "true".equals(request.getParameter("actionplan")) %>' param='<%= "orgId=" + OrgDetails.getOrgId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId|actionplan") %>'>
  <%
    String permission_doc_folders_add ="osa-osa-documents-add";
    String permission_doc_files_upload = "osa-osa-documents-add";
    String permission_doc_folders_edit = "osa-osa-documents-edit";
    String documentFolderAdd ="OsAnimaliDocumentsFolders.do?command=Add&orgId="+OrgDetails.getOrgId();
    String documentFileAdd = "OsAnimaliDocuments.do?command=Add&orgId="+ OrgDetails.getOrgId();
    String documentFolderModify = "OsAnimaliDocumentsFolders.do?command=Modify&orgId="+ OrgDetails.getOrgId();
    String documentFolderList = "OsAnimaliDocuments.do?command=View&orgId="+OrgDetails.getOrgId()+"&popup=true";
    String documentFileDetails = "OsAnimaliDocuments.do?command=Details&orgId="+ OrgDetails.getOrgId();
...
%>    
  <tr>
    <td class="row<%= rowid %>" width="100%">
      ...
      <a href="OsAnimaliDocuments.do?command=Download&orgId=<%= OrgDetails.getOrgId() %>&folderId=<%= thisFile.getFolderId() %>&fid=<%= thisFile.getId() %>"><%= toHtml(thisFile.getSubject()) %></a>
    </td>
...
&nbsp;<br/>
<form method="post" name="inputForm" action="OsAnimaliDocuments.do?command=Upload&popup=true" onSubmit="return checkFileForm(this);" enctype="multipart/form-data">
