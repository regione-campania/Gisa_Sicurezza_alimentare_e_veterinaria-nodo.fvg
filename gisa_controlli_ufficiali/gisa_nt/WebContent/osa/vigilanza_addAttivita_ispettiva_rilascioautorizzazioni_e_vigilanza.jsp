...
<body onload = "provaFunzione('addticket'); resetElementiNucleoIspettivo('<%=TicketDetails.getNucleoasList().size() %>'); costruisci_rel_ateco_attivita('<%= OrgDetails.getOrgId() %>', 'id_linea_sottoposta_a_controllo'); ">
 ...
 TitoloNucleoDue.setJsEvent("onChange=mostraCampo2('addticket')");
    ...
    TipoCampione.setJsEvent("onChange=javascript:reloadAddCU('OSAnimaliVigilanza.do?command=Add&orgId="+OrgDetails.getOrgId()+"&tipoCampione='+this.value)"); 
	

%>


<form method="post" name="addticket" action="OSAnimaliVigilanza.do?command=Insert&auto-populate=true">
<%-- Trails --%>
...
<input type="submit" value="Annulla" onClick="javascript:this.form.action='OsAnimali.do?command=ViewVigilanza&orgId=<%=OrgDetails.getOrgId()%>'">
<br>
