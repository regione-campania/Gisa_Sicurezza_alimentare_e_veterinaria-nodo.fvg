...
<form method="POST" name="inputForm" action="OsAnimaliDocumentsFolders.do?command=Save&auto-populate=true<%= addLinkParams(request, "popup|popupType|actionId|actionplan") %>" onSubmit="return checkForm(this);">
...
<a href="OsAnimaliDocuments.do?command=List&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label name="accounts.accounts_documents_details.Documents">Documents</dhv:label></a> >
...
<br>
  ...
  <input type="submit" value="Annulla" onClick="javascript:this.form.dosubmit.value='false';this.form.action='OsAnimaliDocuments.do?command=View&orgId=<%= OrgDetails.getOrgId() %><%= addLinkParams(request, "popup|popupType|actionId|actionplan") %>';" /><br />
  <dhv:formMessage />
  <br />
