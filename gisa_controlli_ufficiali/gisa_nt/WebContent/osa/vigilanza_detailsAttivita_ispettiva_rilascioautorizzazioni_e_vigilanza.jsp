...
<jsp:useBean id="descrizioneTipiChecklist" class="java.util.HashMap" scope="request"/>
...
<script language="JavaScript" TYPE="text/javascript" SRC="javascript/controlli_ufficiali_osa.js"></script>
...
<form name="details" action="OSAnimaliVigilanza.do?command=ModifyTicket&auto-populate=true" method="post">
...
<table cellpadding="4" cellspacing="0" width="100%" class="empty">
  ...
  <tr>
   ...
   <td>
      ...
      <a href="CheckListOsa.do?command=View&id=<%= thisAudit.getId()%>&aggiorna=<%=thisAudit.isAggiornaCategoria() %>"><%= toHtml((String)descrizioneTipiChecklist.get(thisAudit.getTipoChecklist())) %></a>
	</td>
	<%}else if(TicketDetails.getTipoCampione()==5){ %>
	 <td>
	    
	   <a href="CheckListOsa.do?command=View&id=<%= thisAudit.getId()%>&aggiorna=<%=thisAudit.isAggiornaCategoria() %>"><%= toHtml(OrgCategoriaRischioList2.getSelectedValue(thisAudit.getTipoChecklist())) %></a>
	</td>
	<%} %>
