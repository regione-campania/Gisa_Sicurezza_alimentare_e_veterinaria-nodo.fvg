...
<form name="details" action="<%=OrgDetails.getPrefissoAction("")%>AltreNonConformita.do?command=UpdateTicket&auto-populate=true<%=addLinkParams(request, "popup|popupType|actionId")%>" onSubmit="return checkForm(this);" method="post">
...
<td>
  ...
  <a href="<%=TicketDetails.getURlDettaglio()%>.do?command=Details&altId=<%=OrgDetails.getAltId()%>"><dhv:label name="">Scheda Operatore</dhv:label></a> > 
 <a href="<%=TicketDetails.getURlDettaglio()%>.do?command=ViewVigilanza&altId=<%=OrgDetails.getAltId()%>"><dhv:label name="">Controlli Ufficiali</dhv:label></a> >
<a href="<%=TicketDetails.getURlDettaglio()%>Vigilanza.do?command=TicketDetails&id=<%=request.getAttribute("idC")%>&altId=<%=OrgDetails.getAltId()%>"><dhv:label name="">Controllo Ufficiale</dhv:label></a> >
...
<dhv:container name="<%=TicketDetails.getContainer()%>" selected="vigilanza" object="Operatore" param='<%="altId=" + OrgDetails.getAltId()%>' appendToUrl='<%=addLinkParams(request, "popup|popupType|actionId")%>'>
  ...
  <dhv:evaluate if="<%=SiteIdList.size() <= 1%>">
    ...
    <td>
	  ...
	  <%@ include file="../nonconformita/alt_provvedimenti_edit.jsp" %>
