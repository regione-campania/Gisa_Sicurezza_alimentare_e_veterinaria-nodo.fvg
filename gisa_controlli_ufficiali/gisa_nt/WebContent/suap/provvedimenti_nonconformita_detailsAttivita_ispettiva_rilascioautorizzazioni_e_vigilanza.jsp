...
<form name="details" action="<%=OrgDetails.getAction()%>AltreAltreNonConformita.do?command=ModifyTicket&auto-populate=true" method="post">
<%-- Trails --%>
...
%>
<dhv:container name="suaprichieste" selected="vigilanza" object="Operatore" param='<%="altId=" + OrgDetails.getAltId()%>' hideContainer='<%=isPopup(request) || (defectCheck != null && !"".equals(defectCheck.trim()))%>'>
	...
	<%@ include file="../controlliufficiali/alt_header_provvedimenti.jsp" %>
...
<dhv:evaluate if="<%=hasText(TicketDetails.getCause())%>">
  ...
  <%@ include file="../nonconformita/alt_provvedimenti_view.jsp" %>
