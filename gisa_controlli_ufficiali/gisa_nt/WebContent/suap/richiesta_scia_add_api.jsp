...
<a href="GisaSuapStab.do?command=Default"><dhv:label name="">Pratiche SUAP</dhv:label></a> > Inserimento
</td>
</tr>
</table>
<%}%>
...
asfsadfasdfasdfadsfsadfsdafadfs
...
<form method="post" id="example-advanced-form" name="addstabilimento"  action="#" enctype="multipart/form-data" acceptcharset="UTF-8">
        ...
        <input type = "text" name="dataRichiesta" value = "<%= toDateasString(newStabilimento.getDataRichiestaSciaAsl()) %>">
		...
		<input type="text" name="descrizioneProvincia" id="descrizioneProvincia" value="<%=User.getUserRecord().getSuap().getDescrizioneProvincia() %>">
		<input type="text" name="idProvinciaSuap" id="idProvinciaSuap" value="<%=User.getUserRecord().getSuap().getIdProvinciaSuap() %>">
        <input type="text" name="fissa" id="fissa" value="<%=fissa%>">
        
        <input type="text" name = "tipoCarattere" id = "tipoCarattere" value="1">
        ...
        <fieldset>
                ...
                %>
<select name = "tipo_impresa" id = "tipo_impresa" required="required" onchange="onChangeTipoImpresa();">
		<option value="" selected="selected">seleziona</option>
		<%
		for (int i = 0 ; i <TipoImpresaList.size(); i++ )
		{
			LookupElement le = (LookupElement)TipoImpresaList.get(i);
			%>
			<option value="<%=le.getCode() %>"><%=le.getDescription() %></option>
		...
		%>
                ...
                <br><br>
                <center>
                <input type = "button" value = "Torna Indietro" onclick="location.href='SuapStab.do?command=Scelta&tipoInserimento=<%=newStabilimento.getTipoInserimentoScia() %>&stato=<%=newStabilimento.getStato() %>&fissa=<%=fissa%>&dataRichiesta=<%= toDateasString(newStabilimento.getDataRichiestaSciaAsl()) %>'">
                </center>
                
                 <input type="hidden" name="tipoAttivita" id="tipoAttivita" value ="<%= ((LookupElement)TipoAttivita.get(0)).getCode()%>">
           		
           		<input type ="hidden" name="idLineaProduttiva" id="idLineaProduttiva" value="<%= ((LineaProduttiva)newStabilimento.getListaLineeProduttive().get(0)).getId()%>">
