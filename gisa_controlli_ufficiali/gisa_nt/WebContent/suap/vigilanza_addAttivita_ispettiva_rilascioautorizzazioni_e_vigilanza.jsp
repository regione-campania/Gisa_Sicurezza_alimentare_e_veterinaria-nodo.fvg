	...
	function load_linee_attivita_per_alt_id_callback(returnValue) {
	  ...
	  function costruisci_rel_ateco_attivita( alt_id, campo_combo_da_costruire ) {
		 ...
		 // alert(campo_combo_da_costruire);
		  PopolaCombo.load_linee_attivita_per_alt_id(alt_id , campo_combo_da_costruire, null, load_linee_attivita_per_alt_id_callback)
	  }
...
<body onload = "provaFunzione('addticket'); gestioneVisibilitaCodiceAteco('addticket');onloadAllerta('addticket'); resetElementiNucleoIspettivo('<%=TicketDetails.getNucleoasList().size() %>'); costruisci_rel_ateco_attivita('<%= OrgDetails.getAltId() %>', 'id_linea_sottoposta_a_controllo'); ">
 ...
 <%
    ...
    TipoCampione.setJsEvent("onChange=javascript:reloadAddCU('GisaSuapStabVigilanza.do?command=Add&altId=" + OrgDetails.getAltId() + "&opId="  + OrgDetails.getIdOperatore() +"&tipoCampione='+this.value)"); 
