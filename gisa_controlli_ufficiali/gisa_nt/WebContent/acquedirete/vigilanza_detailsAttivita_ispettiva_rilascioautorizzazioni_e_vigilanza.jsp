...
<jsp:useBean id="OrgListOp" class="org.aspcfs.modules.acquedirete.base.OrganizationList" scope="request"/>
...
<script language="JavaScript" TYPE="text/javascript" SRC="javascript/controlli_ufficiali_acquedirete.js"></script>
...
<%-- End Trails --%>
<%
String param1 = "id=" + TicketDetails.getId()+"&orgId="+OrgDetails.getOrgId()+"&idControllo="+TicketDetails.getIdControlloUfficiale();
...
<table cellpadding="4" cellspacing="0" width="100%" class="empty">
  ...
  <tr>
   ...
   <%@ include file="../controlliufficiali/controlli_ufficiali_allarmerapido_view.jsp" %>
	...
	<%@ include file="../controlliufficiali/controlli_ufficiali_acque_di_rete_view.jsp" %>
<br>
   ...
   <tr class="row<%=rowid%>">
	 ...
	 <td>
	   ...
	   <a href="CheckListAcqueRete.do?command=View&id=<%= thisAudit.getId()%>&aggiorna=<%=thisAudit.isAggiornaCategoria() %>"><%= toHtml(OrgCategoriaRischioList2.getSelectedValue(thisAudit.getTipoChecklist())) %></a>
	</td>
...
<% String verificaPresenzaCampioniPreaccettazione = DwrPreaccettazione.verificaPresenzaCampioniPreaccettazioneSuCu(TicketDetails.getId());
if (verificaPresenzaCampioniPreaccettazione!=null && !verificaPresenzaCampioniPreaccettazione.equals("")){%>
<script>
alert('<%=verificaPresenzaCampioniPreaccettazione%>');
</script>
<% } %>
