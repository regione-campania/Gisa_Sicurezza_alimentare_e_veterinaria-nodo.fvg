<%-- 
  ...
  -
  - Version: $Id: acquedirete_tickets_modify.jsp 18488 2007-01-15 20:12:32Z matt $
  - Description:
  --%>
...
<body>
<form name="details" action="AcqueReteProvvedimenti.do?command=UpdateTicket&auto-populate=true<%= addLinkParams(request, "popup|popupType|actionId") %>" method="post" onsubmit="return controllaForm(this)">
<dhv:evaluate if="<%= !isPopup(request) %>">
<%-- Trails --%>
<table class="trails" cellspacing="0">
<tr>
<td>
  <a href="AcqueRete.do?command=SearchForm"><dhv:label name="">Operatori Abusivi</dhv:label></a> >
  <a href="AcqueRete.do?command=Search"><dhv:label name="acquedirete.SearchResults">Search Results</dhv:label></a> >
  <a href="AcqueRete.do?command=Details&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label name="">Scheda Operatore Abusivo</dhv:label></a> > 
<a href="AcqueRetedo?command=ViewVigilanza&orgId=<%=TicketDetails.getOrgId()%>"><dhv:label name="">Controlli Ufficiali</dhv:label></a> >
...
<a href="AcqueReteNonConformita.do?command=TicketDetails&id=<%= request.getAttribute("idNC")%>&orgId=<%=TicketDetails.getOrgId()%>"><dhv:label name="">Non Conformità Rilevata</dhv:label></a> >
  <% if (request.getParameter("return") == null) {%>
  <a href="AcqueReteProvvedimenti.do?command=TicketDetails&id=<%=TicketDetails.getId()%>"><dhv:label name="followup.dettagli">Scheda Follow Up</dhv:label></a> >
  <%}%>
...
<dhv:container name="AcqueRete" selected="vigilanza" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId") %>'>
    <%--@ include file="acquedirete_ticket_header_include.jsp" --%>
     <dhv:evaluate if="<%= !TicketDetails.isTrashed() %>" >
      <dhv:evaluate if="<%= TicketDetails.getClosed() != null %>" >
          <dhv:permission name="operatoriregione-operatoriregione-followup-edit">
            <input type="submit" value="<dhv:label name="button.reopen">Reopen</dhv:label>" onClick="javascript:this.form.action='AcqueReteProvvedimenti.do?command=ReopenTicket&id=<%=TicketDetails.getId()%><%= addLinkParams(request, "popup|popupType|actionId") %>'">
           </dhv:permission>
           ...
           <dhv:evaluate if='<%= !"list".equals(request.getParameter("return"))%>' >
            <input type="submit" value="Annulla" onClick="javascript:this.form.action='AcqueReteProvvedimenti.do?command=TicketDetails&id=<%= TicketDetails.getId() %><%= addLinkParams(request, "popup|popupType|actionId") %>'" />
           </dhv:evaluate>
      </dhv:evaluate>
      <dhv:evaluate if="<%= TicketDetails.getClosed() == null %>" >
          <dhv:permission name="operatoriregione-operatoriregione-followup-edit">
