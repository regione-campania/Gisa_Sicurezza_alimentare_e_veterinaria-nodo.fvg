function eliminaCheckList (idAudit,idControllo,orgId)
{
	...
	{
	    document.location = 'CheckListRiproduzioneAnimale.do?command=Delete&id='+idAudit+'&idControllo='+idControllo+'&orgId='+orgId;
	}
...
function compilaCheckList(messaggio,orgId,idControllo,idControlloUfficiale,isPrincipale,form)
{
	...
	//{
		...
		if(isPrincipale=='1')
		{
			if(confirm(messaggio))
			{
			document.forms[form].action='CheckListRiproduzioneAnimale.do?command=Add&orgId='+orgId+'&idC='+idControllo+'&isPrincipale=true&idControllo='+idControlloUfficiale
		...
		else
		{
			...
			{
				document.forms[form].action='CheckListRiproduzioneAnimale.do?command=Add&orgId='+orgId+'&idC='+idControllo+'&isPrincipale=false&idControllo='+idControlloUfficiale
				setTimestampStartRichiesta();
				document.forms[form].submit();
			}
			}
...
document.forms[form].action = "RiproduzioneAnimaleVigilanza.do?command=UploadListaDistribuzione&goto="+gotoPage;
formTest = true;
message = "";
...
function eliminaFileAllegato(fid,orgId,folderid,form,gotoPage)
{
	...
	document.forms[form].encoding="multipart/form-data";
document.forms[form].action = "RiproduzioneAnimaleVigilanza.do?command=DeleteListaDistribuzione&goto="+gotoPage+"&fid="+fid+"&orgId="+orgId+"&folderId="+folderid;
...
function aggiornaCategoria(idControllo,orgId)
{
	...
	{
		document.details.action='CheckListRiproduzioneAnimale.do?command=UpdateCategoria&idC='+idControllo+'&orgId='+orgId ;
	...
	if(form.tipoCampione.value!="-1"){
		...
		if(form.tipoAudit.value=="-1"){
			...
			if(form.ispezione.value=="-1" || form.ispezione.value==""){//se non ho selezionato nessun piano di monitoraggio
								 ...
								 message += label("check.vigilanza.richiedente.selezionato","- Controllare che il campo \"Oggetto del controllo\" sia stato popolato\r\n");
							      formTest = false;
