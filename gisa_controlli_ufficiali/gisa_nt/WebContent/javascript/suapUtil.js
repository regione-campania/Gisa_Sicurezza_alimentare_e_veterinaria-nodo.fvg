...
function onChangeTipoImpresaCallBack(tipoImpresa)
  	{
		...
		{
		document.getElementById("dati_titolare").style.display="none";
		$('input[name="nome"]').val("DIRETTORE GENERALE");
		$('input[name="cognome"]').val("ASL");
		}
	else
		{
		document.getElementById("dati_titolare").style.display="block";
		$('input[name="nome"]').val("");
...
var previstoCodiceNazionale =false;
  function mostraAttivitaProduttiveCallBack(attivita) 
  {
  	...
  	var combo = "<select onchange='if(controlloMacroareaCanile(this.value,2) && checkNext(\""+idTab+"\")==\"true\"){mostraAttivitaProduttive(\""+idTab+"\",\""+attivita.nextLivello+"\",this.value, "+ricerca+","+tipoRegistrazione+"); mostraCodiceNazionaleRichiesto(\""+idTab+"\",\""+attivita.nextLivello+"\",this.value);} else if(!controlloMacroareaCanile(this.value,2)){  alert(\"Attenzione!!! Per inserire le linee relative alla macroarea selezionata rivolgersi al servizio di Help Desk.\");this.value=\"\";} ' style='color:"+attivita.bgcolor+";width:100%;'  label='"+attivita.label+"' foglia='false' name='idLineaProduttiva2' id='"+attivita.label+"'>" ;
