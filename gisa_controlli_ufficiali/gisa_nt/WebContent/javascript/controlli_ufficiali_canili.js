function eliminaCheckList (idAudit,idControllo,orgId)
{
	...
	{
	    document.location = 'CheckListCanili.do?command=Delete&id='+idAudit+'&idControllo='+idControllo+'&orgId='+orgId;
	}
...
function compilaCheckList(messaggio,orgId,idControllo,idControlloUfficiale,isPrincipale,form)
{
	...
	{*/
		...
		if(isPrincipale=='1')
		{
			if(confirm(messaggio))
			{
			document.forms[form].action='CheckListCanili.do?command=Add&orgId='+orgId+'&idC='+idControllo+'&isPrincipale=true&idControllo='+idControlloUfficiale
		...
		else
		{
			...
			{
				document.forms[form].action='CheckListCanili.do?command=Add&orgId='+orgId+'&idC='+idControllo+'&isPrincipale=false&idControllo='+idControlloUfficiale
				setTimestampStartRichiesta();
				document.forms[form].submit();
			}
			}
...
document.forms[form].action = "CaniliVigilanza.do?command=UploadListaDistribuzione&goto="+gotoPage;
formTest = true;
message = "";
...
function eliminaFileAllegato(fid,orgId,folderid,form,gotoPage)
{
	...
	document.forms[form].encoding="multipart/form-data";
document.forms[form].action = "CaniliVigilanza.do?command=DeleteListaDistribuzione&goto="+gotoPage+"&fid="+fid+"&orgId="+orgId+"&folderId="+folderid;
...
{
	if (confirm("Attenzione! Aggiornando la categoria di rischio si procede contestualmente anche alla chiusura automatica del controllo ufficiale. \n\nAttenzione! Procedendo alla chiusura del CU e al conseguente aggiornamento della categoria di rischio, non sarà possibile inserire una seconda checklist. Si prega di verificare bene i dati inseriti prima di confermare l'operazione.\n\n Saranno impossibili ulteriori modifiche. Proseguire?")) 
	{
		document.details.action='CheckListCanili.do?command=UpdateCategoria&idC='+idControllo+'&orgId='+orgId ;
