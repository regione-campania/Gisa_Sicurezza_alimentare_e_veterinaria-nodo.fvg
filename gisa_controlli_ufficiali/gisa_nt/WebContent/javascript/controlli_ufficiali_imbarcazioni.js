function eliminaCheckList (idAudit,idControllo,orgId)
{
	...
	{
	    document.location = 'CheckListImbarcazioni.do?command=Delete&id='+idAudit+'&idControllo='+idControllo+'&orgId='+orgId;
	}
...
function compilaCheckList(messaggio,orgId,idControllo,idControlloUfficiale,isPrincipale,form)
{
	...
	//{
		...
		if(isPrincipale=='1')
		{
			if(confirm(messaggio))
			{
			document.forms[form].action='CheckListImbarcazioni.do?command=Add&orgId='+orgId+'&idC='+idControllo+'&isPrincipale=true&idControllo='+idControlloUfficiale
		...
		else
		{
			...
			{
				document.forms[form].action='CheckListImbarcazioni.do?command=Add&orgId='+orgId+'&idC='+idControllo+'&isPrincipale=false&idControllo='+idControlloUfficiale
				setTimestampStartRichiesta();
				document.forms[form].submit();
			}
			}
...
document.forms[form].action = "ImbarcazioniVigilanza.do?command=UploadListaDistribuzione&goto="+gotoPage;
formTest = true;
message = "";
...
function eliminaFileAllegato(fid,orgId,folderid,form,gotoPage)
{
	...
	document.forms[form].encoding="multipart/form-data";
document.forms[form].action = "ImbarcazioniVigilanza.do?command=DeleteListaDistribuzione&goto="+gotoPage+"&fid="+fid+"&orgId="+orgId+"&folderId="+folderid;
...
function aggiornaCategoria(idControllo,orgId)
{
	...
	{
		document.details.action='CheckListImbarcazioni.do?command=UpdateCategoria&idC='+idControllo+'&orgId='+orgId ;
