...
<table cellpadding="0" cellspacing="0" width="100%" border="1" rules="cols">
  <tr class="section">
    ...
    <dhv:evaluate if="<%= FileFolder.getParentId() != -1 %>">
      <a href="AccountsDocumentsFoldersOsmRegistrati.do?command=SaveMove&orgId=<%= OrgDetails.getOrgId() %>&id=<%= FileFolder.getId() %>&popup=true&folderId=0&return=AccountsDocumentsOsmRegistrati&param=<%= OrgDetails.getOrgId() %>&param2=<%= FileFolder.getParentId() %>"><dhv:label name="stabilimenti.stabilimenti_documents_file_move.Home">Home</dhv:label></a>
    </dhv:evaluate>
    <dhv:evaluate if="<%= FileFolder.getParentId() == -1 %>">
...
%>    
  <tr class="row<%= rowid %>">
    ...
    <% if(FileFolder.getParentId() != thisFolder.getId() && (FileFolder.getId() != thisFolder.getId())){  %>
      <a href="AccountsDocumentsFoldersOsmRegistrati.do?command=SaveMove&orgId=<%= OrgDetails.getId() %>&id=<%= FileFolder.getId() %>&popup=true&folderId=<%= thisFolder.getId() %>&return=AccountsDocumentsOsmRegistrati&param=<%= OrgDetails.getOrgId() %>&param2=<%= FileFolder.getParentId() %>"><%= toHtml(thisFolder.getSubject()) %></a>
