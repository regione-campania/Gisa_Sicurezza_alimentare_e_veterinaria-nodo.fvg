...
<body onload="verificaChiusuraNc(<%=request.getAttribute("Chiudi")%>,<%=request.getAttribute("numSottoAttivita")%>,'<%=request.getAttribute("Messaggio2")%>',<%=TicketDetails.getIdControlloUfficiale() %>,'OsmRegistratiVigilanza')">

<form name="details" action="OsmRegistratiNonConformita.do?command=ModifyTicket&auto-populate=true" method="post">
<%-- Trails --%>
...
<td>
  ...
  <a href="OsmRegistrati.do?command=Details&orgId=<%=TicketDetails.getOrgId()%>"><dhv:label name="">Scheda Osm</dhv:label></a> >
...
<dhv:container name="osmregistratinonconformita" selected="details" object="TicketDetails"
	param="<%= param1 %>"
