...
<td>
  ...
  <a href="OsmRegistrati.do?command=ViewCessazionevariazione&orgId=<%=TicketDetails.getOrgId()%>"><dhv:label name="cessazionevariazione2">Cessazioni/Variazioni</dhv:label></a> >
...
%>
<dhv:container name="osm" selected="cessazionevariazione" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId() %>' hideContainer='<%= isPopup(request) || (defectCheck != null && !"".equals(defectCheck.trim())) %>'>
<dhv:container name="stabilimenticessazionevariazione" selected="details" object="TicketDetails"
	param="<%= param1 %>"
	...
	%>
	<dhv:permission name="osmregistrati-osmregistrati-cessazionevariazione-delete">
		<input type="button"
			value="Ripristina"
			onClick="javascript:this.form.action='OsmCessazionevariazione.do?command=Restore&id=<%= TicketDetails.getId()%>';submit();">
	</dhv:permission>
	<%
	...
	<%--<dhv:permission name="campioni-campioni-edit">
		<input type="button"
			...
			onClick="javascript:this.form.action='OsmRegistratiCampioni.do?command=ReopenTicket&id=<%= TicketDetails.getId()%><%= (defectCheck != null && !"".equals(defectCheck.trim())?"&defectCheck="+defectCheck:"") %>';submit();">
	</dhv:permission>--%>
	<%
		} else {
	%>
	<dhv:permission name="osmregistrati-osmregistrati-cessazionevariazione-edit">
		<input type="button"
			value="Modifica"
			onClick="javascript:this.form.action='OsmCessazionevariazione.do?command=ModifyTicket&auto-populate=true<%= (defectCheck != null && !"".equals(defectCheck.trim())?"&defectCheck="+defectCheck:"") %>';submit();">
	</dhv:permission>
	<%--
	...
	<dhv:permission name="osmregistrati-osmregistrati-cessazionevariazione-delete">
		...
		<input type="button"
			...
			onClick="javascript:popURL('OsmCessazionevariazione.do?command=ConfirmDelete&id=<%= TicketDetails.getId() %>&orgId=<%= TicketDetails.getOrgId()%>&return=searchResults&popup=true', 'Delete_ticket','320','200','yes','no');">
		<%
			} else {
		%>
		<input type="button"
			value="<dhv:label name="global.button.delete">Delete</dhv:label>"
			onClick="javascript:popURL('OsmCessazionevariazione.do?command=ConfirmDelete&id=<%= TicketDetails.getId() %>&orgId=<%= TicketDetails.getOrgId()%>&popup=true', 'Delete_ticket','320','200','yes','no');">
		<%
			}
		%>
	</dhv:permission>
	<dhv:permission name="osmregistrati-osmregistrati-cessazionevariazione-edit">
		<input type="button"
			...
			onClick="javascript:this.form.action='OsmCessazionevariazione.do?command=Chiudi&id=<%= TicketDetails.getId() %>';if( confirm('Sei sicuro di voler chiudere questa cessazione?') ){submit()};">
	</dhv:permission>
	<%
		}
	%>
	<dhv:permission name="osmregistrati-osmregistrati-cessazionevariazione-edit,osmregistrati-osmregistrati-cessazionevariazione-delete">
		<br />&nbsp;<br />
	</dhv:permission>
	<%-- Ticket Information --%>
	<%-- Primary Contact --%>
		...
		<table cellpadding="4" cellspacing="0" width="100%" class="details">
			...
			<tr class="containerBody">
				...
				<td><dhv:evaluate
					...
					<dhv:permission name="osmregistrati-osmregistrati-contacts-view">
						<a
					...
					<dhv:permission name="osmregistrati-osmregistrati-contacts-view" none="true">
			...
			<tr class="containerBody">
				<td class="formLabel"><dhv:label
					name="stabilimenti.stabilimenti_contacts_add.Title">Title</dhv:label></td>
				<td><%=toHtml(TicketDetails.getThisContact()
											.getTitle())%></td>
			</tr>
			<tr class="containerBody">
				<td class="formLabel"><dhv:label
					name="stabilimenti.stabilimenti_add.Email">Email</dhv:label></td>
			...
			<tr class="containerBody">
				<td class="formLabel"><dhv:label
					name="stabilimenti.stabilimenti_add.Phone">Phone</dhv:label></td>
				<td><%=toHtml(TicketDetails.getThisContact()
...
<br />
  ...
  --%>
	...
	<dhv:permission name="osmregistrati-osmregistrati-cessazionevariazione-edit">
		<input type="button"
			value="<dhv:label name="global.button.close">Chiudi</dhv:label>"
			onClick="javascript:this.form.action='OsmCessazionevariazione.do?command=Chiudi&id=<%= TicketDetails.getId() %>';if( confirm('Sei sicuro di voler chiudere questa variazione?') ){submit()};">
	</dhv:permission>
	<%
		}
	%>
