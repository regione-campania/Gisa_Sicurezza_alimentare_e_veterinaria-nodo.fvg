...
<form name="addticket" action="OsmCessazioneVariazione.do?command=InsertTicket&auto-populate=true<%= addLinkParams(request, "popup|popupType|actionId") %>" method="post">
<%-- Trails --%>
...
<td>
  ...
  <a href="OsmRegistrati.do?command=Details&orgId=<%=OrgDetails.getOrgId()%>">Scheda Osm</a> >
  <%--a href="Accounts.do?command=ViewCessazionevariazione&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label name="">Cessazioni/Variazioni</dhv:label></a> >--%>
    <a href="OsmRegistrati.do?command=ViewCessazionevariazione&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label name="">Volture</dhv:label></a> >
  <dhv:label name="cessazionevariazione.aggiungi">Voltura</dhv:label>
</td>
</tr>
</table>
<%-- End Trails --%>
<dhv:container name="osmregistrati" selected="cessazionevariazione" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId") %>'>
<input type="submit" value="Invia Richiesta" name="Save" onClick="return checkForm(this.form)">
<input type="submit" value="Annulla" onClick="javascript:this.form.action='OsmRegistrati.do?command=ViewCessazionevariazione&orgId=<%=OrgDetails.getOrgId()%>'">
<br>
<dhv:formMessage />
  ...
  <dhv:include name="" none="true">
    <tr class="containerBody">
      ...
      <td>
        ...
        <input type="text" size="16" name="codiceFiscaleRappresentante" maxlength="16"   value="<%= toHtml(OrgDetails.getCodiceFiscaleRappresentante() )%>"><font color="#00FF00"><b>* Inserire la variazione</b></font>
      </td>
    </tr>
  </dhv:include>
    <dhv:include name="" none="true">
