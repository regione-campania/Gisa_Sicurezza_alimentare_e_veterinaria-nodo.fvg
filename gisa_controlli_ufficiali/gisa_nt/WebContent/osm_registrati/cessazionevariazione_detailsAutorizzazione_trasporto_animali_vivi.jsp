...
<form name="details" action="OsmCessazioneVariazione.do?command=ModifyTicket&auto-populate=true" method="post">
<%-- Trails --%>
...
<td>
  ...
  <%--<a href="Accounts.do?command=ViewCessazionevariazione&orgId=<%=TicketDetails.getOrgId()%>"><dhv:label name="cessazionevariazione2">Cessazioni/Variazioni</dhv:label></a> >--%>
    <a href="OsmRegistrati.do?command=ViewCessazionevariazione&orgId=<%=TicketDetails.getOrgId()%>"><dhv:label name="cessazionevariazione2">Volture</dhv:label></a> >
...
%>
<dhv:container name="osmregistrati" selected="cessazionevariazione" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId() %>' hideContainer='<%= isPopup(request) || (defectCheck != null && !"".equals(defectCheck.trim())) %>'>
<dhv:container name="osmregistraticessazionevariazione" selected="details" object="TicketDetails"
	param="<%= param1 %>"
...
<%--</dhv:evaluate>  --%>
 ...
 <tr class="containerBody">
  ...
  <input type="hidden" name="tipo_richiesta" value="attivita_ispettiva_rilascioautorizzazioni_e_vigilanza" />
    ...
    <dhv:include name="osmregistrati-number" none="true">
    <tr class="containerBody">
      <td nowrap class="formLabel">
        <dhv:label name="organization.accountNumber">Account Number</dhv:label>
      </td>
      <td>
              <%= ((OrgDetails.getAccountNumber()!=null && (!OrgDetails.getAccountNumber().equals("")))?(toHtmlValue(OrgDetails.getAccountNumber())):(toHtmlValue(""))) %>
      </td>
    </tr>
  </dhv:include>
      <tr class="containerBody">
