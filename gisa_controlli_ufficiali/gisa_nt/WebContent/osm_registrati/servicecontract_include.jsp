...
<script language="JavaScript">
  ...
  function checkForm(form) {
    ...
    if (form.assetId.value == "-1") { 
      message += label("check.impianto","- Impianto is required\r\n");
      formTest = false;
    }
  ...
  function popListaImpianti(hiddenFieldId1, displayFieldId1, params, hiddenFieldId2, displayFieldId2, ubicazioneId) {
	  title  = 'Assets';
	  width  =  '575';
	  height =  '400';
	  resize =  'yes';
	  ...
	  if (!ubicazioneId || ubicazioneId == '-1') {
	    ubicazioneId = document.getElementById('ubicazioneId').value;
	  }
	  if (!ubicazioneId || ubicazioneId == '-1') {
	    alert("Deve essere prima selezionata una ubicazione");
	  } else {
	    var firstParam = 'AssetSelector.do?command=ListAssets&listType=single&reset=true&orgId=' + ubicazioneId + '&contractId=-1&previousSelection='+document.getElementById(hiddenFieldId1).value+'&displayFieldId1='+displayFieldId1+'&hiddenFieldId1='+hiddenFieldId1;
  ...
  function changeDivContent(divName, divContents) {
    ...
    if (divName == 'changeaccount') {
      changeDivContent('addUbicazione',label('none.selected','None Selected'));
      resetNumericFieldValue('ubicazioneId');
      changeDivContent('addAsset',label('none.selected','None Selected'));
      resetNumericFieldValue('assetId');
    }
    if (divName == 'addUbicazione') {
      changeDivContent('addAsset',label('none.selected','None Selected'));
      resetNumericFieldValue('assetId');
    }
  }
...
<table cellpadding="4" cellspacing="0" border="0" width="100%" class="details">
  ...
  <tr class="containerBody">
    <td class="formLabel" nowrap>
      <dhv:label name="account.sc.asset">Asset</dhv:label>
    </td>
    <td>
     <table cellspacing="0" cellpadding="0" border="0" class="empty">
      <tr>
        <td>
          <div id="addAsset">
            <input type="hidden" name="hiddenUbicazioneId" id="ubicazioneId" value="<%= OrgDetails.getOrgId() %>">
          
            <% if(serviceContract.getImpiantoId() != -1) {%>
              <%= request.getAttribute("matricola") %>
            <%} else {%>
              <dhv:label name="stabilimenti.stabilimenti_add.NoneSelected">None Selected</dhv:label>
            <%}%>
          </div>
        </td>
        <td>
          <input type="hidden" name="impiantoId" id="assetId" value="<%= serviceContract.getImpiantoId() %>">
          ...
          <% if(serviceContract.getImpiantoId() == -1) {%>
            ...
            [<a href="javascript:popListaImpianti('assetId','addAsset', 'filters=allassets|allassetsandcomponenti','ubicazioneId','addUbicazione');"><dhv:label name="stabilimenti.stabilimenti_add.select">Select</dhv:label></a>]
          ...
          <td>
            ...
            %>
             ...
             <%}
                ...
                <option value="-1"><dhv:label name="stabilimenti.stabilimenti_add.NoneSelected">None Selected</dhv:label></option>
             <%}%>
            </select>
            <input type="hidden" name="previousSelection" value="" />
          </td>
          <td valign="top">
            &nbsp;[<a href="javascript:popProductCatalogSelectMultiple('selectedList','<%=serviceContract.getId()%>');"><dhv:label name="stabilimenti.stabilimenti_add.select">Select</dhv:label></a>]
          </td>
        </tr>
      </table>
    </td>
     ...
     <table cellspacing="0" cellpadding="0" border="0" class="empty">
      <tr>
        ...
        <td>
          ...
          &nbsp [<a href="javascript:popURL('AccountsServiceContractsOsmRegistrati.do?command=HoursHistory&id=<%= serviceContract.getId() %>&popup=true&popupType=inline','Details','650','500','yes','yes');"><dhv:label name="accountsassets.history.long_html">History</dhv:label></a>]
          </dhv:evaluate>&nbsp;
        </td>
        </tr>
      </table>
