...
<jsp:useBean id="Partita"				class="org.aspcfs.modules.macellazioninewsintesis.base.PartitaSeduta"			scope="request" />
	...
	try
	  {
		...
		if ( document.getElementById('cd_bse').value == '-1' )
		{
			...
			document.getElementById('cd_bse').checked = false;
			//alert('checked = false');
		} else {
...
function controllaForm( toLiberoConsumo )
{
	...
	try{
		...
		//Geocodifica.isCapoEsistente( form.cd_partita.value, gestisciCapoEsistente );
		message ="";
		ret = true;
		var BOVINI=1;
		var BUFALINI=5;
		...
		*/
		...
		if( form.cd_bse.checked && form.cd_bse.value == '-1' )
		{
...
{
	var form = document.main;
	var matricola = form.cd_partita.value	;
	...
	form.cd_partita.value = data.matricola;
...
function gestisciUnload(){
	...
	if(document.getElementById('submitOK').value == 'no'){
		if(confirm('Stai uscendo dalla scheda "<%= update ? "Modifica" : "Aggiungi"%> Partita". Cliccare su OK per salvare i dati, altrimenti le informazioni inserite verranno perse.')){
...
<body onbeforeunload="return gestisciUnload();" onload="gestisciUtenteVeterinario();contralla_cd_bse();mostra_tab_test_bse_onload();">
	...
	<tr>
		<td>
			...
			<%
					...
					%>
			<a href="MacellazioniNewSintesis.do?command=List&altId=<%=OrgDetails.getAltId()%>">Macellazioni</a> > <%= update ? configTipo4.getNomeFunzioneModificaCapo() : configTipo4.getNomeFunzioneAggiungiCapo() %>
		</td>
	</tr>
</table>
...
<form name="main" action="MacellazioniNewSintesis.do?command=UpdateSedutaEsitoCampioni" method="post" onsubmit="return controllaForm( false );" >
...
<input type="hidden" name="id_capo" value="<%=Partita.getId() %>" />
<input type="hidden" name="id" value="<%=Partita.getId() %>" />
<input type="hidden" name="capo_esistente" value="no" />
<input type="hidden" id="capo_in_bdn" name="capo_in_bdn" value="<%= PartitaLog.isInBdn() ? "si" : "no" %>" />
<input type="hidden" name="codice_azienda_nascita_from_bdn" value="<%= PartitaLog.getCodiceAziendaNascitaFromBdn() != null ? PartitaLog.getCodiceAziendaNascitaFromBdn() : "" %>" />
<input type="hidden" name="data_nascita_from_bdn" value="<%= PartitaLog.getDataNascitaFromBdn() != null ? sdf.format( PartitaLog.getDataNascitaFromBdn() ) : "" %>" />
<input type="hidden" name="specie_from_bdn" value="<%= PartitaLog.getSpecieFromBdn() %>" />
<input type="hidden" name="razza_from_bdn" value="<%= PartitaLog.getRazzaFromBdn() %>" />
<input type="hidden" name="asl_speditore_from_bdn" value="<%= PartitaLog.getAslSpeditoreFromBdn() %>" />
<input type="hidden" name="comune_speditore_from_bdn" value="<%= PartitaLog.getComuneSpeditoreFromBdn() != null ? PartitaLog.getComuneSpeditoreFromBdn() : "" %>" />
<input type="hidden" name="sesso_from_bdn" value="<%= PartitaLog.getSessoFromBdn() != null ? PartitaLog.getSessoFromBdn() : "" %>" />
...
<div id="tabs">
	<ul>
	        <li id="li-3" ><a href="#tabs-3" onclick="javascript:showHide('show');settaSoloCD('false');" >Evidenza Visita PM</a></li>
           ...
           <%-- <tr class="containerBody">
            ...
            <tr class="containerBody">
            	<td class="formLabel">Codice Azienda di provenienza</td>
            	 <td>
                	<input type="text" name="cd_codice_azienda_provenienza" value="<%=toHtmlValue( Partita.getCd_codice_azienda_provenienza() ) %>" /><font color="red" >*</font>
				</td>
            </tr>
            ...
            <tr>
                ...
                <%if( update ){ %>
                	<input type="hidden" id="cd_partita" name="cd_partita" maxlength="64" value="<%=toHtmlValue( Partita.getCd_partita() ) %>" />
                	<%=toHtmlValue( Partita.getCd_partita() ) %>
                <%} else { %>
                	<input type="text" id="cd_partita" name="cd_partita" onchange="Geocodifica.isCapoEsistente( this.value, gestisciCapoEsistente );" value="<%=toHtmlValue( Partita.getCd_partita() ) %>" /><font color="red" >*</font>
                <%} %>
                </td>
            </tr>
...
%>
                ...
                <td>
                	...
                	<tr><td>
                	1 <input type = "radio" id = "cd_categoria_rischio_1" name = "cd_categoria_rischio" value = "1" <%if(Partita.getCd_categoria_rischio()==1   ){%>checked="checked"<%} %> />	[bovini sani sopra i 72 mesi di eta']
                	</td></tr>
                	<tr><td>
                	2 <input type = "radio" id = "cd_categoria_rischio_2" name = "cd_categoria_rischio" value = "2" <%if(Partita.getCd_categoria_rischio()==2  ){%>checked="checked"<%} %> /> 	[mac. emergenza]->test BSE se età sopra i 48 mesi <br> 
                	</td></tr> 	
                	<tr><td>
                	3 <input type = "radio" id = "cd_categoria_rischio_3" name = "cd_categoria_rischio" value = "3" <%if(Partita.getCd_categoria_rischio()==3  ){%>checked="checked"<%} %> /> 	[mac. differita in caso di Bruc., TBC, Leucosi)<br>
                	</td></tr>
                	<tr><td>
                	5 <input type = "radio" id = "cd_categoria_rischio_5" name = "cd_categoria_rischio" value = "5" <%if(Partita.getCd_categoria_rischio()==5  ){%>checked="checked"<%} %> /> 	[Bovini morti durante il trasporto] BSE obbligatorio <br>
                	</td></tr>
                	</table>
				</td> 
            </tr>
...
<%--              		<zeroio:dateSelect field="cd_data_mod4" form="main" showTimeZone="false" timestamp="<%=Partita.getCd_data_mod4() %>" /> --%>
            ...
            <tr class="containerBody">
              ...
              <td id="test_bse_valore">
              	<input id="cd_bse" onchange="gestisciObbligatorietaCd();" name="cd_bse" type="checkbox" <%=(Partita.getCd_bse()) ? ("checked=\"checked\"") : ("") %> />
				<br/>
...
<%              	
				...
				if(Partita.getCd_num_capi_ovini_da_testare()>0)
					num_capi_ovini = Partita.getCd_num_capi_ovini_da_testare()+"";
				if(Partita.getCd_num_capi_caprini_da_testare()>0)
					num_capi_caprini = Partita.getCd_num_capi_caprini_da_testare()+"";
%>
...
<!--***************************************************************-->
	...
	<input id="liberoConsumoButton" type="button" value="Libero Consumo >>" onclick="if(controllaForm( true )){document.main.action='MacellazioniNewSintesis.do?command=ToLiberoConsumo';settaSoloCD('false');document.main.submit();};" />
...
<%--              	<zeroio:dateSelect field="vpm_data" form="main" showTimeZone="false" timestamp="<%=Partita.getVpm_data() %>" />  --%>
            ...
            <tr class="containerBody">
                ...
                %>                               
                <%=EsitiVpm.getHtmlSelect( "vpm_esito", Partita.getVpm_esito() ) %>
                </td>
            </tr>
...
<!--            </tr>              -->
            ...
            <tr 
            	...
            	<%=(update && (Partita.getVpm_esito() == 2 || Partita.getVpm_esito() == 4 )) ? ("") : ("style=\"display: none\"") %> >
					    ...
					    <tr class="containerBody">
					        ...
					        <td>
						        ...
						        <div id ="destinatario_label_1" align="center">
						        	<%=(Partita.getDestinatario_1_id() != -1) ? (toHtmlValue( Partita.getDestinatario_1_nome() )) : ("-- Seleziona Esercente --") %>
						        </div>
					        	<input 
					        		type="hidden" 
						    ...
						    <td>
					        	...
					        	<div id ="destinatario_label_2" align="center">
						        	<%=(Partita.getDestinatario_2_id() != -1) ? (toHtmlValue( Partita.getDestinatario_2_nome() )) : ("-- Seleziona Esercente --") %>	
						        </div>
					        	<input 
					        		type="hidden" 
					    ...
					    <tr class="containerBody">
					        ...
					        <td>
						        ...
						        <div id ="destinatario_label_3" align="center">
						        	<%=(Partita.getDestinatario_3_id() != -1) ? (toHtmlValue( Partita.getDestinatario_3_nome() )) : ("-- Seleziona Esercente --") %>
						        </div>
					        	<input 
					        		type="hidden" 
						    ...
						    <td>
					        	...
					        	<div id ="destinatario_label_4" align="center">
						        	<%=(Partita.getDestinatario_4_id() != -1) ? (toHtmlValue( Partita.getDestinatario_4_nome() )) : ("-- Seleziona Esercente --") %>	
						        </div>
					        	<input 
					        		type="hidden" 
    ...
    <tr 
    	id="lcso" 
    	style="<%=( Partita.getVpm_esito() == 3 || Partita.getVpm_esito() == 4 ) ? ( "" ) : ( "display:none" ) %>" >
    	<td>
    		<table class="details" width="100%" border="0" cellpadding="4" cellspacing="0">
    		<tr>
...
<tr id="lcpr" <%=( Partita.getVpm_esito() == 4 ) ? ("") : ("style=\"display: none\"") %> >
    	<td>
    		<table class="details" width="100%" border="0" cellpadding="4" cellspacing="0">
