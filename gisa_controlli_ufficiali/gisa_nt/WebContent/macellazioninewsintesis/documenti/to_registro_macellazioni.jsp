...
function openPopupRegistroMacellazioniPdf(altId){
	...
	window.open('MacellazioniDocumentiNewSintesis.do?command=RegistroMacellazioni&altId='+altId+'&notEsitoBSE='+notEsitoBSE+'&ante_macellazione='+ante_macellazione+'&comboDateMacellazione='+date,'popupSelect',
	...
	nuoveStampe = true;
	if (!nuoveStampe){
		window.open('MacellazioniDocumentiNewSintesis.do?command=RegistroMacellazioni&altId='+altId+'&comboDateMacellazione='+date,'popupSelect',
		'toolbar=no,directories=no,status=no,continued from previous linemenubar=no,scrollbars=no,resizable=no ,modal=yes');
		}
		else{
	...
	<tr>
		<td>
			...
			<%
				...
				{}
			%>
			<a href="MacellazioniNewSintesis.do?command=List&altId=<%=OrgDetails.getAltId() %>">Macellazioni</a> > Registro Macellazioni
		</td>
	</tr>
</table>
