<%@page import="org.aspcfs.modules.macellazioninewsintesis.base.CategoriaRischio"%>
...
<jsp:useBean id="PartitaPrincipale"				class="org.aspcfs.modules.macellazioninewsintesis.base.Partita"			scope="request" />
...
function controllaForm( toLiberoConsumo )
{
	...
	try{
		...
		{
			message += label("","- [Controllo documentale] : \"Numero Capi Ovini\" non può essere inferiore alla somma dei capi ovini specificati nelle sezioni successive\r\n" );
			ret = false;
		}
		...
		{
			message += label("","- [Controllo documentale] : \"Numero Capi Caprini\" non può essere inferiore alla somma dei capi caprini specificati nelle sezioni successive\r\n" );
			ret = false;
		}
		...
		*/
		...
		if(!toLiberoConsumo){
			...
			{
					message += label("","- [Evidenza Visita AM] : \"Numero Capi Ovini\" specificati nella Visita AM e Morte Ant.macellazione non può essere superiore alla somma dei capi ovini specificati nel controllo documentale\r\n" );
					ret = false;
			}
			
			if(numCapiCapriniDestTotale>numCapiCapriniControlloDocumentale)
			{
					message += label("","- [Evidenza Visita AM] : \"Numero Capi Caprini\" specificati nella Visita AM e Morte Ant.macellazione non può essere superiore alla somma dei capi caprini specificati nel controllo documentale\r\n" );
					ret = false;
			}
			...
			if(calcolaNumCapiOviniDestCorrente()+num_capi_ovini_esito_2_val!=vam_num_capi_ovini && parseInt(form.cd_num_capi_ovini.value)>0)
			{
			...
			if(calcolaNumCapiCapriniDestCorrente()+num_capi_caprini_esito_2_val!=vam_num_capi_caprini && parseInt(form.cd_num_capi_caprini.value)>0)
			{
		...
		*/
		...
		//if( sonoPassatiNAnni( form.cd_data_nascita.value, form.vpm_data.value, <%= ApplicationProperties.getProperty("numeroAnniTestBse") %> )  )
		//{
...
function checkControlloDocumentale( )
{
	try{
		...
		//if( sonoPassatiNAnni( form.vpm_data.value, <%= ApplicationProperties.getProperty("numeroAnniTestBse") %> ) 
		//{
			//if(document.getElementById('cd_bse_check').checked==false){
				/*
				message += label("", "- [Controllo Documentale] : Il capo ha più di 4 anni. E\' obbligatorio l\'inserimento dei dati sul test BSE" );
...
function gestisciTabellaNonConformitaVam(){
	if( document.getElementById('vam_esito2').checked ){ //se è "non favorevole"
...
<body onload="displayVPMpatologie(),displayLCSO();displayNumCapiEsito();gestisciObbligatorietaVisitaAnteMortem();<%if(update){}%>gestisciUtenteVeterinario();displayAbbattimento(); mostraDestinazione();gestisciTabellaNonConformitaVam();controlla_esito_testBSE();">
	...
	<tr>
		<td>
			...
			<%
					...
					%>
			<a href="MacellazioniNewSintesis.do?command=List&altId=<%=OrgDetails.getAltId()%>">Macellazioni</a> > <%= update ? "Modifica Seduta" : "Aggiungi Seduta" %>
		</td>
	</tr>
</table>
<%@page import="java.text.SimpleDateFormat"%>
...
<form name="main" action="MacellazioniNewSintesis.do?command=UpdateSeduta" method="post" onsubmit="return controllaForm( false );" >
...
<input type="hidden" name="id_partita" value="<%=PartitaPrincipale.getId() %>" />
<input type="hidden" name="partita_esistente" value="no" />
<input type="hidden" name="cd_partita" value="<%=PartitaPrincipale.getCd_partita() %>" />
            ...
            <tr class="containerBody">
                ...
                <td>
						<input 
							...
							name="vam_esito" 
							value="Favorevole" 
							<%=(update && Partita.getVam_esito()!=null && !Partita.getVam_esito().equalsIgnoreCase("Favorevole")) ? ("") : ("checked=\"checked\"") %> >Favorevole<br/>
						<input 
							type=radio 
							onclick="if(this.checked==true) document.getElementById('vam_tabella_non_conformita').style.display='none';gestisciObbligatorietaVisitaAnteMortem();"
							id= "vam_esito1" 
							...
							<%=(update && Partita.getVam_esito()!=null && Partita.getVam_esito().equalsIgnoreCase("Favorevole con riserva") ) ? ("checked=\"checked\"") : ("") %> >Favorevole con riserva<br/>
						<input 
							type=radio 
							...
							<%=(update && Partita.getVam_esito()!=null && Partita.getVam_esito().equalsIgnoreCase("Non favorevole") ) ? ("checked=\"checked\"") : ("") %> >Non favorevole 
...
%>
                	<input <% if(Partita.getCd_num_capi_ovini()<=0){out.println("disabled=\"disabled\"");} %> type="text" id="vam_num_capi_ovini" name="vam_num_capi_ovini"  onblur="gestisciObbligatorietaVisitaAnteMortem()" onchange="isIntPositivo(this.value,'Numero Capi Ovini',this);" value="<%=toHtmlValue(num_capi_ovini) %>" />
                	<font id="vam_num_capi_ovini_asterisco" align="center"  <% if(Partita.getCd_num_capi_ovini()<=0){out.println("style=\"display:none;\"");} %> color="red" >*</font>
                </td>
            </tr>
...
%>
                	<input <% if(Partita.getCd_num_capi_caprini()<=0){out.println("disabled=\"disabled\"");} %> type="text" id="vam_num_capi_caprini" name="vam_num_capi_caprini" onblur="gestisciObbligatorietaVisitaAnteMortem()" onchange="isIntPositivo(this.value,'Numero Capi Caprini',this);" value="<%=toHtmlValue(num_capi_caprini) %>" />
                	<font id="vam_num_capi_caprini_asterisco" align="center" <% if(Partita.getCd_num_capi_caprini()<=0){out.println("style=\"display:none;\"");} %>  color="red" >*</font>
                </td>
            </tr>
    ...
    <tr id="vam_tabella_non_conformita" <%=(update && Partita.getVam_esito()!=null && !Partita.getVam_esito().equalsIgnoreCase("Favorevole")) ? ("") : ("style=\"display: none;\"") %> >
    	<td>
    		<table  class="details" width="100%" border="0" cellpadding="4" cellspacing="0">
    			<tr>
...
%>
            <tr class="containerBody" style="display:none;" id="trNumCapiOviniEsito<%=thisElement.getId()%>">
                ...
                <td>
               		 <input <% if(Partita.getCd_num_capi_ovini()<=0){out.println("disabled=\"disabled\"");} %> type="text" id="num_capi_ovini_esito_<%=thisElement.getId()%>" name="num_capi_ovini_esito_<%=thisElement.getId()%>" onchange="isIntPositivo(this.value,'Numero Capi Ovini',this);" value="<%=num_capi_ovini_esito%>" />
             ...
             <tr class="containerBody" style="display:none;" id="trNumCapiCapriniEsito<%=thisElement.getId()%>">
                ...
                <td>
               		 <input <% if(Partita.getCd_num_capi_caprini()<=0){out.println("disabled=\"disabled\"");} %> type="text" id="num_capi_caprini_esito_<%=thisElement.getId()%>" name="num_capi_caprini_esito_<%=thisElement.getId()%>" onchange="isIntPositivo(this.value,'Numero Capi Caprini',this);" value="<%=num_capi_caprini_esito%>" />
					    ...
					    <tr class="containerBody">
					        ...
					        <td colspan="2">
						        ...
						        <div id ="destinatario_label_1" align="center">
						        	<%=(Partita.getDestinatario_1_id() != -1) ? (toHtmlValue( Partita.getDestinatario_1_nome() )) : ("-- Seleziona Destinatario/Esercente --") %>
						        </div>
					        	<input 
					        		type="hidden" 
						    ...
						    <td>
					        	...
					        	<div id ="destinatario_label_2" align="center">
						        	<%=(Partita.getDestinatario_2_id() != -1) ? (toHtmlValue( Partita.getDestinatario_2_nome() )) : ("-- Seleziona Destinatario/Esercente --") %>	
						        </div>
					        	<input 
					        		type="hidden" 
            	...
            	<td colspan="2">
		            <table width="100%" border="0" cellpadding="2" cellspacing="0" align="left">
					    ...
					    <tr class="containerBody">
					        ...
					        <td>
						        ...
						        <div id ="destinatario_label_3" align="center">
						        	<%=(Partita.getDestinatario_3_id() != -1) ? (toHtmlValue( Partita.getDestinatario_3_nome() )) : ("-- Seleziona Destinatario/Esercente --") %>
						        </div>
					        	<input 
					        		type="hidden" 
						    ...
						    <td>
					        	...
					        	<div id ="destinatario_label_4" align="center">
						        	<%=(Partita.getDestinatario_4_id() != -1) ? (toHtmlValue( Partita.getDestinatario_4_nome() )) : ("-- Seleziona Destinatario/Esercente --") %>	
						        </div>
					        	<input 
					        		type="hidden" 
					    ...
					    <tr class="containerBody">
			            	<td  class="formLabel">Numero Capi <label class="specie2">Caprini</label></td>
					        <td> 
						        <input disabled="disabled" type="text" name="destinatario_3_num_capi_caprini" onchange="isIntPositivo(this.value,'Numero Capi Destinatari',this);" value="<%=((Partita.getDestinatario_3_num_capi_caprini()>0)?(Partita.getDestinatario_1_num_capi_caprini()):("")) %>" id="num_capi_caprini_3" /> 
					        	<font style="display:none;" id="num_capi_caprini_asterisco_3" color="red" >*</font>
					        </td>
					        <td> 
						        <input disabled="disabled" type="text" name="destinatario_4_num_capi_caprini" onchange="isIntPositivo(this.value,'Numero Capi Destinatari',this);" value="<%=((Partita.getDestinatario_4_num_capi_caprini()>0)?(Partita.getDestinatario_2_num_capi_caprini()):("")) %>" id="num_capi_caprini_4" /> 
...
%>						
				<tr id="rigaDest<%=(i+1)/2 %>" style="display:none;">
            	<td colspan="2">
		            <table width="100%" border="0" cellpadding="2" cellspacing="0" align="left">
			            <tr class="containerBody">
			            	<td  class="formLabel">In Regione </td>
          ...
          <tbody>
            <tr>
             ...
             <tr class="containerBody">
                <td class="formLabel">Data</td>
                <td>
                	<%=DateUtils.timestamp2string(Partita.getRca_data())%>
                </td>
             </tr>
             
             <tr class="containerBody">
                <td class="formLabel">Note</td>
                <td><%=toHtmlValue( Partita.getRca_note() ) %></td>
             </tr>
          </tbody>
         </table>
        </td>
...
<input type="button" value="Annulla" onclick="location.href='MacellazioniNewSintesis.do?command=DetailsSeduta&id=<%=Partita.getId()%>&altId=<%=OrgDetails.getAltId()%>';" >
</div><!-- End demo -->
</form>
