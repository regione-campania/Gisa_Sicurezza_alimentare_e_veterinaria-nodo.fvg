...
<input type="button" class="yellowBigButton" style="width: 300px;" 
		onclick="loadModalWindowCustom('Attendere Prego...'); window.location.href='GestionePraticheAction.do?command=CreaPraticaApicoltura';" 
		value="NUOVA PRATICA" />
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
<input type="button" class="yellowBigButton" style="width: 300px;" 
		onclick="loadModalWindowCustom('Attendere Prego...'); window.location.href='GestionePraticheAction.do?command=SearchFormPraticaApi';" 
		value="RICERCA IN TUTTE LE PRATICHE" />
</div>
<br>
<center>
<h3>
<b>Lista ultime pratiche apicoltura</b><br/>
<i>Di seguito sono elencate le ultime 100 pratiche apicoltura inserite nel sistema.</i>
</h3>
<br>
...
<tr>
	...
	<th style="text-align:center; width:25%;"><p>Dati impresa e stabilimento</p></th>
	...
	<th style="text-align:center; ">
		<p>
			<select id="stato_col"  
				...
				<option value="pratica da validare">PRATICA DA VALIDARE</option>
...
<td align="center">
	...
	<a style="text-decoration:none"
		href='GestioneAllegatiGins.do?command=ListaAllegati&numeroPratica=<%=p.getNumeroPratica()%>&desc_operatore=<%=desc_operatore.replaceAll("'", " ")%>&alt_id=0&stab_id=-2&idComunePratica=<%=p.getIdComuneRichiedente()%>'>
...
<td align="center">
		...
		<a style="" href="#" onclick="if(confirm('Passare alla gestione scheda?')==true)
				{
						...
						window.location.href='GestionePraticheAction.do?command=CompletaPratica&stabId=<%=p.getIdStabilimento()%>&altId=<%=p.getAltId()%>&numeroPratica=<%=p.getNumeroPratica()%>&tipoPratica=<%=p.getIdTipologiaPratica()%>&dataPratica=<%=toDateasString(p.getDataOperazione())%>&comunePratica=<%=p.getIdComuneRichiedente()%>&apicoltura=1';		
				}">
	...
	<% } else if (p.getStatoPratica() == 1) { %>
		<a style="" href="#" onclick="if(confirm('Visualizzare pratica?')==true)
				{
						loadModalWindowCustom('Attendere Prego...');
						window.location.href='GisaSuapStab.do?command=Details&altId=<%=p.getAltId()%>';		
				}">
		pratica da validare
		</a>
	<% } else if (p.getStatoPratica() == 4) { %>
		<a style="" href="#" onclick="if(confirm('Visualizzare scheda?')==true)
				{
						loadModalWindowCustom('Attendere Prego...');
						window.location.href='ApicolturaApiari.do?command=Details&stabId=<%=p.getIdStabilimento() %>';		
				}">
		scheda completa
	<% } %>
