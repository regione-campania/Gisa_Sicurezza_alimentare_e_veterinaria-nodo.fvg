...
<script language="JavaScript">
	...
	function load_linee_attivita_per_alt_id_callback(returnValue) {
		  ...
		  <% if (TicketDetails.getSintesisAutomezzo()!=null) {  
		  		 if (TicketDetails.getSintesisAutomezzo().getDataDismissione()==null) { %>
		  			document.getElementById("automezzoSoa").value = <%=TicketDetails.getSintesisAutomezzo().getId()%>;
		  	<%} else { %>
		  			alert("Attenzione. L'automezzo su cui era stato svolto questo controllo e' stato dismesso. La modifica del controllo richiedera' il suo spostamento su un altro automezzo.");
		  <% } } %>
	  ...
	  function costruisci_rel_ateco_attivita( alt_id, campo_combo_da_costruire, default_value ) {
		  ...
		  PopolaCombo.load_linee_attivita_per_alt_id_sintesis(alt_id , campo_combo_da_costruire, default_value, {callback:load_linee_attivita_per_alt_id_callback,async:false}  )
...
<body onLoad="abilitaCodiceAllerta('details');initprovaFunzione('details');abilitaSistemaAllarmeRabido('details');gestioneVisibilitaCodiceAteco('details');resetElementiNucleoIspettivo('<%=TicketDetails.getNucleoasList().size() %>');costruisci_rel_ateco_attivita('<%= OrgDetails.getAltId() %>', 'id_linea_sottoposta_a_controllo', '<%= TicketDetails.getId_imprese_linee_attivita() %>' );">
<%
String param = "altId="+OrgDetails.getAltId()+"&id="+TicketDetails.getIdMacchinetta();
      ...
      <dhv:evaluate if="<%= TicketDetails.getClosed() != null %>" >
          <dhv:permission name="accounts-accounts-vigilanza-edit">
           ...
           <dhv:evaluate if='<%= "list".equals(request.getParameter("return"))%>' >
            <input type="submit" value="Annulla" onClick="javascript:this.form.action='<%=OrgDetails.getAction() %>.do?command=ViewVigilanza&altId=<%=OrgDetails.getAltId()%><%= addLinkParams(request, "popup|popupType|actionId") %>'" />
           </dhv:evaluate>
           <dhv:evaluate if='<%= !"list".equals(request.getParameter("return"))%>' >
...
<%@ include file="../controlliufficiali/alt_controlli_ufficiali_modify.jsp" %>
