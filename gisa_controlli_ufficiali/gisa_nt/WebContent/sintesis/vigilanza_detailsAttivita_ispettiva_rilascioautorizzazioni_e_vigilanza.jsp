...
<script language="JavaScript" TYPE="text/javascript" SRC="javascript/controlli_ufficiali_sintesis.js"></script>
...
<tr>
<td>
  <dhv:label name=""><a href="StabilimentoSintesisAction.do" >Gestione Anagrafica Impresa </a>-><a  href="<%=OrgDetails.getAction()+".do?command=Details&altId="+OrgDetails.getAltId()%>">Scheda Impresa</a> -><a href="<%=OrgDetails.getAction()+".do?command=ViewVigilanza&altId="+OrgDetails.getAltId()%>"> Controlli Ufficiali </a>-> Dettaglio Controllo</dhv:label>
...
<dhv:container name="<%=nomeContainer %>" selected="vigilanza" object="Operatore" param='<%= param%>' >
	...
	<% if (!TicketDetails.isChecklistLibera()){ %>
<table cellpadding="4" cellspacing="0" width="100%" class="empty">
  ...
  <% } %>
	...
	<%
	String permission_op_edit = "sintesis-sintesis-vigilanza-edit" ;
	String permission_op_del = "sintesis-sintesis-vigilanza-delete" ;
	...
	<% if (TicketDetails.getSintesisAutomezzo()!=null) { %>
	<table cellpadding="4" cellspacing="0" class="details" width="100%">
	<col width="20%">
	<tr><th colspan="2"><strong><dhv:label name="">Automezzo SOA: Informazioni automezzo soggetto a controllo</dhv:label></strong>	</th></tr>
	<tr><td class="formLabel">Targa</td> <td><%=TicketDetails.getSintesisAutomezzo().getAutomezzoTarga() %></td></tr>
	<tr><td class="formLabel">Tipo</td> 	<td><%=TicketDetails.getSintesisAutomezzo().getAutomezzoTipo() %></td></tr>
	</table>
	<% } %>
   ...
   <table cellpadding="4" cellspacing="0" width="100%">
     <tr>
	   ...
	   <% if(  TicketDetails.getClosed()==null  && !TicketDetails.isflagBloccoCu()  && TicketDetails.getTipoCampione()!=5 && User.getUserRecord().getGruppo_ruolo()==UserUtils.getUserFormId(request, TicketDetails.getEnteredBy()).getGruppo_ruolo()  && TicketDetails.getTipoCampione() != 3 && TicketDetails.getTipoCampione() != 23 && TicketDetails.getTipoCampione() != 7){ // sorveglianza %>
...
<!--     	</td> -->
    	...
    	%>
    	<td>
    		<a href="<%=OrgDetails.getAction() %>NonConformita.do?command=Add&idC=<%=TicketDetails.getId()%>&idControllo=<%=TicketDetails.getPaddedId() %>&dataC=<%=TicketDetails.getAssignedDate() %>&altId=<%= (OrgDetails.getAltId()==-1)?(TicketDetails.getAltId()):(OrgDetails.getAltId()) %><%= addLinkParams(request, "popup|popupType|actionId") %>"><dhv:label name="accounts.richiesta.add">Inserisci Non Conformità Rilevate</dhv:label></a>
    	</td>
    	<%} %>
