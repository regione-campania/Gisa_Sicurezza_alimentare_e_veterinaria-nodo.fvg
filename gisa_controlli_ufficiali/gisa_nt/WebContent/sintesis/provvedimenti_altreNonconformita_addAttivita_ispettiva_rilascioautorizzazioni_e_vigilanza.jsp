...
<form name="addticket" action="<%=TicketDetails.getURlDettaglio(OrgDetails.getTipologia(), "")%>AltreNonConformita.do?command=Insert&auto-populate=true" method="post">
<input type = "hidden" id = "dosubmit" name = "dosubmit" value = "0">
<%-- Trails --%>
<table class="trails" cellspacing="0">
<tr>
<td>
  <a href="<%=OrgDetails.getAction()%>.do">Operatori</a> > 
  <a href="<%=OrgDetails.getAction()%>.do?command=Search"><dhv:label name="accounts.SearchResults">Search Results</dhv:label></a> >
  <a href="<%=OrgDetails.getAction()%>.do?command=Details&altId=<%=OrgDetails.getAltId()%>">Scheda Operatore</a> >
  <a href="<%=OrgDetails.getAction()%>.do?command=ViewVigilanza&altId=<%=OrgDetails.getAltId()%>"><dhv:label name="">Controlli Ufficiali</dhv:label></a> >
  <a href="<%=OrgDetails.getAction()%>Vigilanza.do?command=TicketDetails&id=<%=request.getAttribute("idC")%>&altId=<%=OrgDetails.getAltId()%>"><dhv:label name="">Controllo Ufficiale</dhv:label></a> >
...
<input type="button" value="Annulla" onClick="if(confirm('ATTENZIONE: si sta per abbandonare la pagina di aggiunta di una non conformita. Lasciando questa pagina il lavoro svolto finora verra perso.Sicuro di voler uscire ?')==true){window.location.href='StabilimentoSintesisActionStabVigilanza.do?command=TicketDetails&id=<%=request.getAttribute("idC")%>&altId=<%=OrgDetails.getAltId()%>';this.form.dosubmit.value='0';}" />
<br>
...
<table cellpadding="4" cellspacing="0" width="100%" class="details">
  ...
  <tr>
    <td class="formLabel">
      <dhv:label name="nonconformita.richiedente">Soggetto Ispezionato</dhv:label>
    </td>
      ...
      <td>
        ...
        <input type="hidden" name="idStabilimento" value="<%=OrgDetails.getOrgId()%>">
 ...
 <%@ include file="../nonconformita/alt_provvedimenti_add.jsp" %>
...
<input type="button" value="Annulla" onClick="javascript:if(confirm('ATTENZIONE: si sta per abbandonare la pagina di aggiunta di una non conformita. Lasciando questa pagina il lavoro svolto finora verra perso.Sicuro di voler uscire ?')==true){window.location.href='StabilimentoSintesisActionVigilanza.do?command=TicketDetails&id=<%= request.getAttribute("idC") %>&altId=<%=OrgDetails.getOrgId()%>';this.form.dosubmit.value='0';}" />
</form>
</body>
