      ...
      <table border="0" cellspacing="0" cellpadding="10" class="empty"  >
        ...
        <%
        	if ((CU.getTipoCampione()==4  || CU.getTipoCampione()==2 ||  (CU.getTipoCampione()==3 && CU.getAssignedDate().after(java.sql.Timestamp.valueOf(org.aspcf.modules.controlliufficiali.base.ApplicationProperties.getProperty("TIMESTAMP_NUOVA_GESTIONE_OGGETTO_DEL_CONTROLLO_AUDIT"))) )) && CU.getLisaElementi_Ispezioni().size()!=0)
        	{
        		...
        		%>
        		<select name = "Provvedimenti2_1" id = "Provvedimenti2_1" size = "6" onchange="abilitaFlagSignificative();abilitaStatoSignificative('<%=CU.getId()%>');calcolaPuntiNonConformita(document.getElementById('elementi_nc_significative').value,'Provvedimenti2_','puntiSignificativi','nonConformitaSignificative_',<%=CU.getTipoCampione() %>,'<%=CU.getAssignedDate() %>'); gestisciBenessereNonConformita(this)" size = "9">	
        	...
        	Provvedimenti.setJsEvent("onchange=abilitaFlagSignificative();abilitaStatoSignificative('"+CU.getId()+"');calcolaPuntiNonConformita(document.getElementById('elementi_nc_significative').value,'Provvedimenti2_','puntiSignificativi','nonConformitaSignificative_',"+CU.getTipoCampione() +","+CU.getAssignedDate() +"'); gestisciBenessereNonConformita(this)"); %>
         ...
         <%=ProvvedimentiBenessereMacellazione.getHtmlSelect("ProvvedimentiBenessereMacellazione2_1", -1) %>
         <%ProvvedimentiBenessereTrasporto.setJsEvent("style=\"display:none\""); %>
         <%=ProvvedimentiBenessereTrasporto.getHtmlSelect("ProvvedimentiBenessereTrasporto2_1", -1) %>
