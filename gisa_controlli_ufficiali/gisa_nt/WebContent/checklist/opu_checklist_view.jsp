...
<tr class="row<%= rowid+1%>">
  ...
  <td>
    <input disabled="disabled" type="text" style="width: 40px;" id="valoreRange<%= indexChecklistList%>" name="valoreRange<%= checklistTypeId%>" value="<%=numero%>" />
    <input type="hidden" id="operazione<%= indexChecklistList%>" name="operazione<%= indexChecklistList%>"/>
  </td>
</tr>
<tr class="row<%= rowid+1%>" >
  <td colspan="7">
  <textarea readonly="readonly" rows="3" cols="80" id="nota<%= indexChecklistList%>"   name="nota<%= indexChecklistList%>" >
  <%=nota %>
  </textarea>
  </td>
</tr>
