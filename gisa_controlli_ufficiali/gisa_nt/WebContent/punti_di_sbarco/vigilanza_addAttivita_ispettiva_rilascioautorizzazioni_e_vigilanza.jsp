...
costruisci_rel_ateco_attivita('<%= OrgDetails.getOrgId() %>', 'id_linea_sottoposta_a_controllo');">
 ...
 <%
    ...
    TipoCampione.setJsEvent("onChange=javascript:reloadAddCU('PuntiSbarcoVigilanza.do?command=Add&orgId="+OrgDetails.getOrgId()+"&tipoCampione='+this.value)"); 


%>


<form method="post" name="addticket" action="PuntiSbarcoVigilanza.do?command=Insert&auto-populate=true"  >
<%-- Trails --%>
...
<td>
  ...
  <a href="PuntiSbarco.do?command=Details&orgId=<%=OrgDetails.getOrgId()%>">Scheda Punti Di Sbarco</a> >
...
<input type="submit" value="Annulla" onClick="javascript:this.form.action='PuntiSbarcoVigilanza.do?command=ViewVigilanza&orgId=<%=OrgDetails.getOrgId()%>'">
<br>
...
<%@ include file="../controlliufficiali/controlli_ufficiali_operatorePuntiSbarco.jsp" %> 
<br><br>
...
<br>
<input type="button" value="<dhv:label name="button.inserta">Inserisci </dhv:label>" name="Save" onClick="<%--javascript:this.form.action='PuntiSbarcoVigilanza.do?command=InsertContinua&auto-populate=true';--%>return controlloCuSorveglianza()">
<input type="submit" value="Annulla" onClick="javascript:this.form.action='PuntiVigilanza.do?command=ViewVigilanza&orgId=<%=OrgDetails.getOrgId()%>'">
</form>
</body>
