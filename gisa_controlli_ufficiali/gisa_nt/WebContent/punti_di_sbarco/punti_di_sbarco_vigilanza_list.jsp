...
<%@ page import="java.util.*,java.text.DateFormat,org.aspcfs.modules.punti_di_sbarco.base.*,org.aspcfs.modules.vigilanza.base.Ticket,com.zeroio.iteam.base.*" %>
...
<%@ include file="punti_di_sbarco_vigilanza_list_menu.jsp" %>
...
<a href="PuntiSbarco.do?command=Search"><dhv:label name="">Ricerca Punto di Sbarco</dhv:label></a> >
<a href="PuntiSbarco.do?command=Details&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label name="">Dettaglio Punto di sbarco</dhv:label></a> >
...
<dhv:container name="punti_di_sbarco" selected="vigilanza" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId") %>'>
  <dhv:evaluate if="<%= !OrgDetails.isTrashed() %>">
    ...
    <dhv:permission name="punti_di_sbarco-vigilanza-add"><a href="PuntiSbarcoVigilanza.do?command=Add&orgId=<%= OrgDetails.getOrgId() %><%= addLinkParams(request, "popup|popupType|actionId") %>"><dhv:label name="accounts.richiesta.add">Aggiungi Nuovo Controllo Ufficiale</dhv:label></a></dhv:permission>
  </dhv:evaluate>
