...
<form name="viewAccountAudit" action="CheckListPuntiSbarco.do?command=Modify&auto-populate=true&id=<%=Audit.getId()%>&return=details<%= addLinkParams(request, "popup|popupType|actionId") %>" method="post">
...
<dhv:permission name="punti_di_sbarco-report-view">
    ...
    <tr>
       <td nowrap align="right">
        ...
        <%--<a href="Accounts.do?command=PrintReport&id=<%= OrgDetails.getId() %>"><dhv:label name="accounts.osa.print">Stampa Scheda Audit</dhv:label></a>--%>
         ...
         <input type="button" value="Stampa Scheda Check List Provvisorio"	onClick='javascript:location.href="CheckListPuntiSbarco.do?command=Stampa&id=<%= Audit.getId() %>";'>
      <%
    }
    else {
    %>
        <input type="button" value="Stampa Scheda Check List"	onClick='javascript:location.href= "CheckListPuntiSbarco.do?command=Stampa&id=<%= Audit.getId() %>";'>
    <%	
    }
    --%>
...
<dhv:evaluate if="<%= !OrgDetails.isTrashed() %>">
  <input type="hidden" name="idC" value="<%=Audit.getIdControllo() %>">
     ...
     <dhv:permission name="punti_di_sbarco-audit-edit"><input type="submit" value="Modifica"></dhv:permission>
