...
function apriVerbale(idCampione,tipoAnalisi)
{
	...
	document.location.href="PuntiSbarcoCampioni.do?command=StampaScheda&tipoAnalisi="+tipoAnalisi+"&idCampione="+idCampione+"&tipoAlimenti="+tipoAlimenti;
		
}
</script>

<%@ include file="../utils23/initPage.jsp" %>
<form id="details" name="details" action="PuntiSbarcoCampioni.do?command=ModifyTicket&auto-populate=true" method="post">
...
<td>
<td>
  <a href="PuntiSbarco.do?command=SearchForm">PuntiSbarco</a> > 
...
<dhv:container name="punti_di_sbarco" selected="vigilanza" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId() %>' hideContainer='<%= isPopup(request) || (defectCheck != null && !"".equals(defectCheck.trim())) %>'>
	...
	else
	{
		...
		<tr>
		<td>
			<a href = "PuntiSbarcoCampioni.do?command=StampaScheda&TipoAnalisi=<%=TicketDetails.getTipoCampione()%>&idCampione=<%=TicketDetails.getId() %>">Stampa PRI-18-Mod-2-Verbale-campione-Viruss</a>
		</td>
	</tr>		
	<% }
	%> 
...
if(request.getAttribute("Messaggio2")!=null)
{
	...
	function doSubmit(idTicket){
		...
		document.details.action="PuntiSbarcoCampioni.do?command=Chiudi&id="+idTicket+"&chiudiCu=true'"
