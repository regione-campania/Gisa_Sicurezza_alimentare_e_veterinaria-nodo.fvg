...
<jsp:useBean id="OrgListOp" class="org.aspcfs.modules.punti_di_sbarco.base.OrganizationList" scope="request"/>
...
<script language="JavaScript" TYPE="text/javascript" SRC="javascript/controlli_ufficiali_puntisbarco.js"></script>

<script language="JavaScript" TYPE="text/javascript">

function openPopupDetails(orgId,tipo){
			var res;
			var result;
			if(tipo == 1){
				window.open('Accounts.do?command=Details&orgId='+orgId+'&popup=true','popupSelect',
				...
				window.open('Abusivismi.do?command=Details&orgId='+orgId+'&popup=true','popupSelect',
				'height=300px,width=580px,toolbar=no,directories=no,status=no,continued from previous linemenubar=no,scrollbars=no,resizable=no ,modal=yes');
				}
			} 
</script>		
...
<form name="details" action="PuntiSbarcoVigilanza.do?command=ModifyTicket&auto-populate=true" method="post">
<table class="trails" cellspacing="0">
<tr>
<td>
  <a href="PuntiSbarco.do?command=SearchForm">Punti Di Sbarco</a> > 
  <a href="PuntiSbarco.do?command=Search"><dhv:label name="">Risultati Ricerca</dhv:label></a> >
  <a href="PuntiSbarco.do?command=Details&orgId=<%=TicketDetails.getOrgId()%>">Scheda Punti Di Sbarco</a> >
  <a href="PuntiSbarcoVigilanza.do?command=ViewVigilanza&orgId=<%=TicketDetails.getOrgId()%>"><dhv:label name="vigilanza">Tickets</dhv:label></a> >
...
%>
<dhv:container name="punti_di_sbarco" selected="vigilanza" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId() %>' hideContainer='<%= isPopup(request) || (defectCheck != null && !"".equals(defectCheck.trim())) %>'>
...
<table cellpadding="4" cellspacing="0" width="100%" class="empty">
  ...
  <tr>
   ...
   <td>
      ...
      <a href="CheckListPuntiSbarco.do?command=View&id=<%= thisAudit.getId()%>&aggiorna=<%=thisAudit.isAggiornaCategoria() %>"><%= toHtml(OrgCategoriaRischioList.getSelectedValue(thisAudit.getTipoChecklist())) %></a>
	</td>
	<%}%>
...
<%--} --%>
	...
	<table cellpadding="5" cellspacing="0" width="100%" class="details">
	    ...
	    <tr>
		   <th><b><dhv:label name="">Tipo Operatore </dhv:label></b></th>
   		   <th><b><dhv:label name="">Ragione Sociale</dhv:label></b></th>
   		   <th><b><dhv:label name="">Num. Reg</dhv:label></b></th>
   		   <th><b><dhv:label name="">Azione</dhv:label></b></th>
       </tr>
   <%
   			
   Iterator op_p = OrgDetails.getOpControllatiList().iterator();
   if (op_p.hasNext()) {
     while (op_p.hasNext()) {
    	 org.aspcfs.modules.punti_di_sbarco.base.Organization thisOpP = (org.aspcfs.modules.punti_di_sbarco.base.Organization)op_p.next();
    %> 
   <tr>
    <td>
    	<% if(thisOpP.getTipologia() == 1) { %>
       		<%= toHtml("IMPRESA") %>
       <% } else {
    	   
    	    if(thisOpP.getTipologia() == 17) { %>
	...
	<td>
	<% if(thisOpP.getAccountNumber() != null && !thisOpP.getAccountNumber().equals("")) { %>
       		<%= thisOpP.getAccountNumber() %>
       <% } else { %>
      	<%= toHtml("ND") %>
       <% } %>
	</td>
	<td>
	 	<a href="javascript:openPopupDetails('<%= thisOpP.getOrgId() %>','<%= thisOpP.getTipologia()%>');">Dettaglio</a>
	</td>
<%-- <td>
		<a href="javascript:popLookupSelectorCustomCU('<%= thisOpP.getTipologia() %>','<%= thisOpP.getOrgId() %>')">Aggiungi C.U.</a>
	</td>
	--%>	
   </tr>
   <% } %>
