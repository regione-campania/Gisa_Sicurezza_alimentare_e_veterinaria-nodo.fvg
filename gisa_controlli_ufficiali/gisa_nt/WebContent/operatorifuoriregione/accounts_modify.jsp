...
<form name="addAccount" action="OperatoriFuoriRegione.do?command=Update&orgId=<%= OrgDetails.getOrgId() %>&tipoD=<%=OrgDetails.getTipoDest()%>&auto-populate=true<%= (request.getParameter("popup") != null?"&popup=true":"") %>" onSubmit="return doCheck(this);" method="post">
 <input type="hidden" id="tipoD" value="<%=OrgDetails.getTipoDest()%>" />
<%
  boolean popUp = false;
...
<%} else {%>
<a href="OperatoriFuoriRegione.do?command=Search"><dhv:label name="accounts.SearchResults">Search Results</dhv:label></a> >
<a href="OperatoriFuoriRegione.do?command=Details&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label name="">Dettaglio Attività Mobili Fuori Ambito ASL</dhv:label></a> >
<%}%>
<dhv:label name="">Modifica Attività Fuori Ambito ASL</dhv:label>
</td>
</tr>
</table>
<%-- End Trails --%>
...
<table cellpadding="4" cellspacing="0" border="0" width="100%" class="details">
  ...
  <tr class="containerBody">
      ...
      <td>
            ...
            <input class="date_picker" type="text" id="data_nascita" name="data_nascita" size="10" value = "<%=toDateString(Conducente.getData_nascita()) %>" />
