...
<body
	onload="provaFunzione('addticket'); onloadAllerta('addticket'); 
	...
	<form name="addticket"
		action="OperatoriFuoriRegioneVigilanza.do?command=Insert&auto-populate=true"
		method="post">
		<%-- Trails --%>
		<table class="trails" cellspacing="0">
			<tr>
				<td><a href="OperatoriFuoriRegione.do"><dhv:label name="">Attività Mobile Fuori Ambito ASL</dhv:label></a>
					> <a href="OperatoriFuoriRegione.do?command=Search"><dhv:label
							name="accounts.SearchResults">Search Results</dhv:label></a> > <a
					href="OperatoriFuoriRegione.do?command=Details&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label
							name="">Scheda Opertore Altre ASL della Campania</dhv:label></a> > <a
					href="OperatoriFuoriRegione.do?command=ViewVigilanza&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label
							name="vigilanza">Tickets</dhv:label></a> > <dhv:label
						name="campioni.aggiungi">Aggiungi Controllo Ufficiale</dhv:label>
				</td>
			</tr>
		</table>
		<%-- End Trails --%>
		<input type="button"
			...
			name="Save"
			onClick="<%--javascript:this.form.action='AccountVigilanza.do?command=InsertContinua&auto-populate=true';--%>return controlloCuSorveglianza()">
		<input type="submit"
			value="Annulla"
			onClick="javascript:this.form.action='OperatoriFuoriRegione.do?command=ViewVigilanza&orgId=<%=OrgDetails.getOrgId()%>'">
		<br>
		<dhv:formMessage />
		...
		<jsp:useBean id="AuditTipo" class="org.aspcfs.utils.web.LookupList"
			scope="request" />
		<jsp:useBean id="resolutionList"
			class="org.aspcfs.utils.web.LookupList" scope="request" />
		<jsp:useBean id="EscalationList"
			class="org.aspcfs.utils.web.LookupList" scope="request" />
		<jsp:useBean id="OrgList"
			class="org.aspcfs.modules.operatorifuoriregione.base.OrganizationList"
			scope="request" />
		...
		<script language="JavaScript" TYPE="text/javascript"
			SRC="javascript/controlli_ufficiali_operatorifr.js"></script>

		<script type="text/javascript"
			src="dwr/interface/ControlliUfficiali.js?n=2"> </script>
		...
		<body onload="provaFunzione('addticket'); onloadAllerta('addticket')">
			...
			<%
				...
				TipoCampione.setJsEvent("onChange=javascript:reloadAddCU('OperatoriFuoriRegioneVigilanza.do?command=Add&orgId="+OrgDetails.getOrgId()+"&tipoCampione='+this.value)");
