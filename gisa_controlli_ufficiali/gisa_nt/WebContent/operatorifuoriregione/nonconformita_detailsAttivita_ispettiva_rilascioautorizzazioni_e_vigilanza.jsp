...
<body onload="verificaChiusuraNc(<%=request.getAttribute("Chiudi")%>,<%=request.getAttribute("numSottoAttivita")%>,'<%=request.getAttribute("Messaggio2")%>',<%=TicketDetails.getIdControlloUfficiale() %>,'OperatoriFuoriRegioneVigilanza')">

<form name="details" action="OperatoriFuoriRegioneNonConformita.do?command=ModifyTicket&auto-populate=true" method="post">
<input type = "hidden" name = "idC" value = "<%=TicketDetails.getIdControlloUfficiale() %>">
<table class="trails" cellspacing="0">
<tr>
<td>
<td>
  <a href="OperatoriFuoriRegione.do"><dhv:label name="">OperatoriFuoriRegiones</dhv:label></a> > 
...
%>
<dhv:container name="operatoriregione" selected="vigilanza" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId() %>' hideContainer='<%= isPopup(request) || (defectCheck != null && !"".equals(defectCheck.trim())) %>'>
<dhv:container name="operatorifuoriregionenonconformita" selected="details" object="TicketDetails"
	param="<%= param1 %>"
