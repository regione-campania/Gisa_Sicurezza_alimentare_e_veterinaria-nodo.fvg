...
<%@ page
	import="java.util.*,java.text.DateFormat,org.aspcfs.modules.operatorifuoriregione.base.*,org.aspcfs.utils.web.*,org.aspcfs.modules.contacts.base.*"%>
...
<jsp:useBean id="TipoStruttura" class="org.aspcfs.utils.web.LookupList"
	scope="request" />
<jsp:useBean id="TipoLocale" class="org.aspcfs.utils.web.LookupList"
	scope="request" />
<jsp:useBean id="IstatList" class="org.aspcfs.utils.web.LookupList"
	scope="request" />
<jsp:useBean id="Conducente"
	class="org.aspcfs.modules.operatorifuoriregione.base.AttoriOpfuoriasl"
	scope="request" />
<jsp:useBean id="Mittente"
	class="org.aspcfs.modules.operatorifuoriregione.base.AttoriOpfuoriasl"
	scope="request" />
<jsp:useBean id="Address"
	class="org.aspcfs.modules.operatorifuoriregione.base.OrganizationAddress"
	scope="request" />
...
<jsp:useBean id="OrgDetails"
	class="org.aspcfs.modules.operatorifuoriregione.base.Organization"
	scope="request" />
<jsp:useBean id="OrgDetailsAttori"
	class="org.aspcfs.modules.operatorifuoriregione.base.AttoriOpfuoriasl"
	scope="request" />
<jsp:useBean id="indirizzoImpresa"
	class="org.aspcfs.modules.accounts.base.OrganizationAddress"
	scope="request" />
<jsp:useBean id="OrgDetailsImpresa"
...
<jsp:useBean id="SiteList" class="org.aspcfs.utils.web.LookupList"
	scope="request" />
<jsp:useBean id="Site_2" class="org.aspcfs.utils.web.LookupList"
	scope="request" />
...
<script language="JavaScript" TYPE="text/javascript">
	...
	function changeDivContent(divName, divContents) {
		...
		if (divName == 'changeaccount') {
			...
			changeDivContent('addServiceContract', label('none.selected',
					'None Selected'));
			resetNumericFieldValue('contractId');
			</dhv:include>
			<dhv:include name="ticket.contractNumber" none="false">
			changeDivContent('addAsset',
...
<%-- dhv:include name="ticket.laborCategory" none="false">
	...
	function updateFormElementsNew(index) {
		...
		if (index == 1) {
			...
			if (document.getElementById("starMobil10"))
				document.getElementById("starMobil10").style.display = "";
		} else if (index == 0) {
			if (document.getElementById("starMobil3") != undefined) {
			...
			if (document.getElementById("starMobil10"))
				document.getElementById("starMobil10").style.display = "none";
	...
	function update(countryObj, stateObj, selectedValue) {
		...
		var url = "ExternalContacts.do?command=States&country=" + country
				+ "&obj=" + stateObj + "&selected=" + selectedValue
				+ "&form=addAccount&stateObj=address" + stateObj + "state";
	...
	function resetStateList(country, stateObj) {
		var stateSelect = document.forms['addAccount'].elements['address'
				+ stateObj + 'state'];
		var i = 0;
		if (initStates == false) {
			for (i = stateSelect.options.length - 1; i > 0; i--) {
				var state = new Array(stateSelect.options[i].value,
						stateSelect.options[i].text);
		...
		for (i = states.length - 1; i > 0; i--) {
			var state = states[i];
			if (state[0].indexOf(country) != -1
					|| country == label('option.none', '-- None --')) {
...
<script>
	function gestisciTipoImpresa(val) {
		if (val == 'fissa') {
...
<dhv:evaluate if="<%=!"true".equals(popUp)%>">
	<%-- Trails --%>
...
<dhv:evaluate
	if='<%=(request.getParameter("form_type") == null || "organization".equals((String) request
						.getParameter("form_type")))%>'>
...
<form name="addAccount"
	action="OperatoriFuoriRegione.do?command=Insert&auto-populate=true"
	method="post">
	...
	<input type="radio" id="tipo_mobile" name="tipo_impresa" value="mobile"
		onClick="gestisciTipoImpresa(this.value)" /> Mobile <input
		type="radio" id="tipo_fissa" name="tipo_impresa" value="fissa" checked
		onClick="gestisciTipoImpresa(this.value)" /> Fissa
	...
	<table cellpadding="4" cellspacing="0" border="0" width="100%"
		class="details">
		<tr>
			<th colspan="2"><strong><dhv:label name="">Impresa</dhv:label></strong>
			</th>
		</tr>

		<input type="hidden" name="siteId" value="16">
		<input type="hidden" name="fuori_regione" value="true" checked>
		<input type="hidden" name="orgId" id="orgId"
			value="<%=OrgDetailsImpresa.getOrgId()%>" />

		<dhv:include name="accounts-name" none="true">
			<tr>
				<td nowrap class="formLabel" name="orgname1" id="orgname1"><dhv:label
						name="accounts.accounts_add.OrganizationName">Organization Name</dhv:label>
				</td>
				<td><input onFocus="if (indSelected == 1) { tabNext(this) }"
					type="text" size="50" maxlength="80" name="name"
					value="<%=toHtmlValue(OrgDetailsImpresa.getName())%>"><font
    ...
    <td>
      <input type="text" size="50" maxlength="80" name="banca" value="<%= toHtmlValue(OrgDetails.getBanca()) %>">
		...
		<dhv:include name="organization.accountNumber" none="true">
			<tr>
				<td nowrap class="formLabel"><dhv:label name="">Numero Registrazione</dhv:label>
				</td>
				<td><input type="text" size="30" name="codiceImpresaInterno"
					maxlength="30"
					value="<%=toHtmlValue(OrgDetailsImpresa.getCodiceImpresaInterno())%>">
		...
		<tr>
			...
			<td><input type="text" size="20" maxlength="11"
				name="partitaIva"
				value="<%=toHtmlValue(OrgDetailsImpresa.getPartitaIva())%>">
			</td>
		</tr>
		...
		<tr>
			...
			<td><input type="text" size="20" maxlength="16"
				name="codiceFiscale"
				value="<%=toHtmlValue(OrgDetailsImpresa.getCodiceFiscale())%>">
			</td>
		</tr>
	...
	<table cellpadding="4" cellspacing="0" border="0" width="100%"
		...
		<tr>
			<th colspan="2"><strong><dhv:label name="">Titolare o Legale Rappresentante</dhv:label></strong>
				<%--input type="hidden" name="address1type" value="1"--%></th>
		</tr>
		<tr>
			<td class="formLabel" nowrap>Codice Fiscale
			</td>
			<td><input type="text" size="30" maxlength="16"
				name="codiceFiscaleRappresentante"
				value="<%=toHtmlValue(OrgDetailsImpresa.getCodiceFiscaleRappresentante())%>">
			</td>
		</tr>
		<tr>
			<td class="formLabel" nowrap>Nome
			</td>
			<td><input type="text" size="30" maxlength="50"
				name="nomeRappresentante"
				value="<%=toHtmlValue(OrgDetailsImpresa.getNomeRappresentante())%>">
			</td>
		</tr>
		<tr>
			<td class="formLabel" nowrap>Cognome
			</td>
			<td><input type="text" size="30" maxlength="50"
				name="cognomeRappresentante"
				value="<%=toHtmlValue(OrgDetailsImpresa.getCognomeRappresentante())%>">
			</td>
		</tr>
		<tr>
			<td nowrap class="formLabel"><dhv:label name="">Data Nascita</dhv:label>
			</td>
			<td><zeroio:dateSelect form="addAccount"
					field="dataNascitaRappresentante"
					timestamp="<%=OrgDetailsImpresa.getDataNascitaRappresentante()%>"
					showTimeZone="false" /> <%=showAttribute(request, "alertDateError")%>
				<%=showWarningAttribute(request, "alertDateWarning")%></td>
		</tr>
		<tr>
			<td class="formLabel" nowrap><dhv:label name="">Comune di nascita</dhv:label>
			</td>
			<td><input type="text" size="30" maxlength="50"
				name="luogoNascitaRappresentante"
				value="<%=toHtmlValue(OrgDetailsImpresa.getLuogoNascitaRappresentante())%>">
			</td>
		</tr>

		<tr>
			<td class="formLabel" nowrap><dhv:label name="">Email</dhv:label>
			</td>
			<td><input type="text" size="30" maxlength="50"
				name="emailRappresentante"
				value="<%=toHtmlValue(OrgDetailsImpresa.getEmailRappresentante())%>">
			</td>

		</tr>

		<tr>
			<td class="formLabel" nowrap><dhv:label name="">Telefono</dhv:label>
			</td>
			<td><input type="text" size="30" maxlength="50"
				name="telefonoRappresentante"
				value="<%=toHtmlValue(OrgDetailsImpresa.getTelefonoRappresentante())%>">
			</td>

		</tr>

		<tr>
			<td class="formLabel" nowrap><dhv:label name="">Fax</dhv:label>
			</td>
			<td><input type="text" size="30" maxlength="50" name="fax"
				value="<%=toHtmlValue(OrgDetailsImpresa.getFax())%>"></td>
	...
	<table cellpadding="4" cellspacing="0" border="0" width="100%"
		...
		<tr>
			<td nowrap class="formLabel">
				<div id="label_fuori" style="display: block;">
					<dhv:label name="">Nazione D'origine</dhv:label>
				</div>
				<div id="label_in" style="display: none;">
			...
			<td>
				...
				<input type="hidden" size="40" name="address1line3" maxlength="80"
				value="<%=(OrgDetailsImpresa.getSiteId() > 0 ? (OrgDetailsImpresa.getSiteId()) : (""))%>">
		...
		<tr>
			<td nowrap class="formLabel" name="province" id="province"><dhv:label
					name="requestor.requestor_add.City">City</dhv:label></td>
			<td><input type="text" name="address1city" id="address1city"
				value="<%=toHtmlValue(indirizzoImpresa.getCity())%>"></td>
		</tr>
		<tr>
			<td nowrap class="formLabel"><dhv:label
					name="requestor.requestor_add.AddressLine1">Address Line 1</dhv:label>
			</td>
			<td><input type="text" size="40" name="address1line1"
				maxlength="80"
				value="<%=toHtmlValue(indirizzoImpresa.getStreetAddressLine1())%>">
			</td>
		</tr>

		<tr>
			<td nowrap class="formLabel"><dhv:label
					name="requestor.requestor_add.ZipPostalCode">Zip/Postal Code</dhv:label>
			</td>
			<td><input type="text" size="28" name="address1zip"
				maxlength="12" value="<%=toHtmlValue(indirizzoImpresa.getZip())%>">
			</td>
		</tr>
		<tr>
			<td nowrap class="formLabel"><dhv:label
					name="requestor.requestor_add.StateProvince">State/Province</dhv:label>
			</td>
			<td><input type="text" size="28" name="address1state"
				maxlength="80"
				value="<%=toHtmlValue(indirizzoImpresa.getCityState())%>">
			</td>
		</tr>
	...
	<table cellpadding="4" cellspacing="0" border="0" width="100%"
		...
		<tr>
			<th colspan="2"><strong><dhv:label
						name="accounts.accounts_add.AdditionalDetails">Additional Details</dhv:label></strong>
			</th>
		</tr>
		<tr>
			<td valign="middle" nowrap class="formLabel"><dhv:label name="">Luogo di controllo/Note</dhv:label>
			</td>
			<td><TEXTAREA NAME="notes" ROWS="3" COLS="50"><%=toString(OrgDetails.getNotes())%></TEXTAREA></td>
		</tr>
	</table>

	<input type="hidden" id="tipoD" name="tipoDest" value="Operatori">

	<input type="hidden" name="onlyWarnings"
		value='<%=(OrgDetails.getOnlyWarnings() ? "on" : "off")%>' /> <input
		type="button" value="Inserisci" onClick="checkForm(this.form)">

	<input type="button"
		value="Annulla"
		onClick="window.location.href='OperatoriFuoriRegione.do?command=SearchForm'"> <input type="hidden"
		name="tipologia" id="tipologia" value="Operatori" /> <input
		type="hidden" name="popUp" id="popUp" value="<%=popUp%>" /> <input
		type="hidden" name="dosubmit" value="true" />
</form>
</body>