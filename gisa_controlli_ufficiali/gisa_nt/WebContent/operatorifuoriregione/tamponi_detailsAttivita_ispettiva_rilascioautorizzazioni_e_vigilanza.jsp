...
<form name="details" action="OperatoriFuoriRegioneTamponi.do?command=ModifyTicket&auto-populate=true" method="post">
...
<td>
<td>
  <a href="OperatoriFuoriRegione.do">Operatori Fuori Regione</a> > 
  <a href="OperatoriFuoriRegione.do?command=Search"><dhv:label name="accounts.SearchResults">Search Results</dhv:label></a> >
  <a href="OperatoriFuoriRegione.do?command=Details&orgId=<%=OrgDetails.getOrgId()%>">Scheda Operatore Fuori Regione</a> >
...
%>
<dhv:container name="operatorifuoriregione" selected="vigilanza" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId() %>' hideContainer='<%= isPopup(request) || (defectCheck != null && !"".equals(defectCheck.trim())) %>'>

	<%@ include file="ticket_header_include_tamponi.jsp"%>
	<%
		if (TicketDetails.isTrashed()) {
	%>
	<dhv:permission name="operatoriregione-operatoriregione-tamponi-delete">
		<input type="button"
			value="Ripristina"
			onClick="javascript:this.form.action='OperatoriFuoriRegioneTamponi.do?command=Restore&id=<%= TicketDetails.getId()%>';submit();">
	</dhv:permission>
	<%
	...
	<dhv:permission name="reopen-reopen-view">
		<input type="button"
			...
			onClick="javascript:this.form.action='OperatoriFuoriRegioneTamponi.do?command=ReopenTicket&id=<%= TicketDetails.getId()%><%= (defectCheck != null && !"".equals(defectCheck.trim())?"&defectCheck="+defectCheck:"") %>';submit();">
	</dhv:permission>
	<%
		} else {
	%>
	<dhv:permission name="operatoriregione-operatoriregione-tamponi-edit">
		<input type="button"
			value="<dhv:label name="global.button.modifya">Modifica/Inserisci Esito</dhv:label>"
			onClick="javascript:this.form.action='OperatoriFuoriRegioneTamponi.do?command=ModifyTicket&auto-populate=true<%= (defectCheck != null && !"".equals(defectCheck.trim())?"&defectCheck="+defectCheck:"") %>';submit();">
	</dhv:permission>
	<%--
	...
	<dhv:permission name="operatoriregione-operatoriregione-tamponi-delete">
		...
		<input type="button"
			...
			onClick="javascript:popURL('OperatoriFuoriRegioneTamponi.do?command=ConfirmDelete&id=<%= TicketDetails.getId() %>&orgId=<%=OrgDetails.getOrgId() %>&return=searchResults&popup=true', 'Delete_ticket','320','200','yes','no');">
		<%
			} else {
		%>
		<input type="button"
			value="<dhv:label name="global.button.delete">Delete</dhv:label>"
			onClick="javascript:popURL('OperatoriFuoriRegioneTamponi.do?command=ConfirmDelete&id=<%= TicketDetails.getId() %>&orgId=<%=OrgDetails.getOrgId() %>&popup=true', 'Delete_ticket','320','200','yes','no');">
		<%
			}
		%>
	</dhv:permission>
	<dhv:permission name="operatoriregione-operatoriregione-tamponi-edit">
		<input type="button"
			...
			onClick="javascript:this.form.action='OperatoriFuoriRegioneTamponi.do?command=Chiudi&id=<%= TicketDetails.getId() %>';if( confirm('Sei sicuro di voler chiudere il tampone? \n Attenzione! La pratica verrà chiusa e non sarà più possibile fare modifiche \n sulla scheda se non con il permesso del supervisore o dell amministratore') ){submit()};">
	</dhv:permission>
	<%
		}
	%>
...
if(request.getAttribute("Messaggio2")!=null)
{
	...
	function doSubmit(idTicket){
		...
		document.details.action="OperatoriFuoriRegioneTamponi.do?command=Chiudi&id="+idTicket+"&chiudiCu=true'"
