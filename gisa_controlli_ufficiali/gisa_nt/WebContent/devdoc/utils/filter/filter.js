...
<div id="container-filtro">
    ...
    <form id="filtro">
        ...
        <div class="campo-filtro" id="campo-id">
        <label for="id">ID</label>
        <input id="id" name="id" placeholder="Cerca per ID">
    </div>
        ...
        <div class="campo-filtro" id="campo-referente">
        <label for="referente">Referente</label>
        <input id="referente" name="referente" placeholder="Cerca per referente">
    </div>
...
function Filter() {
    ...
    this.fields = {
        ...
        id: this.form.elements.namedItem('id'),
        richiesta: this.form.elements.namedItem('richiesta'),
        referente: this.form.elements.namedItem('referente'),
...
Filter.prototype.apply = function () {
    ...
    Array.from(table.rows).forEach((tr, index) => {
        ...
        const idValue = this.fields.id.value.trim()
        if (isRowVisible && idValue !== '') {
            let tdId = tr.cells[indiciColonne["id"]]
            if (!(tdId.innerText.find(idValue)))
                isRowVisible = false
        }
        ...
        const referenteValue = this.fields.referente.value.trim()
        if (isRowVisible && referenteValue !== '') {
            let tdReferente = tr.cells[indiciColonne["referente"]]
            if (!(tdReferente.innerText.find(referenteValue)))
                isRowVisible = false
        }
        ...
        if (isRowVisible && statoValue !== 'all') {
            ...
            switch (statoValue) {
                case 'aperto': if (!(tdStato.innerText.find('aperto'))) isRowVisible = false; break;
                case 'consegnato': if (!(tdStato.innerText.find('consegnato'))) isRowVisible = false; break;
                case 'standby': if (!(tdStato.innerText.find('standby'))) isRowVisible = false; break;
                case 'annullato': if (!(tdStato.innerText.find('annullato'))) isRowVisible = false; break;
                case 'collaudato': if (!(tdStato.innerText.find('collaudato'))) isRowVisible = false; break;
                default: break;
            }
        }
