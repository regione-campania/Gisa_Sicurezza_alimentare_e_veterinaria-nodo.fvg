...
<body>
<form name="details" action="GestioneAnagraficaFollowup.do?command=UpdateTicket&auto-populate=true<%= addLinkParams(request, "popup|popupType|actionId") %>"  method="post">
...
<%--   <a href="GestioneAnagraficaAction.do?command=Search"><dhv:label name="accounts.SearchResults">Search Results</dhv:label></a> > --%>
  <a href="GestioneAnagraficaAction.do?command=Details&altId=<%=OrgDetails.getAltId()%>"><dhv:label name="accounts.details">GestioneAnagrafica Details</dhv:label></a> > 
...
<%} %>
  ...
  <a href="GestioneAnagraficaFollowup.do?command=TicketDetails&id=<%=TicketDetails.getId()%>"><dhv:label name="followup.dettagli">Scheda Follow Up</dhv:label></a> >
  <%}%>
...
<dhv:container name="gestioneanagrafica" selected="vigilanza" object="OrgDetails" param='<%= "altId=" + OrgDetails.getAltId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId") %>'>
   	...
   	<%@ include file="../controlliufficiali/alt_followup_modify.jsp" %>
