...
<script language="JavaScript">
	...
	function load_linee_attivita_per_alt_id_stabilimento_opu_callback(returnValue) {
	  ...
	  function costruisci_rel_ateco_attivita( alt_id, campo_combo_da_costruire, default_value ) {
		  //alert(org_id);
		  //alert(campo_combo_da_costruire);
		  //alert(default_value);
		
		  PopolaCombo.load_linee_attivita_per_alt_id_stabilimento_opu(alt_id , campo_combo_da_costruire, default_value, {callback:load_linee_attivita_per_alt_id_stabilimento_opu_callback,async:false}  )
...
<body onLoad="resetElementiNucleoIspettivo('<%=TicketDetails.getNucleoasList().size() %>'); abilitaCodiceAllerta('details');initprovaFunzione('details');abilitaSistemaAllarmeRabido('details'); costruisci_rel_ateco_attivita('<%= OrgDetails.getAltId() %>', 'id_linea_sottoposta_a_controllo', '<%= TicketDetails.getId_imprese_linee_attivita() %>' );">
<form name="details" id="gestioneanagrafica" action="GestioneAnagraficaVigilanza.do?command=UpdateTicket&auto-populate=true<%= addLinkParams(request, "popup|popupType|actionId") %>" onSubmit="return checkForm(this);" method="post">
...
<td>
  ...
  <a href="GestioneAnagraficaAction.do?command=Search"><dhv:label name="">Risultati Ricerca</dhv:label></a> >
  <a href="GestioneAnagraficaAction.do?command=Details&altId=<%=OrgDetails.getAltId()%>"><dhv:label name="gestioneanagrafica.details">GestioneAnagrafica Details</dhv:label></a> >
  ...
  <a href="GestioneAnagraficaVigilanza.do?command=TicketDetails&id=<%=TicketDetails.getId()%>"><dhv:label name="campioni.dettagli">Scheda Controllo Ufficiale</dhv:label></a> >
  <%}%>
...
<dhv:container name="gestioneanagrafica" selected="vigilanza" object="OrgDetails" param='<%= "altId=" + OrgDetails.getAltId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId") %>'>
  <dhv:container name="GestioneAnagraficaVigilanza" selected="details" object="TicketDetails" param='<%= "id=" + TicketDetails.getId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId") %>'>
    <%--@ include file="abusivismi_ticket_header_include_vigilanza.jsp" --%>
     <dhv:evaluate if="<%= !TicketDetails.isTrashed() %>" >
      <dhv:evaluate if="<%= TicketDetails.getClosed() != null %>" >
          <dhv:permission name="gestioneanagrafica-gestioneanagrafica-vigilanza-edit">
            <input type="submit" value="<dhv:label name="button.reopen">Reopen</dhv:label>" onClick="javascript:this.form.action='GestioneAnagraficaVigilanza.do?command=ReopenTicket&id=<%=TicketDetails.getId()%><%= addLinkParams(request, "popup|popupType|actionId") %>'">
           </dhv:permission>
           <dhv:evaluate if='<%= "list".equals(request.getParameter("return"))%>' >
            <input type="button" value="Annulla" onClick="loadModalWindow(); window.location.href='GestioneAnagraficaAction.do?command=ViewVigilanza&altId=<%=OrgDetails.getAltId()%><%= addLinkParams(request, "popup|popupType|actionId") %>'" />
           </dhv:evaluate>
           <dhv:evaluate if='<%= !"list".equals(request.getParameter("return"))%>' >
            <input type="button" value="Annulla" onClick="loadModalWindow(); window.location.href='GestioneAnagraficaVigilanza.do?command=TicketDetails&id=<%= TicketDetails.getId() %><%= addLinkParams(request, "popup|popupType|actionId") %>'" />
           </dhv:evaluate>
      </dhv:evaluate>
      <dhv:evaluate if="<%= TicketDetails.getClosed() == null %>" >
          <dhv:permission name="gestioneanagrafica-gestioneanagrafica-vigilanza-edit">
	...
	<%@ include file="../controlliufficiali/anagrafica_controlli_ufficiali_modify.jsp" %>
   ...
   <dhv:evaluate if="<%= !TicketDetails.isTrashed() %>" >
    <dhv:evaluate if="<%= TicketDetails.getClosed() != null %>" >
        <dhv:permission name="gestioneanagrafica-gestioneanagrafica-vigilanza-edit">
         ...
         <dhv:evaluate if='<%= "list".equals(request.getParameter("return"))%>' >
          <input type="submit" value="Annulla" onClick="javascript:this.form.action='GestioneAnagraficaAction.do?command=ViewVigilanza&altId=<%=OrgDetails.getAltId()%><%= addLinkParams(request, "popup|popupType|actionId") %>'" />
         </dhv:evaluate>
         <dhv:evaluate if='<%= !"list".equals(request.getParameter("return"))%>' >
          <input type="submit" value="Annulla" onClick="javascript:this.form.action='GestioneAnagraficaVigilanza.do?command=TicketDetails&id=<%= TicketDetails.getId() %><%= addLinkParams(request, "popup|popupType|actionId") %>'" />
         </dhv:evaluate>
    </dhv:evaluate>
    <dhv:evaluate if="<%= TicketDetails.getClosed() == null %>" >
        <dhv:permission name="gestioneanagrafica-gestioneanagrafica-vigilanza-edit">
