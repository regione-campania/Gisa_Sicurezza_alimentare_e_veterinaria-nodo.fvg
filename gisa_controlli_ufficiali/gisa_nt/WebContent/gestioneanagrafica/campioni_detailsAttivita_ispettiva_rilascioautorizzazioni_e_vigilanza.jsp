...
function apriVerbale(idCampione,tipoAnalisi)
{
	...
	document.location.href="GestioneAnagraficaCampioni.do?command=StampaScheda&tipoAnalisi="+tipoAnalisi+"&idCampione="+idCampione+"&tipoAlimenti="+tipoAlimenti;
...
function openPopupModulesPdf(orgId, ticketId){
	var res;
	var result;
		window.open('ManagePdfModules.do?command=PrintSelectedModules&altId='+orgId+'&ticketId='+ticketId,'popupSelect',
...
<form id="details" name="details" action="GestioneAnagraficaCampioni.do?command=ModifyTicket&auto-populate=true" method="post">
...
<%--   <a href="GestioneAnagraficaAction.do?command=Search"><dhv:label name="accounts.SearchResults">Search Results</dhv:label></a> > --%>
  <a href="GestioneAnagraficaAction.do?command=Details&altId=<%=OrgDetails.getAltId()%>">Scheda Gestione Anagrafica</a> >
  ...
  <a href="GestioneAnagraficaVigilanza.do?command=TicketDetails&id=<%= request.getAttribute("idC")%>&altId=<%=OrgDetails.getAltId()%>"><dhv:label name="">Controllo Ufficiale</dhv:label></a> >
   <%--a href="Accounts.do?command=ViewCampioni&altId=<%=TicketDetails.getAltId()%>"><dhv:label name="campioni">Tickets</dhv:label></a> --%>
...
if(TicketDetails.getClosed()!=null && request.getAttribute("Messaggio2")!=null)
{
	%>
	<script>
	...
	function doSubmit(idTicket){
		...
		document.details.action="GestioneAnagraficaCampioni.do?command=Chiudi&id="+idTicket+"&chiudiCu=true'"
