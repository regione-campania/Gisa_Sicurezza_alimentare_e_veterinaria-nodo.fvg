...
%>
					...
					<input type="button" value="Elimina" onclick="if(confirm('Attenzione. Procedere alla CANCELLAZIONE di questo dato aggiuntivo?')) { location.href='GestioneAnagraficaAction.do?command=EliminaMobileDaLinea&indice=<%=indice%>&ldaStabId=<%=request.getParameter("ldaStabId") %>&stabId=<%=request.getParameter("stabId") %>'}"/>
					</dhv:permission>
					</td></tr>
