...
<jsp:useBean id="OrgListOp" class="org.aspcfs.modules.molluschibivalvi.base.OrganizationList" scope="request"/>
...
<form name="details" action="MolluschiBivalviVigilanza.do?command=ModifyTicket&auto-populate=true" method="post">
<table class="trails" cellspacing="0">
<tr>
<td>
  <a href="MolluschiBivalvi.do">Molluschi Bivalvi</a> > 
  <a href="MolluschiBivalvi.do?command=Search"><dhv:label name="">Risultati Ricerca</dhv:label></a> >
  <a href="MolluschiBivalvi.do?command=Details&orgId=<%=TicketDetails.getOrgId()%>">Scheda Molluschi Bivalvi</a> >
  <a href="MolluschiBivalviVigilanza.do?command=ViewVigilanza&orgId=<%=TicketDetails.getOrgId()%>"><dhv:label name="vigilanza">Tickets</dhv:label></a> >
...
<table cellpadding="4" cellspacing="0" width="100%" class="empty">
  ...
  <tr>
   ...
   <%
	if (TicketDetails.getTipoCampione()==3 || TicketDetails.getTipoCampione()==4 || TicketDetails.getTipoCampione()==5)
	{
	%>
   ...
   <tr class="row<%=rowid%>">
	 ...
	 <td>
	   ...
	   <a href="CheckListMolluschiBivalvi.do?command=View&id=<%= thisAudit.getId()%>&aggiorna=<%=thisAudit.isAggiornaCategoria() %>"><%= toHtml(OrgCategoriaRischioList2.getSelectedValue(thisAudit.getTipoChecklist())) %></a>
	</td>
...
<%--} --%>
 ...
 <tr class="containerBody">
      <td  nowrap class="formLabel">
       L'esito condiziona l'inter ospecchio d'acqua 
      </td>
    
			<td>
			SI
			<input type = "radio" value ="SI" name = "esito" <%if (TicketDetails.getEsitoDeclassamento()!=null && TicketDetails.getEsitoDeclassamento().equals("SI")){%>checked="checked"<%} %> disabled="disabled">
			NO
			<input type = "radio" name = "esito" value="NO" <%if (TicketDetails.getEsitoDeclassamento()!=null && TicketDetails.getEsitoDeclassamento().equals("NO")){%>checked="checked"<%} %> disabled="disabled">
			</td>
			<%if (TicketDetails.getEsitoDeclassamento()!=null && TicketDetails.getEsitoDeclassamento().equals("SI")) 
			 { 
			 %>
			<td ><%=Declassamento.getSelectedValue(TicketDetails.getDeclassamento()) %></td>
			<%}else
				{
				%><td>&nbsp;</td><%
				}
