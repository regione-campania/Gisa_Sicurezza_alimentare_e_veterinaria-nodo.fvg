...
<jsp:useBean id="Haccp" class="org.aspcfs.utils.web.LookupList"
	scope="request" />
<jsp:useBean id="Declassamento" class="org.aspcfs.utils.web.LookupList"
	scope="request" />
<jsp:useBean id="MotivoControlloNonEseguito"
	class="org.aspcfs.utils.web.LookupList" scope="request" />
...
<jsp:useBean id="OrgList"
	class="org.aspcfs.modules.molluschibivalvi.base.OrganizationList"
	scope="request" />
...
<body
	onload="provaFunzione('addticket'); onloadAllerta('addticket'); resetElementiNucleoIspettivo('<%=TicketDetails.getNucleoasList().size()%>');
...
<%
	TipoIspezione
			.setJsEvent("onChange=javascript:mostraMenuTipoIspezione('addticket');gestioneVisibilitaCodiceAteco('addticket')");
	TitoloNucleoDue.setJsEvent("onChange=mostraCampo2('addticket')");
	PianoMonitoraggio1
			.setJsEvent("onChange=javascript:piani('addticket')");
	PianoMonitoraggio2
			.setJsEvent("onChange=javascript:piani('addticket')");
	PianoMonitoraggio3
			.setJsEvent("onChange=javascript:piani('addticket')");
	AuditTipo
			.setJsEvent("onChange=javascript:mostraMenu4('addticket')");
	TipoAudit
			.setJsEvent("onChange=javascript:mostraMenu2('addticket')");
	TipoCampione
			.setJsEvent("onChange=javascript:reloadAddCU('MolluschiBivalviVigilanza.do?command=Add&orgId="+OrgDetails.getOrgId()+"&tipoCampione='+this.value)");
%>


<form method="post" name="addticket"
	action="MolluschiBivalviVigilanza.do?command=Insert&auto-populate=true">
<%-- Trails --%>
<table class="trails" cellspacing="0">
	<tr>
		<td><a href="MolluschiBivalvi.do">Molluschi Bivalvi</a> > <a
			href="MolluschiBivalvi.do?command=Search"><dhv:label name="">Risultati Ricerca</dhv:label></a>
		> <a
			href="MolluschiBivalvi.do?command=Details&orgId=<%=OrgDetails.getOrgId()%>">Scheda
		Molluschi Bivalvi</a> > <a
			href="MolluschiBivalviVigilanza.do?command=ViewVigilanza&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label
			name="vigilanza">Tickets</dhv:label></a> > <dhv:label
			name="campioni.aggiungi">Aggiungi Controllo Ufficiale</dhv:label></td>
	</tr>
</table>

<input type="button"
	value="<dhv:label name="button.inserta">Inserisci</dhv:label>"
	name="Save" onClick="return controlloCuSorveglianza()"> <input
	type="submit"
	value="Annulla"
	onClick="javascript:this.form.action='MolluschiBivalviVigilanza.do?command=ViewVigilanza&orgId=<%=OrgDetails.getOrgId()%>'">
<br>

<table cellpadding="4" cellspacing="0" width="100%" class="details">

	<tr>
		<th colspan="2"><strong><dhv:label name="">Aggiungi Controllo Ufficiale</dhv:label></strong>
		</th>
	</tr>
...
<%@ include
	file="../controlliufficiali/controlli_ufficiali_allarmerapido_add.jsp"%>

<br>
<br>

<%@ include
	file="../controlliufficiali/controlli_ufficiali_laboratori_haccp.jsp"%>
...
<%@ include
	file="../controlliufficiali/controlli_ufficiali_laboratori_haccp_non_in_regione.jsp"%>


<input type="hidden" name="close" value=""> <input type="hidden"
	name="refresh" value="-1"> <input type="hidden" name="modified"
	value="<%=TicketDetails.getModified()%>" /> <input type="hidden"
	name="currentDate" value="<%=request.getAttribute("currentDate")%>" />
<input type="hidden" name="statusId"
	value="<%=TicketDetails.getStatusId()%>" /> <input type="hidden"
	name="isAllegato"
	value="<%=TicketDetails.isListaDistribuzioneAllegata()%>"> <input
	type="hidden" name="trashedDate"
	value="<%=TicketDetails.getTrashedDate()%>" /> <%=addHiddenParams(request,
									"popup|popupType|actionId")%>


<br>
<table width="100%" class="details" style="display: none">
	<tr>
		<th colspan="2"><strong><dhv:label name="">Esito del Controllo</dhv:label></strong>
		</th>
	</tr>

	<tr>
		<td valign="top" class="formLabel"><dhv:label
			name="sanzioni.note">L'esi Condiziona L'intero Specchio d'acqua</dhv:label>
		</td>
		<td>
		<table border="0" cellspacing="0" cellpadding="0" class="empty">
			<tr>
				<td>SI<input type="radio" value="SI" name="esito"
					onclick="javascript: document.getElementById('declassamento').style.display=''">NO<input
					type="radio" name="esito" value="NO"
					onclick="javascript: document.getElementById('declassamento').style.display='none'">
				</td>
				<td>&nbsp;</td>
				<td id="declassamento" style="display: none"><%=Declassamento.getHtmlSelect("declassamento", -1)%></td>
			</tr>
		</table>
		</td>
	</tr>
...
<input type="button"
	value="<dhv:label name="button.inserta">Inserisci </dhv:label>"
	name="Save"
	onClick="<%--javascript:this.form.action='MolluschiBivalviVigilanza.do?command=InsertContinua&auto-populate=true';--%>return controlloCuSorveglianza()">
<input type="submit"
	value="Annulla"
	onClick="javascript:this.form.action='MolluschiBivalviVigilanza.do?command=ViewVigilanza&orgId=<%=OrgDetails.getOrgId()%>'">
</form>
