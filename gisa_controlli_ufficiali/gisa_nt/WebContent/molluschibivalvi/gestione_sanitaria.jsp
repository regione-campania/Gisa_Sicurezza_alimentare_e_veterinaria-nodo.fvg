...
function checkForm()
{
	...
	if(document.addAccount.motivi.value =='')
	{
		formtest = false ;
		msg += 'Il campo SOSPENSIONE/REVOCA deve essere popolato\n'
	}
...
// 	}
	...
	if(formtest==true){
		var text = document.getElementById('motivi').options[document.getElementById('motivi').selectedIndex].text;
		document.getElementById('tipoSospensione').value=text;
...
<form id="addAccount" name="addAccount"
	action="MolluschiBivalvi.do?command=UpdateClassificazione&auto-populate=true&updateProvvedimento=1&id=<%=OrgDetails.getId() %>"
	method="post">

<table class="trails" cellspacing="0">
	<tr>
		<td width="100%">Molluschi	Bivalvi > Gestione Sanitaria <%=OrgDetails.getName() %></td>
	</tr>
</table>
...
<table cellpadding="4" cellspacing="0" border="0" width="100%"class="details" id = "zc">
	<tr>
		<th colspan="2"><strong>Gestione sanitaria</strong></th></tr>
	<tr id = "classeId">
		<td nowrap class="formLabel" name="orgname1" id="orgname1"> Sospensione/Revoca</td>
		<td>
  		<%HashMap<Integer,String> motivi = (HashMap<Integer,String>)request.getAttribute("Motivi");
  		HashMap<Integer,String> motiviPresenti =  OrgDetails.getTipoMotiviInZone();
  		
  		%>
  		<select name = "motivi" id = "motivi" multiple="multiple">
  		<%
  		
  		Iterator<Integer> it = motivi.keySet().iterator();
  		while (it.hasNext())
  		{
  			int idMotivo = it.next();
  			String path = motivi.get(idMotivo);
  			%>
  			<option value = "<%=idMotivo %>" <%if(idMotivo >0 && motiviPresenti.containsKey(idMotivo)){%>selected="selected" <%} %>><%=path %></option>
  			<%
  		}
  		%>
