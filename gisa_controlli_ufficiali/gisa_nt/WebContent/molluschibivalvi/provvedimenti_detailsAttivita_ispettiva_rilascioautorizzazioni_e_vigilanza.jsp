...
<form name="details" action="MolluschiBivalviProvvedimenti.do?command=ModifyTicket&auto-populate=true" method="post">
...
<td>
<td>
  <a href="MolluschiBivalvi.do">Operatori</a> > 
  <a href="MolluschiBivalvi.do?command=Search"><dhv:label name="molluschibivalvi.SearchResults">Search Results</dhv:label></a> >
  <a href="MolluschiBivalvi.do?command=Details&orgId=<%=OrgDetails.getOrgId()%>">Scheda Operatore </a> >
  <a href="MolluschiBivalvi.do?command=ViewVigilanza&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label name="">Controlli Ufficiali</dhv:label></a> >
  <a href="MolluschiBivalviVigilanza.do?command=TicketDetails&id=<%= request.getAttribute("idC")%>&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label name="">Controllo Ufficiale</dhv:label></a> >
   <%--a href="MolluschiBivalvi.do?command=ViewCampioni&orgId=<%=TicketDetails.getOrgId()%>"><dhv:label name="campioni">Tickets</dhv:label></a> --%>
<dhv:label name="campione.dettagli">Scheda Prvvedimento</dhv:label>
</td>
</tr>
</table>
<%-- End Trails --%>
...
%>
<dhv:container name="molluschibivalvi" selected="vigilanza" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId() %>' 
	>
	<%
	...
	%>
		...
		<input type="button"
			value="Ripristina"
			onClick="javascript:this.form.action='MolluschiBivalviProvvedimenti.do?command=Restore&id=<%= TicketDetails.getId()%>';submit();">
	...
	<dhv:permission name="reopen-reopen-view">
		<input type="button"
			...
			onClick="javascript:this.form.action='MolluschiBivalviProvvedimenti.do?command=ReopenTicket&id=<%= TicketDetails.getId()%><%= (defectCheck != null && !"".equals(defectCheck.trim())?"&defectCheck="+defectCheck:"") %>';submit();">
	</dhv:permission>
	<%
		} else {
	%>
		...
		<input type="button"
			...
			onClick="javascript:this.form.action='MolluschiBivalviProvvedimenti.do?command=ModifyTicket&auto-populate=true<%= (defectCheck != null && !"".equals(defectCheck.trim())?"&defectCheck="+defectCheck:"") %>';submit();">
		...
		<input type="button"
			...
			onClick="javascript:this.form.action='MolluschiBivalviProvvedimenti.do?command=Chiudi&id=<%= TicketDetails.getId() %>';if( confirm('Sei sicuro di voler chiudere il campione? \n Attenzione! La pratica verrà chiusa e non sarà più possibile fare modifiche \n sulla scheda se non con il permesso del supervisore o dell amministratore') ){submit()};">		
	...
	<table cellpadding="4" cellspacing="0" width="100%" class="details">
		<tr>
		  ...
		  <%@ include file="/controlliufficiali/provvedimenti_view.jsp" %>
...
if(request.getAttribute("Messaggio2")!=null)
{
	...
	function doSubmit(idTicket){
		...
		document.details.action="MolluschiBivalviProvvedimenti.do?command=Chiudi&id="+idTicket+"&chiudiCu=true'"
