...
<%@ page import="java.util.*,com.zeroio.iteam.base.*,org.aspcfs.modules.base.Constants" %>
...
<form method="post" name="inputForm" action="MolluschiBivalviDocuments.do?command=UploadVersion<%= addLinkParams(request, "popup|popupType|actionId|actionplan") %>" enctype="multipart/form-data" onSubmit="return checkFileForm(this);">
...
<tr>
<td>
<a href="AcqueClassificate.do?command=Home"><dhv:label name="accounts.accounts">Molluschi Bivalvi</dhv:label></a> > 
<%=OrgDetails.getTipologia()%> >
<%=OrgDetails.getName()%> >
<a href="MolluschiBivalviDocuments.do?command=View&orgId=<%= OrgDetails.getOrgId() %>"><dhv:label name="accounts.accounts_documents_details.Documents">Documents</dhv:label></a> >
...
<dhv:container name="molluschicoltori" selected="documents" object="OrgDetails" hideContainer='<%= "true".equals(request.getParameter("actionplan")) %>' param='<%= "orgId=" + OrgDetails.getOrgId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId|actionplan") %>'>
<table border="0" cellpadding="4" cellspacing="0" width="100%">
  <tr class="subtab">
    <td>
      <% String documentLink = "MolluschiBivalviDocuments.do?command=View&orgId="+OrgDetails.getOrgId()+ addLinkParams(request, "popup|popupType|actionId|actionplan"); %>
  ...
  <input type="submit" value="Annulla" onClick="javascript:this.form.dosubmit.value='false';this.form.action='MolluschiBivalviDocuments.do?command=View&orgId=<%= OrgDetails.getOrgId() %>&folderId=<%= (String)request.getAttribute("folderId") %><%= addLinkParams(request, "popup|popupType|actionId|actionplan") %>';" />
</dhv:container>
</form>
</body>
