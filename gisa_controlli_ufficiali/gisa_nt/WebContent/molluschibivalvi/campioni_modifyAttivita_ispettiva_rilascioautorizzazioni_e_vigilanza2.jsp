...
<%@page import="org.aspcfs.modules.molluschibivalvi.base.Coordinate"%><jsp:useBean id="TipoCampione_batteri" class="org.aspcfs.utils.web.LookupList" scope="request"/>
...
<body onload="nascondiResponsabilita();mostraDescrizioneNonAccettato('details');mostraUlteririNote('details');abilitaNote(details); abilitaFlag('details'); abilitaLista_tipoAnalisi('details');  abilitaLista_tipoChimico('details');">
...
<form name="details" action="MolluschiBivalviCampioni.do?command=UpdateTicket&auto-populate=true<%= addLinkParams(request, "popup|popupType|actionId") %>"  method="post" >
...
<td>
  ...
  <a href="MolluschiBivalvi.do?command=Details&orgId=<%=OrgDetails.getOrgId()%>">Scheda Molluschi Bivalvi</a> >
<a href="MolluschiBivalviVigilanza.do?command=ViewVigilanza&orgId=<%=TicketDetails.getOrgId()%>"><dhv:label name="">Controlli Ufficiali</dhv:label></a> >
  ...
  <a href="MolluschiBivalviCampioni.do?command=TicketDetails&id=<%=TicketDetails.getId()%>"><dhv:label name="campioni.dettagli">Scheda Campione</dhv:label></a> >
  <%}%>
...
<dhv:container name="molluschibivalvi" selected="vigilanza" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId") %>'>
    <%--@ include file="accounts_ticket_header_include_campioni.jsp" --%>
     <dhv:evaluate if="<%= !TicketDetails.isTrashed() %>" >
      <dhv:evaluate if="<%= TicketDetails.getClosed() != null %>" >
          <dhv:permission name="molluschibivalvi-campioni-edit">
            <input type="submit" value="<dhv:label name="button.reopen">Reopen</dhv:label>" onClick="javascript:this.form.action='MolluschiBivalviCampioni.do?command=ReopenTicket&id=<%=TicketDetails.getId()%><%= addLinkParams(request, "popup|popupType|actionId") %>'">
           </dhv:permission>
           <dhv:evaluate if='<%= "list".equals(request.getParameter("return"))%>' >
            <input type="submit" value="Annulla" onClick="javascript:this.form.action='MolluschiBivalvi.do?command=ViewCampioni&orgId=<%=OrgDetails.getOrgId()%><%= addLinkParams(request, "popup|popupType|actionId") %>'" />
           </dhv:evaluate>
           <dhv:evaluate if='<%= !"list".equals(request.getParameter("return"))%>' >
            <input type="submit" value="Annulla" onClick="javascript:this.form.action='MolluschiBivalviCampioni.do?command=TicketDetails&id=<%= TicketDetails.getId() %><%= addLinkParams(request, "popup|popupType|actionId") %>'" />
           </dhv:evaluate>
      </dhv:evaluate>
      <dhv:evaluate if="<%= TicketDetails.getClosed() == null %>" >
          <dhv:permission name="molluschibivalvi-campioni-edit">
            <input type="submit" value="<dhv:label name="global.button.update" >Update</dhv:label>" onclick="return controllo_data_esito(document.details);return checkEsitoForm('details');"  />
          </dhv:permission>
           <dhv:evaluate if='<%= "list".equals(request.getParameter("return"))%>' >
            <input type="submit" value="Annulla" onClick="javascript:this.form.action='MolluschiBivalvi.do?command=ViewCampioni&orgId=<%=OrgDetails.getOrgId()%><%= addLinkParams(request, "popup|popupType|actionId") %>'" />
           </dhv:evaluate>
    ...
    <td>
    <input type = "button" value ="Aggiungi Coppia Coordinate" onclick="javascript:clonaCoordinate()">
    <input type = "hidden" id = "size_coordinate" name = "size_coordinate" value = "<%=(TicketDetails.getListaCoordinateCampione().size()>0)? TicketDetails.getListaCoordinateCampione().size() :"1" %>">
    <br>
    <br>
    ...
    if(TicketDetails.getListaCoordinateCampione().size()>0)
    {
    for (Coordinate c : TicketDetails.getListaCoordinateCampione())
    	{%>
    <tr id = "riga_coordinate_molluschi<%=indiceCoordinate %>">
    <td>Latitudine  <input type ="text" id = "latitudine_campione<%=indiceCoordinate %>" value = "<%=c.getLatitudine() %>" name = "latitudine_campione<%=indiceCoordinate %>"></td>
    <td>Longitudine  <input type ="text" id = "longitudine_campione<%=indiceCoordinate %>"  value = "<%=c.getLongitudine() %>" name = "longitudine_campione<%=indiceCoordinate %>"></td>
  <td><a href="#" onclick="javascript:removeCoordinate(<%=indiceCoordinate %>);" >Elimina</a></td>
    </tr>
    <%
    indiceCoordinate++ ;
    	}
    ...
    <tr id = "riga_coordinate_molluschi1">
    <td>Latitudine <input type ="text" id = "latitudine_campione1" name = "latitudine_campione1"></td>
    <td>Longitudine <input type ="text" id = "longitudine_campione1" name = "longitudine_campione1"></td>
    <td><a href="#" onclick="javascript:removeCoordinate(1);" >Elimina</a></td>
    </tr>
