...
<%@page import="org.aspcfs.modules.molluschibivalvi.base.OrganizationAddress"%>
...
<form name="addticket" action="MolluschiBivalviCampioni.do?command=Insert&auto-populate=true" method="post">
<% request.setAttribute("isMolluschi Bivalvi", "si"); %>
<%-- Trails --%>
...
<td>
  ...
  <a href="MolluschiBivalviVigilanza.do?command=ViewVigilanza&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label name="">Controlli Ufficiali</dhv:label></a> >
  <a href="MolluschiBivalviVigilanza.do?command=TicketDetails&id=<%= request.getAttribute("idC")%>&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label name="">Controllo Ufficiale</dhv:label></a> >
...
<%-- End Trails --%>
<!--  <input type="submit" id="bottoneInserimentoCampioneSu" value="<dhv:label name="button.insert">Insert</dhv:label>" name="Save" onClick="if( confirm('Attenzione! Controlla bene tutti i dati inseriti perchè non potrai più modificarli. Vuoi concludere l\'inserimento?') ){return checkForm(this.form)}else return false;"> -->
...
<%-- <input type="button" value="Annulla" onClick="window.location.href='MolluschiBivalviVigilanza.do?command=TicketDetails&id=<%= request.getAttribute("idC") %>&orgId=<%=OrgDetails.getOrgId()%>';this.form.dosubmit.value='false';" /> --%>
<br>
<dhv:formMessage />
...
<table cellpadding="4" cellspacing="0" width="100%" class="details">
   ...
   <tr class="containerBody">
    ...
    {
    	int indice = 0;
    	for (CoordinateMolluschiBivalvi cc : oa.getListaCoordinatePuntiDiPrelievo())
    	{
    		if (cc.getLatitude()>0 && cc.getLongitude()>0) {
    		%>
    		
    <tr id = "riga_coordinate_molluschi<%=indice%>">
    <td><input type="checkbox" id="campionePuntoPrelievo_<%=indice %>" name="campionePuntoPrelievo_<%=indice %>" value="<%=cc.getId()%>"/>
   	<td><b><%=cc.getIdentificativo() %></b> </td>
    <td>Lat</td> <td><%=cc.getLatitude()%></td>
    <td>Lon</td> <td><%=cc.getLongitude()%></td>
    </tr>
    		<%
    		indice ++ ;
    	}
   ...
   <input type="hidden" id="size_coordinate" name="size_coordinate" value="<%=oa.getListaCoordinatePuntiDiPrelievo().size()%>"/>
...
<input type="hidden" id="orgIdC" name="orgIdC" value="<%=OrgDetails.getOrgId()%>" />
...
<br>
<!--  <input type="submit" id="bottoneInserimentoCampioneGiu" value="<dhv:label name="button.insert">Insert</dhv:label>" name="Save" onClick="if( confirm('Attenzione! Controlla bene tutti i dati inseriti perchè non potrai più modificarli. Vuoi concludere l\'inserimento?') ){return checkForm(this.form)}else return false;"> -->
<!-- <input type="submit" value="<dhv:label name="button.insert">Insert</dhv:label>" name="Save" class="Save"> -->
