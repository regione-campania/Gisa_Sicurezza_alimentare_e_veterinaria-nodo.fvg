...
<%
%>
<form name="details" action="ColonieSanzioni.do?command=ModifyTicket&auto-populate=true" method="post">
...
<tr>
<td>
<a href="Colonie.do"><dhv:label name="colonie.colonie">Colonie</dhv:label></a> > 
<a href="Colonie.do?command=Search"><dhv:label name="colonie.SearchResults">Search Results</dhv:label></a> >
<a href="Colonie.do?command=Details&orgId=<%=TicketDetails.getOrgId()%>"><dhv:label name="colonie.details"> Dettaglio Colonie</dhv:label></a> >
...
<%--a href="Colonie.do?command=ViewSanzioni&orgId=<%=TicketDetails.getOrgId() %>"><dhv:label name="">Sanzioni Amministrative</dhv:label></a> --%>
<%
	if (defectCheck != null && !"".equals(defectCheck.trim())) {
%>
  <a href="ColonieSanzioni.do?command=TicketDetails&Id=<%=TicketDetails.getId()%>&orgId=<%=OrgDetails.getOrgId() %>"><dhv:label name="tickets.defects.viewDefects">View Defects</dhv:label></a> >
  <a href="ColonieSanzioniDefects.do?command=Details&defectId=<%= defectCheck %>"><dhv:label name="tickets.defects.defectDetails">Defect Details</dhv:label></a> >
<%
  	} else {
  %>
<%
...
%>
  <a href="ColonieSanzioni.do?command=SearchTicketsForm"><dhv:label name="tickets.searchForm">Search Form</dhv:label></a> >
  <a href="AccountSanzioni.do?command=SearchTickets"><dhv:label name="colonie.SearchResults">Search Results</dhv:label></a> >
<%
  	} else {
  %> 
 <%--  <a href="Colonie.do?command=ViewSanzioni&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label name="sanzioni.visualizza">Visualizza Sanzioni Amministrative</dhv:label></a> > --%>
<%
   	}
   %>
<%
