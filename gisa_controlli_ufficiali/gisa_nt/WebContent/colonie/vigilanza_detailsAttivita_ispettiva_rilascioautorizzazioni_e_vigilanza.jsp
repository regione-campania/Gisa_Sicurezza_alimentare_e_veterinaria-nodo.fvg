...
<script language="JavaScript" TYPE="text/javascript" SRC="javascript/controlli_ufficiali_colonie.js"></script>
...
<form name="details" action="ColonieVigilanza.do?command=ModifyTicket&auto-populate=true" method="post">
...
<td>
  ...
  <a href="Colonie.do?command=ViewVigilanza&orgId=<%=TicketDetails.getOrgId()%>"><dhv:label name="vigilanza">Tickets</dhv:label></a> >
...
<table cellpadding="4" cellspacing="0" width="100%" class="empty">
  ...
  <tr>
   ...
   <td>
      ...
      <a href="CheckListColonie.do?command=View&id=<%= thisAudit.getId()%>&aggiorna=<%=thisAudit.isAggiornaCategoria() %>"><%= toHtml(OrgCategoriaRischioList.getSelectedValue(thisAudit.getTipoChecklist())) %></a>
	</td>
	<%} %>
