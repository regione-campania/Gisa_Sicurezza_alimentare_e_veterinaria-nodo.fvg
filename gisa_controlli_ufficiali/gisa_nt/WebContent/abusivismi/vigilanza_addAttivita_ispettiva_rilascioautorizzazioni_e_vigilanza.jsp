...
<form name="addticket" action="AbusivismiVigilanza.do?command=Insert&auto-populate=true" method="post">
<%-- Trails --%>
...
%>
<td>
  <a href="Abusivismi.do"><dhv:label name="">Abusivismi</dhv:label></a> > 
...
<%
	if(!popUp){
%>
<input type="submit" value="Annulla" onClick="javascript:this.form.action='Abusivismi.do?command=ViewVigilanza&orgId=<%=OrgDetails.getOrgId()%>' ">
<% } else { %>
...
<script language="JavaScript" TYPE="text/javascript"
	SRC="javascript/controlli_ufficiali_abusivi.js"></script>
 ...
 //gestioneVisibilitaCodiceAteco('addticket')
    ...
    TipoCampione.setJsEvent("onChange=javascript:reloadAddCU('AbusivismiVigilanza.do?command=Add&orgId="+OrgDetails.getOrgId()+"&tipoCampione='+this.value)"); 
