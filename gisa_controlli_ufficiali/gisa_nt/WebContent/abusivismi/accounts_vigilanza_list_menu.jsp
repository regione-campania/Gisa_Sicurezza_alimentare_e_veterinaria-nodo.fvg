...
<script language="javascript">
  ...
  //Menu link functions
  function details() {
    window.location.href='AbusivismiVigilanza.do?command=TicketDetails&id=' + thisTicId+'<%= addLinkParams(request, "popup|popupType|actionId") %>';
  }

  function modify() {
    window.location.href='AbusivismiVigilanza.do?command=ModifyTicket&id=' + thisTicId + '&return=list<%= addLinkParams(request, "popup|popupType|actionId") %>';
  }

  function deleteTic() {
    popURL('AbusivismiVigilanza.do?command=ConfirmDelete&orgId=' + thisOrgId + '&id=' + thisTicId + '&popup=true<%= isPopup(request) ? "&popupType=inline":"" %>', 'Delete_ticket','320','200','yes','no');
  }
...
<div id="menuTicContainer" class="menu">
  <div id="menuTicContent">
    <table id="menuTicTable" class="pulldown" width="170" cellspacing="0">
      <dhv:permission name="abusivismi-abusivismi-vigilanza-view">
      ...
      <dhv:permission name="abusivismi-abusivismi-vigilanza-delete">
