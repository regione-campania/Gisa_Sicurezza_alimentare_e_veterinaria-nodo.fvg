...
<a href="Abusivismi.do?command=Details&orgId=<%=TicketDetails.getOrgId()%>">Scheda</a> >
...
<%} %>
<%--a href="Abusivismi.do?command=ViewSanzioni&orgId=<%=TicketDetails.getOrgId() %>"><dhv:label name="">Sanzioni Amministrative</dhv:label></a> --%>
<%
...
%>
  ...
  <a href="AccountSanzioni.do?command=SearchTickets">Risultati Ricerca</a> >
<%
  	} else {
  %> 
 <%--  <a href="Abusivismi.do?command=ViewSanzioni&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label name="sanzioni.visualizza">Visualizza Sanzioni Amministrative</dhv:label></a> > --%>
<%
   	}
   %>
<%
