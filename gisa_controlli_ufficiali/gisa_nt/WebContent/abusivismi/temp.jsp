...
<form name="addticket" action="AbusivismiSequestri.do?command=Insert&auto-populate=true" method="post">
<%-- Trails --%>
...
<input type="submit" value="Annulla" onClick="javascript:this.form.action='AbusivismiNonConformita.do?command=TicketDetails&id=<%= request.getAttribute("idC")%>&orgId=<%=OrgDetails.getOrgId()%>'">
<br>
<dhv:formMessage />
...
if(form.tipo_richiesta.value==""){
  ...
  if(document.forms['addticket'].SequestroDi.value=="-1"){
    ...
    if (form.orgId.value == "-1") {
      message += label("check.sequestri.richiedente.selezionato","- Controllare che l'Trasgressore sia stata selezionata\r\n");
      formTest = false;
    }
    </dhv:include>
...
<%--</dhv:evaluate>  --%>
  ...
  <tr>
    <td class="formLabel">
      <dhv:label name="sequestri.richiedente">Trasgressore</dhv:label>
    </td>
