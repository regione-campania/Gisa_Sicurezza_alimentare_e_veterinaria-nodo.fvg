<%-- 
  ...
  - 
  - Version: $Id: troubletickets_documents_file_move.jsp 11310 2005-04-13 20:05:00Z mrajkowski $
  - Description: 
  --%>
...
<table cellpadding="0" cellspacing="0" width="100%" border="1" rules="cols">
  <tr class="section">
    <td valign="top" width="100%">
      ...
      <a href="TroubleTicketsDocumentsAllerte.do?command=SaveMove&tId=<%= TicketDetails.getId() %>&fid=<%= FileItem.getId() %>&popup=true&folderId=0&return=TroubleTicketsDocuments&param=<%= TicketDetails.getId() %>&param2=<%= FileItem.getFolderId() %>"><dhv:label name="accounts.accounts_documents_file_move.Home">Home</dhv:label></a>
...
%>
  <tr class="row<%= rowid %>">
    ...
    <% for(int j=1;j<thisFolder.getLevel();j++){ %>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<%}%>
      ...
      <a href="TroubleTicketsDocumentsAllerte.do?command=SaveMove&tId=<%= TicketDetails.getId() %>&fid=<%= FileItem.getId() %>&popup=true&folderId=<%= thisFolder.getId() %>&return=TroubleTicketsDocuments&param=<%= TicketDetails.getId() %>&param2=<%= FileItem.getFolderId() %>"><%= toHtml(thisFolder.getSubject()) %></a>
