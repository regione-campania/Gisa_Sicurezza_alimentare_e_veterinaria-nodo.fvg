<%-- 
  ...
  - 
  - Version: $Id: mycfs_inbox.jsp 18832 2007-01-26 19:21:02Z matt $
  - Description: 
  --%>
...
<%@ include file="messaggi_inbox_menu.jsp" %>
...
<script>function filtraOggetto(){
	var oggetto =document.getElementById("filtroOggetto").value;
	window.location.href = 'Messaggi.do?command=FiltraInbox&oggetto=' + oggetto;

}
function filtraTesto(){
	var testo =document.getElementById("filtroTesto").value;
	window.location.href = 'Messaggi.do?command=FiltraInbox&testo=' + testo;
...
<%-- End Trails --%>
<a href="javascript:window.location.href='Messaggi.do?command=NewMessage';"><dhv:label name="actionList.nuovoMessaggio">Nuovo Messaggio</dhv:label></a><br />
<br />
<table width="100%" border="0">
  <tr>
    <form name="listView" method="post" action="Messaggi.do?command=Inbox">
    <td align="left">
      <select size="1" name="listView" onChange="javascript:document.listView.submit();">
        <option <%= InboxInfo.getOptionValue("new") %>><dhv:label  name="calendar.messages.inbox.brackets.2">Messaggi (In arrivo)</dhv:label></option>
        <option <%= InboxInfo.getOptionValue("old") %>><dhv:label name="accounts.accounts_list_menu.Archiv">Archivio</dhv:label></option>
...
<table cellpadding="4" cellspacing="0" border="0" width="100%" class="pagedList">
  <tr>
    ...
    <th width="40%" nowrap>
      <strong><a href="Messaggi.do?command=Inbox&column=m.subject"><dhv:label name="accounts.accounts_contacts_calls_details_include.Subject">Subject</dhv:label></a></strong>
      <%String oggetto = (String)request.getAttribute("oggetto");
      String testo = (String)request.getAttribute("testo");
      %>
      <input type="text" id="filtroOggetto" name="filtroOggetto" value="<%=(oggetto!=null) ? oggetto : "" %>" placeholder="Ricerca nell'oggetto"/> <input type="button" value="FILTRA" onClick="filtraOggetto()"/>
      <input type="text" id="filtroTesto" name="filtroTesto" value="<%=(testo!=null) ? testo : "" %>" placeholder="Ricerca nel testo"/> <input type="button" value="FILTRA" onClick="filtraTesto()"/>
 
      <%= InboxInfo.getSortIcon("m.subject") %>
    </th>
  <% if(!InboxInfo.getListView().equalsIgnoreCase("sent")){%>
    <th width="30%" nowrap>
      <strong><a href="Messaggi.do?command=Inbox&column=sent_namelast"><dhv:label name="campaign.from">From</dhv:label></a></strong>
      <%= InboxInfo.getSortIcon("sent_namelast") %>
    </th>
    <th width="30%" nowrap>
      <strong><a href="Messaggi.do?command=Inbox&column=m.entered"><dhv:label name="calendar.received">Received</dhv:label></a></strong>
  ...
  <%} else {%>
    ...
    <th width="30%"  nowrap>
      <strong><a href="Messaggi.do?command=Inbox&column=m.sent"><dhv:label name="calendar.sent">Sent</dhv:label></a></strong>
...
%>      
  ...
  <%}%>
		<td>
      <a href="Messaggi.do?command=CFSNoteDetails&id=<%= thisNote.getId() %>"><%= toHtml(thisNote.getSubject()) %></a>
		</td>
  <% if (!InboxInfo.getListView().equalsIgnoreCase("sent")){ %>
