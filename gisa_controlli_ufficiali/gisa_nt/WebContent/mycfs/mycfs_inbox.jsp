...
<%-- End Trails --%>
<a href="javascript:window.location.href='MyCFSInbox.do?command=NewMessage&checkcontact1=1&hiddencontactid1=1&previousSelection=1&previousSelectionDisplay=amministratore&finalsubmit=true&listType=list';"><dhv:label name="actionList.newMessage">New Message</dhv:label></a><br />
<br />
  ...
  <tr>
    ...
    <td align="left">
      <select size="1" name="listView" onChange="javascript:document.listView.submit();">
        ...
        <option <%= InboxInfo.getOptionValue("sent") %>>Inviate</option>
      </select>
    </td>
    <td>
...
%>      
  ...
  <%}%>
	  <td valign="center" nowrap>
	      <%= TipoMessaggio.getSelectedValue( thisNote.getTipo() ) %>
	    </td>
		<td>
  ...
  <%}else{
      ...
      if (i.hasNext()) {
		...
		<td valign="center" nowrap>
      <zeroio:tz timestamp="<%= thisNote.getEntered() %>" timeZone="<%= User.getTimeZone() %>" showTimeZone="false"/>
    </td>
  </tr>
<%}%>
</table>
<%} else {%>
  <tr class="containerBody"><td colspan="6" valign=center><dhv:label name="accounts.accounts_contacts_messages_view.NoMessagesFound">No messages found.</dhv:label></td></tr>
</table>
<%}%>
<br>
