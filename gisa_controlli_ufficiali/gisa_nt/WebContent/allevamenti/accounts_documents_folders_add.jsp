...
<form method="POST" name="inputForm" action="AccountsDocumentsFoldersAllev.do?command=Save&auto-populate=true<%= addLinkParams(request, "popup|popupType|actionId|actionplan") %>" onSubmit="return checkForm(this);">
...
<a href="AccountsDocumentsAllev.do?command=List&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label name="allevamenti.allevamenti_documents_details.Documents">Documents</dhv:label></a> >
<% if(fileFolder.getId() > -1) {%>
  <dhv:label name="allevamenti.allevamenti_documents_folders_add.ModifyFolder">Modify Folder</dhv:label>
<%} else {%>
...
<br>
  ...
  <input type="submit" value="Annulla" onClick="javascript:this.form.dosubmit.value='false';this.form.action='AccountsDocumentsAllev.do?command=View&orgId=<%= OrgDetails.getOrgId() %><%= addLinkParams(request, "popup|popupType|actionId|actionplan") %>';" /><br />
  <dhv:formMessage />
  <br />
  <table cellpadding="4" cellspacing="0" width="100%" class="pagedList">
    <tr>
      <th colspan="2">
        <strong>
          <% if(fileFolder.getId() > -1) {%>
            <dhv:label name="allevamenti.allevamenti_documents_list_menu.RenameFolder">Rinomina Cartella</dhv:label>
          <%} else {%>
            <dhv:label name="documents.documents.newFolder">New Folder</dhv:label>
          <%}%>
        </strong>
