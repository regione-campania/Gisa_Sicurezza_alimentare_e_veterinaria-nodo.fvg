...
%>
	...
	%>
		...
		<% Timestamp dataLimiteChecklist = Timestamp.valueOf("2021-03-05 00:00:00");  
		if (TicketDetails.getAssignedDate().before(dataLimiteChecklist)){ %>
			<%@ include file="benessere/old/gestione_benessere.jsp"%>
		<% } else { %>
			<%@ include file="benessere/gestione_benessere.jsp"%>
		<% } %>
		
		<dhv:permission name="chkbio-chkbio-view">
		<%@ include file="../controlliufficiali/controlli_ufficiali_stampa_biosicurezza.jsp"%>
		</dhv:permission>
		
		<dhv:permission name="chkfarmacosorveglianza-chkfarmacosorveglianza-view">
		<%@ include file="../controlliufficiali/controlli_ufficiali_stampa_farmacosorveglianza.jsp"%>
		</dhv:permission>
    ...
    %>
			<tr class="row<%=rowid%>">
				...
				<td>
					...
					href="javascript:eliminaCheckList(<%=thisAudit.getId() %>,<%=TicketDetails.getId() %>,<%=OrgDetails.getOrgId() %>)">Elimina</a>
					<%}else	{%> &nbsp; <%	}%>
				</td>
			</tr>
			<%}%>
...
if(flag.equals("1")){ %>
<script>
	alert("Questo Controllo Ufficiale non puo essere chiuso .Ci sono Attività collegate/liste di riscontro sul benessere/checklist che non sono state ancora chiuse.");
</script>
...
<script>	alert("Controllo Ufficiale chiuso in attesa di esito (sottosezione Tamponi o Campioni)."); </script>
<%	}}  %>
