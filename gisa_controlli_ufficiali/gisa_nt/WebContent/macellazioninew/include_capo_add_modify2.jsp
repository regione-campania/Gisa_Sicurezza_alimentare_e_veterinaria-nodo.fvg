...
function controllaForm( toLiberoConsumo )
{
	...
	try{
		...
		*/
		...
		if(calcolaNumCapiCapriniDestCorrente()+num_capi_caprini_esito_2_val!=vam_num_capi_caprini && document.getElementById('cd_specie').options[2].selected )
		{
...
// 		}
		...
		//Test BSE obbligatorio se l'animale muore prima della macellazione
		if(document.getElementById('cd_bse').checked==false && document.getElementById('mavam_luogo')!=null && document.getElementById('mavam_luogo').value != '-1' && ( (id_specie_selezionata == BOVINI || id_specie_selezionata == BUFALINI)  ) ){
	...
	<tr>
		<td>
			...
			<%
					...
					%>
			<a href="MacellazioniNew.do?command=List&orgId=<%=OrgDetails.getOrgId()%>">Macellazioni</a> > <%= update ? "Modifica Partita" : "Aggiungi Partita" %>
		</td>
	</tr>
</table>
<%@page import="java.text.SimpleDateFormat"%>
...
<jsp:useBean id="PartitaLog"			class="org.aspcfs.modules.macellazioninew.base.PartitaLog"		scope="request" />
            ...
            <tr class="containerBody">
            	<td class="formLabel">Codice Azienda di provenienza</td>
            	 <td>
					...
					<input type="text" onchange="searchBdn(this.value,2)" name="cd_codice_azienda_provenienza" value="<%=toHtmlValue( Partita.getCd_codice_azienda_provenienza() ) %>" /><font color="red" >*</font>
<br>			
			<textarea rows="5" cols="33"  name = "cd_info_azienda_provenienza" id = "infoAzienda" ><%=toHtml2( Partita.getCd_info_azienda_provenienza())%></textarea>
...
<%--              		<zeroio:dateSelect field="cd_data_mod4" form="main" showTimeZone="false" timestamp="<%=Partita.getCd_data_mod4() %>" /> --%>
              		<input readonly type="text" name="cd_data_mod4" size="10" value="<%=DateUtils.timestamp2string(Partita.getCd_data_mod4())%>" />&nbsp;  
...
<%--              	<zeroio:dateSelect field="cd_data_arrivo_macello" form="main" showTimeZone="false" timestamp="<%=Partita.getCd_data_arrivo_macello() %>" /> --%>
					<input onfocus="gestisciDataCorrente();" onchange="alert('wewe')" readonly type="text" id="cd_data_arrivo_macello" name="cd_data_arrivo_macello" size="10" value="<%=DateUtils.timestamp2string(Partita.getCd_data_arrivo_macello())%>" />  
			        <font color="red" >*</font>
...
<%--						<zeroio:dateSelect 
 ...
 --%>							
						<input readonly type="text" name="bse_data_prelievo" size="10" value="<%=DateUtils.timestamp2string(Partita.getBse_data_prelievo())%>" />&nbsp;  
...
<%--						<zeroio:dateSelect 
 ...
 --%>
						<input readonly type="text" name="bse_data_ricezione_esito" size="10" value="<%=DateUtils.timestamp2string(Partita.getBse_data_ricezione_esito())%>" />&nbsp;  
...
%>
	<input id="liberoConsumoButton" type="button" value="Libero Consumo >>" onclick="if(controllaForm( true )){document.main.action='MacellazioniNew.do?command=ToLiberoConsumo&fromModifica=si';settaSoloCD('false');document.main.submit();};" />
...
<%--               	<zeroio:dateSelect field="vam_data" form="main" showTimeZone="false" timestamp="<%=Partita.getVam_data() %>" /> --%>
					<input readonly type="text" id="vam_data" name="vam_data" onfocus="riportaDataArrivoMacello(this);gestisciObbligatorietaVisitaAnteMortem();" size="10" value="<%=DateUtils.timestamp2string(Partita.getVam_data())%>" />&nbsp;  
...
<%--              	<zeroio:dateSelect field="vpm_data" form="main" showTimeZone="false" timestamp="<%=Partita.getVpm_data() %>" />  --%>
					<input readonly type="text" id="vpm_data" name="vpm_data"  onfocus="riportaDataArrivoMacello(this);gestisciObbligatorietaVisitaPostMortem();" size="10" value="<%=DateUtils.timestamp2string(Partita.getVpm_data())%>" />&nbsp;  
						    ...
						    <td>
						    	<div style="" id="imprese_2">
						        	<a href = "javascript:popLookupSelectorDestinazioneCarni( 'si', 2, 'impresa' );" onclick="selectDestinazione(2);gestioneObbligNumCapiDestCarni(2,1);" >[Seleziona Impresa]</a><br/>
						      		...
						      		<a href="javascript:mostraTextareaEsercente('esercenteNoGisa2');" onclick="selectDestinazioneFromLinkTextarea(2);gestioneObbligNumCapiDestCarni(2,4);" >[Inserisci Esercente non in G.I.S.A.]</a><br/>
...
<%--              	<zeroio:dateSelect field="mavam_data" showTimeZone="false" form="main" timestamp="<%=Partita.getMavam_data() %>" />   --%>
              		<input readonly type="text" id="mavam_data" name="mavam_data" onfocus="riportaDataArrivoMacello(this);gestisciObbligatorietaMorteAnteMacellazione();" size="10" value="<%=DateUtils.timestamp2string(Partita.getMavam_data())%>" />&nbsp;  
...
<%--            	<zeroio:dateSelect field="casl_data" form="main" showTimeZone="false" timestamp="<%=Partita.getCasl_data() %>" />  --%>
              		<input readonly type="text" id="casl_data" name="casl_data" onfocus="riportaDataArrivoMacello(this);gestisciObbligatorietaComunicazioniEsterne();" size="10" value="<%=DateUtils.timestamp2string(Partita.getCasl_data())%>" />&nbsp;
...
<%--                <zeroio:dateSelect field="rca_data" form="main" showTimeZone="false" timestamp="<%=Partita.getRca_data() %>" /> --%>
                	<input readonly type="text" id="rca_data" name="rca_data" onfocus="riportaDataArrivoMacello(this);" size="10" value="<%=DateUtils.timestamp2string(Partita.getRca_data())%>" />&nbsp;  
      ...
      <tr>
	   ...
	   <%-- 	 		<zeroio:dateSelect field="seqa_data" form="main" showTimeZone="false" timestamp="<%=Partita.getSeqa_data() %>"/>	--%>          
	              		<input readonly type="text" name="seqa_data" size="10" value="<%=DateUtils.timestamp2string(Partita.getSeqa_data())%>" />&nbsp;
...
<%-- 	              		<zeroio:dateSelect field="seqa_data_sblocco" form="main" showTimeZone="false" timestamp="<%=Partita.getSeqa_data_sblocco() %>"/> 	--%>
		              	<input readonly type="text" name="seqa_data_sblocco" size="10" value="<%=DateUtils.timestamp2string(Partita.getSeqa_data_sblocco())%>" />&nbsp;
...
<dhv:evaluate if="<%= update==true%>">
<input type="button" value="Annulla" onclick="location.href='MacellazioniNew.do?command=Details&id=<%=Partita.getId()%>&orgId=<%=OrgDetails.getOrgId()%>';" >
</dhv:evaluate>
