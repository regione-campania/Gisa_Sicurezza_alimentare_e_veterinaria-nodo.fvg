...
<body onload="verificaChiusuraNc(<%=request.getAttribute("Chiudi")%>,<%=request.getAttribute("numSottoAttivita")%>,'<%=request.getAttribute("Messaggio2")%>',<%=TicketDetails.getIdControlloUfficiale() %>,'CaniPadronaliVigilanza')">

<form name="details" action="CaniPadronaliNonConformita.do?command=ModifyTicket&auto-populate=true" method="post">
<%-- Trails --%>
...
<td>
  ...
  <a href="CaniPadronaliVigilanza.do?command=TicketDetails&id=<%=TicketDetails.getIdControlloUfficiale()%>&orgId=<%=OrgDetails.getOrgId() %>">Scheda Controllo Ufficiale</a> >
	Scheda non Conformita
</td>
</tr>
</table>
<%-- End Trails --%>
