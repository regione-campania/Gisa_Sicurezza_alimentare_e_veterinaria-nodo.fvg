<%-- 
  ...
  -
  - Version: $Id: canipadronali_tickets_modify.jsp 18488 2007-01-15 20:12:32Z matt $
  - Description:
  --%>
...
<td>
  ...
  <a href="CaniPadronali.do?command=Details&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label name="canipadronali.details"> Dettaglio CaniPadronali</dhv:label></a> > 
...
<%} %>
  ...
  <a href="CaniPadronaliSanzioni.do?command=TicketDetails&id=<%=TicketDetails.getId()%>"><dhv:label name="sanzioni.dettagli">Scheda Sanzione</dhv:label></a> >
  <%}%>
...
<dhv:container name="canipadronali" selected="vigilanza" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId") %>'>
  <dhv:container name="canipadronalisanzioni" selected="details" object="TicketDetails" param='<%= "id=" + TicketDetails.getId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId") %>'>
    <%--@ include file="canipadronali_ticket_header_include.jsp" --%>
