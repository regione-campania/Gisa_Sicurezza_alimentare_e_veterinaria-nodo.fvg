...
<form name="details" action="CaniPadronaliNonConformita.do?command=UpdateTicket&auto-populate=true<%= addLinkParams(request, "popup|popupType|actionId") %>" onSubmit="return checkForm(this);" method="post">
...
<td>
   ...
   <a href="CaniPadronaliVigilanza.do?command=TicketDetails&id=<%=TicketDetails.getIdControlloUfficiale()%>&orgId=<%=OrgDetails.getOrgId() %>">Scheda Anagrafica Cani di proprieta</a> >

  <%--<a href="CaniPadronaliVigilanza.do?command=TicketDetails&id=<%=TicketDetails.getIdControlloUfficiale()%>&orgId=<%=OrgDetails.getOrgId() %>">Scheda Cane Padronale</a> >--%>

  <a href="CaniPadronaliNonConformita.do?command=TicketDetails&id=<%=TicketDetails.getId()%>"><dhv:label name="nonconformita.dettagli">Scheda non Conformità Rilevata</dhv:label></a> >
	Modifica
</td>
</tr>
</table>
...
<dhv:container name="canipadronali" selected="vigilanza" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId()%>' >
    <%--@ include file="accounts_ticket_header_include.jsp" --%>
     <dhv:evaluate if="<%= !TicketDetails.isTrashed() %>" >
      <dhv:evaluate if="<%= TicketDetails.getClosed() != null %>" >
          <dhv:permission name="canipadronali-nonconformita-edit">
            <input type="submit" value="<dhv:label name="button.reopen">Reopen</dhv:label>" onClick="javascript:this.form.action='CaniPadronaliNonConformita.do?command=ReopenTicket&id=<%=TicketDetails.getId()%><%= addLinkParams(request, "popup|popupType|actionId") %>'">
           </dhv:permission>
           ...
           <dhv:evaluate if='<%= !"list".equals(request.getParameter("return"))%>' >
            <input type="submit" value="Annulla" onClick="javascript:this.form.action='CaniPadronaliNonConformita.do?command=TicketDetails&id=<%= TicketDetails.getId() %><%= addLinkParams(request, "popup|popupType|actionId") %>'" />
           </dhv:evaluate>
      </dhv:evaluate>
      <dhv:evaluate if="<%= TicketDetails.getClosed() == null %>" >
          <dhv:permission name="canipadronali-nonconformita-edit">
  ...
  <tr class="containerBody">
    ...
    <td>
      ...
      <input type="text" name="siteId" value="<%=TicketDetails.getSiteId()%>" >
    </td>
  </tr>
  </dhv:evaluate> 
  <dhv:evaluate if="<%= SiteIdList.size() <= 1 %>">
