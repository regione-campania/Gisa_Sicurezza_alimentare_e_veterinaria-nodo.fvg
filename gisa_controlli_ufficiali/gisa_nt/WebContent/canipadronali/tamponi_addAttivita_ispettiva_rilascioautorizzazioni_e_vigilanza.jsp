...
<body >
<form name="addticket" action="CaniPadronaliTamponi.do?command=Insert&auto-populate=true" method="post">
...
<td>
  ...
  <a href="CaniPadronaliVigilanza.do?command=TicketDetails&id=<%= request.getAttribute("idC")%>&orgId=<%=OrgDetails.getOrgId() %>&assetId=<%=OrgDetails.getId() %>">Scheda Controllo Ufficiale</a> >
   ...
   <dhv:label name="campioni.aggiungi">Aggiungi Tampone</dhv:label>
</td>
</tr>
</table>
<%-- End Trails --%>
<input type="submit" value="<dhv:label name="button.insert">Insert</dhv:label>" name="Save" onClick="return checkForm(this.form)">
<input type="button" value="Annulla" onClick="window.location.href='CaniPadronaliVigilanza.do?command=TicketDetails&assetId=<%=OrgDetails.getId() %>&id=<%= request.getAttribute("idC") %>&orgId=<%=OrgDetails.getOrgId()%>';this.form.dosubmit.value='false';" />
<br>
<dhv:formMessage />
