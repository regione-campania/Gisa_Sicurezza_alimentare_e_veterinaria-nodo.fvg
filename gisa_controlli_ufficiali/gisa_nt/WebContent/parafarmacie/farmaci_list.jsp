...
<jsp:useBean id="OrgList" class="org.aspcfs.modules.parafarmacie.base.FarmaciList" scope="request"/>
...
<dhv:permission name="parafarmacie-parafarmacie-add"><a href="ParafarmacieFar.do?command=AddFar"><dhv:label name="">Aggiungi Farmaco</dhv:label></a></dhv:permission>
...
<table cellpadding="8" cellspacing="0" border="0" width="100%" class="pagedList">
  <tr>
    ...
    <th nowrap <% ++columnCount; %>>
      <%--strong><a href="ParafarmacieFar.do?command=SearchFar&column=o.ragione_sociale"--%><dhv:label name="">Nome Commerciale</dhv:label><%--/a></strong>
...
%>
  <tr class="row<%= rowid %>">
	...
	<td>
      <a href="ParafarmacieFar.do?command=DetailsFar&idFarmaco=<%=thisOrg.getIdFarmaco()%>"><%= toHtml(thisOrg.getNomeCommerciale()) %></a>
	</td>
...
<%-- if(thisOrg.getAccountSize() > 0) {%>
    <td valign="top" nowrap>
      <%= OrgCategoriaRischioList.getSelectedValue(thisOrg.getAccountSize()) %>
       &nbsp;
       [<a href="ParafarmacieFar.do?command=ModificaCatRischio&orgId=<%=thisOrg.getOrgId()%>"><dhv:label name="">Modifica</dhv:label></a>]
    </td>
<%}else { %>
	<td valign="top" nowrap>
      Nessuna
       &nbsp;
       [<a href="ParafarmacieFar.do?command=ModificaCatRischio&orgId=<%=thisOrg.getOrgId()%>"><dhv:label name="accountsa.accounts_add.select">Aggiungi</dhv:label></a>]
    </td>
    <%} --%>
...
<%} else {%>
  <tr class="containerBody">
    <td colspan="<%= SearchOrgListInfoFcie.getSearchOptionValue("searchcodeOrgSiteId").equals(String.valueOf(Constants.INVALID_SITE))?columnCount+1:columnCount %>">
      ...
      <a href="ParafarmacieFar.do?command=SearchFormFar"><dhv:label name="">Modifica Ricerca</dhv:label></a>.
    </td>
  </tr>
<%}%>
</table>
