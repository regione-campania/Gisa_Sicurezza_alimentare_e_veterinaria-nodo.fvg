...
<form name="details" action="ParafarmacieSequestri.do?command=UpdateTicket&auto-populate=true<%= addLinkParams(request, "popup|popupType|actionId") %>" onSubmit="return checkForm(this); " method="post">
...
<td>
  ...
  <a href="Parafarmacie.do?command=Details&orgId=<%=OrgDetails.getOrgId()%>">Scheda Parafarmacie</a> > 
...
<%} %>  <% if (request.getParameter("return") == null) {%>
  <a href="ParafarmacieSequestri.do?command=TicketDetails&id=<%=TicketDetails.getId()%>"><dhv:label name="sequestri.dettagli">Scheda Sequestro/Blocco</dhv:label></a> >
  <%}%>
...
<dhv:container name="parafarmacie" selected="sequestri" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId") %>'>
