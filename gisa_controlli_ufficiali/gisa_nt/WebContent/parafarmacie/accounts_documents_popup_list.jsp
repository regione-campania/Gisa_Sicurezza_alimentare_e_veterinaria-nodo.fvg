...
<dhv:container name="parafarmacie" selected="documents" object="OrgDetails" hideContainer='<%= "true".equals(request.getParameter("actionplan")) %>' param='<%= "orgId=" + OrgDetails.getOrgId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId|actionplan") %>'>
  <%
    String permission_doc_folders_add ="parafarmacie-documents-add";
    String permission_doc_files_upload = "parafarmacie-documents-add";
    String permission_doc_folders_edit = "parafarmacie-documents-edit";
    String documentFolderAdd ="ParafarmacieDocumentsFolders.do?command=Add&orgId="+OrgDetails.getOrgId();
    String documentFileAdd = "ParafarmacieDocuments.do?command=Add&orgId="+ OrgDetails.getOrgId();
    String documentFolderModify = "ParafarmacieDocumentsFolders.do?command=Modify&orgId="+ OrgDetails.getOrgId();
    String documentFolderList = "ParafarmacieDocuments.do?command=View&orgId="+OrgDetails.getOrgId()+"&popup=true";
    String documentFileDetails = "ParafarmacieDocuments.do?command=Details&orgId="+ OrgDetails.getOrgId();
...
%>    
  <tr>
    <td class="row<%= rowid %>" width="100%">
      ...
      <a href="ParafarmacieDocuments.do?command=Download&orgId=<%= OrgDetails.getOrgId() %>&folderId=<%= thisFile.getFolderId() %>&fid=<%= thisFile.getId() %>"><%= toHtml(thisFile.getSubject()) %></a>
    </td>
...
&nbsp;<br/>
<form method="post" name="inputForm" action="ParafarmacieDocuments.do?command=Upload&popup=true" onSubmit="return checkFileForm(this);" enctype="multipart/form-data">
