...
<form name="details" action="ParafarmacieSequestri.do?command=ModifyTicket&auto-populate=true" method="post">
...
<a href="Parafarmacie.do?command=SearchFcie">Scheda </a> >
...
<%--a href="Parafarmacie.do?command=ViewSequestri&orgId=<%=TicketDetails.getOrgId() %>"><dhv:label name="sequestri">Sequestri</dhv:label></a> --%>
<%
	if (defectCheck != null && !"".equals(defectCheck.trim())) {
%>
  <a href="ParafarmacieSequestri.do?command=TicketDetails&Id=<%=TicketDetails.getId()%>&orgId=<%=OrgDetails.getOrgId() %>"><dhv:label name="tickets.defects.viewDefects">View Defects</dhv:label></a> >
  <a href="ParafarmacieSequestriDefects.do?command=Details&defectId=<%= defectCheck %>"><dhv:label name="tickets.defects.defectDetails">Defect Details</dhv:label></a> >
<%
  	} else {
  %>
<%
...
%>
  <a href="ParafarmacieSequestri.do?command=SearchTicketsForm"><dhv:label name="tickets.searchForm">Search Form</dhv:label></a> >
  <a href="ParafarmacieSequestri.do?command=SearchTickets"><dhv:label name="accounts.SearchResults">Search Results</dhv:label></a> >
<%
  	} else {
  %> 
 <%--  <a href="Parafarmacie.do?command=ViewSequestri&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label name="sequestri.visualizza">Visualizza Sequestri Amministrative</dhv:label></a> > --%>
<%
   	}
   %>
<%
