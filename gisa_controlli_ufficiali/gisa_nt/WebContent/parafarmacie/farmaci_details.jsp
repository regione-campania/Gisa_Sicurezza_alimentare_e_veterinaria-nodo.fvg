...
<a href="ParafarmacieFar.do?command=SearchFar"><dhv:label name="">Risultati Ricerca</dhv:label></a> >
<%} else if (request.getParameter("return").equals("dashboard")) {%>
<a href="ParafarmacieFar.do?command=DashboardFar">Cruscotto</a> >
<%}--%>
  ...
  <table width="100%" border="0">
    <tr>
      <td nowrap align="right">
        ...
        <a href="ParafarmacieFar.do?command=PrintReport&id=<%= OrgDetails.getId() %>"><dhv:label name="accounts.osa.print">Stampa Scheda O.S.A.</dhv:label></a>
       --%>
      </td>
    </tr>
  </table>
...
<dhv:evaluate if="<%=OrgDetails.isTrashed()%>">
    <dhv:permission name="parafarmacie-parafarmacie-edit">
      <input type="button" value="Ripristina"	onClick="javascript:window.location.href='ParafarmacieFar.do?command=Restore&idFarmaco=<%= OrgDetails.getIdFarmaco() %>';">
    </dhv:permission>
</dhv:evaluate>
<dhv:evaluate if="<%=!OrgDetails.isTrashed()%>">
 
    <dhv:permission name="parafarmacie-parafarmacie-edit"><input type="button"  value="Modifica"	onClick="javascript:window.location.href='ParafarmacieFar.do?command=ModifyFar&idFarmaco=<%= OrgDetails.getIdFarmaco() %><%= addLinkParams(request, "popup|actionplan") %>';"></dhv:permission>
  
  
    <%--dhv:permission name="parafarmacie-parafarmacie-edit">
      <input type="button" value="<dhv:label name="global.button.Enable">Enable</dhv:label>" 	onClick="javascript:window.location.href='ParafarmacieFar.do?command=Enable&idFarmaco=<%= OrgDetails.getIdFarmaco() %>';">
    </dhv:permission--%>
 
  <dhv:evaluate if='<%= (request.getParameter("actionplan") == null) %>'>
    <dhv:permission name="parafarmacie-parafarmacie-delete"><input type="button" value="<dhv:label name="accounts.accounts_details.DeleteAccount">Delete Account</dhv:label>" onClick="javascript:popURLReturn('ParafarmacieFar.do?command=ConfirmDeleteFar&id=<%=OrgDetails.getIdFarmaco()%>&popup=true','ParafarmacieFar.do?command=SearchFar', 'Delete_account','320','200','yes','no');"></dhv:permission>
  </dhv:evaluate>
</dhv:evaluate>
...
<dhv:evaluate if="<%=OrgDetails.isTrashed()%>">
    <dhv:permission name="parafarmacie-parafarmacie-edit">
      <input type="button" value="Ripristina"	onClick="javascript:window.location.href='ParafarmacieFar.do?command=RestoreFar&idFarmaco=<%= OrgDetails.getIdFarmaco() %>';">
    </dhv:permission>
</dhv:evaluate>
<dhv:evaluate if="<%=!OrgDetails.isTrashed()%>">
    ...
    <%--dhv:permission name="parafarmacie-parafarmacie-edit">
      <input type="button" value="<dhv:label name="global.button.Enable">Enable</dhv:label>" 	onClick="javascript:window.location.href='ParafarmacieFar.do?command=EnableFar&idFarmaco=<%= OrgDetails.getIdFarmaco() %>';">
    </dhv:permission--%>
