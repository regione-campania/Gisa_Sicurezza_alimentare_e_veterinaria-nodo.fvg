...
<form name="viewAccountAudit" action="CheckListParafarmacie.do?command=Modify&auto-populate=true&id=<%=Audit.getId()%>&return=details<%= addLinkParams(request, "popup|popupType|actionId") %>" method="post">
  ...
  <table width="100%" border="0">
    <tr>
       <td nowrap align="right">
        ...
        <%--<a href="Parafarmacie.do?command=PrintReport&id=<%= OrgDetails.getId() %>"><dhv:label name="accounts.osa.print">Stampa Scheda Audit</dhv:label></a>--%>
          
          <%--if(OrgDetails.getCategoriaRischio() == -1){ %>
          
         <input type="button" value="Stampa Scheda Check List Provvisorio"	onClick='javascript:location.href="CheckListParafarmacie.do?command=Stampa&id=<%= Audit.getId() %>";'>
      <%
    }
    else {
    %>
        <input type="button" value="Stampa Scheda Check List"	onClick='javascript:location.href= "CheckListParafarmacie.do?command=Stampa&id=<%= Audit.getId() %>";'>
    <%	
    }
    --%>
