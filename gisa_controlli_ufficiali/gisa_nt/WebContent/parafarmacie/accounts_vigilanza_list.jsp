...
<%@ page import="java.util.*,java.text.DateFormat,org.aspcfs.modules.parafarmacie.base.*,org.aspcfs.modules.vigilanza.base.Ticket,com.zeroio.iteam.base.*" %>
...
<a href="Parafarmacie.do?command=DetailsFcie&idFarmacia=<%=OrgDetails.getOrgId()%>"><dhv:label name="">Dettaglio Farmacie / Grossisti / Parafarmacie</dhv:label></a> >
...
<dhv:container name="parafarmacie" selected="vigilanza" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId") %>'>
  <dhv:evaluate if="<%= !OrgDetails.isTrashed() %>">
    ...
    <dhv:permission name="parafarmacie-parafarmacie-vigilanza-add"><a href="ParafarmacieVigilanza.do?command=Add&orgId=<%= OrgDetails.getOrgId() %><%= addLinkParams(request, "popup|popupType|actionId") %>"><dhv:label name="accounts.richiesta.add">Aggiungi Nuovo Controllo Ufficiale</dhv:label></a></dhv:permission>
  </dhv:evaluate>
