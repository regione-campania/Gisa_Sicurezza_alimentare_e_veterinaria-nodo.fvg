...
<%@ page import="java.util.*,java.text.DateFormat,org.aspcfs.modules.parafarmacie.base.*,org.aspcfs.controller.SystemStatus"%>
...
<jsp:useBean id="FarmaciList" class="org.aspcfs.modules.parafarmacie.base.FarmaciList" scope="request"/>
<jsp:useBean id="ParafarmacieList" class="org.aspcfs.modules.parafarmacie.base.ParafarmacieList" scope="request"/>
<jsp:useBean id="VeterinariList" class="org.aspcfs.modules.parafarmacie.base.VeterinariList" scope="request"/>
...
<body >
<form name="addAccount" action="ParafarmaciePre.do?command=UpdatePre&idPrescrizione=<%= OrgDetails.getIdPrescrizione() %>&auto-populate=true<%= (request.getParameter("popup") != null?"&popup=true":"") %>" onSubmit="return doCheck(this);" method="post">
<%
  boolean popUp = false;
...
<%-- if (request.getParameter("return") != null) {%>
	...
	<a href="ParafarmaciePre.do?command=SearchFar"><dhv:label name="accounts.SearchResults">Search Results</dhv:label></a> >
	<%} else if (request.getParameter("return").equals("dashboard")) {%>
	<a href="ParafarmaciePre.do?command=DashboardFar">Cruscotto</a> >
	<%}%>
<%} else {%>
<a href="ParafarmaciePre.do?command=SearchFar"><dhv:label name="accounts.SearchResults">Search Results</dhv:label></a> >--%>
<a href="ParafarmaciePre.do?command=DetailsPre&idPrescrizione=<%=OrgDetails.getIdFarmacia()%>"><dhv:label name="">Scheda Prescrizione</dhv:label></a> >
<%--}--%>
...
<dhv:container name="parafarmacie" selected="details" object="OrgDetails" param='<%= "idPrescrizione=" + OrgDetails.getIdPrescrizione() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId") %>'>
...
<% if (request.getParameter("return") != null && "list".equals(request.getParameter("return"))) {%>
  <input type="submit" value="Annulla" onClick="javascript:this.form.action='ParafarmaciePre.do?command=DetailsPre&idPrescrizione=<%= OrgDetails.getIdPrescrizione() %>';this.form.dosubmit.value='false';" />
...
<table cellpadding="4" cellspacing="0" border="0" width="100%" class="details">
  ...
  <td class="formLabel">
    ...
    <%    
	Iterator k = ParafarmacieList.iterator();
	if ( k.hasNext() ) {
    int rowid = 0;
    int l = 0;
    while (k.hasNext()) {
    l++;
    rowid = (rowid != 1 ? 1 : 2);
    Parafarmacie thisOrg2 = (Parafarmacie)k.next();
%>
...
<table cellpadding="4" cellspacing="0" border="0" width="100%" class="details">
  ...
  <input type="submit" value="<dhv:label name="global.button.update">Update</dhv:label>" name="Save" onClick="this.form.dosubmit.value='true';" />
<% if (request.getParameter("return") != null && "list".equals(request.getParameter("return"))) {%>
  <input type="submit" value="Annulla" onClick="javascript:this.form.action='ParafarmaciePre.do?command=DetailsPre&idPrescrizione=<%= OrgDetails.getIdFarmacia() %>';this.form.dosubmit.value='false';" />
<% } else if (isPopup(request)) { %>
  <input type="button" value="Annulla" onclick="javascript:window.close();" />
<% } else { %>
  <input type="submit" value="Annulla" onClick="javascript:this.form.action='ParafarmaciePre.do?command=DetailsPre&idPrescrizione=<%= OrgDetails.getIdFarmacia() %>';this.form.dosubmit.value='false';" />
