...
<%@ page import="java.util.*,java.text.DateFormat,org.aspcfs.modules.parafarmacie.base.*,org.aspcfs.modules.contacts.base.*, org.aspcfs.modules.base.Constants" %>
...
<jsp:useBean id="OrgDetails" class="org.aspcfs.modules.parafarmacie.base.Prescrizioni" scope="request"/>
...
<a href="ParafarmaciePre.do?command=SearchFar"><dhv:label name="">Risultati Ricerca</dhv:label></a> >
<%} else if (request.getParameter("return").equals("dashboard")) {%>
<a href="ParafarmaciePre.do?command=DashboardFar">Cruscotto</a> >
<%}--%>
  ...
  <table width="100%" border="0">
    <tr>
      <td nowrap align="right">
        ...
        <a href="ParafarmaciePre.do?command=PrintReport&id=<%= OrgDetails.getId() %>"><dhv:label name="accounts.osa.print">Stampa Scheda O.S.A.</dhv:label></a>
       --%>
      </td>
    </tr>
  </table>
...
<dhv:evaluate if="<%=OrgDetails.isTrashed()%>">
    <dhv:permission name="parafarmacie-parafarmacie-edit">
      <input type="button" value="Ripristina"	onClick="javascript:window.location.href='ParafarmaciePre.do?command=Restore&idPrescrizione=<%= OrgDetails.getIdPrescrizione() %>';">
    </dhv:permission>
</dhv:evaluate>
<dhv:evaluate if="<%=!OrgDetails.isTrashed()%>">
 
    <dhv:permission name="parafarmacie-parafarmacie-edit"><input type="button"  value="Modifica"	onClick="javascript:window.location.href='ParafarmaciePre.do?command=ModifyPre&idPrescrizione=<%= OrgDetails.getIdPrescrizione() %><%= addLinkParams(request, "popup|actionplan") %>';"></dhv:permission>
  
  
    <%--dhv:permission name="parafarmacie-parafarmacie-edit">
      <input type="button" value="<dhv:label name="global.button.Enable">Enable</dhv:label>" 	onClick="javascript:window.location.href='ParafarmaciePre.do?command=Enable&idPrescrizione=<%= OrgDetails.getIdPrescrizione() %>';">
    </dhv:permission--%>
 
  <dhv:evaluate if='<%= (request.getParameter("actionplan") == null) %>'>
    <dhv:permission name="parafarmacie-parafarmacie-delete"><input type="button" value="<dhv:label name="accounts.accounts_details.DeleteAccount">Delete Account</dhv:label>" onClick="javascript:popURLReturn('ParafarmaciePre.do?command=ConfirmDeleteFar&id=<%=OrgDetails.getIdPrescrizione()%>&popup=true','ParafarmaciePre.do?command=SearchFar', 'Delete_account','320','200','yes','no');"></dhv:permission>
  </dhv:evaluate>
</dhv:evaluate>
<dhv:permission name="parafarmacie-parafarmacie-edit,parafarmacie-parafarmacie-delete"><br>&nbsp;</dhv:permission>
...
<dhv:evaluate if="<%=OrgDetails.isTrashed()%>">
    <dhv:permission name="parafarmacie-parafarmacie-edit">
      <input type="button" value="Ripristina"	onClick="javascript:window.location.href='ParafarmaciePre.do?command=RestoreFar&idPrescrizione=<%= OrgDetails.getIdPrescrizione() %>';">
    </dhv:permission>
</dhv:evaluate>
<dhv:evaluate if="<%=!OrgDetails.isTrashed()%>">
    ...
    <%--dhv:permission name="parafarmacie-parafarmacie-edit">
      <input type="button" value="<dhv:label name="global.button.Enable">Enable</dhv:label>" 	onClick="javascript:window.location.href='ParafarmaciePre.do?command=EnableFar&idPrescrizione=<%= OrgDetails.getIdPrescrizione() %>';">
    </dhv:permission--%>
