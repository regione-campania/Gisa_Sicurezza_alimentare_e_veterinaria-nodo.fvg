...
<%@ page import="java.util.*,org.aspcfs.modules.parafarmacie.base.*,com.zeroio.iteam.base.*,org.aspcfs.modules.base.Constants" %>
...
<form method="post" name="inputForm" action="ParafarmacieDocuments.do?command=UploadVersion<%= addLinkParams(request, "popup|popupType|actionId|actionplan") %>" enctype="multipart/form-data" onSubmit="return checkFileForm(this);">
...
<tr>
<td>
<a href="Farmacosorveglianza.do?command=DashboardScelta">Farmacie / Grossisti / Parafarmacie</a> > 
<a href="Parafarmacie.do?command=SearchFcie"><dhv:label name="accounts.SearchResults">Search Results</dhv:label></a> >
<a href="Parafarmacie.do?command=DetailsFcie&orgId=<%= OrgDetails.getOrgId() %>">Scheda Canile</a> >
<a href="ParafarmacieDocuments.do?command=View&orgId=<%= OrgDetails.getOrgId() %>"><dhv:label name="accounts.accounts_documents_details.Documents">Documents</dhv:label></a> >
...
<dhv:container name="parafarmacie" selected="documents" object="OrgDetails" hideContainer='<%= "true".equals(request.getParameter("actionplan")) %>' param='<%= "orgId=" + OrgDetails.getOrgId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId|actionplan") %>'>
<table border="0" cellpadding="4" cellspacing="0" width="100%">
  <tr class="subtab">
    <td>
      <% String documentLink = "ParafarmacieDocuments.do?command=View&orgId="+OrgDetails.getOrgId()+ addLinkParams(request, "popup|popupType|actionId|actionplan"); %>
  ...
  <input type="submit" value="Annulla" onClick="javascript:this.form.dosubmit.value='false';this.form.action='ParafarmacieDocuments.do?command=View&orgId=<%= OrgDetails.getOrgId() %>&folderId=<%= (String)request.getAttribute("folderId") %><%= addLinkParams(request, "popup|popupType|actionId|actionplan") %>';" />
</dhv:container>
</form>
</body>
