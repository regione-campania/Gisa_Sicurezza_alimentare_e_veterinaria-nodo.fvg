<%@page import="org.aspcfs.modules.izsmibr.base.InvioMassivoMolluschi"%>
...
<table class="trails" cellspacing="0">
	<tr>
		<td width="100%"><a href="GestioneInvioMolluschi.do?command=ToImportMolluschi"><dhv:label
			name="">INVIO ESITI PRELIEVI MOLLUSCHI</dhv:label></a> > <dhv:label
...
<dhv:container name="inviocamoll" selected="Lista Invii IBR" object="">
...
<table class="details" width="100%">
 ...
 <%
 ArrayList<InvioMassivoMolluschi> rImport = ( ArrayList<InvioMassivoMolluschi> ) request.getAttribute("ListaInvii");
 %>
	<div class="pager">
	...
	<% 
		...
		if ( rImport.size() > 0 ) {
				...
				for ( int i=0; i< rImport.size(); i++ ) {
					...
					System.out.println("HEADERRRRRRRRR "+ ((rImport.get(i).getDocUplodato()!=null)?(rImport.get(i).getDocUplodato().getIdHeader()):("documento non uploadato")));
	...
	%>
			...
			<tr>
				...
				<%
						...
						out.println("Documento non caricato. <br/> Errori durante l'elaborazione.");
				%>
				</td>
				<td align="right"><%=(rImport.get(i).getErroreElaborazione()==null)?(""):(rImport.get(i).getErroreElaborazione())%> </td>
				<td align="right"><input type = "button" value="Download Esito"onclick="openRichiestaPDF_LogMolluschi(<%=rImport.get(i).getId() %>)"></td>
				<td align="right"><input type = "button" value="Download CSV KO"onclick="location.href='GestioneInvioMolluschi.do?command=EsportaCsv&idInvio=<%=rImport.get(i).getId() %>'"></td>
