...
<a href="GestioneBacheca.do?storeId=<%=storeId%>&storeArchiviato=true">Scheda archivio documenti</a>
</td>
</tr>
</tbody></table>

<%if (!folderId.equals("-1")){ %>
<a href="GestioneBacheca.do?command=ListaAllegati&storeId=<%=storeId%>&folderId=<%=parentId%>&parentId=<%=grandparentId %>&storeArchiviato=true">
  ...
  <col width="15%">
	...
	if (listaAllegati.size()>0)
		for (int i=0;i<listaAllegati.size(); i++){
			...
			<a href="GestioneBacheca.do?command=ListaAllegati&storeId=<%=storeId%>&folderId=<%=doc.getIdCartella()%>&parentId=<%=folderId%>&storeArchiviato=true"><%= doc.getNomeDocumento() %></a> </td> 
			<td>CARTELLA</td>
...
<td>Pagine </td>
	<td>
	<%if (pagSel>1){ %>
	<a href="GestioneBacheca.do?command=ListaAllegati&storeId=<%=storeId%>&pag=<%=pagSel-1%>"><%= "<<" %></a>
	<%} %>
	</td>
	...
	<td>(<%=pagTotali%> <a href="GestioneBacheca.do?command=ListaAllegati&storeId=<%=storeId%>&pagine=no"><%= "Tutto" %></a>)</td></tr></table>
...
<!-- aggiornamento pagina per modifica indirizzo in request --> 
   if(window.location.href.substr(-9) !== "&def=true") {
	  loadModalWindow();
      window.location.href = 'GestioneBacheca.do?command=ListaAllegati&storeId=<%=storeId%>&folderId=<%=folderId%>&parentId=<%=parentId%>&messaggioPost=<%=messaggioPost%>&storeArchiviato=true&&pag=<%=pag%>&pagTot=<%=pagTot%>&pagine=<%=pagine%>&def=true';
    }
</script>
