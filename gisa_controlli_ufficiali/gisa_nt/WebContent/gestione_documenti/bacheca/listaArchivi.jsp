...
function gestioneArchiviati(soloArchiviati){
	 loadModalWindow();
	if (soloArchiviati=='1')
	  window.location.href = 'GestioneBacheca.do?command=ListaAllegati&storeId=<%=storeId%>&folderId=<%=folderId%>&parentId=<%=parentId%>&archiviati=true&def=true';
	else
	 window.location.href = 'GestioneBacheca.do?command=ListaAllegati&storeId=<%=storeId%>&folderId=<%=folderId%>&parentId=<%=parentId%>&def=true';
...
Lista
 ...
 <dhv:permission name="documentale_bacheca-add">
  ...
  <col width="30%">
	...
	if (listaAllegati.size()>0)
		for (int i=0;i<listaAllegati.size(); i++){
			...
			<tr <%=(doc.getPriorita()>=100) ? "style=\"background:#b2ffb2\"" : "class=\"row"+(i%2)+"\"" %>>
			<td>
...
<% if (doc.getDataArchiviazione()==null || doc.getDataArchiviazione().equals("null")) { %>
 ...
 <a href="GestioneBacheca.do?command=GestisciArchivio&storeId=<%=doc.getStoreId()%>&operazione=cancellaArchivio" onClick="if (confirm('ATTENZIONE! Stai per cancellare l\'archivio e tutto il suo contenuto. Sei sicuro di continuare?')) { loadModalWindow(); return true;}">
...
<% } %>			
			...
			&nbsp; </td>
			<td><a href="GestioneBacheca.do?command=ListaAllegati&pagine=no&storeId=<%=doc.getStoreId()%>
			<% if (doc.getDataArchiviazione()!=null && !doc.getDataArchiviazione().equals("null")) {%>
			&storeArchiviato=true
			<%}%>
			"><b><%= doc.getFileTitolo() %></a></b>&nbsp;  <b>(<%=doc.getContaFile() %>)</b> <%=(doc.getPriorita()>=100) ? "<font color=\"red\" size=\"1px\"><b>IMPORTANT!</b></font>" : "" %></td>
...
<td>Pagine </td>
	<td>
	<%if (pagSel>1){ %>
	<a href="GestioneBacheca.do?command=ListaAllegati&storeId=-1&pag=<%=pagSel-1%>"><%= "<<" %></a>
	<%} %>
	</td>
	...
	<td>(<%=pagTotali%> <a href="GestioneBacheca.do?command=ListaAllegati&storeId=-1&pagine=no"><%= "Tutto" %></a>)</td></tr></table>
