...
<dhv:container name="canili" selected="vigilanza" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId() %>' hideContainer='<%= isPopup(request) || (defectCheck != null && !"".equals(defectCheck.trim())) %>'>
	...
	%>
	<dhv:permission name="canili-vigilanza-delete">
		<input type="button"
			value="Ripristina"
			onClick="javascript:this.form.action='CaniliVigilanza.do?command=Restore&id=<%= TicketDetails.getId()%>';submit();">
	</dhv:permission>
	<%
	...
	<%--<dhv:permission name="accounts-accounts-vigilanza-edit">
		<input type="button"
			...
			onClick="javascript:this.form.action='AccountVigilanza.do?command=ReopenTicket&id=<%= TicketDetails.getId()%><%= (defectCheck != null && !"".equals(defectCheck.trim())?"&defectCheck="+defectCheck:"") %>';submit();">
	</dhv:permission>--%>
	<%
		} else {
	%>
	<dhv:permission name="canili-vigilanza-add">
		<input type="button"
			value="<dhv:label name="">Continua</dhv:label>"
			onClick="javascript:this.form.action='CaniliVigilanza.do?command=ModifyTicket&auto-populate=true<%= (defectCheck != null && !"".equals(defectCheck.trim())?"&defectCheck="+defectCheck:"") %>';submit();">
	</dhv:permission>
	...
	<dhv:permission name="canili-vigilanza-delete">
		...
		<input type="button"
			...
			onClick="javascript:popURL('CaniliVigilanza.do?command=ConfirmDelete&id=<%= TicketDetails.getId() %>&orgId=<%=TicketDetails.getOrgId()%>&return=searchResults&popup=true', 'Delete_ticket','320','200','yes','no');">
		<%
			} else {
		%>
		<input type="button"
			value="<dhv:label name="global.button.delete">Delete</dhv:label>"
			onClick="javascript:popURL('CaniliVigilanza.do?command=ConfirmDelete&id=<%= TicketDetails.getId() %>&orgId=<%=TicketDetails.getOrgId()%>&popup=true', 'Delete_ticket','320','200','yes','no');">
		<%
			}
		%>
	</dhv:permission>
	<dhv:permission name="canili-vigilanza-edit">
		<input type="button"
			...
			onClick="javascript:this.form.action='CaniliVigilanza.do?command=Chiudi&id=<%= TicketDetails.getId() %>';if( confirm('Sei sicuro di voler chiudere il campione?') ){submit()};">
	</dhv:permission>
	<%
		}
	%>
	<dhv:permission name="canili-vigilanza-edit,canili-vigilanza-delete">
		<br />&nbsp;<br />
	</dhv:permission>
	<%-- Ticket Information --%>
	<%-- Primary Contact --%>
		...
		<table cellpadding="4" cellspacing="0" width="100%" class="details">
			...
			<tr class="containerBody">
				...
				<td><dhv:evaluate
					...
					<dhv:permission name="canili-contacts-view">
						<a
					...
					<dhv:permission name="canili-contacts-view" none="true">
