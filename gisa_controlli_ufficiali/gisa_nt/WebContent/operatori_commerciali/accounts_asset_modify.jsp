<%-- 
  ...
  - 
  - Version: $Id: accounts_asset_modify.jsp 18491 2007-01-15 21:07:55Z matt $
  - Description:
  --%>
...
<form name="addAccountAsset" action="AccountsAssetsAbus.do?command=Update&auto-populate=true&return<%= request.getParameter("return") %><%= addLinkParams(request, "popup|popupType|actionId") %>" onSubmit="return doCheck(this);" method="post">
<dhv:evaluate if="<%= !isPopup(request) %>">
<%-- Trails --%>
<table class="trails" cellspacing="0">
<tr>
<td width="100%">
  <a href="Abusivismi.do"><dhv:label name="accounts.accounts">Accounts</dhv:label></a> > 
  <a href="Abusivismi.do?command=Search"><dhv:label name="accounts.SearchResults">Search Results</dhv:label></a> >
  <a href="Abusivismi.do?command=Details&orgId=<%= OrgDetails.getOrgId() %>"><dhv:label name="accounts.details">Account Details</dhv:label></a> >
  <a href="AccountsAssetsAbus.do?command=List&orgId=<%= OrgDetails.getOrgId() %>"><dhv:label name="accounts.Assets">Assets</dhv:label></a> >
...
<%  } %>
  ...
  <dhv:container name='accountsassets' selected='details' object='asset' param='<%= param2 %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId") %>'>
    ...
    <%if ("list".equals(request.getParameter("return"))) { %>
      <input type="button" value="Annulla" onClick="window.location.href='AccountsAssetsAbus.do?command=List&orgId=<%=OrgDetails.getOrgId()%>&parentId=<%= asset.getParentId() %><%= addLinkParams(request, "popup|popupType|actionId") %>';this.form.dosubmit.value='false';" />
    <%}else{ %>
      <input type="button" value="Annulla" onClick="window.location.href='AccountsAssetsAbus.do?command=View&orgId=<%=OrgDetails.getOrgId()%>&id=<%=asset.getId()%>&parentId=<%= asset.getParentId() %><%= addLinkParams(request, "popup|popupType|actionId") %>';this.form.dosubmit.value='false';" />
    <%}%>
    ...
    <input type="submit" value="<dhv:label name="global.button.update">Update</dhv:label>" onClick="this.form.dosubmit.value='true';" />
    <%if ("list".equals(request.getParameter("return"))) { %>
      <input type="button" value="Annulla" onClick="window.location.href='AccountsAssetsAbus.do?command=List&orgId=<%=OrgDetails.getOrgId()%><%= addLinkParams(request, "popup|popupType|actionId") %>';this.form.dosubmit.value='false';" />
    <%}else{ %>
      <input type="button" value="Annulla" onClick="window.location.href='AccountsAssetsAbus.do?command=View&orgId=<%=OrgDetails.getOrgId()%>&id=<%=asset.getId()%><%= addLinkParams(request, "popup|popupType|actionId") %>';this.form.dosubmit.value='false';" />
    <%}%>
