...
var result;
 ...
 <% if (Boolean.parseBoolean((String)request.getAttribute("linee_pregresse"))) {%>
   <script>
        ...
        window.location.href='OperatoriCommerciali.do?command=Details&orgId=<%= OrgDetails.getOrgId() %>';
    } else {
...
costruisci_rel_ateco_attivita('<%= OrgDetails.getOrgId() %>', 'id_linea_sottoposta_a_controllo');">
 ...
 <%
    ...
    TipoCampione.setJsEvent("onChange=javascript:reloadAddCU('OperatoriCommercialiVigilanza.do?command=Add&orgId="+OrgDetails.getOrgId()+"&tipoCampione='+this.value)"); 


%>


<form method="post" name="addticket" action="OperatoriCommercialiVigilanza.do?command=Insert&auto-populate=true"  >
<%-- Trails --%>
...
<input type="submit" value="Annulla" onClick="javascript:this.form.action='OperatoriCommerciali.do?command=ViewVigilanza&orgId=<%=OrgDetails.getOrgId()%>'">
<br>
