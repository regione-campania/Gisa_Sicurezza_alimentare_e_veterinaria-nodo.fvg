...
<form name="viewAccountAudit" action="CheckListStabilimenti.do?command=Modify&auto-populate=true&id=<%=Audit.getId()%>&return=details<%= addLinkParams(request, "popup|popupType|actionId") %>" method="post">
	...
	<% if (OrgDetails.isOperatoreIttico()) { %>
		<table class="trails" cellspacing="0">
				...
				<td>
				    ...
				    <dhv:label name="audit.aggiungiAudit">Scheda Check List</dhv:label>
				</td>
			</tr>
		</table>
	<% } else { %>
    ...
    <tr>
       <td nowrap align="right">
        ...
        <%--<a href="Accounts.do?command=PrintReport&id=<%= OrgDetails.getId() %>"><dhv:label name="accounts.osa.print">Stampa Scheda Audit</dhv:label></a>--%>
         ...
         <input type="button" value="Stampa Scheda Check List"	onClick='javascript:location.href="CheckListStabilimenti.do?command=Stampa&id=<%= Audit.getId() %>";'>
      <%
    }
    else {
    %>
...
<dhv:evaluate if="<%= !OrgDetails.isTrashed() %>">
  <input type="hidden" name="idC" value="<%=Audit.getIdControllo() %>">
     ...
     <dhv:permission name="stabilimenti-stabilimenti-audit-edit"><input type="submit" value="Modifica"></dhv:permission>
