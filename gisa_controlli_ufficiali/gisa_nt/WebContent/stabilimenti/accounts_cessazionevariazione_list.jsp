...
<%@ page import="java.util.*,java.text.DateFormat,org.aspcfs.modules.stabilimenti.base.*,org.aspcfs.modules.cessazionevariazione.base.Ticket,com.zeroio.iteam.base.*" %>
...
<dhv:container name="stabilimenti" selected="cessazionevariazione" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId") %>'>
  <dhv:evaluate if="<%= !OrgDetails.isTrashed() %>">
    <dhv:permission name="stabilimenti-stabilimenti-cessazionevariazione-add"><a href="StabilimentiCessazionevariazione.do?command=AddTicket&tipo_richiesta=autorizzazione_trasporto_animali_vivi&orgId=<%= OrgDetails.getOrgId() %><%= addLinkParams(request, "popup|popupType|actionId") %>"><dhv:label name="accounts.richiesta.add">Aggiungi Nuova Voltura</dhv:label></a></dhv:permission>
  </dhv:evaluate>
  ...
  %>
    <tr class="row<%= rowid %>">
		...
		<td width="10%" valign="top" nowrap>
			...
			<a href="StabilimentiCessazionevariazione.do?command=TicketDetails&id=<%= thisTic.getId() %>&orgId=<%= thisTic.getOrgId()%><%= addLinkParams(request, "popup|popupType|actionId") %>"><%= thisTic.getPaddedId() %></a>
		</td>
		<td>
		<% 
		String color ="orange" ;
...
<tr class="row<%= rowid %>">
      <td colspan="7" valign="top">
        ...
        %>
          <a href="StabilimentiCessazionevariazioneDocuments.do?command=Download&stream=true&tId=<%= thisTic.getId() %>&fid=<%= thisFile.getId() %>"><img src="images/file-audio.gif" border="0" align="absbottom"></a>
        <%
              }
            }
          }
