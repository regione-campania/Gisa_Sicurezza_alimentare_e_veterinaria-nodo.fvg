...
<form name="details" action="StabilimentiTamponi.do?command=ModifyTicket&auto-populate=true" method="post">
	...
	<% if (OrgDetails.isOperatoreIttico()) { %>
		<table class="trails" cellspacing="0">
				...
				<td>
					...
					<%--a href="Stabilimenti.do?command=ViewTamponi&orgId=<%=TicketDetails.getOrgId()%>"><dhv:label name="tamponi">Tamponi</dhv:label></a> --%>
...
<dhv:container name="accountstamponi" selected="details" object="TicketDetails"
	...
	%>
	<dhv:permission name="stabilimenti-stabilimenti-tamponi-delete">
		<input type="button"
			value="Ripristina"
			onClick="javascript:this.form.action='StabilimentiTamponi.do?command=Restore&id=<%= TicketDetails.getId()%>';submit();">
	</dhv:permission>
	<%
	...
	<dhv:permission name="reopen-reopen-view">
		<input type="button"
			...
			onClick="javascript:this.form.action='StabilimentiTamponi.do?command=ReopenTicket&id=<%= TicketDetails.getId()%><%= (defectCheck != null && !"".equals(defectCheck.trim())?"&defectCheck="+defectCheck:"") %>';submit();">
	</dhv:permission>
	<%
		} else {
	%>
	<dhv:permission name="stabilimenti-stabilimenti-tamponi-edit">
		<input type="button"
			value="<dhv:label name="global.button.modifya">Modifica/Inserisci Esito</dhv:label>"
			onClick="javascript:this.form.action='StabilimentiTamponi.do?command=ModifyTicket&auto-populate=true<%= (defectCheck != null && !"".equals(defectCheck.trim())?"&defectCheck="+defectCheck:"") %>';submit();">
	</dhv:permission>
	<%--
	...
	<dhv:permission name="stabilimenti-stabilimenti-tamponi-delete">
		...
		<input type="button"
			...
			onClick="javascript:popURL('StabilimentiTamponi.do?command=ConfirmDelete&id=<%= TicketDetails.getId() %>&orgId=<%=OrgDetails.getOrgId() %>&return=searchResults&popup=true', 'Delete_ticket','320','200','yes','no');">
		<%
			} else {
		%>
		<input type="button"
			value="<dhv:label name="global.button.delete">Delete</dhv:label>"
			onClick="javascript:popURL('StabilimentiTamponi.do?command=ConfirmDelete&id=<%= TicketDetails.getId() %>&orgId=<%=OrgDetails.getOrgId() %>&popup=true', 'Delete_ticket','320','200','yes','no');">
		<%
			}
		%>
	</dhv:permission>
...
if(request.getAttribute("Messaggio2")!=null)
{
	...
	function doSubmit(idTicket){
		...
		document.details.action="StabilimentiTamponi.do?command=Chiudi&id="+idTicket+"&chiudiCu=true'"
