...
<form name="details" action="StabilimentiCessazionevariazione.do?command=ModifyTicket&auto-populate=true" method="post">
...
<td>
  ...
  <%--<a href="Accounts.do?command=ViewCessazionevariazione&orgId=<%=TicketDetails.getOrgId()%>"><dhv:label name="cessazionevariazione2">Cessazioni/Variazioni</dhv:label></a> >--%>
    <a href="Stabilimenti.do?command=ViewCessazionevariazione&orgId=<%=TicketDetails.getOrgId()%>"><dhv:label name="cessazionevariazione2">Volture</dhv:label></a> >
...
%>
<dhv:container name="stabilimenti" selected="cessazionevariazione" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId() %>' hideContainer='<%= isPopup(request) || (defectCheck != null && !"".equals(defectCheck.trim())) %>'>
	...
	%>
	<dhv:permission name="stabilimenti-stabilimenti-cessazionevariazione-delete">
		<input type="button"
			value="Ripristina"
			onClick="javascript:this.form.action='StabilimentiCessazionevariazione.do?command=Restore&id=<%= TicketDetails.getId()%>';submit();">
	</dhv:permission>
	<%
	...
	<dhv:permission name="stabilimenti-stabilimenti-cessazionevariazione-edit">
		<input type="button"
			value="Modifica"
			onClick="javascript:this.form.action='StabilimentiCessazionevariazione.do?command=ModifyTicket&id=<%=TicketDetails.getId() %>&auto-populate=true<%= (defectCheck != null && !"".equals(defectCheck.trim())?"&defectCheck="+defectCheck:"") %>';submit();">
	</dhv:permission>
	...
	<dhv:permission name="stabilimenti-stabilimenti-cessazionevariazione-delete">
		...
		<input type="button"
			...
			onClick="javascript:popURL('StabilimentiCessazionevariazione.do?command=ConfirmDelete&id=<%= TicketDetails.getId() %>&orgId=<%= TicketDetails.getOrgId()%>&return=searchResults&popup=true', 'Delete_ticket','320','200','yes','no');">
		<%
			} else {
		%>
		<input type="button"
			value="<dhv:label name="global.button.delete">Delete</dhv:label>"
			onClick="javascript:popURL('StabilimentiCessazionevariazione.do?command=ConfirmDelete&id=<%= TicketDetails.getId() %>&orgId=<%= TicketDetails.getOrgId()%>&popup=true', 'Delete_ticket','320','200','yes','no');">
		<%
			}
		%>
	</dhv:permission>
	<dhv:permission name="stabilimenti-stabilimenti-cessazionevariazione-edit">
		<input type="button"
			...
			onClick="javascript:this.form.action='StabilimentiCessazionevariazione.do?command=Chiudi&id=<%= TicketDetails.getId() %>';if( confirm('Sei sicuro di voler chiudere questa variazione?') ){submit()};">
	</dhv:permission>
	<%
		}
	}
	...
	{
	%>
	<dhv:permission name="stabilimenti-stabilimenti-cessazionevariazione-inoltra-vet-view">
		
		<div class="buttonwrapper">
<a class="ovalbutton" href = "#dialog4" name="modal" ><span >Invia A Funzionari Regionali</span></a>
</div>
	...
	{
	%>
	<dhv:permission name="stabilimenti-stabilimenti-cessazionevariazione-inoltra-stap-view">
...
<a class="ovalbutton" href = "#dialog4" name="modal" ><span >Approva</span></a>
	...
	<dhv:permission name="stabilimenti-stabilimenti-cessazionevariazione-approva-view">
		...
		<div class="buttonwrapper">
<input type = "button"  onclick="javascript:location.href='StabilimentiCessazionevariazione.do?command=ConfermaVoltura&approvazione=1&idVoltura=<%=TicketDetails.getId() %>'" value = "approva"  />
</div>
	...
	<tr class="containerBody">
      <td nowrap class="formLabel">
       ...
       <td>
        ...
        <%= (TicketDetails.getDataNascitaRappresentante()!=null)?(toHtml(DateUtils.getDateAsString(TicketDetails.getDataNascitaRappresentante(),Locale.ITALY))):("") %>
         </td>
    </tr>
    </dhv:evaluate>
...
<form method="post" action="StabilimentiCessazionevariazione.do?command=ConfermaVoltura" name = "motivazioneForm">
    ...
    <th colspan="2">
      <strong><dhv:label name="">Approvazione</dhv:label></strong>
   	...
   	<tr class="containerBody">
			...
			<dhv:permission name="stabilimenti-stabilimenti-cessazionevariazione-inoltra-vet-view">
			<option value = "3">Inoltra Ai Funzionari Regionali</option>
			</dhv:permission>
