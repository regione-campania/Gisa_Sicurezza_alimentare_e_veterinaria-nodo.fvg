...
<td>
  ...
  <a href="Stabilimenti.do?command=Details&orgId=<%=TicketDetails.getOrgId()%>"><dhv:label name="stabilimenti.details">Account Details</dhv:label></a> >
  <a href="Stabilimenti.do?command=ViewVigilanza&orgId=<%=TicketDetails.getOrgId()%>"><dhv:label name="vigilanza">Tickets</dhv:label></a> >
  <dhv:label name="campione.dettagli">Scheda Controlli Ufficiali</dhv:label>
</td>
</tr>
</table>
<%-- End Trails --%>
...
%>
<dhv:container name="<%=(OrgDetails.isMacelloUngulati()) ? ("stabilimenti_macellazioni_ungulati") : ("stabilimenti") %>" selected="vigilanza" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId() %>' hideContainer='<%= isPopup(request) || (defectCheck != null && !"".equals(defectCheck.trim())) %>'>
<dhv:container name="accountsvigilanza" selected="details" object="TicketDetails"
	param="<%= param1 %>"
	...
	<dhv:permission name="vigilanza-vigilanza-delete">
		<input type="button"
			value="Ripristina"
			onClick="javascript:this.form.action='StabilimentiVigilanza.do?command=Restore&id=<%= TicketDetails.getId()%>';submit();">
	</dhv:permission>
	<%
	...
	<%--<dhv:permission name="vigilanza-vigilanza-edit">
		<input type="button"
			...
			onClick="javascript:this.form.action='StabilimentiVigilanza.do?command=ReopenTicket&id=<%= TicketDetails.getId()%><%= (defectCheck != null && !"".equals(defectCheck.trim())?"&defectCheck="+defectCheck:"") %>';submit();">
	</dhv:permission>--%>
	<%
		} else {
	%>
	<dhv:permission name="vigilanza-vigilanza-add">
		<input type="button"
			value="<dhv:label name="">Continua</dhv:label>"
			onClick="javascript:this.form.action='StabilimentiVigilanza.do?command=ModifyTicket&auto-populate=true<%= (defectCheck != null && !"".equals(defectCheck.trim())?"&defectCheck="+defectCheck:"") %>';submit();">
	</dhv:permission>
	<dhv:permission name="vigilanza-vigilanza-edit">
		<input type="button"
			value="Modifica"
	...
	<dhv:permission name="vigilanza-vigilanza-delete">
		...
		<input type="button"
			...
			onClick="javascript:popURL('StabilimentiVigilanza.do?command=ConfirmDelete&id=<%= TicketDetails.getId() %>&orgId=<%=TicketDetails.getOrgId()%>&return=searchResults&popup=true', 'Delete_ticket','320','200','yes','no');">
		<%
			} else {
		%>
		<input type="button"
			value="<dhv:label name="global.button.delete">Delete</dhv:label>"
			onClick="javascript:popURL('StabilimentiVigilanza.do?command=ConfirmDelete&id=<%= TicketDetails.getId() %>&orgId=<%=TicketDetails.getOrgId()%>&popup=true', 'Delete_ticket','320','200','yes','no');">
		<%
			}
		%>
	</dhv:permission>
	<dhv:permission name="vigilanza-vigilanza-edit">
		<input type="button"
			...
			onClick="javascript:this.form.action='StabilimentiVigilanza.do?command=Chiudi&id=<%= TicketDetails.getId() %>';if( confirm('Sei sicuro di voler chiudere il campione?') ){submit()};">
	</dhv:permission>
	<%
		}
	%>
	<dhv:permission name="vigilanza-vigilanza-edit,vigilanza-vigilanza-delete">
		<br />&nbsp;<br />
	</dhv:permission>
	<%-- Ticket Information --%>
	<%-- Primary Contact --%>
		...
		<table cellpadding="4" cellspacing="0" width="100%" class="details">
			...
			<tr class="containerBody">
				...
				<td><dhv:evaluate
					...
					<dhv:permission name="stabilimenti-stabilimenti-contacts-view">
						<a
					...
					<dhv:permission name="stabilimenti-stabilimenti-contacts-view" none="true">
...
<dhv:evaluate if="<%= TicketDetails.getDestinatarioCampione()!= -1%>">
   <tr class="containerBody">
      <td name="tipoCampione1" id="tipoCampione1" nowrap class="formLabel">
        <dhv:label name="">Destinatario del Controllo Ufficiale</dhv:label>
      </td>
    <td>
