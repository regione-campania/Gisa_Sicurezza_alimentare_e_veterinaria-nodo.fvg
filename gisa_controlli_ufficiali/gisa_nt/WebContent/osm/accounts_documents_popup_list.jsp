...
<%@ page import="java.util.*,java.text.DateFormat,org.aspcfs.modules.osm.base.*,com.zeroio.iteam.base.*" %>
...
<dhv:container name="osm" selected="documents" object="OrgDetails" hideContainer='<%= "true".equals(request.getParameter("actionplan")) %>' param='<%= "orgId=" + OrgDetails.getOrgId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId|actionplan") %>'>
  <%
    String permission_doc_folders_add ="osm-osm-documents-add";
    String permission_doc_files_upload = "osm-osm-documents-add";
    String permission_doc_folders_edit = "osm-osm-documents-edit";
    String documentFolderAdd ="AccountsDocumentsFoldersOsm.do?command=Add&orgId="+OrgDetails.getOrgId();
    String documentFileAdd = "AccountsDocumentsOsm.do?command=Add&orgId="+ OrgDetails.getOrgId();
    String documentFolderModify = "AccountsDocumentsFoldersOsm.do?command=Modify&orgId="+ OrgDetails.getOrgId();
    String documentFolderList = "AccountsDocumentsOsm.do?command=View&orgId="+OrgDetails.getOrgId()+"&popup=true";
    String documentFileDetails = "AccountsDocumentsOsm.do?command=Details&orgId="+ OrgDetails.getOrgId();
...
%>    
  <tr>
    <td class="row<%= rowid %>" width="100%">
      ...
      <a href="AccountsDocumentsOsm.do?command=Download&orgId=<%= OrgDetails.getOrgId() %>&folderId=<%= thisFile.getFolderId() %>&fid=<%= thisFile.getId() %>"><%= toHtml(thisFile.getSubject()) %></a>
    </td>
...
&nbsp;<br/>
<form method="post" name="inputForm" action="AccountsDocumentsOsm.do?command=Upload&popup=true" onSubmit="return checkFileForm(this);" enctype="multipart/form-data">
