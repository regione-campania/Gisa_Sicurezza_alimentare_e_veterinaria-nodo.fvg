...
<%@page import="org.aspcfs.modules.osm.base.Organization"%>
...
<form name="viewAccountAudit" action="ChecklistOsm.do?command=Modify&auto-populate=true&id=<%=Audit.getId()%>&return=details<%= addLinkParams(request, "popup|popupType|actionId") %>" method="post">
    ...
    <tr>
       <td nowrap align="right">
        ...
        <%--<a href="Accounts.do?command=PrintReport&id=<%= OrgDetails.getId() %>"><dhv:label name="accounts.osa.print">Stampa Scheda Audit</dhv:label></a>--%>
         ...
         <input type="button" value="Stampa Scheda Check List"	onClick='javascript:location.href="ChecklistOsm.do?command=Stampa&id=<%= Audit.getId() %>";'>
      <%
    }
    else {
    %>
