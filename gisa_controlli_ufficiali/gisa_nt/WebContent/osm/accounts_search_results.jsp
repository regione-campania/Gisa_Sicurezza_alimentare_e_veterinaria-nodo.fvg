...
<script language="JavaScript" type="text/javascript">
  ...
  function reopen() {
    scrollReload('Osm.do?command=Search');
  }
</script>
<%-- Trails --%>
...
<%-- End Trails --%>
<dhv:permission name="osm-osm-add"><a href="Osm.do?command=Add">Aggiungi Allevamento</a></dhv:permission>
<dhv:permission name="osm-osm-add" none="true"><br></dhv:permission>
...
<tr>
  ...
  <th width="30%" nowrap="true">
    <strong><a href="Osm.do?command=Search&column=c.namelast">Nome</a></strong>
    <%= SearchOrgListInfo.getSortIcon("c.namelast") %>
  </th>
  <th width="10%" >
    <strong><dhv:label name="stabilimenti.stabilimenti_contacts_add.Title">Title</dhv:label></strong>
  </th>   
  <th width="20%" nowrap="true">
    <strong><a href="Osm.do?command=Search&column=c.org_name"><dhv:label name="organization.name">Account Name</dhv:label></a></strong>
  ...
  <th width="30%">
        <strong><dhv:label name="stabilimenti.phoneFax">Phone/Fax</dhv:label></strong>
  </th>
  <th>
        <strong><dhv:label name="stabilimenti.stabilimenti_add.Email">Email</dhv:label></strong>
  </th>
</tr>
<%
...
%>      
      ...
      <td valign="center">
        <a href="ContactsOsm.do?command=Details&id=<%=thisContact.getId()%>"><%= toHtml(thisContact.getNameLastFirst()) %></a>
      </td>
      <td valign="center">
        <%= toHtml(thisContact.getTitle()) %>
      </td>
      <td valign="center" nowrap>
        <a href="Osm.do?command=Details&orgId=<%= thisContact.getOrgId() %>"><%= toHtml(thisContact.getOrgName()) %></a>
      </td>
      <dhv:evaluate if='<%= SearchOrgListInfo.getSearchOptionValue("searchcodeSiteId") == String.valueOf(Constants.INVALID_SITE) %>'>
