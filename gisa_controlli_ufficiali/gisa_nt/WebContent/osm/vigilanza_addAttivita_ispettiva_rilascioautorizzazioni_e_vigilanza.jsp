...
<form name="addticket" action="OsmVigilanza.do?command=Insert&auto-populate=true" method="post">
<%-- Trails --%>
...
<input type="submit" value="Annulla" onClick="javascript:this.form.action='Osm.do?command=ViewVigilanza&orgId=<%=OrgDetails.getOrgId()%>'">
<br>
<dhv:formMessage />
...
var result;
 ...
 <% if (Boolean.parseBoolean((String)request.getAttribute("linee_pregresse"))) {%>
   <script>
        ...
        window.location.href='Osm.do?command=Details&orgId=<%= OrgDetails.getOrgId() %>';
    } else {
...
costruisci_rel_ateco_attivita('<%= OrgDetails.getOrgId() %>', 'id_linea_sottoposta_a_controllo', '<%= TicketDetails.getId_imprese_linee_attivita() %>' );">
 ...
 <%
    ...
    TipoCampione.setJsEvent("onChange=javascript:reloadAddCU('OsmVigilanza.do?command=Add&orgId="+OrgDetails.getOrgId()+"&tipoCampione='+this.value)"); 
