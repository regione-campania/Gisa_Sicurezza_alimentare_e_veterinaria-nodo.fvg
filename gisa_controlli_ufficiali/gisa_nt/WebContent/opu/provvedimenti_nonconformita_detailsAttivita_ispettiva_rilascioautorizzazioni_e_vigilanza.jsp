...
<td>
 <a href="<%=TicketDetails.getURlDettaglio() %>.do">Operatori</a> > 
  ...
  <a href="<%=TicketDetails.getURlDettaglio() %>.do?command=Details&stabId=<%=OrgDetails.getIdStabilimento()%>&idStabilimentoopu=<%=OrgDetails.getIdStabilimento()%>%>">Scheda Operatore</a> >
  <a href="<%=TicketDetails.getURlDettaglio() %>.do?command=ViewVigilanza&stabId=<%=OrgDetails.getIdStabilimento()%>&idStabilimentoopu=<%=OrgDetails.getIdStabilimento()%>"><dhv:label name="">Controlli Ufficiali</dhv:label></a> >
  <a href="<%=TicketDetails.getURlDettaglio() %>Vigilanza.do?command=TicketDetails&id=<%= request.getAttribute("idC")%>&stabId=<%=OrgDetails.getIdStabilimento()%>&idStabilimentoopu=<%=OrgDetails.getIdStabilimento()%>"><dhv:label name="">Controllo Ufficiale</dhv:label></a> >
...
<%
	...
	String param1 = "id=" + TicketDetails.getId()+"&stabId="+TicketDetails.getIdStabilimento()+"&opId="+OrgDetails.getOperatore().getIdOperatore()+"&idControllo="+request.getAttribute("idC")+"&idNonConformita="+TicketDetails.getId()+"&dataC="+TicketDetails.getAssignedDate()+"&identificativo="+TicketDetails.getIdentificativo();
	
%>
<dhv:container name="<%=OrgDetails.getContainer() %>" selected="vigilanza" object="Operatore" param="<%="stabId=" + TicketDetails.getIdStabilimento() +"&opId="+OrgDetails.getIdOperatore()%>" hideContainer='<%= isPopup(request) || (defectCheck != null && !"".equals(defectCheck.trim())) %>'>
...
<dhv:evaluate if="<%= hasText(TicketDetails.getCause()) %>">
  ...
  <%@ include file="../nonconformita/opu_provvedimenti_view.jsp" %>
