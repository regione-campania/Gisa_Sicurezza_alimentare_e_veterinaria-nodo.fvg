...
<jsp:useBean id="OrgList" class="org.aspcfs.modules.opu.base.StabilimentoList" scope="request"/>
...
<form name="addticket" action="<%=TicketDetails.getURlDettaglio(OrgDetails.getTipologia(),"AziendeAgricoleOpuStabAltreNonConformita") %>AltreNonConformita.do?command=Insert&auto-populate=true" method="post">
...
<td>
  ...
  <a href="<%=TicketDetails.getURlDettaglio(OrgDetails.getTipologia(),"") %>.do?command=Details&stabId=<%=OrgDetails.getOrgId()%>">Scheda Operatore</a> >
  <a href="<%=TicketDetails.getURlDettaglio(OrgDetails.getTipologia(),"") %>.do?command=ViewVigilanza&stabId=<%=OrgDetails.getOrgId()%>"><dhv:label name="">Controlli Ufficiali</dhv:label></a> >
  <a href="<%=TicketDetails.getURlDettaglio(OrgDetails.getTipologia(),"") %>Vigilanza.do?command=TicketDetails&id=<%= request.getAttribute("idC")%>&idStabilimentoopu=<%=OrgDetails.getOrgId()%>"><dhv:label name="">Controllo Ufficiale</dhv:label></a> >
...
<input type="button" value="Annulla" onClick="if(confirm('ATTENZIONE: si sta per abbandonare la pagina di aggiunta di una non conformita. Lasciando questa pagina il lavoro svolto finora verra perso.Sicuro di voler uscire ?')==true){window.location.href='<%=TicketDetails.getURlDettaglio(OrgDetails.getTipologia(),"") %>Vigilanza.do?command=TicketDetails&id=<%= request.getAttribute("idC") %>&idStabilimentoopu=<%=OrgDetails.getIdStabilimento()%>';this.form.dosubmit.value='0';}" />
<br>
...
<table cellpadding="4" cellspacing="0" width="100%" class="details">
 ...
 <%@ include  file="../nonconformita/opu_provvedimenti_add.jsp" %>
