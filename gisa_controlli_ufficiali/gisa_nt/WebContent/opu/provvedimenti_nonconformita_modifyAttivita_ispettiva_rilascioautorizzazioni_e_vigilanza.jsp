...
<form name="details" action="<%=TicketDetails.getURlDettaglio() %>AltreNonConformita.do?command=UpdateTicket&auto-populate=true<%= addLinkParams(request, "popup|popupType|actionId") %>" onSubmit="return checkForm(this);" method="post">
...
<tr> 
<td>
  <a href="<%=TicketDetails.getURlDettaglio() %>.do"><dhv:label name="">Operatori</dhv:label></a> >
  <a href="<%=TicketDetails.getURlDettaglio() %>.do?command=Search"><dhv:label name="Allevamenti.SearchResults">Search Results</dhv:label></a> >
  <a href="<%=TicketDetails.getURlDettaglio() %>.do?command=Details&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label name="">Scheda Operatore</dhv:label></a> > 
 <a href="<%=TicketDetails.getURlDettaglio() %>.do?command=ViewVigilanza&orgId=<%=TicketDetails.getOrgId()%>"><dhv:label name="">Controlli Ufficiali</dhv:label></a> >
<a href="<%=TicketDetails.getURlDettaglio() %>Vigilanza.do?command=TicketDetails&id=<%= request.getAttribute("idC")%>&orgId=<%=TicketDetails.getOrgId()%>"><dhv:label name="">Controllo Ufficiale</dhv:label></a> >
...
<dhv:container name="<%=TicketDetails.getContainer() %>" selected="vigilanza" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId") %>'>
     <dhv:evaluate if="<%= !TicketDetails.isTrashed() %>" >
      <dhv:evaluate if="<%= TicketDetails.getClosed() != null %>" >
          <dhv:permission name="accounts-accounts-nonconformita-edit">
            <input type="submit" value="<dhv:label name="button.reopen">Reopen</dhv:label>" onClick="javascript:this.form.action='<%=TicketDetails.getURlDettaglio()%>AltreNonConformita.do?command=ReopenTicket&id=<%=TicketDetails.getId()%><%= addLinkParams(request, "popup|popupType|actionId") %>'">
           </dhv:permission>
           ...
           <dhv:evaluate if='<%= !"list".equals(request.getParameter("return"))%>' >
            <input type="submit" value="Annulla" onClick="javascript:this.form.action='<%=TicketDetails.getURlDettaglio()%>AltreNonConformita.do?command=TicketDetails&id=<%= TicketDetails.getId() %><%= addLinkParams(request, "popup|popupType|actionId") %>'" />
           </dhv:evaluate>
      </dhv:evaluate>
      <dhv:evaluate if="<%= TicketDetails.getClosed() == null %>" >
          <dhv:permission name="accounts-accounts-nonconformita-edit">
   ...
   <table cellpadding="4" cellspacing="0" width="100%" class="details">
	<tr>
      <th colspan="2">
        <strong><dhv:label name="nonconformita.information">Scheda Non Conformità</dhv:label></strong>
      </th>
	</tr>
  ...
  <dhv:evaluate if="<%= SiteIdList.size() <= 1 %>">
    ...
    <td>
	  ...
	  <%@ include file="../nonconformita/opu_provvedimenti_edit.jsp" %>
