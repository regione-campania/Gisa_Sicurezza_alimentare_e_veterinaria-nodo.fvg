...
<%-- 			<td headers="ente3">  <%=(trasgr.getEnte3()!=null) ? trasgr.getEnte3() : "" %> &nbsp; </td> --%>
  ...
  <table cellpadding="4" cellspacing="0" border="0" width="100%" class="details">
    <thead>
  <tr> <th colspan="11"> <center>REGISTRO SANZIONI AMMINISTRATIVE ANNO <%= anno %></center> </th></tr>
			...
			<th id="dataultimanotifica"><strong>Data ultima notifica</strong></th>
			<th id="pvoblato"><strong>PV oblato in misura ridotta</strong></th>
			<th id="versato1"><strong>Importo effettivamente introitato (1)</strong></th>
			<th id="datapagamento"><strong>Data pagamento</strong></th>
			<th id="ridottoconsentito"><strong> Pagamento effettuato nei termini</strong></th>
	...
	if (listaTrasgressioni.size()>0){
		for (int i=0;i<listaTrasgressioni.size(); i++){
			...
			<td headers="dataultimanotifica">  <%=(trasgr.getDataUltimaNotifica()!=null) ?  toDateasString(trasgr.getDataUltimaNotifica()) : "" %> </td>
			<td headers="oblato"> <% if (trasgr.isCompetenzaRegionale() && trasgr.isPvOblato()){ %> X <%} %> &nbsp; </td>
			<td headers="versato1">  <% if (trasgr.isCompetenzaRegionale()) { %> <%=trasgr.getImportoEffettivamenteVersato1()%> &#x20ac; <%} %> &nbsp; </td>
			<td headers="datapagamento">  <%=(trasgr.getDataPagamento()!=null) ?  toDateasString(trasgr.getDataPagamento()) : "" %> </td>
			<td headers="ridottoconsentito"> <% if (trasgr.isPagamentoRidottoConsentito()){ %> X <%} %> &nbsp; </td>
