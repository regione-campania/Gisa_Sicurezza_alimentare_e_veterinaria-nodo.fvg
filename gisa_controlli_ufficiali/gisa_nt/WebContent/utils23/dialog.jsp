   ...
   <title><%=Dialog.getTitle()%></title>
    <frameset rows="50,100%,50" frameborder="0">
        <% if ("true".equals((String)getServletConfig().getServletContext().getAttribute("ForceSSL"))) { %>
          <frame name="topframe" src="https://<%= getServerUrl(request) %>/utils23/loadframes.jsp">
          <frame marginheight="0" name="middleframe" src="https://<%= request.getServerName() %><%= request.getContextPath() %>/utils23/loadmiddleframe.jsp">
          <frame name="bottomframe" src="https://<%= getServerUrl(request) %>/utils23/loadbottomframe.jsp">
        <%} else {%>
          <frame name="topframe" src="utils23/loadframes.jsp">
          <frame marginheight="0" name="middleframe" src="utils23/loadmiddleframe.jsp">
          <frame name="bottomframe" src="utils23/loadbottomframe.jsp">
        <%}%>
    </frameset>
  </html>
<%}
