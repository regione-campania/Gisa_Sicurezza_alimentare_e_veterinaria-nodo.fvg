...
function checkForm()
{
	    ...
	    <%
		//Attivazione flusso 356
				if(ApplicationProperties.getProperty("flusso356").equals("true")){
		if(Operatore.getIdTipoAttivita()==2)
		{
		%>
		if ((document.getElementById("capacita").value) == "" || (document.getElementById("capacita").value)<10){
	        message += "- Capacità strutturale richiesta(non puo' essere inferiore di 10)\r\n";
	        formTest = false;
	     }
	    ...
	    if (parseInt(form.numAlveari.value)<0){
	        message += "- Num alveari non puo' essere minore di 0\r\n";
	        formTest = false;
	     }
	    if (parseInt(form.numSciami.value)<0){
	        message += "- Num sciami non puo' essere minore di 0\r\n";
	        formTest = false;
	     }
	    ...
	    if (parseInt(form.numAlveari.value) > parseInt(document.getElementById("capacita").value)){
	        message += "- Numero alveari non può essere maggiore della capacità strutturale\r\n";
	        formTest = false;
	     }
	    <%}else{%>
	    if (parseInt(form.numAlveari.value) > parseInt(document.getElementById("capacita").value)){
	    ...
	    if(ApplicationProperties.getProperty("flusso356_2").equals("true")){
			...
			console.log('ca: '+<%=Operatore.getCapacita()%>+'')
			console.log('alv: '+<%=Operatore.getSommaAlveari()%>+form.numAlveari.value)
			console.log('cond: '+(<%=Operatore.getSommaAlveari()%>+form.numAlveari.value)>(<%=Operatore.getCapacita()%>)+'')

			if(<%=Operatore.getCapacita()%> == 0){
				if((<%=Operatore.getSommaAlveari()%>+parseInt(form.numAlveari.value))>document.getElementById("capacita").value)
				{
					if (confirm("- Numero di alveari superiore alla capacita' strutturale,i valori di capacita' e numero alveari saranno aggiornati e controllati in fase di censimento, Sei sicuro di voler continuare?") == true) {
						} else {
						  return false;
						}
				}	
			...
			if((<%=Operatore.getSommaAlveari()%>+parseInt(form.numAlveari.value))>(<%=Operatore.getCapacita()%>))
				{
				if (confirm("- Numero di alveari superiore alla capacita' strutturale,i valori di capacita' e numero alveari saranno aggiornati e controllati in fase di censimento, Sei sicuro di voler continuare?") == true) {
				} else {
				  return false;
...
<input type="hidden" name="opId" id="opId" value="<%=Operatore.getIdOperatore()%>">
	...
	if(ApplicationProperties.getProperty("flusso356_2").equals("true")){
	%>
	<div id="datiAtt" style="display:none">
	<fieldset>
		...
		class="details">
		 <tr>
    <th colspan="2">
      <strong>DATI MANCANTI ATTIVITA</strong>
    </th>
  </tr>
		...
		<tr>
			...
			<td>
			<input type="number" name = "capacita" id="capacita" value="" required disabled  min="11"><font color = "red">*</font>
			</td>
		</tr>
	</table>
	</fieldset>
   ...
   <%
 	//Attivazione flusso 356
		...
		%>
		<tr>
			<td nowrap class="formLabel">Capacità strutturale</td>
			<td>
			<input type = "number" name = "capacita" id = "capacita" style="width: 50px;" value="10" hidden>
   ...
   %>
		<tr>
			<td nowrap class="formLabel">Capacità strutturale</td>
			<td>
			<input type = "number" name = "capacita" id = "capacita" style="width: 50px;" value="11" min="11">
			<font color = "red">*</font>
			</td>
		</tr>
   <%
...
<table cellpadding="4" cellspacing="0" border="0" width="100%"
	...
	<tr>
    		...
    		<td>
      			<input type="email" size="70" name="domicilioDigitale" id="domicilioDigitale" pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,4}$" required>    
    		</td>
  		</tr>
  		...
  		<tr>
    		...
    		<td>
      			<input type="text" size="70" name="telefono1"  id="telefono1Detentore">    
    		</td>
  		</tr>
  		...
  		<tr>
    		...
    		<td>
      			<input type="text" size="70" name="telefono2"  id="telefono2">    
    		</td>
  		</tr>
</table>
</form>
...
$("#addpersona").submit(function(e)
		{
	if(document.getElementById('domicilioDigitale').value=='')
	{
		alert("Valorizzare domicilio digitale");	
	}
	else if(trim(document.getElementById('telefono1Detentore').value)=='')
	{
		alert("Valorizzare telefono");	
	}else{
...
if(ApplicationProperties.getProperty("flusso356_2").equals("true")){
%>
if(<%=Operatore.getIdTipoAttivita()%>==1 && <%=Operatore.getCapacita()%>==0){
alert("ATTENZIONE, attivita' senza capacita' strutturale, si prega di inserire la capacita' strutturale dell'impresa.")
document.getElementById("datiAtt").style.display="block";
document.getElementById("capacita").removeAttribute("disabled");
}
<%}%>
