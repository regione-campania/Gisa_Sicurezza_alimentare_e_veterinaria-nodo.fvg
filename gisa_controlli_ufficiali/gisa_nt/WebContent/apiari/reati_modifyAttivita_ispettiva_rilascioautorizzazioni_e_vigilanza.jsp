...
<body>
<form name="details" action="ApicolturaApiariReati.do?command=UpdateTicket&auto-populate=true<%= addLinkParams(request, "popup|popupType|actionId") %>" onSubmit="return checkForm(this);" method="post">
...
<%} %>  <% if (request.getParameter("return") == null) {%>
  <a href="ApicolturaApiariReati.do?command=TicketDetails&id=<%=TicketDetails.getId()%>"><dhv:label name="reati.dettagli">Scheda note di reato</dhv:label></a> >
  <%}%>
...
<%-- <%@ include file="../controlliufficiali/opu_reati_modify.jsp"%>--%>
 ...
 <dhv:evaluate if="<%= !TicketDetails.isTrashed() %>" >
      <dhv:evaluate if="<%= TicketDetails.getClosed() != null %>" >
            <input type="submit" value="<dhv:label name="button.reopen">Reopen</dhv:label>" onClick="javascript:this.form.action='ApicolturaApiariReati.do?command=ReopenTicket&id=<%=TicketDetails.getId()%><%= addLinkParams(request, "popup|popupType|actionId") %>'">
           <dhv:evaluate if='<%= !"list".equals(request.getParameter("return"))%>' >
          <input type="submit" value="Annulla" onClick="javascript:this.form.action='ApicolturaApiariReati.do?command=TicketDetails&id=<%= TicketDetails.getId()%>&idNodo=null&idC=<%= TicketDetails.getIdControlloUfficiale()%>&orgId=-1&idNC=<%=request.getAttribute("idNC")%>&stabId=<%=OrgDetails.getIdStabilimento()%><%= addLinkParams(request, "popup|popupType|actionId") %>'" />
           </dhv:evaluate>
      </dhv:evaluate>
      <dhv:evaluate if="<%= TicketDetails.getClosed() == null %>" >
            <input type="submit" value="<dhv:label name="global.button.update">Update</dhv:label>" onClick="return checkForm(this.form)" />
           <dhv:evaluate if='<%= !"list".equals(request.getParameter("return"))%>' >
            <input type="submit" value="Annulla" onClick="javascript:this.form.action='ApicolturaApiariReati.do?command=TicketDetails&id=<%= TicketDetails.getId()%>&idNodo=null&idC=<%= TicketDetails.getIdControlloUfficiale()%>&orgId=-1&idNC=<%=request.getAttribute("idNC")%>&stabId=<%=TicketDetails.getIdApiario()%><%= addLinkParams(request, "popup|popupType|actionId") %>'" />
 ...
 <dhv:include name="organization.source" none="true">
  ...
  <tr class="containerBody">
   ...
   <dhv:evaluate if="<%= !TicketDetails.isTrashed() %>" >
    ...
    <% }%>
    <input type="hidden" name="idApiario" id="idApiario" value="<%=  TicketDetails.getIdApiario() %>" />
