...
<jsp:useBean id="ListaMovimentazioni"
	class="ext.aspcfs.modules.apiari.base.MovimentazioniList" scope="request" />

<jsp:useBean id="LookupTipoMovimentazione"
	class="org.aspcfs.utils.web.LookupList" scope="request" />

<jsp:useBean id="SiteIdList"
	class="org.aspcfs.utils.web.LookupList" scope="request" />
...
<script>
function openPopupModulesPdf(idMovimentazione){
	var res;
	var result;
		window.open('ApicolturaMovimentazioni.do?command=DetailsMovimentazioni&idMovimentazione='+idMovimentazione,'popupSelect',
...
<th
						...
						data-placeholder="FILTRO PER ASL"
						class="first-name filter-select"><div
							class="tablesorter-header-inner">ASL</th>
						<th
						aria-label="Tipo attivita ( filter-match ): No sort applied, activate to apply an ascending sort"
						aria-sort="none" style="-moz-user-select: none;" unselectable="on"
						aria-controls="table" aria-disabled="false" role="columnheader"
						scope="col" tabindex="0" data-column="1"
						data-placeholder="FILTRO TIPO MOVIMENTAZIONE"
						class="first-name filter-select"><div
							class="tablesorter-header-inner">TIPO MOVIMENTAZIONE</th>
						...
						class="filter-false tablesorter-header"><div
							class="tablesorter-header-inner">DATA MOVIMENTAZIONE</div></th>
					...
					<th
						...
						class="filter-match tablesorter-header tablesorter-headerUnSorted"><div
							class="tablesorter-header-inner">CODICE AZIENDA ORIGINE</div></th>
					...
					<th
						...
						class="filter-match tablesorter-header tablesorter-headerUnSorted"><div
							class="tablesorter-header-inner">PROG. APIARIO ORIGINE</th>
					<th
						aria-label="Codice Azienda: No sort applied, activate to apply an ascending sort"
						aria-sort="none" style="-moz-user-select: none;" unselectable="on"
						aria-controls="table" aria-disabled="false" role="columnheader"
						scope="col" tabindex="0" data-column="0"
						class="filter-false tablesorter-header"><div
							class="tablesorter-header-inner">NUM. ALVEARI ORIGINE</div></th>
							<th
						aria-label="Codice Azienda: No sort applied, activate to apply an ascending sort"
						aria-sort="none" style="-moz-user-select: none;" unselectable="on"
						aria-controls="table" aria-disabled="false" role="columnheader"
						scope="col" tabindex="0" data-column="0"
						class="filter-false tablesorter-header"><div
							class="tablesorter-header-inner">NUM ALVEARI SPOSTATI</div></th>
					...
					<th
						...
						class="filter-match tablesorter-header tablesorter-headerUnSorted"><div
							class="tablesorter-header-inner">CODICE AZIENDA DESTINAZIONE</th>
					<th
						...
						class="filter-match tablesorter-header tablesorter-headerUnSorted"><div
							class="tablesorter-header-inner">PROG. APIARIO DESTINAZIONE</th>
				<th
						aria-label="Codice Azienda: No sort applied, activate to apply an ascending sort"
						aria-sort="none" style="-moz-user-select: none;" unselectable="on"
						aria-controls="table" aria-disabled="false" role="columnheader"
						scope="col" tabindex="0" data-column="0"
						class="filter-false tablesorter-header"><div
							class="tablesorter-header-inner">NUM ALVEARI DESTINAZIONE</div></th>
			...
			<tbody aria-relevant="all" aria-live="polite">
				...
				<%
					if (ListaMovimentazioni.size() > 0) {
						Iterator<ModelloC> itOperatori = ListaMovimentazioni.iterator();
						while (itOperatori.hasNext()) {
							ModelloC thisOperatore = itOperatori.next();
							
				%>
				<tr>
					<td><%=toHtml2(SiteIdList.getSelectedValue(thisOperatore.getApiarioOrigine().getIdAsl()))%></td>
					<td><%=toHtml2(LookupTipoMovimentazione.getSelectedValue(thisOperatore.getIdTipoMovimentazione())) %></td>
					<td><%=toDateasString(thisOperatore.getDataMovimentazione())%></td>
					<td><%=toHtml2(thisOperatore.getCodiceAziendaOrigine())%></td>
					<td><%=toHtml2(thisOperatore.getProgressivoApiarioOrigine())%></td>
					<td><%=toHtml2(thisOperatore.getNumApiariOrigine()+"")%></td>
					<td><%=toHtml2(thisOperatore.getNumApiariSpostati()+"")%></td>
					<td><%=toHtml2(thisOperatore.getCodiceAziendaDestinazione())%></td>
					<td><%=toHtml2(thisOperatore.getProgressivoApiarioDestinazione())%></td>
					<td><%=toHtml2(thisOperatore.getNumApiariDestinazione()+"")%></td>


					<td width="10%">
						<a href="#" onclick="javascript:openPopupModulesPdf(<%=thisOperatore.getId() %>)">Apri Dettaglio</a>
						<br>
<%
						if(thisOperatore.getApiarioOrigine().getIdAsl()==User.getSiteId() || User.getSiteId()==-1)
						{
							if(thisOperatore.getErrore_sincronizzazione()!=null)
							{
%>
								Validazione non consentita. Rivolgersi all'Help Desk per risolvere l'errore: <%=thisOperatore.getErrore_sincronizzazione()%>				
<%
							}
							else
							{
%>						
								<button onclick="loadModalWindow();location.href='ApicolturaMovimentazioni.do?command=ValidaMovimentazione&idMovimentazione=<%=thisOperatore.getId()%>'">VALIDA E INVIA ALLA BDN</button>
<%
							}
						}
%>					
...
<div class="pager">
	...
	<div id="searchAzienda">
		...
		<form method="post" action="ApicolturaAttivita.do?command=AssegnaNumero" name="generanumero">
			...
			<td width="50%">	<input type="text" value=""   name="searchAziendaField1" id="searchAziendaField1"   size="8" maxlength="8"   placeholder="">  </td>
			...
			<input type="text" name="searchAziendaField2" id="searchAziendaField2"  size="60  class="required" readonly="readonly" placeholder="CLICCA PER RICERCARE CODICE DISPONIBILE">  </td>
						 ...
						 <input type="text" name="searchAziendaField" id ="searchAziendaField" value="">
