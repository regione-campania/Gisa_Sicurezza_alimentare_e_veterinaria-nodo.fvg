...
<jsp:useBean id="idApiarioScelto" class="java.lang.String" scope="request"/>
...
<!--   <link rel="stylesheet" type="text/css" media="screen" href="http://combogrid.justmybit.com/resources/css/smoothness/jquery-ui-1.10.1.custom.css"/> -->
  ...
  function checkForm()
  {
	  ...
	  formTest = true;
	    ...
	    if (document.addmovimentazione.comuneDestinazione!=null && document.addmovimentazione.comuneDestinazione.value=='' )
		  {
		  message += "- Comune di destinazione richiesto\r\n";
	        formTest = false;
		  }
	    if (document.addmovimentazione.idImpresaIn!=null && (document.addmovimentazione.idImpresaIn.value=='' || document.addmovimentazione.idImpresaIn.value=='-1') )
		  {
		  message += "- Azienda di destinazione richiesta (scrivere il codice azienda nel campo apposito e selezionarlo dalla lista che sarà mostrata)\r\n";
	        formTest = false;
		  }
  ...
  $( document ).ready(function() {
	  ...
	  function selectApiarioDestinazione(progressivo,idApiario,comuneUbicazione,comuneIstat,provinciaUbicazione,indirizzoApiario,detentore,cfDetentore,numAlveari,latitudine,longitudine)
	  {
  	 ...
  	 $( "#codiceApiarioDestinazione" ).combogrid({
		  ...
		  success :function( event, ui ) {alert(ui);},
				  select: function( event, ui ) {
					  ...
					  if(ui.item.id>0)
					  {
					  		selectApiarioDestinazione(ui.item.progressivo,ui.item.id,ui.item.comune,ui.item.comuneIstat,ui.item.provincia,ui.item.indirizzo,ui.item.detentore,ui.item.cfDetentore,ui.item.numAlveari,ui.item.latitudine,ui.item.longitudine);
			 		  }
					  else
					  {
  ...
  function controlloDataMovimentazione(dataMovimentazione,validazioneAsl)
  {
	  ...
	  if(validazioneAsl && dataMovimentazione!='')
	  {
		    if(giorni_differenza(dataOdierna,dataMovimentazione)<2) 
		    {
  			...
  			var limite_giorni = 7;
  			if(false)
  			//if(giorni_differenza(dataMovimentazione, currentDate.value) > limite_giorni )
  			{
  				alert("La data di movimentazione non può essere più vecchia di 7 giorni");
  				controlloDate = false;
  			}
  }
...
<a href="ApicolturaMovimentazioni.do?command=Search&stabId=<%=idApiarioScelto%>&opId=<%=Operatore.getIdOperatore()%>&searchcodeidApiario=<%=idApiarioScelto%>&searchcodeidAzienda=<%=Operatore.getIdOperatore()%>&searchcodeCodiceAziendaSearch=<%=Operatore.getCodiceAzienda()%>&searchcodeProgressivoApiarioSearch=<%=ApiarioOrigine.getProgressivoBDA()%>">MOVIMENTAZIONI </a> >
<a href="ApicolturaMovimentazioni.do?command=AddRichiesta&tipoMovimentazione=Compravendita&idApiarioScelto=<%=idApiarioScelto%>">NUOVA MOVIMENTAZIONE</a> >
COMPRAVENDITA
</td>
</tr>
</table>
...
%>				
			<input type="hidden" name="datacorrente" id="datacorrente" value="<%=sdf.format(new Date()) %>"/>
			</td>
		</tr>
