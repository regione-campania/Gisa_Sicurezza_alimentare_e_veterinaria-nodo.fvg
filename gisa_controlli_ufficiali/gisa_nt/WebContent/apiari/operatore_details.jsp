...
$( document ).ready( function(){
	calenda('data_modifica_titolare','','0');
	calenda('data_assegnazione_detentore','','0');
	calenda('data_nascita_nuovo_proprietario','','-18y');
	calenda('data_cessazione_apiario','','0');
	calenda('data_assegnazione_ubicazione','','0');
	calenda('dataCessazione','','0');
...
$(function () {
	 $( "#dialogProprietario" ).dialog({
	        ...
	        buttons:{
	        	 ...
	        	 "Esci" : function() {
         			...
         			document.getElementById('nominativoDetNome').style.display="";
         			document.getElementById('nominativoDetCognome').style.display="";
	 ...
	 function checkFormDetentore()
	 {
	 	   ...
	 	   if (form.nomeDet.value == ""){
	 	        message += "- Nome richiesto\r\n";
	 	        formTest = false;
	 	     }
	 	   
	 	  if (form.cognomeDet.value == ""){
	 ...
	 function checkFormCensimento()
	 {
	 	   ...
	 	   %>
	 	   if (parseInt(form.capacita.value)<parseInt(form.numAlveari.value)){
	 		   console.log("CAP: "+document.addcensimento.capacita.value)
	 		   	 		   console.log("ALV: "+form.numAlveari.value)

	 		  message += "-il Num Alveari non puo' essere maggiore della capacita' strutturale\r\n";
	 	        formTest = false;
	 		   
	 	   }
	 	   
	 	   
	 	  if (parseInt(form.capacita.value)<11 && form.tipo.value=="1"){
	 		   console.log("CAP: "+document.addcensimento.capacita.value)
	 		   	 		   console.log("ALV: "+form.numAlveari.value)

	 		  message += "- La capacita' strutturale non può essere inferiore a 11\r\n";
	 	        formTest = false;
	 	   ...
	 	   <%}%>
			 ...
			 var numero = parseInt(document.getElementById("numAlveariCensimento").value) - parseInt(document.getElementById("numAlveariPre").value)
				
				var somma = parseInt(<%=OperatoreDettagli.getSommaAlveari()%>) + parseInt(numero)
				
			
				
			 
				 if(<%=OperatoreDettagli.getIdTipoAttivita()%>==1){
					 if(somma>document.getElementById("capacita").value){
					 message += "- Numero di alveari superiore alla capacita' strutturale\r\n";
					 console.log("CAP1: "+document.getElementById("capacita").value )
					 console.log("SUM1: "+somma )

			        	formTest = false;
					 }
					}else{
						if(somma><%=OperatoreDettagli.getCapacita()%>){
							 console.log("CAP2: "+<%=OperatoreDettagli.getCapacita()%> )
...
function cambioDetentore(jsonImpresa)
{
	...
	$('#data_assegnazione_detentore').datepicker('hide');
	$('#data_assegnazione_detentore').blur();
...
function nuovocensimento(jsonImpresa)
{
	...
	<%if(ApplicationProperties.getProperty("flusso356").equals("true")){
	 	   %>
	document.addcensimento.capacita.value=json.capacita;
	
	
	if(parseInt(json.capacita)>10){
		document.getElementById("capacita").removeAttribute("readonly");
		document.addcensimento.tipo.value="1";

	}else{
		document.getElementById("capacita").setAttribute("readonly","readonly");
	...
	<%if(ApplicationProperties.getProperty("flusso356_2").equals("true")){
	 	   %>
	document.addcensimento.numAlveariPre.value=json.alveari;
...
function cessazioneApiario(jsonImpresa)
{
	...
	$('#data_cessazione_apiario').datepicker('hide');
	$('#data_cessazione_apiario').blur();
...
<script>
		console.log('SOMMAAA: '+<%=OperatoreDettagli.getSommaAlveari()%>+'')
		</script>
...
if(User.getRoleId()==org.aspcfs.modules.admin.base.Role.RUOLO_DELEGATO)
	codiceFiscaleCompare = User.getSoggetto().getCodFiscale();
else
	codiceFiscaleCompare = User.getContact().getVisibilitaDelega();
if (codiceFiscaleCompare.equalsIgnoreCase(OperatoreDettagli.getCodFiscale())) { %>
...
<input type ="hidden" name = "idStabilimento" id="idStabilimentoCensimento" value = "" />
	...
	<tr>
		...
		<td>
					...
					<input type ="hidden" id="data_assegnazione_censimento" name="data_assegnazione_censimento" readonly="readonly" value="<%=(String)request.getAttribute("dataCensimentoDaAssegnare")%>" required="required">
	...
	<tr>
				<td nowrap class="formLabel">Capacita Strutturale</td>
				<td>
				
				<input type = "text" name = "capacita"  id="capacita" required="required" readonly>
	...
	<%if(OperatoreDettagli.getIdTipoAttivita()==1){
		...
		<tr>
					...
					<input type = "text" name = "capacita"  id="capacita" required="required" value="<%=OperatoreDettagli.getCapacita()%>">
					</td>
	...
	<tr>
				...
				<td>
				<input type ="hidden" name = "numAlveariPre" id="numAlveariPre" value = "" />
				
				<input type = "text" name = "numAlveari" id="numAlveariCensimento" required="required" >
				</td>
...
<input type ="hidden" name = "idStabilimento" id = "idStabilimentoCessazioneApiario" value = "" />
	...
	<tr>
		...
		<td>
			<input type ="text" name="data_cessazione_apiario" id="data_cessazione_apiario" class="date_picker" value="" required="required">
		</td>
	</tr>
			</table>
		...
		<form name="modificaTitolareForm" id="modificaTitolareForm" action="ApicolturaAttivita.do?command=ModificaTitolare&autopopulate=true" method="POST">
			<table cellpadding="4" cellspacing="0" border="0" width="100%" class="details">
				...
				<tr>
					...
					<td>
						<input type ="text" class="date_picker" id="data_modifica_titolare" name="data_modifica_titolare" value="" required="required" tabindex="-1">
					</td>
				</tr>
				<tr>
					<th colspan="2">
				...
				<tr>
					...
					<td>
						<input type="text" size="70" name="dataNascita" class="date_picker" id="data_nascita_nuovo_proprietario" class="required" placeholder="dd/MM/YYYY">
					</td>
				</tr>
				<tr>
					<td nowrap class="formLabel">
				...
				<tr>
					<td nowrap class="formLabel">
						<label for="nazioneN-2">Nazione Sede Legale *</label>
					</td>
					<td>
...
<font color="red">ATTENZIONE! Il mittente di ogni movimentazione specificata nel file deve essere uguale all'azienda per la quale si sta lavorando: <b><%=OperatoreDettagli.getCodiceAzienda()%></b></font>
<table cellpadding="4" cellspacing="0" border="0" width="100%"
		class="details">
...
<input type ="hidden" name = "indirizzoOld" value = "" />
	...
	<tr>
		...
		<td>
					...
					<input type ="text" name="data_assegnazione_ubicazione" id="data_assegnazione_ubicazione" class="date_picker" value="" required="required">
...
<table cellpadding="4" cellspacing="0" border="0" width="100%"
		...
		<tr>
				...
				<td><input type ="text" name="dataCessazione" id="dataCessazione" value="" class="date_picker">
...
<table cellpadding="4" cellspacing="0" border="0" width="100%"
		...
		<tr>
				...
				<td><input type ="text" class="date_picker" name="data_assegnazione_detentore" id="data_assegnazione_detentore" value="">
		...
		<tr>
			...
			<tr>
				...
				<td><input type="text" name="codFiscaleSoggetto"  placeholder="RICERCA DIGITANDO ILCODICE FISCALE"
					...
					onclick="javascript:CalcolaCF(document.addpersona.sesso,document.getElementById('nomedetentore'),document.getElementById('cognomedetentore'),document.addpersona.comuneNascita,document.addpersona.dataNascita,'codFiscaleSoggettoAddinput')"></input>
								
				</td>
				
			</tr>
			
			<tr id ="nominativoDetNome">
				<td nowrap class="formLabel">Nome</td>
				<td><input type="text" size="70" id="nomeDet" name="nome" class="required">
			...
			<tr id ="nominativoDetCognome">
				<td nowrap class="formLabel">Cognome</td>
				<td><input type="text" size="70" id="cognomeDet" name="cognome" class="required">
				<font color="red">*</font>
				</td>
	...
	<table cellpadding="4" cellspacing="0" border="0" width="100%"
		...
		<tr>
				<td nowrap class="formLabel">Nome </td>
				<td><input type="text" size="70" id="nomedetentore" name="nome" class="required">
			...
			<tr >
				...
				<td><input type="text" size="70" id="cognomedetentore" name="cognome"
					class="required">
			...
			<tr >
				...
				<td><input type="text" size="70" name="dataNascita" class="date_picker"
...
<tr>
	...
	<td>
<%
		String via =    OperatoreDettagli.getListaStabilimenti().getLaboratorio().getSedeOperativa().getVia();
		String civico = ", " + OperatoreDettagli.getListaStabilimenti().getLaboratorio().getSedeOperativa().getCivico();
		if(civico==null)
			civico = "";
		out.println(via + civico);
...
{ 
	if (User.getRoleId()==Role.RUOLO_APICOLTORE && User.getContact().getVisibilitaDelega().equalsIgnoreCase(OperatoreDettagli.getCodFiscale())) 
	{ 
%>
...
<table  class="tablesorter">
	...
	<thead>
		<tr class="tablesorter-headerRow" role="row">
			...
			<th aria-label="DATA CESSAZIONE No sort applied, activate to apply an ascending sort" aria-sort="none" style="-moz-user-select: none;" unselectable="on" aria-controls="table" aria-disabled="false" role="columnheader" scope="col" tabindex="0" data-column="0" class="filter-false tablesorter-header"><div class="tablesorter-header-inner">DATA CESSAZIONE</div></th>
...
<%=thisStab.isSincronizzatoBdn()==true ? "<img src=\"images/verde.gif\">" : "<img src=\"images/rosso.gif\">" %>			
			...
			if(OperatoreDettagli.getIdAsl()!=16 && (OperatoreDettagli.getCodFiscale().equals(User.getUserRecord().getContact().getCodiceFiscale()) || OperatoreDettagli.getCodFiscale().equals(User.getSoggetto().getCodFiscale()) || OperatoreDettagli.getCodFiscale().equalsIgnoreCase(User.getContact().getVisibilitaDelega().trim()) || User.getUserRecord().getRoleId()==270)) 
			{
...
%>
				...
				<button <%=disabled %>  <%=title %> onclick="javascript: nuovocensimento('<%=thisStab.toString().replaceAll("'"," ").replaceAll("\""," ")%>')">CENSIMENTI<%=suffissoLabelTastoCensimenti%></button>
				<!-- /dhv:permission -->
<%
			}
%>
