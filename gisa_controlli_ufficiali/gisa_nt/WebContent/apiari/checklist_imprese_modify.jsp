...
<form name="addAccountAudit"  method="post" action="<%=OrgDetails.getAction() %>CheckList.do?command=Update&auto-populate=true" onSubmit="return checkForm();" >
  ...
  <input type="button" value="Annulla" onClick="window.location.href='<%=OrgDetails.getAction() %>Vigilanza.do?command=TicketDetails&id=<%= request.getAttribute("idCon")%>&stabId=<%=OrgDetails.getIdStabilimento()%>';this.form.dosubmit.value='false';" />
