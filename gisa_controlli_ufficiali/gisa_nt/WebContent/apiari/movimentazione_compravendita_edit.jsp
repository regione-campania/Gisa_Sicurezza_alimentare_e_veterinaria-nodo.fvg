...
<!--   <link rel="stylesheet" type="text/css" media="screen" href="http://combogrid.justmybit.com/resources/css/smoothness/jquery-ui-1.10.1.custom.css"/> -->
  ...
  if(OperatoreDestinazione.getIdOperatore()<=0)
  {
	  OperatoreDestinazione = ModelloC.getApiarioDestinazione().getOperatore();
  }
  
  boolean comuneDestinazione = false;
  %>
  
  
  
  function PopolaListaComuni()
  {
  	PopolaCombo.getValoriComboComuni1Provincia(-1,{callback:PopolaListaComuniCallback,async:false}) ;
  ...
  function PopolaListaComuniCallback(value)
  {
  	...
  	var select = "<select id=\"comuneDestinazione\" name=\"comuneDestinazione\">";
  	select= select+ "<option value=\"\">&lt;--- Selezionare una voce ---&gt;</option>";
  	...
  	sortSelect(document.getElementById("comuneDestinazione"));
  }
  ...
  function checkForm()
  {
	  ...
	  formTest = true;
	    ...
	    form = document.editmovimentazione ;
	    ...
	    if (document.editmovimentazione.comuneDestinazione!=null && document.editmovimentazione.comuneDestinazione.value=='' )
		  {
		  message += "- Comune di destinazione richiesto\r\n";
	        formTest = false;
		  }
  ...
  $( document ).ready(function() {
	  ...
	  function selectApiarioDestinazione(progressivo,idApiario,comuneUbicazione,provinciaUbicazione,comuneIstat,indirizzoApiario,detentore,cfDetentore,numAlveari,latitudine,longitudine)
	  {
	  	...
	  	document.getElementById("comuneIstat").value=comuneIstat;
  	 ...
  	 $( "#codiceApiarioDestinazione" ).combogrid({
		  ...
		  success :function( event, ui ) {alert(ui);},
				  select: function( event, ui ) {
					  ...
					  if(ui.item.id>0)
					  {
						  selectApiarioDestinazione(ui.item.progressivo,ui.item.id,ui.item.comune,ui.item.provincia,ui.item.comuneIstat,ui.item.indirizzo,ui.item.detentore,ui.item.cfDetentore,ui.item.numAlveari,ui.item.latitudine,ui.item.longitudine);
			 		  }
					  else
					  {
  ...
  function svuotaDestinazione()
  {
		...
		document.getElementById("idApiarioIn").value="";
	  	...
	  	document.getElementById("comuneIstat").value="";
...
<input type="hidden" name="numero_modello" id="numero_modello" value="<%=ModelloC.getNumero_modello()%>">
    ...
    <th colspan="2">
      <strong>AZIENDA</strong>
		...
		<tr>
			...
			<td>
			<input type= "text" id = "dataMovimentazione" name = "dataMovimentazione" value="<%=toDateasString(ModelloC.getDataMovimentazione())%>" required onchange="controlloDataMovimentazione(document.getElementById('dataMovimentazione').value,document.getElementById('richiestaValidazione').checked)" />
		...
		<tr>
			...
			<td>
				...
				%>
				type="checkbox" name="richiestaValidazione" id="richiestaValidazione" onclick="controlloDataMovimentazione(document.getElementById('dataMovimentazione').value,document.getElementById('richiestaValidazione').checked)"  ><i>Per le destinazioni in regione la validazione non sarà in nessun caso necessaria</i>
			</td>
		</tr>
   ...
   <tr><th colspan="2"><strong>APIARIO DA MOVIMENTARE</strong></th></tr>
    ...
    <tr><td nowrap class="formLabel">Numero Alveari Da Spostare</td><td><input type="number"  size="50"  value="<%=ModelloC.getNumAlveariDaSpostare() %>"       name="numAlveariDaSpostare"  id="numAlveariDaSpostare"></td></tr>
    <tr><td nowrap class="formLabel">Numero Sciami/Nuclei Da Spostare</td><td><input type="number"  size="50"  value="<%=ModelloC.getNumSciamiDaSpostare() %>" name="numSciamiDaSpostare"  id="numSciamiDaSpostare"></td></tr>
    <tr><td nowrap class="formLabel">Numero Pacchi Da Spostare</td><td><input type="number"  size="50"  value="<%=ModelloC.getNumPacchiDaSpostare() %>"        name="numPacchiDaSpostare"  id="numPacchiDaSpostare"></td></tr>
    <!-- tr><td nowrap class="formLabel">Numero Api Regine Da Spostare</td><td><input type="number"  size="50"  value="<!--%=ModelloC.getNumRegineDaSpostare() %>"    name="numRegineDaSpostare"  id="numRegineDaSpostare"></td></tr -->
  ...
  <tr><td nowrap class="formLabel">CERCA PER CODICE AZIENDA</td><td><input type="text"   size="50" min="5" onchange="svuotaDestinazione()" name="codiceAziendaDestinazione" value="<%= ModelloC.getCodiceAziendaDestinazione()%>" required id="codiceAziendaDestinazione" placeholder="DIGITARE IL CODICE AZIENDA"></td></tr>

<%
if(comuneDestinazione)
{
%>				 
				 <tr><td nowrap class="formLabel">COMUNE DI DESTINAZIONE</td><td><div id="divComune"></div></td></tr>
<%
}
%>	

				 <tr><td nowrap class="formLabel">RAGIONE SOCIALE</td><td><input type="hidden" id="idImpresaIn" name="idImpresaIn" required value=""><input type="text" value=""  size="50"  readonly="readonly"  id="ragioneSocialeIn"></td></tr>
				 ...
				 <tr><td nowrap class="formLabel">INDIRIZZO</td><td><input type="text"  size="50"  value="<%=ModelloC.getIndirizzo_dest()%>" readonly="readonly" name="indirizzoIn" id="indirizzoIn"></td></tr>	
				 <tr><td nowrap class="formLabel">COMUNE</td><td><input type="text"  size="50"  value="<%=ModelloC.getComune_dest()%>" readonly="readonly" name="comuneIn" required id="comuneIn"></td></tr>	
  ...
  <br>
   ...
   <tr><th colspan="2"><strong>APIARIO DI DESTINAZIONE</strong></th></tr>
				 ...
				 <input type="hidden" id="comuneIstat" name="comuneIstat" value="">
...
<script type="text/javascript">
		document.getElementById("ragioneSocialeIn").value='<%=OperatoreDestinazione.getRagioneSociale()%>';
		document.getElementById("nominativoIn").value='<%=(OperatoreDestinazione!=null && OperatoreDestinazione.getRappLegale()!=null)?(OperatoreDestinazione.getRappLegale().getNome() + " " + OperatoreDestinazione.getRappLegale().getCognome()):(ModelloC.getProprietarioDestinazione())%>';
		document.getElementById("codiceAziendaDestinazione").value='<%=OperatoreDestinazione.getCodiceAzienda()%>';
...
%>
		selectApiarioDestinazione('<%=ModelloC.getApiarioDestinazione().getProgressivoBDA()%>','<%=ModelloC.getApiarioDestinazione().getIdStabilimento()%>','<%=ModelloC.getApiarioDestinazione().getSedeOperativa().getDescrizioneComune()%>','<%=ModelloC.getApiarioDestinazione().getSedeOperativa().getSiglaProvincia()%>','<%=ModelloC.getApiarioDestinazione().getSedeOperativa().getCodiceIstatComune()%>','<%=ModelloC.getApiarioDestinazione().getSedeOperativa().getVia()%>','<%=ModelloC.getApiarioDestinazione().getDetentore().getCognome() + " " + ModelloC.getApiarioDestinazione().getDetentore().getNome()%>','<%=ModelloC.getApiarioDestinazione().getDetentore().getCodFiscale()%>','<%=ModelloC.getApiarioDestinazione().getNumAlveari()%>','<%=ModelloC.getApiarioDestinazione().getSedeOperativa().getLatitudine()%>','<%=ModelloC.getApiarioDestinazione().getSedeOperativa().getLongitudine()%>');
		document.getElementById('codiceApiarioDestinazione').value='<%=ModelloC.getApiarioDestinazione().getSedeOperativa().getDescrizioneComune()%>';
<%
	}		
%>	
...
%>	
			PopolaListaComuni();
			document.getElementById('comuneDestinazione').value=comuni['<%=ModelloC.getComune_dest()%>'] ;
<%
}
%>
