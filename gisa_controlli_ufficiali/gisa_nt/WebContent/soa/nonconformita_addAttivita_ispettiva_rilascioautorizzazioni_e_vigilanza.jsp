...
<form name="addticket" action="SoaNonConformita.do?command=Insert&auto-populate=true" method="post">
<%-- Trails --%>
...
<a href="Soa.do?command=ViewNonConformita&orgId=<%=OrgDetails.getOrgId() %>"><dhv:label name="nonconformita">NonConformita Amministrative</dhv:label></a> > 
...
<input type="submit" value="Annulla" onClick="javascript:this.form.action='Soa.do?command=ViewNonConformita'">
<br>
<dhv:formMessage />
...
<dhv:include name="ticket.subCat3" none="true">
  ...
  function checkForm(form){
   ...
   // if (form.cause.value == "" && form.NonConformitaAmministrative.value!=-1) {
    ...
    if (form.orgId.value == "-1") {
      message += label("check.nonconformita.richiedente.selezionato","- Controllare che il soa sia stato selezionato\r\n");
      formTest = false;
    }
    </dhv:include>
  ...
  function addNewContact(){
    <dhv:permission name="soa-soa-contacts-add">
      var acctPermission = true;
    </dhv:permission>
    <dhv:permission name="soa-soa-contacts-add" none="true">
...
<%--</dhv:evaluate>  --%>
  ...
  <tr>
    <td class="formLabel">
      <dhv:label name="nonconformita.richiedente">Soa</dhv:label>
    </td>
