...
<td>
  ...
  <a href="Soa.do?command=Details&orgId=<%=TicketDetails.getOrgId()%>"><dhv:label name="soa.details">Account Details</dhv:label></a> >
...
<dhv:container name="accountsvigilanza" selected="details" object="TicketDetails"
	...
	<dhv:permission name="vigilanza-vigilanza-delete">
		<input type="button"
			value="Ripristina"
			onClick="javascript:this.form.action='SoaVigilanza.do?command=Restore&id=<%= TicketDetails.getId()%>';submit();">
	</dhv:permission>
	<%
	...
	<%--<dhv:permission name="vigilanza-vigilanza-edit">
		<input type="button"
			...
			onClick="javascript:this.form.action='SoaVigilanza.do?command=ReopenTicket&id=<%= TicketDetails.getId()%><%= (defectCheck != null && !"".equals(defectCheck.trim())?"&defectCheck="+defectCheck:"") %>';submit();">
	</dhv:permission>--%>
	<%
		} else {
	%>
	<dhv:permission name="vigilanza-vigilanza-add">
		<input type="button"
			...
			onClick="javascript:this.form.action='SoaVigilanza.do?command=ModifyTicket&auto-populate=true<%= (defectCheck != null && !"".equals(defectCheck.trim())?"&defectCheck="+defectCheck:"") %>';submit();">
	</dhv:permission>
	<dhv:permission name="vigilanza-vigilanza-edit">
		<input type="button"
			value="Modifica"
	...
	<dhv:permission name="vigilanza-vigilanza-delete">
		...
		<input type="button"
			...
			onClick="javascript:popURL('SoaVigilanza.do?command=ConfirmDelete&id=<%= TicketDetails.getId() %>&orgId=<%=TicketDetails.getOrgId()%>&return=searchResults&popup=true', 'Delete_ticket','320','200','yes','no');">
		<%
			} else {
		%>
		<input type="button"
			value="<dhv:label name="global.button.delete">Delete</dhv:label>"
			onClick="javascript:popURL('SoaVigilanza.do?command=ConfirmDelete&id=<%= TicketDetails.getId() %>&orgId=<%=TicketDetails.getOrgId()%>&popup=true', 'Delete_ticket','320','200','yes','no');">
		<%
			}
		%>
	</dhv:permission>
	<dhv:permission name="vigilanza-vigilanza-edit">
		<input type="button"
			...
			onClick="javascript:this.form.action='SoaVigilanza.do?command=Chiudi&id=<%= TicketDetails.getId() %>';if( confirm('Sei sicuro di voler chiudere il campione?') ){submit()};">
	</dhv:permission>
	<%
		}
	%>
		...
		<table cellpadding="4" cellspacing="0" width="100%" class="details">
			...
			<tr class="containerBody">
				...
				<td><dhv:evaluate
					...
					<dhv:permission name="soa-soa-contacts-view">
						<a
					...
					<dhv:permission name="soa-soa-contacts-view" none="true">
