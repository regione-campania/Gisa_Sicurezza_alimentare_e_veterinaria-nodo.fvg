...
<form name="addAccountAudit"  method="post" action="CheckListSoa.do?command=Update&auto-populate=true&return<%= request.getParameter("return") %>" onSubmit="return checkForm();">
<%-- Trails --%>
<table class="trails" cellspacing="0">
<tr>
<td width="100%">
  <a href="Soa.do"><dhv:label name="accounts.accounts">Accounts</dhv:label></a> > 
  <a href="Soa.do?command=Search"><dhv:label name="accounts.SearchResults">Search Results</dhv:label></a> >
  <a href="Soa.do?command=Details&orgId=<%= OrgDetails.getOrgId() %>"><dhv:label name="accounts.details">Account Details</dhv:label></a> >
  ...
  <a href="CheckListSoa.do?command=View&id=<%= Audit.getId() %>"><dhv:label name="accounts.Audit">Check List</dhv:label></a> >
...
<dhv:container name="accounts" selected="audit" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId() %>'>
  ...
  <input type="button" value="Annulla" onClick="window.location.href='CheckListSoa.do?command=View&id=<%=Audit.getId()%>';this.form.dosubmit.value='false';" />
