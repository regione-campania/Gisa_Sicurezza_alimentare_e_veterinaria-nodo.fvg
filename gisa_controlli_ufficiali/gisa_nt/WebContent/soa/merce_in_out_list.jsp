...
<%@page import="org.aspcfs.modules.soa.base.MerceInOut"%>
...
<dhv:container name="soa" selected="merce_in_out" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId") %>'>
  <dhv:evaluate if="<%= !OrgDetails.isTrashed() %>">
    <dhv:permission name="soa-soa-merce_in_out-add"><a href="SoaMerceInOut.do?command=Add&orgId=<%= OrgDetails.getOrgId() %><%= addLinkParams(request, "popup|popupType|actionId") %>"><dhv:label name="merce_in_out.new">Nuova Merce in Ingresso/Uscita</dhv:label></a></dhv:permission><br/>
  </dhv:evaluate>
  ...
  %>
    <tr class="row<%= rowid %>">
      <td width="8" valign="top" nowrap>
        <a href="SoaMerceInOut.do?command=Details&id=<%=thisTic.getId() %>" >Scheda</a>
      </td>
