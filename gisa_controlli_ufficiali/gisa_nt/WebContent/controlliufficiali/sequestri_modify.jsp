<jsp:useBean id="CodiceArticolo" class="org.aspcfs.utils.web.LookupList" scope="request"/>
...
function checkForm(form) {
	...
	if(form.tipo_richiesta.value==""){
		message += "- Controllare che il campo \"numero verbale sequestro\" sia stato popolato \n";
	    formTest = false;
...
// 			 message += " - Controllare che il campo \"Valutazione del rischio n.c. gravi\" sia stato popolato\r\n";
// 		     formTest = false;
// 		}
// 	}
...
function abilitaTipoSequestro(){
	...
	if(document.getElementById("TipoSequestro").value == 1){
		...
		document.getElementById("codiceArticolo").value = "6";
		document.getElementById("articolo").value = "6";	
	}
	...
	//document.getElementById("seq").style.display="none";
		...
		document.getElementById("codiceArticolo").value = "5";
		document.getElementById("articolo").value = "5";		
		...
		if (document.getElementById("seq_sp")!=null)
			document.getElementById("seq_sp").style.display="block";
	//	document.getElementById("valutazione_rischio_gravi").disabled = true;
		document.getElementById("codiceArticolo").value = "1";
	...
	//Caso in cui è -1
		...
		if (document.getElementById("seq_a")!=null)
...
function abilitaTipoSequestro1(){
	if (document.forms['details'].SequestroDi!=null){
...
function abilitaTipoSequestro1_sp(){
	if (document.forms['details'].SequestroDi_sp!=null){
...
<dhv:evaluate if="<%= !TicketDetails.isTrashed() %>" >
      <dhv:evaluate if="<%= TicketDetails.getClosed() != null %>" >
          <dhv:permission name="<%=permission%>">
           ...
           <dhv:evaluate if='<%= !"list".equals(request.getParameter("return"))%>' >
            <input type="submit" value="Annulla" onClick="javascript:this.form.action='<%=url %>Sequestri.do?command=TicketDetails&id=<%= TicketDetails.getId() %><%= addLinkParams(request, "popup|popupType|actionId") %>'" />
           </dhv:evaluate>
      </dhv:evaluate>
      <dhv:evaluate if="<%= TicketDetails.getClosed() == null %>" >
          <dhv:permission name="<%=permission%>">
...
<tr class="containerBody">
  ...
  <tr class="containerBody">
    ...
    Riferimento Normativo
     ...
     <td><% CodiceArticolo.setJsEvent("disabled");%>
    <%=CodiceArticolo.getHtmlSelect("codiceArticolo", TicketDetails.getCodiceArticolo()) %>
...
<table cellpadding="4" cellspacing="0" width="100%" class="details" style="display: none">
  ...
  <tr class="containerBody">
    ...
    <td>
      	...
      	<input readonly type="text" id="estimatedResolutionDate" name="estimatedResolutionDate" size="10" value = "<%=toDateasString(TicketDetails.getEstimatedResolutionDate()) %>" />
