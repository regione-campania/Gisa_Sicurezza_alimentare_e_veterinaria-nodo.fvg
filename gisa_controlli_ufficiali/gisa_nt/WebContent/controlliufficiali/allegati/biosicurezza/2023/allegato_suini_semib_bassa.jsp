...
<link rel="stylesheet" type="text/css" media="screen" documentale_url="" href="controlliufficiali/allegati/biosicurezza/2023/css/screen.css" />
<link rel="stylesheet" type="text/css" media="print" documentale_url="" href="controlliufficiali/allegati/biosicurezza/2023/css/print.css" />
...
<tr>
<td align="center"></td>
<td align="center"><center><img documentale_url="" src="controlliufficiali/allegati/biosicurezza/2023/img/classyfarm.png"  height="100px"/></center></td>
<td align="center"></td>
</table>
</center>
...
<center>
<div class="header">
<b>VALUTAZIONE DELLA BIOSICUREZZA: SUINI - ALLEVAMENTI SEMIBRADI A BASSA CAPACITA <br/>
CONTROLLO UFFICIALE REV 1.1_2023</b>
</div>
</center> <br/><br/>
...
<tr>
<td><b>TIPOLOGIA DI ALLEVAMENTO</b><br/>(biologico, intensivo, semiestensivo)</td> 
<td>SEMIBRADO</td>
</tr>

<tr>
<td><b>RAZZE-LINEE GENETICHE</b><br/> (Incrocio, Large white, Landrace, Duroc, Danbred, Goland, Hypor, Topigs, PIC, Hermitage, altro)</td> 
<td><label class="layout">______________________</label></td>
</tr>
...
for (int i = 0; i<Checklist.getDomande().size(); i++) {
	...
	if (domanda.getListaRisposte().size() == 1)
	{
		colspan[0] = 4;
		colspan[1] = 0;
		colspan[2] = 0;
	...
	if (domanda.getListaRisposte().size() == 2)
	{
		colspan[0] = 2;
		colspan[1] = 2;
		colspan[2] = 0;
	...
	if (domanda.getListaRisposte().size() == 3)
	{
		colspan[0] = 1;
		colspan[1] = 1;
		colspan[2] = 2;
	...
	if (domanda.getListaRisposte().size() == 4)
	{
		colspan[0] = 1;
		colspan[1] = 1;
		colspan[2] = 1;
	...
	<% for (int j = 0; j<domanda.getListaRisposte().size(); j++) {
		...
		<td colspan="<%=colspan[j] %>" style="word-wrap: break-word; word-break: break-all; white-space: normal;"> 
...
<!-- 		le textarea non funzionano nello stampa pdf! -->
		<input type="text" class="editField" id="dom_<%=domanda.getId() %>_risposta_<%=risposta.getId() %>" name="dom_<%=domanda.getId() %>_risposta_<%=risposta.getId() %>" data-dom_id="<%=domanda.getId() %>" data-dom_ordine="<%=domanda.getOrdine() %>" data-dom_classyfarm_id="<%=domanda.getIdClassyfarm() %>" data-risposta_classyfarm_id="<%=risposta.getIdClassyfarm() %>" placeholder="<%=risposta.getRisposta() %>" maxlength="80" <%= risposta.getNote().length() > 30 ? "size=\""+risposta.getNote().length()+"\"" : "size=\"20\"" %> value="<%=risposta.getNote() %>"/> </td>
...
<jsp:param name="tipo" value="ChecklistBiosicurezzaSuiniSemibBassa2023" />
