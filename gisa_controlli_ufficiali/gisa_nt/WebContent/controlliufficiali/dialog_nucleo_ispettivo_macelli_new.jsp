<%ArrayList<org.aspcfs.modules.gestionecu.base.Qualifica> listaQualificheMacelli = (ArrayList<org.aspcfs.modules.gestionecu.base.Qualifica>)request.getAttribute("ListaQualificheMacelli");%>	

<select id="qualifica" name="qualifica">
<option value="-1">--- SELEZIONA ---</option>
<% 
boolean wasGruppo = false;
for (int i = 0; i<listaQualificheMacelli.size(); i++)  {
	org.aspcfs.modules.gestionecu.base.Qualifica q = (org.aspcfs.modules.gestionecu.base.Qualifica) listaQualificheMacelli.get(i);%> 
	...
	<% if (i == listaQualificheMacelli.size()-1) { %>
	</optgroup>
	<% } %>
...
function apriComponenti(){
	...
	$('#dialogNucleoIspettivoMacelli' ).dialog('open');
	
	link = "NucleoIspettivo.do?command=ListMacelli&idAsl="+idAsl+"&idQualifica="+idQualifica;
	$.ajax({
        url: link,
...
$(function () {
	 ...
	 $( "#dialogNucleoIspettivoMacelli" ).dialog({
	    	autoOpen: false,
	        resizable: false,
	        closeOnEscape: false,
...
// 	        	 "ESCI" : function() {  $( "#tablelistanucleo td").parent().remove(); $(this).dialog("close");}
	...
	<div id ="dialogNucleoIspettivoMacelli">
		<%
		%>		
