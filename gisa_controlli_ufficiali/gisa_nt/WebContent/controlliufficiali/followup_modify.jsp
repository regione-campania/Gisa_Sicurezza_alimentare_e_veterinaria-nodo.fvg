...
function checkForm(form) {
  ...
  if(form.nonConformitaGraviValutazione.value == "" || document.getElementById('nonConformitaGraviValutazione').value == 'INSERIRE LA VALUTAZIONE DEL RISCHIO DELLE NON CONFORMITA\' GRAVI RISCONTRATE'){// se non ho slezionato niente 
     ...
     <dhv:evaluate if="<%= !TicketDetails.isTrashed() %>" >
      <dhv:evaluate if="<%= TicketDetails.getClosed() != null %>" >
          <dhv:permission name="<%=permission%>">
           ...
           <dhv:evaluate if='<%= !"list".equals(request.getParameter("return"))%>' >
            <input type="submit" value="Annulla" onClick="javascript:this.form.action='<%=url %>Followup.do?command=TicketDetails&id=<%= TicketDetails.getId() %><%= addLinkParams(request, "popup|popupType|actionId") %>'" />
           </dhv:evaluate>
      </dhv:evaluate>
      <dhv:evaluate if="<%= TicketDetails.getClosed() == null %>" >
          <dhv:permission name="<%=permission%>">
  ...
  <dhv:evaluate if="<%= SiteIdList.size() <= 1 %>">
	...
	<tr class="containerBody">
      ...
      <td>
		...
		value="<%= toDateasString(TicketDetails.getAssignedDate()) %>"/>	
    ...
    <tr>
     <td  nowrap class="formLabel">
        <dhv:label name="">Valutazione del rischio n.c. gravi</dhv:label>
      	...
      	<td>
    		<textarea name="nonConformitaGraviValutazione" id="nonConformitaGraviValutazione" cols="55" rows="8"><%=toString(TicketDetails.getValutazione()) %></textarea>
    		<font color="red">*</font>
        </td>  
    </tr>
    <% } %>
