...
$( document ).ready(function (){
	var data = '<%= TicketDetails.getAssignedDate().toString() %>';
	var y = data.substring(0,4);
	var m = data.substring(5,7);
	var d = data.substring(8,10);
	data = d+'/'+m+'/'+y;
	$('#assignedDate').val(data);
	calenda('assignedDate',data,'');
});
 ...
 <%--@ include file="accounts_ticket_header_include.jsp" --%>
      ...
      <dhv:evaluate if="<%= TicketDetails.getClosed() != null %>" >
          <dhv:permission name="<%=permission%>">
           ...
           <dhv:evaluate if='<%= "list".equals(request.getParameter("return"))%>' >
            <input type="submit" value="Annulla" onClick="javascript:this.form.action='Accounts.do?command=ViewTickets&stabId=<%=OrgDetails.getIdStabilimento()%><%= addLinkParams(request, "popup|popupType|actionId") %>'" />
           </dhv:evaluate>
           <dhv:evaluate if='<%= !"list".equals(request.getParameter("return"))%>' >
  ...
  <dhv:evaluate if="<%= SiteIdList.size() <= 1 %>">
	...
	<tr class="containerBody">
      ...
      <td>
      <input type="text" class="date_picker" id="assignedDate" name="assignedDate" value="<%= TicketDetails.getAssignedDate() %>"/>
