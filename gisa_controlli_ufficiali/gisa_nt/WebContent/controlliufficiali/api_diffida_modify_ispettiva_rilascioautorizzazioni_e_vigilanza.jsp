...
<form name="details" action="<%=OrgDetails.getAction() %>Diffida.do?command=UpdateTicket&auto-populate=true<%= addLinkParams(request, "popup|popupType|actionId") %>" method="post">
...
<dhv:container name="<%=container %>" selected="vigilanza" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId()+"&opId=" + OrgDetails.getIdOperatore()%>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId") %>'>
...
<input type="hidden" name="idStabilimento" value="<%=TicketDetails.getIdApiario() %>">
</form>
</body>
