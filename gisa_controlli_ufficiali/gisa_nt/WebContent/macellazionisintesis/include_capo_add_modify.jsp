...
<jsp:useBean id="Tampone"			class="org.aspcfs.modules.macellazionisintesis.base.Tampone"		scope="request" />
...
<%@page import="org.aspcfs.modules.macellazionisintesis.base.Casl_Non_Conformita_Rilevata"%>
<%@page import="org.aspcfs.modules.macellazionisintesis.base.ProvvedimentiCASL"%>
<%@page import="org.aspcfs.modules.macellazionisintesis.base.Organi"%>
<%@page import="org.aspcfs.modules.contacts.base.Contact"%>
<%@page import="org.aspcfs.modules.gestionecu.base.Qualifica"%>
<%@page import="java.util.Date"%>
<%@page import="org.aspcfs.modules.macellazionisintesis.base.CampioneNew"%>
...
<jsp:useBean id="CapoLog"			class="org.aspcfs.modules.macellazionisintesis.base.CapoLog"		scope="request" />
...
<jsp:useBean id="CampioniNew"			class="java.util.ArrayList"				scope="request" />
...
<jsp:useBean id="ListaQualificheAC"	class="java.util.ArrayList"				scope="request" />
...
<%
	...
	if(Capo!=null && Capo.getMavam_data()!=null)
		mavamData = DateUtils.timestamp2string(Capo.getMavam_data());
	else if(request.getAttribute("mavamData")!=null)
		mavamData = (String)request.getAttribute("mavamData");
	Integer cdSedutaMacellazione =1;
	if(Capo!=null && Capo.getCd_seduta_macellazione()>0)
		 cdSedutaMacellazione = Capo.getCd_seduta_macellazione();
	if(request.getAttribute("cdSedutaMacellazione")!=null && !((String)request.getAttribute("cdSedutaMacellazione")).equals("") && !((String)request.getAttribute("cdSedutaMacellazione")).equals("null"))
...
$( document ).ready(function() {
	calenda('cd_data_nascita','','<%= vpmData %>');
	calenda('cd_data_mod4','','0');
	calenda('cd_data_arrivo_macello','','<%= vpmData %>');
	calenda('bse_data_prelievo','','0');
	calenda('bse_data_ricezione_esito','','0');
	calenda('vam_data','','<%= vpmData %>');
	calenda('abb_data','','0');
	calenda('casl_data','','0');
	calenda('rca_data','','0');
	calenda('seqa_data','','0');
	calenda('seqa_data_sblocco','','0');
	
	if('<%= request.getParameter("clona")%>' != 'si'){
		$('#vpm_data').val('');
	}
});

function specialistaSostituto( vpm ){
	//se passi vpm='Vpm' la funzione agira' sul sostituto della visita post mortem
	if($('#componenteSos' + vpm).is(':checked')){
		if($("#divComponenti"+ vpm +" i").length >= 3){
			alert("Limite componenti raggiunto, impossibile aggiungerne degli altri.");
			$('#componenteSos' + vpm).attr('checked', false);
			return;
		}
		$('#nominativoSos' + vpm).attr('readonly', false);
		$('#divComponenti' + vpm).append('<i id="specialistaSos'+ vpm +'"></i>');
	}else{
		$('#nominativoSos' + vpm).attr('readonly', true);
		$('#nominativoSos' + vpm).val('');
		$('#specialistaSos'+ vpm).remove();
	}
}

function check_unckeck_qualifica_sanitaria( ele ){
	
	if(ele.checked == true){
		$("[name*='cd_qualifica_sanitaria_']").each(function( index ) {
	 		  if(ele.id != $( this ).attr('id')){
	 			 $( this ).prop("checked", false);
	 			 check_uncheck_all($( this ).attr('id').replaceAll("_sanitaria",""), 3, false);
	 		  }
	 	});
	}
...
function check_uncheck_all(nome, len, check) {
	var nome_campo = nome;
	   for (i=1;i<=len;i++) {
	     if (check) {
	        document.getElementById(nome_campo+"_"+i).checked=true;
	        }
	     else {
	    	 document.getElementById(nome_campo+"_"+i).checked=false;
	        }
	     }
}
...
function impostaDataVpmMavam(data1,data2,sezione)
{
	var data = data1;
	if(data=='' || data =='null')
		data = data2;
	if(sezione=='vpm')
	{
		document.getElementById('vpmData').value=data;	
		document.getElementById('vpm_data').value=data;	
		document.getElementById('mavam_data').value="";
		document.getElementById('mavamData').value="";
	}
	else
	{
		document.getElementById('vpmData').value="";
		document.getElementById('mavamData').value=data;
		document.getElementById('vpm_data').value="";	
		document.getElementById('mavam_data').value=data;	
	}
...
function controllaForm( toLiberoConsumo )
{
	...
	try{
		...
		if(!toLiberoConsumo && $('#altreOperazioni').val()=='true')
		...
		if( form.cd_maschio.value == '' || form.cd_maschio.value == 'null' )
		{
			message += label("","- [Controllo documentale] : \"Sesso\" obbligatorio\r\n" );
			ret = false;
		}
...
// 		if( trim(form.veterinari_cd1.value) == '-1' && trim(form.veterinari_cd2.value) == '-1' && trim(form.veterinari_cd3.value) == '-1' ){
// 			message += label("","- [Controllo documentale] : Selezionare almeno un veterinario\r\n" );
// 			ret = false;
// 		}
// 		if( trim(form.veterinari_cd1.value) == '' && trim(form.veterinari_cd2.value) == '' && trim(form.veterinari_cd3.value) == '' ){
// 			message += label("","- [Controllo documentale] : Selezionare almeno un veterinario\r\n" );
// 			ret = false;
// 		}

		if($("#divComponenti i").length < 1){
			message += label("","- [Controllo documentale] : Selezionare almeno un veterinario\r\n" );
			ret = false;
		}
		
		if($('#componenteSos').is(':checked') && $('#nominativoSos').val() == ''){
			message += label("","- [Controllo documentale] : Indicare il nominativo del sostituto\r\n" );
			ret = false;
		}
		
		//inserire il controllo qui sul sostituto
// 		if(trim($("#veterinari_cd1 option:selected").text()) == 'SPECIALISTA SOSTITUTO' && trim(form.cd_veterinario_1.value) == '' ){
// 			message += label("","- [Controllo documentale] : Specificare il nome dello specialista sostituto 1\r\n" );
// 			ret=false;
// 		}
// 		if(trim($("#veterinari_cd2 option:selected").text()) == 'SPECIALISTA SOSTITUTO' && trim(form.cd_veterinario_2.value) == '' ){
// 			message += label("","- [Controllo documentale] : Specificare il nome dello specialista sostituto 2\r\n" );
// 			ret=false;
// 		}
// 		if(trim($("#veterinari_cd3 option:selected").text()) == 'SPECIALISTA SOSTITUTO' && trim(form.cd_veterinario_3.value) == '' ){
// 			message += label("","- [Controllo documentale] : Specificare il nome dello specialista sostituto 3\r\n" );
// 			ret=false;
// 		}
		...
		if ( form.vpm_data.value != '' && !toLiberoConsumo)
		{
			if ( confrontaDate( form.cd_data_arrivo_macello.value, form.vpm_data.value ) == 1 ) { 
				message += label("", "- [Controllo documentale] : Inserire \"Data Arrivo al macello\" antecedente o uguale alla \"Data Visita PM\" del capo("+form.vpm_data.value+")\r\n" );
				ret = false;
			}
		}
		else if(form.mavam_data.value != '' && !toLiberoConsumo)
		{
			if ( confrontaDate( form.cd_data_arrivo_macello.value, form.mavam_data.value ) ==1 ) { 
				message += label("", "- [Controllo documentale] : Inserire \"Data Arrivo al macello\" antecedente o uguale alla \"Data Morte Ante Macellazione\"("+form.mavam_data.value+")\r\n" );
				ret = false;
			}
		}
		
		if ( form.mavam_data.value == '' && form.vpm_data.value == '')
		{
			if ( confrontaDate( form.cd_data_arrivo_macello.value,'<%=vpmData%>' ) == 1 ) { 
				message += label("", "- [Controllo documentale] : Inserire \"Data Arrivo al macello\" antecedente o uguale alla data della sessione corrente(<%=vpmData%>)\r\n" );
				ret = false;
			}
		}
		...
		if (form.cd_specie.value == '-1')
		{
			message += label("","- [Controllo documentale] : \"Specie\" obbligatoria\r\n" );
			ret = false;
		}
		if (form.cd_data_nascita.value == '')
		{
			message += label("","- [Controllo documentale] : \"Data nascita\" obbligatoria\r\n" );
			ret = false;
		}
		
		if (form.cd_vincolo_sanitario.checked == true && form.cd_vincolo_sanitario_motivo.value.trim() == ''){
			message += label("","- [Controllo documentale] : \"Vincolo Sanitario\" obbligatorio\r\n" );
			ret = false;
		}
		...
		if(!toLiberoConsumo){
			...
			var comunicazioniEsterneA = document.getElementById('casl_to_asl_origine').checked || 
										...
										document.getElementById('casl_to_pcf').checked ||
		...
		if(!toLiberoConsumo){
			...
			var compilareVisitaAnteMortem = document.getElementById('vam_compilare1').checked;
			
			//se almeno uno di essi è valorizzato...
			if(dataVisitaAnteMortem || provvedimentoVisitaAnteMortem){
	
				//...allora vedi se ce n'è almeno uno NON valorizzato
				if(!dataVisitaAnteMortem || (!provvedimentoVisitaAnteMortem && compilareVisitaAnteMortem)){
		...
		*/
		...
		if(!toLiberoConsumo){
			...
			//se almeno uno di essi è valorizzato...
// 			if((dataVisitaPostMortem || destinatarioVisitaPostMortem) && $('#altreOperazioni').val() == 'true'){
...
// 			}
			...
			if((dataVisitaPostMortem) && $('#altreOperazioni').val() == 'true'){
				
				//...allora vedi se ce n'è almeno uno NON valorizzato
				if(!dataVisitaPostMortem ){
					message += label("","- [Evidenza Visita PM] :\r\n \tIl seguente campo non è valorizzato:\r\n \t*Data Macellazione\r\n \t*\n \tValorizzarlo.\r\n"  );
					ret = false;
				}
		...
		if ( form.cmp_data_ricezione_esito_1 != null && form.cmp_data_ricezione_esito_1.value != '' )
		{
		...
		if ( form.cmp_data_ricezione_esito_2 != null && form.cmp_data_ricezione_esito_2.value != '' )
		{
		...
		if ( form.cmp_data_ricezione_esito_3 != null && form.cmp_data_ricezione_esito_3.value != '' )
		{
		...
		if ( form.cmp_data_ricezione_esito_4 != null && form.cmp_data_ricezione_esito_4.value != '' )
		{
		...
		if ( form.cmp_data_ricezione_esito_5 != null && form.cmp_data_ricezione_esito_5.value != '' )
		{
		...
		if ( document.getElementById('vpm_data').value != '' && $('#altreOperazioni').val() == 'true' )
		{
			if($("#divComponentiVpm i").length < 1){ 
				message += label("","- [Evidenza Visita PM] : Inserire almeno un Veterinario  \r\n" );
				ret = false;
			}
			
			if($('#componenteSosVpm').is(':checked') && $('#nominativoSosVpm').val() == ''){
				message += label("","- [Evidenza Visita PM] : Indicare il nominativo del sostituto\r\n" );
				ret = false;
			}
		}	
		...
		*/
		...
		if ( (form.cmp_matrice_1 != null && form.cmp_matrice_1.value == '-1' && form.cmp_tipo_1 != null && form.cmp_tipo_1.value != '-1') ||
			 (form.cmp_matrice_2 != null && form.cmp_matrice_2.value == '-1' && form.cmp_tipo_2 != null && form.cmp_tipo_2.value != '-1') ||
			 (form.cmp_matrice_3 != null && form.cmp_matrice_3.value == '-1' && form.cmp_tipo_3 != null && form.cmp_tipo_3.value != '-1') ||
			 (form.cmp_matrice_4 != null && form.cmp_matrice_4.value == '-1' && form.cmp_tipo_4 != null && form.cmp_tipo_4.value != '-1') ||
			 (form.cmp_matrice_5 != null && form.cmp_matrice_5.value == '-1' && form.cmp_tipo_5 != null && form.cmp_tipo_5.value != '-1'))
		{
...
function set_vet( select, campo )
{
    ...
    if(value_to_set == "SPECIALISTA SOSTITUTO"){
    	value_to_set = "";
    }
...
function gestisciUtenteVeterinario(){
	...
	var id_capo = document.getElementById('id_capo').value;
	var clona = '<%= request.getParameter("clona")%>';
    
	if(id_capo > 0 || clona=='si'){
	
// 		if(document.getElementById('veterinari_cd1').value == '999' && document.getElementById('cd_veterinario_1').value != null){
// 			//document.getElementById('').option[value="SPECIALISTA SOSTITUTO"].selected = true;
// 			$('#veterinari_cd1  option[value="999"]').prop("selected", true);
// 			$('#veterinari_vpm option[value="999"]').prop("selected", true); 

// 		}
		
// 		if(document.getElementById('veterinari_cd2').value == '999' && document.getElementById('cd_veterinario_2').value != null){
// 			//document.getElementById('').option[value="SPECIALISTA SOSTITUTO"].selected = true;
// 			$('#veterinari_cd2  option[value="999"]').prop("selected", true);
// 			$('#veterinari_vpm2  option[value="999"]').prop("selected", true); 

// 		}
// 		if(document.getElementById('veterinari_cd3').value == '999' && document.getElementById('cd_veterinario_3').value != null){
// 			//document.getElementById('').option[value="SPECIALISTA SOSTITUTO"].selected = true;
// 			$('#veterinari_cd3  option[value="999"]').prop("selected", true);
// 			$('#veterinari_vpm3  option[value="999"]').prop("selected", true); 
...
function getDatiCapo( data )
{
	...
	if(data.errore == 0){
		...
		if (confirm('Attenzione il Codice Azienda Inserito non corrisponde a quello associato alla matricola del capo.Sovrascrivere il codice di provenienza '+form.cd_codice_azienda_provenienza.value)==true)
		{	form.cd_codice_azienda_provenienza.value=data.codice_azienda;
			searchBdn(data.codice_azienda,"1");
		}
		var BOVINI = 1;
		var BUFALINI = 5;
...
var sesso = 1;
		 ...
		 if (data.sesso==true) {
	            ...
	            sesso = 0;
	            form.cd_maschio[0].selected = true;
	            form.cd_maschio[1].selected = false;
	            form.cd_maschio[2].selected = false;
	        }
	        else if (data.sesso==false) {
	            form.sesso_from_bdn.value = 'F';
	            sesso = 1;
	            form.cd_maschio[1].selected = true;
	            form.cd_maschio[0].selected = false;
	            form.cd_maschio[2].selected = false;
	        }
	        else if (data.sesso==null) {
	            form.sesso_from_bdn.value = '';
	            sesso = 2;
	            form.cd_maschio[1].selected = false;
	            form.cd_maschio[0].selected = false;
	            form.cd_maschio[2].selected = true;
	        }
...
function checkProgressivoMacellazione(){
	...
	if (isNaN(progr)){
		...
		Geocodifica.controlloProgressivoMacellazioneBovini(document.getElementById('cd_seduta_macellazione').value,document.getElementById('id_macello').value, document.getElementById('vpm_data').value, progr, document.getElementById('cd_matricola').value, controlloProgressivoMacellazioneBoviniRet );
	}
...
function refreshDaVisitaAM(){
	var vamData = document.getElementById("vam_data").value;
	var vamEsitoRadio = document.getElementsByName("vam_esito");
	var vamEsito = "";
	if (vamEsitoRadio[0].checked)
		vamEsito = "FAVOREVOLE";
	if (vamEsitoRadio[1].checked)
		vamEsito = "FAVOREVOLE CON RISERVA";
	if (vamEsitoRadio[2].checked)
		vamEsito = "NON FAVOREVOLE";
	var vamProvvedimentoAdottato = document.getElementById("vam_provvedimenti").options[document.getElementById("vam_provvedimenti").selectedIndex].text;
	document.getElementById("div_vam_riepilogo").innerHTML = "Data visita AM: "+vamData+"<br/>Esito visita AM: "+vamEsito+"<br/>Provvedimento visita AM: "+vamProvvedimentoAdottato;

}

function refreshProspettoAnimale(tab){
	var matricola = document.getElementById("cd_matricola").value;
	var specie = document.getElementById("cd_specie").options[document.getElementById("cd_specie").selectedIndex].text;
	var sesso = document.getElementById("cd_maschio").options[document.getElementById("cd_maschio").selectedIndex].text;
	document.getElementById("prospetto_matricola_"+tab).innerHTML = matricola;
	document.getElementById("prospetto_specie_"+tab).innerHTML = specie;
	document.getElementById("prospetto_sesso_"+tab).innerHTML = sesso;
	
	if(tab == 'vam' || tab == 'vpm'){
		$('#'+tab+'_data').val('<%= vpmData %>');
	}
...
function gestisciCompilare(val){
	
	if (val=="1") {
		document.getElementById("vam_provvedimenti_TR").style.display="table-row";
		document.getElementById("vam_comunicazione_TR").style.display="table-row";
		document.getElementById("vam_provvedimenti_note_TR").style.display="table-row";
		
	}
	else if (val=="0"){
		
		document.getElementById("vam_data").value= "<%=vpmData%>";
		document.getElementById("vam_esito").checked="checked";
		document.getElementById("vam_esito").value="Favorevole";
		document.getElementById("vam_provvedimenti").value="-1";
		document.getElementById("vam_to_asl_origine").checked="";
		document.getElementById("vam_to_proprietario_animale").checked="";
		document.getElementById("vam_to_regione").checked="";
		document.getElementById("vam_to_azienda_origine").checked="";
		document.getElementById("vam_to_proprietario_macello").checked="";
		document.getElementById("vam_to_pif").checked="";
		document.getElementById("vam_to_uvac").checked="";
		document.getElementById("vam_to_altro_testo").value="";
		document.getElementById("vam_provvedimenti_note").value="";
		document.getElementById("vam_provvedimenti_TR").style.display="none";
		document.getElementById("vam_comunicazione_TR").style.display="none";
		document.getElementById("vam_provvedimenti_note_TR").style.display="none";
...
function checkObbVincoloSanitario(){
	if(document.getElementById('cd_vincolo_sanitario').checked){
		$('#obbVincoloSanitario').show();
	}else{
		$('#obbVincoloSanitario').hide();
		$('#cd_vincolo_sanitario_motivo').val('');
	}
}
...
<body onbeforeunload="return gestisciUnload();" onload="gestisciObbligatorietaComunicazioniEsterne();gestisciObbligatorietaMorteAnteMacellazione();gestisciObbligatorietaVisitaAnteMortem();gestisciObbligatorietaVisitaPostMortem();gestisciUtenteVeterinario();gestisciRazza();gestisciCategoria();contralla_cd_bse();mostra_tab_test_bse_onload();gestisciBloccoAnimale();displayAbbattimento(); mostraDestinazione();gestisciTabellaNonConformitaVam();">
	...
	<tr>
		<td>
			...
			%>
					...
					<a href="StabilimentoSintesisAction.do?command=DettaglioStabilimento&stabId=<%=OrgDetails.getIdStabilimento()%>">Scheda Stabilimento</a> >
			<%
						} else if (request.getParameter("return").equals("dashboard")) {}
					%>
			<a href="MacellazioniSintesis.do?command=List&altId=<%=OrgDetails.getAltId()%>">Macellazioni</a> > <%= update ? "Modifica Capo" : "Aggiungi Capo" %>
		</td>
	</tr>
...
<form name="main" action="MacellazioniSintesis.do?command=<%=(update) ? ("Update") : ("Save") %>" method="post" onsubmit="return controllaForm( false );" >
...
<input type="hidden" id="id_capo" name="id_capo" value="<%=Capo.getId() %>" />
...
<input type="hidden" id="mavamData" name="mavamData" value="<%=mavamData%>" />
<input type="hidden" name="cdSedutaMacellazione" id="cdSedutaMacellazione" value="<%=cdSedutaMacellazione %>" />

<%
if(vpmData!=null && !vpmData.equals("") && !vpmData.equals("null"))
{
%>
	SESSIONE DI APPARTENENZA: <%=vpmData+"-"+cdSedutaMacellazione %> <%=(sessionePregressa!=null) ? sessionePregressa : "" %><br/><br/>
<%
}
else if(mavamData!=null && !mavamData.equals("") && !mavamData.equals("null"))
{
%>
	SESSIONE DI APPARTENENZA: <%=mavamData+"-"+cdSedutaMacellazione %> <%=(sessionePregressa!=null) ? sessionePregressa : "" %><br/><br/>
<%
}
%>
...
<div id="tabs">
	<ul>
		<li id="li-1" ><a href="#tabs-1" onclick="javascript:showHide('hide');$('#Vpm').val('');" >Controllo Documentale</a></li>
<!--		<li id="tab_test_bse"><a href="#tabs-14" >Test BSE</a></li> 	-->
		<li id="li-7" style="display: <%= update ? "" : "none" %>;" ><a href="#tabs-7" onclick="javascript:showHide('hide');settaSoloCD('false'); refreshProspettoAnimale('ce');" >Comunicazioni Esterne</a></li>
        <%-- <li><a href="#tabs-5">Respingimento</a></li> --%>
		<%-- li><a href="#tabs-6">Blocco Animale</a></li --%>
		<li id="li-4" style="display: <%= update ? "" : "none" %>;" ><a href="#tabs-4" onclick="javascript:showHide('hide');settaSoloCD('false');refreshProspettoAnimale('am');" >Morte Ant.Macellazione</a></li>
		<li id="li-2" style="display: <%= update ? "" : "none" %>;" ><a href="#tabs-2" onclick="javascript:showHide('hide');settaSoloCD('false');refreshProspettoAnimale('vam');" >Evidenza Visita AM</a></li>
        <li id="li-3" style="display: <%= update ? "" : "none" %>;" ><a href="#tabs-3" onclick="javascript:showHide('show');settaSoloCD('false');refreshProspettoAnimale('vpm'); refreshDaVisitaAM();$('#Vpm').val('Vpm');" >Evidenza Visita PM</a></li>
            ...
            <tr>
             ...
             <tr class="containerBody">
            	<td class="formLabel">Codice Azienda di provenienza</td>
            	 <td>
                	<input type="text" onchange="return false; searchBdn(this.value,'1')" name="cd_codice_azienda_provenienza" value="<%=toHtmlValue( Capo.getCd_codice_azienda_provenienza() ) %>" /><font color="red" >*</font>
					
				
<br>			
			<textarea style="display:none" rows="5" cols="33"  name = "cd_info_azienda_provenienza" id = "infoAzienda" ><%=toHtml2( Capo.getCd_info_azienda_provenienza())%></textarea>
										
					<input type = "hidden" name = "rag_soc_azienda_provenienza" id = "rag_soc_azienda_provenienza" value ="<%=toHtml2( Capo.getRag_soc_azienda_prov())%>"/>
					<input type = "hidden" name = "denominazione_asl_azienda_prov" id = "denominazione_asl_azienda_prov" value ="<%=toHtml2( Capo.getDenominazione_asl_azienda_prov())%>"/>
					<input type = "hidden" name = "id_asl_azienda_prov" id = "id_codice_asl_azienda_prov" value ="<%=toHtml2( Capo.getId_asl_azienda_prov())%>"/>
					<input type = "hidden" name = "cod_asl_azienda_prov" id = "cod_asl_azienda_prov" value ="<%=toHtml2( Capo.getCod_asl_azienda_prov())%>"/>
...
<%
                ...
                {
%>
                	<input type="text" id="cd_matricola" name="cd_matricola" onchange="Geocodifica.isCapoEsistenteUpdateCapo( this.value, <%=Capo.getId()%>, gestisciCapoEsistente );" value="<%=toHtmlValue( Capo.getCd_matricola() ) %>" /><font color="red" >*</font>
<%
                }
                else
                {
...
%>  
            ...
            <tr class="containerBody" >
                ...
                <td>
                	<% Specie.setJsEvent("onChange=\"javascript:mostra_tab_test_bse();contralla_cd_bse();gestisciRazza();gestisciCategoria();\"");  %>
              		<%=Specie.getHtmlSelect("cd_specie", Capo.getCd_specie())%>  <font color="red">*</font>
				</td>
            </tr>
            ...
            <tr class="containerBody">
              ...
              <td>
                 <select name="cd_maschio" id="cd_maschio">
                   <option value="true" <%=(Boolean.TRUE.equals(Capo.isCd_maschio())) ? ("selected=\"selected\"") : ("") %> >maschio</option>
                   <option value="false" <%=(Boolean.FALSE.equals(Capo.isCd_maschio())) ? ("selected=\"selected\"") : ("") %> >femmina</option>
                   <option value="null" <%=(Capo.isCd_maschio()==null) ? ("selected=\"selected\"") : ("") %> >-- Selezionare --</option>
     ...
     <%--          		<zeroio:dateSelect field="cd_data_nascita" form="main" showTimeZone="false" timestamp="<%=Capo.getCd_data_nascita() %>" /><font color="red" >*</font> --%>
			      	<input type="text" id="cd_data_nascita" name="cd_data_nascita" size="10" class="date_picker" value="<%=DateUtils.timestamp2string(Capo.getCd_data_nascita())%>" />&nbsp;
			 		
			 		<a href="#" style="cursor: pointer;" onclick="svuotaData(document.forms[0].cd_data_nascita);"><img src="images/delete.gif" align="absmiddle"/></a>
			 		 <font color="red">*</font>
              </td>
            </tr>
...
<!--             </tr> -->
            ...
            <!-- nuova gestione flusso 173 RQ.4 -->
            <tr class="containerBody">
                <td class="formLabel" nowrap="nowrap">Categoria di Rischio MSR</td>
                <td>
                	<table class = "noborder">
                	...
                	<tr><td>
                	6 <input type = "radio" id = "cd_categoria_rischio_1" name = "cd_categoria_rischio" value = "6" <%if(Capo.getCd_categoria_rischio()==6   ){%>checked="checked"<%} %> />	Animali provenienti da aree a rischio trascurabile BSE <br>
                	</td></tr>
                	<tr><td>
                	7 <input type = "radio" id = "cd_categoria_rischio_2" name = "cd_categoria_rischio" value = "7" <%if(Capo.getCd_categoria_rischio()==7  ){%>checked="checked"<%} %> /> Animali provenienti da aree a rischio controllato BSE <br> 
                	</td></tr> 	
                	<tr><td>
                	8 <input type = "radio" id = "cd_categoria_rischio_3" name = "cd_categoria_rischio" value = "8" <%if(Capo.getCd_categoria_rischio()==8  ){%>checked="checked"<%} %> /> Animali provenienti da aree a rischio sconosciuto BSE <br>
                	</td></tr>
                	</table>
				</td> 
            </tr>
             ...
             <tr class="containerBody">
              ...
              <td valign="middle">
              	<table style="width: 100%;">
              		<tr>
              			<td><input type="checkbox" id="cd_vincolo_sanitario_piano"  name="cd_vincolo_sanitario_piano"  onclick="check_uncheck_all('cd_piano_risanamento',3, this.checked);" <%=(Capo.isCd_vincolo_sanitario_piano()) ? ("checked=\"checked\"") : ("") %> />
                			Piani risanamento 
              			</td>
              			<td><input type = "radio" id = "cd_piano_risanamento_1" name = "cd_piano_risanamento"  onclick="javascript:if(this.checked){form.cd_vincolo_sanitario_piano.checked=true;}" value = "1" <%if(Capo.getCd_piano_risanamento()==1){%>checked="checked"<%} %> />BRC
                		 	<input type = "radio" id = "cd_piano_risanamento_2" name = "cd_piano_risanamento"  onclick="javascript:if(this.checked){form.cd_vincolo_sanitario_piano.checked=true;}" value = "2" <%if(Capo.getCd_piano_risanamento()==2){%>checked="checked"<%} %> />TBC
                 			<input type = "radio" id = "cd_piano_risanamento_3" name = "cd_piano_risanamento"  onclick="javascript:if(this.checked){form.cd_vincolo_sanitario_piano.checked=true;}" value = "3" <%if(Capo.getCd_piano_risanamento()==3){%>checked="checked"<%} %> />LEB 
             			</td>
              		</tr>
              		<tr>
              			<td><input type="checkbox" id="cd_vincolo_sanitario" name="cd_vincolo_sanitario" onchange="checkObbVincoloSanitario()" <%=(Capo.isCd_vincolo_sanitario()) ? ("checked=\"checked\"") : ("") %> />
                			Altro
                		</td>
              			<td><textarea rows="2" cols="40" id="cd_vincolo_sanitario_motivo" name="cd_vincolo_sanitario_motivo"><%=toHtmlValue( Capo.getCd_vincolo_sanitario_motivo() ) %></textarea>
              			<font id="obbVincoloSanitario" color="red">*</font></td>
              			<script>checkObbVincoloSanitario();</script>
              		</tr>
              	</table>
              </td>
            </tr>
            ...
            <tr class="containerBody">
              <td class="formLabel" nowrap="nowrap">Qualifica sanitaria Azienda (per TBC e BRC)</td>
              <td>
              	<table style="width: 100%;">
              		<tr>
              			<td><input type="checkbox" id="cd_qualifica_sanitaria_tbc" name="cd_qualifica_sanitaria_tbc" onclick="check_unckeck_qualifica_sanitaria(this);check_uncheck_all('cd_qualifica_tbc',3, this.checked);" <%=(Capo.isCd_qualifica_sanitaria_tbc()) ? ("checked=\"checked\"") : ("") %> />
							TBC
						</td>
              			<td>
              				<input type = "radio" id = "cd_qualifica_tbc_1" name = "cd_qualifica_tbc" onclick="javascript:if(this.checked){form.cd_qualifica_sanitaria_tbc.checked=true;check_unckeck_qualifica_sanitaria(form.cd_qualifica_sanitaria_tbc);}" value = "1" <%if(Capo.getCd_qualifica_tbc()==1){%>checked="checked"<%}%> /> Ufficialmente indenne
			                <input type = "radio" id = "cd_qualifica_tbc_2" name = "cd_qualifica_tbc" onclick="javascript:if(this.checked){form.cd_qualifica_sanitaria_tbc.checked=true;check_unckeck_qualifica_sanitaria(form.cd_qualifica_sanitaria_tbc);}" value = "2" <%if(Capo.getCd_qualifica_tbc()==2){%>checked="checked"<%}%> /> Focolaio 
			                <input type = "radio" id = "cd_qualifica_tbc_3" name = "cd_qualifica_tbc" onclick="javascript:if(this.checked){form.cd_qualifica_sanitaria_tbc.checked=true;check_unckeck_qualifica_sanitaria(form.cd_qualifica_sanitaria_tbc);}" value = "3" <%if(Capo.getCd_qualifica_tbc()==3){%>checked="checked"<%}%> /> Sospesa
              			</td>
              		</tr>
              		<tr>
              			<td><input type="checkbox" id = "cd_qualifica_sanitaria_brc" name="cd_qualifica_sanitaria_brc" onclick="check_unckeck_qualifica_sanitaria(this);check_uncheck_all('cd_qualifica_brc',3, this.checked);" <%=(Capo.isCd_qualifica_sanitaria_brc()) ? ("checked=\"checked\"") : ("") %> />
                			BRC
                		</td>
              			<td>
              				<input type = "radio" id = "cd_qualifica_brc_1" name = "cd_qualifica_brc" onclick="javascript:if(this.checked){form.cd_qualifica_sanitaria_brc.checked=true;check_unckeck_qualifica_sanitaria(form.cd_qualifica_sanitaria_brc);}" value = "1" <%if(Capo.getCd_qualifica_brc()==1){%>checked="checked"<%} %> /> Ufficialmente indenne
                			<input type = "radio" id = "cd_qualifica_brc_2" name = "cd_qualifica_brc" onclick="javascript:if(this.checked){form.cd_qualifica_sanitaria_brc.checked=true;check_unckeck_qualifica_sanitaria(form.cd_qualifica_sanitaria_brc);}" value = "2" <%if(Capo.getCd_qualifica_brc()==2){%>checked="checked"<%} %> /> Focolaio
              				<input type = "radio" id = "cd_qualifica_brc_3" name = "cd_qualifica_brc" onclick="javascript:if(this.checked){form.cd_qualifica_sanitaria_brc.checked=true;check_unckeck_qualifica_sanitaria(form.cd_qualifica_sanitaria_brc);}" value = "3" <%if(Capo.getCd_qualifica_brc()==3){%>checked="checked"<%} %> /> Sospesa
              			</td>
              		</tr>
              		<tr>
              			<td><input type="checkbox" id = "cd_qualifica_sanitaria_leucosi" name="cd_qualifica_sanitaria_leucosi" onclick="check_unckeck_qualifica_sanitaria(this);check_uncheck_all('cd_qualifica_leucosi',3, this.checked);" <%=(Capo.isCd_qualifica_sanitaria_leucosi()) ? ("checked=\"checked\"") : ("") %> />
              				LEUCOSI BOVINA
              			</td>
              			<td>
              				<input type = "radio" id = "cd_qualifica_leucosi_1" name = "cd_qualifica_leucosi" onclick="javascript:if(this.checked){form.cd_qualifica_sanitaria_leucosi.checked=true;check_unckeck_qualifica_sanitaria(form.cd_qualifica_sanitaria_leucosi);}" value = "1" <%if(Capo.getCd_qualifica_leucosi()==1){%>checked="checked"<%} %> /> Ufficialmente indenne
                			<input type = "radio" id = "cd_qualifica_leucosi_2" name = "cd_qualifica_leucosi" onclick="javascript:if(this.checked){form.cd_qualifica_sanitaria_leucosi.checked=true;check_unckeck_qualifica_sanitaria(form.cd_qualifica_sanitaria_leucosi);}" value = "2" <%if(Capo.getCd_qualifica_leucosi()==2){%>checked="checked"<%} %> /> Focolaio
              				<input type = "radio" id = "cd_qualifica_leucosi_3" name = "cd_qualifica_leucosi" onclick="javascript:if(this.checked){form.cd_qualifica_sanitaria_leucosi.checked=true;check_unckeck_qualifica_sanitaria(form.cd_qualifica_sanitaria_leucosi);}" value = "3" <%if(Capo.getCd_qualifica_leucosi()==3){%>checked="checked"<%} %> /> Sospesa
              			</td>
              		</tr>
...
<%--              		<zeroio:dateSelect field="cd_data_mod4" form="main" showTimeZone="false" timestamp="<%=Capo.getCd_data_mod4() %>" /> --%>
              		<input type="text" id="cd_data_mod4" name="cd_data_mod4" size="10" class="date_picker" value="<%=DateUtils.timestamp2string(Capo.getCd_data_mod4())%>" />&nbsp;  
...
<%--              	<zeroio:dateSelect field="cd_data_arrivo_macello" form="main" showTimeZone="false" timestamp="<%=Capo.getCd_data_arrivo_macello() %>" /> --%>
					<input class="date_picker" type="text" id="cd_data_arrivo_macello" name="cd_data_arrivo_macello" size="10" value="<%=DateUtils.timestamp2string(Capo.getCd_data_arrivo_macello())%>" />  
			        <font color="red" >*</font>
...
<%--						<zeroio:dateSelect 
 ...
 --%>							
						<input type="text" class="date_picker" name="bse_data_prelievo" id="bse_data_prelievo" size="10" value="<%=DateUtils.timestamp2string(Capo.getBse_data_prelievo())%>" />&nbsp;  
...
<%--						<zeroio:dateSelect 
 ...
 --%>
						<input type="text" id="bse_data_ricezione_esito" name="bse_data_ricezione_esito" size="10" class="date_picker" value="<%=DateUtils.timestamp2string(Capo.getBse_data_ricezione_esito())%>" />&nbsp;  
...
<!--***************************************************************-->
            ...
            <tr>
                <th colspan="2"><strong>Veterinari addetti al controllo documentale</strong>                </th>
            </tr>
            	...
            	<td colspan="2">
		            <table>
		            	<tr class="containerBody" style="display: block;">
		                	<td>
		                		<input type="hidden" id="Vpm" value="" />
			                	<input type="hidden" id="siteId" name="siteId" value="<%= OrgDetails.getIdAsl() %>"/>
								...
								<% } %>
 			                	...
 			                	<%@ include file="../macellazionisintesis/dialog_veterinari.jsp" %>
 			                	<a href="#" onclick="apriComponenti(''); return false;"><font  color="#006699" style="font-weight: bold;">Seleziona Componente</font></a>
 			                	<div id="divComponenti"></div><br>
 			                	<script type="text/javascript">
 			                		$('#Vpm').val('');
 			                		<% if(Capo.getCd_id_veterinario_1() > 0){ %>
 			                			creaComponente('Clonato_Clonato_<%= Capo.getCd_id_veterinario_1() %>', 'Clonato', 'Qualifica', 'Clonato', 'Struttura', '<%= Capo.getCd_id_veterinario_1() %>', '<%= Capo.getCd_veterinario_1() %>');
 			                		<% } %>	
 			                		<% if(Capo.getCd_id_veterinario_2() > 0){ %>
 			                			creaComponente('Clonato_Clonato_<%= Capo.getCd_id_veterinario_2() %>', 'Clonato', 'Qualifica', 'Clonato', 'Struttura', '<%= Capo.getCd_id_veterinario_2() %>', '<%= Capo.getCd_veterinario_2() %>');
 			                		<% } %>	
 			                		<% if(Capo.getCd_id_veterinario_3() > 0){ %>
 			                			creaComponente('Clonato_Clonato_<%= Capo.getCd_id_veterinario_3() %>', 'Clonato', 'Qualifica', 'Clonato', 'Struttura', '<%= Capo.getCd_id_veterinario_3() %>', '<%= Capo.getCd_veterinario_3() %>');
 			                		<% } %>	
 			                	</script>
 			                	<input type="checkbox" id="componenteSos" name="componenteSos" onchange="specialistaSostituto('')" <%= Capo.getCd_veterinario_sostituto() != null ? "checked" : "" %> /> SPECIALISTA SOSTITUTO -
 			                	<input type="textbox" id="nominativoSos" name="nominativoSos" size="35" readonly value="<%= Capo.getCd_veterinario_sostituto() != null ? Capo.getCd_veterinario_sostituto() : "" %>" />
		                		<script type="text/javascript">
 			                		<%= Capo.getCd_veterinario_sostituto() != null ? "specialistaSostituto('');" : "" %>
 			                	</script>
		                	</td>
	...
	<input id="liberoConsumoButton" type="button" value="Libero Consumo >>" onclick="if(controllaForm( true )){document.main.action='MacellazioniSintesis.do?command=ToLiberoConsumo&cdSedutaMacellazione=<%=cdSedutaMacellazione%>&vpmData=<%=vpmData%>&mavamData=<%=mavamData%>';settaSoloCD('false');document.main.submit();};" />
...
<%-- 	<input type="button" value="Altre operazioni" onclick="if(checkControlloDocumentale()){document.getElementById('mavam_data').value='<%=mavamData%>';document.getElementById('vpm_data').value='<%=vpmData%>';}" /> --%>
		<input type="button" value="Altre operazioni" onclick="if(checkControlloDocumentale()){$('#altreOperazioni').val('true');}" />
		<input type="hidden" id="altreOperazioni" />
<!--	</div>-->
	<%} %>
...
<table width="100%" border="0" cellpadding="2" cellspacing="2">
    ...
    <tbody><tr>
        ...
        <table class="details" width="100%" border="0" cellpadding="4" cellspacing="0">
        <tbody>
        <tr><th colspan="2"><strong>Animale</strong></th></tr>
         
        <tr><td style="background-color:yellow;text-align: right" nowrap="nowrap">Matricola</td>
        <td style="background-color:yellow;"><label id="prospetto_matricola_vam"></label></td>
        </tr>
            
        <tr class="containerBody" >
        <td class="formLabel" nowrap="nowrap">Specie<br></td>
        <td><label id="prospetto_specie_vam"></label></td>
        </tr>
          
        <tr class="containerBody">
        <td class="formLabel" nowrap="nowrap">Sesso</td>
        <td><label id="prospetto_sesso_vam"></label></td>
        </tr>
            ...
            <tr>
             ...
             <tr class="containerBody">
              ...
              <td>
              <input type="radio" id="vam_compilare1" name="vam_compilare" value="1" onClick="gestisciCompilare(1)" <%= Capo.getVam_data() != null && request.getParameter("clona") == "si" ? "checked" : "" %>/> COMPILARE
              <input type="radio" id="vam_compilare0" name="vam_compilare" value="0" onClick="gestisciCompilare(0)" <%= Capo.getVam_data() == null ? "checked" : "" %>/> NON COMPILARE
              </td>
            </tr>
...
<%--               	<zeroio:dateSelect field="vam_data" form="main" showTimeZone="false" timestamp="<%=Capo.getVam_data() %>" /> --%>
					<input type="text" id="vam_data" name="vam_data" class="date_picker" onfocus="riportaDataArrivoMacello(this);gestisciObbligatorietaVisitaAnteMortem();" size="10" value="<%=DateUtils.timestamp2string(Capo.getVam_data())%>" />&nbsp;
            ...
            <tr class="containerBody" id="vam_provvedimenti_TR">
            ...
            <tr class="containerBody" id="vam_comunicazione_TR">
...
<!--             			/> U.V.A.C. <br/>	 -->
					...
					<input  
            			...
            			id="vam_to_pcf"
            			name="vam_to_pcf" 
            			<%=(Capo.isVam_to_pcf()) ? ("checked=\"checked\"") : ("") %>
            			/> P.C.F. <br/>	
            		<input 
            			type="checkbox" 
            			id="vam_to_regione"
              ...
              <tr class="containerBody" id="vam_provvedimenti_note_TR">
		 		<td class="formLabel">Note</td>
		 		<td><textarea rows="2" cols="40" id="vam_provvedimenti_note" name="vam_provvedimenti_note"><%=toHtmlValue( Capo.getVam_provvedimenti_note() ) %></textarea></td>
			  </tr>
...
<%--			<zeroio:dateSelect field="abb_data" form="main" showTimeZone="false" timestamp="<%=Capo.getAbb_data() %>"/>  --%>              
              	<input type="text" id="abb_data" class="date_picker" name="abb_data" size="10" value="<%=DateUtils.timestamp2string(Capo.getAbb_data())%>" />&nbsp;  
...
<div id="tabs-3">
    ...
    <tbody><tr>
        ...
        <tr><td style="background-color:yellow;text-align: right" nowrap="nowrap">Matricola</td>
        <td style="background-color:yellow;"><label id="prospetto_matricola_vpm"></label></td>
        </tr>
            
        <tr class="containerBody" >
        <td class="formLabel" nowrap="nowrap">Specie<br></td>
        <td><label id="prospetto_specie_vpm"></label></td>
        </tr>
          
        <tr class="containerBody">
        <td class="formLabel" nowrap="nowrap">Sesso</td>
        <td><label id="prospetto_sesso_vpm"></label></td>
        </tr>
...
<!--              <td >-->
	              	<input type="hidden" name="mac_progressivo" value="<%=Capo.getMac_progressivo() %>" />
<!--              </td>-->
<!--            </tr>-->
        ...
        <td valign="top" width="100%"><table class="details" width="100%" border="0" cellpadding="4" cellspacing="0">
          <tbody>
            <tr>
             ...
             <tr class="
             containerBody">
              <td class="formLabel" >Riepilogo visita AM</td>
              <td>
          	<div id="div_vam_riepilogo"/>
			</td>
            </tr>
...
<%--              	<zeroio:dateSelect field="vpm_data" form="main" showTimeZone="false" timestamp="<%=Capo.getVpm_data() %>" />  --%>
					<input readonly type="text" id="vpm_data" name="vpm_data"  size="10" value="<%= vpmData != "" ? vpmData : "" %>" />&nbsp; 
					<input type="button" id="settaDataVpmMavam" name="settaDataVpmMavam" value="Imposta data"  onclick="impostaDataVpmMavam('<%=vpmData%>','<%=mavamData%>','vpm');"/>&nbsp; 
...
<!--            </tr>              -->
            ...
            <tr 
                ...
                <td class="formLabel">PATOLOGIE RILEVATE E/O MOTIVI DI DISTRUZIONE DELLA CARCASSA</td>
                <%
             ...
             <tr>
                <th colspan="2"><strong>Veterinari addetti al controllo visita post mortem</strong></th>
            </tr>
            ...
            <tr class="containerBody">
            	<td colspan="2">
		            <table>
		            	<tr class="containerBody" style="display: block;">
		                	<td>
 			                	<select id="qualificaVpm" name="qualificaVpm">
								<option value="-1">--- SELEZIONA ---</option>
								<% 
								for (int i = 0; i<listaQualificheAc.size(); i++)  {
									org.aspcfs.modules.gestionecu.base.Qualifica q = (org.aspcfs.modules.gestionecu.base.Qualifica) listaQualificheAc.get(i);%>
								...
								<% } %>
 			                	...
 			                	<%@ include file="../macellazionisintesis/dialog_veterinari.jsp" %>
 			                	<a href="#" onclick="apriComponenti('Vpm'); return false;"><font  color="#006699" style="font-weight: bold;">Seleziona Componente</font></a>
			                	<div id="divComponentiVpm"></div><br>
			                	<script type="text/javascript">
			                		$('#Vpm').val('Vpm');
 			                		<% if(Capo.getVpm_id_veterinario_1() > 0){ %>
 			                			creaComponente('Clonato_Clonato_<%= Capo.getVpm_id_veterinario_1() %>', 'Clonato', 'Qualifica', 'Clonato', 'Struttura', '<%= Capo.getVpm_id_veterinario_1() %>', '<%= Capo.getVpm_veterinario() %>');
 			                		<% } %>	
 			                		<% if(Capo.getVpm_id_veterinario_2() > 0){ %>
 			                			creaComponente('Clonato_Clonato_<%= Capo.getVpm_id_veterinario_2() %>', 'Clonato', 'Qualifica', 'Clonato', 'Struttura', '<%= Capo.getVpm_id_veterinario_2() %>', '<%= Capo.getVpm_veterinario_2() %>');
 			                		<% } %>	
 			                		<% if(Capo.getVpm_id_veterinario_3() > 0){ %>
 			                			creaComponente('Clonato_Clonato_<%= Capo.getVpm_id_veterinario_3() %>', 'Clonato', 'Qualifica', 'Clonato', 'Struttura', '<%= Capo.getVpm_id_veterinario_3() %>', '<%= Capo.getVpm_veterinario_3() %>');
 			                		<% } %>	
 			                	</script>
			                	<input type="checkbox" id="componenteSosVpm" name="componenteSosVpm" onchange="specialistaSostituto('Vpm')"
			                	<%= Capo.getVpm_veterinario_sostituto() != null ? "checked" : "" %> <%= Capo.getVpm_veterinario_sostituto() != null ? "checked" : "" %> /> SPECIALISTA SOSTITUTO -
 			                	<input type="textbox" id="nominativoSosVpm" name="nominativoSosVpm" size="35" readonly value="<%= Capo.getVpm_veterinario_sostituto() != null ? Capo.getVpm_veterinario_sostituto() : "" %>" />
 			                	<script type="text/javascript">
 			                		<%= Capo.getCd_veterinario_sostituto() != null ? "specialistaSostituto('Vpm');" : "" %>
 			                	</script>
			                </td>
			            </tr>
		            </table>
	...
	<% } %>
            ...
            <tr>
                <th colspan="2"><strong>Destinatario Carni</strong>                </th>
            </tr>
            <tr>
            	<td colspan="2">
...
<% if (!update) { %>
<tr><td> <%@include file="include_campioni_add_new.jsp" %> </td></tr>
<% } else { %>
<tr><td><%@include file="include_campioni_edit_new.jsp" %></td></tr>
<% } %>
...
<table width="100%" border="0" cellpadding="2" cellspacing="2">
    ...
    <tbody><tr>
        ...
        <tr><td style="background-color:yellow;text-align: right" nowrap="nowrap">Matricola</td>
        <td style="background-color:yellow;"><label id="prospetto_matricola_am"></label></td>
        </tr>
            
        <tr class="containerBody" >
        <td class="formLabel" nowrap="nowrap">Specie<br></td>
        <td><label id="prospetto_specie_am"></label></td>
        </tr>
          
        <tr class="containerBody">
        <td class="formLabel" nowrap="nowrap">Sesso</td>
        <td><label id="prospetto_sesso_am"></label></td>
        </tr>
...
<%--              	<zeroio:dateSelect field="mavam_data" showTimeZone="false" form="main" timestamp="<%=Capo.getMavam_data() %>" />   --%>
              		<input readonly type="text" id="mavam_data" name="mavam_data" size="10" value="<%=DateUtils.timestamp2string(Capo.getMavam_data())%>" />&nbsp;
              		<% if(Capo.getMavam_data() != null){ %>
              		<script type="text/javascript">
              			$(document).ready(function(){
              				settaSoloCD('false');
              				refreshProspettoAnimale('am');
              			});
              			
              		</script>
              		<% } %> 
			        <input type="button" id="settaDataVpmMavam2" name="settaDataVpmMavam2" value="Imposta data"  onclick="impostaDataVpmMavam('<%=vpmData%>','<%=mavamData%>','mavam');"/>&nbsp; 
			        ...
			        <!--  a href="#" onClick="cal19.select(document.forms[0].mavam_data,'anchor19','dd/MM/yyyy'); return false;" NAME="anchor19" ID="anchor19">
			 		...
			 		<!-- a href="#"  style="cursor: pointer;" onclick="svuotaData(document.forms[0].mavam_data);"><img src="images/delete.gif" align="absmiddle"/></a -->
              		<input type="hidden" name="cd_seduta_macellazione2" id="cd_seduta_macellazione2" value="<%=cdSedutaMacellazione %>" />
              </td>
...
<!--             			/> U.V.A.C. <br/> -->
            		<input 
            			...
            			id="mavam_to_pcf"
            			name="mavam_to_pcf" 
            			<%=(Capo.isMavam_to_pcf()) ? ("checked=\"checked\"") : ("") %>
            			/> P.C.F. <br/>
            		<input 
            			type="checkbox" 
            			id="mavam_to_regione"
...
<table width="100%" border="0" cellpadding="2" cellspacing="2">
    ...
    <tr>
        ...
        <tr><td style="background-color:yellow;text-align: right" nowrap="nowrap">Matricola</td>
        <td style="background-color:yellow;"><label id="prospetto_matricola_ce"></label></td>
        </tr>
            
        <tr class="containerBody" >
        <td class="formLabel" nowrap="nowrap">Specie<br></td>
        <td><label id="prospetto_specie_ce"></label></td>
        </tr>
          
        <tr class="containerBody">
        <td class="formLabel" nowrap="nowrap">Sesso</td>
        <td><label id="prospetto_sesso_ce"></label></td>
        </tr>
...
<!--             			/> U.V.A.C. <br/> -->
            		<input 
            			...
            			name="casl_to_pcf" 
            			onclick="gestisciObbligatorietaComunicazioniEsterne();"
            			<%=(Capo.isCasl_to_pcf()) ? ("checked=\"checked\"") : ("") %>
            			/> P.C.F. <br/>	
            		<input 
            			type="checkbox" 
            			id="casl_to_regione"
...
<%--            	<zeroio:dateSelect field="casl_data" form="main" showTimeZone="false" timestamp="<%=Capo.getCasl_data() %>" />  --%>
              		<input type="text" id="casl_data" class="date_picker" name="casl_data" onfocus="riportaDataArrivoMacello(this);gestisciObbligatorietaComunicazioniEsterne();" size="10" value="<%=DateUtils.timestamp2string(Capo.getCasl_data())%>" />&nbsp;
...
<%--                <zeroio:dateSelect field="rca_data" form="main" showTimeZone="false" timestamp="<%=Capo.getRca_data() %>" /> --%>
                	<input type="text" id="rca_data" class="date_picker" name="rca_data" onfocus="riportaDataArrivoMacello(this);" size="10" value="<%=DateUtils.timestamp2string(Capo.getRca_data())%>" />&nbsp;  
      ...
      <tr>
	   ...
	   <%-- 	 		<zeroio:dateSelect field="seqa_data" form="main" showTimeZone="false" timestamp="<%=Capo.getSeqa_data() %>"/>	--%>          
	              		<input type="text" name="seqa_data" id="seqa_data" class="date_picker" size="10" value="<%=DateUtils.timestamp2string(Capo.getSeqa_data())%>" />&nbsp;
...
<%-- 	              		<zeroio:dateSelect field="seqa_data_sblocco" form="main" showTimeZone="false" timestamp="<%=Capo.getSeqa_data_sblocco() %>"/> 	--%>
		              	<input type="text" name="seqa_data_sblocco" id="seqa_data_sblocco" class="date_picker" size="10" value="<%=DateUtils.timestamp2string(Capo.getSeqa_data_sblocco())%>" />&nbsp;
...
<%
  if(update && !Capo.getStato_macellazione().equals("Incompleto: Inseriti solo i dati sul controllo documentale"))
  { 
%>

<script type="text/javascript">
  document.getElementById('vpm_data').value='<%=vpmData%>';
</script>
<%
  }
%>
