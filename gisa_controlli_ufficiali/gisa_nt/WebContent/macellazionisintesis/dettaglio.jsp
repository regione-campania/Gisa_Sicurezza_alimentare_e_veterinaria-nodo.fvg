...
<%@page import="org.aspcfs.modules.macellazionisintesis.base.Campione"%>
...
<%@page import="org.aspcfs.modules.macellazionisintesis.base.TipoRicerca"%><jsp:useBean id="Tampone"			class="org.aspcfs.modules.macellazionisintesis.base.Tampone"	scope="request" />
...
function openPopupErrataCorrigeArt17(idCapo, altId, tipo){
        var res;
        var result;
     
        	  window.open('MacellazioniDocumentiSintesis.do?command=ViewModuleErrataCorrigeArt17&altId='+altId+'&idCapo='+idCapo+'&tipoMacello='+tipo,'popupSelect',
...
<script language="javascript">
function openPopupListaErrataCorrigeArt17(idCapo, altId, tipo){
        var res;
        var result;
        
  
        	  window.open('MacellazioniDocumentiSintesis.do?command=ViewListaModuleErrataCorrigeArt17&altId='+altId+'&idCapo='+idCapo+'&tipoMacello='+tipo,'popupSelect',
              'height=800px,width=1280px,toolbar=no,directories=no,status=no,continued from previous linemenubar=no,scrollbars=yes,resizable=yes ,modal=yes');
...
function openPopupCancellazioneCapo(idCapo){
    var res;
    var result;
   	  window.open('MacellazioniSintesis.do?command=PrepareCancellazioneCapo&idCapo='+idCapo,'popupSelect',
	...
	<tr>
		<td>
			...
			<%
				...
				{}
			%>
			<a href="MacellazioniSintesis.do?command=List&altId=<%=OrgDetails.getAltId() %>">Macellazioni</a> > Dettaglio
		</td>
	</tr>
</table>
...
<dhv:permission name="stabilimenti-stabilimenti-macellazioni-view">
	<a href="MacellazioniSintesis.do?command=PrintInvioCampioniTBC&file=invio_campioni_TBC.xml&<%=param2 %>">Invio campioni TBC</a>
</dhv:permission>

<dhv:permission name="stabilimenti-stabilimenti-macellazioni-view">
	<a href="MacellazioniSintesis.do?command=PrintModelloLBE&file=modello_LBE.xml&<%=param2 %>">Modello LBE</a>
</dhv:permission>

<dhv:permission name="stabilimenti-stabilimenti-macellazioni-view">
	<a href="MacellazioniSintesis.do?command=PrintModello1033TBC&file=modello_10_33_TBC.xml&<%=param2 %>">Modello 10 33 TBC</a>
</dhv:permission>
<%} %>
...
<dhv:permission name="stabilimenti-stabilimenti-macellazioni-edit">
	<input type="button" value="Modifica" onclick="location.href='MacellazioniSintesis.do?command=ModificaCapo&cdSedutaMacellazione=<%=Capo.getCd_seduta_macellazione()%>&vpmData=<%=(Capo.getVpm_data()==null ? ("") : (toDateasString(Capo.getVpm_data())))%>&mavamData=<%=(Capo.getMavam_data()==null ? ("") : (toDateasString(Capo.getMavam_data())))%>&id=<%=Capo.getId() %>&altId=<%=OrgDetails.getAltId() %>'" />
</dhv:permission>

<dhv:permission name="stabilimenti-stabilimenti-macellazioni-delete">
	<input type="button" value="Elimina" onclick="if(confirm('Sicuro di voler eliminare la registrazione?')){location.href='MacellazioniSintesis.do?command=Delete&id=<%=Capo.getId() %>&altId=<%=OrgDetails.getAltId() %>'}" />
</dhv:permission>
<%} else{ 
...
%>
<dhv:permission name="stabilimenti-stabilimenti-macellazioni-edit">
	<input type="button" value="Inserisci Esito Campioni" onclick="location.href='MacellazioniSintesis.do?command=ModificaCapo&id=<%=Capo.getId() %>&altId=<%=OrgDetails.getAltId() %>'" />
</dhv:permission>
<%	
}else
{
...
Capo non modificabile: la seduta di appartenenza è stata chiusa.
...
<input type="button" value="Errata corrige art.17" onclick="openPopupErrataCorrigeArt17('<%=Capo.getId() %>', '<%=OrgDetails.getAltId() %>', '<%=request.getAttribute("tipo") %>')" />
<%} else { %>
; Raggiunto numero massimo moduli Errata Corrige generabili per questo capo.
<%} %>
<a href="javascript:openPopupListaErrataCorrigeArt17('<%=Capo.getId() %>', '<%=OrgDetails.getAltId() %>', '<%=request.getAttribute("tipo") %>')"> Moduli EC generati</a> 
<br/><br/>
</dhv:permission>
<%}}} %>
...
--%>            
            ...
            <tr class="containerBody">
              ...
              <td valign="middle">
                <%=Capo.isCd_vincolo_sanitario_piano() ? "PIANI RISANAMENTI: SI - " : " PIANI RISANAMENTI: NO" %>
                 <%
                	if(Capo.getCd_piano_risanamento()==1){
                %>
                        BRC
                <%		
                	}else if (Capo.getCd_piano_risanamento()==2){
                		%>
                        TBC
                <%		
                	}else if (Capo.getCd_piano_risanamento()==3){
                		%>
                        LEB
                <%		
                	}
                %>
              	...
              	<%=Capo.isCd_vincolo_sanitario() ? "ALTRO: SI - " : " ALTRO: NO" %>
                <%
             ...
             <tr class="containerBody">
              <td class="formLabel" >Qualifica Sanitaria (per TBC e BRC)</td>

              <td valign="middle">
                <%=Capo.isCd_qualifica_sanitaria_tbc() ? "TBC SI - " : "TBC NO" %>
                 <%
                	if(Capo.getCd_qualifica_tbc()==1){
                %>
                        Ufficialmente indenne
                <%		
                	}else if (Capo.getCd_qualifica_tbc()==2){
                		%>
                        Focolaio
                <%		
                	}else if (Capo.getCd_qualifica_tbc()==3){
                		%>
                        Sospesa
                <%		
                	}
                %>
              	...
              	<%=Capo.isCd_qualifica_sanitaria_brc() ? "BRC: SI - " : " BRC: NO" %>
                <%
                	if(Capo.getCd_qualifica_brc()==1){
                %>
                        Ufficialmente indenne
                <%		
                	}else if (Capo.getCd_qualifica_brc()==2){
                		%>
                        Focolaio
                <%		
                	}else if (Capo.getCd_qualifica_brc()==3){
                		%>
                        Sospesa
                <%		
                	}
                %>
              	...
              	<%=Capo.isCd_qualifica_sanitaria_leucosi() ? "LEUCOSI: SI - " : " LEUCOSI: NO" %>
                <%
                	if(Capo.getCd_qualifica_leucosi()==1){
                %>
                        Ufficialmente indenne
                <%		
                	}else if (Capo.getCd_qualifica_leucosi()==2){
                		%>
                        Focolaio
                <%		
                	}else if (Capo.getCd_qualifica_leucosi()==3){
                		%>
                        Sospesa
                <%		
                	}
...
<!--            -->
             ...
             <tr>
              ...
              <tr class="containerBody">
                ...
                <td>&nbsp;1. <%= Capo.getCd_veterinario_1() != null ? Capo.getCd_veterinario_1() : "" %></td>
            </tr>
            
            <tr class="containerBody">
                <td>&nbsp;2. <%= Capo.getCd_veterinario_2() != null ? Capo.getCd_veterinario_2() : "" %></td>
            </tr>
            <tr class="containerBody">
                <td>&nbsp;3. <%= Capo.getCd_veterinario_3() != null ? Capo.getCd_veterinario_3() : "" %></td>
            </tr>
            
            <% if(Capo.getCd_veterinario_sostituto() != null){ %>
            <tr class="containerBody">
                <td class="formLabel">SPECIALISTA SOSTITUTO</td>
                <td>&nbsp;<%= Capo.getCd_veterinario_sostituto() %></td>
            </tr> 
            <% } %>
        ...
        <td valign="top" width="100%"><table class="details" width="100%" border="0" cellpadding="4" cellspacing="0">
          ...
          <tr>
            ...
            <tr class="containerBody">
              ...
              <td>
<%--                  <%=Capo.isCd_maschio() ? ("M") : ("F") %> --%>
                <%=Boolean.TRUE.equals(Capo.isCd_maschio()) ? "M" : Boolean.FALSE.equals(Capo.isCd_maschio()) ? "F" : null==Capo.isCd_maschio() ? "" : "" %>
...
<%
				...
				if(stato==null || !stato.equals("Incompleto: Inseriti solo i dati sul controllo documentale") || !stato.equals("Incompleto: Veterinario mancante") )
...
<!--            <%} %>        -->
            ...
            <% if(Capo.getVpm_veterinario_sostituto() != null){ %>
            <tr class="containerBody">
                <td class="formLabel">SPECIALISTA SOSTITUTO</td>
                <td>&nbsp;<%= Capo.getVpm_veterinario_sostituto() %></td>
            </tr> 
            <% } %>
            <tr>
  ...
  <%} %>
   ...
   <tr>
    ...
    <tr>
    	...
    	<% } } else if (CampioniNew.size() > 0) { %>
    		<%@include file="dettaglio_campioni_new.jsp" %>
    	<% }  else { %>
    	   <tr><td colspan="5" class="formLabel"> Nessun Campione</td></tr>
    	<%} %>
