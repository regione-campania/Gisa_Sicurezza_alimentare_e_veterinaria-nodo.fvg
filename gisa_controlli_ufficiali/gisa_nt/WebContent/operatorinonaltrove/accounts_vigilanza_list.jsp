...
<%@ page import="java.util.*,java.text.DateFormat,org.aspcfs.modules.operatorinonaltrove.base.*,org.aspcfs.modules.vigilanza.base.Ticket,com.zeroio.iteam.base.*" %>
...
<jsp:useBean id="OpnonAltroveTicketInfo" class="org.aspcfs.utils.web.PagedListInfo" scope="session"/>
...
<dhv:container name="operatorinonaltrove" selected="vigilanza" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId") %>'>
  <dhv:evaluate if="<%= !OrgDetails.isTrashed() %>">
    ...
    <dhv:permission name="operatorinonaltrove-operatorinonaltrove-vigilanza-add"><a href="OpnonAltroveVigilanza.do?command=Add&orgId=<%= OrgDetails.getOrgId() %><%= addLinkParams(request, "popup|popupType|actionId") %>"><dhv:label name="accounts.richiesta.add">Aggiungi Nuovo Controllo Ufficiale</dhv:label></a></dhv:permission>
  </dhv:evaluate>
