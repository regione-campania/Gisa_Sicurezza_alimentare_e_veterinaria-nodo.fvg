...
<dhv:permission name="stabilimenti-stabilimenti-macellazioni-add">
	<a href="MacellazioniNewOpu.do?command=NuovoCapo&<%=param1 %>">Aggiungi Capo</a>
</dhv:permission>
...
<dhv:permission name="stabilimenti-stabilimenti-macellazioni-view">
	<a href="MacellazioniDocumentiNewOpu.do?command=ToMod10&<%=param1 %>">Modello 10</a>
</dhv:permission>
...
if(listaDateMacellazione.size() > 0){
%>
<form name="macellazioniForm" action="MacellazioniNewOpu.do?command=AddSedutaMacellazione" onsubmit="return(confirm('Cliccando su OK verrà chiusa definitivamente la corrente seduta di Macellazione e aperta la nuova per la giornata selezionata'))" method="post">
<input type="hidden" name="altId" value="<%=OrgDetails.getAltId() %>" />
