...
if (request.getAttribute("idMacchinetta")!=null)
	{
	idMacchinetta=""+OrgDetails.getId();
	
	} %>
<form name="addticket" action="OiaNonConformita.do?command=Insert&idNodo=<%=idMacchinetta %>&auto-populate=true&idNodo=<%=idMacchinetta %>" method="post">
<%-- Trails --%>
<table class="trails" cellspacing="0">
<tr>
<td>
<a href="Oia.do?command=Home"><dhv:label name="">Autorita' Competenti</dhv:label></a> > 
...
<input type="button" value="Annulla" onClick="if(confirm('ATTENZIONE: si sta per abbandonare la pagina di aggiunta di una non conformita. Lasciando questa pagina il lavoro svolto finora verra perso.Sicuro di voler uscire ?')==true){window.location.href='OiaVigilanza.do?command=TicketDetails&id=<%= OrgDetails.getId() %><%= addLinkParams(request, "popup|popupType|actionId") %>&idNodo=<%=idMacchinetta %>';this.form.dosubmit.value='0';}" />
   ...
   <dhv:include name="stabilimenti-sites" none="true">
	...
	UserBean user=(UserBean)session.getAttribute("User");
      ...
      %>
       ...
       <%= SiteIdList.getSelectedValue(OrgDetails.getId_asl())%>
          <input type="hidden" name="siteId" value="<%=OrgDetails.getId_asl()%>" >
...
<br><input type="submit" id = "btn_salva2" disabled="disabled" value="Inserisci" name="Save" onClick="return checkForm(this.form)">
<input type="button" value="Annulla" onClick="if(confirm('ATTENZIONE: si sta per abbandonare la pagina di aggiunta di una non conformita. Lasciando questa pagina il lavoro svolto finora verra perso.Sicuro di voler uscire ?')==true){window.location.href='OiaVigilanza.do?command=TicketDetails&id=<%= OrgDetails.getId() %><%= addLinkParams(request, "popup|popupType|actionId") %>&idNodo=<%=idMacchinetta %>';this.form.dosubmit.value='0';}" />
</form>
</body>
