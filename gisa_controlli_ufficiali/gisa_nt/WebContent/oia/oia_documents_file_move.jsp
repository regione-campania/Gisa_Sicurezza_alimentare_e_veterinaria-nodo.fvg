<%-- 
  ...
  - 
  - Version: $Id: oia_Documenti_file_move.jsp 11310 2005-04-13 20:05:00Z mrajkowski $
  - Description: 
  --%>
...
<table cellpadding="0" cellspacing="0" width="100%" border="1" rules="cols">
  <tr class="section">
    <td valign="top" width="100%">
      ...
      <a href="OiaDocuments.do?command=SaveMove&orgId=<%= OrgDetails.getOrgId() %>&fid=<%= FileItem.getId() %>&popup=true&folderId=0&return=OiaDocuments.do?param=<%= OrgDetails.getOrgId() %>&param2=<%= FileItem.getFolderId() %>"><dhv:label name="oia.oia_Documenti_file_move.Home">Home</dhv:label></a>
      <dhv:evaluate if="<%= (FileItem.getFolderId() == 0) || (FileItem.getFolderId() == -1) %>">
      <dhv:label name=".oia_Documenti_file_move.CurrentFolder">(current folder)</dhv:label>
      </dhv:evaluate>
    </td>
  </tr>
<%
...
%>
  <tr class="row<%= rowid %>">
    ...
    <% for(int j=1;j<thisFolder.getLevel();j++){ %>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<%}%>
      ...
      <a href="OiaDocuments.do?command=SaveMove&orgId=<%= OrgDetails.getOrgId() %>&fid=<%= FileItem.getId() %>&popup=true&folderId=<%= thisFolder.getId() %>&return=OiaDocuments.do?param=<%= OrgDetails.getOrgId() %>&param2=<%= FileItem.getFolderId() %>"><%= toHtml(thisFolder.getSubject()) %></a>
