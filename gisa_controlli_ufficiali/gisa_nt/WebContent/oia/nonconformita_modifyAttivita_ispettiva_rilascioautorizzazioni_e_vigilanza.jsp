...
<form name="details" action="OiaNonConformita.do?command=UpdateTicket&idNodo<%=idMacchinetta %>&auto-populate=true<%= addLinkParams(request, "popup|popupType|actionId") %>" onSubmit="return checkForm(this);" method="post">
...
<td>
  ...
  <a href="OiaNonConformita.do?command=TicketDetails&idMacchinetta=<%=idMacchinetta %>&id=<%=TicketDetails.getId()%>"><dhv:label name="nonconformita.dettagli">Scheda Sequestro/Blocco</dhv:label></a> >
  <%}%>
     ...
     <dhv:evaluate if="<%= !TicketDetails.isTrashed() %>" >
      <dhv:evaluate if="<%= TicketDetails.getClosed() != null %>" >
          <dhv:permission name="oia-oia-nonconformita-edit">
           ...
           <dhv:evaluate if='<%= !"list".equals(request.getParameter("return"))%>' >
            <input type="submit" value="Annulla" onClick="javascript:this.form.action='OiaNonConformita.do?command=TicketDetails&idMacchinetta=<%=idMacchinetta %>&id=<%= TicketDetails.getId() %><%= addLinkParams(request, "popup|popupType|actionId") %>'" />
           </dhv:evaluate>
      </dhv:evaluate>
      <dhv:evaluate if="<%= TicketDetails.getClosed() == null %>" >
          <dhv:permission name="oia-oia-nonconformita-edit">
