...
<form name="details" action="DistributoriSequestri.do?command=UpdateTicket&idNodo=<%=idMacchinetta %>&auto-populate=true<%= addLinkParams(request, "popup|popupType|actionId") %>" onSubmit="return checkForm(this); " method="post">
...
<td>
  ...
  <a href="Oia.do?command=Details&id=<%=idMacchinetta %>&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label name="accounts.details">Scheda Dipartimento</dhv:label></a> > 
...
<a href="OiaNonConformita.do?command=TicketDetails&idNodo=<%=idMacchinetta %>&id=<%= request.getAttribute("idNC")%>&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label name="">Non Conformità Rilevata</dhv:label></a> >
  ...
  <a href="OiaSequestri.do?command=TicketDetails&idNodo=<%=idMacchinetta %>&id=<%=TicketDetails.getId()%>"><dhv:label name="sequestri.dettagli">Scheda Sequestro/Blocco</dhv:label></a> >
  <%}%>
     ...
     <dhv:evaluate if="<%= !TicketDetails.isTrashed() %>" >
      <dhv:evaluate if="<%= TicketDetails.getClosed() != null %>" >
          <dhv:permission name="destributori-distributori-sequestri-edit">
            <input type="submit" value="<dhv:label name="button.reopen">Reopen</dhv:label>" onClick="javascript:this.form.action='OiaSequestri.do?command=ReopenTicket&idNodo=<%=idMacchinetta %>&id=<%=TicketDetails.getId()%><%= addLinkParams(request, "popup|popupType|actionId") %>'">
           </dhv:permission>
           ...
           <dhv:evaluate if='<%= !"list".equals(request.getParameter("return"))%>' >
            <input type="submit" value="Annulla" onClick="javascript:this.form.action='OiaSequestri.do?command=TicketDetails&idNodo=<%=idMacchinetta %>&id=<%= TicketDetails.getId() %><%= addLinkParams(request, "popup|popupType|actionId") %>'" />
           </dhv:evaluate>
      </dhv:evaluate>
      <dhv:evaluate if="<%= TicketDetails.getClosed() == null %>" >
