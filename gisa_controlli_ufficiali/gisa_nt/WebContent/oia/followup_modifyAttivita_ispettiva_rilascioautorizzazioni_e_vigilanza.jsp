...
<form name="details" action="OiaFollowup.do?command=UpdateTicket&idNodo=<%=idNodo %>&auto-populate=true<%= addLinkParams(request, "popup|popupType|actionId") %>"  method="post">
...
<a href="OiaNonConformita.do?command=TicketDetails&idNodo=<%=idNodo %>&id=<%= request.getAttribute("idNC")%>&orgId=<%=TicketDetails.getOrgId()%>"><dhv:label name="">Non Conformità Rilevata</dhv:label></a> >
  ...
  <a href="OiaFollowup.do?command=TicketDetails&id=<%=TicketDetails.getId()%>"><dhv:label name="followup.dettagli">Scheda Follow Up</dhv:label></a> >
  <%}%>
