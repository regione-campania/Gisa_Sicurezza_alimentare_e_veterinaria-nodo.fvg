...
<form name="details" action="DistributoriTamponi.do?command=ModifyTicket&idNodo=<%=idMacchinetta %>&auto-populate=true" method="post">
...
%>
<dhv:container name="asl" selected="vigilanza" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId()+"&idNodo="+idMacchinetta %>' >

	<%@ include file="ticket_header_include_tamponi.jsp"%>
	<%
		if (TicketDetails.isTrashed()) {
	%>
	<dhv:permission name="oia-oia-tamponi-delete">
		<input type="button"
			value="Ripristina"
			onClick="javascript:this.form.action='OiaTamponi.do?command=Restore&idNodo=<%=idMacchinetta %>&id=<%= TicketDetails.getId()%>';submit();">
	</dhv:permission>
	<%
	...
	<dhv:permission name="reopen-reopen-view">
		<input type="button"
			...
			onClick="javascript:this.form.action='OiaTamponi.do?command=ReopenTicket&idNodo=<%=idMacchinetta %>&id=<%= TicketDetails.getId()%><%= (defectCheck != null && !"".equals(defectCheck.trim())?"&defectCheck="+defectCheck:"") %>';submit();">
	</dhv:permission>
	<%
		} else {
	%>
	<dhv:permission name="oia-oia-tamponi-edit">
		<input type="button"
			...
			onClick="javascript:this.form.action='OiaTamponi.do?command=ModifyTicket&idNodo=<%=idMacchinetta %>&auto-populate=true<%= (defectCheck != null && !"".equals(defectCheck.trim())?"&defectCheck="+defectCheck:"") %>';submit();">
	</dhv:permission>
	<%--
	...
	<dhv:permission name="oia-oia-tamponi-delete">
		...
		<input type="button"
			...
			onClick="javascript:popURL('OiaTamponi.do?command=ConfirmDelete&idNodo=<%=idMacchinetta %>&id=<%= TicketDetails.getId() %>&orgId=<%=OrgDetails.getOrgId() %>&return=searchResults&popup=true', 'Delete_ticket','320','200','yes','no');">
		<%
			} else {
		%>
		<input type="button"
			value="<dhv:label name="global.button.delete">Delete</dhv:label>"
			onClick="javascript:popURL('OiaTamponi.do?command=ConfirmDelete&idNodo=<%=idMacchinetta %>&id=<%= TicketDetails.getId() %>&orgId=<%=OrgDetails.getOrgId() %>&popup=true', 'Delete_ticket','320','200','yes','no');">
		<%
			}
		%>
	</dhv:permission>
	<dhv:permission name="oia-oia-tamponi-edit">
		<input type="button"
			...
			onClick="javascript:this.form.action='OiaTamponi.do?command=Chiudi&idNodo=<%=idMacchinetta %>&id=<%= TicketDetails.getId() %>';if( confirm('Sei sicuro di voler chiudere il tampone? \n Attenzione! La pratica verrà chiusa e non sarà più possibile fare modifiche \n sulla scheda se non con il permesso del supervisore o dell amministratore') ){submit()};">
	</dhv:permission>
	<%
		}
	%>
...
if(request.getAttribute("Messaggio2")!=null)
{
	...
	function doSubmit(idTicket){
		...
		document.details.action="OiaTamponi.do?command=Chiudi&id="+idTicket+"&chiudiCu=true'"
