<%-- 
  ...
  - 
  - Version: $Id: oia_Documenti_folders_add.jsp 18488 2007-01-15 20:12:32Z matt $
  - Description: 
  --%>
...
<form method="POST" name="inputForm" action="OiaDocuments.do?olders.do?command=Save&auto-populate=true<%= addLinkParams(request, "popup|popupType|actionId|actionplan") %>" onSubmit="return checkForm(this);">
...
<a href="OiaDocuments.do?command=List&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label name="oia.oia_Documenti_details.Documenti">Documenti</dhv:label></a> >
<% if(fileFolder.getId() > -1) {%>
  <dhv:label name="oia.oia_Documenti_folders_add.ModifyFolder">Modify Folder</dhv:label>
<%} else {%>
  <dhv:label name="Documents.do?umenti.newFolder">New Folder</dhv:label>
<%}%>
...
<br>
  ...
  <input type="submit" value="Annulla" onClick="javascript:this.form.dosubmit.value='false';this.form.action='OiaDocuments.do?command=View&orgId=<%= OrgDetails.getOrgId() %><%= addLinkParams(request, "popup|popupType|actionId|actionplan") %>';" /><br />
  <dhv:formMessage />
  <br />
