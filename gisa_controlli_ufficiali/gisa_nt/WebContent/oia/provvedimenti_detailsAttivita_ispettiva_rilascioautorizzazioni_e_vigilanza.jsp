...
<form name="details" action="OiaProvvedimenti.do?command=ModifyTicket&idNodo=<%=idNodo %>&auto-populate=true" method="post">
...
<td>
  ...
  <a href="OiaVigilanza.do?command=TicketDetails&idNodo=<%=idNodo %>&id=<%= request.getAttribute("idC")%>&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label name="">Controllo Ufficiale</dhv:label></a> >
   <%--a href="Oia.do?command=ViewCampioni&orgId=<%=TicketDetails.getOrgId()%>"><dhv:label name="campioni">Tickets</dhv:label></a> --%>
...
%>
<dhv:container name="asl" selected="vigilanza" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId()+"&idNodo="+idNodo %>' 
	>
	<%
	...
	%>
		...
		<input type="button"
			value="Ripristina"
			onClick="javascript:this.form.action='OiaProvvedimenti.do?command=Restore&idNodo=<%=idNodo %>&id=<%= TicketDetails.getId()%>';submit();">
	...
	<dhv:permission name="reopen-reopen-view">
		<input type="button"
			...
			onClick="javascript:this.form.action='OiaProvvedimenti.do?command=ReopenTicket&idNodo=<%=idNodo %>&id=<%= TicketDetails.getId()%><%= (defectCheck != null && !"".equals(defectCheck.trim())?"&defectCheck="+defectCheck:"") %>';submit();">
	</dhv:permission>
	<%
		} else {
	%>
		...
		<input type="button"
			...
			onClick="javascript:this.form.action='AbusivismiProvvedimenti.do?command=ModifyTicket&idNodo=<%=idNodo %>&auto-populate=true<%= (defectCheck != null && !"".equals(defectCheck.trim())?"&defectCheck="+defectCheck:"") %>';submit();">
		...
		<input type="button"
			...
			onClick="javascript:this.form.action='OiaProvvedimenti.do?command=Chiudi&idNodo=<%=idNodo %>&id=<%= TicketDetails.getId() %>';if( confirm('Sei sicuro di voler chiudere il campione? \n Attenzione! La pratica verrà chiusa e non sarà più possibile fare modifiche \n sulla scheda se non con il permesso del supervisore o dell amministratore') ){submit()};">		
...
<br />
  ...
  --%>
		...
		<input type="button"
			value="<dhv:label name="global.button.close">Chiudi</dhv:label>"
			onClick="javascript:this.form.action='AbusivismiProvvedimenti.do?command=Chiudi&idNodo=<%=idNodo %>&id=<%= TicketDetails.getId() %>';if( confirm('Sei sicuro di voler chiudere il campione? \n Attenzione! La pratica verrà chiusa e non sarà più possibile fare modifiche \n sulla scheda se non con il permesso del supervisore o dell amministratore') ){submit()};">
	<%
		}
	%>
</dhv:container>
...
if(request.getAttribute("Messaggio2")!=null)
{
	...
	function doSubmit(idTicket){
		...
		document.details.action="OiaProvvedimenti.do?command=Chiudi&id="+idTicket+"&chiudiCu=true'"
