...
costruisci_rel_ateco_attivita('<%= OrgDetails.getAltId() %>', 'id_linea_sottoposta_a_controllo'); "">
...
<td>
  ...
  <a href="<%=NewTicket.getURlDettaglio() %>.do?command=Details&altId=<%=OrgDetails.getAltId()%>"><dhv:label name="">Scheda Anagrafica Operatore</dhv:label></a> >
  <a href="<%=NewTicket.getURlDettaglio() %>.do?command=ViewVigilanza&altId=<%=OrgDetails.getAltId()%>"><dhv:label name="vigilanza">Tickets</dhv:label></a> >
...
<%
	if(!popUp){
%>
<input type="submit" value="Annulla" onClick="javascript:this.form.action='Accounts.do?command=ViewVigilanza&altId=<%=OrgDetails.getAltId()%>'">
<% } else { %>
