...
<script language="JavaScript" type="text/javascript">
  ...
  function reopen() {
    scrollReload('Operatoriprivati.do?command=Search');
  }
</script>
<%-- Trails --%>
...
<%-- End Trails --%>
<dhv:permission name="operatoriprivati-operatoriprivati-add"><a href="Operatoriprivati.do?command=Add">Aggiungi Privato</a></dhv:permission>
<dhv:permission name="operatoriprivati-operatoriprivati-add" none="true"><br></dhv:permission>
...
<tr>
  ...
  <th width="30%" nowrap="true">
    <strong><a href="Operatoriprivati.do?command=Search&column=c.namelasta">Cognome</a></strong>
  ...
  <th width="20%" nowrap="true">
    <strong><a href="Operatoriprivati.do?command=Search&column=c.org_namea"><dhv:label name="organization.name">Nome</dhv:label></a></strong>
...
%>      
      ...
      <td valign="center" nowrap>
        ...
        <a href="javascript:displayMenu('select<%= i %>','menuContact', '<%= thisContact.getAltId() %>', '<%= thisContact.getId() %>', '<%= thisContact.getPrimaryContact() %>','<%= (((thisContact.getTrashedDate() != null) || !thisContact.getEnabled()) || ((thisContact.getOrgTrashedDate() != null) || !thisContact.getOrgEnabled()))%>');"
      ...
      <td valign="center" nowrap>
        <a href="Operatoriprivati.do?command=Details&altId=<%= thisContact.getAltId() %>"><%= toHtml(thisContact.getOrgName()) %></a>
      </td>
      <dhv:evaluate if='<%= SearchOrgListInfo.getSearchOptionValue("searchcodeSiteId") == String.valueOf(Constants.INVALID_SITE) %>'>
