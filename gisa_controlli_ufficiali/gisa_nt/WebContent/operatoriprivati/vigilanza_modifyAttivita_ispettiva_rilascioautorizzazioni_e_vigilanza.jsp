...
<script language="JavaScript" TYPE="text/javascript" SRC="javascript/controlli_ufficiali_privati.js"></script>
...
function load_linee_attivita_per_alt_id_anagrafica_callback(returnValue) {
 ...
 function costruisci_rel_ateco_attivita( org_id, campo_combo_da_costruire, default_value ) {
		  ...
		  PopolaCombo.load_linee_attivita_per_alt_id_anagrafica(org_id , campo_combo_da_costruire, default_value, {callback:load_linee_attivita_per_alt_id_anagrafica_callback,async:false}  )
		
	  }
</script>

<body onLoad="abilitaCodiceAllerta('details');initprovaFunzione('details');abilitaSistemaAllarmeRabido('details');resetElementiNucleoIspettivo('<%=TicketDetails.getNucleoasList().size() %>');costruisci_rel_ateco_attivita('<%= OrgDetails.getAltId() %>', 'id_linea_sottoposta_a_controllo', '<%= TicketDetails.getId_imprese_linee_attivita() %>' );">
<form name="details" id="privati" action="OperatoriprivatiVigilanza.do?command=UpdateTicket&auto-populate=true<%= addLinkParams(request, "popup|popupType|actionId") %>" onSubmit="return checkForm(this);" method="post">
...
<td>
  ...
  <a href="Operatoriprivati.do?command=Search"><dhv:label name="">Risultati Ricerca</dhv:label></a> >
  <a href="Operatoriprivati.do?command=Details&altId=<%=OrgDetails.getAltId()%>"><dhv:label name="operatoriprivati.details">Operatoriprivati Details</dhv:label></a> >
  <a href="Operatoriprivati.do?command=ViewVigilanza&altId=<%=OrgDetails.getAltId()%>"><dhv:label name="vigilanza">Tickets</dhv:label></a> >
  <% if (request.getParameter("return") == null) {%>
  <a href="OperatoriprivatiVigilanza.do?command=TicketDetails&id=<%=TicketDetails.getId()%>"><dhv:label name="campioni.dettagli">Scheda Controllo Ufficiale</dhv:label></a> >
  <%}%>
...
<dhv:container name="operatoriprivati" selected="vigilanza" object="OrgDetails" param='<%= "altId=" + OrgDetails.getAltId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId") %>'>
  <dhv:container name="operatoriprivativigilanza" selected="details" object="TicketDetails" param='<%= "id=" + TicketDetails.getId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId") %>'>
    <%--@ include file="abusivismi_ticket_header_include_vigilanza.jsp" --%>
     <dhv:evaluate if="<%= !TicketDetails.isTrashed() %>" >
      <dhv:evaluate if="<%= TicketDetails.getClosed() != null %>" >
          <dhv:permission name="operatoriprivati-operatoriprivati-vigilanza-edit">
            <input type="submit" value="<dhv:label name="button.reopen">Reopen</dhv:label>" onClick="javascript:this.form.action='OperatoriprivatiVigilanza.do?command=ReopenTicket&id=<%=TicketDetails.getId()%><%= addLinkParams(request, "popup|popupType|actionId") %>'">
           </dhv:permission>
           <dhv:evaluate if='<%= "list".equals(request.getParameter("return"))%>' >
            <input type="submit" value="Annulla" onClick="javascript:this.form.action='Operatoriprivati.do?command=ViewVigilanza&altId=<%=OrgDetails.getAltId()%><%= addLinkParams(request, "popup|popupType|actionId") %>'" />
           </dhv:evaluate>
           <dhv:evaluate if='<%= !"list".equals(request.getParameter("return"))%>' >
            <input type="submit" value="Annulla" onClick="javascript:this.form.action='OperatoriprivatiVigilanza.do?command=TicketDetails&id=<%= TicketDetails.getId() %><%= addLinkParams(request, "popup|popupType|actionId") %>'" />
           </dhv:evaluate>
      </dhv:evaluate>
      <dhv:evaluate if="<%= TicketDetails.getClosed() == null %>" >
          <dhv:permission name="operatoriprivati-operatoriprivati-vigilanza-edit">
