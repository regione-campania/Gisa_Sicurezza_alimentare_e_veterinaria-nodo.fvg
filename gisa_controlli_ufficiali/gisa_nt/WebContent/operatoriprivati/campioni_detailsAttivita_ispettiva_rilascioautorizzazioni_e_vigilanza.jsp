...
function apriVerbale(idCampione,tipoAnalisi)
{
	...
	document.location.href="OperatoriprivatiCampioni.do?command=StampaScheda&tipoAnalisi="+tipoAnalisi+"&idCampione="+idCampione+"&tipoAlimenti="+tipoAlimenti;
...
<form id="details" name="details" action="OperatoriprivatiCampioni.do?command=ModifyTicket&auto-populate=true" method="post">
...
<td>
  ...
  <a href="OperatoriprivatiVigilanza.do?command=TicketDetails&id=<%= request.getAttribute("idC")%>&altId=<%=OrgDetails.getAltId()%>"><dhv:label name="">Controllo Ufficiale</dhv:label></a> >
...
if(request.getAttribute("Messaggio2")!=null)
{
	...
	function doSubmit(idTicket){
		...
		document.details.action="OperatoriprivatiCampioni.do?command=Chiudi&id="+idTicket+"&chiudiCu=true'"
