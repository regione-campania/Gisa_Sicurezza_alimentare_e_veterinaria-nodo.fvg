...
<%@ page import="java.util.*,java.text.DateFormat,org.aspcfs.modules.operatoriprivati.base.*,org.aspcfs.modules.vigilanza.base.Ticket,com.zeroio.iteam.base.*" %>
...
<td>
Operatori Privati > 
<a href="Operatoriprivati.do?command=Details&altId=<%=OrgDetails.getAltId() %>">Scheda privato</a> >
Controlli ufficiali
</td>
</tr>
</table>
...
%>
  ...
  <dhv:container name="<%=nomeContainer %>" selected="Scheda" object="OrgDetails" param="<%=param%>">
    <dhv:permission name="operatoriprivati-operatoriprivati-vigilanza-add"><a href="OperatoriprivatiVigilanza.do?command=Add&altId=<%= OrgDetails.getAltId() %><%= addLinkParams(request, "popup|popupType|actionId") %>"><dhv:label name="accounts.richiesta.add">Aggiungi Nuovo Controllo Ufficiale</dhv:label></a></dhv:permission>
...
<table cellpadding="4" cellspacing="0" border="0" width="100%" class="pagedList">
  ...
  %>
   ...
   <td width="10%" valign="top" nowrap>
		...
		<td width="10%" valign="top" nowrap>
			<a href="OperatoriprivatiVigilanza.do?command=TicketDetails&TimeIni=<%=System.currentTimeMillis() %>&container=<%=request.getAttribute("container")%>&id=<%= thisTic.getId() %>&altId=<%= thisTic.getAltId()%><%= addLinkParams(request, "popup|popupType|actionId") %>"><zeroio:tz timestamp="<%= thisTic.getAssignedDate() %>" dateOnly="true" timeZone="<%= thisTic.getAssignedDateTimeZone() %>" showTimeZone="false" default="&nbsp;"/>
     		</a>
		</td>
