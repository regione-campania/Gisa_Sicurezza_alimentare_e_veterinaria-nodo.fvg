...
<body onload="verificaChiusuraNc(<%=request.getAttribute("Chiudi")%>,<%=request.getAttribute("numSottoAttivita")%>,'<%=request.getAttribute("Messaggio2")%>',<%=TicketDetails.getIdControlloUfficiale()%>,'OperatoriprivatiVigilanza')">
...
<td>
 <a href="<%=TicketDetails.getURlDettaglio()%>.do">Operatori</a> > 
  ...
  <a href="<%=TicketDetails.getURlDettaglio()%>.do?command=Details&altId=<%=OrgDetails.getAltId()%>">Scheda Operatore</a> >
...
%>
<dhv:container name="operatoriprivati" selected="vigilanza" object="OrgDetails" param='<%="altId=" + OrgDetails.getAltId()%>' hideContainer='<%=isPopup(request) || (defectCheck != null && !"".equals(defectCheck.trim()))%>'>
