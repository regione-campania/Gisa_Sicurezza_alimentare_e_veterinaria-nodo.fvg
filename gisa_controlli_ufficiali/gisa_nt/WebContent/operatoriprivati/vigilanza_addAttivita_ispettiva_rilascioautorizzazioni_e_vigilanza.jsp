...
<form name="addticket" id="privati" action="OperatoriprivatiVigilanza.do?command=Insert&auto-populate=true" method="post">
<%-- Trails --%>
<table class="trails" cellspacing="0">
<tr>
<td>
  <a href="Operatoriprivati.do"><dhv:label name="">Operatoriprivati</dhv:label></a> > 
...
<input type="submit" value="Annulla" onClick="javascript:this.form.action='Operatoriprivati.do?command=ViewVigilanza&altId=<%=OrgDetails.getAltId()%>'">
<br>
<dhv:formMessage />
...
<script language="JavaScript" TYPE="text/javascript" SRC="javascript/popOperatoriprivati.js"></script>
	...
	function load_linee_attivita_per_alt_id_anagrafica_callback(returnValue) {
	  ...
	  function costruisci_rel_ateco_attivita( alt_id, campo_combo_da_costruire ) {
		 ...
		 // alert(campo_combo_da_costruire);
		  PopolaCombo.load_linee_attivita_per_alt_id_anagrafica(alt_id , campo_combo_da_costruire, null, load_linee_attivita_per_alt_id_anagrafica_callback)
	  }
...
<body onload = "provaFunzione('addticket'); onloadAllerta('addticket'); resetElementiNucleoIspettivo('<%=TicketDetails.getNucleoasList().size() %>'); costruisci_rel_ateco_attivita('<%= OrgDetails.getAltId() %>', 'id_linea_sottoposta_a_controllo'); "">
 ...
 <%
    ...
    TipoCampione.setJsEvent("onChange=javascript:reloadAddCU('OperatoriprivatiVigilanza.do?command=Add&orgId="+OrgDetails.getOrgId()+"&tipoCampione='+this.value)"); 
