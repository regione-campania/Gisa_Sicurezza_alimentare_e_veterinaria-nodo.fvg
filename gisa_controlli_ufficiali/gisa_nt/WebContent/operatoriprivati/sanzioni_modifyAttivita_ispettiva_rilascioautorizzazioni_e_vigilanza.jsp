<%-- 
  ...
  -
  - Version: $Id: operatoriprivati_tickets_modify.jsp 18488 2007-01-15 20:12:32Z matt $
  - Description:
  --%>
...
<td>
  ...
  <a href="OperatoriPrivati.do?command=Details&altId=<%=OrgDetails.getAltId()%>"><dhv:label name="operatoriprivati.details">Dettaglio Operatore Privato</dhv:label></a> > 
...
<%} %> 
  ...
  <a href="OperatoriPrivatiSanzioni.do?command=TicketDetails&id=<%=TicketDetails.getId()%>"><dhv:label name="sanzioni.dettagli">Scheda Sanzione</dhv:label></a> >
  <%}%>
...
<dhv:container name="operatoriprivati" selected="vigilanza" object="OrgDetails" param='<%= "altId=" + OrgDetails.getAltId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId") %>'>
  <dhv:container name="operatoriprivatisanzioni" selected="details" object="TicketDetails" param='<%= "id=" + TicketDetails.getId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId") %>'>
    <%--@ include file="operatoriprivati_ticket_header_include.jsp" --%>
