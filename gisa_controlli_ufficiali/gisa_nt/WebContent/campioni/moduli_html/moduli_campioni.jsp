...
<jsp:useBean id="Modulo" class="org.aspcfs.modules.campioni.base.ModuloCampione" scope="request"/>
...
<jsp:useBean id="specieVegetali" class="org.aspcfs.utils.web.LookupList" scope="request"/>
...
<form method="post" name="myform" id="myform" action="PrintModulesHTML.do?command=ViewModules">
	...
	<% if(OrgCampione.getTipoModulo().equals("2")) { %>
		...
		<%@ include file="/campioni/moduli_html/mod2_batter.jsp" %>
	
	<% } if(OrgCampione.getTipoModulo().equals("3")){ %>
	 	
	 	<%@ include file="/campioni/moduli_html/mod3_chimico.jsp" %>
	 	
	<% } if(OrgCampione.getTipoModulo().equals("4")){ %>
 	
 	<%@ include file="/campioni/moduli_html/mod4_fisico.jsp" %>
 	
	<% }  if(OrgCampione.getTipoModulo().equals("1")){ %>
	
		<%@ include file="/campioni/moduli_html/mod1_molluschi.jsp" %>
		
	<% } if(OrgCampione.getTipoModulo().equals("14")){ %>
	 	
	 	<%@ include file="/campioni/moduli_html/mod14_trichine.jsp" %>
		
	<% }  if(OrgCampione.getTipoModulo().equals("19") && (anno_corrente == Integer.parseInt(Anno)) ){ %> 
	
		<%@ include file="/campioni/moduli_html/prelievo_pnaa_1517.jsp" %>
		
	<% }if(OrgCampione.getTipoModulo().equals("19") && (anno_corrente != Integer.parseInt(Anno)) ){ %>
		
		<%@ include file="/campioni/moduli_html/prelievo_pnaa.jsp" %>
