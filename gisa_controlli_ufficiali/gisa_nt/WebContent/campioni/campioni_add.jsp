...
<td colspan="2">
 <jsp:include page="../campioni/preaccettazione_list.jsp">
    <jsp:param name="preaccLinea" value="<%=OrgDetails.getOrgId()+"orgIdorganization"+OrgDetails.getOrgId()+OrgDetails.getTipologia() %>" />
 </jsp:include>
 </td>
 </tr>

<%@page import="com.daffodilwoods.daffodildb.server.serversystem.User"%>
...
$( document ).ready(function (){
	calenda('assignedDate',$('#dataC').val(),'0');
});
...
function listaCodiciPreaccettazione(){
		...
		var identificativoOSA = document.getElementById("preaccRiferimentoId").value + "orgIdorganization" + document.getElementById("preaccIdLinea").value + document.getElementById("preaccTipologiaOperatore").value; 
		var idEnte =  "1";
...
function associaCodiceAllOsa(idCodicePreacc){
	...
	setPreaccettazione(idCodicePreacc, riferimentoId, "orgId", "organization", idLinea, tipologiaOperatore, userIdPreacc);
...
<input type="hidden" id ="preaccRiferimentoId" name="preaccRiferimentoId" value="<%=OrgDetails.getOrgId()%>" >
<input type="hidden" id ="preaccIdLinea" name="preaccIdLinea" value="<%=CU.getId_imprese_linee_attivita()>0 ? CU.getId_imprese_linee_attivita() : OrgDetails.getOrgId()  %>" >
...
<input type="hidden" id="dataC" name="dataC" value="<%=dataCParsed%>"/>
  ...
  <tr class="containerBody">
    ...
    <td>
    	<input class="date_picker" type="text" id="assignedDate" name="assignedDate" size="10" />
...
<%-- onClick="window.location.href='<%=CU.getURlDettaglio(OrgDetails.getTipologia(),"") %>command=TicketDetails&id=<%= request.getAttribute("idC") %>&orgId=<%=OrgDetails.getOrgId()%>';this.form.dosubmit.value='false';" /> --%>
