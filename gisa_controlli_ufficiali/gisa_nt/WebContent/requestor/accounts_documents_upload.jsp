...
<%@ page import="java.util.*,org.aspcfs.modules.requestor.base.*,com.zeroio.iteam.base.*,org.aspcfs.modules.base.Constants" %>
...
<form method="post" name="inputForm" action="RequestorDocuments.do?command=Upload<%= addLinkParams(request, "popup|popupType|actionId|actionplan") %>" enctype="multipart/form-data" onSubmit="return checkFileForm(this);">
...
<a href="RequestorDocuments.do?command=View&orgId=<%= OrgDetails.getOrgId() %>"><dhv:label name="requestor.requestor_documents_details.Documents">Documents</dhv:label></a> >
<dhv:label name="requestor.requestor_documents_upload.UploadDocument">Upload Document</dhv:label>
</td>
</tr>
</table>
<%-- End Trails --%>
</dhv:evaluate>
<dhv:container name="requestor" selected="documents" object="OrgDetails" hideContainer='<%= "true".equals(request.getParameter("actionplan")) %>' param='<%= "orgId=" + OrgDetails.getOrgId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId|actionplan") %>'>
<table border="0" cellpadding="4" cellspacing="0" width="100%">
  <tr class="subtab">
    <td>
      <% String documentLink = "RequestorDocuments.do?command=View&orgId="+OrgDetails.getOrgId()+ addLinkParams(request, "popup|popupType|actionId|actionplan"); %>
  ...
  <input type="submit" value="Annulla" onClick="javascript:this.form.dosubmit.value='false';this.form.action='RequestorDocuments.do?command=View&orgId=<%= OrgDetails.getOrgId() %>&folderId=<%= (String)request.getAttribute("folderId") %><%= addLinkParams(request, "popup|popupType|actionId|actionplan") %>';" />
</dhv:container>
</form>
</body>
