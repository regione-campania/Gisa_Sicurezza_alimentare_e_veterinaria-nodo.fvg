...
<%@ page import="java.util.*,org.aspcfs.modules.zonecontrollo.base.*, org.aspcfs.modules.base.*" %>
...
<%@ include file="zonecontrollo_list_menu.jsp" %>
...
<table cellpadding="8" cellspacing="0" border="0" width="100%" class="pagedList">
  <tr>
    <th nowrap <% ++columnCount; %>>
      <%--strong><a href="LaboratoriHACCP.do?command=Search&column=o.ragione_sociale"--%><dhv:label name="">Descrizione luogo di controllo</dhv:label><%--/a></strong>
...
%>
  <tr class="row<%= rowid %>">
	<td>
      <a href="ZoneControllo.do?command=Details&orgId=<%=thisOrg.getOrgId()%>"><%= toHtml(thisOrg.getAlert()) %></a>
	</td>
    <td>
	...
	<% Iterator iaddress = thisOrg.getAddressList().iterator();
       ...
       <td>
          ...
          while (iaddress.hasNext()) {
        	  ...
        	  org.aspcfs.modules.zonecontrollo.base.OrganizationAddress thisAddress = (org.aspcfs.modules.zonecontrollo.base.OrganizationAddress)iaddress.next(); 
...
<%} else {%>
  <tr class="containerBody">
    <td colspan="<%= SearchOrgListInfo.getSearchOptionValue("searchcodeOrgSiteId").equals(String.valueOf(Constants.INVALID_SITE))?columnCount+1:columnCount %>">
      <dhv:label name="">Nessun Zona di Controllo trovato.</dhv:label><br />
      <a href="ZoneControllo.do?command=SearchForm"><dhv:label name="">Modifica Ricerca</dhv:label></a>.
    </td>
  </tr>
<%}%>
</table>
