...
<form name="addticket" action="ZoneControlloCampioni.do?command=Insert&auto-populate=true" method="post">
<% request.setAttribute("isZona di Controllo", "si"); %>
<%-- Trails --%>
...
<td>
  ...
  <a href="ZoneControlloVigilanza.do?command=ViewVigilanza&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label name="">Controlli Ufficiali</dhv:label></a> >
...
<%-- <input type="button" value="Annulla" onClick="window.location.href='ZoneControlloVigilanza.do?command=TicketDetails&id=<%= request.getAttribute("idC") %>&orgId=<%=OrgDetails.getOrgId()%>';this.form.dosubmit.value='false';" /> --%>
<br>
<dhv:formMessage />
...
<jsp:useBean id="OrgList" class="org.aspcfs.modules.zonecontrollo.base.OrganizationList" scope="request"/>
...
<input type="hidden" id="orgIdC" name="orgIdC" value="<%=OrgDetails.getOrgIdC()%>" />
