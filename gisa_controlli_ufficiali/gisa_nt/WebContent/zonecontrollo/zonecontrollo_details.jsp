...
<%@ page import="java.util.*,java.text.DateFormat,org.aspcfs.modules.zonecontrollo.base.*,org.aspcfs.modules.contacts.base.*, org.aspcfs.modules.base.Constants" %>
...
<a href="ZoneControllo.do?command=Search&tipoRicerca=<%= request.getAttribute("tipoRicerca")%>"><dhv:label name="">Risultati Ricerca</dhv:label></a> >
<%} else if (request.getParameter("return").equals("dashboard")) {%>
<%}--%>
<dhv:label name="">Scheda Zone di Controllo</dhv:label>
</td>
</tr>
</table>
<%-- End Trails --%>
...
<dhv:container name="<%= !isPopup(request) ? "zonecontrollo" : "" %>" selected="details" object="OrgDetails" param="<%= param1 %>" appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId") %>' hideContainer="<%= OrgDetails.isTrashed() %>">
<input type="hidden" name="orgId" value="<%= OrgDetails.getOrgId() %>"> 

<%if (!isPopup(request)) {%>
<dhv:evaluate if="<%=OrgDetails.isTrashed()%>">
    <dhv:permission name="zonecontrollo-edit">
      <input type="button" value="Ripristina"	onClick="javascript:window.location.href='ZoneControllo.do?command=Restore&orgId=<%= OrgDetails.getOrgId() %>';">
    </dhv:permission>
</dhv:evaluate>

<dhv:evaluate if="<%=!OrgDetails.isTrashed()%>">
 
    <dhv:permission name="zonecontrollo-edit"><input type="button"  value="Modifica"	onClick="javascript:window.location.href='ZoneControllo.do?command=Modify&orgId=<%= OrgDetails.getOrgId() %><%= addLinkParams(request, "popup|actionplan") %>';"></dhv:permission>
 
  <dhv:evaluate if='<%= (request.getParameter("actionplan") == null) %>'>
    <dhv:permission name="zonecontrollo-delete"><input type="button" value="<dhv:label name="">Elimina</dhv:label>" onClick="javascript:popURLReturn('ZoneControllo.do?command=ConfirmDelete&id=<%=OrgDetails.getOrgId()%>&popup=true','ZoneControllo.do?command=Search', 'Delete_account','320','200','yes','no');"></dhv:permission>
  	
  </dhv:evaluate>
</dhv:evaluate>
<dhv:permission name="zonecontrollo-edit,zonecontrollo-delete"><br>&nbsp;</dhv:permission>
<%} %>
...
<jsp:include page="../gestionecodicesinvsa/gestione_codice_sinvsa.jsp">
	<jsp:param name="action" value="ZoneControllo" />
	<jsp:param name="riferimentoId" value="<%=OrgDetails.getOrgId() %>" />
	<jsp:param name="riferimentoIdNomeTab" value="organization" />
 ...
 <jsp:include page="../schede_centralizzate/iframe.jsp">
    <jsp:param name="objectId" value="<%=OrgDetails.getOrgId() %>" />
     <jsp:param name="tipo_dettaglio" value="42" />
     </jsp:include>
...
<dhv:evaluate if="<%=!OrgDetails.isTrashed()%>">
    ...
    <%--dhv:permission name="zonecontrollo-edit">
      <input type="button" value="<dhv:label name="global.button.Enable">Enable</dhv:label>" 	onClick="javascript:window.location.href='ZoneControllo.do?command=Enable&idFarmacie=<%= OrgDetails.getOrgId() %>';">
    </dhv:permission--%>
