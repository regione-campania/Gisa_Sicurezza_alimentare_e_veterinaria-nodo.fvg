...
<form name="details" action="ZoneControlloProvvedimenti.do?command=ModifyTicket&auto-populate=true" method="post">
...
<td>
<td>
  <a href="ZoneControllo.do">Operatori</a> > 
  <a href="ZoneControllo.do?command=Search"><dhv:label name="accounts.SearchResults">Search Results</dhv:label></a> >
  <a href="ZoneControllo.do?command=Details&orgId=<%=OrgDetails.getOrgId()%>">Scheda Operatore Abusivo</a> >
  <a href="ZoneControllo.do?command=ViewVigilanza&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label name="">Controlli Ufficiali</dhv:label></a> >
  <a href="ZoneControlloVigilanza.do?command=TicketDetails&id=<%= request.getAttribute("idC")%>&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label name="">Controllo Ufficiale</dhv:label></a> >
   <%--a href="ZoneControllo.do?command=ViewCampioni&orgId=<%=TicketDetails.getOrgId()%>"><dhv:label name="campioni">Tickets</dhv:label></a> --%>
...
%>
<dhv:container name="zonecontrollo" selected="vigilanza" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId() %>' 
	>
	<%
	...
	%>
		...
		<input type="button"
			value="Ripristina"
			onClick="javascript:this.form.action='ZoneControlloProvvedimenti.do?command=Restore&id=<%= TicketDetails.getId()%>';submit();">
	...
	<dhv:permission name="reopen-reopen-view">
		<input type="button"
			...
			onClick="javascript:this.form.action='ZoneControlloProvvedimenti.do?command=ReopenTicket&id=<%= TicketDetails.getId()%><%= (defectCheck != null && !"".equals(defectCheck.trim())?"&defectCheck="+defectCheck:"") %>';submit();">
	</dhv:permission>
	<%
		} else {
	%>
...
if(request.getAttribute("Messaggio2")!=null)
{
	...
	function doSubmit(idTicket){
		...
		document.details.action="ZoneControlloProvvedimenti.do?command=Chiudi&id="+idTicket+"&chiudiCu=true'"
