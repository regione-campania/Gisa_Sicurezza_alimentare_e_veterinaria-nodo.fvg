...
<jsp:useBean id="OrgListOp" class="org.aspcfs.modules.zonecontrollo.base.OrganizationList" scope="request"/>
...
<script language="JavaScript" TYPE="text/javascript" SRC="javascript/controlli_ufficiali_zonecontrollo.js"></script>
...
function openPopupDetails(orgId,tipo){
			...
			var result;
				...
				window.open('ZoneControllo.do?command=Details&orgId='+orgId+'&popup=true','popupSelect',
...
<form name="details" action="ZoneControlloVigilanza.do?command=ModifyTicket&auto-populate=true" method="post">
<table class="trails" cellspacing="0">
<tr>
<td><u>
  <a href="ZoneControllo.do">Zone di Controllo</a></u> > 
  <u><a href="ZoneControllo.do?command=Search"><dhv:label name="">Risultati Ricerca</dhv:label></a> </u> >
  <u><a href="ZoneControllo.do?command=Details&orgId=<%=TicketDetails.getOrgId()%>">Scheda Zone di Controllo</a></u> >
  <u><a href="ZoneControlloVigilanza.do?command=ViewVigilanza&orgId=<%=TicketDetails.getOrgId()%>"><dhv:label name="vigilanza">Tickets</dhv:label></a></u> >
...
%>
<dhv:container name="zonecontrollo" selected="vigilanza" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId() %>' hideContainer='<%= isPopup(request) || (defectCheck != null && !"".equals(defectCheck.trim())) %>'>
...
<table cellpadding="4" cellspacing="0" width="100%" class="empty">
  ...
  <tr>
   ...
   <td>
      ...
      <a href="CheckListZoneControllo.do?command=View&id=<%= thisAudit.getId()%>&aggiorna=<%=thisAudit.isAggiornaCategoria() %>"><%= toHtml(OrgCategoriaRischioList.getSelectedValue(thisAudit.getTipoChecklist())) %></a>
	</td>
	<%}%>
...
<%--} --%>
	...
	<table cellpadding="5" cellspacing="0" width="100%" class="details" style="display:none">
		<th colspan="5" style="background-color: rgb(204, 255, 153);" >
			<strong>
				<dhv:label name=""><center>Lista Zone Controllate</center></dhv:label>
		    </strong>
		</th>
	    <tr>
   ...
   if (op_p.hasNext()) {
     while (op_p.hasNext()) {
    	 org.aspcfs.modules.zonecontrollo.base.Organization thisOpP = (org.aspcfs.modules.zonecontrollo.base.Organization)op_p.next();
    %> 
   <tr>
    <td>
    	<%= thisOpP.getName() %>
    ...
    <td>
    	<%= toHtml2( thisOpP.getAlert()) %>
    </td>
    <td>
	  <%= thisOpP.getAddressList().getAddress(5).getCity() %>
	</td>
...
<%-- <td>
 ...
 <table cellpadding="5" cellspacing="0" width="100%" class="details">
		<tr><th style="background-color: rgb(204, 255, 153);" >Lista Animali Rilevati</th></tr>
	    <tr><th>Microchip / Identificativo</th></tr>
    <%
   for (int i = 0; i<identificativiList.size(); i++){
    	 String thisIdentificativo = (String)identificativiList.get(i);
    %> 
    	<tr><td><%= thisIdentificativo %></td></tr>
    <% } %>
 </table>	
