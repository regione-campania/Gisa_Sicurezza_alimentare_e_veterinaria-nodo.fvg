...
<form name="details" action="DistributoriReati.do?command=UpdateTicket&idMacchinetta=<%=idMacchinetta %>&auto-populate=true<%= addLinkParams(request, "popup|popupType|actionId") %>" onSubmit="return checkForm(this);" method="post">
...
<td>
  ...
  <a href="Distributori.do?command=Details&idMacchinetta=<%=idMacchinetta %>&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label name="">Scheda Distributore Automatico</dhv:label></a> > 
  <a href="Distributori.do?command=ViewTickets&idMacchinetta=<%=idMacchinetta %>&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label name="reati">Controlli Ufficiali</dhv:label></a> >
...
<%} %>
  ...
  <a href="DistributoriReati.do?command=TicketDetails&idMacchinetta=<%=idMacchinetta %>&id=<%=TicketDetails.getId()%>"><dhv:label name="reati.dettagli">Scheda note di reato</dhv:label></a> >
  <%}%>
...
<dhv:container name="distributori" selected="reati" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId()+"&id="+idMacchinetta %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId") %>'>
    <%--@ include file="accounts_ticket_header_include.jsp" --%>
     <dhv:evaluate if="<%= !TicketDetails.isTrashed() %>" >
      <dhv:evaluate if="<%= TicketDetails.getClosed() != null %>" >
          <dhv:permission name="distributori-distributori-reati-edit">
            <input type="submit" value="<dhv:label name="button.reopen">Reopen</dhv:label>" onClick="javascript:this.form.action='DistributoriReati.do?command=ReopenTicket&idMacchinetta=<%=idMacchinetta %>&id=<%=TicketDetails.getId()%><%= addLinkParams(request, "popup|popupType|actionId") %>'">
           </dhv:permission>
           ...
           <dhv:evaluate if='<%= !"list".equals(request.getParameter("return"))%>' >
            <input type="submit" value="Annulla" onClick="javascript:this.form.action='DistributoriReati.do?command=TicketDetails&idMacchinetta=<%=idMacchinetta %>&id=<%= TicketDetails.getId() %><%= addLinkParams(request, "popup|popupType|actionId") %>'" />
           </dhv:evaluate>
      </dhv:evaluate>
      <dhv:evaluate if="<%= TicketDetails.getClosed() == null %>" >
          <dhv:permission name="distributori-distributori-reati-edit">
