...
<script language="javascript">
  ...
  //Menu link functions
  function details() {
    window.location.href = 'AccountsServiceContractsAbus.do?command=View&orgId=' + thisOrgId + '&id=' + thisContractId+'<%= addLinkParams(request, "popup|popupType|actionId") %>';
  }

  function modify() {
    window.location.href = 'AccountsServiceContractsAbus.do?command=Modify&orgId=' + thisOrgId + '&id=' + thisContractId + '&return=list<%= addLinkParams(request, "popup|popupType|actionId") %>';
  }

  function deleteContract() {
    popURLReturn('AccountsServiceContractsAbus.do?command=ConfirmDelete&orgId=' + thisOrgId + '&id=' + thisContractId + '&popup=true<%= isPopup(request)?"&popupType=inline":"" %>','AccountsServiceContractsAbus.do?command=List&orgId=' + thisOrgId,'Delete_servicecontract','330','200','yes','no');
  }
...
<div id="menuServiceContractContainer" class="menu">
  <div id="menuServiceContractContent">
    <table id="menuServiceContractTable" class="pulldown" width="170" cellspacing="0">
      <dhv:permission name="abusivismi-service-contracts-view">
      ...
      <dhv:permission name="abusivismi-service-contracts-edit">
      ...
      <dhv:permission name="abusivismi-service-contracts-delete">
