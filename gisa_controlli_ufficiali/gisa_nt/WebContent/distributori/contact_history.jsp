<%-- 
  ...
  - 
  - Version: $Id: contact_history.jsp 18488 2007-01-15 20:12:32Z matt $
  - Description:
  --%>
<%@ taglib uri="/WEB-INF/dhv-taglib.tld" prefix="dhv" %>
<%@ taglib uri="/WEB-INF/zeroio-taglib.tld" prefix="zeroio" %>
<%@ page import="java.util.*,org.aspcfs.modules.abusivismi.base.*,org.aspcfs.modules.contacts.base.*" %>
...
<jsp:useBean id="historyList" class="org.aspcfs.modules.abusivismi.base.OrganizationHistoryList" scope="request"/>
...
<SCRIPT LANGUAGE="JavaScript" type="text/javascript">
  ...
  function reopen() {
    window.location.href='AccountContactsHistoryAbus.do?command=View&contactId = <%= ContactDetails.getId() %>';
  }
</script>
<%-- Trails --%>
<table class="trails" cellspacing="0">
 <tr><td>
  <a href="Distributori.do">Operatori Abusivi</a> > 
  <a href="Distributori.do?command=Search"><dhv:label name="accounts.SearchResults">Search Results</dhv:label></a> >
  <a href="Distributori.do?command=Details&orgId=<%=OrgDetails.getOrgId()%>">Scheda</a> >
  <a href="ContactsAbus.do?command=View&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label name="accounts.Contacts">Contacts</dhv:label></a> >
  <a href="ContactsAbus.do?command=Details&id=<%=ContactDetails.getId()%>&orgId=<%=OrgDetails.getOrgId()%>"><dhv:label name="abusivismi.abusivismi_contacts_add.ContactDetails">Contact Details</dhv:label></a> >
  <dhv:label name="reports.helpdesk.ticket.history.history">History</dhv:label>
 </td></tr>
</table>
<%-- End Trails --%>
<dhv:container name="abusivismi" selected="history" object="OrgDetails" param='<%= "orgId=" + OrgDetails.getOrgId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId") %>'>
  <dhv:container name="accountscontactsabus" selected="history" object="ContactDetails" param='<%= "id=" + ContactDetails.getId() %>' appendToUrl='<%= addLinkParams(request, "popup|popupType|actionId") %>'>
<form name="history" action="AccountContactsHistoryAbus.do?command=View&contactId=<%= ContactDetails.getId() %>" method="post" onSubmit="return checkForm(this);">
<input type="hidden" name="searchcodeContactId" value="<%= ContactDetails.getId() %>"/>
  <table cellpadding="4" cellspacing="0" width="100%" class="empty">
    <tr>
      <td>
        <a href=" javascript:popURL('AccountContactsHistoryAbus.do?command=AddNote&contactId=<%= ContactDetails.getId() %>','Note','575','200','yes','yes');" ><dhv:label name="accounts.accountHistory.addANote">Add a Note</dhv:label></a>
      </td>
      <td>
        <table cellpadding="4" cellspacing="0" class="empty">
          <tr>
            ...
            <td><input type="checkbox" name="searchcodeActivities" value="true" <%= accountContactHistoryListInfo.getSearchOptionValue("searchcodeActivities").equals("true")?"checked":"" %> /> <dhv:label name="abusivismi.abusivismi_calls_list.Activities">Activities</dhv:label></td>
            <td><input type="checkbox" name="searchcodeEmail" value="true" <%= accountContactHistoryListInfo.getSearchOptionValue("searchcodeEmail").equals("true")?"checked":"" %> /> <dhv:label name="abusivismi.abusivismi_add.Email">Email</dhv:label></td>
            ...
            <td><input type="checkbox" name="searchcodeOpportunities" value="true" <%= accountContactHistoryListInfo.getSearchOptionValue("searchcodeOpportunities").equals("true")?"checked":"" %> /> <dhv:label name="abusivismi.abusivismi_contacts_oppcomponent_add.Opportunities">Opportunities</dhv:label></td>
  ...
  <table cellpadding="4" cellspacing="0" width="100%" class="pagedList">
    <tr>
      ...
      <th nowrap>
        <strong><a href="AccountContactsHistoryAbus.do?command=View&contactId = <%= ContactDetails.getId() %>&column=modified"><dhv:label name="admin.dateTime">Date/Time</dhv:label></a></strong>
        <%= accountContactHistoryListInfo.getSortIcon("modified") %>
      </th>
      <th nowrap>
        <strong><a href="AccountContactsHistoryAbus.do?command=View&contactId = <%= ContactDetails.getId() %>&column=contact_id"><dhv:label name="accounts.accountasset_include.Contact">Contact</dhv:label></a></strong>
        <%= accountContactHistoryListInfo.getSortIcon("contact_id") %>
      <th nowrap>
        <strong><a href="AccountContactsHistoryAbus.do?command=View&contactId = <%= ContactDetails.getId() %>&column=type"><dhv:label name="reports.accounts.type">Type</dhv:label></a></strong>
