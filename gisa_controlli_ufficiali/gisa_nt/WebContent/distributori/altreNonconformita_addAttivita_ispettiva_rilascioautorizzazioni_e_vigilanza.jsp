...
if (request.getAttribute("idMacchinetta")!=null)
	{
	idMacchinetta=(String)request.getAttribute("idMacchinetta");
	
	} %>
<form name="addticket" action="DistributoriNonConformita.do?command=Insert&auto-populate=true&idMacchinetta=<%=idMacchinetta %>" method="post">
<%-- Trails --%>
...
<input type="button" value="Annulla" onClick="if(confirm('ATTENZIONE: si sta per abbandonare la pagina di aggiunta di una non conformita. Lasciando questa pagina il lavoro svolto finora verra perso.Sicuro di voler uscire ?')==true){window.location.href='DistributoriVigilanza.do?command=TicketDetails&id=<%= OrgDetails.getId() %><%= addLinkParams(request, "popup|popupType|actionId") %>&idmacchinetta=<%=idMacchinetta %>';this.form.dosubmit.value='0';}" />
