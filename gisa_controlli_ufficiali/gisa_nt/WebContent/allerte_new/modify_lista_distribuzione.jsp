...
<form name="addticket"  action="TroubleTicketsAllerteNew.do?command=UpdateListaDistribuzione&auto-populate=true" method="post">
<input type="hidden" id="id" name="id" value="<%=ListaDistribuzione.getId_allerta()%>"/>
<%-- Trails --%>
...
<table cellpadding="4" cellspacing="0" width="100%" class="details">
 ...
 <tr id="tr_datalista">
  ...
  <td>
  <input class="editField date_picker" type="text" id="data_lista" name="data_lista" size="10" value="<%=toDateasString(ListaDistribuzione.getData_lista())%>"/>
	<font color="red">*</font>
 </td></tr>
...
<dhv:evaluate if="<%= SiteIdList.size() > 1 %>">
  ...
  <tr class="containerBody">
    ...
    <td>
        ...
        while(it.hasNext()){
        	...
        	if(asl!=null){
        		...
        		<tr>
        			...
        			<input type="checkbox" name="asl_coinvolte" id="asl_<%=code %>" checked="checked" disabled="disabled" onclick="abilitaCU(<%=code %>)" value="<%=code %>"></td><td id="int_<%=code %>" style="display:block">&nbsp;Numero C.U.&nbsp;</td><td id="cu_<%=code %>" style="display:none"><input type="text" name="cu_<%=code %>"  id="cucampo_<%=code %>" value="<%=asl.getControlliUfficialiRegionaliPianificati() %>"></td>
        ...
        <tr><td><%=descr %>&nbsp;</td><td>
<!--        <input type = "hidden" name = "asl_coinvolte" value ="<%=code %>">-->
        <input type="checkbox" name="asl_coinvolte" id="asl_<%=code %>" checked="checked" onclick="abilitaCU(<%=code %>)" value="<%=code %>"></td><td id="int_<%=code %>" style="display:block">&nbsp;Numero C.U.&nbsp;</td><td id="cu_<%=code %>" ><input type="text" name="cu_<%=code %>" id="cucampo_<%=code %>" value="<%=asl.getControlliUfficialiRegionaliPianificati() %>"></td>
        <td>&nbsp;</td>
...
<input type="hidden" name="idAllerta" id="idAllerta" value="<%=ListaDistribuzione.getId_allerta() %>" />
...
<input type="submit" value="Annulla" onClick="javascript:this.form.action='TroubleTicketsAllerteNew.do?command=Details&id=<%=ListaDistribuzione.getId_allerta()%>'">
