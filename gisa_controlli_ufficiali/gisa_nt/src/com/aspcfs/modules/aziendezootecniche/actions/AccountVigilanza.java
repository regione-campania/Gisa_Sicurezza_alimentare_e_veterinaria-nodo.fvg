...
public final class AccountVigilanza extends CFSModule {
	...
	public String executeCommandAdd(ActionContext context) {
		...
		try {
			...
			IstanzaProduttiva newStabilimento = new IstanzaProduttiva();
			newStabilimento.queryRecord(db, altid);
			...
			context.getRequest().setAttribute("siteId", newStabilimento.getAzienda().getIdAsl());
			context.getRequest().setAttribute("tipologia", org.aspcfs.modules.troubletickets.base.Ticket.TIPO_OPU_RICHIESTE);
	...
	public String executeCommandInsert(ActionContext context) throws SQLException, ParseException {
		...
		try
		{
			...
			context.getRequest().setAttribute("name", newStabilimento.getAllevamento().getRagioneSociale());
	...
	public String executeCommandUpdateTicket(ActionContext context) {
		...
		try {
			...
			context.getRequest().setAttribute("siteId",newStabilimento.getAzienda().getIdAsl());
			context.getRequest().setAttribute("tipologia",org.aspcfs.modules.troubletickets.base.Ticket.ALT_OPU_RICHIESTE);
			org.aspcf.modules.controlliufficiali.action.AccountVigilanza controlliGeneric = new org.aspcf.modules.controlliufficiali.action.AccountVigilanza();
	...
	public String executeCommandConfirmDelete(ActionContext context) {
		...
		{	  
			db = this.getConnection(context);
		      ...
		      IstanzaProduttiva newStabilimento = new IstanzaProduttiva();
				newStabilimento.queryRecord(db, ticket.getAltId());
				newStabilimento.getPrefissoAction(context.getAction().getActionName());
				context.getRequest().setAttribute("OrgDetails", newStabilimento);
				
			     
			     org.aspcf.modules.controlliufficiali.action.AccountVigilanza controlliGeneric = new org.aspcf.modules.controlliufficiali.action.AccountVigilanza();
			context.getRequest().setAttribute("url_confirm", newStabilimento.getAction()+"Vigilanza.do?command=DeleteTicket&id="+ticket.getId()+"&altId="+ticket.getAltId());
	...
	public String executeCommandDeleteTicket(ActionContext context) {
		...
		try {
			...
			org.aspcf.modules.controlliufficiali.action.AccountVigilanza controlliGeneric = new org.aspcf.modules.controlliufficiali.action.AccountVigilanza();
			context.getRequest().setAttribute("url", newStabilimento.getAction()+".do?command=ViewVigilanza&altId="+newStabilimento.getAltId());
			...
			if(thisTicket.getTipoCampione()==5){
				...
				if(!itAudit.hasNext()){
					...
					while(itAudit.hasNext()){
						...
						IstanzaProduttiva newStabilimento = new IstanzaProduttiva();
						newStabilimento.queryRecord(db, thisTicket.getAltId());
						newStabilimento.getPrefissoAction(context.getAction().getActionName());
						context.getRequest().setAttribute("OrgDetails", newStabilimento);
