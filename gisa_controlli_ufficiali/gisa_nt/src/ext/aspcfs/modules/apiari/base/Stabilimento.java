...
public class Stabilimento extends GenericBean {
	...
	Logger logger = Logger.getLogger(Stabilimento.class);
	...
	private Timestamp dataSincronizzazioneCessazione ;
	...
	public String getAslRomaBdn() {
		return aslRomaBdn;
	}



	public void setAslRomaBdn(String aslRomaBdn) {
		this.aslRomaBdn = aslRomaBdn;
	}
	...
	public void setCapacita(String capacita) {
		if (!"".equals(capacita))
			this.capacita = Integer.parseInt(capacita);
	}
	...
	public Timestamp getDataSincronizzazioneCessazione() {
		return dataSincronizzazioneCessazione;
	}



	public void setDataSincronizzazioneCessazione(Timestamp dataSincronizzazioneCessazione) {
		this.dataSincronizzazioneCessazione = dataSincronizzazioneCessazione;
	}

	public void setDataSincronizzazioneCessazione(String dataSincronizzazioneCessazione) throws ParseException {
		if (!"".equals(dataSincronizzazioneCessazione))
		{
			SimpleDateFormat sdf =new SimpleDateFormat("dd/MM/yyyy");
			this.dataSincronizzazioneCessazione = new Timestamp(sdf.parse(dataSincronizzazioneCessazione).getTime());
		}
	}
	...
	public Stabilimento(Api apiario , Connection db,ActionContext context) throws SQLException{
		...
		if(apiario.getCapacita()!=null)
			this.setCapacita(apiario.getCapacita());
		}
	...
	public Stabilimento(Connection db, int idStabilimento) {
	...
	public Stabilimento(Connection db, int idStabilimento,boolean flagOperatore) {
	...
	public void update(Connection db,ActionContext context)
	{
		...
		String sql = "update apicoltura_apiari set id_indirizzo=?,modified=current_timestamp,modified_by=? ,num_sciami=?, num_alveari=?,data_inizio = ?,id_apicoltura_lookup_classificazione=?,id_apicoltura_lookup_sottospecie=?,id_apicoltura_lookup_modalita=?,id_soggetto_fisico=?,num_pacchi=?,num_regine=? " ;
	...
	public boolean insert (Connection db, boolean insertLinee,ActionContext context) throws Exception
	{
		...
		try {
			...
			sql.append("INSERT INTO apicoltura_apiari (id_bda,flag_laboratorio,id_apicoltura_lookup_sottospecie,id_apicoltura_lookup_classificazione,id_apicoltura_lookup_modalita,num_alveari,num_sciami,data_inizio,id_asl,progressivo,stato,num_pacchi,num_regine,");
			...
			sql.append("entered,modified,asl_roma_bdn)");
			...
			pst.setString(++i, aslRomaBdn);

			System.out.println("Query inserimento apiario: " + pst.toString());
			...
			if (doCommit) {
				System.out.println("[DIAGNOSTICA APIARI CHE SI PERDONO] ATTENZIONE: FATTO COMMIT. ID APIARIO: " + this.getIdApiario()); 
				db.commit();
			}
		}  
		catch (Exception e) {
			System.out.println("[DIAGNOSTICA APIARI CHE SI PERDONO] ATTENZIONE: ERRORE INSERIMENTO APIARIO. ID APIARIO: " + this.getIdApiario()); 
			e.printStackTrace();
			if (doCommit) {
				db.rollback();
			}
			...
			System.out.println("[DIAGNOSTICA APIARI CHE SI PERDONO] SONO NEL FINALLY " + ""); 
			if (doCommit) {
				System.out.println("[DIAGNOSTICA APIARI CHE SI PERDONO] SETTO AUTOCOMMIT A TRUE " + ""); 
	...
	protected void buildRecord(ResultSet rs) throws SQLException {
		...
		aslRomaBdn = rs.getString("asl_roma_bdn");
	...
	{
		String sql = "update apicoltura_apiari set data_sincronizzata_cessazione = current_timestamp , stato=? where id =?";
	...
	public int nuovoCensimento(Connection db,ActionContext context, Integer idBda)
			throws SQLException {
		...
		StringBuffer sql = new StringBuffer();
			...
			sql.append("INSERT INTO apicoltura_consistenza (id,id_apicoltura_imprese , id_apicoltura_apiario,data_censimento,num_sciami,num_alveari,entered_by,data_assegnazione_censimento,num_pacchi,num_regine, id_bda");
			if(ApplicationProperties.getProperty("flusso356").equals("true")){
				sql.append(",capacita");
				}	
			sql.append("  ) values (");
			...
			System.out.println("QUERY DEL CENSIMENTO CHE STO PER INSERIRE: " + pst);
			pst.execute();

			sql = new StringBuffer();
			sql.append("update apicoltura_apiari set flag_notifica_variazione_censimento=false,num_alveari_effettivi=0,num_alveari = ?,num_sciami=?,modified=current_timestamp,modified_by=?,id_apicoltura_apiari_variazioni_censimenti=?,data_assegnazione_censimento=?,num_pacchi=?,num_regine=? ");
			if(ApplicationProperties.getProperty("flusso356").equals("true")){
			sql.append(",capacita=? ");
			}
			sql.append(" where id =?;");
			...
			System.out.println("INSERIMENTO CENSIMENTO RETURN: " + idVariazioneEseguita );
	...
	public boolean aggiornaDatiCensimenti( Connection db)
			throws SQLException {

		StringBuffer sql = new StringBuffer();
		try 
		{
			sql.append("update apicoltura_apiari set data_assegnazione_censimento = ?, "
					+ " num_alveari = ?, num_pacchi = ?, num_regine = ?, num_sciami = ?, id_apicoltura_apiari_variazioni_censimenti = ? where id = ? ");
			...
			System.out.println("QUERY UPDATE: " + pst.toString());
	...
	public boolean deleteCensimento( Connection db,ActionContext context, StabilimentoVariazioneCensimento censimento)
		...
		try {
			UserBean utente = (UserBean)context.getSession().getAttribute("User");
			sql.append("update apicoltura_consistenza set trashed_by = ?, note = ? where id = ? ");
			idVariazioneEseguita = censimento.getId();
	...
	public Integer getIdUltimoCensimento( Connection db, Stabilimento stab, Integer idCensimento) throws SQLException {
		...
		try 
		{
			sql.append("select * from apicoltura_apiari_variazioni_censimenti where id_apicoltura_apiario = ? and trashed_by is null and ( ? is null or id <> ?) order by data_assegnazione_censimento desc limit 1 ");

			PreparedStatement  pst = db.prepareStatement(sql.toString());

			pst.setInt(1, stab.getIdStabilimento());
			if(idCensimento==null)
			{
			...
			else
			{
				...
				pst.setInt(3, stab.getIdStabilimento());
			}
	...
	public String toString()
	{
		...
		ret+="_capacita_:_"+this.getCapacita()+"_,";
		}
		
		if(ApplicationProperties.getProperty("flusso356_2").equals("true")){
			
			ret+="_alveari_:_"+this.getNumAlveari()+"_,";
			}
	...
	public void sincronizzaBdn(Connection db,int modifiedby, Integer idBda) throws SQLException
	{
		System.out.println("[DIAGNOSTICA APIARI CHE SI PERDONO] INIZIO SINCRONIZZA BDN " + ""); 
		String sql="update apicoltura_apiari set esito_import='OK',descrizione_errore=null, sincronizzato_bdn = true , sincronizzato_da = ?, id_bda=?, data_sincronizzazione = current_timestamp,stato="+StabilimentoAction.API_STATO_VALIDATO+" where id = ? ";
		PreparedStatement pst = db.prepareStatement(sql);
		pst.setInt(1, modifiedby);
		pst.setObject(2, idBda);
	...
	public void aggiornaDatiModalitaApiario(Connection db, String dataModifica, String note, int idUtente) throws SQLException
	{
		int i = 0 ;
		PreparedStatement pst = null;
		pst =db.prepareStatement("update apicoltura_apiari set id_apicoltura_lookup_modalita=? where id =?");
		pst.setInt(++i, idApicolturaModalita);
		pst.setInt(++i, idStabilimento);
		pst.execute();
		 ...
		 i = 0;
		 pst = null;
		pst =db.prepareStatement("insert into apicoltura_apiari_modalita_log (id_apiario, id_apicoltura_modalita, data_modifica, note, id_utente, entered) values (?, ?, ?, ?, ?, now())");
		pst.setInt(++i, idStabilimento);
		pst.setInt(++i, idApicolturaModalita);
		pst.setString(++i, dataModifica);
	...
	public boolean hasControlli(Connection db) {
		int numCu = 0;
		PreparedStatement pst;
		try {
			pst = db.prepareStatement("select count(*) from ticket where id_apiario = ? and trashed_date is null");
	...
	public boolean cancellaBdn(Connection db, int idUtente)  {
		boolean cancellato = false;
		Api apiario = null;
		
		WsApiAnagraficaApi ws = new WsApiAnagraficaApi();
		try {
			apiario = ws.delete(this, db, idUtente);
			cancellato = true;
	...
	public String aggiornaModalitaBdn(Connection db, int userId) {
		String esito = null;
		
		WsApiAnagraficaApi ws = new WsApiAnagraficaApi();
		try {
			//esito= ws.modificaApiAnagraficaApiario(this, db, userId);
			esito= ws.modificaModalitaAllevamento(this, db, userId);
