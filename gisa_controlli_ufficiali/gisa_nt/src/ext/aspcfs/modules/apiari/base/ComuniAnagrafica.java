...
public void setCodice(String codice) {
  ...
  public void queryRecord(Connection db, String comune) throws SQLException {
    ...
    sql.append("SELECT codice,nome FROM comuni1 c WHERE c.nome ilike ? ");
  ...
  public int getIdAslByComune(Connection db, String comune) throws SQLException {
	    PreparedStatement st = null;
	    ResultSet rs = null;
	    StringBuffer sql = new StringBuffer();
	    sql.append("SELECT codiceistatasl FROM comuni1 c WHERE c.nome ilike ? ");
	    st = db.prepareStatement(sql.toString());
	    st.setString( 1, comune );
	    rs = st.executeQuery();
	    
	    if (rs.next()) {
	    	idAsl = Integer.parseInt(rs.getString(1));
	    }
	    rs.close();
	    st.close();
  ...
  public ComuniAnagrafica(Connection db, int idComune) throws SQLException {
	...
	public void queryRecord(Connection db, int idComune) throws SQLException {
	    ...
	    ResultSet rs = null;
	    StringBuffer sql = new StringBuffer();
	    sql.append("SELECT codice,nome FROM comuni1 c WHERE c.id = ? ");
	    st = db.prepareStatement(sql.toString());
...
public static int getIdComune(String descrizioneComune,Connection db) throws SQLException
{
	int idComune = -1 ;
	...
	public static String getCodComune(Connection db, String comune)
	{
		String cod_comune = null;
		
		String query = "select cod_comune from comuni1 where nome = ? ";
		...
		try
		{
			...
			if (rs.next())
			{
				cod_comune = rs.getString("cod_comune");
			}
		} 
		catch (Exception e) 
		{
	...
	public static String getSiglaProvincia(Connection db, String comune)
	{
		String istat = null;
		
		String query = "select p.cod_provincia as cod_provincia from comuni1 c, lookup_province p where c.nome = ? and c.cod_provincia = p.codistat ";
		...
		try
		{
			...
			if (rs.next())
			{
				istat = rs.getString("cod_provincia");
			}
		} 
		catch (Exception e) 
		{
