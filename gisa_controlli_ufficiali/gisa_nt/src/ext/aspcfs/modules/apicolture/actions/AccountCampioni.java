...
public final class AccountCampioni extends CFSModule {
	...
	public String executeCommandReopenTicket(ActionContext context) {
		if (!hasPermission(context, "apicoltura-campioni-edit")) {
	...
	public String executeCommandTicketDetails(ActionContext context) {
		if (!hasPermission(context, "apicoltura-campioni-view")) {
			return ("PermissionError");
		}
		Connection db = null;
		// Parameters
		...
		try {
			...
			Campioni c = new Campioni();
			int orgId= -1;
			String stabId = String.valueOf(context.getRequest().getAttribute("stabId"));
			
			
			context.getRequest().setAttribute("ActionString", context.getAction().getActionName());
			c.executeCommandTicketDetails(context,db);
			if(stabId != null && !stabId.equals("null") && Integer.parseInt(stabId) > 0){
				orgId = Integer.parseInt(stabId);
			}else if (context.getRequest().getParameter("stabId") != null && Integer.parseInt((String)context.getRequest().getParameter("stabId")) >0)
				orgId = Integer.parseInt( context.getRequest().getParameter("stabId"));			
			else
				if (context.getRequest().getAttribute("stabId") != null && Integer.parseInt(context.getRequest().getParameter("stabId")) <= 0)
					orgId = Integer.parseInt( ""+context.getRequest().getAttribute("idApiario"));			

					
			Stabilimento stab = new Stabilimento(db,orgId);
			context.getRequest().setAttribute("OrgDetails", stab);
			

			// find record permissions for portal users
			/*if (!isRecordAccessPermittedOpu(context, db, orgId)) {
				return ("PermissionError");
			}*/
					retPag = "DettaglioOK";
	...
	public String executeCommandConfirmDelete(ActionContext context) {
		if (!hasPermission(context, "apicoltura-campioni-delete")) {
			return ("PermissionError");
		}
		Connection db = null;
		// Parameters
		String id = context.getRequest().getParameter("id");
		int orgId = Integer.parseInt(context.getRequest().getParameter("stabId"));
		try {
			db = this.getConnection(context);
			Stabilimento stab = new Stabilimento(db,orgId);
	...
	public String executeCommandDeleteTicket(ActionContext context) {
		...
		// Parameters
		int orgId = Integer
				.parseInt(context.getRequest().getParameter("stabId"));
		...
		try {
			...
			if (recordDeleted) {
				...
				context
						...
						.setAttribute(
								"refreshUrl",
								stab.getAction()+"Vigilanza.do?command=TicketDetails&id="+id_controllo+"&idStabilimentoopu="
										+ orgId
										+ (inline != null
	...
	public String executeCommandViewHistory(ActionContext context) {
		if (!hasPermission(context, "apicoltura-campioni-history-view")) {
			return ("PermissionError");
		}
		Connection db = null;
		...
		try {
			...
			Stabilimento thisOrganization = new Stabilimento(db, thisTic
					.getIdStabilimento());
	...
	public String executeCommandAdd(ActionContext context) {
		if (!hasPermission(context, "apicoltura-campioni-add")) {
			return ("PermissionError");
		}
		Connection db = null;
		...
		try {
			...
			Stabilimento stab = new Stabilimento(db,tempid);
			Campioni c = new Campioni ();
	...
	public String executeCommandInsert(ActionContext context)
		...
		{
			db = this.getConnection(context);
			Stabilimento thisOrg = null;
			if(context.getRequest().getParameter("stabId") != null && !context.getRequest().getParameter("stabId").equals("-1")){
				 thisOrg = new Stabilimento(db, Integer.parseInt(context.getRequest().getParameter("stabId")));
			}else{
				 thisOrg = new Stabilimento(db, newTic.getIdApiario());
			...
			context.getRequest().setAttribute("stabId",newTic.getIdApiario());
...
context.getRequest().setAttribute("AnimaliNonAlimentari",
	...
	public String executeCommandChiudi(ActionContext context) {
		...
		try {
			...
			context.getRequest().setAttribute("idC", id_controllo);
			context.getRequest().setAttribute("stabId", thisTicket.getIdApiario());
			...
			String ticketId = context.getRequest().getParameter("id");
			 ...
			 tamponiList.buildListControlli(db, pasId, id_controllo);
				      ...
				      Stabilimento org = new Stabilimento(db , thisTicketV.getIdApiario());
			      ...
			      while(itAudit.hasNext()){
					    	  ...
					    	  Stabilimento orgDetails = new Stabilimento(db,thisTicketV.getIdStabilimento());
