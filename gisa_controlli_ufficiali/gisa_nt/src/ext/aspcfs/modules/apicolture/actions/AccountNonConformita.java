...
public final class AccountNonConformita extends CFSModule {
	...
	public String executeCommandReopenTicket(ActionContext context) {
		if (!hasPermission(context, "apicoltura-nonconformita-edit")) {
	...
	public String executeCommandTicketDetails(ActionContext context) {
		if (!hasPermission(context, "apicoltura-nonconformita-view")) {
			return ("PermissionError");
		}
		...
		try {
			...
			org.aspcfs.modules.nonconformita.actions.AccountNonConformitaOpu nonconformita = new org.aspcfs.modules.nonconformita.actions.AccountNonConformitaOpu();
			db = this.getConnection(context);
	...
	public String executeCommandConfirmDelete(ActionContext context) {
		...
		try {
			...
			htmlDialog.addButton(
					systemStatus.getLabel("global.button.delete"), "javascript:window.location.href='"+stab.getAction()+"NonConformita.do?command=DeleteTicket&stabId="+orgId+"&id=" + id + "&orgId=" + orgId + "&forceDelete=true" + RequestUtils.addLinkParams(
	...
	public String executeCommandDeleteTicket(ActionContext context) {
		if (!hasPermission(context, "apicoltura-nonconformita-delete")) {
			return ("PermissionError");
		}
		...
		try {
			...
			if (recordDeleted) {
				...
				.setAttribute(
						"refreshUrl",
						thisOrganization.getAction()+ "Vigilanza.do?command=TicketDetails&id="+id_controllo+"&orgId="
						+ orgId
						+ (inline != null
	...
	public String executeCommandAdd(ActionContext context) {
		if (!hasPermission(context, "apicoltura-nonconformita-add")) {
			return ("PermissionError");
		}
		Connection db = null;
		Ticket newTic = null;
	...
	public String executeCommandInsert(ActionContext context) throws SQLException {
		...
		try {
			db = this.getConnection(context);
			//db.commit();
			Stabilimento thisOrg = new Stabilimento(db, newTic.getIdApiario());
			org.aspcfs.modules.nonconformita.actions.AccountNonConformitaOpu nc = new org.aspcfs.modules.nonconformita.actions.AccountNonConformitaOpu();
