...
public final class AccountTamponi extends CFSModule {
	...
	public String executeCommandReopenTicket(ActionContext context) {
		if (!hasPermission(context, "apicoltura-tamponi-edit")) {
	...
	public String executeCommandTicketDetails(ActionContext context) {
		if (!hasPermission(context, "apicoltura-tamponi-view")) {
			return ("PermissionError");
		}
		String retPag  	= "";
		Connection db 	= null;
		try
		{
			...
			Integer orgId = 0;
			if(context.getRequest().getParameter("stabId") != null && Integer.parseInt((String)context.getRequest().getParameter("stabId")) > 0){
				 orgId = Integer.parseInt(context.getRequest().getParameter("stabId"));
			}else{
				 orgId = Integer.parseInt(String.valueOf(context.getRequest().getAttribute("idApiario")));
			}
...
//			thisOrganization.getPrefissoAction(context.getAction().getActionName());
	...
	public String executeCommandConfirmDelete(ActionContext context) {
		if (!hasPermission(context, "apicoltura-tamponi-delete")) {
			return ("PermissionError");
		}
		Connection db = null;
		// Parameters
...
//			thisOrganization.getPrefissoAction(context.getAction().getActionName());
	...
	public String executeCommandUpdateTicket(ActionContext context) {
		...
		try {
			...
			Integer orgId = (Integer)context.getRequest().getAttribute("idApiario")		;
...
//			thisOrganization.getPrefissoAction(context.getAction().getActionName());
	...
	public String executeCommandAdd(ActionContext context) {
		if (!hasPermission(context, "apicoltura-tamponi-add")) {
			return ("PermissionError");
		}
		Connection db = null;
...
//			thisOrganization.getPrefissoAction(context.getAction().getActionName());
	...
	public String executeCommandInsert(ActionContext context)
		...
		try {
			...
			if(newTic.getIdApiario() > 0 && newTic.getIdStabilimento() > 0){
	...
	public String executeCommandModifyTicket(ActionContext context) {
		...
		try {
			...
			if (context.getParameter("stabId")!=null)
			{
				 ...
				 orgId = (Integer) context.getRequest().getAttribute("idApiario");
			Stabilimento thisOrganization = new Stabilimento(db, orgId);
//			thisOrganization.getPrefissoAction(context.getAction().getActionName());
			context.getRequest().setAttribute("OrgDetails", thisOrganization);
