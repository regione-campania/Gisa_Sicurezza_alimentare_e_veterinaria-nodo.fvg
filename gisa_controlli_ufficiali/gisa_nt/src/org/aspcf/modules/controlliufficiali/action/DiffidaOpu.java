...
public class DiffidaOpu  extends CFSModule {
	  ...
	  public String executeCommandConfirmDelete(ActionContext context) {
	    ...
	    try {
	      ...
	      htmlDialog.addButton(
	          systemStatus.getLabel("global.button.delete"), "javascript:window.location.href='"+context.getAction().getActionName()+".do?command=DeleteTicket&id=" + id + "&stabId=" + orgId + "&forceDelete=true" + RequestUtils.addLinkParams(
	...
	public String executeCommandAdd(ActionContext context) {
		...
		try {
			...
			if (context.getParameter("stabId")==null && context.getParameter("stabId")!=null)
			{
		...
		try {
			...
			if (recordInserted) {
				...
				if (newTic.getIdStabilimento() != -1) {
					Stabilimento thisOrg = new Stabilimento(db, newTic.getIdStabilimento());
	 ...
	 public String executeCommandViewDiffideOsa(ActionContext context) {
		 ...
		 Connection db = null;
		    ...
		    try {
		      ...
		      thisOrg.queryRecordStabilimento(db, Integer.parseInt(context.getParameter("stabId")));
		      thisOrg.getPrefissoAction(context.getAction().getActionName());
		     
		      Ticket ticket = new Ticket();
		      ticket.setIdStabilimento(thisOrg.getOrgId());
		      ...
		      diffList.setIdStabilimento(Integer.parseInt(context.getParameter("stabId")));
	  ...
	  public String executeCommandModifyTicket(ActionContext context) {
	    ...
	    try {
	     ...
	     //Gestire le api...	
	      ...
	      if(newTic.getIdApiario() > 0){
	    	  ext.aspcfs.modules.apiari.base.Stabilimento stab  = new  ext.aspcfs.modules.apiari.base.Stabilimento();
	    	  stab.queryRecord(db, newTic.getIdApiario());
		      stab.getPrefissoAction("ApicolturaApiari");
		      context.getRequest().setAttribute("OrgDetailsApi", stab);
		      retPage="ModifyApiOK";
	      } else {
	    	  Stabilimento thisOrganization = new Stabilimento();
		      thisOrganization.queryRecordStabilimento(db, newTic.getIdStabilimento());
		      thisOrganization.getPrefissoAction(context.getAction().getActionName());
		      context.getRequest().setAttribute("OrgDetails", thisOrganization);
	      }
		    ...
		    try {
		      ...
		      org.aspcfs.modules.vigilanza.base.Ticket CU = new org.aspcfs.modules.vigilanza.base.Ticket(db,Integer.parseInt(newTic.getIdControlloUfficiale()));
		  	 ...
		  	 if (CU.getIdApiario()>0){ 
		  		    ext.aspcfs.modules.apiari.base.Stabilimento thisOrganization = new  ext.aspcfs.modules.apiari.base.Stabilimento();
			  		thisOrganization.queryRecord(db, newTic.getIdApiario());
			  		thisOrganization.getPrefissoAction("ApicolturaApiari");
			  		context.getRequest().setAttribute("OrgDetails", thisOrganization);
			  		retPag="DettaglioApiOK";
		  	 }
			  	  else  {
			  		 Stabilimento thisOrganization = new Stabilimento();
			  		 thisOrganization.queryRecordStabilimento(db, newTic.getIdStabilimento());
