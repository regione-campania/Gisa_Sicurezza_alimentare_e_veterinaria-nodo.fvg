...
public class Diffida  extends CFSModule {
	 ...
	 public String executeCommandChiudi(ActionContext context)
	  {
		  	    if (!(hasPermission(context, "accounts-accounts-sanzioni-edit"))){
	 ...
	 public String executeCommandReopenTicket(ActionContext context) {
	  ...
	  public String executeCommandConfirmDelete(ActionContext context) {
	    if (!hasPermission(context, "accounts-accounts-sanzioni-delete")) {
	      return ("PermissionError");
	    }
	    Connection db = null;
	    //Parameters
	    ...
	    try {
	      ...
	      htmlDialog.addButton(
	          systemStatus.getLabel("global.button.delete"), "javascript:window.location.href='"+ticket.getURlDettaglio()+"Diffida.do?command=DeleteTicket&id=" + id + "&orgId=" + orgId + "&forceDelete=true" + RequestUtils.addLinkParams(
	...
	public String executeCommandAdd(ActionContext context) {
		...
		try {
			...
			Organization thisOrganization = new Organization(db, cu.getId());
			if (cu.getIdApiario() <= 0 && context.getParameter("orgId")==null && context.getParameter("stabId")!=null)
			{
				...
				thisOrganization.setName(newStabilimento.getOperatore().getRagioneSociale());

			}else if(cu.getIdApiario() > 0 && context.getParameter("orgId")==null && context.getParameter("stabId")!=null){
				
				ext.aspcfs.modules.apiari.base.Stabilimento newStabilimento = new ext.aspcfs.modules.apiari.base.Stabilimento(db,  Integer.parseInt(context.getParameter("stabId")));
				thisOrganization.setIdApiario(Integer.parseInt(context.getParameter("stabId")));
				thisOrganization.setOrgId(-1);
				thisOrganization.setName(newStabilimento.getOperatore().getRagioneSociale());
			
		}else if(cu.getAltId() > 0 && DatabaseUtils.getTipologiaOperatore(db, cu.getTipologia_operatore(), cu.getAltId())==Ticket.TIPO_OPU_RICHIESTE){
			
			org.aspcfs.modules.suap.base.Stabilimento newStabilimento = new org.aspcfs.modules.suap.base.Stabilimento(db, cu.getAltId(), true);
			thisOrganization.setAltId(cu.getAltId());
			thisOrganization.setOrgId(-1);
			thisOrganization.setName(newStabilimento.getOperatore().getRagioneSociale());
		}
		else if(cu.getAltId() > 0 && DatabaseUtils.getTipologiaOperatore(db, cu.getTipologia_operatore(), cu.getAltId())==Ticket.TIPO_SINTESIS){ 
			
			org.aspcfs.modules.sintesis.base.SintesisStabilimento newStabilimento = new org.aspcfs.modules.sintesis.base.SintesisStabilimento(db, cu.getAltId(), true);
			thisOrganization.setAltId(cu.getAltId());
			thisOrganization.setOrgId(-1);
			thisOrganization.setName(newStabilimento.getOperatore().getRagioneSociale());
		}else if(cu.getAltId() > 0 && DatabaseUtils.getTipologiaOperatore(db, cu.getTipologia_operatore(), cu.getAltId())==Ticket.TIPO_OPU){
			
			org.aspcfs.modules.opu.base.Stabilimento newStabilimento = new org.aspcfs.modules.opu.base.Stabilimento(db, cu.getAltId(), true);
			thisOrganization.setAltId(cu.getAltId());
			thisOrganization.setOrgId(-1);
			thisOrganization.setName(newStabilimento.getOperatore().getRagioneSociale());
		}
	...
	public String executeCommandInsert(ActionContext context) throws SQLException {
		...
		if (context.getParameter("stabId") != null  && newTic.getIdApiario() <= 0)
		...
		int id_asl = Integer.parseInt(site);
		if(site.equals("201")){
			site = "AV";	
	 ...
	 public String executeCommandViewDiffideOsa(ActionContext context) {
		 ...
		 Connection db = null;
		    ...
		    try {
		      ...
		      thisOrg.queryRecord(db, Integer.parseInt(context.getParameter("orgId")));
		      Ticket ticket = new Ticket();
		      ticket.setOrgId(thisOrg.getOrgId());
		      ...
		      diffList.setOrgId(Integer.parseInt(context.getParameter("orgId")));
		    ...
		    try {
		      ...
		      org.aspcfs.modules.vigilanza.base.Ticket CU = new org.aspcfs.modules.vigilanza.base.Ticket(db,Integer.parseInt(newTic.getIdControlloUfficiale()));
		  	  ...
		  	  if (CU.getIdApiario()>0) 
		  		thisOrganization.queryRecord(db, newTic.getIdApiario());  
		  	  else  
		  		  thisOrganization.queryRecord(db, newTic.getOrgId());
