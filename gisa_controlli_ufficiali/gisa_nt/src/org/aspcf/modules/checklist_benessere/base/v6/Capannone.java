...
public class Capannone {
	...
	public void insert(Connection db) throws SQLException {
		...
		pst = db.prepareStatement("insert into chk_bns_capannoni_v6 (id_chk_bns_mod_ist, entered_by, indice, numero, num_animali, num_fori, tipo_struttura, tipo_gabbia, tipo_gabbia_desc, ventilazione) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
	...
	public static ArrayList<Capannone> queryListByIdChkBnsModIst(Connection db, int idChkBnsModIst) throws SQLException {
		...
		PreparedStatement pst = db.prepareStatement("select * from chk_bns_capannoni_v6 where id_chk_bns_mod_ist = ? and trashed_date is null order by indice asc");
