...
public class Risposta {
	...
	public Risposta(Connection db, int idDomanda, int idChkBnsModIst) throws SQLException {
		PreparedStatement pst = db.prepareStatement("select * from chk_bns_risposte_v8 where id_chk_bns_mod_ist = ? and id_domanda = ? and trashed_date is null");
	...
	public void insert(Connection db) throws SQLException {
		...
		pst = db.prepareStatement("insert into chk_bns_risposte_v8 (id_chk_bns_mod_ist, id_domanda, risposta, evidenze, num_irregolarita, num_provv_a, num_provv_b, num_provv_c, entered_by) values (?, ?, ?, ?, ?, ?, ?, ?, ?)");
	...
	public static ArrayList<Risposta> queryListByIdChkBnsModIst(Connection db, int idChkBnsModIst) throws SQLException {
		...
		PreparedStatement pst = db.prepareStatement("select * from chk_bns_risposte_v8 where id_chk_bns_mod_ist = ? and trashed_date is null order by id_domanda asc");
		pst.setInt(1, idChkBnsModIst);
		ResultSet rs = pst.executeQuery();
		while (rs.next()){
