...
public class ChecklistIstanza_Ovicaprini extends GenericBean {
	...
	if (this.idChkBnsModIst>0){
		...
		pst = db.prepareStatement("update chk_bns_mod_ist_v7_ovicaprini set trashed_date = now() where id_chk_bns_mod_ist = ? and trashed_date is null");
		pst.setInt(1, this.idChkBnsModIst);
		pst.executeUpdate();
	...
	pst = db.prepareStatement("INSERT INTO public.chk_bns_mod_ist_v7_ovicaprini("+
	...
	private void queryRecordByIdChkBnsModIst(Connection db) throws SQLException {
		PreparedStatement pst = db.prepareStatement("select * from chk_bns_mod_ist_v7_ovicaprini where id_chk_bns_mod_ist = ? and trashed_date is null");
		pst.setInt(1, this.idChkBnsModIst);
		ResultSet rs = pst.executeQuery();
		if (rs.next()){
			buildRecord(rs);
