...
public class ChecklistIstanza_Tacchini extends GenericBean {
	...
	private static final long serialVersionUID = 1L;
	 ...
	 /**
	   ...
	   * @since    1.0
	   */
	  public ChecklistIstanza_Tacchini() { }
	...
	public void recuperaDaForm(ActionContext context, Connection db) throws SQLException {
		...
		sessoAllevato = context.getRequest().getParameter("sessoAllevato");
	...
	if (this.idChkBnsModIst>0){
		...
		pst = db.prepareStatement("update chk_bns_mod_ist_v7_tacchini set trashed_date = now() where id_chk_bns_mod_ist = ? and trashed_date is null");
		pst.setInt(1, this.idChkBnsModIst);
		pst.executeUpdate();
	...
	pst = db.prepareStatement("INSERT INTO public.chk_bns_mod_ist_v7_tacchini("+
            "id_chk_bns_mod_ist, entered_by, "+
            "num_checklist, veterinario_ispettore, ibrido_razza_allevata, sesso_allevato, presenza_manuale, veterinario_aziendale, "+
            "appartenente_condizionalita, criteri_utilizzati, criteri_utilizzati_altro_descrizione, esito_controllo, "+
            ...
            "data_chiusura_relazione, data_primo_controllo " +
            ")"+
            "VALUES (?, ?, "+
            "?, ?, ?, ?, ?, ?, "+
            "?, ?, ?, ?, "+
		...
		pst.setString(++i, this.sessoAllevato);
	...
	public ChecklistIstanza_Tacchini(Connection db, int idControllo, int codAllegato) throws SQLException {
	...
	private void queryRecordByIdChkBnsModIst(Connection db) throws SQLException {
		PreparedStatement pst = db.prepareStatement("select * from chk_bns_mod_ist_v7_tacchini where id_chk_bns_mod_ist = ? and trashed_date is null");
		pst.setInt(1, this.idChkBnsModIst);
		ResultSet rs = pst.executeQuery();
		if (rs.next()){
			buildRecord(rs);
	...
	private void buildRecord(ResultSet rs) throws SQLException {
		...
		this.sessoAllevato = rs.getString("sesso_allevato");
	...
	public String getSessoAllevato() {
		return sessoAllevato;
	}




	public void setSessoAllevato(String sessoAllevato) {
		this.sessoAllevato = sessoAllevato;
	}
