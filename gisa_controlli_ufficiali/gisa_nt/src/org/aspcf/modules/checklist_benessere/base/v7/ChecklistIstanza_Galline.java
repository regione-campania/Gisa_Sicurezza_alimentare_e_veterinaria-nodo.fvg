...
public class ChecklistIstanza_Galline extends GenericBean {
	...
	if (this.idChkBnsModIst>0){
		...
		pst = db.prepareStatement("update chk_bns_mod_ist_v7_galline set trashed_date = now() where id_chk_bns_mod_ist = ? and trashed_date is null");
		pst.setInt(1, this.idChkBnsModIst);
		pst.executeUpdate();
		...
		pst = db.prepareStatement("update chk_bns_boxes_v7 set trashed_date = now() where id_chk_bns_mod_ist = ? and trashed_date is null");
		pst.setInt(1, this.idChkBnsModIst);
		pst.executeUpdate();
	}
	...
	pst = db.prepareStatement("INSERT INTO public.chk_bns_mod_ist_v7_galline("+
            "id_chk_bns_mod_ist, entered_by, num_checklist, "+
            "veterinario_ispettore, extrapiano, num_uova_anno,  "+
            "ibrido_razza, "+
            ...
            "data_chiusura_relazione, intenzionalita, fase_produttiva)"+
            "VALUES (?, ?, ?, "+
            "?, ?, ?, "+
            "?, "+
            "?, ?, ?, "+
	...
	private void queryRecordByIdChkBnsModIst(Connection db) throws SQLException {
		PreparedStatement pst = db.prepareStatement("select * from chk_bns_mod_ist_v7_galline where id_chk_bns_mod_ist = ? and trashed_date is null");
		pst.setInt(1, this.idChkBnsModIst);
		ResultSet rs = pst.executeQuery();
		if (rs.next()){
			buildRecord(rs);
