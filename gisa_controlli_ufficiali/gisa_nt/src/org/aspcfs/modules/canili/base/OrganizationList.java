...
/**
  ...
  public ResultSet queryList(Connection db, PreparedStatement pst) throws SQLException {
    ...
    sqlCount.append(
            ...
            "FROM organization o left join organization_address oa on (oa.org_id = o.org_id and oa.address_type=1) " +
            " LEFT JOIN organization_address oa5 ON (o.org_id = oa5.org_id and oa5.address_type=5  " );

    
    if (addressType==null || addressType==-1)
    {
    	sqlCount.append(" AND oa5.address_type=5 )");
    }
    else
    {
    	sqlCount.append(" )");
    ...
    /***********  UNION CON OPU  *****************/
    String canili_in_opu = "	select count(*) as recordcount from lista_opu_operatori_canili where 1 = 1 ";
    
    if (name!=null && !name.equals(""))
    	canili_in_opu+=" AND  ragione_sociale ilike ? ";

    if (partitaIva!=null && !partitaIva.equals(""))
    	canili_in_opu+=" AND  partita_iva ilike ? ";
    
    if (codiceFiscale!=null && !codiceFiscale.equals(""))
    	canili_in_opu+=" AND  codice_fiscale_impresa ilike ? ";
    
    if (city!=null && !city.equals(""))
    	canili_in_opu+=" AND  comune_stab ilike ? ";
    
    if (orgSiteId!=-1)
    	canili_in_opu+=" AND  id_asl_stab = ? ";
    /********************************************/

    
    if (pagedListInfo != null) {
      //Get the total number of records matching filter
      pst = db.prepareStatement("SELECT sum(recordcount) AS recordcount from  ("+
          sqlCount.toString() +
          sqlFilter.toString()+" UNION "+canili_in_opu+") AS T;");
    ...
    if (pagedListInfo != null) {
      ...
      sqlSelect.append("SELECT ");
    	 ...
    	 sqlSelect.append(
    		        "DISTINCT o.org_id, o.name, oa5.city as o_city, oa5.state as o_state, telefono_rappresentante, o.codice_fiscale, o.categoria_rischio,o.site_id, o.notes "+
    		        "FROM organization o "  + 
    	    ...
    	    /***********  UNION CON OPU  *****************/
    	    canili_in_opu = "	select distinct id_stabilimento as org_id,ragione_sociale as name, comune_stab as o_city, prov_stab as o_state, '' as telefono_rappresentante, "+
    	    				" codice_fiscale_impresa as codice_fiscale, categoria_rischio,id_asl_stab as site_id, 'OPU' as notes from lista_opu_operatori_canili  where 1=1 ";
    	    ...
    	    /********************************************/
	  pst = db.prepareStatement(
        sqlSelect.toString() + sqlFilter.toString() + " union  "+canili_in_opu+sqlOrder.toString().replace("o.name", "name"));
    items = prepareFilter(pst);
    /***********  UNION CON OPU  *****************/
    if (name!=null && !name.equals(""))
    	pst.setString(++items,"%"+name+"%");
  ...
  protected void createFilter(Connection db, StringBuffer sqlFilter) {
    ...
    */
    ...
    if (city != null && !"-1".equals(city)) {
        ...
        if (city.indexOf("%") >= 0) {
          sqlFilter.append(
              " AND  o.org_id IN (SELECT o.org_id FROM organization o join organization_address oa on (o.org_id = oa.org_id) " +
              " where o.tipologia = 10 AND o.trashed_date IS NULL AND " + DatabaseUtils.toLowerCase(db, "city") + " ilike ? " +
  ...
  protected void buildResources(Connection db) throws SQLException {
    ...
    while (i.hasNext()) {
      ...
      if (thisOrganization.getNotes()==null || !thisOrganization.getNotes().equals("OPU")){
