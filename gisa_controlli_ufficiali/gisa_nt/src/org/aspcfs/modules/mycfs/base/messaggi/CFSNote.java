 ...
 */
package org.aspcfs.modules.mycfs.base.messaggi;
...
import org.aspcfs.modules.mycfs.base.Allegato;
 ...
 */
public class CFSNote extends org.aspcfs.modules.mycfs.base.CFSNote {
...
//action list properties
  ...
  private void queryRecord(Connection db, int noteId, int myId, String listView) throws SQLException {
    ...
    sql.append(
        "SELECT m.*, ml.id AS ml_id, ml.sent_to, ml.status, ml.viewed, ml.enabled, ml.sent_from, " +
            ...
            "FROM messaggi_interni_link ml, messaggi_interni m " +
  ...
  private void setListaAllegati(Connection db){
	  String sql = "select * from messaggi_interni_allegati where id_messaggio=?";
	  ...
	  try
	  {
		  ...
		  while(rs.next())
		  {
			  ...
			  String codAllegato = rs.getString("cod_allegato");
			  int idAllegato = rs.getInt("id_allegato");
			  ...
			  allegato.setFolderid(folderid);
			  allegato.setVersione(versione);
			  allegato.setEstensione(estensione);
			  allegato.setIdAllegato(idAllegato);
			  super.listaAllegati.add(allegato);
		  }
	  }
  ...
  public boolean insert(Connection db,ActionContext context) throws SQLException {
    ...
    try {
      ...
      id =  DatabaseUtils.getNextSeq(db,context, "messaggi_interni","id");

      sql.append(
          "INSERT INTO messaggi_interni " +
              "(" + (id > -1 ? "id, " : "") + "enteredby, modifiedby, body, reply_id, " + DatabaseUtils.addQuotes(db, "type") + ", delete_flag, tipo) " +
              "VALUES (" + (id > -1 ? "?, " : "") + "?, ?, ?, ?, ?, ?, ?) ");
      int i = 0;
      PreparedStatement pst = db.prepareStatement(sql.toString());
      if (id > -1) {
  ...
  public boolean insertLink(Connection db, boolean isUser) throws SQLException {
    try {
      ...
      PreparedStatement pst = db.prepareStatement(
          "INSERT INTO messaggi_interni_link " +
  ...
  public boolean insertMessaggiAllegati(Connection db,String descrizione,int idAllegato,String folderid,String estensione,String versione ) throws SQLException {
	    try {
	      int i = 0;
	      PreparedStatement pst = db.prepareStatement(
	          "INSERT INTO messaggi_interni_allegati " +
	              "(id_messaggio, descrizione, id_allegato,folderid,estensione,versione) " +
	              "VALUES (?, ?, ?,?,?,?) ");
	      pst.setInt(++i, this.getId());
	      pst.setString(++i, descrizione);
	      pst.setInt(++i, idAllegato);
	      if(folderid!=null && ! folderid.equals(""))
	      {
	    	  pst.setInt(++i, Integer.parseInt(folderid));
	      }
	      else
	      {
	    	  pst.setInt(++i, -1);
	      }
	      pst.setString(++i, estensione);
	      pst.setString(++i, versione);
	      pst.execute();
	      pst.close();

//	      if (!isUser) {
//	        pst = db.prepareStatement(
//	            "UPDATE messaggi_interni_link " +
//	                "SET status = 3 " +
...
//	      }
  ...
  public boolean insertMessaggiAllegatiDocumentale(Connection db,String descrizione,String codAllegato,String estensione ) throws SQLException {
	    try {
	      int i = 0;
	      PreparedStatement pst = db.prepareStatement(
	          "INSERT INTO messaggi_interni_allegati " +
	              "(id_messaggio, descrizione, cod_allegato,folderid,estensione) " +
	              "VALUES (?, ?, ?,?,?) ");
	      pst.setInt(++i, this.getId());
	      pst.setString(++i, descrizione);
	      pst.setString(++i, codAllegato);
	      pst.setInt(++i, -1);
	      pst.setString(++i, estensione);
	      pst.execute();
	      pst.close();
...
//	      }
  ...
  public boolean delete(Connection db, int myId) throws SQLException {
    try {
      ...
      int inboxCount = -1;
      int outboxCount = -1;
      String outboxQuery = "SELECT count(*) as outboxcount FROM messaggi_interni " +
          " where id = ? " +
          " AND  delete_flag = ? ";

      String inboxQuery = "SELECT count(*) as inboxcount FROM messaggi_interni_link " +
          " where id =" + this.getId() + " " +
          " AND  status <> 3 ";
      ...
      if ((outboxCount == 0 && inboxCount == 1) || (outboxCount == 1 && inboxCount == 0)) {
        pst = db.prepareStatement(
            "DELETE FROM messaggi_interni_link " +
                " where id = ? ");
        pst.setInt(1, this.getId());
        pst.execute();
        pst.close();
        pst = db.prepareStatement(
            "DELETE FROM messaggi_interni " +
                " where id = ? ");
        pst.setInt(1, this.getId());
        pst.execute();
        pst.close();
