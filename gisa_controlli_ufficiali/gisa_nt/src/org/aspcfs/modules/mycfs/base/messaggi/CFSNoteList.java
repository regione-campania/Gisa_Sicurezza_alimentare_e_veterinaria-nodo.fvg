 ...
 *
 * @author chris
 * @version $Id: CFSNoteList.java 17362 2006-11-14 03:59:14Z matt $
 * @created February 21, 2002
 */
public class CFSNoteList extends org.aspcfs.modules.mycfs.base.CFSNoteList {


  public final static String tableName = "messaggi_interni";
  ...
  private boolean tutteInUnaPagina = false;
...
/**
  ...
  public void buildList(Connection db) throws SQLException {
    ...
    if (pagedListInfo != null) {
      ...
      if (rs.next()) {
        ...
        if (tutteInUnaPagina) //TUTTI I RISULTATI IN UNA SOLA PAGINA
  ...
  private void createFilter(StringBuffer sqlFilter) {
    ...
    if (subjectMessage!=null) 
        sqlFilter.append(" AND  m.subject ilike ? ");
    if (bodyMessage!=null) 
        sqlFilter.append(" AND  m.body ilike ? ");
  ...
  private int prepareFilter(PreparedStatement pst) throws SQLException {
    ...
    if (subjectMessage!=null) 
        pst.setString(++i, subjectMessage);
    if (bodyMessage!=null) 
        pst.setString(++i, bodyMessage);
    if (sentTo > -1 && !sentMessagesOnly) {
      pst.setInt(++i, sentTo);
    } else {
...
public boolean isTutteInUnaPagina() {
	return tutteInUnaPagina;
}

public void setTutteInUnaPagina(boolean tutteInUnaPagina) {
	this.tutteInUnaPagina = tutteInUnaPagina;
}
}
