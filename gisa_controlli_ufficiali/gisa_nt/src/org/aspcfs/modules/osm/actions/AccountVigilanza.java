...
public final class AccountVigilanza extends CFSModule {
  ...
  public String executeCommandReopenTicket(ActionContext context) {
    if (!hasPermission(context, "osm-osm-vigilanza-edit")) {
      return ("PermissionError");
    }
    int resultCount = -1;
    Connection db = null;
  ...
  public String executeCommandTicketDetails(ActionContext context) {
    ...
    try {
      ...
      context.getRequest().setAttribute("tipologia",800);
  ...
  public String executeCommandConfirmDelete(ActionContext context) {
    if (!hasPermission(context, "osm-osm-vigilanza-delete")) {
      return ("PermissionError");
    }
    Connection db = null;
    //Parameters
    ...
    try {
      ...
      htmlDialog.addButton(
          systemStatus.getLabel("global.button.delete"), "javascript:window.location.href='OsmVigilanza.do?command=DeleteTicket&id=" + id + "&orgId=" + orgId + "&forceDelete=true" + RequestUtils.addLinkParams(
    ...
    try {
      ...
      if (recordDeleted) {
        ...
        context.getRequest().setAttribute(
            "refreshUrl", "Osm.do?command=ViewVigilanza&orgId=" + orgId +
  ...
  public String executeCommandAdd(ActionContext context) {
	    if (!hasPermission(context, "osm-osm-vigilanza-add")) {
	      return ("PermissionError");
	    }
	    Connection db = null;
	    Ticket newTic = null;
	    try {
	      ...
	      boolean modificato = false;
			...
			if (modificato){
				...
				org.aspcfs.modules.osm.actions.Accounts sa = new org.aspcfs.modules.osm.actions.Accounts();
  ...
  public String executeCommandViewVigilanza(ActionContext context) {
	    ...
	    ticketListInfo.setLink(
	        "OsmVigilanza.do?command=ViewVigilanza&orgId=" + passedId);
