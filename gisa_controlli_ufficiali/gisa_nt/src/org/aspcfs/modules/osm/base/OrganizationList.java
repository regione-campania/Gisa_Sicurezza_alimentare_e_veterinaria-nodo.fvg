...
/**
  ...
  public ResultSet queryList(Connection db, PreparedStatement pst) throws SQLException {
    ...
    sqlCount.append(
        ...
        " FROM organization o "+
        " left join organization_address oa ON (o.org_id = oa.org_id) "+ join() + joinAudit());
   ...
   // pst.setBoolean(++items, true);
	...
	private String join()
	{
		...
		if( ( sottoAttivita > -1 ) 
			|| ( codiceSezione > -1 ) )
		{
			ret = " LEFT JOIN osm_sottoattivita ssa ON (o.org_id = ssa.id_stabilimento) ";
		}
...
/**
  ...
  protected void createFilter(Connection db, StringBuffer sqlFilter) {
    ...
    /*if(categoriaRischio!=-1){
    	if(categoriaRischio==3){
    		sqlFilter.append(" AND  (o.categoria_rischio = 3 OR o.categoria_rischio = -1)");
    	}else{
    		sqlFilter.append(" AND  o.categoria_rischio = ? ");

    	}

    }   */
    if (accountOtherState!=null && !accountOtherState.equals("")) {
    	sqlFilter.append(" AND  oa.state ilike ? ");
        }
    if (accountOtherState3!=null && !accountOtherState3.equals("")) {
    	sqlFilter.append(" AND  oa.state ilike ? ");
        }
    
    
    if (impianto > -1) {
        sqlFilter.append(" AND  o.impianto = ? ");
        }
    if (statoLab > -1) {
  ...
  /*  if(categoriaRischio!=3 && categoriaRischio!=-1){
    ...
    if (accountOtherState!=null && !accountOtherState.equals("")) {
        pst.setString(++i, accountOtherState);
        }
    
    if (accountOtherState3!=null && !accountOtherState3.equals("")) {
    	pst.setString(++i, accountOtherState3);
        }
    if (impianto > -1) {
        pst.setInt(++i, impianto);
        }
    if (statoLab > -1) {
