...
public final class AccountVigilanza extends CFSModule {
	...
	public String executeCommandAdd(ActionContext context) {
		if (!hasPermission(context, "osa-osa-vigilanza-add")) {
		...
		try {
			...
			context.getRequest().setAttribute("tipologia", 850);
	...
	public String executeCommandModifyTicket(ActionContext context) {
		if (!hasPermission(context, "osa-osa-vigilanza-edit")) {
			return ("PermissionError");
		}
		Connection db = null;
		Ticket newTic = null;
...
//			}
	...
	public String executeCommandConfirmDelete(ActionContext context) {
		if (!hasPermission(context, "osa-osa-vigilanza-delete")) {
			return ("PermissionError");
		}
		...
		{	
			...
			org.aspcf.modules.controlliufficiali.action.AccountVigilanza controlliGeneric = new org.aspcf.modules.controlliufficiali.action.AccountVigilanza();
			context.getRequest().setAttribute("url_confirm","OSAnimaliVigilanza.do?command=DeleteTicket");
	...
	public String executeCommandDeleteTicket(ActionContext context) {
		...
		try {
			...
			org.aspcf.modules.controlliufficiali.action.AccountVigilanza controlliGeneric = new org.aspcf.modules.controlliufficiali.action.AccountVigilanza();
			context.getRequest().setAttribute("url","OSAnimaliVigilanza.do?command=ViewVigilanza");
