...
//		        table.addCell( new Phrase( st.getAsl(), f ) );
	...
	public static void main(String[] args)
	{
		try
		{
			FileOutputStream fos = new FileOutputStream( "c:\\registro_macellazioni.pdf" );
...
//		        table.addCell( new Phrase( st.getAsl(), f ) );
	...
	public static String printModuloArt17( ActionContext context, Organization macello, String asl, Organization esercente,
			Art17 art17, String data, ArrayList<Stats> elenco, Stats header, float[] sizes, String reportDir, Connection db )
	{
		try
		{
		    ByteArrayOutputStream out = new ByteArrayOutputStream();

		    moduloArt17( elenco, macello, asl, esercente, data, art17, header, sizes, out, reportDir, db );
	...
	private static void moduloArt17( ArrayList<Stats> elenco, Organization macello, String asl, Organization esercente,
			String data, Art17 art17, Stats header, float[] sizes, ByteArrayOutputStream out, String reportDir, Connection db )
	{
...
//		        table.addCell( new Phrase( st.getAsl(), f ) );
			...
			final int NUM_COLONNE = Integer.parseInt(ApplicationProperties.getProperty("NUM_COLONNE_ALTRE_SPECIE"));
