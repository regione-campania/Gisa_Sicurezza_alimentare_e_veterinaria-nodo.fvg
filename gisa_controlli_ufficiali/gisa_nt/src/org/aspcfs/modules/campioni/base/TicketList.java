...
public class TicketList extends org.aspcfs.modules.troubletickets.base.TicketList
{
	...
	public void buildList(Connection db) throws SQLException {
		    ...
		    sqlCount.append(
		        ...
		        " left join matrici_campioni mc on t.ticketid = mc.id_campione" +
		        " where t.ticketid > 0 AND t.tipologia = 2 ");
		    ...
		    sqlSelect.append(
		        ...
		        "a.manufacturer_code AS assetmanufacturercode,o.tipologia as tipologia_operatore, " +
	...
	public void buildListControlli(Connection db, int org_id, String idC) throws SQLException {
	    ...
	    createFilter(sqlFilter, db);
	    sqlSelect.append(
	        "SELECT t.*,o.tipologia as tipologia_operatore        "
	        + "FROM ticket t " +
	        " left JOIN organization o ON (t.org_id = o.org_id) " +
        	" WHERE t.trashed_Date is null and t.ticketid > 0 and t.tipologia = 2 and  t.id_controllo_ufficiale = '"+idC+"' ");
	...
	public void buildListCampioniPrenotati(Connection db, int org_id) throws SQLException {
	    PreparedStatement pst = null;
	    ResultSet rs = null;
	    int items = -1;
	    StringBuffer sqlSelect = new StringBuffer();
	    ...
	    sqlCount.append(
	        "SELECT COUNT(*) AS recordcount " +
	    		...
	    		" LEFT JOIN asset a ON (t.link_asset_id = a.asset_id) "+
	    		" WHERE t.tipologia = 2 and t.org_id = "+org_id+" and t.id_controllo_ufficiale = '-1' ");   
	    createFilter(sqlFilter, db);
	    if (pagedListInfo != null) {
	      //Get the total number of records matching filter
	      pst = db.prepareStatement(
	    ...
	    sqlSelect.append(
	        ...
	        "o.site_id AS orgsiteid, " +
	        "a.serial_number AS serialnumber,o.tipologia as tipologia_operatore, " +
	       ...
	       // "INNER JOIN ticket tic ON ((tic.tipologia = 3)and(tic.org_id = t.org_id)and(tic.id_controllo_ufficiale = t.id_controllo_ufficiale)) "+
	        " left JOIN asset a ON (t.link_asset_id = a.asset_id) " +
	        " left JOIN matrici_campioni mc on (t.ticketid = mc.id_campione ) " +
	        " left JOIN analiti_campioni ac on (t.ticketid = ac.id_campione ) " +
	        //" left JOIN matrici_campioni mc on (t.ticketid = id_campione )" +
	        " where t.tipologia = 2 and t.org_id = "+org_id+" and t.id_controllo_ufficiale = '-1'");
	    ...
	    while (rs.next()) {
	      Ticket thisTicket = new Ticket(rs);
	      thisTicket.setTipologia_operatore(rs.getInt("tipologia_operatore"));
	      this.add(thisTicket);
	    }
	    rs.close();
	    pst.close();
	...
	public void buildListbyOrgId(Connection db, int org_id) throws SQLException {
	    PreparedStatement pst = null;
	    ResultSet rs = null;
	    int items = -1;
	    StringBuffer sqlSelect = new StringBuffer();
	    ...
	    sqlCount.append(
	        ...
	        " left JOIN asset a ON (t.link_asset_id = a.asset_id) " +
	        " left join matrici_campioni mc on t.ticketid = mc.id_campione" +
	        " where t.ticketid > 0 AND t.tipologia = 2 and t.trashed_date is null  and t.org_id = "+org_id);
	    //createFilter(sqlFilter, db);
	    if (pagedListInfo != null) {
	      //Get the total number of records matching filter
	      pst = db.prepareStatement(
	      ...
	      pst.close();
	      // Declare default sort, if unset
	      pagedListInfo.setDefaultSort("t.entered DESC", null);
	      //Determine the offset, based on the filter, for the first record to show
	      if (pagedListInfo.getMode() == PagedListInfo.DETAILS_VIEW && id > 0) {
	        String direction = null;
	        if ("desc".equalsIgnoreCase(pagedListInfo.getSortOrder())) {
	   ...
	   // items = prepareFilter(pst);
	    ...
	    while (rs.next()) {
	      Ticket thisTicket = new Ticket(rs);
	      thisTicket.getTipoCampioneSelezionatoNuovaGestione(db);
	      thisTicket.getMatriceCampioneSelezionatoNuovaGestione(db);
	      this.add(thisTicket);
	    }
	    rs.close();
	    pst.close();
...
protected void createFilter(StringBuffer sqlFilter, Connection db) {
    ...
    if ((tipologia == 2)) {
	      sqlFilter.append(" AND  t.tipologia = 2 ");
	} 
