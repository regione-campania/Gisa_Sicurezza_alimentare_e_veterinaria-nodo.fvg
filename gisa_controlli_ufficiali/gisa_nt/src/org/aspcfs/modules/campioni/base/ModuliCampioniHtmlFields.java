...
public class ModuliCampioniHtmlFields {

private int id;
private int tipo_modulo;
...
public void setTipo_modulo(int tipo_modulo) {
...
public void buildRecord(Connection db, ResultSet rs) throws SQLException{
	this.id = rs.getInt("id");
	  this.tipo_modulo=rs.getInt("tipo_modulo");
	  ...
	  setValore_campione(rs.getString("valore_campione"));
...
public LinkedHashMap<String, String> costruisciHtmlDaHashMap(Connection db, LinkedHashMap<String, ArrayList<ModuliCampioniHtmlFields>> campi) throws SQLException{

	String nomeCampo="";
	String html = "";
	String nomeLabel ="";
	ArrayList<ModuliCampioniHtmlFields> campiHash = new ArrayList<ModuliCampioniHtmlFields>();
	LinkedHashMap<String, String> risultato = new LinkedHashMap<String, String>();
	
	for (Map.Entry<String, ArrayList<ModuliCampioniHtmlFields>> entry : campi.entrySet()) {
	  ...
	  if (!(campiHash.get(0).getTipo_campo().equals("select") && campiHash.get(0).isMultiple())){ //SE NON E' UNA SELECT MULTIPLA
		  if (campiHash.get(0).getTipo_campo().equalsIgnoreCase("select")){
				 ...
				 html =  l.getHtmlSelect(campiHash.get(0).getNome_campo(), campiHash.get(0).getValore_campione());
			}
		  else if (campiHash.get(0).getTipo_campo().equalsIgnoreCase("text")){
				String text = "<input type=\"text\" id=\""+campiHash.get(0).getNome_campo()+"\" name=\""+campiHash.get(0).getNome_campo()+"\" value=\""+campiHash.get(0).getValore_campione()+"\"/>";
				 html=  text;
				}
		  else if (campiHash.get(0).getTipo_campo().equalsIgnoreCase("number")){
				String text = "<input type=\"number\" id=\""+campiHash.get(0).getNome_campo()+"\" name=\""+campiHash.get(0).getNome_campo()+"\" value=\""+campiHash.get(0).getValore_campione()+"\"/>";
				 html=  text;
				}
		  else if (campiHash.get(0).getTipo_campo().equalsIgnoreCase("hidden")){
				String text = "<input type=\"hidden\" id=\""+campiHash.get(0).getNome_campo()+"\" name=\""+campiHash.get(0).getNome_campo()+"\" value=\""+campiHash.get(0).getValore_campione()+"\"/>";
				 html=  text;
				}
		  else if (campiHash.get(0).getTipo_campo().equalsIgnoreCase("data")){
				String text = "<input type=\"text\" readonly id=\""+campiHash.get(0).getNome_campo()+"\" name=\""+campiHash.get(0).getNome_campo()+"\" value=\""+campiHash.get(0).getValore_campione()+"\"/>";
				...
				String cancel = "<a href=\"#\" onClick=\"document.forms[0]."+campiHash.get(0).getNome_campo()+".value=''; return false;\"><img src=\"images/icons/stock_calc-cancel-16.gif\" border=\"0\" align=\"absmiddle\"></a>";
				html= text+data+cancel;
		  ...
		  for (int i=0; i<campiHash.size();i++){
			  ...
			  el.setCode(campiHash.get(i).getValore_campione());
...
public void gestisciUpdate (ActionContext context, Connection db, int idCampione, int tipoModulo) {
...
//		 } 
	 ...
	 CampioniUtil.insertSchedaModulo(db, t, tipoModulo, context);
