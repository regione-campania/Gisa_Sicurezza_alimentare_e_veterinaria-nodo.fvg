...
public final class AccountCampioni extends CFSModule {
	...
	public String executeCommandTicketDetails(ActionContext context) {
		if (!hasPermission(context, "opu-campioni-view")) {
			return ("PermissionError");
		}
		Connection db = null;
		// Parameters
		...
		try {
			...
			if (context.getRequest().getParameter("stabId") != null )
				...
				if (context.getRequest().getAttribute("stabId") != null )
					orgId = Integer.parseInt( ""+context.getRequest().getAttribute("stabId"));			
			if(orgId<1 && context.getRequest().getParameter("orgId")!=null )
	...
	public String executeCommandConfirmDelete(ActionContext context) {
		if (!hasPermission(context, "opu-campioni-delete")) {
			return ("PermissionError");
		}
		Connection db = null;
		// Parameters
	...
	public String executeCommandViewHistory(ActionContext context) {
		if (!hasPermission(context, "opu-campioni-history-view")) {
			return ("PermissionError");
		}
		Connection db = null;
	...
	public String executeCommandAdd(ActionContext context) {
		if (!hasPermission(context, "opu-campioni-add")) {
			return ("PermissionError");
		}
		Connection db = null;
		...
		try {
			...
			if(temporgId==null)
			{
				Object temp = context.getRequest().getAttribute("stabId");
				if(temp instanceof Integer)
					temporgId = (Integer)context.getRequest().getAttribute("stabId")+"";
				else
			...
			String isCanile = null;
			for (LineaProduttiva lp : (Vector<LineaProduttiva>)stab.getListaLineeProduttive())
			{	
			...
			context.getRequest().setAttribute("isCanile", isCanile);
			
			String isOperatorePrivato = null;
			for (LineaProduttiva lp : (Vector<LineaProduttiva>)stab.getListaLineeProduttive())
			{	
				if(lp.getCodice().contains("OPR-OPR-X") ) //gestione valida per ML8 e ML10
				{
					isOperatorePrivato="si" ;
				}
			}
			context.getRequest().setAttribute("isOperatorePrivato", isOperatorePrivato);
	...
	public String executeCommandInsert(ActionContext context)
		...
		try
		{
			...
			Stabilimento thisOrg = new Stabilimento(db, ((newTic.getIdStabilimento()>0)?(newTic.getIdStabilimento()):(orgId))   );
			...
			context.getRequest().setAttribute("stabId", ((newTic.getIdStabilimento()>0)?(newTic.getIdStabilimento()):(orgId)) );
...
context.getRequest().setAttribute("AnimaliNonAlimentari",
	...
	public String executeCommandChiudi(ActionContext context) {
		if (!(hasPermission(context, "opu-campioni-edit"))) {
			return ("PermissionError");
		}
		int resultCount = -1;
		Connection db = null;
		...
		try {
			...
			String ticketId = context.getRequest().getParameter("id");
			 ...
			 tamponiList.buildListControlli(db, pasId, id_controllo);
				      ...
				      Stabilimento org = new Stabilimento(db , thisTicketV.getIdStabilimento());
