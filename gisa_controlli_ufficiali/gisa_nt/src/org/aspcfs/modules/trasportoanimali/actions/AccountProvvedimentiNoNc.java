...
public final class AccountProvvedimentiNoNc extends CFSModule {
	...
	public String executeCommandTicketDetails(ActionContext context) {
		if (!hasPermission(context, "trasportoanimali-trasportoanimali-campioni-view")) {
	...
	public String executeCommandConfirmDelete(ActionContext context) {
		if (!hasPermission(context, "trasportoanimali-trasportoanimali-campioni-delete")) {
			return ("PermissionError");
		}
		Connection db = null;
		// Parameters
		...
		try {
			...
			htmlDialog.addButton(systemStatus.getLabel("global.button.delete"),
					"javascript:window.location.href='AbusivismiCampioni.do?command=DeleteTicket&id="
							+ id
							+ "&orgId="
							+ orgId
							+ "&forceDelete=true"
		...
		try {
			...
			if (recordDeleted) {
				...
				context
						...
						.setAttribute(
								"refreshUrl",
								"AbusivismiVigilanza.do?command=TicketDetails&id="+id_controllo+"&orgId="
										+ orgId
										+ (inline != null
	...
	public String executeCommandUpdateTicket(ActionContext context) {
		if (!hasPermission(context, "trasportoanimali-trasportoanimali-campioni-edit")) {
			return ("PermissionError");
		}
		Connection db = null;
	...
	public String executeCommandAdd(ActionContext context) {
		if (!hasPermission(context, "trasportoanimali-trasportoanimali-campioni-add")) {
			return ("PermissionError");
		}
		Connection db = null;
		...
		try {
			...
			String ticketId = context.getRequest().getParameter("id");
			 ...
			 tamponiList.buildListControlli(db, pasId, id_controllo);
				      ...
				      Organization org = new org.aspcfs.modules.trasportoanimali.base.Organization(db , thisTicketV.getOrgId());
