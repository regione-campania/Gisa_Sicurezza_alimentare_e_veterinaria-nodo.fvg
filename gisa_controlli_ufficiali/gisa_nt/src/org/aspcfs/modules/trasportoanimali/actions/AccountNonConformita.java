...
public final class AccountNonConformita extends CFSModule {
	...
	public String executeCommandReopenTicket(ActionContext context) {
		if (!hasPermission(context, "trasporti-trasporti-nonconformita-edit")) {
	...
	public String executeCommandTicketDetails(ActionContext context) {
		if (!hasPermission(context, "trasporti-trasporti-nonconformita-view")) {
			return ("PermissionError");
		}
	...
	public String executeCommandConfirmDelete(ActionContext context) {
		...
		try {
			...
			htmlDialog.addButton(
					systemStatus.getLabel("global.button.delete"), "javascript:window.location.href='TrasportiNonConformita.do?command=DeleteTicket&id=" + id + "&orgId=" + orgId + "&forceDelete=true" + RequestUtils.addLinkParams(
	...
	public String executeCommandDeleteTicket(ActionContext context) {
		if (!hasPermission(context, "trasporti-trasporti-nonconformita-delete")) {
			return ("PermissionError");
		}
		...
		try {
			...
			if (recordDeleted) {
				...
				.setAttribute(
						"refreshUrl",
						"TrasportiVigilanza.do?command=TicketDetails&id="+id_controllo+"&orgId="
						+ orgId
						+ (inline != null
	...
	public String executeCommandAdd(ActionContext context) {
		if (!hasPermission(context, "trasporti-trasporti-nonconformita-add")) {
			return ("PermissionError");
		}
		Connection db = null;
		Ticket newTic = null;
