...
public class OrganizationList extends Vector implements SyncableList {
  ...
  private static Logger log = Logger.getLogger(org.aspcfs.modules.trasportoanimali.base.OrganizationList.class);
  static {
  ...
  private String id_autorizzazione = null;
...
public void setNumAut(String numAut) {
  ...
  public ResultSet queryList(Connection db, PreparedStatement pst) throws SQLException {
    ...
    sqlCount.append(
        ...
        "FROM organization o left join organization_autoveicoli o_a on o.org_id=o_a.org_id and o_a.elimina is null " +
        " where o.org_id >= 0 ");
    ...
    sqlSelect.append(
        ...
        "'' as stage_name, o_a.targa as targa "+
        "FROM organization o " +
        ...
        " left JOIN organization_autoveicoli o_a ON (o.org_id = o_a.org_id and o_a.elimina is null) " +
        " where o.org_id >= 0 and tipologia=9  " );
...
//    pst.setBoolean(++items, false);
  ...
  protected void createFilter(Connection db, StringBuffer sqlFilter) {
    ...
    if (!"".equals(numAut) && numAut!=null) {
    	sqlFilter.append(" AND  o.numaut ILIKE ? ");
    }
    if (!"".equals(dunsType) && dunsType!=null) {
    	sqlFilter.append(" AND  o.duns_type ILIKE ? ");
    }
...
//      sqlFilter.append(" AND  miner_only = ? ");
  ...
  if ( ( nomeRappresentante != null) && !"".equals( nomeRappresentante ) ) {
  	sqlFilter.append(" AND  nome_rappresentante = ?");
  }
  if ( ( cognomeRappresentante != null) && !"".equals( cognomeRappresentante ) ) {
  	sqlFilter.append(" AND  cognome_rappresentante = ?");
  }
    ...
    if (orgSiteId == -1) {
      if (includeOrganizationWithoutSite) {
        sqlFilter.append(" AND  o.site_id>-1 ");
      }
    }
    ...
    if (state != null && !"-1".equals(state)) {
      ...
      sqlFilter.append(
          ...
          " AND  state IS NOT NULL AND address_type = 5) ");
      } else {
        sqlFilter.append(
            " AND  o.org_id IN (SELECT org_id FROM organization_address " +
    ...
    if (targa!=null && !targa.equals("")){
    	sqlFilter.append(" AND  o_a.targa like ? ");
    }
    
    /*Filtro sull'identificativo dell'autorizzazione*/
    if (this.id_autorizzazione!=null && !this.id_autorizzazione.equals("")){
    	sqlFilter.append(" AND  o.account_number like ? ");
    }
    
    /*Filtro sulla denominazione*/
    if (this.denominazione!=null && this.denominazione.equals("")){
    	sqlFilter.append(" AND  o.name like ? ");
    }
  }
...
//    }
    ...
    if (city != null && !"-1".equals(city)) {
      pst.setString(++i, city.toLowerCase().replace("-", "'"));
    }
    if (state != null && !"-1".equals(state)) {
    ...
    /*Filtro sull'identificativo dell'autorizzazione*/
    if (this.id_autorizzazione!=null && !this.id_autorizzazione.equals("")){
    	pst.setString(++i, id_autorizzazione);
    }
