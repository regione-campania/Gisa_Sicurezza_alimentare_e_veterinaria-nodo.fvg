...
public final class AccountVigilanza extends CFSModule {
	...
	public String executeCommandAdd(ActionContext context) {
		if (!hasPermission(context, "imbarcazioni-imbarcazioni-vigilanza-add")) {
		...
		try {
			...
			context.getRequest().setAttribute("tipologia", 17);
	...
	public String executeCommandModifyTicket(ActionContext context) {
		if (!hasPermission(context, "imbarcazioni-imbarcazioni-vigilanza-edit")) {
			return ("PermissionError");
		}
		Connection db = null;
		Ticket newTic = null;
...
//			}
	...
	public String executeCommandConfirmDelete(ActionContext context) {
		if (!hasPermission(context, "imbarcazioni-imbarcazioni-vigilanza-delete")) {
			return ("PermissionError");
		}
		...
		{	
			db = this.getConnection(context);
			org.aspcf.modules.controlliufficiali.action.AccountVigilanza controlliGeneric = new org.aspcf.modules.controlliufficiali.action.AccountVigilanza();
			context.getRequest().setAttribute("url_confirm","ImbarcazioniVigilanza.do?command=DeleteTicket");
	...
	public String executeCommandDeleteTicket(ActionContext context) {
		...
		try {
			...
			org.aspcf.modules.controlliufficiali.action.AccountVigilanza controlliGeneric = new org.aspcf.modules.controlliufficiali.action.AccountVigilanza();
			context.getRequest().setAttribute("url","ImbarcazioniVigilanza.do?command=ViewVigilanza");
...
public String executeCommandSupervisiona(ActionContext context) {
	...
	public String executeCommandViewHistory(ActionContext context) {
		if (!hasPermission(context, "imbarcazioni-imbarcazioni-vigilanza-history-view")) {
			return ("PermissionError");
		}
		Connection db = null;
		String ticketId = context.getRequest().getParameter("id");
	...
	public String executeCommandViewVigilanza(ActionContext context) {
		...
		PagedListInfo ticketListInfo = this.getPagedListInfo(
				context, "ImbarcazioniTicketInfo", "t.entered", "desc");
		ticketListInfo.setLink(
				"ImbarcazioniVigilanza.do?command=ViewVigilanza&orgId=" + passedId);
