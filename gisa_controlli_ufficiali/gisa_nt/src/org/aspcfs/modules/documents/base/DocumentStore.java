...
public class DocumentStore extends GenericBean {
  ...
  public DocumentStore(Connection db, int tmpId,boolean reperibilitaAsl) throws SQLException {
	    this.queryRecordAsl(db, tmpId);
	  }
...
/**
  ...
  private void queryRecordAsl(Connection db, int thisId) throws SQLException {
	    if ((thisId) == -1) {
	      throw new SQLException("Invalid Document Store");
	    }
	    StringBuffer sql = new StringBuffer();
	    sql.append(
	        "SELECT * " +
	        "FROM document_store_reperibilita_asl ds " +
	        " where ds.document_store_id = ? ");
  ...
  public boolean insert(Connection db,ActionContext  context) throws SQLException {
    ...
    try {
      ...
      id = DatabaseUtils.getNextSeq(db,context, "document_accounts","document_store_id");
      int i = 0;
      ...
      DocumentStorePermissionList.insertDefaultPermissions(db, id,context);
  ...
  private void buildRecordAsl(ResultSet rs) throws SQLException {
	    this.id = rs.getInt("document_store_id");
	    this.templateId = rs.getInt("template_id");
	    this.title = rs.getString("title");
	    this.shortDescription = rs.getString("shortDescription");
