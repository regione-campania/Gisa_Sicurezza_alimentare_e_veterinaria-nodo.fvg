 ...
 */
public final class RiproduzioneAnimaleVigilanza extends CFSModule {
	...
	public String executeCommandViewVigilanza(ActionContext context) {
		...
		ticketListInfo.setLink(
				"RiproduzioneAnimaleVigilanza.do?command=ViewVigilanza&orgId=" + passedId);
	...
	public String executeCommandAdd(ActionContext context) {
		if (!hasPermission(context, "riproduzioneanimale-vigilanza-add")) {
			return ("PermissionError");
		}
		Connection db = null;
		...
		try {
			...
			context.getRequest().setAttribute("tipologia", 8);
	...
	public String executeCommandInsert(ActionContext context) throws SQLException, ParseException {
		...
		try
		{
			...
			addModuleBean(context, "Riproduzione Animale CU", "Riproduzione Animale CU");
	...
	public String executeCommandModifyTicket(ActionContext context) {
		if (!hasPermission(context, "riproduzioneanimale-vigilanza-edit")) {
			return ("PermissionError");
		}
		Connection db = null;
		Ticket newTic = null;
...
//			}
	...
	public String executeCommandUpdateTicket(ActionContext context) {
		...
		try {
			...
			context.getRequest().setAttribute("tipologia",7);
	...
	public String executeCommandConfirmDelete(ActionContext context) {
		if (!hasPermission(context, "riproduzioneanimale-vigilanza-delete")) {
			return ("PermissionError");
		}
		...
		{	db=this.getConnection(context);
			org.aspcf.modules.controlliufficiali.action.AccountVigilanza controlliGeneric = new org.aspcf.modules.controlliufficiali.action.AccountVigilanza();
			context.getRequest().setAttribute("url_confirm","RiproduzioneAnimaleVigilanza.do?command=DeleteTicket");
	...
	public String executeCommandDeleteTicket(ActionContext context) {
		...
		try {
			...
			context.getRequest().setAttribute("OrgDetails", newOrg);
			org.aspcf.modules.controlliufficiali.action.AccountVigilanza controlliGeneric = new org.aspcf.modules.controlliufficiali.action.AccountVigilanza();
			context.getRequest().setAttribute("url","RiproduzioneAnimaleVigilanza.do?command=ViewVigilanza");
	...
	public String executeCommandDeleteListaDistribuzione(ActionContext context) {
		...
		try {
			...
			String filePath = this.getPath(context, "riproduzioneanimale");
