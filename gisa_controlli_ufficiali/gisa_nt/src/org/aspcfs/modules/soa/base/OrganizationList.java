...
public void setListaStatiIstruttoria(String[] listaStatiIstruttoria) {
	this.listaStatiIstruttoria = listaStatiIstruttoria;
  	...
  	public int getTipologiaSoa() {
	return tipologiaSoa;
}

public void setTipologiaSoa(int tipologiaSoa) {
	this.tipologiaSoa = tipologiaSoa;
}

public void setTipologiaSoa(String tipologiaSoa) {
	if (tipologiaSoa != null && ! "".equals(tipologiaSoa))
	this.tipologiaSoa = Integer.parseInt(tipologiaSoa);
}
...
public int getStatoLab() {
  ...
  public ResultSet queryList(Connection db, PreparedStatement pst) throws SQLException {
    ...
    sqlCount.append(
        " SELECT COUNT( distinct org_id ) AS recordcount " +
        " FROM organization o " + join() +
        " WHERE o.org_id >= 0 ");
    ...
    sqlSelect.append( " distinct on (jjj.org_id) jjj.* from ( select " );
	...
	private void and(StringBuffer sqlFilter)
	{
		...
		if( tipologiaSoa > -1 )
		{
			sqlFilter.append( " AND lcatsoa.tipo_soa = ? " );
		}
	}
	...
	private String join()
	{
		...
		if( (tipologiaSoa>0 ||  sottoAttivita > -1 ) 
			|| ( codiceSezione > -1 ) )
		{
			ret = " JOIN soa_sottoattivita ssa ON (o.org_id = ssa.id_soa)  " +
					" left join lookup_categoria_soa  lcatsoa on lcatsoa.code = ssa.codice_sezione ";
		}
