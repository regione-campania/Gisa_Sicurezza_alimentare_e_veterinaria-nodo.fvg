 ...
 */
package org.aspcfs.modules.zonecontrollo.base;
...
public class OrganizationList extends Vector implements SyncableList {
  ...
  private static final int TIPOLOGIA_zonecontrollo = 15 ;
...
public int getSiteId() {
  ...
  public void setAlert(String tmp) {
	    this.alert = tmp;
	  }
  ...
  public ResultSet queryList(Connection db, PreparedStatement pst) throws SQLException {
	    ...
	    sqlCount.append(
	        ...
	        " WHERE o.org_id > 0 and tipologia = "+TIPOLOGIA_zonecontrollo +" ");	   
...
//	            +" AND  " + DatabaseUtils.toLowerCase(db) + "(o.name) < ? ");
	    ...
	    sqlSelect.append(
	            ...
	            " FROM organization o " +
	            " LEFT JOIN organization_address oa on ( o.org_id = oa.org_id and oa.address_type = 5) " +
	            " LEFT JOIN lookup_site_id asl ON (o.site_id = asl.code) WHERE o.org_id > 0 and tipologia = "+TIPOLOGIA_zonecontrollo+" " );
  ...
  protected void createFilter(Connection db, StringBuffer sqlFilter) {
	  ...
	  //City legale
	    ...
	    //Descrizione luogo di controllo
	    if (alert != null && (!alert.equals("")) ) {
	        if (alert.indexOf("%") >= 0) {
	        	sqlFilter.append(" AND (o.alert ILIKE ? ");
		    	sqlFilter.append(" OR o.name ILIKE ? ) ");

	        } else {
	        	sqlFilter.append(" AND (o.alert = ? ");
		    	sqlFilter.append(" OR o.alert = ? ) ");
  ...
  protected int prepareFilter(PreparedStatement pst) throws SQLException {
	    ...
	    //alert luogo di controllo
	    if (alert != null && !(alert.equals(""))) {
		      pst.setString(++i, alert.toLowerCase().replaceAll(" ", "%"));
		      pst.setString(++i, alert.toLowerCase().replaceAll(" ", "%"));
		}
  ...
  public void buildListOperatori(Connection db, String ticketId)  throws SQLException {
      ...
      while (rs.next()) {
		  ...
		  thisOrganization.setAlert(rs.getString("alert"));
		  thisOrganization.getAddressList().setOrgId(thisOrganization.getOrgId());
		  thisOrganization.getAddressList().buildList(db);
		  this.add(thisOrganization);
      }
	  rs.close();
