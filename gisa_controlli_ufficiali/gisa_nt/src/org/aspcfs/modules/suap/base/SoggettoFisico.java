...
public class SoggettoFisico extends GenericBean  {
	...
	public SoggettoFisico(String cf,Connection db) throws SQLException {
		...
		sqlSelect.append(
				"SELECT distinct i.toponimo,i.civico,comnasc.id as id_comune_nascita," +
						"o.nome,o.cognome," +
						...
						" left join suap_ric_scia_rel_operatore_soggetto_fisico os on (o.id = os.id_soggetto_fisico) " +
						" left join suap_ric_scia_operatore op on (os.id_operatore = op.id)  where trim(o.codice_fiscale) ilike ? and o.comune_nascita!='' and o.nome!='' and o.cognome !='' and o.data_nascita is not null " 
						);
	...
	public SoggettoFisico(HttpServletRequest request,org.aspcfs.utils.web.LookupList nazioniList) {
		...
		if("106".equals(nazioneResidenza))
		{
			...
			if(request.getParameter("addressLegaleCityinput")!=null && !request.getParameter("addressLegaleCityinput").equals(""))
			{
				this.indirizzo.setDescrizioneComune(request.getParameter("addressLegaleCityinput"));
			}
			else if(request.getParameter("addressLegaleCitta")!=null && !request.getParameter("addressLegaleCitta").equals(""))
			{
		...
		else
		{
			...
			this.indirizzo.setVia(request.getParameter("addressLegaleLine1"));
		}
	...
	public SoggettoFisico(Connection db, HttpServletRequest request,MultipartRequest multiPart,org.aspcfs.utils.web.LookupList nazioniList) {




		this.nome = (String) multiPart.getParameter("nome");
		this.cognome = (String) multiPart.getParameter("cognome");
		this.sesso = (String) multiPart.getParameter("sesso");
		this.setDataNascita((String) multiPart.getParameter("dataNascita"));
		this.comuneNascita = (String) multiPart.getParameter("comuneNascitainput");
		this.provinciaNascita = (String) multiPart.getParameter("provinciaNascita");
		this.nazioneNascita= (String) multiPart.getParameter("nazioneNascita");
		this.codFiscale = (String) multiPart.getParameter("codFiscaleSoggetto");

		this.nazioneResidenza= (String) multiPart.getParameter("nazioneResidenza");
		this.indirizzo = new Indirizzo();
		...
		{
			indirizzo.setNazione(nazioniList.getSelectedValue(Integer.parseInt(nazioneResidenza)));
			if (multiPart.getParameter("addressLegaleLine1input") != null)
			{
				
				this.indirizzo.setVia(multiPart.getParameter("addressLegaleLine1input"));

			}
			else
			{
				this.indirizzo.setVia(multiPart.getParameter("addressLegaleLine1Testo"));
			}

			if(multiPart.getParameter("addressLegaleCity")!=null && !multiPart.getParameter("addressLegaleCity").equals(""))
				this.indirizzo.setComune(multiPart.getParameter("addressLegaleCity"));
			else
				this.indirizzo.setComune(multiPart.getParameter("addressLegaleCityId"));
			if(request.getParameter("addressLegaleCityinput")!=null && !request.getParameter("addressLegaleCityinput").equals(""))
			{
				this.indirizzo.setDescrizioneComune(multiPart.getParameter("addressLegaleCityinput"));
			}
			else if(request.getParameter("addressLegaleCitta")!=null && !request.getParameter("addressLegaleCitta").equals(""))
			{
				this.indirizzo.setDescrizioneComune(multiPart.getParameter("addressLegaleCitta"));
			}
			
			
			if(multiPart.getParameter("addressLegaleCountry")!=null && !"".equals(multiPart.getParameter("addressLegaleCountry")))
			{
				this.indirizzo.setProvincia(multiPart.getParameter("addressLegaleCountry"));
				this.indirizzo.setIdProvincia(Integer.parseInt(multiPart.getParameter("addressLegaleCountry")));
				this.indirizzo.setDescrizione_provincia(multiPart.getParameter("addressLegaleCountryinput"));
			} 
			else if(multiPart.getParameter("addressLegaleCountrySigla")!=null && !"".equals(multiPart.getParameter("addressLegaleCountrySigla")))
			{
				Provincia p = new Provincia().getProvinciaBySigla(db, multiPart.getParameter("addressLegaleCountrySigla"));
		...
		else
		{
			this.indirizzo.setNazione(nazioniList.getSelectedValue(Integer.parseInt(nazioneResidenza)));
			this.indirizzo.setComune(-1);
			this.indirizzo.setComuneTesto(multiPart.getParameter("addressLegaleCityinput"));
			this.indirizzo.setVia(multiPart.getParameter("addressLegaleLine1"));
		}
		
		if(multiPart.getParameter("toponimoResidenza")!=null && !multiPart.getParameter("toponimoResidenza").equals(""))
			this.indirizzo.setToponimo(multiPart.getParameter("toponimoResidenza"));
		else
			this.indirizzo.setToponimo(multiPart.getParameter("toponimoResidenzaId"));

		this.indirizzo.setCivico(multiPart.getParameter("civicoResidenza"));
		

		this.indirizzo.setCap(multiPart.getParameter("capResidenza"));
	...
	public SoggettoFisico(Connection db, int idSoggetto) throws SQLException{
		...
		if (idSoggetto>0)
		{
			...
			sqlSelect.append(
					"SELECT distinct " +
							"o.nome,o.cognome," +
							"o.codice_fiscale,o.comune_nascita,o.provincia_nascita,o.data_nascita,o.sesso,o.telefono1,o.telefono,comnasc.id as id_comune_nascita," +
							"o.email,o.fax,o.enteredby,o.modifiedby,o.documento_identita,o.provenienza_estera," +
							" o.id as id_soggetto ," +
							"i.id,i.comune,i.provincia,i.cap,i.via,i.nazione,'R'||comuni1.codiceasl_bdn as codice_asl,i.latitudine,i.longitudine,asl.code , asl.description ,comuni1.cod_provincia,comuni1.nome as descrizione_comune," +
							"lp.description as descrizione_provincia   " +
							"FROM suap_ric_scia_soggetto_fisico o " +
							" left join opu_indirizzo i on o.indirizzo_id=i.id " +
							" left join comuni1 on (comuni1.id = i.comune) " +
							...
							" left join lookup_province lp on lp.code = comuni1.cod_provincia::int " +
							" left join suap_ric_scia_rel_operatore_soggetto_fisico os on (o.id = os.id_soggetto_fisico) " +
							" left join suap_ric_scia_operatore op on (os.id_operatore = op.id)  where o.id = ?" 
							 );
	...
	public int insert(Connection db,ActionContext context) throws SQLException{
		...
		try{
			...
			sql.append("INSERT INTO suap_ric_scia_soggetto_fisico (");

			if (idSoggetto > -1)
				sql.append("id,");

			sql.append("cognome, nome, data_nascita, comune_nascita, provincia_nascita, codice_fiscale ");
	...
	public boolean aggiungiRelazione(Connection db, int idOperatore, int tipoLegame,ActionContext context) throws SQLException{
		...
		try{
			...
			//Controllare se c'e' gia' soggetto fisico, se no inserirlo
			int idRelazione = DatabaseUtils.getNextSeq(db,context, "suap_ric_scia_rel_operatore_soggetto_fisico","id");

			sql.append("INSERT INTO suap_ric_scia_rel_operatore_soggetto_fisico (");
