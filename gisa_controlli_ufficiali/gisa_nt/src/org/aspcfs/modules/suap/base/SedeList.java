...
public class SedeList extends Vector implements SyncableList {
	...
	public ResultSet queryListSediOperatore(Connection db, PreparedStatement pst) throws SQLException {
		...
		sqlCount.append(
				...
				" left join suap_ric_scia_operatore o on (o.id = r.id_operatore) " +
		...
		sqlSelect.append(
				"distinct i.*, 1 as tipologia_sede,comuni1.cod_provincia,comuni1.nome as descrizione_comune,"
				+ "lp.description as descrizione_provincia, lt.description as descrizione_toponimo " +
				"FROM opu_indirizzo i  " +
				" left join suap_ric_scia_operatore o on o.id_indirizzo =i.id " +
				" left join comuni1 on (comuni1.id = i.comune) " +
				" left join lookup_toponimi lt on lt.code = i.toponimo " +
	...
	public ResultSet queryListSediOperatoreStorico(Connection db, PreparedStatement pst) throws SQLException {
		...
		sqlSelect.append(
				...
				"FROM opu_indirizzo i " +
				" left join opu_operatore o on o.id_indirizzo = i.id " +
				" left join comuni1 on (comuni1.id = i.comune) " +
		" left join lookup_province lp on lp.code = comuni1.cod_provincia::int ");
