...
public void setCodice(String codice) {
  ...
  public ArrayList<Comuni> buildList(Connection db, int asl) throws SQLException {
	    ...
	    if(asl!=-1)
	    sql.append("SELECT id,nome FROM comuni1 c , lookup_site_id asl WHERE c.codiceistatasl =asl.codiceistat and asl.code = ? and c.notused is not true order by nome");
	    else
	    	sql.append("SELECT id,nome FROM comuni1 c , lookup_site_id asl WHERE  c.codiceistatasl =asl.codiceistat and c.notused is not true order by nome");
