...
public final class DistributoriList extends CFSModule
{
	...
	{	
		...
		return executeCommandDetails(context);
				...
				if(context.getRequest().getParameter("command").equals("AddOpu"))
					return executeCommandAddOpu(context);
				else
					if(context.getRequest().getParameter("command").equals("DetailsOpu"))
...
public String executeCommandAddOpu(ActionContext context) {
		...
		try {
			...
			PreparedStatement	stat	= db.prepareStatement( "select * from distributori_automatici ,lookup_tipo_distributore where alimenti_distribuiti=code and org_id="+newOrg.getIdStabilimento());
			ResultSet			rs		= stat.executeQuery();
