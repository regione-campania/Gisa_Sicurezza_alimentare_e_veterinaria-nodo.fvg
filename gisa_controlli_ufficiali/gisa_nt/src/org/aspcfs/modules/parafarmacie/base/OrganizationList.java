...
public class OrganizationList extends Vector implements SyncableList {
  ...
  private static Logger log = Logger.getLogger(org.aspcfs.modules.parafarmacie.base.OrganizationList.class);
  static {
    if (System.getProperty("DEBUG") != null) {
      log.setLevel(Level.DEBUG);
    }
  }
  private static final int TIPOLOGIA_FARMACIE = 802 ;
  ...
  public final static String tableName = "farmacie";
  public final static String uniqueField = "id_farmacia";
...
public void setLongitudine(double tmp) {
  ...
  public String getHtmlSelect(String selectName, int defaultKey) {
    ...
    while (i.hasNext()) {
      ...
      orgListSelect.addItem(
          thisOrg.getIdFarmacia(),
          thisOrg.getRagioneSociale());
    }
  ...
  public void buildShortList(Connection db) throws SQLException {
    ...
    while (rs.next()) {
      ...
      thisOrg.setIdFarmacia(rs.getInt("id_farmacia"));
      thisOrg.setOrgId(thisOrg.getIdFarmacia());
      thisOrg.setSiteId(rs.getString("site_id"));
      thisOrg.setRagioneSociale(rs.getString("ragione_sociale"));
      thisOrg.setIndirizzo(rs.getString("indirizzo"));
      ...
      thisOrg.setDataRicDettaglio(rs.getString("data_ric_dettaglio"));
      thisOrg.setNumRicDettaglio(rs.getString("num_ric_dettaglio"));
      thisOrg.setDataRicIngrosso(rs.getString("data_ric_ingrosso"));
      thisOrg.setNumRicIngrosso(rs.getString("num_ric_ingrosso"));
      thisOrg.setDataVerificaAnnua(rs.getString("data_verifica_annua"));
      thisOrg.setProvincia(rs.getString("provincia"));
      thisOrg.setLatitudine(rs.getString("latitudine"));
      thisOrg.setLongitudine(rs.getString("longitudine"));
  ...
  public ResultSet queryList2(Connection db, PreparedStatement pst) throws SQLException {
	  ...
	  PreparedStatement st = null;
	    ...
	    sql.append("SELECT * FROM farmacie ORDER BY ragione_sociale");
	    st = db.prepareStatement(sql.toString());
	    
	    rs = st.executeQuery();
	    
	    if (rs.next()) {
	      ragioneSociale = rs.getString(3);
	    }
	   return rs;
	  }
  ...
  public ResultSet queryList(Connection db, PreparedStatement pst) throws SQLException {
	    ...
	    sqlCount.append(
	        ...
	        " WHERE o.org_id >= 0 and o.tipologia = "+TIPOLOGIA_FARMACIE);
	    ...
	    sqlSelect.append(
	            "asl.description,oa.addrline1,oa.city,oa.state,oa.latitude,oa.longitude,o.*" +
	            " FROM organization o " +
	            " left join organization_address oa on ( o.org_id = oa.org_id) "+
  ...
  public ResultSet queryListV(Connection db, PreparedStatement pst) throws SQLException {
    ...
    if (pagedListInfo != null) {
      ...
      //Determine column to sort by
      pagedListInfo.setDefaultSort("f.id_farmacia", null);
      //pagedListInfo.appendSqlTail(db, sqlOrder);
      sqlOrder.append(" ORDER BY o.name LIMIT 10 OFFSET 0;");
    ...
    sqlSelect.append(
        "f.*, asl.description" +
        " FROM farmacie f " +
        " left JOIN lookup_site_id asl ON (f.site_id = asl.code) " +
        " where f.id_farmacia >= 0 " );
  ...
  protected void createFilter(Connection db, StringBuffer sqlFilter) {
	    ...
	    if (tipoAutorizzazzione != -1)
	    {
	 
	    	if (tipoAutorizzazzione==1){ //TIPO AUTORIZZAZZIONE INGROSSO
	    		sqlFilter.append(" AND ((trim(o.num_ric_ingrosso) not in ('') and trim(num_ric_ingrosso) is not null) or (data_ric_ingrosso is not null ))");
	    		if (numAutorizzazzione!=null && !numAutorizzazzione.equals(""))
	    		{
	    			sqlFilter.append(" AND num_ric_ingrosso ilike '%"+numAutorizzazzione+"%'");
	    		}
	    	}
	    		else
	    		if (tipoAutorizzazzione==2)//TIPO AUTORIZZAZZIONE DETTAGLIO
	    		{
	    			sqlFilter.append(" AND ((trim(o.num_ric_dettaglio) not in ('') and trim(num_ric_dettaglio) is not null) or (data_ric_dettaglio is not null ))");
	    			if (numAutorizzazzione!=null && !numAutorizzazzione.equals(""))
		    		{
		    			sqlFilter.append(" AND num_ric_dettaglio ilike '%"+numAutorizzazzione+"%'");
		    		}
	    		}
	    }
	    ...
	    if ((ragioneSociale != null) &&(!ragioneSociale.equals(""))) {
	    	  sqlFilter.append(" AND  o.name ilike '%"+ragioneSociale.replaceAll("'", "''")+"%' ");
	        }
	      if (orgSiteId > -1) {
	    ...
	    if ((citta != null)&&(!citta.equals(""))) {
	    	  ...
	    	  citta = citta.trim();	
	    	  sqlFilter.append(" AND  oa.city ilike '%"+citta.replaceAll("'", "''")+"%' ");
	        }
	    if (stageId > -1) {
  ...
  protected void createFilterV(Connection db, StringBuffer sqlFilter) {
    ...
    /*if (idFarmacia > -1) {
      ...
      if ((ragioneSociale != null) ||(!ragioneSociale.equals(""))) {
    	  sqlFilter.append(" AND  f.ragione_sociale like '%" + ragioneSociale + "%' ");
        }
      if (siteId > -1) {
