 ...
 */
package org.aspcfs.modules.diffida.base;
...
public class TicketList extends org.aspcfs.modules.troubletickets.base.TicketList
{
	...
	private Boolean dataChiusura = null;
	
	
	public Boolean getDataChiusura() 
	{
		return dataChiusura;
	}

	public void setDataChiusura(Boolean dataChiusura) 
	{
	...
	public int contaSanzioni(Connection db) throws SQLException {
	    ...
	    sqlCount.append(
	        ...
	        "FROM registro_ordinanze_view t ");
	    rs = db.prepareStatement(sqlCount.toString()).executeQuery();
	    if (rs.next())
	    	return rs.getInt(1);
	    return 0 ;
	...
	public void buildListDiffideOsaCentralizzato(Connection db) throws SQLException {
	    PreparedStatement pst = null;
	    ResultSet rs = null;
	    int items = -1;
	    StringBuffer sqlSelect = new StringBuffer();
	    ...
	    sqlCount.append(
	        ...
	        "FROM ticket t " 
	    	 ...
	    	 sqlCount.append(" left JOIN organization o ON (t.org_id = o.org_id) " +
		        "INNER JOIN ticket tic ON ((tic.tipologia = 8)and(tic.id_controllo_ufficiale = t.id_controllo_ufficiale)) " +
		        " JOIN ticket cu on (cu.id_controllo_ufficiale = tic.id_controllo_ufficiale and cu.tipologia=3) ");
	 	    sqlCount.append("INNER JOIN salvataggio_nc_note snn ON (snn.idticket =t.id_nonconformita and snn.tipologia = t.tipo_nc and  t.tipo_nc = 3) ");

	    sqlCount.append(

	        " where t.ticketid > 0 AND t.tipologia = 11  and current_timestamp<= (cu.assigned_date + interval '5 years')");
	    
	    if (this.orgId>0)
	    	sqlCount.append(" and t.org_id = "+orgId+" ");
	    else
	    	if (this.idStabilimento>0)
	    		sqlCount.append(" and t.id_stabilimento = "+idStabilimento+ " ");
	    	else if (this.altId>0)
	    		sqlCount.append(" and t.alt_id = "+altId+ " ");
	    	else
	    		sqlCount.append(" and t.id_apiario = "+idApiario+ " ");
	    ...
	    if (pagedListInfo != null) {
	      ...
	      sqlOrder.append(" ORDER BY t.assigned_date desc ");
	    }
	    ...
	    sqlSelect.append(
	        ...
	        "o.site_id AS orgsiteid,o.tipologia as tipo_operatore,cu.id_macchinetta as id_distributore ," +
	        "o.site_id AS orgsiteid,o.tipologia as tipo_operatore,  st.id_operatore as id_opu_operatore " +

	        " FROM ticket t "+
	        " JOIN ticket nonconf ON nonconf.ticketid = t.id_nonconformita AND nonconf.tipologia = 8 " +
	        " JOIN ticket cu ON cu.id_controllo_ufficiale::text = nonconf.id_controllo_ufficiale::text AND cu.tipologia = 3 " +
	        " left JOIN organization o ON (t.org_id = o.org_id)"
	        + " left JOIN opu_stabilimento st on st.id=t.id_stabilimento or st.alt_id = t.alt_id" +
	        " WHERE t.ticketid > 0 AND t.tipologia = 11    and current_timestamp< (cu.assigned_date + interval '5 years')  and t.trashed_date is null and cu.trashed_date is null and nonconf.trashed_date is null ");
		    	
	    if (this.orgId>0)
	    	sqlSelect.append(" and t.org_id = "+orgId+" and o.trashed_date is null ");
	    else
	    	if (this.idStabilimento>0)
		    	sqlSelect.append(" and t.id_stabilimento = "+idStabilimento+ " ");
	    	else if (this.altId>0)
	    		sqlSelect.append(" and t.alt_id = "+altId+ " ");
	    	else 
	...
	public void buildListDiffideOsa(Connection db) throws SQLException {
	    PreparedStatement pst = null;
	    ResultSet rs = null;
	    int items = -1;
	    StringBuffer sqlSelect = new StringBuffer();
	    ...
	    sqlSelect.append(
	        ...
	        " JOIN ticket cu ON cu.id_controllo_ufficiale::text = nonconf.id_controllo_ufficiale::text AND cu.tipologia = 3 " +
	        " left JOIN organization o ON (t.org_id = o.org_id) " +
	        " where t.ticketid > 0 AND t.tipologia = 11  and o.trashed_date is null  and current_timestamp< (cu.assigned_date + interval '5 years')  and t.trashed_date is null and cu.trashed_date is null and nonconf.trashed_date is null ");
		    	
	    if (this.orgId>0)
	    	sqlSelect.append(" and t.org_id = "+orgId+" ");
	    else
	    	if (this.idStabilimento>0)
		    	sqlSelect.append(" and t.id_stabilimento = "+idStabilimento+ " ");
	...
	public void buildListControlliHandler(Connection db, int org_id, String idC) throws SQLException{
		if (this.altId>0)
				buildListControlliAlt(db, org_id, idC);
		else
				buildListControlli(db, org_id, idC);				 
	}
	...
	public void buildListControlli(Connection db, int org_id, String idC) throws SQLException {
	    ...
	    sqlCount.append(
	        ...
	        " where t.ticketid > 0 AND t.tipologia = 11 and tic.ticketid = "+idC+" ");
	   ...
	   // " left JOIN opu_operatori_denormalizzati_view oo ON (t.id_stabilimento = oo.id_stabilimento) "  +
	        " JOIN ticket nonconf ON nonconf.ticketid = t.id_nonconformita AND nonconf.tipologia = 8 " +
	        " JOIN ticket cu ON cu.id_controllo_ufficiale::text = nonconf.id_controllo_ufficiale::text AND cu.tipologia = 3 " +
	        " left JOIN organization o ON (t.org_id = o.org_id and o.trashed_date is null) " +
	        " where  t.tipologia = 11 and nonconf.ticketid = "+idC+"   and t.trashed_date is null and cu.trashed_date is null and nonconf.trashed_date is null ");
	...
	public void buildListControlliAlt(Connection db, int org_id, String idC) throws SQLException {
	    PreparedStatement pst = null;
	    ResultSet rs = null;
	    int items = -1;
	    StringBuffer sqlSelect = new StringBuffer();
	...
	public void buildList(Connection db) throws SQLException {
		    ...
		    sqlCount.append(
		        ...
		        " where t.ticketid > 0 AND t.tipologia = 11 and o.trashed_date is null  and t.trashed_date is null and cu.trashed_date is null and nonconf.trashed_date is null ");
		    createFilter(sqlFilter, db);
		    if (pagedListInfo != null) {
		      //Get the total number of records matching filter
		      pst = db.prepareStatement(
		    ...
		    sqlSelect.append(
		        ...
		        "o.site_id AS orgsiteid ,cu.id_macchinetta as id_distributore " +
		    ...
		    while (rs.next()) {
		      Ticket thisTicket = new Ticket(rs);
		      thisTicket.setIdRuoloUtente(idRuoloUtente);
		      thisTicket.setFieldsRegistro(db);
		      this.add(thisTicket);
		    }
		    rs.close();
		    pst.close();
	...
	public void buildListRegistroOrdinanze(Connection db) throws SQLException {
	    PreparedStatement pst = null;
	    ResultSet rs = null;
	    int items = -1;
	    StringBuffer sqlSelect = new StringBuffer();
	    ...
	    sqlSelect.append(
	        ...
	        "FROM ticket t " +
	        " left JOIN organization o ON (t.org_id = o.org_id) " +
	        " where t.ticketid > 0 AND t.tipologia = 11 ");
	    pst = db.prepareStatement(
	        sqlSelect.toString() + sqlFilter.toString() + sqlOrder.toString());
	    items = prepareFilter(pst);
	    if (pagedListInfo != null) {
...
protected void createFilter(StringBuffer sqlFilter, Connection db) {
    ...
    if ((tipologia == 1)) {
	      sqlFilter.append(" AND  t.tipologia = 1 ");
	} 
	...
	if (enteredBy > -1) {
	    ...
	    if (id > -1) {
	      if (pagedListInfo == null || pagedListInfo.getMode() != PagedListInfo.DETAILS_VIEW) {
			...
			if (dataChiusura!=null && dataChiusura) 
			{
				sqlFilter.append(" AND  ((t.data_chiusura is not null and t.tipologia = 11) or t.tipologia<> 11)");
			}
			else if (dataChiusura!=null && !dataChiusura) 
	    	{
				sqlFilter.append(" AND  ((t.data_chiusura is null and t.tipologia = 11) or t.tipologia<> 11)");
	    	} 
