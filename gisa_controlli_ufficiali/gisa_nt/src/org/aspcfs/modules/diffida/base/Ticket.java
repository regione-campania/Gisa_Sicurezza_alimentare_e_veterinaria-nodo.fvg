...
public class Ticket extends org.aspcfs.modules.troubletickets.base.Ticket
{
	...
	private String partitaIvaVecchia;
	...
	public String[] getIdNormeViolate() {
		return idNormeViolate;
	}
	public void setIdNormeViolate(String[] idNormeViolate) {
		this.idNormeViolate = idNormeViolate;
	}
	...
	public String getPartitaIvaVecchia() {
		return partitaIvaVecchia;
	}
	public void setPartitaIvaVecchia(String partitaIvaVecchia) {
		this.partitaIvaVecchia = partitaIvaVecchia;
	}
	...
	public void setTipo_richiesta(String tipo_richiesta) {
	  ...
	  public void queryRecord(Connection db, int id) throws SQLException {
		    if (id == -1) {
		      throw new SQLException("Invalid Ticket Number");
		       ...
		       String sql =  "SELECT t.*,cu.ticketid as id_cu, t.partita_iva_vecchia,  " +
		        ...
		        sql += " where t.ticketid = ? AND t.tipologia = 11 ";
		        PreparedStatement pst = db.prepareStatement(sql);
		    pst.setInt(1, id);
		    ResultSet rs = pst.executeQuery();
		    if (rs.next()) {
		      buildRecord(rs);
		      this.setNormeViolateDiffida(db);
		      if (soa == false)
		      {
	  ...
	  public void updateNormeViolate(Connection db , String[] valori)throws SQLException{
			...
			String sql="delete from norme_violate_sanzioni where idticket=? and org_id = ? ";
			PreparedStatement pst=db.prepareStatement(sql);
			pst.setInt(1, id);
			if(orgId>0)
				pst.setInt(2,orgId);
	  ...
	  public void setNormeViolate(Connection db)throws SQLException{
			...
			String sql=" select code,description, " +
			           "        CASE WHEN norme_violate_sanzioni.stato_diffida = '1' then ' [DIFFIDA NON PIU'' ATTIVA IN QUANTO L''OSA E'' STATO SANZIONATO PER QUESTA NORMA]' "+ 
					   "             WHEN t.data_chiusura is not null then '[DIFFIDA NON PIU'' ATTIVA IN QUANTO L''OSA HA SUBITO UNA VARIAZIONE DI TITOLARITA'']' " + 
			           "             WHEN norme_violate_sanzioni.stato_diffida = '0' then '[DIFFIDA ATTIVA PER I PROSSIMI 5 ANNI]'  " + 
					   "        END AS stato " + 
			           " from lookup_norme, norme_violate_sanzioni, ticket t " + 
					   " where t.ticketid = norme_violate_sanzioni.idticket and lookup_norme.enabled = true and norme_violate_sanzioni.id_norma=lookup_norme.code  "
					+ " AND  norme_violate_sanzioni.idticket= ? ";//and org_id = ? ";
			//ELIMINATA GESTIONE ORG_ID PERCHE' 1) IN UNA SOLA COLONNA C'ERA TUTTO 2) INUTILE SE ABBIAMO GIA' ID DIFFIDA
			PreparedStatement pst=db.prepareStatement(sql);
			pst.setInt(1, id);
//			if(orgId > 0){
//				pst.setInt(2, orgId);
...
//			}
		...
		while(rs.next()){
			...
			String value=rs.getString("description")+ "  "+ rs.getString("stato");
			listaNorme.put(code, value);
			
		}
	  }
	  
	  public void setNormeViolateDiffida(Connection db)throws SQLException{
		  
			String sql="select code,description from lookup_norme, norme_violate_sanzioni where lookup_norme.enabled = true and norme_violate_sanzioni.id_norma=lookup_norme.code and norme_violate_sanzioni.idticket= ? and org_id = ? ";
			PreparedStatement pst=db.prepareStatement(sql);
			pst.setInt(1, id);
			if (orgId>0)
				pst.setInt(2, orgId);
	  ...
	  public synchronized boolean insert(Connection db,ActionContext context) throws SQLException {
			...
			if (user.getUserRecord().getGruppo_ruolo()==2)
		      ...
		      sql.append("?, ?, ?, ?, " +
		      		     "11, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? "); //ho aggiunto 2 punti interrogativi
		      String asl = null;
		      if(siteId==201){
					asl = "AV";	
				}else if(siteId == 202){
	  ...
	  protected void buildRecord(ResultSet rs) throws SQLException {
		    ...
		    try {rs.findColumn("partita_iva_vecchia"); partitaIvaVecchia = rs.getString("partita_iva_vecchia"); } catch (SQLException sqlex){ }
		  		    
		   tipologia_operatore = rs.getInt("tipo_operatore");
		   if (idStabilimento>0)
			   tipologia_operatore = Ticket.TIPO_AZIENDE_AGRICOLE;
		   if(idApiario > 0){
		   ...
		   chiusura_attesa_esito = rs.getBoolean("chiusura_attesa_esito");
		    ...
		    try
		    {
		      rs.findColumn("id_opu_operatore");
		      idOpuOperatore = rs.getInt("id_opu_operatore");
		    ...
		    {
		    	System.out.println("Campo data_chiusura non presente nel resultset");
		    }
...
" ORDER by ordine_campo ");
		...
		public TicketList getDiffide(Connection db, boolean scadute, Integer idStabilimento, Integer idApiario,Integer orgId, String action) throws SQLException 
		{
			TicketList diffList = new TicketList();
			diffList.setDataChiusura(scadute);
			if(idStabilimento!=null)
				diffList.setIdStabilimento(idStabilimento);
			else if(idApiario!=null)
				diffList.setIdApiario(idApiario);
			else if(orgId!=null)
				diffList.setOrgId(orgId);
			diffList.buildListDiffideOsaCentralizzato(db);
			
			if(action!=null)
			{
				for(int i = 0 ; i <diffList.size();i++ )
					((Ticket) diffList.get(i)).setAction(action);
			}
			
			return diffList;
		}
		
		public TicketList getDiffide(Connection db, boolean scadute, Integer idStabilimento, Integer idApiario,Integer orgId, Integer altId, String action) throws SQLException  
		{
			TicketList diffList = new TicketList();
			diffList.setDataChiusura(scadute);
			if(idStabilimento!=null)
		...
		public int contaCU(Connection db, int idStabilimento, int altId) {
			// TODO Auto-generated method stub
			int totale = 0;
			String sql = "select count(*) as numero_cu "
					+ "from ticket "
					+ " where trashed_date is null and tipologia = 3 "
					+ " AND  (id_stabilimento = ? or alt_id = ?)";
