...
import org.aspcfs.modules.macellazionisintesis.base.Azione;
import org.aspcfs.modules.macellazionisintesis.base.Campione;
import org.aspcfs.modules.macellazionisintesis.base.Capo;
import org.aspcfs.modules.macellazionisintesis.base.Condizione;
...
//		}
		...
		if("".equals(capo.getVpm_veterinario()) || "".equals(capo.getCd_veterinario_1())){
			ret = orange;
		}
		return ret;
	}
	...
	public static String getStatoMacellazione( Capo capo, Connection db, ArrayList<Campione> cmps )
	{
		...
		{
//			if (ret.equals("OK") ) {
//				ret = "OK (Anche se manca il destinatario delle carni)"; 
//			} else {
...
//	}
	...
	public static boolean isSedutaChiusa( int idMacello, Timestamp data, int numeroSeduta, Connection db )
			throws SQLException
		{
			boolean chiusa = false;
			
			String sql = "select seduta_chiusa from m_capi_sedute where id_macello = ? and data = ? and numero = ? ";
			PreparedStatement stat = db.prepareStatement( sql );
			stat.setInt( 1, idMacello );
			stat.setTimestamp( 2, data );
			stat.setInt( 3, numeroSeduta );
			...
			if( res.next() )
			{
				chiusa = res.getBoolean("seduta_chiusa");
			}
			return chiusa;
		}

	public static int getIdControlloSeduta(int idMacello, Timestamp data, int numeroSeduta, Connection db) throws SQLException { 
		int idCu = -1;
		
		String sql = "select id_cu from m_capi_sedute where id_macello = ? and data = ? and numero = ? ";
		PreparedStatement stat = db.prepareStatement( sql );
		stat.setInt( 1, idMacello );
		stat.setTimestamp( 2, data );
		stat.setInt( 3, numeroSeduta );
		...
		if( res.next() )
		{
			idCu = res.getInt("id_cu");
		}
		return idCu;
	}
	
	public static boolean hasSedutaCapi( int idMacello, Timestamp data, int numeroSeduta, Connection db) throws SQLException{

		int numCapi = 0;
		
		String sql = "select count(*) as tot_capi from m_capi where id_macello = ? and vpm_data = ? and cd_seduta_macellazione = ? and trashed_date is null ";
		PreparedStatement stat = db.prepareStatement( sql );
		stat.setInt( 1, idMacello );
		stat.setTimestamp( 2, data );
		stat.setInt( 3, numeroSeduta );
