...
import org.aspcfs.modules.macellazioninewsintesis.base.Tipo;
import org.aspcfs.modules.macellazioninewsintesis.utils.ConfigTipo;
import org.aspcfs.modules.macellazionisintesis.base.Campione;
import org.aspcfs.modules.macellazionisintesis.base.CampioneNew;
import org.aspcfs.modules.macellazionisintesis.base.Capo;
import org.aspcfs.modules.macellazionisintesis.base.CapoAjax;
import org.aspcfs.modules.macellazionisintesis.base.CapoLog;
import org.aspcfs.modules.macellazionisintesis.base.CapoLogDao;
import org.aspcfs.modules.macellazionisintesis.base.Casl_Non_Conformita_Rilevata;
import org.aspcfs.modules.macellazionisintesis.base.LogCancellazioneCapiPartite;
import org.aspcfs.modules.macellazionisintesis.base.NonConformita;
import org.aspcfs.modules.macellazionisintesis.base.Organi;
import org.aspcfs.modules.macellazionisintesis.base.PatologiaRilevata;
import org.aspcfs.modules.macellazionisintesis.base.ProvvedimentiCASL;
import org.aspcfs.modules.macellazionisintesis.base.RegistroTumoriRemoteUtil;
import org.aspcfs.modules.macellazionisintesis.base.RichiestaIstopatologico;
import org.aspcfs.modules.macellazionisintesis.base.Tampone;
import org.aspcfs.modules.macellazionisintesis.base.TipoRicerca;
...
public final class Macellazioni extends CFSModule {
		...
		if (context.getParameter("matricola") != null && !"".equals(context.getParameter("matricola"))) {
			...
			try {
				...
				if (c != null && c.getId() > 0) {
					...
					SintesisStabilimento org = new SintesisStabilimento(db, c.getId_macello(), true);
	...
	public String executeCommandToStampeModuli(ActionContext context) throws NumberFormatException {
		...
		try {
			...
			Iterator<Object> key = org.aspcfs.modules.macellazionisintesis.actions.ApplicationProperties
					.getApplicationProperties().keySet().iterator();
			while (key.hasNext()) {
				String kiave = (String) key.next();
				if (kiave.startsWith("GET_DATE")) {
					String select1 = org.aspcfs.modules.macellazionisintesis.actions.ApplicationProperties
							.getProperty(kiave);
			...
			SintesisStabilimento macello = new SintesisStabilimento(db,
					Integer.parseInt(context.getParameter("altId")), true);
	...
	public String executeCommandListTipi(ActionContext context) {
		...
		try {
			...
			SintesisStabilimento org = new SintesisStabilimento(db, Integer.parseInt(context.getRequest().getParameter(
					"altId")), true);
			context.getRequest().setAttribute("OrgDetails", org);
	...
	public String executeCommandToLiberoConsumo(ActionContext context) {
		...
		try {
			...
			ArrayList<Qualifica> listaQualifiche =	Qualifica.getQualificheAutoritaCompetenti(db);
		 	 ArrayList<Qualifica> listaQualificheVeterinari = new ArrayList<Qualifica>();
		 	 
		 	 for(int i=0;i<listaQualifiche.size();i++){
		 		 Qualifica q = (Qualifica) listaQualifiche.get(i);
		 		 if(q.getId() == 43 || q.getId() == 19 || q.getId() == 46 || q.getId() == 222){
		 			listaQualificheVeterinari.add(q);
		 		 }
		 	 }
		 	 
		 	 context.getRequest().setAttribute("ListaQualificheAC", listaQualificheVeterinari);
	...
	public String executeCommandDetails(ActionContext context) {
		...
		try {
			...
			SintesisStabilimento stab = new SintesisStabilimento(db, c.getId_macello(), true);
			...
			if ("true".equalsIgnoreCase(org.aspcfs.modules.util.imports.ApplicationProperties.getProperty("NUOVA_GESTIONE_CAMPIONI_MACELLI"))){
				ArrayList<CampioneNew> campioniNew = CampioneNew.load(db, c.getId());
				context.getRequest().setAttribute("CampioniNew", campioniNew);
			}
			...
			sessionePregressa = MacelliUtil.isSessionePregressa(c.getId_macello(), c.getVpm_data(),
					c.getCd_seduta_macellazione(), db);
	...
	/*
	 * d.zanfardino Aggiunta di una seduta di macellazione per il giorno scelto
	 */
	public String executeCommandAddSedutaMacellazione(ActionContext context) {
		...
		try {
			...
			if (maxSedutaMacellazione >= 3) {
				String sessioneCorrente = sessioneMacellazioneString + "-" + (maxSedutaMacellazione);
				context.getRequest().setAttribute("sessioneCorrente", sessioneCorrente);

				context.getRequest()
						.setAttribute("messaggio",
								"Impossibile aggiungere una seduta a questa data. Numero massimo di sedute consentite raggiunto.");
	...
	public String executeCommandList(ActionContext context) {
		...
		if ("true".equalsIgnoreCase(org.aspcfs.modules.util.imports.ApplicationProperties.getProperty("NUOVA_GESTIONE_CAMPIONI_MACELLI"))){
			...
			if (tipoInt > -1) {
				...
				ConfigTipo prova = new ConfigTipo(tipo);
				HttpSession prova2 = context.getSession();
				prova2.setAttribute("configTipo", prova);
			}
			...
			try {
				...
				if (sessioneMacellazioneString == null || sessioneMacellazioneString.equals("")
						|| sessioneMacellazioneString.equals("null"))
				...
				ArrayList<String> listaSessioniMacellazioneCompleta = Capo.loadSessioniMacellazioneByStabilimentoCompleta(
						altIdString, db);
				context.getRequest().setAttribute("listaSessioniMacellazioneCompleta", listaSessioniMacellazioneCompleta);
	
				if ((listaSessioniMacellazione != null && !listaSessioniMacellazione.isEmpty())
						&& (sessioneMacellazioneString == null || sessioneMacellazioneString.equals("") || sessioneMacellazioneString
								.equals("null")))
					sessioneMacellazioneString = listaSessioniMacellazione.get(0);
	
				if (sessioneMacellazioneString != null && !sessioneMacellazioneString.equals("")) {
					if (!sessioneMacellazioneString.equals("-1")) {
						Timestamp dataMacellazioneTimestamp = new Timestamp(sdf.parse(
								sessioneMacellazioneString.split("-")[0]).getTime());
						...
						capi = Capo.loadByStabilimentoPerSessioneMacellazione(altIdString, dataMacellazioneTimestamp,
								numSessioneMacellazione, db);
					} else {
					...
					else if (context.getRequest().getAttribute("AltId") != null)
				...
				if (sessioneMacellazioneString != null && !sessioneMacellazioneString.equals("")
						&& !sessioneMacellazioneString.equals("-1") && sessioneMacellazioneString.split("-").length > 1) {
				...
				} else if (sessioneMacellazioneString != null && sessioneMacellazioneString.equals("-1")
						|| sessioneMacellazioneString.split("-").length <= 1) {
					context.getRequest().setAttribute("comboSessioniMacellazione", "-1");
					if (sessioneMacellazioneString != null && !sessioneMacellazioneString.equals("")
							&& !sessioneMacellazioneString.equals("-1"))
				...
				;
				try {
					numeroSeduta = Integer.parseInt(numeroSessione);
				} catch (Exception e) {
				}
				;
				boolean sessionePregressa = false;
				...
				boolean sedutaChiusa = false;
				if (altId > 0 && numeroSeduta > 0 && data != null)
					sedutaChiusa = MacelliUtil.isSedutaChiusa(altId, data, numeroSeduta, db);
				context.getRequest().setAttribute("sedutaChiusa", String.valueOf(sedutaChiusa));
				
				if (sedutaChiusa){ 
					int idControlloSeduta  = -1;
					idControlloSeduta = MacelliUtil.getIdControlloSeduta(altId, data, numeroSeduta, db);
					context.getRequest().setAttribute("CUSedutaId", String.valueOf(idControlloSeduta));	
				}
				
				TableFacade tf = TableFacadeFactory.createTableFacade("7", context.getRequest());
	
				tf.addFilterMatcher(new MatcherKey(Integer.class, "progressivo_macellazione"), new NumberFilterMatcher(
						"##############00000000000000"));
				tf.addFilterMatcher(new MatcherKey(Integer.class, "cd_categoria_rischio"), new NumberFilterMatcher(
						"##############00000000000000"));
	
				tf.addFilterMatcher(new MatcherKey(Timestamp.class, "vpm_data"), new DateFilterMatcher("dd/MM/yyyy"));
	
				tf.setItems(capi);
				// tf.setColumnProperties( "cd_matricola", "stato_macellazione",
				// "cd_codice_azienda", "cd_data_nascita", "vpm_data",
				// "progressivo_macellazione", "entered" );
				if (enabledIstopatologico) {
					tf.setColumnProperties("cd_matricola", "stato_macellazione", "cd_codice_azienda_provenienza",
							"cd_data_nascita", "progressivo_macellazione", "cd_categoria_rischio", "vpm_data",
							"articolo17", "clona", "istopatologico");
				} else {
					tf.setColumnProperties("cd_matricola", "stato_macellazione", "cd_codice_azienda_provenienza",
							"cd_data_nascita", "progressivo_macellazione", "cd_categoria_rischio", "vpm_data",
							"articolo17", "clona"
				...
				tf.getTable().getRow().getColumn("articolo17").setTitle("Completato"); 
				if (enabledIstopatologico) {
					tf.getTable().getRow().getColumn("istopatologico").setTitle("Richiesta istopatologico");
				}
				// tf.getTable().getRow().getColumn( "entered" ).setTitle(
				// "Data Inserimento" );
				...
				if (limit.isExported()) {
					...
					cg.getCellRenderer().setCellEditor(new CellEditor() {
						public Object getValue(Object item, String property, int rowCount) {
							...
							if (idImportString != null && !idImportString.equals("") && !idImportString.equals("null")) {
								try {
									idImport = Integer.parseInt(idImportString);
								} catch (Exception e) {
								}
							}
							...
							if (temp.toLowerCase().contains("incompleto") && idImport > 0
									&& !temp.toLowerCase().contains("incompleto: veterinario mancante"))
								vaiLiberoConsumo = "<input type=\"button\" value=\"PROCESSA LIBERO CONSUMO\" onClick=\" liberoConsumoVeloce('"
										+ id + "', '" + dataMac + "'); \"/>";
					...
					cg.getCellRenderer().setCellEditor(new CellEditor() {
						public Object getValue(Object item, String property, int rowCount) {
							...
							// return "<div style=\"background-color:" + colore +
							// "\"><a href=\"Macellazioni.do?command=Details&id=" +
							// id + "&orgId=" + id_mac + "\">" + temp +
							// "</a></div>";
							return "<a href=\"MacellazioniSintesis.do?command=Details&id=" + id + "&altId=" + id_mac
									+ "\">" + temp + "</a>";
							// return temp;
					...
					if (!sedutaChiusa) {
						cg.getCellRenderer().setCellEditor(new CellEditor() {
							public Object getValue(Object item, String property, int rowCount) {
								...
								String id_mac = (String) (new HtmlCellEditor()).getValue(item, "id_macello", rowCount);
								String dataMac = (String) (new HtmlCellEditor()).getValue(item, "vpm_data", rowCount);
								String seduta = (String) (new HtmlCellEditor()).getValue(item, "cd_seduta_macellazione",
										rowCount);
								...
								// id + "&orgId=" + id_mac + "\">" + temp +
								// "</a></div>";
								return "<center><a href=\"MacellazioniSintesis.do?command=NuovoCapo&cdSedutaMacellazione="
										+ seduta
										+ "&mavamData="
										+ mavamData
										+ "&vpmData="
										...
										+ "&altId="
										+ id_mac
										+ "&clona=si \"><img src=\"images/icons/clone.png\" height=\"20px\" width=\"20px\" title=\"Clona capo\" /></a></center>";
								// return temp;
					...
					if (enabledIstopatologico) {
						cg = (HtmlColumn) tf.getTable().getRow().getColumn("istopatologico"); // COMMENTARE/DECOMMENTARE
																								// DA
																								// QUA
						cg.setFilterable(false);
						cg.getCellRenderer().setCellEditor(new CellEditor() {
							public Object getValue(Object item, String property, int rowCount) {
								...
								if (hasPermission(context, "stabilimenti-stabilimenti-istopatologico-view")) {
									...
									String id = (String) (new HtmlCellEditor()).getValue(item, "id", rowCount);
									String id_mac = (String) (new HtmlCellEditor()).getValue(item, "id_macello", rowCount);
									String istopatologico = (String) (new HtmlCellEditor()).getValue(item,
											"istopatologico_richiesta", rowCount);
									String istopatologico_id = (String) (new HtmlCellEditor()).getValue(item,
											"istopatologico_id", rowCount);
									boolean istopatologicoEseguito = new Boolean(istopatologico).booleanValue();
	
									// ArrayList<Organi> organi =
									// Organi.loadByOrgani (Integer.parseInt(id),
									// configTipo );
									...
									if (istopatologico_id != null && !("").equals(istopatologico_id)) {
										try {
											richiesteIstopatologico = RichiestaIstopatologico.load(
													Integer.valueOf(istopatologico_id), null);
	
											if (organiTumorali != null && organiTumorali.size() > 0) {
												// Iterator i = organi.iterator();
	
												// while (i.hasNext()){
												// Organi thisOrgano = (Organi)
												// i.next();
												...
												if (!istopatologicoEseguito) {
													...
													// + id + "&orgId=" + id_mac +
													// "\">" + temp + "</a></div>";
													toReturn += "<center><a href=\"MacellazioniSintesis.do?command=RichiestaIstopatologico&id="
															+ id
															+ "&altId="
															+ id_mac
															+ " \"><img src=\"images/icons/clone.png\" height=\"20px\" width=\"20px\" title=\"Richiedi istopatologico\" /></a></center>";
	
												} else if (istopatologicoEseguito) {
													toReturn += "<div>"
															+ "<a href=\"GestioneDocumenti.do?command=GeneraPDFInvioCampioniIstopatologico&idIstopatologico="
															+ istopatologico_id + " \">Stampa Modello Invio Campioni</a>"
															+ "</div> ";
													if (richiesteIstopatologico.getIdEsito() > 0)
														toReturn += "<div>"
																+ "<a onclick=\"window.open('Macellazioni.do?command=VisualizzaEsitoRichiestaIstopatologico&idIstopatologico="
																+ istopatologico_id
																+ "&popup=true', 'titolo', 'width=600, height=600, resizable, status, scrollbars=1, location');\" href=\"#\" + \">Visualizza esito</a>"
																+ "</div> ";
													if (richiesteIstopatologico.getIdEsito() <= 0)
														toReturn += "<div>"
																+ "<a onclick=\"window.open('Macellazioni.do?command=PrepareInserisciEsitoRichiestaIstopatologico&idIstopatologico="
																+ istopatologico_id
																+ "&popup=true', 'titolo', 'width=600, height=600, resizable, status, scrollbars=1, location');\" href=\"#\" + \">Inserisci esito</a>"
																+ "</div> ";
					...
					// cg = (HtmlColumn)
					// tf.getTable().getRow().getColumn("cd_codice_azienda");
					// cg.getFilterRenderer().setFilterEditor(new
					...
					cg.setFilterable(true);
					cg.getCellRenderer().setCellEditor(new DateCellEditor() {
						public Object getValue(Object item, String property, int rowCount) {
							SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm");
							SimpleDateFormat sdf2 = new SimpleDateFormat("dd/MM/yyyy");
							...
							if (stato.equals("Incompleto: Inseriti solo i dati sul controllo documentale")
									|| stato.equals("Incompleto: Veterinario mancante"))
								return "";
							else
								return vpmData;
						}
					...
					cg.getCellRenderer().setCellEditor(new NumberCellEditor() {
						public Object getValue(Object item, String property, int rowCount) {
							String temp = (String) (new HtmlCellEditor()).getValue(item, property, rowCount);
					...
					/*
					 * cg = (HtmlColumn)
					 * tf.getTable().getRow().getColumn("vpm_data");
					 * cg.setFilterable( true ); cg.getCellRenderer().setCellEditor(
					 * new CellEditor() { public Object getValue(Object item, String
					 * property, int rowCount) { Timestamp tm = ((Capo)
					 * item).getVpm_data(); SimpleDateFormat sdf = new
					 * SimpleDateFormat("dd/MM/yyyy"); return sdf.format(tm); } } );
					 */
			...
			try {
				...
				/*
				 * ArrayList<String> listaDateMacellazione =
				 * Capo.loadDateMacellazioneByStabilimento( context.getParameter(
				 * "orgId" ), db ); context.getRequest().setAttribute(
				 * "listaDateMacellazione", listaDateMacellazione );
				 */
	...
	public String executeCommandNuovoCapo(ActionContext context) {
		...
		if(cdSedutaMacellazione==null)
			cdSedutaMacellazione = context.getParameter("cd_seduta_macellazione");
		context.getRequest().setAttribute("cdSedutaMacellazione", cdSedutaMacellazione);
		try {
			...
			if (context.getRequest().getParameter("clona") != null
					&& context.getRequest().getParameter("clona").equals("si")) {
				...
				if (idCapo != null) {
					...
					/*
					 * org.aspcfs.modules.speditori.base.Organization speditore
					 * = new org.aspcfs.modules.speditori.base.Organization( db,
					 * c.getCd_id_speditore() );
					 * context.getRequest().setAttribute( "Speditore", speditore
					 * ); if(speditore.getAddressList().size() > 0){
					 * OrganizationAddress speditoreAddress =
					 * (OrganizationAddress)(speditore.getAddressList().get(0));
					 * context.getRequest().setAttribute( "SpeditoreAddress",
					 * speditoreAddress ); }
					 */
					...
					capoClonato.setCd_codice_azienda(c.getCd_codice_azienda());

					capoClonato.setCd_maschio(null);
					...
					// new Clona per Flusso 173-rq1
					capoClonato.setCd_id_razza(c.getCd_id_razza());
					capoClonato.setCd_categoria_rischio(c.getCd_categoria_rischio());
					capoClonato.setCd_vincolo_sanitario(c.isCd_vincolo_sanitario());
					capoClonato.setCd_vincolo_sanitario_motivo(c.getCd_vincolo_sanitario_motivo());
					capoClonato.setCd_vincolo_sanitario_piano(c.isCd_vincolo_sanitario_piano());
					capoClonato.setCd_piano_risanamento(c.getCd_piano_risanamento());
					capoClonato.setCd_qualifica_brc(c.getCd_qualifica_brc());
					capoClonato.setCd_qualifica_tbc(c.getCd_qualifica_tbc());
					capoClonato.setCd_qualifica_sanitaria_brc(c.isCd_qualifica_sanitaria_brc());
					capoClonato.setCd_qualifica_sanitaria_tbc(c.isCd_qualifica_sanitaria_tbc());
					// new Flusso 339 rq 2 e 7
					capoClonato.setCd_qualifica_sanitaria_tbc(c.isCd_qualifica_sanitaria_leucosi());
					capoClonato.setCd_qualifica_leucosi(c.getCd_qualifica_leucosi());
					
					capoClonato.setCd_macellazione_differita(c.getCd_macellazione_differita());
					capoClonato.setCd_bse(c.getCd_bse());
					capoClonato.setBse_data_prelievo(c.getBse_data_prelievo());
					capoClonato.setBse_data_ricezione_esito(c.getBse_data_ricezione_esito());
					capoClonato.setBse_esito(c.getBse_esito());
					capoClonato.setBse_note(c.getBse_note());
					capoClonato.setCd_note(c.getCd_note());
					...
					//flusso 339 - Simone Liccardo
					
					// RQ 4
					capoClonato.setCd_id_veterinario_1(c.getCd_id_veterinario_1());
					capoClonato.setCd_id_veterinario_2(c.getCd_id_veterinario_2());
					capoClonato.setCd_id_veterinario_3(c.getCd_id_veterinario_3());
					capoClonato.setCd_veterinario_sostituto(c.getCd_veterinario_sostituto());
					
					capoClonato.setVpm_id_veterinario_1(c.getVpm_id_veterinario_1());
					capoClonato.setVpm_id_veterinario_2(c.getVpm_id_veterinario_2());
					capoClonato.setVpm_id_veterinario_3(c.getVpm_id_veterinario_3());
					capoClonato.setVpm_veterinario_sostituto(c.getVpm_veterinario_sostituto());
					
					//inizio REQ2 - 399
					
					//casl
					
					capoClonato.setCasl_to_asl_origine(c.isCasl_to_asl_origine());
					capoClonato.setCasl_to_proprietario_animale(c.isCasl_to_proprietario_animale());
					capoClonato.setCasl_to_azienda_origine(c.isCasl_to_azienda_origine());
					capoClonato.setCasl_to_proprietario_macello(c.isCasl_to_proprietario_macello());
					capoClonato.setCasl_to_pcf(c.isCasl_to_pcf());
					capoClonato.setCasl_to_regione(c.isCasl_to_regione());
					capoClonato.setCasl_to_altro(c.isCasl_to_altro());
					capoClonato.setCasl_to_altro_testo(c.getCasl_to_altro_testo());
					
					capoClonato.setCasl_data(c.getCasl_data());
					...
					capoClonato.setMavam_data(c.getMavam_data());
					capoClonato.setMavam_luogo(c.getMavam_luogo());
					capoClonato.setMavam_motivo(c.getMavam_motivo());
					capoClonato.setMvam_destinazione_carcassa(c.getMvam_destinazione_carcassa());
					
					
					capoClonato.setMavam_to_asl_origine(c.isMavam_to_asl_origine());
					capoClonato.setMavam_to_proprietario_animale(c.isMavam_to_proprietario_animale());
					capoClonato.setMavam_to_azienda_origine(c.isMavam_to_azienda_origine());
					capoClonato.setMavam_to_proprietario_macello(c.isMavam_to_proprietario_macello());
					capoClonato.setMavam_to_pcf(c.isMavam_to_pcf());
					capoClonato.setMavam_to_regione(c.isMavam_to_regione());
					capoClonato.setMavam_to_altro(c.isMavam_to_altro());
					capoClonato.setMavam_to_altro_testo(c.getMavam_to_altro_testo());
					
					capoClonato.setMavam_note(c.getMavam_note());
					
					
					//vam
					
					capoClonato.setVam_data(c.getVam_data());
					capoClonato.setVam_esito(c.getVam_esito());
					
					capoClonato.setVam_provvedimenti(c.getVam_provvedimenti());
					capoClonato.setVam_provvedimenti_note(c.getVam_provvedimenti_note());
					
					capoClonato.setVam_to_asl_origine(c.isVam_to_asl_origine());
					capoClonato.setVam_to_proprietario_animale(c.isVam_to_proprietario_animale());
					capoClonato.setVam_to_azienda_origine(c.isVam_to_azienda_origine());
					capoClonato.setVam_to_proprietario_macello(c.isVam_to_proprietario_macello());
					capoClonato.setVam_to_pcf(c.isVam_to_pcf());
					capoClonato.setVam_to_regione(c.isVam_to_regione());
					capoClonato.setVam_to_altro(c.isVam_to_altro());
					capoClonato.setVam_to_altro_testo(c.getVam_to_altro_testo());
					
					
					//vpm
					
					capoClonato.setMac_tipo(c.getMac_tipo());
					capoClonato.setVpm_esito(c.getVpm_esito());
					
					if(c.getDestinatario_1_id()!= -1){
						capoClonato.setDestinatario_1_id(c.getDestinatario_1_id());
						capoClonato.setDestinatario_1_nome(c.getDestinatario_1_nome());
					}
					
					if(c.getDestinatario_2_id() != -1){
						capoClonato.setDestinatario_2_id(c.getDestinatario_2_id());
						capoClonato.setDestinatario_2_nome(c.getDestinatario_2_nome());
					}
					
					if(c.getDestinatario_3_id() != -1){
						capoClonato.setDestinatario_3_id(c.getDestinatario_3_id());
						capoClonato.setDestinatario_3_nome(c.getDestinatario_3_nome());
					}
					
					if(c.getDestinatario_4_id() != -1){
						capoClonato.setDestinatario_4_id(c.getDestinatario_4_id());
						capoClonato.setDestinatario_4_nome(c.getDestinatario_4_nome());
					}
			...
			;
			try {
				numeroSeduta = Integer.parseInt(cdSedutaMacellazione);
			} catch (Exception e) {
			}
			;
			boolean sessionePregressa = false;
	...
	public String executeCommandModificaCapo(ActionContext context) {
		...
		try {
			...
			SintesisStabilimento stab = new SintesisStabilimento(db, c.getId_macello(), true);
			/*
			 * org.aspcfs.modules.speditori.base.Organization speditore = new
			 * org.aspcfs.modules.speditori.base.Organization( db,
			 * c.getCd_id_speditore() ); if(speditore.getAddressList().size() >
			 * 0){ OrganizationAddress speditoreAddress =
			 * (OrganizationAddress)(speditore.getAddressList().get(0));
			 * context.getRequest().setAttribute( "SpeditoreAddress",
			 * speditoreAddress ); }
			 */
			...
			if ("true".equalsIgnoreCase(org.aspcfs.modules.util.imports.ApplicationProperties.getProperty("NUOVA_GESTIONE_CAMPIONI_MACELLI"))){
				boolean sedutaChiusa = false;
					sedutaChiusa = MacelliUtil.isSedutaChiusa(c.getId_macello(), c.getVpm_data(), c.getCd_seduta_macellazione(), db);
					context.getRequest().setAttribute("sedutaChiusa", String.valueOf(sedutaChiusa));
			}
	...
	public String executeCommandSave(ActionContext context) {
		...
		boolean clona = (context.getRequest().getParameter("clona") != null && context.getRequest()
				.getParameter("clona").equals("si"));
		...
		try {
			...
			if (context.getParameter("vpmData") != null && !context.getParameter("vpmData").equals("")
					&& !context.getParameter("vpmData").equals("null"))
				vpmData = stringa("vpmData", context);
			context.getRequest().setAttribute("vpmData", vpmData);
			String mavamData = stringa("mavam_data", context);
			if (context.getParameter("mavamData") != null && !context.getParameter("mavamData").equals("")
					&& !context.getParameter("mavamData").equals("null"))
				mavamData = stringa("mavamData", context);
			context.getRequest().setAttribute("mavamData", mavamData);
			String cdSedutaMacellazione = stringa("cd_seduta_macellazione", context);
			if (context.getParameter("cdSedutaMacellazione") != null
					&& !context.getParameter("cdSedutaMacellazione").equals("")
					&& !context.getParameter("cdSedutaMacellazione").equals("null"))
			...
			if (c.getCd_matricola() != null && !c.getCd_matricola().equals("")) {
				...
				CapoAjax ca = ac.isCapoEsistenteSintesis(c.getCd_matricola());
				if (ca.isEsistente()) {
					this.freeConnection(context, db);
			...
			if (PopolaCombo.verificaStampaMod10(c.getId_macello(), context.getParameter("vpm_data"), 0) == true) {
				context.getRequest().setAttribute("idCapo", c.getId());
				context.getRequest().setAttribute("messaggio",
						"Capo NON inserito. Per questa seduta di macellazione e' stato stampato il Mod 10!");
				return executeCommandNuovoCapo(context);
			}

			// Controllo Progressivo di Macellazione
			if (c.getVpm_data() != null
					&& c.getStato_macellazione() != null
					&& (c.getStato_macellazione().equals("Incompleto: Inseriti solo i dati sul controllo documentale") || c
							.getStato_macellazione().equals("Incompleto: Veterinario mancante"))) {
				AjaxCalls ac = new AjaxCalls();
				if (!ac.controlloProgressivoMacellazioneBovini(c.getCd_seduta_macellazione(), c.getId_macello(),
						new SimpleDateFormat("dd/MM/yyyy").format(new Date(c.getVpm_data().getTime())),
						c.getProgressivo_macellazione(), c.getCd_matricola())) {
					this.freeConnection(context, db);
					context.getRequest().setAttribute("messaggio",
							"Capo NON inserito. Progressivo Macellazione gia' esistente!");
					return executeCommandNuovoCapo(context);
				}
			}
			...
			if (context.getRequest().getParameterValues("idCampioneCapo")!=null)
				CampioneNew.aggiornaCampioniCapo(db, c.getId(), c.getCd_matricola(), context.getRequest().getParameterValues("idCampioneCapo"), getUserId(context) );

			// Inizio Log del capo in m_capi_log

			// org.aspcfs.modules.speditori.base.Organization speditore = new
			// org.aspcfs.modules.speditori.base.Organization(db,c.getCd_id_speditore());

			/*
			 * org.aspcfs.modules.speditori.base.Organization speditore = new
			 * org
			 * .aspcfs.modules.speditori.base.Organization(db,c.getCd_id_speditore
			 * ());
			 * 
			 * 
			 * 
			 * org.aspcfs.modules.speditori.base.OrganizationAddress
			 * speditoreAddress = null;
			 * 
			 * 
			 * /*if(speditore.getAddressList().size() > 0){ speditoreAddress =
			 * (OrganizationAddress)(speditore.getAddressList().get(0)); }
			 */
			...
			// Inserito da Alberto
			String[] casl_provvedimenti_selezionati = context.getRequest().getParameterValues(
					"casl_provvedimenti_selezionati");
			casl_provvedimenti_selezionati = (casl_provvedimenti_selezionati == null) ? (new String[0])
					: (casl_provvedimenti_selezionati);
			...
			// c.storico_vpm_non_conformita_rilevate = PatologiaRilevata.load(
			// c.getId(), db );
	...
	public String executeCommandUpdate(ActionContext context) {
		...
		try {
			...
			ArrayList<Casl_Non_Conformita_Rilevata> vecchieCaslNC = Casl_Non_Conformita_Rilevata.load(
					vecchioCapo.getId(), db);
			...
			if (c.getVpm_data() != null
				...
				if (!ac.controlloProgressivoMacellazioneBovini(c.getCd_seduta_macellazione(), c.getId_macello(),
						new SimpleDateFormat("dd/MM/yyyy").format(new Date(c.getVpm_data().getTime())),
						c.getProgressivo_macellazione(), vecchioCapo.getCd_matricola())) {
					this.freeConnection(context, db);
					context.getRequest().setAttribute("messaggio",
							"Capo NON modificato. Progressivo Macellazione gia' esistente!");
			...
			// capoLog.setAslSpeditore(speditore != null ? speditore.getSiteId()
			// : 0);
			capoLog.setCodiceAziendaNascita(c.getCd_codice_azienda());
			// capoLog.setComuneSpeditore(speditoreAddress != null ?
			// speditoreAddress.getCity() : "");
			capoLog.setDataNascita(c.getCd_data_nascita());
			capoLog.setEnteredBy(c.getEntered_by());
			capoLog.setIdMacello(c.getId_macello());
			capoLog.setInBdn(context.getRequest().getParameter("capo_in_bdn") != null
					&& context.getRequest().getParameter("capo_in_bdn").equals("si"));
			...
			capoLog.setSesso(Boolean.TRUE.equals(c.isCd_maschio()) ? "M" : Boolean.FALSE.equals(c.isCd_maschio()) ? "F"
					: null == c.isCd_maschio() ? "" : "");
			...
			if (capoLog.isInBdn()) {
				...
				try {
					if (context.getRequest().getParameter("asl_speditore_from_bdn") != null) {
						capoLog.setAslSpeditoreFromBdn(Integer.parseInt(context.getRequest().getParameter(
								"asl_speditore_from_bdn")));
					}
				} catch (Exception e) {
					logger.severe("Il parametro asl_speditore_from_bdn non e' un intero: "
							+ context.getRequest().getParameter("asl_speditore_from_bdn"));
				}

				capoLog.setCodiceAziendaNascitaFromBdn(context.getRequest().getParameter(
						"codice_azienda_nascita_from_bdn") != null ? context.getRequest().getParameter(
						"codice_azienda_nascita_from_bdn") : "");
				capoLog.setComuneSpeditoreFromBdn(context.getRequest().getParameter("comune_speditore_from_bdn") != null ? context
						.getRequest().getParameter("comune_speditore_from_bdn") : "");

				try {
					if (context.getRequest().getParameter("data_nascita_from_bdn") != null) {
						Timestamp t = new Timestamp(sdf.parse(
								context.getRequest().getParameter("data_nascita_from_bdn")).getTime());
						capoLog.setDataNascitaFromBdn(t);
					}
				} catch (Exception e) {
					logger.severe("Il parametro data_nascita_from_bdn non e' una data corretta: "
							+ context.getRequest().getParameter("data_nascita_from_bdn"));
				}
				...
				try {
					...
					logger.severe("Il parametro razza_from_bdn non e' un intero: "
							+ context.getRequest().getParameter("razza_from_bdn"));
				}

				capoLog.setSessoFromBdn(context.getRequest().getParameter("sesso_from_bdn") != null ? context
						.getRequest().getParameter("sesso_from_bdn") : "");
				...
				try {
					...
					logger.severe("Il parametro specie_from_bdn non e' un intero: "
							+ context.getRequest().getParameter("specie_from_bdn"));
				}
			...
			// eliminare le casl_non_conformita non piu selezionate ed aggiorno
			// le altre
			for (Casl_Non_Conformita_Rilevata vp : vecchieCaslNC) {
				...
				for (String pat : casl_non_conformita) {
					...
					if (cancellare) // cancellazione delle casl_non_conformita
									// rimosse
					{
			...
			// eliminare le provvedimenti non piu selezionate ed aggiorno le
			// altre
			...
			ArrayList<NonConformita> ncs = loadNC(context, c);
			// eliminare le non conformita' non piu selezionate ed aggiornare le
			// altre
			...
			// nuovoCapo.storico_vpm_non_conformita_rilevate =
			// PatologiaRilevata.load( nuovoCapo.getId(), db );
	...
	public String executeCommandPrintBRCRilevazioneMacelli(ActionContext context) {
		...
		try {
			...
			// Replace the font based on the system language to support i18n
			// chars
	...
	public String executeCommandToModelloIdatidosi(ActionContext context) {
		...
		try {
			db = this.getConnection(context);
			SintesisStabilimento org = new SintesisStabilimento(db, Integer.parseInt(context.getParameter("altId")),
					true);
			context.getRequest().setAttribute("OrgDetails", org);
		} catch (Exception e1) {
			context.getRequest().setAttribute("Error", e1);
	...
	public String executeCommandSaveToLiberoConsumo(ActionContext context) {
		...
		try {
			...
			if (c.getVpm_data() != null
				...
				if (!ac.controlloProgressivoMacellazioneBovini(c.getCd_seduta_macellazione(), c.getId_macello(),
						new SimpleDateFormat("dd/MM/yyyy").format(new Date(c.getVpm_data().getTime())),
						c.getProgressivo_macellazione(), c_old.getCd_matricola())) {
					this.freeConnection(context, db);
					context.getRequest().setAttribute("messaggio",
							"Capo NON inserito. Progressivo Macellazione gia' esistente!");
					return executeCommandNuovoCapo(context);
...
//			c_old.setVpm_veterinario_3(c.getVpm_veterinario_3());
			...
			String[] componentiVpm = context.getRequest().getParameterValues("qualificaStrutturaComponenteIdVpm");
			
			if(componentiVpm != null){
				if(componentiVpm.length >= 1){
					String vetName = stringa("componenteNominativoVpm"+componentiVpm[0], context);
					String[] part = componentiVpm[0].split("_");
					int vetId = Integer.parseInt(part[2]);
					
					c_old.setVpm_veterinario(vetName);
					c_old.setVpm_id_veterinario_1(vetId);
				}
				
				if(componentiVpm.length >= 2){
					String vetName = stringa("componenteNominativoVpm"+componentiVpm[1], context);
					String[] part = componentiVpm[1].split("_");
					int vetId = Integer.parseInt(part[2]);
					
					c_old.setVpm_veterinario_2(vetName);
					c_old.setVpm_id_veterinario_2(vetId);
				}
				
				if(componentiVpm.length >= 3){
					String vetName = stringa("componenteNominativoVpm"+componentiVpm[2], context);
					String[] part = componentiVpm[2].split("_");
					int vetId = Integer.parseInt(part[2]);
					
					c_old.setVpm_veterinario_3(vetName);
					c_old.setVpm_id_veterinario_3(vetId);
				}
			}else{
				c_old.setVpm_veterinario("");
				c_old.setVpm_id_veterinario_1(0);
				c_old.setVpm_veterinario_2("");
				c_old.setVpm_id_veterinario_2(0);
				c_old.setVpm_veterinario_3("");
				c_old.setVpm_id_veterinario_3(0);
			}
			
			if(context.getRequest().getParameter("componenteSosVpm") != null && context.getRequest().getParameter("componenteSosVpm").equals("on"))
				c_old.setVpm_veterinario_sostituto(stringa("nominativoSosVpm", context));
			...
			// Inizio Log del capo in m_capi_log
			// org.aspcfs.modules.speditori.base.Organization speditore = new
			// org.aspcfs.modules.speditori.base.Organization(db,c_old.getCd_id_speditore());
			// org.aspcfs.modules.speditori.base.OrganizationAddress
			// speditoreAddress = null;
			//
			// if(speditore.getAddressList().size() > 0){
			...
			capoLog.setSesso(Boolean.TRUE.equals(c_old.isCd_maschio()) ? "M" : Boolean.FALSE.equals(c_old
					.isCd_maschio()) ? "F" : null == c_old.isCd_maschio() ? "" : "");
			...
			if (capoLog.isInBdn()) {
				...
				// try{
				// if(context.getRequest().getParameter("asl_speditore_from_bdn")
				// != null){
				// capoLog.setAslSpeditoreFromBdn(
				// Integer.parseInt(context.getRequest().getParameter("asl_speditore_from_bdn"))
				// );
				// }
				// }
				// catch(Exception e){
				...
				capoLog.setCodiceAziendaNascitaFromBdn(context.getRequest().getParameter(
						"codice_azienda_nascita_from_bdn") != null ? context.getRequest().getParameter(
						"codice_azienda_nascita_from_bdn") : "");
				// capoLog.setComuneSpeditoreFromBdn(
				// context.getRequest().getParameter("comune_speditore_from_bdn")
				// != null ?
				// context.getRequest().getParameter("comune_speditore_from_bdn")
				// : "" );
			...
			if (context.getRequest().getParameterValues("idCampioneCapo")!=null) 
				CampioneNew.aggiornaCampioniCapo(db, c_old.getId(), c.getCd_matricola(), context.getRequest().getParameterValues("idCampioneCapo"), getUserId(context) );
			...
			// c_old.storico_vpm_non_conformita_rilevate =
			// PatologiaRilevata.load( c_old.getId(), db );
...
//			c_old.setVpm_veterinario_3(c.getVpm_veterinario_3());
	...
	private ArrayList<Organi> loadOrgani(ActionContext context, Capo c) {
		...
		// ArrayList<Parameter> stadio = ParameterUtils.list(
		// context.getRequest(), "lcso_stadio_" );
		...
		ArrayList<Parameter> lesione_apparato_genitale = ParameterUtils.list(context.getRequest(),
				"lesione_apparato_genitale_");
		...
		ArrayList<Parameter> lesione_osteomuscolari = ParameterUtils.list(context.getRequest(),
				"lesione_osteomuscolari_");
		...
		for (int i = 0; i < organo.size(); i++) {
			...
			if (!"-1".equalsIgnoreCase(tipo.getValore())) {
				...
				for (int idPatologia : idPatologie) {
					...
					temp.setId_capo(c.getId());
					// temp.setLcso_patologia ( Integer.parseInt( patologia.get(
					// i ).getValore() ) );
					...
					// temp.setLcso_stadio ( Integer.parseInt( stadio.get( i
					// ).getValore() ) );
	...
	public ArrayList<Organi> loadOrganiRichiestaIstopatologico(ActionContext context, Connection db) {
		...
		// ArrayList<Parameter> stadio = ParameterUtils.list(
		// context.getRequest(), "lcso_stadio_" );
		ArrayList<Parameter> interessamento_altri_organi = ParameterUtils.list(context.getRequest(),
				"istopatologico_interessamento_altri_organi_");
		...
		for (int i = 0; i < id_organi.size(); i++) {
			...
			if (idOrganoInt > 0) {
				...
				thisOrgani.setIstopatologico_interessamento_altri_organi(context.getRequest().getParameter(
						"istopatologico_interessamento_altri_organi_" + idOrganoInt));
				thisOrgani.setIstopatologico_topografia(context.getRequest().getParameter(
						"istopatologico_topografia_" + idOrganoInt));
				ret.add(thisOrgani);
	...
	private Capo loadCapo(ActionContext context) {
		...
		c.setCasl_to_pcf(booleano("casl_to_pcf", context));
		...
		c.setCd_maschio(BooleanoPrimitive("cd_maschio", context));
		...
		// new Flusso 173 rq.5
		c.setCd_vincolo_sanitario_piano(booleano("cd_vincolo_sanitario_piano", context));
		c.setCd_piano_risanamento(intero("cd_piano_risanamento", context));

		// new Flusso 173 rq 6
		c.setCd_qualifica_sanitaria_brc(booleano("cd_qualifica_sanitaria_brc", context));
		c.setCd_qualifica_sanitaria_tbc(booleano("cd_qualifica_sanitaria_tbc", context));
		c.setCd_qualifica_brc(intero("cd_qualifica_brc", context));
		c.setCd_qualifica_tbc(intero("cd_qualifica_tbc", context));

		// new Flusso 339 rq 7
		c.setCd_qualifica_sanitaria_leucosi(booleano("cd_qualifica_sanitaria_leucosi", context));
		c.setCd_qualifica_leucosi(intero("cd_qualifica_leucosi", context));
		...
		if(componenti != null){
			if(componenti.length >= 1){
				String vetName = stringa("componenteNominativo"+componenti[0], context);
				String[] part = componenti[0].split("_");
				int vetId = Integer.parseInt(part[2]);
				
				c.setCd_veterinario_1(vetName);
				c.setCd_id_veterinario_1(vetId);
			}
			
			if(componenti.length >= 2){
				String vetName = stringa("componenteNominativo"+componenti[1], context);
				String[] part = componenti[1].split("_");
				int vetId = Integer.parseInt(part[2]);
				
				c.setCd_veterinario_2(vetName);
				c.setCd_id_veterinario_2(vetId);
			}
			
			if(componenti.length >= 3){
				String vetName = stringa("componenteNominativo"+componenti[2], context);
				String[] part = componenti[2].split("_");
				int vetId = Integer.parseInt(part[2]);
				
				c.setCd_veterinario_3(vetName);
				c.setCd_id_veterinario_3(vetId);
			}
		}
		
		if(context.getRequest().getParameter("componenteSos") != null && context.getRequest().getParameter("componenteSos").equals("on"))
			c.setCd_veterinario_sostituto(stringa("nominativoSos", context));
...
//		c.setCd_veterinario_3(stringa("cd_veterinario_3", context));
		...
		c.setMavam_to_pcf(booleano("mavam_to_pcf", context));
		...
		c.setVam_to_pcf(booleano("vam_to_pcf", context));
		...
		c.setVpm_data_esito(data("vpm_data_esito", context)); // Inserito da
																// Alberto
																// Campanile
...
//		c.setVpm_veterinario_3(stringa("vpm_veterinario_3", context));
		...
		if(componentiVpm != null){
			if(componentiVpm.length >= 1){
				...
				c.setVpm_veterinario(vetName);
				c.setVpm_id_veterinario_1(vetId);
			}
			...
			if(componentiVpm.length >= 2){
				...
				c.setVpm_veterinario_2(vetName);
				c.setVpm_id_veterinario_2(vetId);
			}
			...
			if(componentiVpm.length >= 3){
				...
				c.setVpm_veterinario_3(vetName);
				c.setVpm_id_veterinario_3(vetId);
			}
		}else{
			c.setVpm_veterinario("");
			c.setVpm_id_veterinario_1(0);
			c.setVpm_veterinario_2("");
			c.setVpm_id_veterinario_2(0);
			c.setVpm_veterinario_3("");
			c.setVpm_id_veterinario_3(0);
		}
		
		if(context.getRequest().getParameter("componenteSosVpm") != null && context.getRequest().getParameter("componenteSosVpm").equals("on"))
			c.setVpm_veterinario_sostituto(stringa("nominativoSosVpm", context));
	...
	private void setCategoria(Capo c) {
		...
		if (c.getCd_specie() == 1) {
			if (c.getMavam_data() != null && !"".equals(c.getMavam_data()) && c.getMavam_luogo() > 0
					&& !"".equals(c.getMavam_luogo())) {
				c.setCd_categoria_rischio(5);
			} else {
				if (c.getCd_macellazione_differita() > 0) {
					// se il capo ha oltre 48 mesi di eta' categoria 3
				} else {
					// se 2. [macellazione in emergenza] - sopra 48 mesi d eta'
					// 2
				}
			}
	...
	private boolean booleano(String param, ActionContext context) {
		...
		return "true".equalsIgnoreCase(temp) || "ok".equalsIgnoreCase(temp) || "si".equalsIgnoreCase(temp)
				|| "yes".equalsIgnoreCase(temp) || "on".equalsIgnoreCase(temp);
	}
	...
	private void caricaLookup(ActionContext context, boolean select) {
		...
		try {
			...
			org = new SintesisStabilimento(db, tempid, true);
			context.getRequest().setAttribute("OrgDetails", org);
	...
	private void getConfigTipo(ActionContext context) {
		if (context.getSession().getAttribute("configTipo") != null) {
			...
			try {
				...
				try {
					org.aspcfs.modules.macellazioninewsintesis.utils.ConfigTipo configTipo2 = (org.aspcfs.modules.macellazioninewsintesis.utils.ConfigTipo) context
							.getSession().getAttribute("configTipo");
					final int tipo = configTipo2.getIdTipo();
					configTipo = new ConfigTipo(tipo);
					context.getSession().setAttribute("configTipo", configTipo);
				} catch (Exception e2) {
					org.aspcfs.modules.macellazioninew.utils.ConfigTipo configTipo3 = (org.aspcfs.modules.macellazioninew.utils.ConfigTipo) context
							.getSession().getAttribute("configTipo");
					final int tipo = configTipo3.getIdTipo();
					configTipo = new ConfigTipo(tipo);
					context.getSession().setAttribute("configTipo", configTipo);
				}
	...
	public String executeCommandSaveIstopatologico(ActionContext context) {
		...
		try {
			...
			thisRichiesta = (RichiestaIstopatologico) context.getRequest().getAttribute("RichiestaIstopatologico");
		...
		// sdf.format(new
		// Date(richiestaIstopatologico.getCapo().getVpm_data()));%>
	...
	public String executeCommandRichiestaIstopatologicoStampaModelloInvioCampioni(ActionContext context) {
		...
		try {
			...
			if (istopatologico.getCapo().getId_macello() > 0) {
				org = new SintesisStabilimento(db, istopatologico.getCapo().getId_macello(), true);
			}
	...
	public String executeCommandToLiberoConsumoMassivo(ActionContext context) {
		...
		try {
			db = this.getConnection(context);
			org = new SintesisStabilimento(db, altId, true);
			capi = Capo.loadByStabilimentoCapiNonMacellati(altIdString, db);
			...
			if (cb != null && cb.equals("on")) {
				...
				try {
					...
					if (macellabile) {
						...
						c.setVpm_note("Macellazione avvenuta tramite libero consumo massivo in data "
								+ new SimpleDateFormat("dd/MM/yyyy").format(dataOperazione));
						c.setSolo_cd(false);
						c.aggiornaLiberoConsumo(db);
						messaggio += "<b><font color=\"green\">Capo " + c.getCd_matricola()
								+ " correttamente macellato in libero consumo alla data "
								+ new SimpleDateFormat("dd/MM/yyyy").format(c.getVpm_data()) + ".</font></b><br/>";
					}
	...
	public String executeCommandStoricoLiberoConsumoMassivo(ActionContext context) {
		...
		try {
			...
			PreparedStatement pst = db
					.prepareStatement("select log.*, c.cd_matricola, c.id_macello from m_capi_libero_consumo_massivo_log log left join m_capi c on log.id_capo = c.id order by log.data_operazione desc");
			...
			while (rs.next()) {
				...
				String riga = idCapo + ";;" + idUtente + ";;" + dataOperazione + ";;" + note + ";;" + matricola + ";;"
						+ idMacello;
	...
	private Boolean BooleanoPrimitive(String param, ActionContext context) {
		String temp = context.getParameter(param);
	...
	public String executeCommandToChiudiSeduta(ActionContext context) {

		Connection db = null;

		int idMacello = Integer.parseInt(context.getRequest().getParameter("idMacello"));
		int numero = Integer.parseInt(context.getRequest().getParameter("numero"));
		String dataSeduta = context.getRequest().getParameter("data");
		
		Timestamp data = null;
		try {
			Date initDate = new SimpleDateFormat("dd/MM/yyyy").parse(dataSeduta);
			SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd");
			String parsedDate = formatter.format(initDate);
			SimpleDateFormat sdf2 = new SimpleDateFormat("yyyy-MM-dd");
			java.util.Date date = sdf2.parse(parsedDate);
		...
		try {
			...
			stab = new SintesisStabilimento(db, idMacello, true);
			
			 ArrayList<Qualifica> listaQualifiche =	Qualifica.getQualificheMacelli(db);
		 	 context.getRequest().setAttribute("ListaQualificheMacelli", listaQualifiche);
		 	 
		      LookupList oggettoControllo = new LookupList(); 
		      //oggettoControllo.addItem(-1, "-- SELEZIONA VOCE --");
		      oggettoControllo.setTable("(SELECT code, description, default_item, ROW_NUMBER() OVER (ORDER BY level DESC, gruppo DESC) as level, enabled, gruppo from(select code, description, default_item, level, enabled, true as gruppo from lookup_ispezione_macrocategorie where enabled UNION select code, description, default_item, level, enabled, false as gruppo from lookup_ispezione where enabled order by level asc, gruppo desc, description asc) oggetti)");
		      oggettoControllo.buildList(db);
		      context.getRequest().setAttribute("OggettoControllo", oggettoControllo);
			
		      LookupList SiteIdList = new LookupList(db, "lookup_site_id");
		      context.getRequest().setAttribute("SiteIdList", SiteIdList);
		      
		      if (!MacelliUtil.hasSedutaCapi(idMacello, data, numero, db)){
		  			context.getRequest().setAttribute("Errore", "Impossibile chiudere la seduta. Non sono presenti capi.");
				}
		...
		context.getRequest().setAttribute("Macello", stab);
		context.getRequest().setAttribute("Data", dataSeduta);
		context.getRequest().setAttribute("Numero", String.valueOf(numero));

		return "ToChiudiSedutaOK";

	}
	
	public String executeCommandChiudiSeduta(ActionContext context) {

		Connection db = null;

		int idMacello = Integer.parseInt(context.getRequest().getParameter("idMacello"));
		int numero = Integer.parseInt(context.getRequest().getParameter("numero"));
		String data = context.getRequest().getParameter("dataInizioControllo");
		int idAsl = Integer.parseInt(context.getRequest().getParameter("idAsl"));
		String idMotivo = context.getRequest().getParameter("idMotivo");
		String idPiano = context.getRequest().getParameter("idPiano");
		String idPerContoDi = context.getRequest().getParameter("idPerContoDi");
		String oggetto[] = context.getRequest().getParameterValues("oggettoControllo");
		String componenti = context.getRequest().getParameter("listaComponenti");
		
		String oggetti_input = "";
		for (int i =0; i<oggetto.length; i++)
			oggetti_input =oggetti_input+oggetto[i]+"-";
		if (oggetti_input.endsWith("-"))
			oggetti_input = oggetti_input.substring(0, oggetti_input.length()-1);
		
		String nucleo_input = "";
		String componentiArr[] = componenti.split(";");
		for (int i =0; i<componentiArr.length; i++)
			nucleo_input =nucleo_input+componentiArr[i]+"-";
		if (nucleo_input.endsWith("-"))
			nucleo_input = nucleo_input.substring(0, nucleo_input.length()-1);
		
		String motivi[] = idMotivo.split(";");
		String piani[] = idPiano.split(";");
		String perContoDi[] = idPerContoDi.split(";");
		...
		for (int i = 0; i<motivi.length; i++){
			...
			int motivo = -1;
			try { motivo = Integer.parseInt(motivi[i]);} catch (Exception e) {}
			
			if (motivo == 89)
				listaPiani = listaPiani + piani[i]+"-"+perContoDi[i]+"/";
			else
				listaMotivi = listaMotivi + motivi[i]+"-"+perContoDi[i]+"/";
		}

		if (listaPiani.endsWith("/"))
			listaPiani = listaPiani.substring(0, listaPiani.length()-1);
		if (listaMotivi.endsWith("/"))
			listaMotivi = listaMotivi.substring(0, listaMotivi.length()-1);
		...
		try {
			...
			PreparedStatement pst = db.prepareStatement("select * from chiudi_seduta_new(?, ?, ?, ?, ?, ?, ?, ?, ?);");
			int i = 0;
			pst.setInt(++i, idMacello);
			pst.setString(++i, data);
			pst.setInt(++i, numero);
			pst.setInt(++i, idAsl);
			pst.setString(++i, listaMotivi);
			pst.setString(++i, listaPiani);
			pst.setString(++i, oggetti_input);
			pst.setString(++i, nucleo_input);
			pst.setInt(++i, getUserId(context));
			System.out.println("[Chiudi seduta] "+pst.toString());
			ResultSet rs = pst.executeQuery();
			if (rs.next())
