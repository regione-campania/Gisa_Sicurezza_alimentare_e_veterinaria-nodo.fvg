...
public final class AccountCampioni extends CFSModule {
	...
	public String executeCommandConfirmDelete(ActionContext context) {
		if (!hasPermission(context, "distributori-distributori-campioni-delete")) {
			return ("PermissionError");
		}
		Connection db = null;
		// Parameters
		...
		try {
			...
			htmlDialog.addButton(systemStatus.getLabel("global.button.delete"),
					"javascript:window.location.href='DistributoriCampioni.do?command=DeleteTicket&id="
							+ id
							+ "&orgId="
							+ orgId
							+ "&forceDelete=true"
		...
		try {
			...
			if (recordDeleted) {
				...
				context
						...
						.setAttribute(
								"refreshUrl",
								"DistributoriVigilanza.do?command=TicketDetails&id="+id_controllo+"&orgId="
										+ orgId
										+ (inline != null
	...
	public String executeCommandRestoreTicket(ActionContext context) {
		...
		if (!(hasPermission(context, "distributori-distributori-campioni-edit"))) {
			return ("PermissionError");
		}
		...
		{
			Campioni c = new Campioni();
			c.executeCommandRestoreTicket(context);
	...
	public String executeCommandAdd(ActionContext context) {
		if (!hasPermission(context, "distributori-distributori-campioni-add")) {
			return ("PermissionError");
		}
		Connection db = null;
		...
		try {
			...
			String ticketId = context.getRequest().getParameter("id");
			 ...
			 tamponiList.buildListControlli(db, pasId, id_controllo);
				      ...
				      Organization org = new org.aspcfs.modules.distributori.base.Organization(db , thisTicketV.getOrgId());
