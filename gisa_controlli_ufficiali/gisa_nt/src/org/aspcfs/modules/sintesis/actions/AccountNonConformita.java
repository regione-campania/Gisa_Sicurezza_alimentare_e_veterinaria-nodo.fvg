...
public final class AccountNonConformita extends CFSModule {
	...
	public String executeCommandReopenTicket(ActionContext context) {
		if (!hasPermission(context, "sintesis-sintesis-nonconformita-edit")) {
	...
	public String executeCommandTicketDetails(ActionContext context) {
		if (!hasPermission(context, "sintesis-sintesis-nonconformita-view")) {
			return ("PermissionError");
		}
		...
		try {
			...
			org.aspcfs.modules.nonconformita.actions.AccountNonConformitaAlt nonconformita = new org.aspcfs.modules.nonconformita.actions.AccountNonConformitaAlt();
			db = this.getConnection(context);
			...
			SintesisStabilimento thisOrganization = new SintesisStabilimento(db, newTic.getAltId(), true);
	...
	public String executeCommandConfirmDelete(ActionContext context) {
		...
		try {
			...
			htmlDialog.addButton(
					systemStatus.getLabel("global.button.delete"), "javascript:window.location.href='"+stab.getAction()+"NonConformita.do?command=DeleteTicket&altId="+altId+"&id=" + id + "&forceDelete=true" + RequestUtils.addLinkParams(
	...
	public String executeCommandDeleteTicket(ActionContext context) {
		if (!hasPermission(context, "sintesis-sintesis-nonconformita-delete")) {
			return ("PermissionError");
		}
		...
		try {
			...
			SintesisStabilimento thisOrganization = new SintesisStabilimento(db, altId, true);
			thisOrganization.getPrefissoAction(context.getAction().getActionName());
	...
	public String executeCommandUpdateTicket(ActionContext context) {
		...
		try {
			...
			SintesisStabilimento orgDetails = new SintesisStabilimento(db, Integer.parseInt(context.getParameter("altId")), true);
	...
	private static void aggiornaPunteggio(Connection db, int idNononformita) throws SQLException
	{
		...
		String update = "update ticket set punteggio = (select sum (punteggio) from ticket where id_controllo_ufficiale= ? and tipologia not in (3) and trashed_date is null) where ticketid = ?";
	...
	public String executeCommandAdd(ActionContext context) {
		if (!hasPermission(context, "sintesis-sintesis-nonconformita-add")) {
			return ("PermissionError");
		}
		Connection db = null;
		Ticket newTic = null;
		try {
			...
			SintesisStabilimento thisOrganization = new SintesisStabilimento(db, Integer.parseInt(context.getParameter("altId")), true);
			thisOrganization.getPrefissoAction(context.getAction().getActionName());
			context.getRequest().setAttribute("OrgDetails", thisOrganization);
	...
	public String executeCommandInsert(ActionContext context) throws SQLException {
		...
		try {
			db = this.getConnection(context);
			//db.commit();
			SintesisStabilimento thisOrg = new SintesisStabilimento(db, newTic.getAltId(), true);
			thisOrg.getPrefissoAction(context.getAction().getActionName());
			org.aspcfs.modules.nonconformita.actions.AccountNonConformitaAlt nc = new org.aspcfs.modules.nonconformita.actions.AccountNonConformitaAlt();
