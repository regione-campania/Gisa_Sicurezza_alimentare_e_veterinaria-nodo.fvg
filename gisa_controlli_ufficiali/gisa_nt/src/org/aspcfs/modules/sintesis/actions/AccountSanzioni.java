...
public final class AccountSanzioni extends CFSModule {
  ...
  public String executeCommandReopenTicket(ActionContext context) {
    if (!hasPermission(context, "sintesis-sintesis-sanzioni-edit")) {
  ...
  public String executeCommandTicketDetails(ActionContext context) {
    if (!hasPermission(context, "sintesis-sintesis-sanzioni-view")) {
      return ("PermissionError");
    }
    Connection db = null;
    //Parameters
  ...
  public String executeCommandConfirmDelete(ActionContext context) {
    if (!hasPermission(context, "sintesis-sintesis-sanzioni-delete")) {
      return ("PermissionError");
    }
    Connection db = null;
    //Parameters
    ...
    try {
      ...
      htmlDialog.addButton(
          systemStatus.getLabel("global.button.delete"), "javascript:window.location.href='"+stab.getAction()+"Sanzioni.do?command=DeleteTicket&id=" + id + "&altId=" + altId + "&forceDelete=true" + RequestUtils.addLinkParams(
  ...
  public String executeCommandDeleteTicket(ActionContext context) {
    ...
    try {
      db = this.getConnection(context);
      SintesisStabilimento newOrg = new SintesisStabilimento(db, altId, true);
	...
	if (recordDeleted) {
		...
		context
				...
				.setAttribute(
						"refreshUrl",
						newOrg.getAction()+"NonConformita.do?command=TicketDetails&id="+thisTic.getId_nonconformita()+"&altId="
  								+ altId
								+ (inline != null
...
//	  if (!hasPermission(context, "opu_sanzioni-edit")) {
//	      return ("PermissionError");
//	    }	
    Connection db = null;
