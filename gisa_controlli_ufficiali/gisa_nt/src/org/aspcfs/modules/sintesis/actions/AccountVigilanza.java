...
//			operatore.getListaStabilimenti().setIdAsl(((UserBean)context.getRequest().getSession().getAttribute("User")).getSiteId());
			...
			if ("true".equalsIgnoreCase(org.aspcfs.modules.util.imports.ApplicationProperties.getProperty("NUOVA_GESTIONE_CAMPIONI_MACELLI"))){
				...
				pst = db.prepareStatement("select * from m_capi_sedute where id_cu = ? and trashed_date is null");
				pst.setInt(1, Integer.parseInt(ticketId));
				rs = pst.executeQuery();;
				if (rs.next()){
					context.getRequest().setAttribute("CUSedutaData", rs.getString("data"));
					context.getRequest().setAttribute("CUSedutaNumero", rs.getString("numero"));
					context.getRequest().setAttribute("CUSedutaIdMacello", rs.getString("id_macello"));
				}
			}
