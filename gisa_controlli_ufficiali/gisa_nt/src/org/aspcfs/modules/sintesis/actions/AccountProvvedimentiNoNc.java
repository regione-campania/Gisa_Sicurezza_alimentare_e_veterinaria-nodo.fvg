...
public final class AccountProvvedimentiNoNc extends CFSModule {
	...
	public String executeCommandTicketDetails(ActionContext context) {
		...
		try {
			...
			if (context.getRequest().getParameter("idSintesisStabilimento")!=null)
				orgId = Integer.parseInt( context.getRequest().getParameter("idSintesisStabilimento"));	
			else
				if (context.getRequest().getAttribute("stabId")!=null)
					orgId = (Integer) context.getRequest().getAttribute("stabId");			
			SintesisStabilimento thisOrganization = new SintesisStabilimento(db,orgId);
	...
	public String executeCommandConfirmDelete(ActionContext context) {
		...
		try {
			...
			SintesisStabilimento stab = new SintesisStabilimento(db,ticket.getIdStabilimento());
	...
	public String executeCommandDeleteTicket(ActionContext context) {
		...
		try {
			db = this.getConnection(context);
			SintesisStabilimento newOrg = new SintesisStabilimento(db, orgId);
			...
			if (recordDeleted) {
				...
				context
						...
						.setAttribute(
								"refreshUrl",
								newOrg.getAction()+"Vigilanza.do?command=TicketDetails&id="+id_controllo+"&stabId="
										+ orgId
										+ (inline != null
	...
	public String executeCommandUpdateTicket(ActionContext context) {
		...
		try {
			...
			SintesisStabilimento orgDetails = new SintesisStabilimento(db, Integer.parseInt(context.getParameter("stabId")));
	...
	public String executeCommandAdd(ActionContext context) {
		...
		try {
			...
			SintesisStabilimento newOrg = new SintesisStabilimento(db, tempid);
			newOrg.getPrefissoAction(context.getAction().getActionName());
	...
	public String executeCommandInsert(ActionContext context)
		...
		try
		{
			...
			SintesisStabilimento thisOrg = new SintesisStabilimento(db, newTic.getIdStabilimento());
	...
	public String executeCommandModifyTicket(ActionContext context) {
		...
		try {
			...
			if (context.getRequest().getParameter("stabId")!=null)
				...
				if (context.getRequest().getAttribute("stabId")!=null)
					orgId = (Integer) context.getRequest().getAttribute("stabId");						
			SintesisStabilimento orgDetails = new SintesisStabilimento(db, orgId);
			orgDetails.getPrefissoAction(context.getAction().getActionName());
			context.getRequest().setAttribute("OrgDetails", orgDetails);
	...
	public String executeCommandModify(ActionContext context) {
		...
		try {
			...
			SintesisStabilimento orgDetails = new SintesisStabilimento(db, newTic.getIdStabilimento());
			orgDetails.getPrefissoAction(context.getAction().getActionName());
			context.getRequest().setAttribute("OrgDetails", orgDetails);
		...
		try {
			...
			String ticketId = context.getRequest().getParameter("id");
			 ...
			 tamponiList.buildListControlli(db, pasId, id_controllo);
				      ...
				      SintesisStabilimento org =new SintesisStabilimento(db , thisTicketV.getIdStabilimento());
			      ...
			      while(itAudit.hasNext()){
					    	  ...
					    	  SintesisStabilimento orgDetails = new SintesisStabilimento(db,thisTicketV.getIdStabilimento());
				      ...
				      if(flag==1 || flag==2){
				    	  ...
				    	  context.getRequest().setAttribute("stabId", org.getIdStabilimento());
				    	  return (executeCommandTicketDetails(context));
