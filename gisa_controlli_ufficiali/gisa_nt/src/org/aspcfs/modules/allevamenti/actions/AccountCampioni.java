...
public final class AccountCampioni extends CFSModule {
	...
	public String executeCommandReopenTicket(ActionContext context) {
		if (!hasPermission(context, "allevamenti-allevamenti-campioni-edit")) {
	...
	public String executeCommandTicketDetails(ActionContext context) {
		...
		try {
			...
			String codice_azienda = thisOrganization.getAccountNumber();
			int check = thisOrganization.checkCodiceSIN(db,codice_azienda);
			if(check > 0){
				context.getRequest().setAttribute("link_visibile", "si");
			}
			else {
	...
	public String executeCommandConfirmDelete(ActionContext context) {
		if (!hasPermission(context, "allevamenti-allevamenti-campioni-delete")) {
			return ("PermissionError");
		}
		Connection db = null;
		// Parameters
		...
		try {
			...
			htmlDialog.addButton(systemStatus.getLabel("global.button.delete"),
					"javascript:window.location.href='"+ticket.getURlDettaglio()+"Campioni.do?command=DeleteTicket&id="
							+ id
							+ "&orgId="
							+ orgId
							+ "&forceDelete=true"
	...
	public String executeCommandViewHistory(ActionContext context) {
		if (!hasPermission(context, "allevamenti-allevamenti-campioni-history-view")) {
			return ("PermissionError");
		}
		Connection db = null;
	...
	public String executeCommandAdd(ActionContext context) {
		if (!hasPermission(context, "allevamenti-allevamenti-campioni-add")) {
			return ("PermissionError");
		}
		Connection db = null;
		...
		try {
			...
			String ticketId = context.getRequest().getParameter("id");
			 ...
			 tamponiList.buildListControlli(db, pasId, id_controllo);
				      ...
				      Organization org = new org.aspcfs.modules.allevamenti.base.Organization(db , thisTicketV.getOrgId());
			      ...
			      while(itAudit.hasNext()){
					  ...
					  if(flag==0 ){
						  if(chiudiCu !=null)
						  {
								...
								if(!thisTicketV.isControlloChiudibileBaSa(db, false) || !thisTicketV.isControlloChiudibileBaSa(db, true)){
									context.getRequest().setAttribute("Messaggio3", "Il controllo non puo essere chiuso per checklist del benessere ancora aperte.");
									context.getRequest().setAttribute("Chiudi", ""+1);
							    	  return (executeCommandTicketDetails(context));
								}
								if(!thisTicketV.isControlloChiudibileFarmacosorveglianza(db)){
									context.getRequest().setAttribute("Messaggio3", "Il controllo non puo essere chiuso per checklist farmacosorveglianza ancora aperta.");
									context.getRequest().setAttribute("Chiudi", ""+1);
							    	  return (executeCommandTicketDetails(context));
								}	
