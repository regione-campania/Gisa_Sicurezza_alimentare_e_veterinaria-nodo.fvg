...
public class OrganizationList extends Vector implements SyncableList {
  ...
  private String specie_allevamento="-1";
  private String gruppo_specie="-1";
  public int getSpecie_allevamento() {
	return Integer.parseInt(specie_allevamento);
}


public void setSpecie_allevamento(String specie_allevata) {
	this.specie_allevamento = specie_allevata;
}



public int getGruppo_specie() {
	return Integer.parseInt(gruppo_specie);
}


public void setGruppo_specie(String gruppo_specie) {
	this.gruppo_specie = gruppo_specie;
}
...
private static final long serialVersionUID = 2268314721560915731L;
  ...
  private String cityOperativa = null;
  ...
  private String cessato = "-1";
  private String tipologiaStruttura = "-1";
  private String orientamentoProduttivo = "-1";
  ...
  private boolean flagRicercaAllevamentiAssociabiliAdOsm;
  
	public boolean isFlagRicercaAllevamentiAssociabiliAdOsm() {
		return flagRicercaAllevamentiAssociabiliAdOsm;
	}

	public void setFlagRicercaAllevamentiAssociabiliAdOsm(boolean flagRicercaAllevamentiAssociabiliAdOsm) {
		this.flagRicercaAllevamentiAssociabiliAdOsm = flagRicercaAllevamentiAssociabiliAdOsm;
	}
...
/**
  ...
  public void setBanca(String tmp) {
	    this.banca = tmp;
	  }
  /**
  ...
  public void buildList(Connection db, PreparedStatement pst) throws SQLException {
	  ...
	  if(cityOperativa!=null)
	        this.cityOperativa=this.cityOperativa.replaceAll("%", " ").trim();
	  
	    ResultSet rs = queryList2(db, pst);
	    while (rs.next()) {
	      Organization thisOrganization = this.getObject2(rs);
...
public ResultSet queryList2(Connection db, PreparedStatement pst) throws SQLException {
	  ...
	  PreparedStatement st = null;
	    ...
	    sql.append("SELECT * FROM organization WHERE tipologia = 2 ORDER BY name");
  ...
  public Organization getObject2(ResultSet rs) throws SQLException {
	  int i = 0;
	    Organization thisOrganization = new Organization(rs, i);
	    return thisOrganization;
	  }
  ...
  public ResultSet queryList(Connection db, PreparedStatement pst) throws SQLException {
    ...
    StringBuffer sqlSelect = new StringBuffer();
    StringBuffer sqlSelect1 = new StringBuffer();
    ...
    sqlCount.append(
        ...
        " left JOIN lookup_tipologia_struttura tipologia_all ON (o.tipologia_strutt = tipologia_all.description) "+
        " left JOIN lookup_orientamento_produttivo orientamento ON (o.orientamento_prod = orientamento.description) "+
        " where o.org_id >= 0 and tipologia = 2 and o.trashed_date is null ");
    ...
    sqlSelect.append(
        "distinct o.*, azosm.id AS id_associazione_osm, operatori.nominativo,operatori.comune,operatori.indirizzo,operatori.prov,operatori.cap," +
        ...
        "specie.description as specie_allev, " +
  ...
  protected void createFilter(Connection db, StringBuffer sqlFilter) {
    ...
    if(confronto != null && !confronto.replaceAll("%", " ").trim().equals("-1"))
    {
  	 
    if (confronto != null && confronto.replaceAll("%", " ").trim().equals("0"))
    {
  	  	sqlFilter.append(" AND num_capi > "+parametro.replaceAll("%", " ").trim());
    }
    else
    {
  	  if(confronto != null && confronto.replaceAll("%", " ").trim().equals("1"))
    ...
    if((city==null || city.equals("-1")) && accountState!=null && !accountState.replaceAll("%", " ").trim().equals("") &&  !accountState.replaceAll("%", " ").trim().equals("-1"))
    {
    	sqlFilter.append(" AND o.account_number ilike '%"+accountState+"%'");
    ...
    if(!cessato.equals("-1"))
    {
    	if(cessato.equals("0"))
    	{
    		 sqlFilter.append(" AND  o.date2 is null ");
    	}
    	else
    	{
    		 sqlFilter.append(" AND  o.date2 is not null ");
    	}
    	
    }
    if(! tipologiaStruttura.equals("-1"))
    {
    	 sqlFilter.append(" AND  tipologia_all.code = ? ");
    	
    }
    
    if(! orientamentoProduttivo.equals("-1"))
    {
    	 sqlFilter.append(" AND  orientamento.code = ? ");
    ...
    if (codiceFiscaleRappresentante != null) {
        sqlFilter.append(" AND  cf_correntista ilike ? ");
      }
    
    if(!gruppo_specie.equals("-1"))
    {
    	String specie_allevata 	= "";
    	String specieAllev 		= "";
    	
    		sqlFilter.append(" and specie_allevamento in (select code from lookup_specie_allevata where codice_categoria = "+gruppo_specie+")");
    }

    
    if(!specie_allevamento.equals("-1"))
    {
    	String specie_allevata = "";
    	String specieAllev = "";
    	if (specie_allevamento.equals("111"))
    	{
    		sqlFilter.append(" and specie_allevamento in (select code from lookup_specie_allevata where level = 6)");
    	}
    	else
    	{
    		sqlFilter.append(" and specie_allevamento ="+specie_allevamento);
    ...
    if (name != null) {
      if (name.indexOf("%") >= 0) {
        sqlFilter.append(
            " AND  " + DatabaseUtils.toUpperCase(db) + "(o.name) ILIKE ? ");
      } else {
        sqlFilter.append(
            " AND  " + DatabaseUtils.toUpperCase(db) + "(o.name) Ilike ? ");
      }
    ...
    // associazione OSM - azienda zootecnica
    if (flagRicercaAllevamentiAssociabiliAdOsm == true) {
        sqlFilter.append(" AND  o.account_number NOT IN (SELECT cod_azienda FROM aziendezootecniche_osm)");
    }
    ...
    if ((banca != null)) {
    	...
    	if (banca.indexOf("%") >= 0) {
            sqlFilter.append(
                " AND  " + DatabaseUtils.toLowerCase(db) + "(o.banca) LIKE ? ");
          } else {
            sqlFilter.append(
                " AND  " + DatabaseUtils.toLowerCase(db) + "(o.banca) = ? ");
          }
    ...
    if (types != null) {
      sqlFilter.append(
          " AND  o.org_id IN (select atl.org_id from allevamenti_type_levels atl where atl.type_id IN (" + types + ")) ");
    }
    ...
    if (accountNumber != null) {
      if (accountNumber.indexOf("%") >= 0) {
        ...
        sqlFilter.append(
            " AND  " + DatabaseUtils.toLowerCase(db) + "(o.account_number) Ilike ? ");
      }
    }
  ...
  if (tipologia > -1) {
    ...
    if (specie_allev != null) {
        if (specie_allev.indexOf("%") >= 0) {
          sqlFilter.append(
              " AND  " + DatabaseUtils.toLowerCase(db) + "(o.specie_allev) LIKE ? ");
        } else {
          sqlFilter.append(
              " AND  " + DatabaseUtils.toLowerCase(db) + "(o.specie_allev) = ? ");
        }
      }
    if (projectId > 0) {
      sqlFilter.append(
    ...
    /*if (city != null && !"-1".equals(city)) {
    	...
    	sqlFilter.append(" AND o.account_number ilike '"+city+"%'");
      }*/
    if (city != null && !"-1".equals(city)) {
      if (city.indexOf("%") >= 0) {
        sqlFilter.append(
            " AND  o.org_id IN (SELECT org_id FROM organization_address " +
            " where " + DatabaseUtils.toLowerCase(db, "city") + "ilike (select comune from comuni where substr(codiceistatcomune, 3) = '" + city + "' )  " +
            ") ");
      } else {
        sqlFilter.append(
        		  " AND  o.org_id IN (SELECT org_id FROM organization_address " +
    ...
    if (state != null && !"-1".equals(state)) {
      ...
      sqlFilter.append(
          " AND  o.org_id IN (SELECT org_id FROM organization_address " +
          " where " + DatabaseUtils.toLowerCase(db, "state") + " ILIKE ? " +
    ...
    if (cityOperativa != null && !"-1".equals(cityOperativa)) {
          sqlFilter.append(
          		  " AND o.org_id in (select org_id from organization where tipologia = 2 and UPPER(account_number) in (select UPPER(cod_azienda) from aziende where cod_comune_azienda::integer in (select codiceistatcomune::integer from comuni where substr(codiceistatcomune, 3) = " + cityOperativa +"::text) and cod_comune_azienda ~ '^[0-9\\.]+$'))");
       }
    ...
    if(malattia != null && ! malattia.replaceAll("%", "").equals("Tutti") && ! malattia.equals("") )
    {
    	 sqlFilter.append(" and o.account_number in (select  inf.codice_azienda from informazioni_sanitarie_allevamenti inf where inf.codice_malattia ilike '"+malattia+"')");
    }
    
    if(qualifica != null && ! qualifica.equals("") && ! qualifica.replaceAll("%", "").equals("Tutti") )
    {
   	 sqlFilter.append(" and o.account_number in (select  inf.codice_azienda from informazioni_sanitarie_allevamenti inf where inf.qualifica ilike '"+qualifica+"')");
  ...
  protected int prepareFilter(PreparedStatement pst) throws SQLException {
    ...
    if(! tipologiaStruttura.equals("-1"))
    {
    	 pst.setInt(++i, Integer.parseInt(tipologiaStruttura));
    	
    }
    
    if(! orientamentoProduttivo.equals("-1"))
    {
    	 pst.setInt(++i, Integer.parseInt(orientamentoProduttivo));
    ...
    if (banca != null) {
        pst.setString(++i, banca.toLowerCase());
      }
    ...
    if (specie_allev != null) {
        pst.setString(++i, specie_allev.toLowerCase());
      }
    if (projectId > 0) {
    ...
    /*if (city != null && !"-1".equals(city)) {
      pst.setInt(++i, Integer.parseInt(city));
    }*/
    if (state != null && !"-1".equals(state)) {
...
public String getCityOperativa() {
	return cityOperativa;
}


public void setCityOperativa(String cityOperativa) {
	this.cityOperativa = cityOperativa;
}
}
