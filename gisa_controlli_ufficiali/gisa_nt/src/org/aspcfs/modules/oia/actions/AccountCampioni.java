...
public final class AccountCampioni extends CFSModule {
	...
	public String executeCommandTicketDetails(ActionContext context) {
		...
		try {
		    ...
		    idnodo = Integer.parseInt((String)context.getRequest().getAttribute("idNodo")) ;
		    }
		    else
		    {
	...
	public String executeCommandConfirmDelete(ActionContext context) {
		...
		try {
			...
			htmlDialog.addButton(systemStatus.getLabel("global.button.delete"),
					"javascript:window.location.href='OiaCampioni.do?command=DeleteTicket&id="
							+ id
							+ "&orgId="
							+ orgId
							+ "&forceDelete=true"
		...
		try {
			...
			if (recordDeleted) {
				...
				context
						...
						.setAttribute(
								"refreshUrl",
								"OiaVigilanza.do?command=TicketDetails&id="+id_controllo+"&orgId="
										+ orgId
										+ (inline != null
	...
	public String executeCommandInsert(ActionContext context)
		...
		try
		{
			...
			context.getRequest().setAttribute("idNodo", orgDetail.getId());
	...
	public String executeCommandDettaglio(ActionContext context) {
		if (!hasPermission(context, "oia-oia-view")) {
			return ("PermissionError");
		}
		Connection db = null;
		Ticket newTic = null;
			...
			String ticketId = context.getRequest().getParameter("id");
			 ...
			 tamponiList.buildListControlli(db, pasId, id_controllo);
			      ...
			      while(itAudit.hasNext()){
							  ...
							  OiaNodo orgDetails = new OiaNodo(db, Integer.parseInt(context.getParameter("idNodo")), this.getSystemStatus(context));
