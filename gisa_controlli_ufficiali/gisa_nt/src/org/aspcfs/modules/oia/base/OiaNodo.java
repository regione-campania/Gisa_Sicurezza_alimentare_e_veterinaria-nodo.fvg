...
public class OiaNodo extends GenericBean {
	...
	{
		try { this.setIdLookupAreaStruttura(rs.getInt("id_lookup_area_struttura_asl"));} catch(SQLException e)	{}
		try { this.setIdStrumentoCalcolo(rs.getInt("id_strumento_calcolo"));} catch(SQLException e)	{}
		try { this.setId(rs.getInt("id"));} catch(SQLException e)	{}
		try { this.setId_padre(rs.getInt("id_padre"));} catch(SQLException e)	{}
		try { this.setId_asl(rs.getInt("id_asl"));} catch(SQLException e)	{}
		try { this.setIdAsl(rs.getInt("id_asl"));} catch(SQLException e)	{}
		try { this.setSiteId(rs.getInt("id_asl"));} catch(SQLException e)	{}
		try { this.setN_livello(rs.getInt("n_livello"));} catch(SQLException e)	{}
		try { this.setTipologia_struttura(rs.getInt("tipologia_struttura"));} catch(SQLException e)	{}
		try { this.setDescrizione_lunga(rs.getString("descrizione_struttura"));} catch(SQLException e)	{}
		try { this.setCodiceInternoFK(rs.getInt("codice_interno_fk"));} catch(SQLException e)	{}
		try { this.setDataScadenza(rs.getTimestamp("data_scadenza"));} catch(SQLException e)	{}
		try	{ this.setAnno(rs.getInt("anno"));} catch(SQLException e)	{}
		try { this.setDescrizionePadre(rs.getString("descrizione_padre"));} catch(SQLException e)	{}
		try { this.setPercentuale_area_a(rs.getInt("percentuale_area_a"));} catch(SQLException e)	{}
		try { this.setTipologia(rs.getInt("tipologia"));} catch(SQLException e)	{}
		try { this.setOrgId(rs.getInt("org_id"));} catch(SQLException e)	{}
		try { this.setIdUtenteEdit(rs.getInt("id_utente_edit"));} catch(SQLException e)	{}
		try { this.setSommaUiArea(rs.getInt("somma_ui_area"));} catch(SQLException e)	{}
		try { this.setSommaUiAreaInizialeBloccata(rs.getInt("ui_struttura_foglio_att_iniziale"));} catch(SQLException e)	{}
		try { setStato(rs.getInt("stato"));} catch(SQLException e)	{}
		try { setStato_all2(rs.getInt("stato_all2"));} catch(SQLException e)	{}
		try { setStato_all6(rs.getInt("stato_all6"));} catch(SQLException e)	{}
		try { descrizioneAreaStruttureComplesse=rs.getString("descrizione_area_struttura");} catch(SQLException e)	{}
		try { this.setDescrizione_tipologia_struttura(rs.getString("descrizione_tipologia_struttura"));} catch(SQLException e)	{}
		try { this.setCodiceInternoUnivoco(rs.getInt("codice_interno_univoco"));} catch(SQLException e)	{}
	...
	public ArrayList<OiaNodo> loadbyidAsl(String id,int anno, Connection db)
	{
		...
		if( (id != null) && ! id.equals("") )
		{
			try
			{
				...
				//System.out.println("OiaNodo query : "+stat.toString());
	...
	public  ArrayList<OiaNodo> loadbyRegione(Connection db)
	{
		...
		ResultSet			res		= null;
			...
			try
			{
				...
				" left JOIN lookup_tipologia_nodo_oia tipooia ON oia_n.tipologia_struttura = tipooia.code " +
				" left JOIN lookup_site_id asl ON oia_n.id_asl = asl.code " +
				" where  oia_n.n_livello =0 and id_asl =14  order by oia_n.descrizione_lunga ";
...
PreparedStatement pst = null;
	...
	public ArrayList<OiaNodo> getStruttureDaControllareAutoritaCompetentiPerIdAsl(Connection db,int idAsl)
	{
ArrayList<OiaNodo> listaStrutture = new ArrayList<OiaNodo>();
...
PreparedStatement pst = null;
		...
		try
		{
			...
			int anno = calCorrente.get(Calendar.YEAR);
				...
				String t = "select * from get_strutture_da_controllare_ac(?, ?)"; 				
	...
	public ArrayList<OiaNodo> getPerContoDiAutoritaCompetentiPerIdAsl(Connection db,int idAsl)
	{
		ArrayList<OiaNodo> listaStrutture = new ArrayList<OiaNodo>();
		...
		try
		{
			...
			int anno = calCorrente.get(Calendar.YEAR);
				...
				String t = "select * from get_per_conto_di_ac(?, ?)"; 				
