...
import org.aspcfs.modules.acquedirete.base.AcqueReteMotiviNc;
 ...
 */
public final class AcqueReteCampioni extends CFSModule {
	...
	public String executeCommandReopenTicket(ActionContext context) {
		if (!hasPermission(context, "acquedirete-acquedirete-campioni-edit")) {
	...
	public String executeCommandTicketDetails(ActionContext context) {
		if (!hasPermission(context, "acquedirete-acquedirete-campioni-view")) {
			return ("PermissionError");
		}
		Connection db = null;
		// Parameters
		...
		try {
			...
			ArrayList<AcqueReteMotiviNc> motiviAcqueNcList = new ArrayList<AcqueReteMotiviNc>();
			motiviAcqueNcList = AcqueReteMotiviNc.getElencoMotiviNc(db); 
			context.getRequest().setAttribute("motiviAcqueNcList", motiviAcqueNcList); 
	...
	public String executeCommandConfirmDelete(ActionContext context) {
		if (!hasPermission(context, "acquedirete-acquedirete-campioni-delete")) {
			return ("PermissionError");
		}
		Connection db = null;
		// Parameters
	...
	public String executeCommandViewHistory(ActionContext context) {
		if (!hasPermission(context, "acquedirete-acquedirete-campioni-history-view")) {
			return ("PermissionError");
		}
		Connection db = null;
	...
	public String executeCommandAdd(ActionContext context) {
		if (!hasPermission(context, "acquedirete-acquedirete-campioni-add")) {
			return ("PermissionError");
		}
		Connection db = null;
		...
		try {
			...
			String ticketId = context.getRequest().getParameter("id");
			 ...
			 tamponiList.buildListControlli(db, pasId, id_controllo);
				      ...
				      Organization org = new org.aspcfs.modules.acquedirete.base.Organization(db , thisTicketV.getOrgId());
