 ...
 */
public final class MolluschiBivalviVigilanza extends CFSModule {
	...
	public String executeCommandAdd(ActionContext context) {
		if (!hasPermission(context, "molluschibivalvi-vigilanza-add")) {
		...
		try {
			...
			context.getRequest().setAttribute("tipologia", 201);
	...
	public String executeCommandInsert(ActionContext context) throws SQLException, ParseException {
		...
		try
		{
		    ...
		    if (esitDeclassamento!=null && esitDeclassamento.equals("SI"))
		    {
		    		thisOrg.updateDeclassamento(db, esitDeclassamento, Integer.parseInt(declassamento), newTic.getAssignedDate(), thisOrg.getOrgId());
		    	
		    }
			
			newTic.setCompanyName(thisOrg.getName());
			addModuleBean(context, "Aziende Agricole CU", "Aziende Agricole CU");
	...
	public String executeCommandModifyTicket(ActionContext context) {
		if (!hasPermission(context, "molluschibivalvi-vigilanza-edit")) {
			return ("PermissionError");
		}
		Connection db = null;
		Ticket newTic = null;
...
//			}
		...
		try {
			...
			Organization orgDetails = new Organization(db, Integer.parseInt(context.getParameter("orgId")));
			  String esitDeclassamento = context.getRequest().getParameter("esito");
			    ...
			    if (esitDeclassamento!=null && esitDeclassamento.equals("SI"))
			    {
			    	orgDetails.updateDeclassamento(db, esitDeclassamento, Integer.parseInt(declassamento),  newTic.getAssignedDate(), orgDetails.getOrgId());
	...
	public String executeCommandConfirmDelete(ActionContext context) {
		if (!hasPermission(context, "molluschibivalvi-vigilanza-delete")) {
			return ("PermissionError");
		}
		...
		try 
		{
			db = this.getConnection(context);
			org.aspcf.modules.controlliufficiali.action.AccountVigilanza controlliGeneric = new org.aspcf.modules.controlliufficiali.action.AccountVigilanza();
			context.getRequest().setAttribute("url_confirm","MolluschiBivalviVigilanza.do?command=DeleteTicket");
...
public String executeCommandSupervisiona(ActionContext context) {
	...
	public String executeCommandDeleteTicket(ActionContext context) {
		...
		try {
			...
			org.aspcf.modules.controlliufficiali.action.AccountVigilanza controlliGeneric = new org.aspcf.modules.controlliufficiali.action.AccountVigilanza();
			context.getRequest().setAttribute("url","MolluschiBivalviVigilanza.do?command=ViewVigilanza");
	...
	public String executeCommandTicketDetails(ActionContext context) {
		if (!hasPermission(context, "molluschibivalvi-vigilanza-view")) {
			return ("PermissionError");
		}
		Connection db = null;
		//Parameters
	...
	public String executeCommandViewHistory(ActionContext context) {
		if (!hasPermission(context, "molluschibivalvi-vigilanza-history-view")) {
			return ("PermissionError");
		}
		Connection db = null;
		String ticketId = context.getRequest().getParameter("id");
	...
	public String executeCommandViewVigilanza(ActionContext context) {
		...
		ticketListInfo.setLink(
				"MolluschiBivalviVigilanza.do?command=ViewVigilanza&orgId=" + passedId);
