 ...
 */
public final class MolluschiBivalviCampioni extends CFSModule {
	...
	public String executeCommandReopenTicket(ActionContext context) {
		if (!hasPermission(context, "canili-campioni-edit")) {
	...
	public String executeCommandTicketDetails(ActionContext context) {
		if (!hasPermission(context, "molluschibivalvi-campioni-view")) {
			return ("PermissionError");
		}
		Connection db = null;
		// Parameters
	...
	public String executeCommandConfirmDelete(ActionContext context) {
		if (!hasPermission(context, "molluschibivalvi-campioni-delete")) {
			return ("PermissionError");
		}
		Connection db = null;
		// Parameters
		...
		try {
			...
			htmlDialog.addButton(systemStatus.getLabel("global.button.delete"),
					"javascript:window.location.href='MolluschiBivalviCampioni.do?command=DeleteTicket&id="
							+ id
							+ "&orgId="
							+ orgId
							+ "&forceDelete=true"
	...
	public String executeCommandViewHistory(ActionContext context) {
		if (!hasPermission(context, "molluschibivalvi-campioni-history-view")) {
			return ("PermissionError");
		}
		Connection db = null;
	...
	public String executeCommandAdd(ActionContext context) {
		if (!hasPermission(context, "soa-soa-campioni-add")) {
			return ("PermissionError");
		}
		Connection db = null;
	...
	public String executeCommandInsert(ActionContext context)
			throws SQLException {
		if (!(hasPermission(context, "molluschibivalvi-campioni-add"))) {
			return ("PermissionError");
		}
		Connection db = null;
	...
	public String executeCommandChiudi(ActionContext context) {
		if (!(hasPermission(context, "molluschibivalvi-campioni-edit"))) {
			return ("PermissionError");
		}
		int resultCount = -1;
		Connection db = null;
		...
		try {
			...
			if(resultCount==-2)
				context.getRequest().setAttribute("Messaggio", "Chiusura non avvenuta assicurarsi di aver inserito la data dell'esito campione");
			...
			String ticketId = context.getRequest().getParameter("id");
			 ...
			 tamponiList.buildListControlli(db, pasId, id_controllo);
				      ...
				      Organization org = new Organization(db , thisTicketV.getOrgId());
