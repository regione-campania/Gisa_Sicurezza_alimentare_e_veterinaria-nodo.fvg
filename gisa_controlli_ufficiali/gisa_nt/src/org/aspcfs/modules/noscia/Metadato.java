...
public class Metadato implements Serializable{
	...
	public void getItems(Connection conn) {
		...
		if (this.html_type.equalsIgnoreCase("gruppo_calcolato") ||  this.html_type.equalsIgnoreCase("indirizzo") || 
				this.html_type.equalsIgnoreCase("coordinate") || this.html_type.equalsIgnoreCase("fiscale") || this.html_type.equalsIgnoreCase("gruppo_campi_estesi")){
	...
	private void popolaListaLookup(ResultSet rs, Connection db) throws SQLException{
			while(rs.next()){
	        	Metadato metadatoTemp = new Metadato();
	        	metadatoTemp.setId(this.id);
	        	metadatoTemp.setId_configuratore(rs.getInt("id"));
	        	metadatoTemp.setCodice_univoco_ml(this.codice_univoco_ml);
	        	metadatoTemp.setDesc_linea("");
	        	metadatoTemp.setHtml_label_sezione(this.html_label_sezione);
	        	metadatoTemp.setFtl_name(this.ftl_name);
	        	metadatoTemp.setHtml_label(rs.getString("html_label"));
	        	metadatoTemp.setHtml_enabled(this.html_enabled);
	        	...
	        	this.listaLookup.add(metadatoTemp);
	    }
	}
