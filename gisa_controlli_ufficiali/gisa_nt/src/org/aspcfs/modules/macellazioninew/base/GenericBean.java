...
public class GenericBean {
	...
	public static GenericBean loadByIdentificativoIdMacello(String identificativo, int idMacello, Connection db, ConfigTipo configTipo)
			throws Exception {
		...
		if ((identificativo != null) && (identificativo.trim().length() > 0)) {
			try {
				...
				stat = db.prepareStatement("SELECT * FROM " + configTipo.getNomeTabella() + "  WHERE "
						+ configTipo.getNomeCampoIdentificativoTabella() + " ilike ? and id_macello = ?  ORDER BY entered DESC");
				stat.setString(1, identificativo);
				stat.setInt(2, idMacello);
				System.out.println("[Macelli] Query bean: "+stat.toString());
				res = stat.executeQuery();
				if (res.next()) {
					ret = loadResultSet(res, db, configTipo);
				}
	...
	public static GenericBean loadByIdIdMacello(int id, int idMacello, Connection db, ConfigTipo configTipo)
			throws Exception {
		...
		if (id>0) {
			try {
				...
				stat = db.prepareStatement("SELECT * FROM " + configTipo.getNomeTabella() + "  WHERE id = ? and id_macello = ?  ORDER BY entered DESC");
				stat.setInt(1, id);
				stat.setInt(2, idMacello);
				System.out.println("[Macelli] Query bean: "+stat.toString());
				res = stat.executeQuery();
	...
	public static ArrayList<GenericBean> load(String asl, String identificativo, String provenienza, Connection db,
		...
		String sql = "SELECT * FROM "
				...
				+ " c, organization o WHERE c.trashed_date IS NULL AND o.trashed_date IS NULL AND c.id_macello = o.org_id ";
	...
	public static ArrayList<GenericBean> loadByStabilimentoPerDataMacellazione(String idStabilimento,
		...
		sql += " FROM "
				+ configTipo.getNomeTabella()
				+ " c, organization o "
				+ " where c.trashed_date IS NULL AND o.trashed_date IS NULL AND c.id_macello = o.org_id AND o.org_id = ? AND ";
