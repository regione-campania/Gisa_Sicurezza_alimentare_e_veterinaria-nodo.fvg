...
public class TicketList extends org.aspcfs.modules.troubletickets.base.TicketList
{
	...
	public void buildListControlli(Connection db, int org_id, String idC,int tipologiaNonConformita) throws SQLException {
	    ...
	    sqlCount.append(
	        ...
	        "FROM ticket t " 
	    	 ...
	    	 sqlCount.append(" left JOIN organization o ON (t.org_id = o.org_id) " +
		        "INNER JOIN ticket tic ON ((tic.tipologia = "+tipologiaNonConformita+")and(tic.id_controllo_ufficiale = t.id_controllo_ufficiale)) " +
	    ...
	    sqlCount.append(
	        ...
	        " where t.ticketid > 0 AND t.tipologia = 1 and tic.ticketid = "+idC+" ");
	    
	    if (this.orgId>0 && this.idStabilimento<= 0  && this.idApiario<= 0)
	    	sqlCount.append(" and t.org_id = "+org_id+" ");
	    else
	    	if (this.idStabilimento>0)
	    		sqlCount.append(" and t.id_stabilimento = "+idStabilimento+ " ");
	    	else 
	    		sqlCount.append(" and t.id_apiario = "+org_id+ " ");
	    ...
	    sqlSelect.append(
	        ...
	        "o.site_id AS orgsiteid,case when t.org_id>0 then o.tipologia else 999 end as tipo_operatore,cu.id_macchinetta as id_distributore " +

	        " FROM ticket t "+
	    //" left JOIN opu_operatori_denormalizzati_view oo ON (t.id_stabilimento = oo.id_stabilimento) "  +
	        " JOIN ticket nonconf ON nonconf.ticketid = t.id_nonconformita AND nonconf.tipologia = "+tipologiaNonConformita +
	        ...
	        " WHERE t.tipologia = 1 and nonconf.ticketid = "+idC+" and t.trashed_date is null and cu.trashed_date is null and nonconf.trashed_date is null ");
		    
	    if (this.orgId>0 && this.idStabilimento <= 0 && this.idApiario<=0)
	    	sqlSelect.append(" and o.trashed_date is null and t.org_id = "+org_id+" ");
	    else
	    	if (this.idStabilimento>0)
		    	sqlSelect.append(" and t.id_stabilimento = "+idStabilimento+ " ");
	    	else 
		    	sqlSelect.append(" and t.id_apiario = "+org_id+ " ");
	    pst = db.prepareStatement(
	...
	public void buildListControlliAlt(Connection db, int org_id, String idC,int tipologiaNonConformita) throws SQLException {
	    PreparedStatement pst = null;
	    ResultSet rs = null;
	    int items = -1;
	    StringBuffer sqlSelect = new StringBuffer();
	...
	public void buildList(Connection db,int tipologiaNonConformita) throws SQLException {
		    PreparedStatement pst = null;
		    ResultSet rs = null;
		    int items = -1;
		    StringBuffer sqlSelect = new StringBuffer();
		    ...
		    sqlCount.append(
		        ...
		        " where t.ticketid > 0 AND t.tipologia = 1 and t.trashed_date is null and cu.trashed_date is null and nonconf.trashed_date is null ");
		    createFilter(sqlFilter, db);
		    if (pagedListInfo != null) {
		      //Get the total number of records matching filter
		      pst = db.prepareStatement(
		    ...
		    sqlSelect.append(
		        ...
		        "o.site_id AS orgsiteid,cu.id_macchinetta " +
	...
	public void buildListRegistroOrdinanze(Connection db) throws SQLException {
	    ...
	    sqlSelect.append(
	        ...
	        "FROM ticket t " +
	        " left JOIN organization o ON (t.org_id = o.org_id and o.trashed_date is null  ) " +
	        " where t.ticketid > 0 AND t.tipologia = 1 ");
	    pst = db.prepareStatement(
	        sqlSelect.toString() + sqlFilter.toString() + sqlOrder.toString());
	    items = prepareFilter(pst);
	    if (pagedListInfo != null) {
