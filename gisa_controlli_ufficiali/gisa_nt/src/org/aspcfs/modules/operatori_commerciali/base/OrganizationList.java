...
public void setAutorizzato(int autorizzato) {
	this.autorizzato = autorizzato;
}

public void setAutorizzato(String autorizzato) {
    this.autorizzato = Integer.parseInt(autorizzato);
  }
...
public void setCodiceFiscaleCorrentista(String codiceFiscaleCorrentista) {
  ...
  public void buildList(Connection db) throws SQLException {
	    ...
	    while (rs.next()) {
	     ...
	     OrganizationAddressList lista =  thisOrganization.getAddressList();
	     lista.setOrgId(thisOrganization.getOrgId());
	    		 lista.buildList(db);
...
/**
   ...
   */
	public ResultSet queryList(Connection db, PreparedStatement pst) throws SQLException {
		...
		sqlCount.append("SELECT  COUNT(distinct o.*) AS recordcount "
				+ "FROM organization o left join organization_address oa on (oa.org_id = o.org_id ");

		if (addressType == null || addressType == -1) {
			sqlCount.append(" AND oa.address_type=2 )");
		} else {
			sqlCount.append(")");
		}

		/*
		 * if(codiceAllerta != null){ if(!codiceAllerta.equals("")){
		 * sqlCount.append(
		 * " join ticket cufficiali on o.org_id = cufficiali.org_id and cufficiali.codice_allerta ilike '"
		 * +codiceAllerta+"' and cufficiali.tipologia=3");
		 * 
		 * }
		 * 
		 * }
		...
		/* Union con gli operatori commerciali presenti innuova anagrafica */
		/*********** UNION CON OPU *****************/
		String operatori_commerciali_in_opu = "	select count(*) as recordcount from lista_opu_operatori_commerciali   where 1=1 ";

		if (name != null && !name.equals(""))
			operatori_commerciali_in_opu += " AND  ragione_sociale ilike ? ";

		if (partitaIva != null && !partitaIva.equals(""))
			operatori_commerciali_in_opu += " AND  partita_iva ilike ? ";

		if (codiceFiscale != null && !codiceFiscale.equals(""))
			operatori_commerciali_in_opu += " AND  codice_fiscale_impresa ilike ? ";

		if (city != null && !city.equals(""))
			operatori_commerciali_in_opu += " AND  comune_stab ilike ? ";

		if (orgSiteId != -1)
			operatori_commerciali_in_opu += " AND  id_asl_stab = ? ";

		/*******************************************************************/

		if (pagedListInfo != null) {
			// Get the total number of records matching filter
			pst = db.prepareStatement("SELECT sum(recordcount) AS recordcount from  (" + sqlCount.toString()
					+ sqlFilter.toString() + " UNION " + operatori_commerciali_in_opu + ") AS T;");
			...
			/*********** UNION CON OPU *****************/
			if (name != null && !name.equals(""))
				pst.setString(++items, "%" + name + "%");

			if (partitaIva != null && !partitaIva.equals(""))
				pst.setString(++items, "%" + partitaIva + "%");

			if (codiceFiscale != null && !codiceFiscale.equals(""))
				pst.setString(++items, "%" + codiceFiscale + "%");

			if (city != null && !city.equals(""))
				pst.setString(++items, "%" + city + "%");

			if (orgSiteId != -1)
				pst.setInt(++items, orgSiteId);
			/********************************************/
			...
			if (!pagedListInfo.getCurrentLetter().equals("")) {
				pst = db.prepareStatement(sqlCount.toString() + sqlFilter.toString() + " AND  "
						+ DatabaseUtils.toLowerCase(db) + "(o.name) < ? ");
				items = prepareFilter(pst);
		...
		/*sqlSelect.append("o.*, " + "ct_owner.namelast as o_namelast, ct_owner.namefirst as o_namefirst, "
		...
		sqlSelect.append(" o.org_id, o.name, o.site_id,oa.city as o_city,o.partita_iva,o.nome_rappresentante, o.cognome_rappresentante,o.categoria_rischio,o.data_chiusura_canile, o.notes "
				+ "FROM organization o "
		...
		if (addressType == null || addressType == -1) {
			sqlSelect.append(" AND oa.address_type=2 )"
					+ " left JOIN lookup_account_stage ast ON (o.stage_id = ast.code) " + " where o.org_id >= 0 ");
		} else {
			sqlSelect.append(")" + " left JOIN lookup_account_stage ast ON (o.stage_id = ast.code) "
					+ " where o.org_id >= 0 ");
		}

		/*********** UNION CON OPU *****************/
		operatori_commerciali_in_opu = "select distinct id_stabilimento as org_id, ragione_sociale as name, id_asl_stab as site_id, comune_stab as o_city, partita_iva, "
		+"nome_rapp_sede_legale as nome_rappresentante, cognome_rapp_sede_legale as nome_rappresentante, categoria_rischio, data_fine_attivita as data_chiusura_canile, 'OPU' as notes from lista_opu_operatori_commerciali   where 1=1 ";
		...
		pst = db.prepareStatement(sqlSelect.toString() + sqlFilter.toString() + " union  "
				+ operatori_commerciali_in_opu + sqlOrder.toString().replace("o.name", "name"));
		items = prepareFilter(pst);
		/*********** UNION CON OPU *****************/
		if (name != null && !name.equals(""))
			pst.setString(++items, "%" + name + "%");
  ...
  protected void createFilter(Connection db, StringBuffer sqlFilter) {
    ...
    else
    {
    	...
    	/*if ("".equals(statoCu))
    	{
    		sqlFilter.append(" AND  o.org_id in (select org_id from ticket where tipologia = 3 and codice_allerta ilike ? and trashed_Date is null)");
    ...
    if(autorizzato == 1){
    	sqlFilter.append(" AND  o.data_chiusura_canile is not null ");
    }
    else if(autorizzato == 2){
    	sqlFilter.append(" AND  o.data_chiusura_canile is null ");
    }
   ...
   /* if (city != null && !"-1".equals(city)) {
      if (city.indexOf("%") >= 0) {
        sqlFilter.append(
            ...
            " AND  city IS NOT NULL and address_type=?) and o.org_id IN (SELECT org_id FROM organization WHERE tipo_dest = ?) ");
      } else {
        sqlFilter.append(
            " AND  o.org_id IN (SELECT org_id FROM organization_address " +
    ...
    */
    ...
    else
    {
    	...
    	{
    		sqlFilter.append(" AND o.tipo_dest = ? ");
    	}
    }
