...
public final class AccountProvvedimentiNoNc extends CFSModule {
	...
	public String executeCommandTicketDetails(ActionContext context) {
		if (!hasPermission(context, "operatori_commerciali-provvedimenti-view")) {
	...
	public String executeCommandConfirmDelete(ActionContext context) {
		if (!hasPermission(context, "operatori_commerciali-provvedimenti-delete")) {
			return ("PermissionError");
		}
		Connection db = null;
		// Parameters
		...
		try {
			...
			htmlDialog.addButton(systemStatus.getLabel("global.button.delete"),
					"javascript:window.location.href='OperatoriCommercialiProvvedimenti.do?command=DeleteTicket&id="
							+ id
							+ "&orgId="
							+ orgId
							+ "&forceDelete=true"
		...
		try {
			...
			if (recordDeleted) {
				...
				context
						...
						.setAttribute(
								"refreshUrl",
								"OperatoriCommercialiVigilanza.do?command=TicketDetails&id="+id_controllo+"&orgId="
										+ orgId
										+ (inline != null
	...
	public String executeCommandUpdateTicket(ActionContext context) {
		if (!hasPermission(context, "operatori_commerciali-provvedimenti-edit")) {
			return ("PermissionError");
		}
		Connection db = null;
	...
	public String executeCommandAdd(ActionContext context) {
		if (!hasPermission(context, "operatori_commerciali-provvedimenti-add")) {
			return ("PermissionError");
		}
		Connection db = null;
	...
	public String executeCommandModify(ActionContext context) {
		if (!hasPermission(context, "operatori_commerciali-operatori_commerciali-edit")) {
			return ("PermissionError");
		}
	...
	public String executeCommandDettaglio(ActionContext context) {
		if (!hasPermission(context, "operatori_commerciali-operatori_commerciali-view")) {
			return ("PermissionError");
		}
		Connection db = null;
		Ticket newTic = null;
...
context.getRequest().setAttribute("AnimaliNonAlimentari",
	AnimaliNonAlimentari);
			...
			Sequestri.addItem(-1, systemStatus
					.getLabel("calendar.none.4dashes"));
			context.getRequest().setAttribute("Sequestri", Sequestri);		} catch (Exception e) {
			context.getRequest().setAttribute("Error", e);
			return ("SystemError");
		} finally {
			this.freeConnection(context, db);
		...
		try {
			...
			String ticketId = context.getRequest().getParameter("id");
			 ...
			 tamponiList.buildListControlli(db, pasId, id_controllo);
				      ...
				      Organization org = new org.aspcfs.modules.operatori_commerciali.base.Organization(db , thisTicketV.getOrgId());
