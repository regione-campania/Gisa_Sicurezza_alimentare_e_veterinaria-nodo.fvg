...
public final class AccountCessazionevariazione extends CFSModule {
  ...
  public String executeCommandReopenTicket(ActionContext context) {
    if (!hasPermission(context, "stabilimenti-stabilimenti-cessazionevariazione-edit")) {
  ...
  public String executeCommandAddTicket(ActionContext context) {
    if (!hasPermission(context, "stabilimenti-stabilimenti-cessazionevariazione-add")) {
      return ("PermissionError");
    }
    Connection db = null;
    Ticket newTic = null;
  ...
  public String executeCommandInsertTicket(ActionContext context) {
    ...
    newTic.setOperazione("Richiesta Voltura");
    newTic.setStatoVoltura(Integer.parseInt(context.getParameter("statoVoltura")));
...
//     datiIniziali.setId(Integer.parseInt(newOrg.getIdVoltura(db, newOrg.getOrgId())));
  ...
  public String executeCommandTicketDetails(ActionContext context) {
    if (!hasPermission(context, "stabilimenti-stabilimenti-cessazionevariazione-view")) {
      return ("PermissionError");
    }
    Connection db = null;
    //Parameters
    ...
    if(ticketId == null)
    	ticketId = ""+((Ticket)context.getRequest().getAttribute("TicketDetails")).getId();
    String retPag = null;
    try {
      db = this.getConnection(context);
      // Load the ticket
  ...
  public String executeCommandConfirmDelete(ActionContext context) {
    if (!hasPermission(context, "stabilimenti-stabilimenti-cessazionevariazione-delete")) {
      return ("PermissionError");
    }
    Connection db = null;
    //Parameters
    ...
    try {
      ...
      htmlDialog.addButton(
          systemStatus.getLabel("global.button.delete"), "javascript:window.location.href='StabilimentiCessazionevariazione.do?command=DeleteTicket&id=" + id + "&orgId=" + orgId + "&forceDelete=true" + RequestUtils.addLinkParams(
  ...
  public String executeCommandDeleteTicket(ActionContext context) {
    ...
    try {
      ...
      if (recordDeleted) {
        ...
        context.getRequest().setAttribute("OrgDetails", newOrg);
        context.getRequest().setAttribute(
            "refreshUrl", "Stabilimenti.do?command=ViewCessazionevariazione&orgId=" + orgId +
  ...
  public String executeCommandTrashTicket(ActionContext context) {
    ...
    try {
      ...
      if (recordUpdated) {
        ...
        String inline = context.getRequest().getParameter("popupType");
        context.getRequest().setAttribute(
            "refreshUrl", "Stabilimenti.do?command=ViewTickets&orgId=" + orgId +
  ...
  public String executeCommandConfermaVoltura(ActionContext context) {
	    ...
	    try {
	      ...
	      SiteIdList.addItem(-1, "-- SELEZIONA VOCE --");
	      context.getRequest().setAttribute("SiteIdList", SiteIdList);
	      newTic.queryRecord(db, Integer.parseInt(context.getParameter("idVoltura")));
	      
	      int statoVoltura = 0 ; // in richiesta
	      String motivazione = context.getParameter("motivazione");
	      String approvazione = context.getParameter("approvazione");
	      
	      
	      /*update sullo stabilimento*/
	      Organization orgDetails = new Organization(db, newTic.getOrgId());
	      if("1".equals(approvazione))
	      {
	    	   statoVoltura = 1 ; // approvata
	      ...
	      orgDetails.setCodiceFiscaleRappresentante(newTic.getCodiceFiscaleRappresentante().trim());
	      ...
	      newTic.setOperazione("Approvata");
	      }
	      else
	      {
	    	  if("2".equals(approvazione))
	    	  {
	    	  statoVoltura = 2 ; // approvata
	    	  newTic.setOperazione("Non Approvata");
	    	  }
	    	  else
	    	  {
	    		  if("3".equals(approvazione))
		    	  {
		    	  statoVoltura = 3 ; // approvata
		    	  newTic.setOperazione("INOLTRO FUNZIONARI REGIONALI");
		    	  }
	    		  else
	    		  {
	    			  if("4".equals(approvazione))
			    	  {
			    	  statoVoltura = 4 ; // approvata
			    	  newTic.setOperazione("INOLTRO SETTORE VETERINARIO");
			    	  }
	      ...
	      newTic.setStatoVoltura(statoVoltura);
	      newTic.setMotivazione(motivazione);
	      newTic.aggiornaStatoVoltura(db);
