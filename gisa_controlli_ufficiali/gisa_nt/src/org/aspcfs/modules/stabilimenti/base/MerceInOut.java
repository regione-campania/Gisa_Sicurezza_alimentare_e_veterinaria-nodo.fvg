...
public class MerceInOut
{
	...
	public static ArrayList<MerceInOut> loadList(String id, Connection db )
	{
		...
		if( (id != null) && (id.trim().length() > 0) )
		{
			...
			{
				int iid = Integer.parseInt( id );
				stat	= db.prepareStatement( "SELECT * FROM merce_in_out WHERE id_stabilimento = ? and trashed_date IS NULL ORDER BY id DESC" );
				stat.setInt( 1, iid );
				res		= stat.executeQuery();
				while( res.next() )
				{
	...
	public void setId_stabilimentoString(String parameter)
	{
		if( (parameter != null) && (parameter.trim().length() > 0) )
		{
			id_stabilimento = Integer.parseInt( parameter.trim() );
		}
	}
