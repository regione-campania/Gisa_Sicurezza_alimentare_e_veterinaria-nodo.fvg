...
//  private String contactOtherState = null;
  ...
  private String[] listaStatiIstruttoria ;
  
  
  public String[] getListaStatiIstruttoria() {
	return listaStatiIstruttoria;
}

public void setListaStatiIstruttoria(String[] listaStatiIstruttoria) {
	this.listaStatiIstruttoria = listaStatiIstruttoria;
}
...
public String getStatoCu() {
  ...
  public void setStatoCu(String statoCu) {
	...
	public void setSottoAttivita(String sottoAttivita) {
		this.sottoAttivita = Integer.parseInt(sottoAttivita);
	}
...
/**
  ...
  public void buildListCondizionatiInScadenza(Connection db) throws SQLException {
	    PreparedStatement pst = null;
	    if(city!=null)
	    {
	        this.city=this.city.replaceAll("%", "").trim();
	    ...
	    ResultSet rs = queryListCondizionatiScadenza(db, pst);
  ...
  public ResultSet queryList(Connection db, PreparedStatement pst) throws SQLException {
    ...
    if( ( sottoAttivita > -1 ) 
        	...
        	else if(  codiceSezione > -1  && ( sottoAttivita == -1 )) {
        		sqlCount.append(join()); 
        	}
    ...
    sqlSelect.append( " distinct on (jjj.org_id) jjj.* from ( select distinct " );
    if( codiceAllerta != null){
    	if(!codiceAllerta.equals(""))
    	{
    sqlSelect.append(
        "o.*, qrcd.id as id_controllo_documentale," +
    ...
    if( ( sottoAttivita > -1 ) 
			|| ( codiceSezione > -1 ) ){
    	sqlSelect.append(join());//joinAudit() +
    	}
    	else if(  codiceSezione > -1  && ( sottoAttivita == -1 )) {
    		sqlCount.append(join()); 
    	}
		...
		" where o.org_id >= 0 and oa.address_type = 5 " );
    	}
    	else
    	{
    		sqlSelect.append(
    ...
    pst = db.prepareStatement(
        sqlSelect.toString() + sqlFilter.toString() + sqlOrder.toString() + " )jjj ");
    items = prepareFilter(pst);
  ...
  public ResultSet queryListCondizionatiScadenza(Connection db, PreparedStatement pst) throws SQLException {
	    ...
	    sqlFilter.append(" AND  stato_lab=5 and ( (stato_istruttoria= 7 and current_date >((data_assegnazione_approval_number+ interval '3 month') - interval '10 days')) or (stato_istruttoria= 9 and current_date >((data_assegnazione_approval_number+ interval '6 month') - interval '10 days'))");
	...
	private void and(StringBuffer sqlFilter)
	{
		
		if( sottoAttivita > -1 )
		{
			sqlFilter.append( " AND ssa.codice_impianto = ? " );
		}
		  
		if( codiceSezione > -1 )
		{
			sqlFilter.append( " AND ssa.codice_sezione = ? " );
		}
	...
	private String join()
	{
		String ret = " ";
			...
			ret = " LEFT JOIN stabilimenti_sottoattivita ssa ON (o.org_id = ssa.id_stabilimento) ";
	...
	private String joinAudit()
	{
		...
		if( categoria > -1 )
		{
			ret = " LEFT JOIN audit ON (o.org_id = audit.org_id) ";
		}
	...
	private int set(PreparedStatement pst, int i) throws SQLException
	{
		  if( sottoAttivita > -1 )
		  {
			  pst.setInt( ++i, sottoAttivita );
		  }
		  
		  if( codiceSezione > -1 )
		  {
			  pst.setInt( ++i, codiceSezione );
		  }
  ...
  protected void createFilter(Connection db, StringBuffer sqlFilter) {
    ...
    if (codiceAllerta != null && ! "".equals(codiceAllerta))
    {
    	...
    	if (codiceAllerta.equalsIgnoreCase("%tutte%"))
    	{
    		...
    		{
    			sqlFilter.append(" AND  o.org_id in (select org_id from ticket where tipologia = "+tipologia+" and closed is null and  codice_allerta in (select id_allerta from ticket where tipologia = 700 and trashed_date is null ) and trashed_Date is null)");

    		}
    		else
    		{
    			sqlFilter.append(" AND  o.org_id in (select org_id from ticket where tipologia = "+tipologia+"  and closed is not null and  codice_allerta in (select id_allerta from ticket where tipologia = 700 and trashed_date is null ) and trashed_Date is null)");
    ...
    else
    {
    	if ("%aperti%".equals(statoCu))
		{
			sqlFilter.append(" AND  o.org_id in (select org_id from ticket where tipologia ="+tipologia+"  and closed is null and  codice_allerta ilike ? and trashed_Date is null)");
        	
		}
    	else
    	{
    		sqlFilter.append(" AND  o.org_id in (select org_id from ticket where tipologia = "+tipologia+"  and closed is not null and  codice_allerta ilike ? and trashed_Date is null)");
    ...
    if( ( sottoAttivita > -1 ) 
    	...
    	sqlFilter.append(" AND ssa.codice_impianto = ?  AND ssa.codice_sezione = ? and o.direct_bill = false ");
    	//sqlFilter.append(" AND (o.impianto = ?) AND ssa.codice_sezione = ? and o.direct_bill = false ");
    
  }
	else if(  codiceSezione > -1  && ( sottoAttivita == -1 )) {
    	sqlFilter.append(" AND ssa.codice_sezione = ? and direct_bill = false  ");
	}
	...
	if ( sottoAttivita == 24 && codiceSezione == 8) {
		sqlFilter.append(" AND direct_bill = true  AND tipologia = "+tipologia);
	} else  {
		sqlFilter.append(" AND direct_bill = false ");
	}
		
    
    if (statoLab > -1) {
        sqlFilter.append(" AND  o.stato_lab = ? ");
        }
    /*else
    {
    	 sqlFilter.append(" AND  o.stato_lab not in (4) ");
    }*/
    ...
    if ((numAut != null)&&(numAut.trim().length()>0)) {
        
          sqlFilter.append(
              " AND  " + DatabaseUtils.toLowerCase(db) + "(o.numaut) LIKE ? ");
    ...
    if (orgSiteId == -1) {
      if (includeOrganizationWithoutSite) {
        sqlFilter.append(" AND  o.site_id IS not NULL ");
      }
    }
   ...
   if (listaStatiIstruttoria!= null && listaStatiIstruttoria.length>0)
   {
	   sqlFilter.append(" AND  o.stato_istruttoria in ( ");  
   for(int indice = 0 ; indice<listaStatiIstruttoria.length-1;indice++)
   {
	   sqlFilter.append(""+listaStatiIstruttoria[indice]+",");
   }
   sqlFilter.append(""+listaStatiIstruttoria[listaStatiIstruttoria.length-1]+") ");
   }
   else
   {
	   sqlFilter.append(" AND  o.stato_istruttoria =0 "); 
   }
    ...
    if (tipo_stab != null) {
        if (tipo_stab.indexOf("%") >= 0) {
          sqlFilter.append(
              " AND  " + DatabaseUtils.toLowerCase(db) + "(o.tipo_stab) LIKE ? ");
        } else {
          sqlFilter.append(
              " AND  " + DatabaseUtils.toLowerCase(db) + "(o.tipo_stab) = ? ");
        }
      }
    ...
    if (city != null && !"-1".equals(city)) {
      if (city.indexOf("%") >= 0) {
        sqlFilter.append(
            ...
            " where (" + DatabaseUtils.toLowerCase(db, "city") + " ilike ? ) "+
 ...
 * @throws IllegalArgumentException 
   */
  public int reassignElements(Connection db, int newOwner) throws SQLException, IllegalArgumentException, IllegalAccessException, InvocationTargetException {
    int total = 0;
    Iterator i = this.iterator();
    while (i.hasNext()) {
      Organization thisOrg = (Organization) i.next();
 ...
 * @throws IllegalArgumentException 
   */
  public int reassignElements(Connection db, int newOwner, int userId) throws SQLException, IllegalArgumentException, IllegalAccessException, InvocationTargetException {
    int total = 0;
    Iterator i = this.iterator();
    while (i.hasNext()) {
      Organization thisOrg = (Organization) i.next();
  ...
  protected int prepareFilter(PreparedStatement pst) throws SQLException {
    ...
    if( ( sottoAttivita > -1 ) 
    	...
    	/*if ( sottoAttivita != 24 && codiceSezione == 8) 
    	{*/
    	pst.setInt(++i, this.getSottoAttivita());
    	// pst.setInt(++i, this.getSottoAttivita());
    	 pst.setInt(++i, this.getCodiceSezione());
    	//}
    ...
    if ((numAut != null)&&(numAut.trim().length()>0)) {
        pst.setString(++i, numAut.toLowerCase());
      }
    ...
    if (tipo_stab != null) {
        pst.setString(++i, tipo_stab.toLowerCase());
      }
