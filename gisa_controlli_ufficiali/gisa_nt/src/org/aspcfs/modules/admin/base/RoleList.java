 ...
 * @version $Id: RoleList.java 24287 2007-12-09 11:28:24Z srinivasar@cybage.com $
  ...
  public void buildList(Connection db,String suffisso) throws SQLException {
    ...
    if (buildUsers || buildUserCount) {
      ...
      while (i.hasNext()) {
        Role thisRole = (Role) i.next();
        if (buildUsers) {
          thisRole.buildUserList(db,suffisso);
        }
  ...
  public PreparedStatement prepareList(Connection db,String suffisso) throws SQLException {
    ...
    sqlCount.append(
        "SELECT COUNT(*) AS recordcount " +
            "FROM " + DatabaseUtils.addQuotes(db, "role")+suffisso + " r " +
            " where r.role_id > -1 ");
