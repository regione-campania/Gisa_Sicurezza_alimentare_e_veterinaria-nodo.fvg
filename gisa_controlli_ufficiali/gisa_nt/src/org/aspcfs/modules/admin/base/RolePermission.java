 ...
 *
 * @author Mathur
 * @version $Id: RolePermission.java 24287 2007-12-09 11:28:24Z srinivasar@cybage.com $
...
public class RolePermission extends GenericBean {
   ...
   * @throws SQLException Description of the Exception
   */
  public RolePermission(Connection db, int thisId,String suffisso) throws SQLException {
    PreparedStatement pst = db.prepareStatement(
        "SELECT * " +
        "FROM role_permission"+suffisso+" rp " +
        " where rp.id = ? ");
    pst.setInt(1, thisId);
   ...
   * @throws SQLException Description of the Exception
   */
  public boolean insert(Connection db,String suffisso) throws SQLException {
   
    StringBuffer sql = new StringBuffer(); 
    sql.append(        
        "INSERT INTO role_permission"+ suffisso+" (role_id, permission_id, role_view, " +
  ...
  public boolean update(Connection db,String suffisso) throws SQLException{
    if(id < 0){
      return false;
    }
    StringBuffer sqlInsert = new StringBuffer( 
      "UPDATE role_permission"+ suffisso+" SET " +
      "role_id = ?, " +
      "permission_id = ?, " +
      "role_view = ?, " +
      "role_add = ?, " +
