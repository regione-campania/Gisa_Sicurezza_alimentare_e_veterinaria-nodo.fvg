...
public final class AccountProvvedimentiNoNc extends CFSModule {
	...
	public String executeCommandTicketDetails(ActionContext context) {
		...
		try {
			...
			Proprietario proprietario = op.dettaglioProprietario(orgId, idControlloUfficiale) ;
	...
	public String executeCommandConfirmDelete(ActionContext context) {
		...
		try {
			...
			htmlDialog.addButton(systemStatus.getLabel("global.button.delete"),
					"javascript:window.location.href='CaniPadronaliProvvedimenti.do?command=DeleteTicket&id="
							+ id
							+ "&orgId="
							+ orgId
							+ "&forceDelete=true"
		...
		try {
			...
			if (recordDeleted) {
				...
				context
						...
						.setAttribute(
								"refreshUrl",
								"CaniPadronaliVigilanza.do?command=TicketDetails&id="+id_controllo+"&orgId="
										+ orgId
										+ (inline != null
	...
	public String executeCommandUpdateTicket(ActionContext context) {
		...
		try {
			...
			String idC = context.getRequest().getParameter("idC");
			int idControlloUfficiale = Integer.parseInt(idC);
			Proprietario proprietario = op.dettaglioProprietario(Integer.parseInt(context.getParameter("orgId")), idControlloUfficiale) ;
	...
	public String executeCommandAdd(ActionContext context) {
		if (!hasPermission(context, "canipadronali-sanzioni-add")) {
			return ("PermissionError");
		}
		Connection db = null;
		...
		try {
			...
			String idC = context.getRequest().getParameter("idC");
			int idControlloUfficiale = Integer.parseInt(idC);
			Proprietario proprietario = op.dettaglioProprietario(tempid, idControlloUfficiale) ;
	...
	public String executeCommandInsert(ActionContext context)
		...
		try
		{
			...
			String idC = context.getRequest().getParameter("idC");
			int idControlloUfficiale = Integer.parseInt(idC);
			Proprietario proprietario = op.dettaglioProprietario(newTic.getOrgId(), idControlloUfficiale) ;
	...
	public String executeCommandDettaglio(ActionContext context) {
		if (!hasPermission(context, "canipadronali-canipadronali-view")) {
			return ("PermissionError");
		}
		Connection db = null;
		Ticket newTic = null;
