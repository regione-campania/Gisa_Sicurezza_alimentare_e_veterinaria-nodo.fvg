...
public class OrganizationList extends Vector implements SyncableList {
  ...
  private static final int TIPOLOGIA_PUNTI_DI_SBARCO = 5 ;
...
public int getSiteId() {
  ...
  public ResultSet queryList(Connection db, PreparedStatement pst) throws SQLException {
	    ...
	    sqlCount.append(
	        ...
	        " WHERE o.org_id > 0 and tipologia = "+TIPOLOGIA_PUNTI_DI_SBARCO +" ");	   
	    ...
	    sqlSelect.append(
	            ...
	            " FROM organization o " +
	            " LEFT JOIN organization_address oa on ( o.org_id = oa.org_id and oa.address_type = 5) " +
	            " LEFT JOIN lookup_site_id asl ON (o.site_id = asl.code) WHERE o.org_id > 0 and tipologia = "+TIPOLOGIA_PUNTI_DI_SBARCO+" " );
  ...
  private ResultSet queryListOpControllati(Connection db, PreparedStatement pst, int ticketId) throws SQLException {
	  ...
	  //sqlSelect.append("SELECT * FROM organization                            o LEFT JOIN punti_sbarco_operatori_controllati p on o.org_id         = p.org_id where ticket_id = ?" );
	    sqlSelect.append("SELECT o.riferimento_id as org_id, case when org.tipologia is not null then org.tipologia else 17 end as tipologia, case when org.account_number is not null then org.account_number else n_reg end as account_number, p.conservato , o.ragione_sociale as name, org.nome_correntista FROM ricerche_anagrafiche_old_materializzata o LEFT JOIN punti_sbarco_operatori_controllati p on o.riferimento_id = p.org_id left join organization org on org.org_id = o.riferimento_id where children_ticket_id = ?" );
