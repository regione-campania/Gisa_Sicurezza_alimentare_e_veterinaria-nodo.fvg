 ...
 */
public final class PuntiSbarcoVigilanza extends CFSModule {
	...
	public String executeCommandAdd(ActionContext context) {
		if (!hasPermission(context, "punti_di_sbarco-vigilanza-add")) {
		...
		try {
			...
			context.getRequest().setAttribute("tipologia", 5);
	...
	public String executeCommandInsert(ActionContext context) throws SQLException, ParseException {
			...
			if (recordInserted && isValid) {
				...
				if(size != null){
					...
					for(int i=1; i<=(Integer.parseInt(size)); i++)
					{
						...
						{
							controlliGeneric.executeCommandInsert(context,db);
							Ticket newTicoperatore = (Ticket) context.getRequest().getAttribute("TicketDetails");
							ticket_new = newTicoperatore.getId();
							newTic.setId(ticket_new);
						}
						...
						if(conservazioneProdotto.equals("Si")){
							newTic.insertCUOperatore(db,org_id_op,true, ticket_id_padre);
						}
						else {
		...
		if(retPage.equals("ViewVigilanzaOK"))
			return executeCommandViewVigilanza(context);
		else
	...
	public String executeCommandModifyTicket(ActionContext context) {
		if (!hasPermission(context, "punti_di_sbarco-vigilanza-edit")) {
			return ("PermissionError");
		}
		Connection db = null;
		Ticket newTic = null;
...
//			}
	...
	public String executeCommandConfirmDelete(ActionContext context) {
		if (!hasPermission(context, "punti_di_sbarco-vigilanza-delete")) {
			return ("PermissionError");
		}
		...
		{	db=this.getConnection(context);
			org.aspcf.modules.controlliufficiali.action.AccountVigilanza controlliGeneric = new org.aspcf.modules.controlliufficiali.action.AccountVigilanza();
			context.getRequest().setAttribute("url_confirm","PuntiSbarcoVigilanza.do?command=DeleteTicket");
	...
	public String executeCommandDeleteTicket(ActionContext context) {
		...
		try {
			...
			org.aspcf.modules.controlliufficiali.action.AccountVigilanza controlliGeneric = new org.aspcf.modules.controlliufficiali.action.AccountVigilanza();
			context.getRequest().setAttribute("url","PuntiSbarcoVigilanza.do?command=ViewVigilanza");
	...
	public String executeCommandTicketDetails(ActionContext context) {
		if (!hasPermission(context, "punti_di_sbarco-vigilanza-view")) {
			return ("PermissionError");
		}
		Connection db = null;
		//Parameters
...
public String executeCommandSupervisiona(ActionContext context) {
	...
	public String executeCommandViewHistory(ActionContext context) {
		if (!hasPermission(context, "punti_di_sbarco-vigilanza-history-view")) {
			return ("PermissionError");
		}
		Connection db = null;
		String ticketId = context.getRequest().getParameter("id");
