...
//		MacelliUtil.setNextNumeroVerbaleSequestro( this, db );
	    ...
	    for( int i = 1; i <= v.size(); i++ )
	    {
	        ...
	        else
	        {
	            ...
	            case STRING:
	            	...
	            	s = s.replaceAll("\\r", " ");
	            	s = s.replaceAll("\\n", " ");
	            	s = s.replaceAll("u00E0", "a");
	            	s = s.replaceAll("u00f2", "o");
	            	s = s.replaceAll("u00f9", "u");
	                stat.setString( i, s );
	                break;
	            case BOOLEAN:
	...
	public static ArrayList<Capo> load( String asl, String identificativo, String provenienza, Connection db )
	{
		...
		String sql = "SELECT * FROM m_capi c, organization o WHERE c.trashed_date IS NULL AND o.trashed_date IS NULL AND c.id_macello = o.org_id ";
	...
	public static ArrayList<Capo> loadByStabilimento(String idStabilimento, Connection db)
	{
		...
		String sql = "SELECT c.* FROM m_capi c, organization o " +
					 " where c.trashed_date IS NULL AND o.trashed_date IS NULL AND c.id_macello = o.org_id AND o.org_id = ? " +
					 " AND  c.vpm_data IS NOT NULL AND c.vpm_data = (SELECT max(m.vpm_data) FROM m_capi m WHERE m.trashed_date IS NULL AND m.vpm_data IS NOT NULL AND m.id_macello = ?)"+
	...
	public static ArrayList<Capo> loadByStabilimentoPerSessioneMacellazione(String idStabilimento, Timestamp dataMacellazione, Integer numSessione, Connection db)
	{
		...
		sql+= " and stato_macellazione != 'Incompleto: Inseriti solo i dati sul controllo documentale' " +
				 " ORDER BY c.entered desc ";
	...
	public static ArrayList<Capo> loadByStabilimentoCapiNonMacellati(String idStabilimento, Connection db)
	{
		ArrayList<Capo>		ret		= new ArrayList<Capo>();
		PreparedStatement	stat	= null;
		ResultSet			res		= null;
		...
		String sql = "SELECT c.* FROM m_capi c, organization o " +
					 " where c.trashed_date IS NULL AND o.trashed_date IS NULL AND c.id_macello = o.org_id AND o.org_id = ? " +
					 " AND  (c.vpm_data IS NULL or stato_macellazione = 'Incompleto: Inseriti solo i dati sul controllo documentale') "+
					 " ORDER BY c.entered DESC ";
		try
		{	
			stat = db.prepareStatement( sql );
...
String sql = "select sed.max_num,sed.data " +
"from  " +
"(  " +
"select  data, max(numero) as max_num, id_macello as id_mac   " +
"from m_capi_sedute sed    " +
" where id_macello = ?  " +  
" group by  data  , id_mac  " +
")  " +
"sed  " +
" JOIN  " +
"(		  " +		     
"select coalesce(vpm_data,mavam_data) as data_seduta, max(cd_seduta_macellazione) as num_seduta,id_macello  " +
"from m_capi c  " +
" where stato_macellazione != 'Incompleto: Inseriti solo i dati sul controllo documentale'  " +
" AND  id_macello = ? and trashed_date is null  " +
" group by  id_macello,coalesce(vpm_data,mavam_data)  " +
")  " +
"capi on capi.data_seduta = sed.data and capi.num_seduta=sed.max_num and capi.id_macello=sed.id_mac order by sed.data desc limit ?";
		...
		{	
			...
			stat.setInt( 3, numDate );
			
			res = stat.executeQuery();
			while( res.next() )
			{
				ret.add( sdf.format(res.getTimestamp("data")) );
			}
		}
		catch (Exception e)
		{
