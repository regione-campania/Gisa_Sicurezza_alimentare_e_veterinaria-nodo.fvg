 ...
 * @created June 1, 2005
 */
public class PrescrizioniHistoryList extends ArrayList implements SyncableList {
  //sync api
  ...
  public void buildList(Connection db) throws SQLException {
    ...
    while (rs.next()) {
      PrescrizioniHistory thisHistoryElement = new PrescrizioniHistory(rs);
  ...
  protected int prepareFilter(PreparedStatement pst) throws SQLException {
    ...
    if (!notes) {
      pst.setInt(++i, PrescrizioniHistory.NOTE);
    }
    if (!activities) {
      pst.setInt(++i, PrescrizioniHistory.COMPLETE_ACTIVITY);
      pst.setInt(++i, PrescrizioniHistory.CANCELED_ACTIVITY);
    }
    if (!email) {
      pst.setInt(++i, PrescrizioniHistory.CFSNOTE);
      pst.setInt(++i, PrescrizioniHistory.CAMPAIGN);
    }
    if (!quotes) {
      pst.setInt(++i, PrescrizioniHistory.QUOTE);
    }
    if (!opportunities) {
      pst.setInt(++i, PrescrizioniHistory.OPPORTUNITY);
    }
    if (!serviceContracts) {
      pst.setInt(++i, PrescrizioniHistory.SERVICE_CONTRACT);
    }
    if (!tickets) {
      pst.setInt(++i, PrescrizioniHistory.TICKET);
    }
    if (!tasks) {
      pst.setInt(++i, PrescrizioniHistory.TASK);
    }
    if (!documents) {
      pst.setInt(++i, PrescrizioniHistory.ACCOUNT_DOCUMENT);
    }
    if (!relationships) {
      pst.setInt(++i, PrescrizioniHistory.RELATIONSHIP);
    }
    if (!assets) {
      pst.setInt(++i, PrescrizioniHistory.ASSET);
    }
    if (!showDisabledWithEnabled) {
  ...
  public boolean delete(Connection db) throws SQLException {
    ...
    while (iterator.hasNext()) {
      PrescrizioniHistory history = (PrescrizioniHistory) iterator.next();
