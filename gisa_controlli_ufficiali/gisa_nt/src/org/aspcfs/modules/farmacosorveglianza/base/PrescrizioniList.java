 ...
 *      Exp $
 */
public class PrescrizioniList extends Vector implements SyncableList {

	Logger logger = Logger.getLogger("MainLogger");
	
private static Logger log = Logger.getLogger(org.aspcfs.modules.farmacosorveglianza.base.PrescrizioniList.class);
  static {
  ...
  public final static String tableName = "prescrizioni2";
  public final static String uniqueField = "id_prescrizioni";
  ...
  private java.sql.Timestamp dataPrescrizione1 = null;
  private java.sql.Timestamp dataPrescrizione2 = null;
...
this.veterinarioC = tmp;
  ...
  public void setDataPrescrizione1(String tmp) {
	    this.dataPrescrizione1 = DateUtils.parseDateStringNew(tmp, "dd/MM/yyyy");
	    
	  }
  
  /*public void setDataPrescrizione1(String tmp) {
	  this.dataPrescrizione1 = DatabaseUtils.parseTimestamp(tmp);
	  }*/
  
  public void setDataPrescrizione2(String tmp) {
	    this.dataPrescrizione2 = DateUtils.parseDateStringNew(tmp, "dd/MM/yyyy");
	  }
...
public void setQuantitaFarmaco1(String tmp) {
  this.setIdFarmaco1(Integer.parseInt(tmp));
}

public void setDataDispensazione(String tmp) {
	  this.dataDispensazione = DateUtils.parseDateStringNew(tmp, "yyyy-MM-dd");
	  }

public void setDataPrescrizione1(java.sql.Timestamp tmp) {
    this.dataPrescrizione1 = tmp;
  }
public void setDataPrescrizione2(java.sql.Timestamp tmp) {
    this.dataPrescrizione2 = tmp;
  }
...
public void setNote1(String tmp) {
  ...
  public void setIdFarmacia(int tmp) {
	...
	public int getIdFarmacia() {
	  ...
	  public java.sql.Timestamp getDataPrescrizione1() {
		    return dataPrescrizione1;
		  }
	  public java.sql.Timestamp getDataPrescrizione2() {
		    return dataPrescrizione2;
		  }

	  public java.sql.Timestamp getDataPrescrizione12() {
		    return dataPrescrizione1 = DateUtils.parseDateStringNew(dataPrescrizione1.toString(), "yyyy-MM-dd");
		  }
	  
	  public java.sql.Timestamp getDataPrescrizione22() {
		    return dataPrescrizione2 = DateUtils.parseDateStringNew(dataPrescrizione2.toString(), "yyyy-MM-dd");
		  }
...
public void setQuantitaFarmaco2(String tmp) {
  this.setIdFarmaco2(Integer.parseInt(tmp));
}
...
public int getQuantitaFarmaco2() {
  ...
  public String getHtmlSelect(String selectName, int defaultKey) {
    ...
    while (i.hasNext()) {
      Prescrizioni thisOrg = (Prescrizioni) i.next();
      orgListSelect.addItem(
          thisOrg.getOrgId(),
          thisOrg.getIdPrescrizione());
    }
  ...
  public HashMap queryRecordCount(Connection db, TimeZone timeZone, HashMap events) throws SQLException {
    ...
    String sqlDate = ("data_prescrizione");

    createFilter(db, sqlFilter);

    sqlSelect.append(
        "SELECT " + sqlDate + " AS " + DatabaseUtils.addQuotes(db, "data_prescrizione")+ ", count(*) AS nocols " +
        "FROM prescrizioni2 o " +
        " where o.id_prescrizione >= 0 ");
  ...
  public void buildShortList(Connection db) throws SQLException {
    ...
    sqlSelect.append(
        "SELECT o.id_prescrizione, o.org_id, " +
        "o.id_farmacia, o.entered, o.enteredby, o.id_farmaco1, o.id_farmaco2, o.id_veterinario " +
        "FROM prescrizioni2 o " +
        " where o.id_prescrizione >= 0 ");
    ...
    rs = pst.executeQuery();
    while (rs.next()) {
      Prescrizioni thisOrg = new Prescrizioni();
      thisOrg.setIdPrescrizione(rs.getInt("id_prescrizione"));
      thisOrg.setOrgId(rs.getString("org_id"));
      thisOrg.setIdFarmacia(rs.getString("id_farmacia"));
      thisOrg.setEntered(rs.getTimestamp("entered"));
      thisOrg.setEnteredBy(rs.getInt("enteredby"));
      thisOrg.setIdFarmaco1(rs.getString("id_farmaco1"));
      thisOrg.setIdFarmaco2(rs.getString("id_farmaco2"));
      thisOrg.setIdVeterinario(rs.getString("id_veterinario"));
      this.add(thisOrg);
    }
    rs.close();
    pst.close();
  ...
  public void buildList(Connection db) throws SQLException {
    ...
    while (rs.next()) {
      Prescrizioni thisOrganization = this.getObject(rs);
   ...
   * @throws  SQLException  Description of the Exception
   */
  public Prescrizioni getObject(ResultSet rs) throws SQLException {
    Prescrizioni thisOrganization = new Prescrizioni(rs);
  ...
  public ResultSet queryListV(Connection db, PreparedStatement pst) throws SQLException {
    ...
    if (pagedListInfo != null) {
      ...
      pagedListInfo.setDefaultSort("o.id_prescrizione", null);
      ...
      sqlOrder.append(" ORDER BY o.id_prescrizione ");
    }
    ...
    sqlSelect.append(
        "o.*, asl.description" +
        "FROM prescrizioni2 o " +
        " left JOIN lookup_site_id asl ON (o.id_prescrizione = asl.code) " +
        " where o.id_prescrizione >= 0 " );
  ...
  public ResultSet queryList(Connection db, PreparedStatement pst) throws SQLException {
	    ...
	    if (pagedListInfo != null) {
	      ...
	      if (!pagedListInfo.getCurrentLetter().equals("")) {
	        pst = db.prepareStatement(
	            sqlCount.toString() +
	            sqlFilter.toString() +
	            " AND  " + DatabaseUtils.toLowerCase(db) + "(o.id_prescrizione) < ? ");
	        items = prepareFilter(pst);
	    ...
	    sqlSelect.append(
		        "o.*, asl.description, vet.cognome, vet.nome" +
		        " FROM prescrizioni2 o " +
  ...
  public ResultSet queryListVN(Connection db, PreparedStatement pst) throws SQLException {
	    ResultSet rs = null;
	    int items = -1;
	    ...
	    if (pagedListInfo != null) {
	      ...
	      pst = db.prepareStatement(
	          ...
	          sqlFilter.toString());
	      items = prepareFilter(pst);
	      logger.info("I select="+pst.toString());
	      rs = pst.executeQuery();
	      if (rs.next()) {
	        int maxRecords = rs.getInt("recordcount");
	        pagedListInfo.setMaxRecords(maxRecords);
	      }
	      logger.info("II select="+pst.toString());
	      rs.close();
	      pst.close();
	      ...
	      pagedListInfo.setDefaultSort("o.id_prescrizione", null);
	      //pagedListInfo.appendSqlTail(db, sqlOrder);
	      sqlOrder.append(" ORDER BY o.data_prescrizione LIMIT 10 OFFSET 0;");
	      
	            
	      //Optimize SQL Server Paging
	      //sqlFilter.append(" AND  o.org_id NOT IN (SELECT TOP 10 org_id FROM organization " + sqlOrder.toString());
	    } else {
	      sqlOrder.append(" ORDER BY o.data_prescrizione;");
	    }
	   ...
	   pst = db.prepareStatement(
	    ...
	    logger.info("III select="+pst.toString());
	    return rs;
	  }
  ...
  protected void createFilter(Connection db, StringBuffer sqlFilter) {
	    ...
	    if ((dataPrescrizione1 != null)) {
	        sqlFilter.append(" AND o.data_prescrizione >= ? ");
	      }
	    if ((dataPrescrizione2 != null)) {
	        sqlFilter.append(" AND o.data_prescrizione <= ? ");
	      }
	    if (siteId > -1) {
  ...
  protected void createFilterVN(Connection db, StringBuffer sqlFilter) {
	    if (sqlFilter == null) {
	      sqlFilter = new StringBuffer();
	    }
	    if ((dataPrescrizione1 != null)) {
	    ...
	    if ((dataPrescrizione2 != null)) {
	      ...
	      /*if ((dataPrescrizione1 != null)||(dataPrescrizione2 != null)||(!dataPrescrizione1.equals(""))||(!dataPrescrizione2.equals(""))) {
	    	  if((dataPrescrizione1 != null)&&(dataPrescrizione2 != null)||(!dataPrescrizione1.equals(""))&&(!dataPrescrizione2.equals(""))){
	    	      sqlFilter.append(" AND o.data_prescrizione >='" + dataPrescrizione1 + "' AND o.data_prescrizione <='" + dataPrescrizione2 + "' ");
	    	  }
	    	  else if((dataPrescrizione1 != null)||(!dataPrescrizione1.equals(""))){
		    	  sqlFilter.append(" AND o.data_prescrizione >='" + dataPrescrizione1 + "' ");
	    	  }else if((dataPrescrizione2 != null)||(!dataPrescrizione2.equals(""))){
		    	  sqlFilter.append(" AND o.data_prescrizione <='" + dataPrescrizione2 + "' ");
	    	  }
	        }*/
	    if (siteId > -1) {
	        sqlFilter.append(" AND  o.site_id= ? ");
	    ...
	    if (idVeterinario > -1) {
	     ...
	     /* if (siteId > -1) {
	      ...
	      if ((idVeterinario>-1)) {
	    	  sqlFilter.append(" AND o.id_veterinario =" + idVeterinario + " ");
	        }	   */
	  }
  /**
  ...
  protected void createFilterV(Connection db, StringBuffer sqlFilter) {
    ...
    if (idPrescrizione > -1) {
        sqlFilter.append("id_prescrizione, ");
      }
      if (orgId > -1) {
    	  sqlFilter.append("org_id, ");
        }
      if (idFarmacia > -1) {
    	  sqlFilter.append("id_farmacia, ");
        }
      if (idFarmaco1 > -1) {
    	  sqlFilter.append("id_farmaco1, ");
        }
      if (idFarmaco2 > -1) {
    	  sqlFilter.append("id_farmaco1, ");
        }
      if (idVeterinario > -1) {
    	  sqlFilter.append("id_veterinario, ");
        }
      if (quantitaFarmaco1 > -1) {
    	  sqlFilter.append("quantita_farmaco1, ");
        }
      if (quantitaFarmaco2 > -1) {
    	  sqlFilter.append("quantita_farmaco2, ");
        }
      if (note1 != null) {
    	  sqlFilter.append("note1, ");
        }
      if (note2 != null) {
    	  sqlFilter.append("note2, ");
        }
      if (dataPrescrizione1 != null) {
    	  sqlFilter.append("data_prescrizione, ");
        }
      if (siteId > -1) {
      ...
      if (dataDispensazione != null) {
          sqlFilter.append("data_dispensazione, ");
        }
  ...
  protected void buildResources(Connection db) throws SQLException {
    Iterator i = this.iterator();
    while (i.hasNext()) {
      Prescrizioni thisOrganization = (Prescrizioni) i.next();
  ...
  protected int prepareFilter(PreparedStatement pst) throws SQLException {
	    ...
	    if ((dataPrescrizione1 != null)) {
	        DatabaseUtils.setTimestamp(
	            pst, ++i, new Timestamp(dataPrescrizione1.getTime()));
	    }
	    if ((dataPrescrizione2 != null)) {
	        DatabaseUtils.setTimestamp(
	                pst, ++i, new Timestamp(dataPrescrizione2.getTime()));
	      }
	    if (siteId > -1) {
	        pst.setInt(++i, siteId);
	      }
   ...
   * @since                 1.2
   */
  protected int prepareFilterVN(PreparedStatement pst) throws SQLException {
    int i = 0;
    if ((dataPrescrizione1 != null)) {
        DatabaseUtils.setTimestamp(
            pst, ++i, new Timestamp(dataPrescrizione1.getTime()));
  ...
  public void delete(Connection db, ActionContext context, String baseFilePath, boolean forceDelete) throws SQLException {
    ...
    while (organizationIterator.hasNext()) {
      Prescrizioni thisOrganization = (Prescrizioni) organizationIterator.next();
  ...
  public static HashMap getParentAndLeafAccounts(Connection db, int typeId, boolean reciprocal) throws SQLException {
    ...
    PreparedStatement pst = db.prepareStatement(
        "SELECT id_prescrizione FROM prescrizioni2 WHERE trashed_date IS NULL ");
    ResultSet rs = pst.executeQuery();
    while (rs.next()) {
      Integer accountId = new Integer(rs.getInt("id_prescrizione"));
   ...
   */
  public Prescrizioni getOrgById(int id) {
    Prescrizioni result = null;
    Iterator iter = (Iterator) this.iterator();
    while (iter.hasNext()) {
      Prescrizioni org = (Prescrizioni) iter.next();
      if (org.getIdPrescrizione() == id) {
        result = org;
        break;
      }
    }
