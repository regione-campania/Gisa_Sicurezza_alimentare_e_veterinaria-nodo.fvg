 ...
 *      Exp $
 */
public class FarmaciList extends Vector implements SyncableList {

  private static Logger log = Logger.getLogger(org.aspcfs.modules.farmacosorveglianza.base.FarmaciList.class);
  static {
  ...
  public final static String tableName = "farmaci2";
  public final static String uniqueField = "id_farmaco";
...
public void setNote1(String tmp) {
  ...
  public String getHtmlSelect(String selectName, int defaultKey) {
    ...
    while (i.hasNext()) {
      Farmaci thisOrg = (Farmaci) i.next();
      orgListSelect.addItem(
          thisOrg.getIdFarmaco(),
          thisOrg.getNomeCommerciale());
    }
  ...
  public void buildShortList(Connection db) throws SQLException {
    ...
    sqlSelect.append(
        "SELECT o.id_farmaco, " +
        "o.nome_commerciale, o.principio_attivo, o.tipo_somministrazione, o.quantita, o.entered, o.enteredby" +
        "FROM farmaci2 o " +
        " where o.id_farmaco >= 0 ");
    ...
    rs = pst.executeQuery();
    while (rs.next()) {
      Farmaci thisOrg = new Farmaci();
      thisOrg.setIdFarmaco(rs.getInt("id_farmaco"));
      thisOrg.setNomeCommerciale(rs.getString("nome_commerciale"));
      thisOrg.setPrincipioAttivo(rs.getString("principio_attivo"));
      thisOrg.setTipoSomministrazione(rs.getString("tipo_somministrazione"));
      thisOrg.setQuantita(rs.getString("quantita"));
  ...
  public void buildList(Connection db, PreparedStatement pst) throws SQLException {
	    //PreparedStatement pst = null;
	    ResultSet rs = queryList2(db, pst);
	    while (rs.next()) {
	      Farmaci thisOrganization = this.getObject(rs);
...
public ResultSet queryList2(Connection db, PreparedStatement pst) throws SQLException {
	   
	  PreparedStatement st = null;
	    ResultSet rs = null;
	    StringBuffer sql = new StringBuffer();
	    sql.append("SELECT * FROM farmaci2 ORDER BY nome_commerciale");
	    st = db.prepareStatement(sql.toString());
	    
	    rs = st.executeQuery();
	    
	    if (rs.next()) {
	      nomeCommerciale = rs.getString(3);
	    }
	   return rs;
	  }
   ...
   * @throws  SQLException  Description of the Exception
   */
  public Farmaci getObject(ResultSet rs) throws SQLException {
    Farmaci thisOrganization = new Farmaci(rs);
	    ...
	    if (pagedListInfo != null) {
	      ...
	      if (!pagedListInfo.getCurrentLetter().equals("")) {
	        pst = db.prepareStatement(
	            sqlCount.toString() +
	            sqlFilter.toString() +
	            " AND  " + DatabaseUtils.toLowerCase(db) + "(o.nome_commerciale) < ? ");
	        items = prepareFilter(pst);
	      ...
	      pagedListInfo.setDefaultSort("o.nome_commerciale", null);
	      ...
	      sqlOrder.append(" ORDER BY o.nome_commerciale ");
	    }
   ...
   * @throws  SQLException  Description of the Exception
   */
  public ResultSet queryListV(Connection db, PreparedStatement pst) throws SQLException {
    ResultSet rs = null;
    int items = -1;
    ...
    if (pagedListInfo != null) {
      ...
      //Determine column to sort by
      pagedListInfo.setDefaultSort("o.id_farmaco", null);
      //pagedListInfo.appendSqlTail(db, sqlOrder);
      sqlOrder.append(" ORDER BY o.nome_commerciale LIMIT 10 OFFSET 0;");
  ...
  protected void createFilter(Connection db, StringBuffer sqlFilter) {
	    ...
	    if ((nomeCommerciale != null) &&(!nomeCommerciale.equals(""))) {
	    	  sqlFilter.append(" AND  o.nome_commerciale like '%" + nomeCommerciale + "%' ");
	        }
	    
	    if (idConfezione > -1) {
	    	  sqlFilter.append(" AND  o.id_confezione = ?");
	        }
	    ...
	    if (excludeIds != null) {
	      sqlFilter.append(" AND  o.id_farmaco NOT IN (" + excludeIds + ") ");
	    }

	    if (types != null) {
	      sqlFilter.append(
	          " AND  o.id_farmaco IN (select atl.org_id from account_type_levels atl where atl.type_id IN (" + types + ")) ");
	    }
	    ...
	    if (!showMyCompany) {
	      sqlFilter.append(" AND  o.id_farmaco >= 0 ");
	    }
	    ...
	    if (revenueOwnerId > -1) {
	      sqlFilter.append(
	          " AND  o.id_farmaco in (SELECT org_id from revenue WHERE owner = ?) ");
	    }
	    ...
	    if (firstName != null) {
	      if (firstName.indexOf("%") >= 0) {
	        sqlFilter.append(" AND  EXISTS (select contact_id from contact c where " + DatabaseUtils.toLowerCase(db) + "(c.namefirst) LIKE ? AND c.org_id = o.id_farmaco) ");
	      } else {
	        sqlFilter.append(" AND  EXISTS (select contact_id from contact c where " + DatabaseUtils.toLowerCase(db) + "(c.namefirst) = ? AND c.org_id = o.id_farmaco) ");
	      }
	    }

	    if (lastName != null) {
	      if (lastName.indexOf("%") >= 0) {
	        sqlFilter.append(" AND  EXISTS (select contact_id from contact c where " + DatabaseUtils.toLowerCase(db) + "(c.namelast) LIKE ? AND c.org_id = o.id_farmaco) ");
	      } else {
	        sqlFilter.append(" AND  EXISTS (select contact_id from contact c where " + DatabaseUtils.toLowerCase(db) + "(c.namelast) = ? AND c.org_id = o.id_farmaco) ");
	      }
	    }

	    if (contactPhoneNumber != null) {
	      if (contactPhoneNumber.indexOf("%") >= 0) {
	        sqlFilter.append(" AND  EXISTS (select contact_id from contact c where c.org_id = o.id_farmaco AND c.contact_id IN (select cp.contact_id from contact_phone cp where " + DatabaseUtils.toLowerCase(db) + "(cp.number) LIKE ?)) ");
	      } else {
	        sqlFilter.append(" AND  EXISTS (select contact_id from contact c where c.org_id = o.id_farmaco AND c.contact_id IN (select cp.contact_id from contact_phone cp where " + DatabaseUtils.toLowerCase(db) + "(cp.number) = ?)) ");
	      }
	    }

	    if (contactCity != null && !"-1".equals(contactCity)) {
	      if (contactCity.indexOf("%") >= 0) {
	        sqlFilter.append(" AND  EXISTS (select contact_id from contact c where c.org_id = o.id_farmaco AND c.contact_id IN (select ca.contact_id from contact_address ca where " + DatabaseUtils.toLowerCase(db) + "(ca.city) LIKE ?)) ");
	      } else {
	        sqlFilter.append(" AND  EXISTS (select contact_id from contact c where c.org_id = o.id_farmaco AND c.contact_id IN (select ca.contact_id from contact_address ca where " + DatabaseUtils.toLowerCase(db) + "(ca.city) = ?)) ");
	      }
	    }

	    if (contactState != null && !"-1".equals(contactState)) {
	      if (contactState.indexOf("%") >= 0) {
	        sqlFilter.append(" AND  EXISTS (select contact_id from contact c where c.org_id = o.id_farmaco AND c.contact_id IN (select ca.contact_id from contact_address ca where " + DatabaseUtils.toLowerCase(db) + "(ca.state) LIKE ?)) ");
	      } else {
	        sqlFilter.append(" AND  EXISTS (select contact_id from contact c where c.org_id = o.id_farmaco AND c.contact_id IN (select ca.contact_id from contact_address ca where " + DatabaseUtils.toLowerCase(db) + "(ca.state) = ?)) ");
	      }
	    }
	    if (contactCountry != null && !"-1".equals(contactCountry)) {
	      sqlFilter.append(" AND  EXISTS (select contact_id from contact c where c.org_id = o.id_farmaco AND c.contact_id IN (select ca.contact_id from contact_address ca where " + DatabaseUtils.toLowerCase(db) + "(ca.country) = ?)) ");
	    }
	    if (typeId > 0) {
	      sqlFilter.append(
	          " AND  o.id_farmaco IN (select atl.org_id from account_type_levels atl where atl.type_id = ?) ");
	    }

	    if (projectId > 0) {
	      sqlFilter.append(
	          " AND  o.id_farmaco IN (SELECT org_id FROM project_accounts WHERE project_id = ?) ");
	    }
	 ...
	 if (postalCode != null) {
	      if (postalCode.indexOf("%") >= 0) {
	        sqlFilter.append(
	            " AND  o.id_farmaco IN (SELECT org_id FROM organization_address " +
	    ...
	    if (assetSerialNumber != null) {
	      sqlFilter.append(
	          " AND  o.id_farmaco IN (SELECT a.account_id FROM asset a " +
  ...
  protected void createFilterV(Connection db, StringBuffer sqlFilter) {
    ...
    /*if (idFarmacia > -1) {
    	sqlFilter.append(" AND o.id_farmaco=? ");
      }*/
      if ((nomeCommerciale != null) ||(!nomeCommerciale.equals(""))) {
    	  sqlFilter.append(" AND  o.nome_commerciale like '%" + nomeCommerciale + "%' ");
        }
       if (idConfezione > -1) {
    	  sqlFilter.append(" AND  o.id_confezione="+idConfezione+" ");
        }
       /*
      if (indirizzo != null) {
      ...
      if ((citta != null)||(!citta.equals(""))) {
    	  sqlFilter.append(" AND  o.citta like '%" + citta + "%' ");
        }/*
      if (provincia != null) {
  ...
  protected void buildResources(Connection db) throws SQLException {
    Iterator i = this.iterator();
    while (i.hasNext()) {
      Farmaci thisOrganization = (Farmaci) i.next();
  ...
  public void delete(Connection db, ActionContext context, String baseFilePath, boolean forceDelete) throws SQLException {
    ...
    while (organizationIterator.hasNext()) {
      Farmaci thisOrganization = (Farmaci) organizationIterator.next();
  ...
  public static HashMap getParentAndLeafAccounts(Connection db, int typeId, boolean reciprocal) throws SQLException {
    ...
    PreparedStatement pst = db.prepareStatement(
        "SELECT id_farmaco FROM farmaci2 WHERE trashed_date IS NULL ");
    ResultSet rs = pst.executeQuery();
    while (rs.next()) {
      Integer accountId = new Integer(rs.getInt("id_farmaco"));
   ...
   */
  public Farmaci getOrgById(int id) {
    Farmaci result = null;
    Iterator iter = (Iterator) this.iterator();
    while (iter.hasNext()) {
      Farmaci org = (Farmaci) iter.next();
      if (org.getIdFarmaco() == id) {
        result = org;
        break;
      }
    }
