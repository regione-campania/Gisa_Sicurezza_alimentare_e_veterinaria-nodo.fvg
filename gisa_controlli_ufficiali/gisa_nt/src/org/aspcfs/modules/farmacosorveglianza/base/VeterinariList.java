 ...
 *      Exp $
 */
public class VeterinariList extends Vector implements SyncableList {

  private static Logger log = Logger.getLogger(org.aspcfs.modules.farmacosorveglianza.base.VeterinariList.class);
  static {
  ...
  public final static String tableName = "veterinari2";
  public final static String uniqueField = "id_veterinario";
...
public void setNome(String tmp) {
  ...
  public String getHtmlSelect(String selectName, int defaultKey) {
    ...
    while (i.hasNext()) {
      Veterinari thisOrg = (Veterinari) i.next();
      orgListSelect.addItem(
          thisOrg.getIdVeterinario(),
          thisOrg.getCognome());
    }
  ...
  public void buildShortList(Connection db) throws SQLException {
    ...
    sqlSelect.append(
        "SELECT o.id_veterinario, " +
        "o.cognome, o.nome, o.citta, o.entered, o.enteredby" +
        "FROM veterinari2 o " +
        " where o.id_veterinario >= 0 ");
    ...
    rs = pst.executeQuery();
    while (rs.next()) {
      Veterinari thisOrg = new Veterinari();
      thisOrg.setIdVeterinario(rs.getInt("id_veterinario"));
      thisOrg.setCognome(rs.getString("cognome"));
      thisOrg.setNome(rs.getString("nome"));
      thisOrg.setCitta(rs.getString("citta"));
  ...
  public void buildList(Connection db) throws SQLException {
    ...
    while (rs.next()) {
      Veterinari thisOrganization = this.getObject(rs);
   ...
   * @throws  SQLException  Description of the Exception
   */
  public Veterinari getObject(ResultSet rs) throws SQLException {
    Veterinari thisOrganization = new Veterinari(rs);
  ...
  public ResultSet queryList(Connection db, PreparedStatement pst) throws SQLException {
	    ...
	    if (pagedListInfo != null) {
	      ...
	      pagedListInfo.setDefaultSort("o.cognome", null);
  ...
  public ResultSet queryListV(Connection db, PreparedStatement pst) throws SQLException {
    ...
    if (pagedListInfo != null) {
      ...
      //Determine column to sort by
      pagedListInfo.setDefaultSort("o.id_veterinario", null);
      //pagedListInfo.appendSqlTail(db, sqlOrder);
      sqlOrder.append(" ORDER BY o.cognome LIMIT 10 OFFSET 0;");
  ...
  public ResultSet queryList2(Connection db, PreparedStatement pst) throws SQLException {
	  ...
	  PreparedStatement st = null;
	    ...
	    sql.append("SELECT * FROM veterinari2 ORDER BY cognome");
  ...
  protected void createFilter(Connection db, StringBuffer sqlFilter) {
	    ...
	    if ((cognome != null) &&(!cognome.equals(""))) {
	    	  sqlFilter.append(" AND  o.cognome like '%" + cognome + "%' ");
	        }
	    ...
	    if (excludeIds != null) {
	      sqlFilter.append(" AND  o.id_veterinario NOT IN (" + excludeIds + ") ");
	    }

	    if (types != null) {
	      sqlFilter.append(
	          " AND  o.id_veterinario IN (select atl.org_id from account_type_levels atl where atl.type_id IN (" + types + ")) ");
	    }
	    ...
	    if (!showMyCompany) {
	      sqlFilter.append(" AND  o.id_veterinario >= 0 ");
	    }
	    ...
	    if (revenueOwnerId > -1) {
	      sqlFilter.append(
	          " AND  o.id_veterinario in (SELECT org_id from revenue WHERE owner = ?) ");
	    }
	    ...
	    if (firstName != null) {
	      if (firstName.indexOf("%") >= 0) {
	        sqlFilter.append(" AND  EXISTS (select contact_id from contact c where " + DatabaseUtils.toLowerCase(db) + "(c.namefirst) LIKE ? AND c.org_id = o.id_veterinario) ");
	      } else {
	        sqlFilter.append(" AND  EXISTS (select contact_id from contact c where " + DatabaseUtils.toLowerCase(db) + "(c.namefirst) = ? AND c.org_id = o.id_veterinario) ");
	      }
	    }

	    if (lastName != null) {
	      if (lastName.indexOf("%") >= 0) {
	        sqlFilter.append(" AND  EXISTS (select contact_id from contact c where " + DatabaseUtils.toLowerCase(db) + "(c.namelast) LIKE ? AND c.org_id = o.id_veterinario) ");
	      } else {
	        sqlFilter.append(" AND  EXISTS (select contact_id from contact c where " + DatabaseUtils.toLowerCase(db) + "(c.namelast) = ? AND c.org_id = o.id_veterinario) ");
	      }
	    }

	    if (contactPhoneNumber != null) {
	      if (contactPhoneNumber.indexOf("%") >= 0) {
	        sqlFilter.append(" AND  EXISTS (select contact_id from contact c where c.org_id = o.id_veterinario AND c.contact_id IN (select cp.contact_id from contact_phone cp where " + DatabaseUtils.toLowerCase(db) + "(cp.number) LIKE ?)) ");
	      } else {
	        sqlFilter.append(" AND  EXISTS (select contact_id from contact c where c.org_id = o.id_veterinario AND c.contact_id IN (select cp.contact_id from contact_phone cp where " + DatabaseUtils.toLowerCase(db) + "(cp.number) = ?)) ");
	      }
	    }

	    if (contactCity != null && !"-1".equals(contactCity)) {
	      if (contactCity.indexOf("%") >= 0) {
	        sqlFilter.append(" AND  EXISTS (select contact_id from contact c where c.org_id = o.id_veterinario AND c.contact_id IN (select ca.contact_id from contact_address ca where " + DatabaseUtils.toLowerCase(db) + "(ca.city) LIKE ?)) ");
	      } else {
	        sqlFilter.append(" AND  EXISTS (select contact_id from contact c where c.org_id = o.id_veterinario AND c.contact_id IN (select ca.contact_id from contact_address ca where " + DatabaseUtils.toLowerCase(db) + "(ca.city) = ?)) ");
	      }
	    }

	    if (contactState != null && !"-1".equals(contactState)) {
	      if (contactState.indexOf("%") >= 0) {
	        sqlFilter.append(" AND  EXISTS (select contact_id from contact c where c.org_id = o.id_veterinario AND c.contact_id IN (select ca.contact_id from contact_address ca where " + DatabaseUtils.toLowerCase(db) + "(ca.state) LIKE ?)) ");
	      } else {
	        sqlFilter.append(" AND  EXISTS (select contact_id from contact c where c.org_id = o.id_veterinario AND c.contact_id IN (select ca.contact_id from contact_address ca where " + DatabaseUtils.toLowerCase(db) + "(ca.state) = ?)) ");
	      }
	    }
	    if (contactCountry != null && !"-1".equals(contactCountry)) {
	      sqlFilter.append(" AND  EXISTS (select contact_id from contact c where c.org_id = o.id_veterinario AND c.contact_id IN (select ca.contact_id from contact_address ca where " + DatabaseUtils.toLowerCase(db) + "(ca.country) = ?)) ");
	    }
	    if (typeId > 0) {
	      sqlFilter.append(
	          " AND  o.id_veterinario IN (select atl.org_id from account_type_levels atl where atl.type_id = ?) ");
	    }

	    if (projectId > 0) {
	      sqlFilter.append(
	          " AND  o.id_veterinario IN (SELECT org_id FROM project_accounts WHERE project_id = ?) ");
	    }
	 ...
	 if (postalCode != null) {
	      if (postalCode.indexOf("%") >= 0) {
	        sqlFilter.append(
	            " AND  o.id_veterinario IN (SELECT org_id FROM organization_address " +
	    ...
	    if (assetSerialNumber != null) {
	      sqlFilter.append(
	          " AND  o.id_veterinario IN (SELECT a.account_id FROM asset a " +
  ...
  protected void createFilterV(Connection db, StringBuffer sqlFilter) {
    ...
    /*if (idVeterinaria > -1) {
    	sqlFilter.append(" AND o.id_veterinario=? ");
      }*/
      if ((cognome != null) ||(!cognome.equals(""))) {
    	  sqlFilter.append(" AND  o.cognome like '%" + cognome + "%' ");
        }
      ...
      /* if (siteId > -1) {
    	  sqlFilter.append(" AND  o.site_id="+siteId+" ");
        }
      if (indirizzo != null) {
  ...
  protected void buildResources(Connection db) throws SQLException {
    Iterator i = this.iterator();
    while (i.hasNext()) {
      Veterinari thisOrganization = (Veterinari) i.next();
  ...
  public void delete(Connection db, ActionContext context, String baseFilePath, boolean forceDelete) throws SQLException {
    ...
    while (organizationIterator.hasNext()) {
      Veterinari thisOrganization = (Veterinari) organizationIterator.next();
  ...
  public static HashMap getParentAndLeafAccounts(Connection db, int typeId, boolean reciprocal) throws SQLException {
    ...
    PreparedStatement pst = db.prepareStatement(
        "SELECT id_veterinario FROM veterinari2 WHERE trashed_date IS NULL ");
    ResultSet rs = pst.executeQuery();
    while (rs.next()) {
      Integer accountId = new Integer(rs.getInt("id_veterinario"));
   ...
   */
  public Veterinari getOrgById(int id) {
    Veterinari result = null;
    Iterator iter = (Iterator) this.iterator();
    while (iter.hasNext()) {
      Veterinari org = (Veterinari) iter.next();
      if (org.getIdVeterinario() == id) {
        result = org;
        break;
      }
    }
