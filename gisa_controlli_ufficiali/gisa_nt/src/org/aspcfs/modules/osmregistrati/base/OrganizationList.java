...
//  private String contactOtherState = null;
  ...
  private boolean flagRicercaOsmAssociabiliAdAllevamento;
  
	public boolean isFlagRicercaOsmAssociabiliAdAllevamento() {
		return flagRicercaOsmAssociabiliAdAllevamento;
	}
	
	public void setFlagRicercaOsmAssociabiliAdAllevamento(
			boolean flagRicercaOsmAssociabiliAdAllevamento) {
		this.flagRicercaOsmAssociabiliAdAllevamento = flagRicercaOsmAssociabiliAdAllevamento;
	}
...
//    this.tipologia = tmp;
  ...
  public ResultSet queryList(Connection db, PreparedStatement pst) throws SQLException {
    ...
    if( codiceAllerta != null){
    	...
    	{
    sqlSelect.append(
        "o.*, azosm.id AS id_associazione_azienda_zootecnica, " +
	...
	private void and(StringBuffer sqlFilter)
	{
		if( attivita > -1 )
		{
			sqlFilter.append( " AND ssa.id_attivita = ? " );
		}
	...
	private String join()
	{
		...
		if( ( attivita > -1 ) )
		{
			ret = " LEFT JOIN elenco_attivita_osm_reg ssa ON (o.org_id = ssa.org_id) ";
		}
...
/**
  ...
  protected void createFilter(Connection db, StringBuffer sqlFilter) {
    ...
    // associazione OSM - azienda zootecnica
    if (flagRicercaOsmAssociabiliAdAllevamento == true) {
        sqlFilter.append(" AND  o.org_id NOT IN (SELECT id_osm FROM aziendezootecniche_osm)");
    }
    ...
    if (types != null) {
      sqlFilter.append(
          " AND  o.org_id IN (select atl.org_id from stabilimenti_type_levels atl where atl.type_id IN (" + types + ")) ");
    }
   ...
   if (tipologia > -1) {
    ...
    if (city != null && !"-1".equals(city) && !"%-1%".equals(city)) {
      if (city.indexOf("%") >= 0) {
        sqlFilter.append(
            " AND  o.org_id IN (SELECT org_id FROM organization_address " +
            " where " + DatabaseUtils.toLowerCase(db, "city") + " LIKE ? " +
            " AND  city IS NOT NULL and address_type=5) ");
      } else {
        sqlFilter.append(
            " AND  o.org_id IN (SELECT org_id FROM organization_address " +
