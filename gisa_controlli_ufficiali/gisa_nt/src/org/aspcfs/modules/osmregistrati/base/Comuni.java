...
public void setCodice(String codice) {
  ...
  public ArrayList<Comuni> buildList(Connection db, int asl) throws SQLException {
	    ...
	    if(asl!=-1)
	    sql.append("SELECT codice,comune FROM comuni c , lookup_site_id asl WHERE c.codiceistatasl =asl.codiceistat and asl.code = ? order by comune");
	    else
	    	sql.append("SELECT codice,comune FROM comuni c , lookup_site_id asl WHERE c.codiceistatasl =asl.codiceistat order by comune");
