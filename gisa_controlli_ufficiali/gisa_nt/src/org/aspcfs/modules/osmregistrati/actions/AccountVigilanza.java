...
public final class AccountVigilanza extends CFSModule {
  ...
  public String executeCommandReopenTicket(ActionContext context) {
    if (!hasPermission(context, "osmregistrati-osmregistrati-vigilanza-edit")) {
      return ("PermissionError");
    }
    int resultCount = -1;
    Connection db = null;
  ...
  public String executeCommandAddTicket(ActionContext context) {
    if (!hasPermission(context, "osmregistrati-osmregistrati-vigilanza-add")) {
      return ("PermissionError");
    }
    Connection db = null;
    Ticket newTic = null;
  ...
  public String executeCommandAggiornaOsm(ActionContext context) {
  ...
  public String executeCommandTicketDetails(ActionContext context) {
    ...
    try {
      ...
      context.getRequest().setAttribute("tipologia",801);
  ...
  public String executeCommandConfirmDelete(ActionContext context) {
    if (!hasPermission(context, "osmregistrati-osmregistrati-vigilanza-delete")) {
      return ("PermissionError");
    }
    Connection db = null;
    //Parameters
    ...
    try {
      ...
      htmlDialog.addButton(
          systemStatus.getLabel("global.button.delete"), "javascript:window.location.href='OsmRegistratiVigilanza.do?command=DeleteTicket&id=" + id + "&orgId=" + orgId + "&forceDelete=true" + RequestUtils.addLinkParams(
  ...
  public String executeCommandDeleteTicket(ActionContext context) {
    ...
    try {
      ...
      if (recordDeleted) {
        ...
        context.getRequest().setAttribute("OrgDetails", newOrg);
        context.getRequest().setAttribute(
            "refreshUrl", "OsmRegistrati.do?command=ViewVigilanza&orgId=" + orgId +
	    ...
	    try {
	      ...
	      boolean modificato = false;
			...
			if (modificato){
				...
				org.aspcfs.modules.osmregistrati.actions.Accounts sa = new org.aspcfs.modules.osmregistrati.actions.Accounts();
...
//			Ticket oldTicket = new Ticket(db, thisTicket.getId());
 ...
 tamponiList.buildListControlli(db, pasId, ticketId);
      ...
      while(itAudit.hasNext()){
		    	  ...
		    	  if(auditTemp.getLivelloRischio()==-1){
		    		  flag=2;
