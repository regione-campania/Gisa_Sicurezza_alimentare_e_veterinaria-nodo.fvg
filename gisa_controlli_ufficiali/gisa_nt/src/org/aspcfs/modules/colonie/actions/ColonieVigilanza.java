 ...
 */
public final class ColonieVigilanza extends CFSModule {
	...
	public String executeCommandAdd(ActionContext context) {
		if (!hasPermission(context, "colonie-vigilanza-add")) {
		...
		try {
			...
			context.getRequest().setAttribute("tipologia", 16);
			...
			addModuleBean(context, "Colonie Add CU", "Add CU");
		} catch (Exception e) {
			e.printStackTrace();
			context.getRequest().setAttribute("Error", e);
			return ("SystemError");
	...
	public String executeCommandInsert(ActionContext context) throws SQLException, ParseException {
		...
		try
		{
			...
			addModuleBean(context, "Colonie CU", "Colonie CU");
			...
			if (recordInserted && isValid) {
				...
				DbiBdu.inserisci_controlo_colonia(context, db, newTic.getOrgId(), newTic.getId(), newTic.getAssignedDate(), newTic.getTipoControllo());
	...
	public String executeCommandModifyTicket(ActionContext context) {
		if (!hasPermission(context, "colonie-vigilanza-edit")) {
			return ("PermissionError");
		}
		Connection db = null;
		Ticket newTic = null;
...
//			}
	...
	public String executeCommandUpdateTicket(ActionContext context) {
		...
		try {
			...
			DbiBdu.aggiorna_controlo_colonia(context, db, newTic.getOrgId(), newTic.getId(), newTic.getAssignedDate(), newTic.getTipoControllo(), newTic.getClosed(), newTic.getTrashedDate());
	...
	public String executeCommandConfirmDelete(ActionContext context) {
		if (!hasPermission(context, "colonie-vigilanza-delete")) {
			return ("PermissionError");
		}
		...
		{	
			db = this.getConnection(context);
			org.aspcf.modules.controlliufficiali.action.AccountVigilanza controlliGeneric = new org.aspcf.modules.controlliufficiali.action.AccountVigilanza();
			context.getRequest().setAttribute("url_confirm","ColonieVigilanza.do?command=DeleteTicket");
	...
	public String executeCommandDeleteTicket(ActionContext context) {
		...
		try {
			...
			DbiBdu.cancella_controlo_colonia(context, db, thisTicket.getOrgId(), thisTicket.getId(), thisTicket.getAssignedDate(), thisTicket.getTipoControllo(), thisTicket.getClosed(), thisTicket.getTrashedDate());


			context.getRequest().setAttribute("refreshUrl","ColonieVigilanza.do?command=ViewVigilanza&orgId=" + orgId);
	...
	public String executeCommandReopenTicket(ActionContext context) {
		...
		try {
			...
			DbiBdu.apri_controlo_colonia(context, db, thisTicket.getOrgId(), thisTicket.getId(), thisTicket.getAssignedDate(), thisTicket.getTipoControllo(), thisTicket.getClosed(), thisTicket.getTrashedDate());
	...
	public String executeCommandTicketDetails(ActionContext context) {
		...
		try {
			...
			if (context.getParameter("msgbdu")!=null)
			{
				context.getRequest().setAttribute("msgbdu", context.getParameter("msgbdu"));
			}
			...
			addModuleBean(context, "Colonie Details CU", "Details CU");	
	...
	public String executeCommandViewVigilanza(ActionContext context) {
		...
		ticketListInfo.setLink(
				"ColonieVigilanza.do?command=ViewVigilanza&orgId=" + passedId);
		...
		try {
			...
			context.getRequest().setAttribute("OrgDetails", newOrg);
			addModuleBean(context, "View Colonie Accounts", "Accounts View");
		} catch (Exception errorMessage) {
			context.getRequest().setAttribute("Error", errorMessage);
			return ("SystemError");
		} finally {
	...
	public String executeCommandDettaglio(ActionContext context) {
		...
		addModuleBean(context, "ViewColonieTickets", "View Tickets");
	...
	public String executeCommandChiudi(ActionContext context)
	{
		...
		try {
			...
			if(flag==0){
				thisTicket.setModifiedBy(getUserId(context));
				resultCount = thisTicket.chiudi(db);
				DbiBdu.chiudi_controlo_colonia(context, db, thisTicket.getOrgId(), thisTicket.getId(), thisTicket.getAssignedDate(), thisTicket.getTipoControllo(), thisTicket.getClosed(), thisTicket.getTrashedDate());
