...
public class SuapMasterListNormaList extends Vector  {
	...
	public void buildList(Connection db) throws SQLException {
		...
		while (rs.next()) {
			SuapMasterListNorma thisNorma = this.getObject(rs);
			this.add(thisNorma);
	...
	public ResultSet queryList(Connection db, PreparedStatement pst) throws SQLException {
		...
		sqlSelect.append("SELECT DISTINCT "+MasterListImportUtil.TAB_NORMA+".* from "+MasterListImportUtil.TAB_NORMA
				...
				+ " where 1=1 "); 
		sqlOrder.append(" order by " + MasterListImportUtil.TAB_NORMA+ ".description ");
	...
	protected void createFilter(Connection db, StringBuffer sqlFilter) {
		...
		if(norma!=null && !"".equals(norma))
		{
			sqlFilter.append(" and "+MasterListImportUtil.TAB_NORMA+".norma ilike ?");
		}
		if(rev>0)
		{
	...
	public SuapMasterListNorma getObject(ResultSet rs) throws SQLException {
		SuapMasterListNorma thisNorma = new SuapMasterListNorma(rs);
		return thisNorma;
	}
