package org.aspcfs.modules.nonconformita.base;
...
public class Ticket extends org.aspcfs.modules.troubletickets.base.Ticket {
	...
	protected ArrayList<ElementoNonConformita> non_conformita_formali = new ArrayList<ElementoNonConformita>();
	protected ArrayList<ElementoNonConformita> non_conformita_significative = new ArrayList<ElementoNonConformita>();
	...
	public ArrayList<ElementoNonConformita> getNon_conformita_formali() {
		return non_conformita_formali;
	}

	public ArrayList<ElementoNonConformita> getNon_conformita_significative() {
	...
	public Ticket(ResultSet rs, Connection db) throws SQLException {
		...
		this.set_NonConformitaFormali(db, cu.getTipoCampione(), cu.getAssignedDate());
		this.setNon_conformita_gravi(db, cu.getTipoCampione(), cu.getAssignedDate());
		this.setNon_conformita_significative(db, cu.getTipoCampione(), cu.getAssignedDate());
	}
	...
	public void queryRecord(Connection db, int id) throws SQLException {
		...
		String sql = "SELECT  t.*,tic.provvedimenti_prescrittivi as tipo_controllo_uff,tic.chiusura_attesa_esito , tic.assigned_date as data_controllo, o.asl_rif AS orgsiteid, o.tipologia_operatore as tipologia_operatore FROM ticket t LEFT JOIN ricerche_anagrafiche_old_materializzata o ON ((t.org_id = o.riferimento_id and o.riferimento_id_nome_tab = 'organization') or (t.id_stabilimento = o.riferimento_id and o.riferimento_id_nome_tab = 'opu_stabilimento') or (t.id_apiario = o.riferimento_id and o.riferimento_id_nome_tab = 'apicoltura_apiari') or (t.alt_id = o.riferimento_id and o.riferimento_id_nome_tab = 'suap_ric_scia_stabilimento') or (t.alt_id = o.riferimento_id and o.riferimento_id_nome_tab = 'sintesis_stabilimento')) INNER JOIN ticket tic ON ((tic.tipologia = 3)and(tic.id_controllo_ufficiale = t.id_controllo_ufficiale)) "
				+ " where t.ticketid = ? AND t.tipologia in(8,10)";
	...
	public void queryRecordTrasporti(Connection db, int id) throws SQLException {
		...
		PreparedStatement pst = db
				.prepareStatement("SELECT t.*,tic.provvedimenti_prescrittivi as tipo_controllo_uff,tic.assigned_date as dataInizioControllo,  "
						+ "o.name AS orgname, "
						...
						" where t.ticketid = ? AND t.tipologia = 8");
		pst.setInt(1, id);
		ResultSet rs = pst.executeQuery();
		org.aspcfs.modules.vigilanza.base.Ticket cu = new org.aspcfs.modules.vigilanza.base.Ticket();
	...
	public boolean insertTipiNonConformita(Connection db, String nc, int tipologia, String note, String idSottoattivita, String idLineaNc, String idSottoattivitaBenessereMacellazione, String idSottoattivitaBenessereTrasporto, String idOperatoreMercato)
			throws SQLException {
		...
		/**
		 * AGGANCIO LE SOTTOATTIVITA ALLA NC INSERITA TRAMITE IDENTIFICATIVO NC
		 * E ID_NONCONFORMITA
		 */
	...
	public synchronized boolean insert(Connection db, ActionContext context) throws SQLException {
		...
		try {
			...
			sql.append("nc_formali_valutazione,nc_significative_valutazione, nc_gravi_valutazione,flag_posticipato,flag_campione_non_conforme, ");
			...
			sql.append("?, ?, ?, ?, " + "8, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) RETURNING ticketid "); // ho
																									// aggiunto
																									// 2
																									// punti
																									// interrogativi
			...
			/*
			 * pezzo aggiunto da d.dauria pst.setTimestamp(++i,new Timestamp(
			 * System.currentTimeMillis() )); pst.setString(++i, "x");
			...
			db.prepareStatement(
					"UPDATE TICKET set identificativo = '" + asl + idControlloUfficiale + "' || trim(to_char( " + id
							+ ", '" + DatabaseUtils.getPaddedFromId(id) + "' )) where ticketid =" + this.getId())
					.execute();
	...
	protected void buildRecord(ResultSet rs) throws SQLException {
		...
		if (altId >= 100000000 && altId <=119999999)
	...
	public int update(Connection db, boolean override) throws SQLException {
		...
		PreparedStatement pst = null;
		StringBuffer sql = new StringBuffer();
		sql.append("UPDATE ticket " + "SET link_contract_id = ?, link_asset_id = ?, department_code = ?, "
				+ "pri_code = ?, scode = ?, " + "cat_code = ?, assigned_to = ?, "
		...
		sql.append("solution = ?, custom_data = ?, location = ?, assigned_date = ?, assigned_date_timezone = ?, "
	...
	public int chiudi(Connection db) throws SQLException {
		...
		try {
			...
			PreparedStatement pst = null;
			String sql = "UPDATE ticket " + "SET closed = ?, modified = " + DatabaseUtils.getCurrentTimestamp(db)
	...
	public boolean delete(Connection db, String baseFilePath) throws SQLException {
		...
		try {
			...
			// Delete the ticket
			pst = db.prepareStatement("UPDATE ticket SET trashed_date = now(), modified=now() , modifiedby = ? WHERE ticketid = ? OR id_controllo_ufficiale = ? OR id_nonconformita = ?;");
	...
	public void updateNonConformita(Connection db, String[] nc_formali, String[] nc_significative, String[] nc_gravi)
			throws SQLException {
	...
	public void set_NonConformitaFormali(Connection db, int tipoControllo, Timestamp dataInizioControllo)
			throws SQLException {
		...
		if (tipoControllo == 4 || (tipoControllo==3 && dataInizioControllo.after(java.sql.Timestamp.valueOf(org.aspcf.modules.controlliufficiali.base.ApplicationProperties.getProperty("TIMESTAMP_NUOVA_GESTIONE_OGGETTO_DEL_CONTROLLO_AUDIT"))))) {
			if (haveOggettoControllo == true) {
				...
				pst_ver.setInt(1, id);
				pst_ver.setInt(2, ElementoNonConformita.NC_FORMALI);
		...
		if (non_conformita_formali.isEmpty()) {
			puntiFormali = "0";
			ElementoNonConformita e = new ElementoNonConformita(id, 1, 1, "", -1);
			e.setDescrizione_nc("");
			e.setId_nc(-1);
			e.setNote("INSERIRE QUI LA DESCRIZIONE PER LA SINGOLA NON CONFORMITA");
			non_conformita_formali.add(e);
		} else {
			...
			puntiFormali = "" + non_conformita_formali.size() * p.getPuntiformali();
		}
		if (ignoraPunteggi) // ACQUE DI RETE NON CONFORMITA': NON CALCOLARE
							// PUNTEGGI
		{
			puntiFormali = "0";
		}
	...
	public void set_NonConformitaFormaliSoa(Connection db) throws SQLException {

		for (int i = 0; i < non_conformita_formali.size(); i++)
			non_conformita_formali.remove(non_conformita_formali.get(i));
		String sql = "";
		if (tipoControllo == 4) {
			sql = "select idticket,id_non_conformita,lnc.description as descrizione ,note,tipologia, id_linea_nc, id_non_conformita_benessere_macellazione, id_non_conformita_benessere_trasporto, id_operatore_mercato from salvataggio_nc_note join lookup_ispezione_soa lnc on ( id_non_conformita=lnc.code) where idticket = ? and tipologia = ? order by tipologia";
		} else {
			sql = "select idticket,id_non_conformita,lnc.description as descrizione ,note,tipologia, id_linea_nc, id_non_conformita_benessere_macellazione, id_non_conformita_benessere_trasporto, id_operatore_mercato from salvataggio_nc_note join lookup_provvedimenti_soa lnc on ( id_non_conformita=lnc.code) where idticket = ? and tipologia = ? order by tipologia";
		...
		if (non_conformita_formali.isEmpty()) {
			...
			puntiFormali = "" + non_conformita_formali.size();
		}

	}

	public void set_NonConformitaFormaliTrasporti(Connection db) throws SQLException {

		for (int i = 0; i < non_conformita_formali.size(); i++)
			non_conformita_formali.remove(non_conformita_formali.get(i));
		String sql = "";

		sql = "select idticket,id_non_conformita,lnc.description as descrizione ,note,tipologia, id_linea_nc, id_non_conformita_benessere_macellazione, id_non_conformita_benessere_trasporto, id_operatore_mercato from salvataggio_nc_note join lookup_tipo_nonconformita_trasporti lnc on ( id_non_conformita=lnc.code) where idticket = ? and tipologia = ? order by tipologia";
	...
	public void setNon_conformita_significative(Connection db, int tipoControllo, Timestamp dataInizioControllo)
			throws SQLException {

		for (int i = 0; i < non_conformita_significative.size(); i++)
			non_conformita_formali.remove(non_conformita_significative.get(i));
		String sql = "";
		...
		if (tipoControllo == 4 || (tipoControllo==3 && dataInizioControllo.after(java.sql.Timestamp.valueOf(org.aspcf.modules.controlliufficiali.base.ApplicationProperties.getProperty("TIMESTAMP_NUOVA_GESTIONE_OGGETTO_DEL_CONTROLLO_AUDIT"))))) {
			if (haveOggettoControllo == true) {
				...
				PreparedStatement pst_ver = db.prepareStatement(sql);
				pst_ver.setInt(1, id);
				pst_ver.setInt(2, ElementoNonConformita.NC_SIGNIFICATIVE);
				ResultSet rs_ver = pst_ver.executeQuery();
				if (!rs_ver.next())
					sql = "select idticket,id_non_conformita,lnc.description as descrizione ,note,tipologia, id_linea_nc, id_non_conformita_benessere_macellazione, id_non_conformita_benessere_trasporto, id_operatore_mercato from salvataggio_nc_note join lookup_provvedimenti lnc on ( id_non_conformita=lnc.code) where idticket = ? and tipologia = ? order by tipologia";
		...
		if (non_conformita_significative.isEmpty()) {
			puntiSignificativi = "0";
			ElementoNonConformita e = new ElementoNonConformita(id, 1, 2, "",-1);
			e.setId_nc(-1);
			e.setNote("INSERIRE QUI LA DESCRIZIONE PER LA SINGOLA NON CONFORMITA");
			non_conformita_significative.add(e);
		} else {

			DwrNonConformita dwrnc = new DwrNonConformita();
			PunteggiNonConformita p = dwrnc.get_punteggio_non_conformita(tipoControllo, dataInizioControllo + "");

			puntiSignificativi = "" + non_conformita_significative.size() * p.getPuntisignificativi();
		}
		if (ignoraPunteggi) // ACQUE DI RETE NON CONFORMITA': NON CALCOLARE
							// PUNTEGGI
		{
	...
	public void setNon_conformita_significativeSoa(Connection db) throws SQLException {
		for (int i = 0; i < non_conformita_significative.size(); i++)
			non_conformita_formali.remove(non_conformita_significative.get(i));
		String sql = "";

		if (tipoControllo == 4 || tipoControllo == 5) {
			sql = "select idticket,id_non_conformita,lnc.description as descrizione ,note,tipologia, id_linea_nc, id_non_conformita_benessere_macellazione, id_non_conformita_benessere_trasporto, id_operatore_mercato from salvataggio_nc_note join lookup_ispezione_soa lnc on ( id_non_conformita=lnc.code) where idticket = ? and tipologia = ? order by tipologia";
		} else {
			sql = "select idticket,id_non_conformita,lnc.description as descrizione ,note,tipologia, id_linea_nc, id_non_conformita_benessere_macellazione, id_non_conformita_benessere_trasporto, id_operatore_mercato from salvataggio_nc_note join lookup_provvedimenti_soa lnc on ( id_non_conformita=lnc.code) where idticket = ? and tipologia = ? order by tipologia";
	...
	public void setNon_conformita_gravi(Connection db, int tipoControllo, Timestamp dataInizioControllo)
			throws SQLException {
		for (int i = 0; i < non_conformita_gravi.size(); i++)
			non_conformita_formali.remove(non_conformita_gravi.get(i));
		String sql = "";
	...
	public void setNon_conformita_graviSoa(Connection db) throws SQLException {
		for (int i = 0; i < non_conformita_gravi.size(); i++)
			non_conformita_formali.remove(non_conformita_gravi.get(i));
		String sql = "";
		if (tipoControllo == 4 || tipoControllo == 5) {
		...
		{
			puntiGravi = "" + non_conformita_gravi.size() * 25;
		}
