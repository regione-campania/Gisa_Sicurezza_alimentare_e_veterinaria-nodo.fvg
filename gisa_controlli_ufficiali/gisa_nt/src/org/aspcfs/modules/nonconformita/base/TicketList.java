...
public class TicketList extends org.aspcfs.modules.troubletickets.base.TicketList
{
	...
	public void buildListControlli(Connection db, String org_id, String idC) throws SQLException {
		int orgId = -1;
		try {orgId = Integer.parseInt(org_id);} catch (Exception e) {}
		buildListControlli(db, orgId, idC);
	}
	...
	public void buildListControlli(Connection db, int org_id, String idC) throws SQLException {
	    ...
	    sqlSelect.append(
	        "select t.* ,o.tipologia as tipologia_operatore ,tic.provvedimenti_prescrittivi as tipo_controllo_uff,tic.assigned_date as data_controllo " +
	        
	        "FROM ticket t " +
	      "INNER JOIN ticket tic ON ((tic.tipologia = 3) and (tic.id_controllo_ufficiale = t.id_controllo_ufficiale)) " +
	      " left JOIN organization o on o.org_id = t.org_id " +
	       ...
	       // " left JOIN opu_operatori_denormalizzati_view oo ON (t.id_stabilimento = oo.id_stabilimento) " +
	        ...
	        sqlSelect.append( " where t.tipologia = 8 and  t.id_controllo_ufficiale = '"+idC+"' ");
	        if (this.orgId>0)
...
//	      if (ignoraPunteggi)
//	    	  thisTicket = new Ticket(rs,db, 99); //ACQUE DI RETE: IGNORA PUNTEGGI
//	      else
	    	  thisTicket= new Ticket(rs,db);
...
//	      else
	...
	public void buildList(Connection db) throws SQLException {
		    ...
		    sqlCount.append(
		        ...
		        " where t.ticketid > 0 AND t.tipologia =8 ");
		    createFilter(sqlFilter, db);
		    if (pagedListInfo != null) {
		    ...
		    while (rs.next()) {
		      Ticket thisTicket = new Ticket(rs , db);
		      this.add(thisTicket);
		    }
		    rs.close();
		    pst.close();
...
protected void createFilter(StringBuffer sqlFilter, Connection db) {
    ...
    if ((tipologia == 8)) {
	      sqlFilter.append(" AND  t.tipologia =8 ");
	} 
	if (enteredBy > -1) {
