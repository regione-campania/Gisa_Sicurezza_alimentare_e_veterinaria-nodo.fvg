...
public class CategoriaRischio  extends GenericBean
{
		...
		throws SQLException, IllegalArgumentException, IllegalAccessException, InvocationTargetException
	{
		...
		String sql = "INSERT INTO m_vpm_categorie_riscontrate( ";
	...
	public static ArrayList<CategoriaRischio> load( int id, ConfigTipo configTipo,Connection db )
	{
		...
		try
		{
			stat	= db.prepareStatement( "SELECT * FROM m_vpm_categorie_riscontrate WHERE "+configTipo.getNomeCampoRifAltreTabelle()+" = ? and trashed_date IS NULL" );
			stat.setInt( 1, id );
	...
	public static CategoriaRischio load(String id, Connection db )
	{

		CategoriaRischio	ret		= null;
		PreparedStatement	stat	= null;
		ResultSet			res		= null;
		...
		if( (id != null) && (id.trim().length() > 0) )
		{
			try
			{
				...
				stat	= db.prepareStatement( "SELECT * FROM m_vpm_categorie_riscontrate WHERE id = ? and trashed_date IS NULL" );
				stat.setInt( 1, iid );
				res		= stat.executeQuery();
				if( res.next() )
				{
	...
	private static CategoriaRischio loadResultSet( ResultSet res ) 
		throws SQLException, IllegalArgumentException, IllegalAccessException, InvocationTargetException
	{
	...
	public void update(Connection db)
		throws SQLException, IllegalArgumentException, IllegalAccessException, InvocationTargetException
	{
		String sql = "UPDATE m_vpm_categorie_riscontrate SET ";
	...
	public static void delete( int id, int user_id, Connection db )
	{
		...
		try
		{
			stat	= db.prepareStatement( 
					"UPDATE m_vpm_categorie_riscontrate SET modified = CURRENT_TIMESTAMP, trashed_date = CURRENT_TIMESTAMP, modified_by = ? " +
