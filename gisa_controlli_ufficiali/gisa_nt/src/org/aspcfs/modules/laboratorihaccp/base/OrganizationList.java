...
public class OrganizationList extends Vector implements SyncableList {
  ...
  protected int codiceMatrice = -1;
  protected int codiceDenominazione = -1;
  protected int codiceEnte = -1;
...
//  private String contactOtherState = null;
  ...
  public String getCognomeRappresentante() {
	...
	public void setCognomeRappresentante(String cognomeRappresentante) {
	 ...
	 public String getAccountCityLeg() {
			return accountCityLeg;
		}

		public void setAccountCityLeg(String accountCity) {
			this.accountCityLeg = accountCity;
		}
		 public String getAccountCityOp() {
				return accountCityOp;
			}

			public void setAccountCityOp(String accountCityOp) {
				this.accountCityOp = accountCityOp;
			}
...
public int getTipoAutorizzazzione() {
	return tipoAutorizzazzione;
}


public void setTipoAutorizzazzione(int tipoAutorizzazzione) {
	this.tipoAutorizzazzione = tipoAutorizzazzione;
}

public void setTipoAutorizzazzione(String tipoAutorizzazzione) {
	if (tipoAutorizzazzione != null &&  ! tipoAutorizzazzione.equals(""))
	this.tipoAutorizzazzione = Integer.parseInt(tipoAutorizzazzione);
}
...
/**
  ...
  public void setCodiceMatrice(int idTip) {
	    this.codiceMatrice = idTip;
	  }
  public void setCodiceMatrice(String idTip) {
	    this.codiceMatrice = Integer.parseInt(idTip);
	  }
  
  
  public void setCodiceDenominazione(int idTip) {
	    this.codiceDenominazione = idTip;
	  }
 
  public void setCodiceDenominazione(String idTip) {

   this.codiceDenominazione = Integer.parseInt(idTip);
  }
public void setCodiceEnte(int idTip) {
    this.codiceEnte = idTip;
  }
public void setCodiceEnte(String idTip) {
    this.codiceEnte = Integer.parseInt(idTip);
  }
...
public void setLongitudine(double tmp) {
  ...
  public void buildShortList(Connection db) throws SQLException {
    ...
    while (rs.next()) {
      ...
      thisOrg.setOrgId(thisOrg.getOrgId());
      thisOrg.setName(rs.getString("name"));
      thisOrg.setCognomeRappresentante(rs.getString("cognome_rappresentante"));
      thisOrg.setDataCambioStato(rs.getString("data_cambio_stato"));
  ...
  public void buildListProve(Connection db, String tipoRicerca) throws SQLException {
	    PreparedStatement pst = null;
	    ResultSet rs = queryListProve(db, pst);
	    while (rs.next()) {
	      Organization thisOrganization = this.getObject(rs);
  ...
  /*Ricerca laboratori senza prove*/
  public ResultSet queryList(Connection db, PreparedStatement pst) throws SQLException {
	    ...
	    StringBuffer sqlFilter = new StringBuffer();
	    StringBuffer sqlFilterProve = new StringBuffer();
	    StringBuffer sqlOrder = new StringBuffer();
	    StringBuffer sqlWhere = new StringBuffer();
	    sqlWhere.append( " WHERE o.org_id > 0 and o.tipologia = "+TIPOLOGIA_LAB +" " );
	    ...
	    if(codiceMatrice>-1 || codiceDenominazione>-1 || codiceEnte > -1){
	    	 sqlFilterProve.append(" left join laboratorihaccp_elenco_prove lep on(o.org_id = lep.org_id ) ");
	    }else{
	    	sqlFilterProve.append(" ");
	    }
...
//	            +" AND  " + DatabaseUtils.toLowerCase(db) + "(o.name) < ? ");
	    ...
	    sqlSelect.append(
	            " o.*, asl.description,oa5.*, oa1.addrline1 as ind_leg, oa1.address_type as addrtype_leg, oa1.city as com_leg, oa1.state as prov_leg, oa1.postalcode as cap_leg " +
	            " FROM organization o " +
	            " LEFT JOIN organization_address oa5 on ( o.org_id = oa5.org_id and oa5.address_type = 5) " +
	            " LEFT JOIN organization_address oa1 on ( o.org_id = oa1.org_id and oa1.address_type = 1) "+
	   ...
	   //createFilter(db, sqlFilter);
	   pst = db.prepareStatement(
	        sqlSelect.toString() + sqlFilterProve.toString() + sqlWhere.toString() + sqlFilter.toString() + sqlOrder.toString());
	   items = prepareFilter(pst);
...
/*Ricerca laboratori con elenco prove*/
  public ResultSet queryListProve(Connection db, PreparedStatement pst) throws SQLException {
	    ResultSet rs = null;
	    int items = -1;
	    ...
	    sqlCount.append(
	        ...
	        " left join organization_address oa5 on ( o.org_id = oa5.org_id and oa5.address_type = 5) "+
	        " left join organization_address oa1 on ( o.org_id = oa1.org_id and oa1.address_type = 1)" +
	        //" left join laboratorihaccp_elenco_prove lep on(o.org_id = lep.org_id ) "+
	        " WHERE o.org_id in ( SELECT org_id from laboratorihaccp_elenco_prove lep where true and lep.trashed_date is null "); 
	        //")and o.tipologia = "+TIPOLOGIA_LAB +" ");// and lep.trashed_date is null ");
	    ...
	    sqlSelect.append(
//	            " distinct o.org_id, o.name, o.data_cambio_stato, o.cognome_rappresentante, o.account_number, o.stato, o.entered, o.enteredby, o.modified, o.modifiedby, o.trashed_date, oa.addrline1 as ind_op, oa.address_type as addrtype_op,oa.city as com_op, oa.state as prov_op,oa.latitude as latitudine, oa.longitude as longitudine, oa1.addrline1 as ind_leg, oa1.address_type as addrtype_leg, oa1.city as com_leg, oa1.state as prov_leg, oa1.postalcode as cap_leg, lep.* " +
	    		" distinct o.partita_iva,o.org_id, o.site_id, o.name, o.data_cambio_stato, o.cognome_rappresentante, o.account_number, o.stato, o.entered, o.enteredby, o.modified, o.modifiedby, o.trashed_date, o.tipologia  " +
	    		" FROM organization o " +
	            " LEFT JOIN organization_address oa5 on ( o.org_id = oa5.org_id and oa5.address_type = 5) " +
	            " LEFT JOIN organization_address oa1 on ( o.org_id = oa1.org_id and oa1.address_type = 1) "+
	            //" LEFT JOIN lookup_site_id asl ON (o.site_id = asl.code) " +
	   ...
	   pst = db.prepareStatement(
	        sqlSelect.toString() + sqlFilter.toString() + " ) and o.tipologia = "+TIPOLOGIA_LAB +" "+ sqlOrder.toString());
	    items = prepareFilter(pst);
  ...
  public ResultSet queryRecord(Connection db) throws SQLException {
	    ...
	    sql.append("SELECT * FROM veterinari2");
  ...
  protected void createFilter(Connection db, StringBuffer sqlFilter) {
	    ...
	    if (stato != null && (!stato.equals("")) ) {
	        if (stato.indexOf("%") >= 0) {
	        	 sqlFilter.append(" AND  o.stato ILIKE ? ");
	        } else {
	          sqlFilter.append(" AND  o.stato = ? ");
	        }
	    }
	    ...
	    if ( cognomeRappresentante!= null && (!cognomeRappresentante.equals("")) ) {
	        if (cognomeRappresentante.indexOf("%") >= 0) {
	        	 sqlFilter.append(" AND  o.cognome_rappresentante ILIKE ? ");
	        } else {
	          sqlFilter.append(" AND  o.cognome_rappresentante = ? ");
	        }
	    }
	    
	  //City legale
	    if (accountCityLeg != null && (!accountCityLeg.equals("")) ) {
	        if (accountCityLeg.indexOf("%") >= 0) {
	        	sqlFilter.append(" AND (oa1.city ILIKE ? ");
		    	sqlFilter.append(" OR oa1.addrline1 ILIKE ? ) ");

	        } else {
	        	sqlFilter.append(" AND (oa1.city = ? ");
		    	sqlFilter.append(" OR oa1.addrline1 = ? ) ");

	        }
	    }
	    
	   //City operativa
	    if (accountCityOp != null && (!accountCityOp.equals("")) ) {
	        if (accountCityOp.indexOf("%") >= 0) {
	        	sqlFilter.append(" AND oa5.city ILIKE ? ");

	        } else {
	        	sqlFilter.append(" AND oa5.city = ? ");
	    ...
	    if (codiceMatrice > -1) {
	    	 // sqlFilter.append(" AND lep.codice_matrice= "+codiceMatrice+" ");
	    	sqlFilter.append(" AND lep.codice_matrice= ? "); 
	        }
	    if (codiceDenominazione > -1) {
	    	 // sqlFilter.append(" AND lep.codice_denominazione= "+codiceDenominazione+" ");
	    	sqlFilter.append(" AND lep.codice_denominazione= ? ");
	        }
	    if (codiceEnte > -1) {
	    	 // sqlFilter.append(" AND lep.codice_ente= "+codiceEnte+" ");
	    	  sqlFilter.append(" AND lep.codice_ente= ? ");
	        }
	    if (orgSiteId > -1 ) {
	   ...
	   /* if ((stato != null) &&(!stato.equals(""))) {
	    	  sqlFilter.append(" AND  o.stato ilike '%"+stato+"%' ");
	        }
	    if ((accountName != null) &&(!accountName.equals(""))) {
	    	  sqlFilter.append(" AND  o.name ilike '%"+accountName.replaceAll("'", "''")+"%' ");
	        }
	    if ((accountNumber != null) &&(!accountNumber.equals(""))) {
	    	  sqlFilter.append(" AND  o.account_number ilike '%"+accountNumber.replaceAll("'", "''")+"%' ");
	        }
	    if ((cognomeRappresentante != null) &&(!cognomeRappresentante.equals(""))) {
	    	  sqlFilter.append(" AND  o.cognome_rappresentante ilike '%"+cognomeRappresentante.replaceAll("'", "''")+"%' ");
	        }
	    if ((accountCityLeg != null) &&(!accountCityLeg.equals(""))) {
	    	  sqlFilter.append(" AND (oa1.city ilike '%"+accountCityLeg.replaceAll("'", "''")+"%' ");
	    	  sqlFilter.append(" OR oa1.addrline1 ilike '%"+accountCityLeg.replaceAll("'", "''")+"%') ");
	        }
	    if ((accountCityOp != null) &&(!accountCityOp.equals(""))) {
	    	  sqlFilter.append(" (AND oa.city ilike '%"+accountCityOp.replaceAll("'", "''")+"%' ");
	    	  sqlFilter.append(" OR oa.addrline1 ilike '%"+accountCityOp.replaceAll("'", "''")+"%') ");
	        }
	    if (codiceMatrice > -1) {
	    	  sqlFilter.append(" AND lep.codice_matrice= "+codiceMatrice+" ");
	        }
	  ...
	  /*  if ((citta != null)&&(!citta.equals(""))) {
	    	  sqlFilter.append(" AND  oa.city ilike '%"+citta+"%' ");
	        }
	*/        
	    if (stageId > -1) {
   ...
   * @since             1.2
   */
  protected void createFilterV(Connection db, StringBuffer sqlFilter) {
    if (sqlFilter == null) {
      sqlFilter = new StringBuffer();
    }
    /*if (idFarmacia > -1) {
    	sqlFilter.append(" AND o.id_farmacia=? ");
      }*/
      if ((accountName != null) ||(!accountName.equals(""))) {
    	  sqlFilter.append(" AND  f.ragione_sociale like '%" + accountName + "%' ");
        }
      if (siteId > -1) {
    	  sqlFilter.append(" AND  f.site_id="+siteId+" ");
        }
     /* if (indirizzo != null) {
    	  sqlFilter.append(" AND  o.indirizzo=? ");
        }*/
      if ((citta != null)||(!citta.equals(""))) {
    	  sqlFilter.append(" AND  f.citta like '%" + citta + "%' ");
        }/*
      if (provincia != null) {
    	  sqlFilter.append(" AND  o.provincia=? ");
        }
      if (postalCode != null) {
    	  sqlFilter.append(" AND  o.postalcode=? ");
        }
      if (latitudine > 0) {
    	  sqlFilter.append(" AND  o.latitude=? ");
        }
      if (longitudine > 0) {
    	  sqlFilter.append(" AND  o.longitude=? ");
        }
      if (note1 != null) {
    	  sqlFilter.append(" AND  o.note1=? ");
        }
      if (note2 != null) {
    	  sqlFilter.append(" AND  o.note2= ? ");
        }
    if (enteredBy > -1) {
      sqlFilter.append(" AND  o.enteredby = ? ");
    }
  ...
  protected void buildResources(Connection db) throws SQLException {
    ...
    while (i.hasNext()) {
    	...
    	thisOrganization.getProveList().buildList(db);
    }
  }
  ...
  protected int prepareFilter(PreparedStatement pst) throws SQLException {
	    ...
	    //Stato
	    if (stato != null && (!stato.equals("")) ) {
		      pst.setString(++i, stato.toLowerCase().replaceAll(" ", "%"));
		}
	    ...
	    //Cognome Rappresentante
	    if (cognomeRappresentante != null && !(cognomeRappresentante.equals("")) ) {
		      pst.setString(++i, cognomeRappresentante.toLowerCase().replaceAll(" ", "%"));
		}
	    
	    //AccountCityLeg
	    if (accountCityLeg != null && !(accountCityLeg.equals(""))) {
		      pst.setString(++i, "%"+accountCityLeg.toLowerCase().replaceAll(" ", "%")+"%");
		      pst.setString(++i, "%"+accountCityLeg.toLowerCase().replaceAll(" ", "%")+"%");
		}
	    
	    //AccountCityLeg
	    if (accountCityOp != null && !(accountCityOp.equals(""))) {
		      pst.setString(++i, accountCityOp.toLowerCase().replaceAll(" ", "%"));
		}
   ...
   * @since                 1.2
   */
  protected int prepareFilterV(PreparedStatement pst) throws SQLException {
    int i = 0;
  ...
  public void buildListStorico(Connection db, int orgId) throws SQLException {
	    PreparedStatement pst = null;
	    ResultSet rs = queryListStorico(db, pst,orgId);
	    while (rs.next()) {
	      Organization thisOrganization = this.getObject(rs);
	      this.add(thisOrganization);
  ...
  public ResultSet queryListStorico(Connection db, PreparedStatement pst,int orgId) throws SQLException {
	    ResultSet rs = null;
	    int items = -1;
	   ...
	   //Need to build a base SQL statement for counting records
	    sqlCount.append(
	        " SELECT COUNT(*) AS recordcount " +
	        " FROM laboratorihaccp_storico_elenco_lab p WHERE p.org_id = ? and lab_originario = ?" );	
		...
		sqlSelect.append(
	        " SELECT lep.* " +
	        " FROM laboratorihaccp_storico_elenco_lab lep WHERE lep.org_id = ? " +
	        " AND  lab_originario = ? ");
