 ...
 */
public class TicketLogList extends org.aspcfs.modules.troubletickets.base.TicketLogList {
  ...
  public TicketLogList(HttpServletRequest request, int userId) throws Exception {
    if (request.getParameter("newticketlogentry") != null) {
      ...
      if (ObjectValidator.validate(null,null, null, thisEntry)) {
        this.add(thisEntry);
      }
    }
  }
  ...
  public boolean setSystemMessages(TicketLog current, TicketLog prev) {
    ...
    if (prev == null) {
      ...
      if (1 == 1) {
        ...
        if (systemStatus != null) {
          tempLog.setEntryText(
              "[ " + systemStatus.getLabel("ticketloglist.richiestaOpened") + " ]");
        } else {
          tempLog.setEntryText("[ Richiesta Aperta ]");
        }
        this.add(tempLog);
      }
      
      //disabilito il messaggio "tichet non assegnato"
      if ( false ) {//(1 == 1) {
        tempLog = new TicketLog();
        tempLog.createSysMsg(current);
      ...
      if (!current.getClosed() && prev.getClosed()) {
        ...
        if (systemStatus != null) {
          tempLog.setEntryText(
              "[ " + systemStatus.getLabel("ticketloglist.richiestaReopened") + " ]");
        } else {
          tempLog.setEntryText("[ Richiesta Riaperta ]");
        }
        this.add(tempLog);
      }
      if (current.getClosed() && !prev.getClosed()) {
        ...
        if (systemStatus != null) {
          tempLog.setEntryText(
              "[ " + systemStatus.getLabel("ticketloglist.richiestaClosed") + " ]");
        } else {
          tempLog.setEntryText("[ Richiesta Chiusa ]");
        }
        this.add(tempLog);
      }
      return true;
