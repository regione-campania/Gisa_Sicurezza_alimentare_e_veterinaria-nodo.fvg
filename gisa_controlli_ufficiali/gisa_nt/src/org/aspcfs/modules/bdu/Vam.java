...
public class Vam extends Thread {

	private static Logger log = Logger .getLogger(Vam.class);
	...
	public Vam(Connection db, int userId, String id, String entita)
	{
		this.id = id;
		this.entita = entita;
		this.db=db;
	...
	if(ApplicationProperties.getProperty("cooperazione_SINAC").equals("true"))
		{
		String net = InetAddress.getByName(ApplicationProperties.getProperty("GISA2VAM_HOST_L")).getHostAddress();

			String[] toReturn = new String[2];
			try {
				  
				  URL url = new URL("http://"+net+":"+ApplicationProperties.getProperty("GISA2VAM_PORT")+"/"+ApplicationProperties.getProperty("GISA2VAM_APP")+"?json={\"token\":\""+idEntita+"@"+entita+"\",\"ambiente\":\""+ApplicationProperties.getProperty("ambiente")+"\",\"forza_allineamento\":\"false\"}"); 
		...
		String[] toReturn = new String[2];
		String [] esito = new Vam().aggiornamento(db, userId, idEntita, null, entita, null);
	...
	{
		if(ApplicationProperties.getProperty("GISA2VAM_UNIFICA_ON").equals("true"))
		{
			...
			try {
				  ...
				  URL url = new URL("http://"+net+":"+ApplicationProperties.getProperty("GISA2VAM_PORT")+"/"+ApplicationProperties.getProperty("GISA2VAM_UNIFICAZIONE")+"?json={\"id_eventi_da_aggiornare\":\""+new Gson().toJson(idEventoAggiornati)+"\",\"id_prop_da_eliminare\":\""+idDaRimuovere+"\",\"id_prop_in_sostituzione\":\""+idDaConservare+"\",\"ambiente\":\""+"sviluppo"+"\"}"); 
				  HttpURLConnection connection = (HttpURLConnection) url.openConnection();
				  System.out.println(url);
				  BufferedReader read = new BufferedReader(new InputStreamReader(connection.getInputStream()));
				  String line = read.readLine();
	...
	{
		String[] toReturn = new String[2];
		String [] esito = new Vam().aggiornamento(db, userId, idEntita, null, entita, "DELETE");
	...
	public String[] vediInSinaaf(Connection db, int userId,String idEntita, String codeEntita, String entita) throws Exception
	{
		...
		String toReturn[] = new String[2];
		String [] esito = new Vam().get(db, userId, idEntita, codeEntita, entita);
	...
	{
		 String[] toReturn = new String[2];
		 Vam t = new Vam(db,userId, idEntita, entita);
		 t.start();
		...
		if(ApplicationProperties.getProperty("cooperazione_SINAC").equals("true"))
	{
		...
		String[] toReturn = new String[2];
		try {
			String net = ApplicationProperties.getProperty("GISA2VAM_HOST_L");
			
			  URL url = new URL("http://"+net+":"+ApplicationProperties.getProperty("GISA2VAM_PORT")+"/"+ApplicationProperties.getProperty("GISA2VAM_APP")+"?caller=gisa&endpoint=vam&id_stab_gisa="+id+"&entita="+entita + "&ambiente="+ApplicationProperties.getProperty("ambiente")); 
			  HttpURLConnection connection = (HttpURLConnection) url.openConnection();
			  System.out.println(url);
			  BufferedReader read = new BufferedReader(new InputStreamReader(connection.getInputStream()));
			  String line = read.readLine();
	...
	String[] esito;
	try {
		esito = new Vam().aggiornamento(db, userId, id, null, entita, null);
