 ...
 */
public final class ChecklistSintesis extends CFSModule {
	...
	*/
	...
	public String executeCommandUpdate(ActionContext context) throws SQLException {
		...
		boolean aggiornato=false;
			...
			Integer idtampone = thisAudit.getId();
				...
				nonCList.setIdStabilimento(newTic.getIdStabilimento());
				nonCList.buildListControlli(db, passIdN, idC);
				context.getRequest().setAttribute("NonCList", nonCList);
			...
			ControlliUfficialiUtil.buildLookupTipoControlloUfficiale(db, context, systemStatus, user.getRoleId(), newTic.getSiteId(), Ticket.TIPO_SINTESIS, null);
	...
	if(!rs_operatore.next())
	{
		...
		if (newTic.isChecklistLibera()){
			...
			sql	=	"select * from get_livello_rischio_soa(?)" ; 
			pst = db.prepareStatement(sql);
			pst.setInt(1, livello);
		    rs = pst.executeQuery();
		    if (rs.next())
		    	livelloRischio = rs.getInt(1);
		    organ.updateLivelloRischioSintesis(db, livelloRischio, Integer.parseInt(altId));
		}
		else
			organ.updateCategoriaRischioSintesis(db, categoria,date, Integer.parseInt(altId), Integer.parseInt(idC));
...
public String executeCommandAdd(ActionContext context) {
	...
	try 
	{
		...
		int punteggioAccumulato = controlliList.buildListControlliUltimiAnniSintesis(db, AuditOrgId,Integer.parseInt(idC));
		context.getRequest().setAttribute("punteggioUltimiAnni", punteggioAccumulato);
