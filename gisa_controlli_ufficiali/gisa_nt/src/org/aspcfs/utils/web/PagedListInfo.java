    ...
    while (parameters.hasMoreElements()) {
      ...
      if (param.startsWith("search")) {
        ...
        if(param.startsWith("searchgroup"))
		{
			...
			if(param.startsWith("searchmultiplecode"))
				this.getSavedCriteria().put(param, request.getParameterValues(param));
			else
				
			if (request.getParameter(param)!= null && ! "".equals(request.getParameter(param).trim()))
      ...
      while (hashIterator.hasNext()) {
        ...
        if (tempKey.startsWith("searchgroup")) {
            ...
            //searchmultiplecode - This is for attributes which has code as well as description.
 					 ...
 					 //Creates an array of parameters to be passed.
 						if(this.getCriteriaValueArray(tempKey)!=null) {
 							 String[] words = this.getCriteriaValueArray(tempKey);
 							 ArrayList arraylist = new ArrayList();
 							 for(int indi=0 ; indi<words.length;indi++)
 							 {
 								arraylist.add(words[indi]);
        ...
        if (this.getCriteriaValue(tempKey) != null && !(this.getCriteriaValue(
          ...
          if (tempKey.startsWith("searchcode")) {
        		...
        		if(tempKey.startsWith("searchcodelistaStati"))
				{
					
					 String[] words = this.getCriteriaValue("searchcodelistaStatiIstruttoria").split(",");
			           
			            ObjectUtils.setParam(obj, "listaStatiIstruttoria", words);
            ...
            ObjectUtils.setParam(obj, tempKey.substring(10), this.getCriteriaValue(tempKey));
        	  ...
        	  ObjectUtils.setParam(
						obj, tempKey.substring(15),  this.getCriteriaValue(
								tempKey) );
           ...
           //searchmultiplecode - This is for attributes which has code as well as description.
					 ...
					 //Creates an array of parameters to be passed.
						...
						ObjectUtils.setParam(
								obj, tempKey.substring(11),  this.getCriteriaValue(
								tempKey));
					}
							else {	
							
								if (this.getCriteriaValue(
										tempKey)!=null  && ! "".equals(this.getCriteriaValue(
										tempKey)))
								{
						ObjectUtils.setParam(
