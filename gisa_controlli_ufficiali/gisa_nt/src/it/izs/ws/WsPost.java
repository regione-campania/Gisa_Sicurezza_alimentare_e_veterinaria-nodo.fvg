...
public class WsPost 
{
		  ...
		  public Boolean sinaaf_prop;
		  public Boolean bdu_prop;
		public String idBdu;
		public Boolean sincronizzatoBdu;
		  ...
		  public String getWithAuthentication(Connection db, int userId, String username, String password, String token)  
		  {
			...
			public void costruisciEnvelope(Connection db)
			{
				...
				System.out.println("CAMPI ENVELOPE: "+campiEnvelope);
			...
			public void setTIMEOUT_SECONDS(int tIMEOUT_SECONDS) {
			 ...
			 public WsPost(Connection db,String idEntita,String entita,String url)
			  ...
			  public String g2b_getLabelSincronizzazione()
			  {
				  if(sincronizzatoBdu)
					  return "INVIATO SINCRONIZZATO";
				  else if(idBdu==null || idBdu.equals(""))
					  return "NON SINCRONIZZATO";
				  else
					  return "INVIATO NON SINCRONIZZATO";
			  ...
			  public String g2b_getColoreSincronizzazione()
			  {
				  if(sincronizzatoBdu)
					  return "verde";
				  else if(idBdu==null || idBdu.equals(""))
			  ...
			  public String post(Connection db, Integer userId,MediaType mediaType,String token, String idTabella, String tabella,String method)  
			  {
				...
				public WsPost getSincronizzato(Connection db, String idTabella, String tabella,String nomeIdTabella)  
				{
					...
					try 
					{
						...
						System.out.println("ID TABELLA: "+idTabella+" TABELLA: "+tabella+" NOME ID: "+nomeIdTabella+" ID_: "+ ws.idSinaaf);
					} 
					catch (SQLException e) 
					{
						// TODO Auto-generated catch block
				...
				public WsPost g2b_getSincronizzato(Connection db, String idTabella, String tabella,String nomeIdTabella)  
				{
					PreparedStatement pst;
					WsPost wsBdu = new WsPost();
					try 
					{
						pst = db.prepareStatement("select * from g2b_is_sincronizzato(?,?,?)");
						...
						if(rs.next() )
						{
							wsBdu.sincronizzatoBdu = rs.getBoolean("sincronizzato");
							wsBdu.idBdu = rs.getString("id_bdu");
				...
				public Boolean getPropagabilita(Connection db, String idEntita, String entita)  
				{
					...
					try 
					{
						...
						if(false)//if(ApplicationProperties.getProperty("SINAAF_ATTIVO")==null || ApplicationProperties.getProperty("SINAAF_ATTIVO").equals("false"))
						{
							propagabile = false;
						}
						else
						{
							...
							if(rs.next() )
							{
								propagabile = rs.getBoolean("sinaaf_get_propagabilita");
							}
						}
					} 
					catch (SQLException e) 
					{
					 ...
					 sinaaf_prop = propagabile;
					return propagabile;
				}
				
				
				public Boolean g2b_getPropagabilita(Connection db, String idEntita, String entita)  
				{
					PreparedStatement pst;
					Boolean propagabile = false;
					try 
					{
						...
						{
							pst = db.prepareStatement("select * from g2b_get_propagabilita(?,?)");
							...
							if(rs.next() )
							{
								propagabile = rs.getBoolean("g2b_get_propagabilita");
							}
						}
					} 
					catch (SQLException e) 
